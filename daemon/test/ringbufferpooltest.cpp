...
void RingBufferPoolTest::testBindUnbindBuffer()
{
    ...
    rbm_->bindCallID(test_id1, RingBufferPool::DEFAULT_ID);

    // unbind test_id1 with RingBufferPool::DEFAULT_ID
    rbm_->unBindCallID(test_id1, RingBufferPool::DEFAULT_ID);

    rbm_->bindCallID(test_id1, RingBufferPool::DEFAULT_ID);
    rbm_->bindCallID(test_id1, RingBufferPool::DEFAULT_ID);

    rbm_->bindCallID(test_id2, RingBufferPool::DEFAULT_ID);
    rbm_->bindCallID(test_id2, RingBufferPool::DEFAULT_ID);
    ...
    rbm_->unBindCallID(RingBufferPool::DEFAULT_ID, test_id2);
    rbm_->unBindCallID(RingBufferPool::DEFAULT_ID, test_id2);

    rbm_->unBindCallID(RingBufferPool::DEFAULT_ID, test_id1);

    // test unbind all function
    rbm_->bindCallID(RingBufferPool::DEFAULT_ID, test_id1);
    rbm_->bindCallID(RingBufferPool::DEFAULT_ID, test_id2);
    rbm_->bindCallID(test_id1, test_id2);
...
void RingBufferPoolTest::testGetPutData()
{
    ...
    auto mainRingBuffer = rbm_->getRingBuffer(RingBufferPool::DEFAULT_ID);
    auto testRingBuffer = rbm_->createRingBuffer(test_id);

    rbm_->bindCallID(test_id, RingBufferPool::DEFAULT_ID);
    ...
    CPPUNIT_ASSERT(rbm_->getData(test_output, RingBufferPool::DEFAULT_ID) == 0);

    // put by test_id, get by RingBufferPool::DEFAULT_ID
    testRingBuffer->put(test_input2);
    CPPUNIT_ASSERT(rbm_->getData(test_output, RingBufferPool::DEFAULT_ID) == 1);
    CPPUNIT_ASSERT(test_sample2 == (*test_output.getChannel(0))[0]);

    rbm_->unBindCallID(test_id, RingBufferPool::DEFAULT_ID);
}
...
void RingBufferPoolTest::testDiscardFlush()
{
    ...
    rbm_->discard(1, RingBufferPool::DEFAULT_ID);
...
RingBufferPoolTest::RingBufferPoolTest() : CppUnit::TestCase("Audio Layer Tests"), rbm_(new RingBufferPool) {}
