...
{
    dispatcher_.reset(); // force reset first
}
...
int DBusClient::initLibrary(int sflphFlags)
{
    ...
    //evHandlers.emplace(DRing::EventHandlerKey::CONFIG, static_cast<void*>(&configEvHandlers));
    //evHandlers.emplace(DRing::EventHandlerKey::PRESENCE, static_cast<void*>(&presEvHandlers));

#ifdef RING_VIDEO
    //evHandlers.emplace(DRing::EventHandlerKey::VIDEO, static_cast<void*>(&videoEvHandlers));
#endif // RING_VIDEO
