...
public:
    ...
    UPnPIGD(IGDdatas d = IGDdatas(), UPNPUrls u = UPNPUrls())
        ...
        , urls(u)
    {
        if (u.controlURL != nullptr) {
            /* have to make a copy of the urls */
            urls.controlURL = strdup(u.controlURL);
            urls.ipcondescURL = strdup(u.ipcondescURL);
            urls.controlURL_CIF = strdup(u.controlURL_CIF);
            urls.controlURL_6FC = strdup(u.controlURL_6FC);
            urls.rootdescURL = strdup(u.rootdescURL);
            // RING_DBG("UPnP : new urls: %s", urls.controlURL);
        }
        // RING_WARN("UPnP : const orig urls: %u\n\t new urls pointer: %u", (void *)u.controlURL, (void *)urls.controlURL);
    };
...
public:
    ...
    /**
     * removes all entries with the local IP
     */
    void removeEntriesByLocalIP();
