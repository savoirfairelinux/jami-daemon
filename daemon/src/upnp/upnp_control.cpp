...
{
    if (upnpContext_->isInitialized()) {
        /* this function can wait up to the search timeout */
        RING_WARN("UPnP: waiting for valid igd");
        if (upnpContext_->hasValidIGD())
            RING_WARN("UPnP: got valid igd");
        else
            RING_WARN("UPnP: did NOT get a valid igd");
    }
}
...
{
#if HAVE_LIBUPNP
    /* TODO: check upnpContext for IGD */
    return false;
#endif
    return false;
}
