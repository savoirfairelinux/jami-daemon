...
constexpr static char const * INVALID_ARGS_STR = "402";
constexpr static int ARRAY_IDX_INVALID = 713;
constexpr static char const * ARRAY_IDX_INVALID_STR = "713";
constexpr static int CONFLICT_IN_MAPPING = 718;
constexpr static char const * CONFLICT_IN_MAPPING_STR = "718";
    ...
    if (initialized_){
...

        {
            ...
            for( auto const &it : validIGDs_) {
                removeMappingsByLocalIPAndDescription(it.second.get(),
 ...
 */
Mapping
UPnPContext::addMapping(IGD* igd, uint16_t port_external, uint16_t port_internal, PortType type,
                        int *upnp_error)
{
    *upnp_error = -1;
    ...
    if (iter != globalMappings->end()) {
        ...
        if (*mapping_ptr == mapping) {
            ...
            *upnp_error = UPNP_E_SUCCESS;
            ++(mapping_ptr->users);
            RING_DBG("UPnp : mapping alreayd exists, incrementing number of users: %d",
                     iter->second.users);
            return mapping;
        } else {
 ...
 */
Mapping
UPnPContext::addAnyMapping(uint16_t port_desired, uint16_t port_local, PortType type, bool use_same_port,
                           bool unique)
{
    if (not hasValidIGD()) {
    ...
    if ( iter != globalMappings->end() ) {
        ...
        if (mapping == *global_mapping ) {
            ...
            if (global_mapping->users > 1) {
                ...
                RING_DBG("UPnP: removing port mapping : %s", mapping.toString().c_str());
...
UPnPContext::isIGDConnected(const IGD* igd)
{
    ...
    int upnp_err = UpnpSendAction(ctrlptHandle_, igd->getControlURL().c_str(), igd->getServiceType().c_str(),
