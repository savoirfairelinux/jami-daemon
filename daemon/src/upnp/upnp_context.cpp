...
UPnPContext::parseIGD(IXML_Document *doc, const Upnp_Discovery* d_event)
{
    ...
    if (found_connected_IGD) {
        ...
        removeMappingsByLocalIPAndDescription(new_igd, "RING");
    }
}
...
IpAddr
UPnPContext::getExternalIP(std::shared_ptr<IGD> igd)
{
    std::unique_ptr<IXML_Document, decltype(ixmlDocument_free)&> action(nullptr, ixmlDocument_free);
...
void
UPnPContext::removeMappingsByLocalIPAndDescription(std::shared_ptr<IGD> igd, const std::string& description)
{
...
bool
UPnPContext::deletePortMapping(std::shared_ptr<IGD> igd, const std::string& port_external, const std::string& protocol)
{
