...
{
#if HAVE_UPNP
    /* make deep copy in case original gets freed */
    if (igdSource.datas)
        datas = new IGDdatas(*igdSource.datas);
    if (igdSource.urls) {
        urls = new UPNPUrls(*igdSource.urls);
...
UPnP::removeRedirection(uint16_t port_external, PortType type, bool existing)
{
    ...
    if(upnp_status!=UPNPCOMMAND_SUCCESS) {
        ...
        /* remove from list of redirections */
        if (existing) {
