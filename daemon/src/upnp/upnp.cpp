...
IGD Controller::defaultIGD_ = IGD();
std::map<uint16_t, Mapping> Controller::udpMappings_;
std::map<uint16_t, Mapping> Controller::tcpMappings_;

#if HAVE_UPNP
IGD::IGD(const IGD &igdSource)
{
...
IGD&
IGD::operator= (const IGD &igdSource)
{
    if (this == &igdSource)
        return *this;
...
Controller::chooseDefaultIGD(void)
{
    ...
    if (devlist) {
        ...
        defaultIGD_ = IGD(newIGDDatas, newIGDURLs);
...
Controller::addMapping(uint16_t port_external, uint16_t port_internal, PortType type, int *upnp_error)
{
    ...
    Mapping mapping = Mapping(defaultIGD_, local_ip, port_external, port_internal, type);
     ...
     */
    auto *mappings = type == PortType::UDP ? &udpMappings_ : &tcpMappings_;
    ...
    upnp_status = UPNP_AddPortMapping(mapping.igd.urls.controlURL,
                                      mapping.igd.datas.first.servicetype,
...
Controller::addAnyMapping(uint16_t port_desired, PortType type, bool unique, uint16_t *port_used)
{
    ...
    int upnp_error;
    bool result = addMapping(port_desired, port_desired, type, &upnp_error);
     ...
     */
    while( result == false and upnp_error == 718 ) {
        /* TODO: make sure we don't try sellecting the same random port twice if it fails ? */
        port_desired = chooseRandomPort(type);
        result = addMapping(port_desired, port_desired, type, &upnp_error);
    }
...
Controller::removeMappings(PortType type) {
    ...
    for (auto instanceIter = instanceMappings->begin(); instanceIter != instanceMappings->end(); ){
        ...
        if (users < 2) {
            ...
            int upnp_status = UPNP_DeletePortMapping(mapping.igd.urls.controlURL,
                                                     mapping.igd.datas.first.servicetype,
                                                     mapping.getExternalPort().c_str(),
                                                     mapping.getType().c_str(),
                                                     NULL);
