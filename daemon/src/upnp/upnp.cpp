...
UPnP::addRedirection(unsigned int port_external, unsigned int port_internal, const std::string& type)
{
    ...
    RING_DBG("UPnP : adding port mapping : %s, %u -> %u", local_ip.toString().c_str(), port_external, port_internal);
    ...
    if(r!=UPNPCOMMAND_SUCCESS) {
        RING_DBG("UPnP : AddPortMapping(%s, %s, %s, %s) failed with error: %d: %s",
...
UPnP::removeRedirection(unsigned int port_external, const std::string& type)
{
    ...
    if(r!=UPNPCOMMAND_SUCCESS) {
        RING_DBG("UPnP : DeletePortMapping(%s, %s) failed with error: %d: %s",
