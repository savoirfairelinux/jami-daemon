...
UPnP::chooseDefaultIGD(void)
{
    ...
    {
        RING_WARN("UPnP : looking for IGD UPnP devices on the network failed with error: %d : %s", upnperror, strupnperror(upnperror));
        return false;
    }
...
UPnP::addRedirection(uint16_t port_external, uint16_t port_internal, PortType type)
{
    ...
    if(upnp_status!=UPNPCOMMAND_SUCCESS) {
        ...
        if (redir.type == PortType::UDP)
            udp_mappings_[redir.external_port] = redir;
        else
            tcp_mappings_[redir.external_port] = redir;
...
UPnP::removeRedirection(uint16_t port_external, PortType type, bool existing)
{
    ...
    if (existing) {
     ...
     = UPnPRedirection(*defaultIGD_, local_ip, port_external, port_internal, type);
    RING_DBG("UPnP : removing entry with port = %u, type = %s", port_external, type.c_str());


    // sprintf(port_str, "%u", port_external);
    upnp_status = UPNP_DeletePortMapping(defaultIGD_->urls->controlURL, defaultIGD_->datas->first.servicetype, port_str, type.c_str(), NULL);
    if(upnp_status!=UPNPCOMMAND_SUCCESS) {
        RING_DBG("UPnP : DeletePortMapping(%s) failed with error: %d: %s",
                 port_str, type.c_str(), upnp_status, strupnperror(upnp_status));
        return false;
    } else {
...
UPnP::removeEntriesByDescription(const std::string& description)
{
    ...
    do {
        ...
        if(r==UPNPCOMMAND_SUCCESS) {
            ...
            RING_DBG("UPnP : GetGenericPortMappingEntry() failed with error code %d : %s", r, strupnperror(r));
        }
        i++;
    } while(r==0);
#endif
...
UPnP::getExternalIP()
{
    ...
    r = UPNP_GetExternalIPAddress(defaultIGD_->urls->controlURL,
                              ...
                              externalIPAddress);
    if(r != UPNPCOMMAND_SUCCESS) {
        RING_DBG("UPnP : GetExternalIPAddress failed with error code %d : %s", r, strupnperror(r));
        return IpAddr();
    } else {
