...
AudioRtpStream::AudioRtpStream(const std::string &id) :
    ...
    , plcDec_()
{
    ringbuffer_ = Manager::instance().getRingBufferManager().createRingBuffer(id_);
...
bool AudioRtpStream::tryToSwitchDecoder(int newPt)
{
    ...
    for (unsigned i=0, n=audioCodecs_.size(); i<n; i++) {
        ...
        AudioFormat f = Manager::instance().getRingBufferManager().getInternalAudioFormat();
...
{
    const auto &mainBuffer = Manager::instance().getRingBufferManager();
...
size_t AudioRtpStream::processDataEncode()
{
    ...
    AudioFormat mainBuffFormat = Manager::instance().getRingBufferManager().getInternalAudioFormat();
    ...
    if (Manager::instance().getRingBufferManager().availableForGet(id_) < samplesToGet)
        return 0;

    micData_.setFormat(mainBuffFormat);
    micData_.resize(samplesToGet);
    const size_t samples = Manager::instance().getRingBufferManager().getData(micData_, id_);

    if (samples != samplesToGet) {
        ERROR("Asked for %d samples from bindings on call '%s', got %d", id_.c_str(), samplesToGet, samples);
        return 0;
    }
