...
void bufferFillMasterKey(std::vector<uint8>& dest)
{
    ...
    uint8_t *key = (uint8_t *)malloc(dest.size());
    if (gnutls_rnd(GNUTLS_RND_NONCE, key, dest.size()) < 0)
        THROW_ERROR(AudioSrtpException, "Random key generation failed");

    std::vector<uint8_t> random_key(*key, dest.size());
    free(key);
