...
void PulseLayer::writeToSpeaker()
{
    ...
    size_t urgentSamples = urgentRingBuffer_.availableForGet(RingBufferManager::DEFAULT_ID);
    ...
    if (urgentBytes) {
        ...
        urgentRingBuffer_.get(linearbuff, RingBufferManager::DEFAULT_ID); // retrive only the first sample_spec->channels channels
        ...
        Manager::instance().getRingBufferManager().discard(urgentSamples, RingBufferManager::DEFAULT_ID);
        return;
    }
    ...
    size_t availSamples = Manager::instance().getRingBufferManager().availableForGet(RingBufferManager::DEFAULT_ID);
    ...
    AudioFormat mainBufferAudioFormat = Manager::instance().getRingBufferManager().getInternalAudioFormat();
    ...
    Manager::instance().getRingBufferManager().getData(linearbuff, RingBufferManager::DEFAULT_ID);
...
void PulseLayer::readFromMic()
{
    ...
    unsigned int mainBufferSampleRate = Manager::instance().getRingBufferManager().getInternalSamplingRate();
