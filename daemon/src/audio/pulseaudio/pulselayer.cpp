...
void PulseLayer::updateServerInfos()
{
    gettingServerInfos_ = true;
...
void PulseLayer::createStreams(pa_context* c)
{
    ...
    readyCv_.wait(lk, [this]{
        return !(enumeratingSinks_ or enumeratingSources_ or gettingServerInfos_);
    });
...
void PulseLayer::server_info_callback(pa_context*, const pa_server_info *i, void *userdata)
{
    ...
    context->defaultAudioFormat_ = {i->sample_spec.rate, i->sample_spec.channels};
    {
        ...
        context->gettingServerInfos_ = false;
    }
...
{
    auto dev = preference_.getPulseDeviceRecord();
    return dev.empty() ? 0 : getAudioDeviceIndexByName(dev, DeviceType::CAPTURE);
}

int PulseLayer::getIndexPlayback() const
{
    auto dev = preference_.getPulseDevicePlayback();
    return dev.empty() ? 0 : getAudioDeviceIndexByName(dev, DeviceType::PLAYBACK);
}

int PulseLayer::getIndexRingtone() const
{
    auto dev = preference_.getPulseDeviceRingtone();
    return dev.empty() ? 0 : getAudioDeviceIndexByName(dev, DeviceType::RINGTONE);
}
