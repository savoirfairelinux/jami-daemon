 ...
 */
bool
_utf8_validate(const char *str, ssize_t max_len, const char **end)
{
    const char *p;
...
utf8_make_valid(const std::string & name)
{
    ...
    while (remaining_bytes != 0) {
        if (_utf8_validate(remainder, remaining_bytes, &invalid))
            break;
    ...
    assert(_utf8_validate(answer.c_str(), -1, NULL));
