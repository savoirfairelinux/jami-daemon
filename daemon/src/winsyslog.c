...
void syslog(int priority, const char *message, ...)
{
	...
	/* default event source */
		if (!loghdl)
		openlog("c-client", WINLOG_PID, WINLOG_MAIL);
...
void vsyslog(int facility_priority, const char* format, va_list arglist)
{
        ...
        switch (facility_priority) {
                case WINLOG_ALERT:
                        alert_type = EVENTLOG_ERROR_TYPE;
                        break;
                case WINLOG_INFO:
                        alert_type = EVENTLOG_INFORMATION_TYPE;
                        break;
                default:
                        alert_type = EVENTLOG_WARNING_TYPE;
        }

        vsprintf(tmp, format, arglist);
        ReportEvent(loghdl, alert_type, (unsigned short)facility_priority,
                2000, NULL, 2, 0, tmp, NULL);
}
