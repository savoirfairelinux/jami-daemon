...
#undef TRY
    ...
    if (preferences.getUseUPnP()) {
        RING_DBG("Remove any old UPnP entries for ring so there are no potential conflicts");
        upnp::UPnP().removeEntriesByDescription("ring");
    }
...
ManagerImpl::finish()
{
    ...
    if (preferences.getUseUPnP()) {
        RING_DBG("Remove any remaning ports mapped for Ring");
        upnp::UPnP().removeEntriesByDescription("ring");
    }
}
