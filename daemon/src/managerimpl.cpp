...
ManagerImpl::finish()
{
    ...
    try {
        ...
        if (accountFactory_.empty())
            SIPVoIPLink::destroy();
        else
            ERROR("Account remaining, could not destroy VoIP stacks now");
...
#if HAVE_IAX
    for (auto &item : accountFactory_.getAllAccounts("IAX"))
...
{
    for (auto& item : getAllAccounts()) {
        auto account = item.second;
        if (account->isEnabled())
            account->unregisterVoIPLink();
        accountFactory_.removeAccount(account->getAccountID());
    }
}
