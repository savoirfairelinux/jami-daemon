...
SipsIceTransport::getInfo(pj_ssl_sock_info *info)
{
    ...
    if (info->established) {
        ...
        /* Verification status */
        info->verify_status = verify_status_;
    }
    ...
    if (state_ == TlsConnectionState::ESTABLISHED) {
        {
            ...
            cv_.wait(l, [&](){
                return state_ != TlsConnectionState::ESTABLISHED || canRead_ || canWrite_;
            });
        }
        if (state_ != TlsConnectionState::ESTABLISHED)
            return;
        while (canRead_) {
            ...
            RING_WARN("gnutls_record_recv : %d (tot %d)", decrypted_size, rdata_.pkt_info.len);
