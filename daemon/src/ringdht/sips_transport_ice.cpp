...
SipsIceTransport::loop()
{
    ...
    if (state_ == TlsConnectionState::COOKIE) {
        ...
        ret = gnutls_dtls_cookie_verify(&cookie_key_, &trData_.base.key.rem_addr, trData_.base.addr_len, tlsInputBuff_.data(), tlsInputBuff_.size(), &prestate_);
        if (ret < 0) {
                      ...
                      pjsip_transport_callback callback)
{
    ...
    return PJ_EPENDING;// flushOutputBuff();
}
