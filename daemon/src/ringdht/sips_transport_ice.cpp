...
SipsIceTransport::loop()
{
    ...
    if (state_ == TlsConnectionState::ESTABLISHED) {
        ...
        {
            Lock2 l(inputBuffMtx_, outputBuffMtx_);
            cv_.wait(l, [&](){
                can_read = not tlsInputBuff_.empty();
                bool can_write = not outputBuff_.empty();
                return state_ != TlsConnectionState::ESTABLISHED || can_read || can_write;
            });
        }
        if (state_ != TlsConnectionState::ESTABLISHED)
            return;
