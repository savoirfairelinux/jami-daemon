...
{
    RING_DBG("SipsIceTransport::tryHandshake");
    pj_status_t status;
    ...
    , destroy_cb_(destroy_cb)
{
    ...
    bool is_server_ = not ice->isInitiator();
    ...
    gnutls_priority_init(&priority_cache, "PERFORMANCE:-VERS-TLS-ALL:+VERS-DTLS1.0:%SERVER_PRECEDENCE", nullptr);
...
{
    RING_DBG("SipsIceTransport::startTls");
    ...
    if (not param_.cert.empty()) {
        /* Load CA if one is specified. */
        if (not param_.ca.empty()) {
            ret = gnutls_certificate_set_x509_trust_file(xcred_, param_.ca.c_str(), GNUTLS_X509_FMT_PEM);
            if (ret < 0)
                ret = gnutls_certificate_set_x509_trust_file(xcred_, param_.ca.c_str(), GNUTLS_X509_FMT_DER);
            if (ret < 0)
                throw std::runtime_error("Can't load CA.");
            RING_WARN("Loaded %s", param_.ca.c_str());
        }

        /* Load certificate, key and pass if one is specified */
        if (not param_.cert.empty() && not param_.privkey.empty()) {
            const char *prikey_pass = param_.privkey_pass.empty() ? nullptr : param_.privkey_pass.c_str();
            ret = gnutls_certificate_set_x509_key_file2(xcred_,
                                                        param_.cert.c_str(),
                                                        param_.privkey.c_str(),
                                                        GNUTLS_X509_FMT_PEM,
                                                        prikey_pass,
                                                        0);
            if (ret != GNUTLS_E_SUCCESS)
            ...
            RING_WARN("Loaded %s", param_.cert.c_str());
        }
    }

    /* Require client certificate if asked */
    if (is_server_ /*&& ssock->param.require_client_cert*/)
    ...
    // start handshake
    if (is_server_)
        state_ = TlsConnectionState::COOKIE; // : TlsConnectionState::HANDSHAKING;
    else {
                      ...
                      pjsip_transport_callback callback)
{
    ...
    while (not outputBuff_.empty()) {
        ...
        if (trySend(f.tdata_op_key) == PJ_SUCCESS)
...
SipsIceTransport::trySend(pjsip_tx_data_op_key *pck)
{
    ...
    auto size = tdata->buf.cur - tdata->buf.start;
    ...
    while (total_written < size) {
        ...
        RING_DBG("gnutls_record_send %lu", size);
        int nwritten = gnutls_record_send(session_, tdata->buf.start + total_written, size - total_written);
        ...
        if (nwritten > 0) {
             ...
             */
            return PJ_ENOMEM;//tls_status_from_err(nwritten);
        }
    }

    if (pck->callback)
        (*pck->callback)(&base, pck->token, size);
...
SipsIceTransport::onRecv()
{
    ...
    RING_DBG("SipsIceTransport::onRecv %lu", next_size);
    ...
    if (state_ == TlsConnectionState::COOKIE) {
        ...
        RING_DBG("gnutls_dtls_cookie_verify %d", ret);
        if (ret < 0) {
            ...
            gnutls_dtls_cookie_send(&cookie_key_, &base.key.rem_addr, base.addr_len, &prestate_, this, [](gnutls_transport_ptr_t t, const void* d , size_t s) -> ssize_t {
                RING_WARN("gnutls_dtls_cookie_send callback %lu", s);
                auto this_ = reinterpret_cast<SipsIceTransport*>(t);
                return this_->tlsSend(d, s);
            });
        } else {
