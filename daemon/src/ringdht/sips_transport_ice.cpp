        ...
        std::bind(&SipsIceTransport::clean, this))
{
    ...
    base.flag = PJSIP_TRANSPORT_DATAGRAM | PJSIP_TRANSPORT_SECURE;
...
SipsIceTransport::onHandshakeComplete(pj_status_t status)
{
    ...
    if (is_server_) {
        ...
        if (state_cb)
            (*state_cb)(getTransportBase(), status ? PJSIP_TP_STATE_DISCONNECTED : PJSIP_TP_STATE_CONNECTED, &state_info);
    }
...
SipsIceTransport::loop()
{
    ...
    if (state_ == TlsConnectionState::COOKIE && tlsInputBuff_.empty() && ice_->getNextPacketSize(comp_id_) == 0)
        ice_->waitForData(comp_id_, 2500);

    while (auto next_size = ice_->getNextPacketSize(comp_id_)) {
        std::vector<uint8_t> incoming_data;
        incoming_data.resize(next_size);
        auto len = ice_->recv(comp_id_, incoming_data.data(), next_size);
        incoming_data.resize(len);
        tlsInputBuff_.emplace_back(std::move(incoming_data));
    }
    ...
    if (state_ == TlsConnectionState::COOKIE) {
        ...
        ret = gnutls_dtls_cookie_verify(&cookie_key_, &trData_.base.key.rem_addr, trData_.base.addr_len, (char*)pck.data(), pck.size(), &prestate_);
        if (ret < 0) {
...
SipsIceTransport::waitForTlsData(unsigned ms)
{
    ...
    if (not tlsInputBuff_.empty())
        return tlsInputBuff_.front().size();
                      ...
                      pjsip_transport_callback callback)
{
    ...
    pj_lock_acquire(trData_.base.lock);
    ...
    pj_lock_release(trData_.base.lock);
    return PJ_EPENDING;
}
