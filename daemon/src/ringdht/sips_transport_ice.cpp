...
SipsIceTransport::verifyCertificate()
{
    ...
    /*if (status & GNUTLS_CERT_SIGNER_NOT_FOUND) {
        ...
        if (status & GNUTLS_CERT_SIGNER_NOT_FOUND)
            verify_status |= PJ_SSL_CERT_EISSUER_NOT_FOUND;
        else if (status & GNUTLS_CERT_EXPIRED || status & GNUTLS_CERT_NOT_ACTIVATED)
            verify_status |= PJ_SSL_CERT_EVALIDITY_PERIOD;
        else if (status & GNUTLS_CERT_SIGNER_NOT_CA || status & GNUTLS_CERT_INSECURE_ALGORITHM)
            verify_status |= PJ_SSL_CERT_EUNTRUSTED;
        else if (status & GNUTLS_CERT_UNEXPECTED_OWNER || status & GNUTLS_CERT_MISMATCH)
...
*/
        ...
        if (param_.cert_check) {
            pj_status_t check_ret = param_.cert_check(status, cert_list, cert_list_size);
            if (check_ret != PJ_SUCCESS) {
                /*verify_status |= check_ret;
                goto out;*/
                return GNUTLS_E_CERTIFICATE_ERROR;
            }
        }
...
SipsIceTransport::SipsIceTransport(pjsip_endpoint* endpt, const TlsParams& param,
        ...
        std::bind(&SipsIceTransport::clean, this))
{
    ...
    base.key.type = PJSIP_TRANSPORT_TLS;//t_type;
    ...
    rdata.tp_info.pool = rxPool_.get();
    ...
    for (unsigned i = 0; ; i++) {
        ...
        if (!suite)
            break;
        cyphers_.emplace_back(SSLCipher{static_cast<uint16_t>((id[0] << 8) | id[1]), suite});
    }
    ...
    ice_->setOnRecv(comp_id_, [this](uint8_t* buf, size_t len) {
        ...
        cv_.notify_all();
        RING_DBG("Ice: got data at %lu", clock::now().time_since_epoch().count());
        return len;
    });
...
{
    RING_DBG("SipsIceTransport::startTlsSession as %s", (is_server_ ? "server" : "client"));
    int ret;
    ...
    if (ret != GNUTLS_E_SUCCESS) {
        RING_ERR("Can't set credentials");
        reset();
        return tls_status_from_err(ret);
    }
...
SipsIceTransport::certGetInfo(pj_pool_t *pool, pj_ssl_cert_info *ci, gnutls_x509_crt_t cert)
{
    ...
    pj_bool_t update_needed = pj_strcmp2(&ci->issuer.info, buf) || std::memcmp(ci->serial_no, serial_no.data(), serialsize);
    if (!update_needed)
        return;
...
SipsIceTransport::onHandshakeComplete(pj_status_t status)
{
    ...
    pjsip_tp_state_callback state_cb = pjsip_tpmgr_get_state_cb(trData_.base.tpmgr);
...
/*#if (defined(PJ_WIN32) && PJ_WIN32 != 0) || (defined(PJ_WIN64) && PJ_WIN64 != 0)
        ...
        if (state_cb)
            (*state_cb)(getTransportBase(), (status != PJ_SUCCESS) ? PJSIP_TP_STATE_DISCONNECTED : PJSIP_TP_STATE_CONNECTED, &state_info);
    }
...
SipsIceTransport::generateDhParams()
{
    ...
    unsigned int bits = gnutls_sec_param_to_pk_bits(GNUTLS_PK_DH, GNUTLS_SEC_PARAM_NORMAL /*GNUTLS_SEC_PARAM_HIGH*/);
    ...
    if (state_ == TlsConnectionState::COOKIE) {
        {
            ...
            if (tlsInputBuff_.empty()) {
                cv_.wait(l, [&](){
                    return state_ != TlsConnectionState::COOKIE || not tlsInputBuff_.empty();
                });
            }
            RING_DBG("Cookie: got data at %lu", clock::now().time_since_epoch().count());
            if (state_ != TlsConnectionState::COOKIE)
                return;
            const auto& pck = tlsInputBuff_.front();
            memset(&prestate_, 0, sizeof(prestate_));
            int ret = gnutls_dtls_cookie_verify(&cookie_key_, &trData_.base.key.rem_addr, trData_.base.addr_len, (char*)pck.data(), pck.size(), &prestate_);
            if (ret < 0) {
                RING_DBG("gnutls_dtls_cookie_send");
                gnutls_dtls_cookie_send(&cookie_key_, &trData_.base.key.rem_addr, trData_.base.addr_len, &prestate_, this, [](gnutls_transport_ptr_t t, const void* d , size_t s) -> ssize_t {
                    auto this_ = reinterpret_cast<SipsIceTransport*>(t);
                    return this_->tlsSend(d, s);
                });
    ...
    if (state_ == TlsConnectionState::ESTABLISHED) {
        {
            ...
            cv_.wait(l, [&](){
                return state_ != TlsConnectionState::ESTABLISHED || canRead || canWrite;
            });
        }
        if (state_ != TlsConnectionState::ESTABLISHED)
            return;
        while (canRead) {
            ...
            RING_WARN("gnutls_record_recv : %d (tot %d)", decrypted_size, rdata.pkt_info.len);

            if (decrypted_size > 0/* || transport error */) {
                rdata.pkt_info.zero = 0;
                pj_gettimeofday(&rdata.pkt_info.timestamp);
                auto eaten = pjsip_tpmgr_receive_packet(trData_.base.tpmgr, &rdata);
                auto rem = rdata.pkt_info.len - eaten;
                RING_WARN("pjsip_tpmgr_receive_packet : %d (remaining %d)", eaten, rem);
...
SipsIceTransport::clean()
{
    ...
    while (not outputBuff_.empty()) {
        ...
        if (f.tdata_op_key->callback) {
            ...
            f.tdata_op_key->callback(getTransportBase(), f.tdata_op_key->token, -PJ_RETURN_OS_ERROR(OSERR_ENOTCONN));
        }
    ...
    if (trData_.base.is_shutdown || trData_.base.is_destroying) {
...
SipsIceTransport::tlsRecv(void* d , size_t s)
{
    ...
    RING_DBG("SipsIceTransport::tlsRecv %lu at %lu", s, clock::now().time_since_epoch().count());
                      ...
                      pjsip_transport_callback callback)
{
    ...
    tdata->op_key.callback = callback;
    {
        ...
        if (tdata->msg && tdata->msg->type == PJSIP_REQUEST_MSG) {
            ...
            dtd.timeout = clock::now() + std::chrono::milliseconds(pjsip_cfg()->tsx.td);
...
SipsIceTransport::flushOutputBuff()
{
    ...
    while (true) {
        ...
        if (f.tdata_op_key->callback) {
            RING_DBG("Calling transaction callback");
            f.tdata_op_key->callback(getTransportBase(), f.tdata_op_key->token, status);
        }
        if (status < 0)
            break;
    }
