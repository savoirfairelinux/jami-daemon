...
struct TlsParams  {
    ...
    std::function<pj_status_t(unsigned status, const gnutls_datum_t *cert_list, unsigned cert_list_size)> cert_check;
};
...
struct SipsIceTransport
{
    ...
    static_assert(std::is_standard_layout<TransportData>::value, "TranportData requires standard-layout");
    ...
    SipsIceTransport(pjsip_endpoint* endpt, const TlsParams& param,
                    const std::shared_ptr<ring::IceTransport>& ice, int comp_id);
    ~SipsIceTransport();
