...
SipIceTransport::SipIceTransport(pjsip_endpoint *endpt, pj_pool_t& /* pool */, long t_type, const std::shared_ptr<sfl::IceTransport>& ice, int comp_id)
 : base(), rdata(), ice_(ice), comp_id_(comp_id)
{
                ...
                pjsip_transport_callback callback)
{
    ...
    if (size > 0) {
        if (size < buf_sz) {
            SFL_WARN("Partial write");
            std::move(tdata->buf.start + size, tdata->buf.start + buf_sz, tdata->buf.start);
            tdata->buf.cur -= size;
        }
...
SipIceTransport::onRecv(uint8_t* buf, size_t len)
{
    ...
    SFL_WARN("SipIceTransport::onRecv %p %lu %lu", buf, len, max_size);
