 ...
 : ice_(ice), comp_id(comp_id)
{
    ...
    base.send_msg = &ice_send_msg;
    base.do_shutdown = &ice_shutdown;
    base.destroy = &ice_destroy_transport;
    ...
    ice->setOnRecv(comp_id, std::bind(&SipIceTransport::onRecv, this, _1, _2));
}
                ...
                pjsip_transport_callback callback)
{
    ...
    auto size = ice_->send(comp_id, (uint8_t*)tdata->buf.start, tdata->buf.cur - tdata->buf.start);
    if (size > 0) {
...
SipIceTransport::onRecv(uint8_t* buf, size_t len)
{
    ...
    memcpy((uint8_t*)rdata.pkt_info.packet + rdata.pkt_info.len, buf, max_size);
...
{
    SFL_WARN("SIP transpoert ICE: shutdown");
}

pj_status_t
SipIceTransport::destroy()
{
    SFL_WARN("SIP transpoert ICE: destroy");
    is_registered = false;
