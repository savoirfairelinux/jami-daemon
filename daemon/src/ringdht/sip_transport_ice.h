...
{
    SipIceTransport(pjsip_endpoint *endpt, pj_pool_t& pool, long t_type, const std::shared_ptr<sfl::IceTransport>& ice, int comp_id);
    ~SipIceTransport();
...
private:
    ...
    static pj_status_t ice_send_msg(pjsip_transport *transport,
                    pjsip_tx_data *tdata,
    ...
    static pj_status_t ice_shutdown(pjsip_transport *transport) {
        auto this_ = reinterpret_cast<SipIceTransport*>(transport);
        return this_->shutdown();
    }

    /* Called by transport manager to destroy transport */
    static pj_status_t ice_destroy_transport(pjsip_transport *transport) {
        auto this_ = reinterpret_cast<SipIceTransport*>(transport);
