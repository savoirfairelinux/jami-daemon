    ...
    dht_.listen(
        ...
        [shared, call, callkey, ice, toUri, replyvid] (const std::vector<std::shared_ptr<dht::Value>>& vals) {
            ...
            for (const auto& v : vals) {
                ...
                //this_.dht_.cancelListen(callkey, listenkey);
                ...
                call->setConnectionState(Call::PROGRESSING);
                //this_.createOutgoingCall(call, toUri, this_.getToUri(toUri+"@"+ice->getRemote(0).toString(true)));
...
void RingAccount::mapPortUPnP()
{
    if (useUPnP_) {
        ...
        SFL_DBG("RING account mapping SIP port %u to %u", publishedPort_, localPort_);
        upnp_add_redir(local_ip.toString().c_str(), publishedPort_, localPort_);

        SFL_DBG("RING account mapping audio port range %u to %u", audioPortRange_.first, audioPortRange_.second);
        for( unsigned int port_to_map = audioPortRange_.first;
            port_to_map <= audioPortRange_.second;
            port_to_map++)
        {
            upnp_add_redir(local_ip.toString().c_str(), port_to_map, port_to_map);
        }

        SFL_DBG("RING account mapping video port range %u to %u", videoPortRange_.first, videoPortRange_.second);
        for( unsigned int port_to_map = videoPortRange_.first;
            port_to_map <= videoPortRange_.second;
            port_to_map++)
        {
            upnp_add_redir(local_ip.toString().c_str(), port_to_map, port_to_map);
        }
            ...
            [shared,listenKey] (const std::vector<std::shared_ptr<dht::Value>>& vals) {
                ...
                for (const auto& v : vals) {
                    try {
                        ...
                        if (!res.second) {
                            SFL_WARN("Ignoring already treated incomming call");
                            continue;
                        }
                        if (v->owner.getId() == this_.dht_.getId()) {
                            SFL_WARN("Ignoring DHT value from self");
                            continue;
                        }
                        ...
                        this_.dht_.putSigned(
                            ...
                            [call,shared,listenKey,reply_vid](bool ok) {
                                ...
                                if (!ok || call->waitForIceNegociation(60) <= 0) {
                                    ...
                                    call->setConnectionState(Call::PROGRESSING);
                                    auto t = this_.link_->sipTransport->getIceTransport(call->getIceTransport());
...
RingAccount::loadTreatedCalls()
{
    std::string treatedcallPath = cachePath_+DIR_SEPARATOR_STR "treatedCalls";
    {
        ...
        if (!file.is_open()) {
            SFL_ERR("Could not load treated calls from %s", treatedcallPath.c_str());
            return;
        }
        SFL_WARN("Reading treatedCalls");
        std::string line;
        while (std::getline(file, line)) {
            std::istringstream iss(line);
            dht::Value::Id vid;
            if (!(iss >> std::hex >> vid)) { break; }
            SFL_WARN("Read treated call %llx", vid);
