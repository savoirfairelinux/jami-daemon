...
RingAccount::createOutgoingCall(const std::shared_ptr<SIPCall>& call, const std::string& to_id, IpAddr target)
{
    ...
    auto codec = getSystemCodecContainer()->searchCodecByName("PCMA", ring::MEDIA_AUDIO);
    auto ac = std::dynamic_pointer_cast<SystemAudioCodecInfo>(codec);
    if (!ac)
    ...
    const bool created = sdp.createOffer(getActiveAudioCodecs(), getActiveVideoCodecs(), getSrtpKeyExchange());
