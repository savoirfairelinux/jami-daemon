...
RingAccount::handleEvents()
{
    ...
    while (c != pendingCalls_.end()) {
        ...
        } else if (now - std::get<0>(*c) > std::chrono::seconds(ICE_NEGOTIATION_TIMEOUT)) {
...
void RingAccount::mapPortUPnP()
{
    if (useUPnP_) {
         ...
         */
        RING_DBG("Mapping RING port %u", dhtPort_);
        uint16_t port_used;
        upnp_.addAnyMapping(dhtPort_, upnp::PortType::UDP, false, &port_used);
    }
}
