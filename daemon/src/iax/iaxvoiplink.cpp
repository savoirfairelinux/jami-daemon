...
IAXVoIPLink::sendAudioFromMic()
{
    for (const auto currentCall : Manager::instance().callFactory.getAllCalls<IAXCall>()) {
        ...
        Manager::instance().getRingBufferManager().setInternalSamplingRate(audioCodec->getClockRate());
        ...
        if (Manager::instance().getRingBufferManager().availableForGet(currentCall->getCallId()) < samples)
            continue;

        // Get bytes from micRingBuffer to data_from_mic
        rawBuffer_.resize(samples);
        samples = Manager::instance().getRingBufferManager().getData(rawBuffer_, currentCall->getCallId());
