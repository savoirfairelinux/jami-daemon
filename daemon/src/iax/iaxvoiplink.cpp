...
IAXVoIPLink::handleEvents()
{
    ...
    while (event != NULL) {
        ...
        if (auto raw_call_ptr = static_cast<Call*>(iax_get_private(event->session))) {
...
void IAXVoIPLink::iaxHandlePrecallEvent(iax_event* event)
{
    ...
    switch (event->etype) {
        case IAX_EVENT_CONNECT:
            ...
            call = std::static_pointer_cast<IAXCall>(account_.newIncomingCall(id));

            call->session = event->session;
            iax_set_private(event->session, call.get());
