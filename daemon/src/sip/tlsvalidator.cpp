...
certificatePath_(certificate), privateKeyPath_(privatekey), caCert_(nullptr), caChecked_(false)
{
    ...
    try {
        //certificateContent_ = fileutils::loadFile(certificatePath_);
...
TlsValidator::CheckResult TlsValidator::hasPrivateKey()
{
    ...
    if (certificateContent_.empty())
        return CheckResult(CheckValues::FAILED, "");
    gnutls_datum_t dt {(uint8_t*)certificateContent_.data(), static_cast<unsigned>(certificateContent_.size())};
