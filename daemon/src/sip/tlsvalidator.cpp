...
std::map<std::string,std::string> TlsValidator::getSerializedChecks()
{
    ...
    else for (const CertificateCheck check : sfl::Matrix0D<CertificateCheck>())
...
std::map<std::string,std::string> TlsValidator::getSerializedDetails()
{
    ...
    if (certificateFound_) {
        for (const CertificateDetails det : sfl::Matrix0D<CertificateDetails>()) {
            ...
            SFL_WARN("FOO! %s   ---   %s",CertificateDetailsNames[det],val.c_str());
...
static TlsValidator::CheckResult checkError(int err, char* copy_buffer, size_t size)
{
    return TlsValidator::TlsValidator::CheckResult(
        err == GNUTLS_E_SUCCESS ? TlsValidator::CheckValues::CUSTOM : TlsValidator::CheckValues::UNSUPPORTED,
        err == GNUTLS_E_SUCCESS ? std::string(copy_buffer,size) : ""
    );
}

/**
 * Some fields, sush as the binary signature need to be converted to HEX
 * before being sent to DBus as it will cause the process to assert
 */
...
{
    gnutls_pk_algorithm_t algo = (gnutls_pk_algorithm_t) gnutls_x509_crt_get_pk_algorithm(x509Certificate_,nullptr);
    if (algo < 0)
        return TlsValidator::CheckResult(CheckValues::UNSUPPORTED,"");
    ...
    return TlsValidator::CheckResult(CheckValues::CUSTOM, std::string(name));
}
...
TlsValidator::CheckResult TlsValidator::getSignatureAlgorithm()
{
    ...
    return TlsValidator::CheckResult(CheckValues::CUSTOM, std::string(algoName));
}
