...
void SIPAccount::registerAccount()
{
    if (hostname_.length() >= PJ_MAX_HOSTNAME)
        return;

    DEBUG("SIPAccount::registerAccount %s ", hostname_.c_str());
...
#if HAVE_IPV6
        if (isIP2IP()) {
            DEBUG("SIPAccount::registerAccount isIP2IP.");
...
void SIPAccount::unregister(std::function<void(bool)> released_cb)
{
    if (isIP2IP()) {
        if (released_cb)
            released_cb(false);
    ...
    try {
        ...
        ERROR("SIPAccount::unregister %s", e.what());
        setTransport();
        if (released_cb)
            released_cb(false);
    }
