...
SIPCall::internalOffHold(const std::function<void()>& /*SDPUpdateFunc*/)
{
    /*
     * TODO ebail : *
     * Hold off use old codec API (based on audiocodec wrapper)
...
SIPCall::startAllMedia()
{
    ...
    try {
        ...
        for (const auto& i : sessionMedia) {
            ...
            const int pl = i->payloadType;

            auto ac = std::static_pointer_cast<SystemAudioCodecInfo>(getSystemCodecContainer()->searchCodecByPayload(pl));
            if (!ac) {
