...
SIPCall::startAllMedia()
{
    ...
    videoRTP.updateDestination(sdp_->getRemoteIP(), sdp_->getRemoteVideoPort());
    if (isIceRunning())
        videoRTP.start(getIceSocket(1));
    else {
	    const auto localVideoPort = sdp_->getLocalVideoPort();
        videoRTP.start(localVideoPort ? localVideoPort : sdp_->getRemoteVideoPort());
    }
#endif
