...
static unsigned char *crypto_file_read(const char *path, size_t *out_len)
{
    ...
    *out_len = 0;
    ...
    if (st.st_size <= 0 || st.st_size > INT_MAX) {
        LOG_ERROR("Invalid file '%s' length %ld.", path, st.st_size);
        goto out;
    }
    ...
    if (!data) {
        LOG_ERROR("Not enough memory to read file '%s'.", path);
        goto out;
    }
...
static int crypto_cert_check_date(gnutls_x509_crt_t cert)
{
    ...
    if (activationTime == -1) {
        LOG_ERROR("Could not retrieve activation time.");
        return -1;
    }
    if (now < activationTime) {
        LOG_ERROR("Certificate not yet activated.");
        return -1;
    }

    expirationTime = gnutls_x509_crt_get_expiration_time(cert);
    if (expirationTime == -1) {
        LOG_ERROR("Could not errrieve expiration time.");
        return -2;
    }
...
static unsigned char *crypto_cert_read(const char *path, size_t *out_len)
{
    ...
    if (gnutls_x509_crt_init(&cert) != GNUTLS_E_SUCCESS) {
        LOG_ERROR("Not enough memory for certificate.");
        goto out;
    }
    ...
    if (err != GNUTLS_E_SUCCESS) {
        LOG_ERROR("Could not import certificate %s - %s", path, gnutls_strerror(err));
        goto out;
    }
    ...
    *out_len = 10000;
    ...
    if (err != GNUTLS_E_SUCCESS) {
        ...
        data = NULL;
        *out_len = 0;
        LOG_ERROR("Certificate %s could not be exported - %s.\n",
...
static int crypto_cert_load_trusted(gnutls_certificate_credentials_t cred)
{
    ...
    if (!trust_store) {
        LOG_ERROR("Failed to open system trusted store.");
        goto out;
    }
    while ((trust_ca = readdir(trust_store)) != NULL) {
        ...
        if (err < 0) {
            ...
            LOG_ERROR("File name too long '%s'.", trust_ca->d_name);
            goto out;
        }
        err = stat(ca_file, &statbuf);
        if (err < 0) {
            LOG_ERROR("Failed to stat file '%s'.", ca_file);
            goto out;
        }
        ...
        if (err == 0) {
            ...
            LOG_ERROR("Could not load trusted certificates - %s", gnutls_strerror(err));
            goto out;
        }
    }
...
int containsPrivateKey(const char *pemPath)
{
    ...
    if (err != GNUTLS_E_SUCCESS) {
        LOG_ERROR("Could not init GnuTLS - %s", gnutls_strerror(err));
        free(dt.data);
        return res;
    }
    ...
    if (err != GNUTLS_E_SUCCESS) {
        LOG_ERROR("Could not init key - %s", gnutls_strerror(err));
        free(dt.data);
    ...
    if (err != GNUTLS_E_SUCCESS) {
        LOG_ERROR("Could not read key - %s", gnutls_strerror(err));
        goto out;
    }
...
int certificateIsValid(const char *caPath, const char *certPath)
{
    ...
    if (err != GNUTLS_E_SUCCESS) {
        LOG_ERROR("Could not init certificate - %s", gnutls_strerror(err));
        goto out;
    }
    ...
    if (err != GNUTLS_E_SUCCESS) {
        LOG_ERROR("Could not read certificate - %s", gnutls_strerror(err));
        goto out;
    }
    free(cert_dt.data);
    cert_dt.data = NULL;
    ...
    if (!self_signed && !caPath) {
        LOG_ERROR("Certificate is not self-signed, and CA is not provided.");
        goto out;
    }
    if (caPath) {
        ...
        if (err != GNUTLS_E_SUCCESS) {
            LOG_ERROR("Could not init CA - %s", gnutls_strerror(err));
            goto out;
        }
        ...
        if (err != GNUTLS_E_SUCCESS) {
            LOG_ERROR("Could not read CA - %s", gnutls_strerror(err));
            goto out;
        }
        free(ca_dt.data);
        ca_dt.data = NULL;
        ...
        if (!self_signed) {
            LOG_ERROR("Certificate is not issued by the provided CA.");
            goto out;
        }
        ...
        if (err < 0) {
            LOG_ERROR("Could not verify cert: %s", gnutls_strerror(err));
            goto out;
        }
        if (output & GNUTLS_CERT_INVALID) {
            LOG_ERROR("Verification failed.");
            if (output & GNUTLS_CERT_SIGNER_NOT_FOUND)
                LOG_ERROR("The certificate hasn't got a known issuer.");
            if (output & GNUTLS_CERT_SIGNER_NOT_CA)
                LOG_ERROR("The certificate issuer is not a CA.");
            if (output & GNUTLS_CERT_REVOKED)
                LOG_ERROR("The certificate has been revoked.");
            if (output & GNUTLS_CERT_EXPIRED)
                LOG_ERROR("The certificate has expired.");
            if (output & GNUTLS_CERT_NOT_ACTIVATED)
                LOG_ERROR("The certificate is not yet activated.");
            goto out;
        }
    }
...
int verifyHostnameCertificate(const char *host, const uint16_t port)
{
    ...
    if (!host || !port) {
        LOG_ERROR("Wrong parameters used - host %s, port %d.", host, port);
        return res;
    }
        ...
        if (errno == EINPROGRESS) {
            do {
                ...
                if (err < 0 && errno != EINTR) {
                    LOG_ERROR("Could not connect to hostname %s at port %d",
                          host, port);
                    goto out;
                } else if (err > 0) {
            ...
            LOG_ERROR("Could not connect to hostname %s at port %d", host, port);
            goto out;
        }
    }
    ...
    if (err < 0) {
        LOG_ERROR("Could not set TCP_NODELAY.");
        goto out;
    }
    ...
    if (err != GNUTLS_E_SUCCESS) {
        LOG_ERROR("Could not allocate credentials - %s", gnutls_strerror(err));
        goto out;
    }
    err = crypto_cert_load_trusted(cred);
    if (err != GNUTLS_E_SUCCESS) {
        LOG_ERROR("Could not load credentials.");
        goto out;
    }
    ...
    if (err != GNUTLS_E_SUCCESS) {
        LOG_ERROR("Could not init session -%s\n", gnutls_strerror(err));
        goto out;
    }
    ...
    if (err != GNUTLS_E_SUCCESS) {
        LOG_ERROR("Could not set up ciphers - %s (%s)", gnutls_strerror(err), errptr);
        goto out;
    }
    ...
    if (err != GNUTLS_E_SUCCESS) {
        LOG_ERROR("Could not set up credentials - %s", gnutls_strerror(err));
        goto out;
    }
    ...
    if (err != GNUTLS_E_SUCCESS) {
        LOG_ERROR("Could not set server name - %s", gnutls_strerror(err));
        goto out;
    }
    ...
    if (err != GNUTLS_E_SUCCESS) {
        LOG_ERROR("Handshake failed - %s", gnutls_strerror(err));
        goto out;
    }
    ...
    if (certs == NULL || certslen == 0) {
        LOG_ERROR("Could not obtain peer certificate - %s", gnutls_strerror(err));
        goto out;
    }
    ...
    if (err != GNUTLS_E_SUCCESS) {
        LOG_ERROR("Could not verify the certificate chain - %s", gnutls_strerror(err));
        goto out;
    }
    if (status != 0) {
        gnutls_datum_t msg;
...
#endif
        if (err == 0) {
            LOG_ERROR("Certificate validation failed - %s\n", msg.data);
            gnutls_free(msg.data);
            goto out;
        } else {
            LOG_ERROR("Certificate validation failed with code 0x%x.", status);
            goto out;
        }
    }
    ...
    if (err != GNUTLS_E_SUCCESS) {
        LOG_ERROR("Could not read peer certificate - %s", gnutls_strerror(err));
        goto out;
    }
    /* Finally check if the hostnames match. */
    err = gnutls_x509_crt_check_hostname(cert, host);
    if (err == 0) {
        LOG_ERROR("Hostname %s does not match certificate.", host);
        goto out;
    }
    ...
    if (err < 0) {
        LOG_ERROR("Send failed - %s", gnutls_strerror(err));
        goto out;
    }
    err = gnutls_record_recv(session, buf, sizeof(buf));
    if (err < 0) {
        LOG_ERROR("Recv failed - %s", gnutls_strerror(err));
        goto out;
    }
