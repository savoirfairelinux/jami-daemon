...
invite_session_state_changed_cb(pjsip_inv_session *inv, pjsip_event *ev)
{
    ...
    if (!call_ptr)
        return;
    std::shared_ptr<SIPCall> call = std::static_pointer_cast<SIPCall>(call_ptr->shared_from_this());
...
onCallTransfered(pjsip_inv_session *inv, pjsip_rx_data *rdata)
{
    ...
    if (!call_ptr)
        return;
    std::shared_ptr<SIPCall> currentCall = std::static_pointer_cast<SIPCall>(call_ptr->shared_from_this());
