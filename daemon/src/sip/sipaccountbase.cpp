...
SIPAccountBase::toYaml(Conf::MappingNode& accountmap) const
{
    ...
    for (const auto& codec : videoCodecList_) {
        MappingNode mapNode {};
        mapNode.setKeyValue(VIDEO_CODEC_NAME, ScalarNode(codec.at(VIDEO_CODEC_NAME)));
        mapNode.setKeyValue(VIDEO_CODEC_ENABLED, ScalarNode(codec.at(VIDEO_CODEC_ENABLED)));
        auto it = codec.find(VIDEO_CODEC_BITRATE);
        if (it != codec.end())
            mapNode.setKeyValue(VIDEO_CODEC_BITRATE, ScalarNode(it->second));
        it = codec.find(VIDEO_CODEC_PARAMETERS);
        if (it != codec.end())
            mapNode.setKeyValue(VIDEO_CODEC_PARAMETERS, ScalarNode(it->second));
