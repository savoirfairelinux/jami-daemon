...
SipTransportBroker::transportStateChanged(pjsip_transport* tp, pjsip_transport_state state, const pjsip_transport_state_info* info)
{
    ...
    if (tp->is_destroying) {
        ...
        if (type == PJSIP_TRANSPORT_UDP || type == PJSIP_TRANSPORT_UDP6) {
            ...
            const auto transport_key = std::find_if(iceTransports_.cbegin(), iceTransports_.cend(), [tp](const SipIceTransport& i) {
...
SipTransportBroker::getIceTransport(const std::shared_ptr<sfl::IceTransport>& ice)
{
    ...
    auto ret = std::make_shared<SipTransport>(reinterpret_cast<pjsip_transport*>(&sip_ice_tr));
    transports_[ret->get()] = ret;
    return ret;
}
