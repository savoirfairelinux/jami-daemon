                            ...
                            const pjsip_transport_state_info *info)
{
    ...
    if (isSecure() && extInfo && extInfo->ssl_sock_info && extInfo->ssl_sock_info->established) {
        ...
        if (peer_crt.ptr && peer_crt.slen) {
            ...
            tlsInfos_.peerCert = {};
            RING_WARN("Updating TLS infos with proto %d cipher %s status %d (no crt)", tlsInfos_.proto, pj_ssl_cipher_name(tlsInfos_.cipher), tlsInfos_.verifyStatus, tlsInfos_.peerCert.getId());
        }
    } else {
