...
{
    static const char* PJ_TRANSPORT_STATE_STR[] = {
        "CONNECTED", "DISCONNECTED", "SHUTDOWN", "DESTROY", "UNKNOWN STATE"
    };
    DEBUG("Transport %s -> %s", tp->info, PJ_TRANSPORT_STATE_STR[std::min((size_t)state, ARRAYSIZE(PJ_TRANSPORT_STATE_STR)-1)]);
...
SipTransportBroker::findTransport(pjsip_transport* t)
{
        ...
        return nullptr;
    {
        ...
        if (i == transports_.end()) {
            WARN("findTransport NOT FOUND %s", t->info);
            auto ret = std::make_shared<SipTransport>(t);
            transports_[t] = ret;
            //if (t->flags & PJSIP_TRANSPORT_DATAGRAM) {}
            return ret;
        }
        else if (auto spt = i->second.lock()) {
            WARN("findTransport FOUND %s", t->info);
            return spt;
        }
        else {
            WARN("findTransport FOUND DESTROYING %s", t->info);
            return nullptr;
        }
    }
}
