...
public:
    ...
    void bootstrap(std::vector<sockaddr_storage> nodes)
    {
        {
            std::unique_lock<std::mutex> lck(dht_mtx);
    ...
    void join() {
        ...
        if (dht_thread.joinable()) {
            std::cout << "Waiting for dht thread" << std::endl;
            dht_thread.join();
        }
        if (rcv_thread.joinable())
            rcv_thread.join();
...
private:
    ...
    {
        std::cout << "Dht::doRun() begin" << std::endl;
        ...
        rcv_thread = std::thread([this,s,s6]() {
            ...
            std::cout << "Dht::rcv_thread begin" << std::endl;
            while (true) {
            ...
            std::cout << "Dht::rcv_thread end" << std::endl;
            if (s >= 0)
                close(s);
            if (s6 >= 0)
                close(s6);
