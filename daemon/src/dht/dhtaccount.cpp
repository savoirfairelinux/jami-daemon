...
void DHTAccount::unserialize(const YAML::Node &node)
{
    ...
    if (nodePath_.empty())
        nodePath_ = fileutils::get_data_dir()+DIR_SEPARATOR_STR+getAccountID();

    if (identityPath_.empty())
        identityPath_ = nodePath_+DIR_SEPARATOR_STR "id_rsa";
}
...
DHTAccount::saveIdentity(const Dht::SecureDht::Identity id) const
{
    ...
    {
        auto buffer = id.second->serialize();
