...
DHTAccount::newOutgoingCall(const std::string& id, const std::string& toUrl)
{
    ...
    dht_.get(Dht::InfoHash(toUri), [=](const std::vector<std::shared_ptr<Dht::Value>>& values) {
        for (const auto& v : values) {
            if (v->type != Dht::Value::Type::Peer || v->data.size() == 2)
                continue;
            std::cerr << *v << std::endl;
            IpAddr peer {v->getPeerAddr()};
            std::string toip = getToUri(peer.toString(true, true));
...
DHTAccount::saveIdentity(const Dht::SecureDht::Identity id) const
{
    ...
    int err = gnutls_x509_privkey_export_pkcs8(id.first, GNUTLS_X509_FMT_PEM, nullptr, GNUTLS_PKCS_PLAIN, buffer.data(), &buf_sz);
    if (err != GNUTLS_E_SUCCESS) {
        ERROR("Could not export key - %s", gnutls_strerror(err));
        return;
    }
    buffer.resize(buf_sz);
    {
    ...
    buf_sz = 8192;
    buffer.resize(buf_sz);
    err = gnutls_x509_crt_export(id.second, GNUTLS_X509_FMT_PEM, buffer.data(), &buf_sz);
    if (err != GNUTLS_E_SUCCESS) {
        ERROR("Could not export certificate - %s", gnutls_strerror(err));
        return;
    }
    buffer.resize(buf_sz);
    {
...
void DHTAccount::doRegister()
{
    ...
    dht_.run(getLocalPort()+5, loadIdentity(), [=](Dht::Dht::Status s4, Dht::Dht::Status s6) {
        ...
        if (s4 == Dht::Dht::Status::Connected || s6 == Dht::Dht::Status::Connected) {
            setRegistrationState(RegistrationState::REGISTERED);
            *announced = true;
            DEBUG("Announcing peer !!");
            dht_.put(dht_.getId(), Dht::Value{getLocalPort()}, [](bool ok) {
