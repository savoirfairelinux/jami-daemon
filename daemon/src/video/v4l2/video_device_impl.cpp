...
void VideoV4l2Size::getFrameRates(int fd, unsigned int pixel_format)
{
    ...
    if (ioctl(fd, VIDIOC_ENUM_FRAMEINTERVALS, &frmival)) {
        rates_.push_back(25);
        LOG_ERROR("could not query frame interval for size");
        return;
    }
    ...
    switch(frmival.type) {
        ...
        case V4L2_FRMIVAL_TYPE_CONTINUOUS:
            rates_.push_back(25);
            // TODO
            LOG_ERROR("Continuous Frame Intervals not supported");
            break;
        case V4L2_FRMIVAL_TYPE_STEPWISE:
            rates_.push_back(25);
            // TODO
            LOG_ERROR("Stepwise Frame Intervals not supported");
            break;
    }
}
    ...
    if (!ioctl(fd, VIDIOC_ENUM_FRAMESIZES, &frmsize)) {
        ...
        switch (frmsize.type) {
            ...
            case V4L2_FRMSIZE_TYPE_CONTINUOUS:
                LOG_ERROR("Continuous Frame sizes not supported");
                break;
            case V4L2_FRMSIZE_TYPE_STEPWISE:
                LOG_ERROR("Stepwise Frame sizes not supported");
                break;
        }
    }
