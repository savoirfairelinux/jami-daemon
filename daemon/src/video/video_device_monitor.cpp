...
VideoDeviceMonitor::serialize(Conf::YamlEmitter &emitter)
{
    ...
    for (auto& pref : preferences_) {
        ...
        node->setKeyValue("name", ScalarNode(pref.find("name")->second));
        node->setKeyValue("channel", ScalarNode(pref.find("channel")->second));
        node->setKeyValue("size", ScalarNode(pref.find("size")->second));
        node->setKeyValue("rate", ScalarNode(pref.find("rate")->second));
