...
VideoRtpSession::VideoRtpSession(const string &callID,
    ...
    mutex_(), txArgs_(txArgs), rxArgs_(), sending_(false), receiving_(false),
    callID_(callID)
{}
...
void VideoRtpSession::start(int localPort)
{
    ...
    WARN("[%s]: start: tx=%u, rx=%u", callID_.c_str(), sending_, receiving_);
    ...
    if (conference_)
        ...
        WARN("[%s]: videoLocal_: %p, %u", callID_.c_str(), videoLocal_.get(), videoLocal_.use_count());
        if (videoLocal_ and videoLocal_->attach(sender_.get()))
            videoCtrl->startCamera();
    } else {
        WARN("[%s]: reset video local (was %p, %u)", callID_.c_str(), videoLocal_.get(),  videoLocal_.use_count());
        videoLocal_.reset();
    }
}
...
{
    assert(conference);
    WARN("[%s]: setup conf(%p), Tx=%p, Rx=%p", callID_.c_str(), conference,
         sender_.get(), receiveThread_.get());
...
void VideoRtpSession::enterConference(Conference* conference)
{
    ...
    WARN("[%s]: in conf(%p)", callID_.c_str(), conference);
