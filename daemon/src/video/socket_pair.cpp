...
SocketPair::readRtpData(void *buf, int buf_size)
{
    ...
    // work with IceSocket
    SFL_DBG("readRtpData(%d)", buf_size);
    return rtp_sock_->recv(static_cast<unsigned char*>(buf), buf_size);
}
...
SocketPair::readRtcpData(void *buf, int buf_size)
{
    ...
    // work with IceSocket
    return rtcp_sock_->recv(static_cast<unsigned char*>(buf), buf_size);
}
