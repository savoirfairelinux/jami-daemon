...
void YamlEmitter::addMappingItem(int mappingid, const std::string &key, const std::shared_ptr<YamlNode>& node)
{
    if (node->getType() == SCALAR) {
        std::shared_ptr<ScalarNode> sclr = std::static_pointer_cast<ScalarNode>(node);
        ...
        std::shared_ptr<MappingNode> map = std::static_pointer_cast<MappingNode>(node);
        ...
        std::shared_ptr<SequenceNode> seqnode = std::static_pointer_cast<SequenceNode>(node);
        ...
        for (const auto &it : *seq) {
            //std::shared_ptr<YamlNode> yamlNode = it;
            int id;
            ...
            std::shared_ptr<MappingNode> mapnode = std::static_pointer_cast<MappingNode>(it);
