...
{
    pj_str_t ufrag, pwd;
    SFL_DBG("ICE: starting negotiation");
...
{
    if (pj_ice_strans_sendto(icest_.get(), comp_id, buf, len, nullptr, 0) == PJ_SUCCESS)
        return len;
    return -1;
}
...
IceTransport::waitForData(int comp_id, unsigned int timeout)
{
    auto& io = compIO_[comp_id];
    std::unique_lock<std::mutex> lk(io.mutex);
    if (!io.cv.wait_for(lk, std::chrono::seconds(timeout),
