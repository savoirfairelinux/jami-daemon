...
void
IceTransport::addCandidate(int comp_id, const IpAddr& localAddr, const IpAddr& publicAddr)
{
    ...
    auto ret = pj_ice_sess_add_cand(pj_ice_strans_get_ice_sess(icest_.get()),
        ...
        NULL);
    if (ret != PJ_SUCCESS) {
        RING_ERR("fail to add candidate for comp_id=%d : %s : %s",
                 comp_id
                ,localAddr.toString().c_str()
                ,publicAddr.toString().c_str());
        sip_utils::sip_strerror(ret);
    } else {
        RING_DBG("sucess to add candidate for comp_id=%d : %s : %s",
                 comp_id
                ,localAddr.toString().c_str()
                ,publicAddr.toString().c_str());
    }
        ...
        if (publicIP) {
            ...
            for(unsigned comp_id = 1; comp_id < component_count_; comp_id++) {
                ...
                for(IpAddr addr : candidates) {
                    RING_DBG("Candidate is %s", addr.toString().c_str());
