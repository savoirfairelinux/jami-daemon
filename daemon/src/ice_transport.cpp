                         ...
                         pj_status_t status)
{
    ...
    if (status != PJ_SUCCESS) {
        ...
        ERROR("ICE %s failed: %s", opname, errmsg);
        return;
    }
    ...
    DEBUG("ICE: ufrag:%s, pwd:%s", local_ufrag_.c_str(), local_pwd_.c_str());

    DEBUG("ICE %s done", opname);
...
void
ICETransport::createICESession(unsigned role)
{
    ...
    if (pj_ice_strans_init_ice(icest_.get(), role, NULL, NULL) != PJ_SUCCESS) {
        ERROR("pj_ice_strans_init_ice() failed");
        return;
    }
...
{
    std::vector<std::string> v {local_ufrag_, local_pwd_};
    return v;
}
...
ICETransport::getICECandidates(unsigned comp_id) const
{
    ...
    if (pj_ice_strans_enum_cands(icest_.get(), comp_id+1, &cand_cnt, cand) != PJ_SUCCESS) {
        ERROR("pj_ice_strans_enum_cands() failed");
        return res;
    }
...
ICETransportPool::createTransport(const char* name,
                                  ...
                                  ICETransportCompleteCb&& on_complete_cb)
{
    return new ICETransport(name, component_count,
