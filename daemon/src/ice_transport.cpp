...
{
    SFL_DBG("rx pkt [%u, %p, %u]", comp_id, pkt, size);
...
IceTransport::recv(int comp_id, unsigned char* buf, size_t len)
{
    ...
    size_t copied = 0;
    while (len > 0 and not io.queue.empty()) {
        auto& packet = io.queue.front();
        size_t to_copy = len > packet.datalen ? packet.datalen : len;
        memcpy(buf, packet.data.get(), to_copy);
        len -= to_copy;
        copied += to_copy;
        size_t remain = packet.datalen - to_copy;
        if (remain > 0) {
            auto data = new char[remain];
            memcpy(data, packet.data.get() + to_copy, remain);
            packet.data.reset(data);
            packet.datalen = remain;
        }
    }
...
{
    SFL_DBG("send(%d, %p, %u)", comp_id, buf, len);
    auto status = pj_ice_strans_sendto(icest_.get(), comp_id, buf, len, nullptr, 0);
    if (status != PJ_SUCCESS) {
        sip_utils::sip_strerror(status);
        SFL_ERR("send failed");
        return -1;
...
IceTransport::getNextPacketSize(int comp_id)
{
    ...
    std::unique_lock<std::mutex> lk(io.mutex);
    if (io.queue.empty()) {
        SFL_DBG("getNextPacketSize: empty");
        return 0;
    }
