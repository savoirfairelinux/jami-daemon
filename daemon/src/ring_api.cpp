...
int ring_init(const std::map<EventHandlerKey, void*>& ev_handlers, enum ring_init_flag flags)
{
    ...
    for ( const auto &entry : ev_handlers ) {
        switch(entry.first) {
            ...
            registerEvHandlers(static_cast<ring::call_ev_handlers*>(entry.second));
            break;
            case EventHandlerKey::CONFIG:
            registerEvHandlers(static_cast<ring::config_ev_handlers*>(entry.second));
            break;
            case EventHandlerKey::PRESENCE:
            registerEvHandlers(static_cast<ring::pres_ev_handlers*>(entry.second));
            break;
            case EventHandlerKey::VIDEO:
#ifdef RING_VIDEO
            registerEvHandlers(static_cast<ring::video_ev_handlers*>(entry.second));
#else
