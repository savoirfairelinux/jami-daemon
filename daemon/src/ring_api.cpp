...
int ring_init(std::map<EventHandlerKey, void*> ev_handlers, enum ring_init_flag flags)
{
    // User handlers of library events
    // FIXME: static evil
    static std::map<EventHandlerKey, void*> evHandlers_;
    ...
    for ( const auto &entry : ev_handlers ) {
        switch(entry.first) {
            ...
            getCallManager()->registerEvHandlers(static_cast<ring::ring_call_ev_handlers*>(entry.second));
            break;
            case EventHandlerKey::CONFIG:
            getConfigurationManager()->registerEvHandlers(static_cast<ring::ring_config_ev_handlers*>(entry.second));
            break;
            case EventHandlerKey::PRESENCE:
            getPresenceManager()->registerEvHandlers(static_cast<ring::ring_pres_ev_handlers*>(entry.second));
            break;
            case EventHandlerKey::VIDEO:
            getVideoManager()->registerEvHandlers(static_cast<ring::ring_video_ev_handlers*>(entry.second));
            break;
        }
    }
