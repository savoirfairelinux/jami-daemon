...
int ring_init(const std::map<EventHandlerKey, void*>& ev_handlers, enum ring_init_flag flags)
{
    ...
    for ( const auto &entry : ev_handlers ) {
        switch(entry.first) {
            ...
            ring::Manager::instance().getCallManager()->registerEvHandlers(static_cast<ring::call_ev_handlers*>(entry.second));
            break;
            case EventHandlerKey::CONFIG:
            ring::Manager::instance().getConfigurationManager()->registerEvHandlers(static_cast<ring::config_ev_handlers*>(entry.second));
            break;
            case EventHandlerKey::PRESENCE:
            ring::Manager::instance().getPresenceManager()->registerEvHandlers(static_cast<ring::pres_ev_handlers*>(entry.second));
            break;
            case EventHandlerKey::VIDEO:
#ifdef RING_VIDEO
            ring::Manager::instance().getVideoManager()->registerEvHandlers(static_cast<ring::video_ev_handlers*>(entry.second));
#else
