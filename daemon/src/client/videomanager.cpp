...
VideoManager::getSettings(const std::string& name) {
    return videoDeviceMonitor_.getSettings(name).to_map();
}
...
DeviceParams
VideoManager::getDeviceParams(const std::string& name) const
{
    return videoDeviceMonitor_.getDeviceParams(name);
}
