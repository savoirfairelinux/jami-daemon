...
void setTlsSettings(const std::map<std::string, std::string>& details)
{
    ...
    ring::emitSignal<DRing::ConfigurationSignal::AccountsChanged>();
}
...
std::vector<int32_t> getAudioCodecList()
{
    ...
    if (list.empty())
        ring::emitSignal<DRing::ConfigurationSignal::Error>(CODECS_NOT_LOADED);
...
void setVolume(const std::string& device, double value)
{
    ...
    ring::emitSignal<DRing::ConfigurationSignal::VolumeChanged>(device, value);
}
