...
void
Account::attachCall(std::shared_ptr<Call> call)
{
    callMap_.insert(std::make_pair(call->getCallId(), std::weak_ptr<Call>(call)));
}

void
Account::detachCall(std::string& id)
{
    callMap_.erase(id);
}
...
Account::getCalls()
{
    ...
    for (const auto& item : callMap_)
        if (auto shared = item.second.lock())
            v.push_back(shared);
