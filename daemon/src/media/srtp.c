...
int ff_srtp_set_crypto_raw(struct SRTPContext *s, const char *suite, const uint8_t *master_key, size_t master_key_len, const uint8_t *master_salt, size_t master_salt_len)
{
    ff_srtp_free(s);
    ...
    memcpy(s->master_key, master_key, master_key_len);
    memcpy(s->master_salt, master_salt, master_salt_len);
