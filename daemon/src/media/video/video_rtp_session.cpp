...
void VideoRtpSession::startSender()
{
    if (local_.enabled) {
        ...
        try {
            sender_.reset(new VideoSender(getRemoteRtpUri(), local_, *socketPair_));
...
void VideoRtpSession::startReceiver()
{
    if (remote_.enabled) {
        ...
        receiveThread_.reset(new VideoReceiveThread(callID_, remote_, remote_.receiving_sdp));
...
void VideoRtpSession::setupConferenceVideoPipeline(Conference* conference)
{
    ...
    videoMixer_->setDimensions(local_.width, local_.height);
