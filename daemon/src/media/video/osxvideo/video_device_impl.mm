...
VideoDeviceImpl::VideoDeviceImpl(const std::string& uniqueID) :
    device(uniqueID), name(uniqueID)
{
    avDevice_ = [AVCaptureDevice deviceWithUniqueID:[NSString stringWithCString:uniqueID.c_str()
...
{
    DeviceParams params;
    params.input = "[%s]", device;
    params.format = "avfoundation";
//    params.channel = channel_.idx;
    AVCaptureDeviceFormat* format = [avDevice_ activeFormat];
    CMVideoDimensions dimensions =
                CMVideoFormatDescriptionGetDimensions(format.formatDescription);
    params.width = dimensions.width;
    params.height = dimensions.height;
    AVFrameRateRange* frameRate = (AVFrameRateRange*)
