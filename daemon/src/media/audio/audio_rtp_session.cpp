...
AudioRtpSession::startSender()
{
    ...
    try {
        sender_.reset(
            new AudioSender(callID_, getRemoteRtpUri(), local_, *socketPair_)
        );
    } catch (const MediaEncoderException &e) {
        RING_ERR("%s", e.what());
...
AudioRtpSession::startReceiver()
{
    ...
    receiveThread_.reset(
        new AudioReceiveThread(callID_, remote_.audioformat, remote_.receiving_sdp)
    );
                          ...
                          std::unique_ptr<IceSocket> rtcp_sock)
{
    ...
    try {
        socketPair_.reset(
            new SocketPair(std::move(rtp_sock), std::move(rtcp_sock))
        );
        if (local_.crypto and remote_.crypto) {
            socketPair_->createSRTP(local_.crypto.getCryptoSuite().c_str(), local_.crypto.getSrtpKeyInfo().c_str(),
                                    remote_.crypto.getCryptoSuite().c_str(), remote_.crypto.getSrtpKeyInfo().c_str());
