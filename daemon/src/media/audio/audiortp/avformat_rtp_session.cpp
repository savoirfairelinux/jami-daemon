...
AVFormatRtpSession::start(int localPort)
{
    ...
    try {
        ...
        if (local_.crypto and remote_.crypto) {
            RING_WARN("Init SRTP: outgoing %s %s", local_.crypto.getCryptoSuite().c_str(), local_.crypto.getSrtpKeyInfo().c_str());
            RING_WARN("Init SRTP: incoming %s %s", remote_.crypto.getCryptoSuite().c_str(), remote_.crypto.getSrtpKeyInfo().c_str());
