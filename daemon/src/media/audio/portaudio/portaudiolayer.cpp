...
namespace ring {
		...
		std::vector<std::string> PortAudioLayer::getCaptureDeviceList() const {
				return this->getDeviceByType(false);
		}

		std::vector<std::string> PortAudioLayer::getPlaybackDeviceList() const {
				return this->getDeviceByType(true);
		}

		int PortAudioLayer::getAudioDeviceIndex(const std::string& name, DeviceType type) const {
				int numDevices = 0;
				(void) type;
		...
		std::string PortAudioLayer::getAudioDeviceName(int index, DeviceType type) const {
				(void) type;
				const PaDeviceInfo *deviceInfo;
		...
		int PortAudioLayer::getIndexCapture() const {
				return this->indexIn_;
		}

		int PortAudioLayer::getIndexPlayback() const {
				return this->indexOut_;
		}

		int PortAudioLayer::getIndexRingtone() const {
				return this->indexRing_;
		}

		void PortAudioLayer::startStream() {
				if (isRunning_)
						return;
				this->initStream();
				isRunning_ = true;
		...
		void PortAudioLayer::updatePreference(AudioPreference &preference, int index, DeviceType type)
		{
				(void) index;

				switch (type) {
						case DeviceType::PLAYBACK:
						preference.setAlsaCardout(indexOut_);
						break;

						case DeviceType::CAPTURE:
						preference.setAlsaCardin(indexIn_);
						break;

						case DeviceType::RINGTONE:
						preference.setAlsaCardring(indexRing_);
						break;
		...
		std::vector<std::string> PortAudioLayer::getDeviceByType(const bool& playback) const {
		...
		int PortAudioLayer::paOutputCallback(const void *inputBuffer, void *outputBuffer,
				...
				bool resample = ref->audioFormat_.sample_rate != mainBufferAudioFormat.sample_rate;
				unsigned urgentFramesToGet = ref->urgentRingBuffer_.availableForGet(RingBufferPool::DEFAULT_ID);
				...
				if (normalFramesToGet > 0) {
						...
						if (resample) {
								resampleFactor = static_cast<double>(ref->audioFormat_.sample_rate) / mainBufferAudioFormat.sample_rate;
						...
						Manager::instance().getRingBufferPool().getData(ref->playbackBuff_, RingBufferPool::DEFAULT_ID);
						ref->playbackBuff_.applyGain(ref->isPlaybackMuted_ ? 0.0 : ref->playbackGain_);
		...
		int PortAudioLayer::paInputCallback(const void *inputBuffer, void *outputBuffer,
				unsigned long framesPerBuffer,
				const PaStreamCallbackTimeInfo* timeInfo,
				PaStreamCallbackFlags statusFlags,
				void *userData) {
				...
				bool resample = ref->audioInputFormat_.sample_rate != mainBufferFormat.sample_rate;
				...
				if (resample) {
						int outSamples = framesPerBuffer / (static_cast<double>(ref->audioInputFormat_.sample_rate) / mainBufferFormat.sample_rate);
		...
		// PRIVATE METHOD
		void PortAudioLayer::handleError(const PaError& err) const {
				RING_ERR("PortAudioLayer error : %s",  Pa_GetErrorText(err));
		}

		void PortAudioLayer::init() const {
		...
		void PortAudioLayer::terminate() const {
		...
		void PortAudioLayer::initStream() {
				this->init();
				...
				outputParameters.channelCount = audioFormat_.nb_channels = outputDeviceInfo->maxOutputChannels;
				//FIX ME : Default is 0....
				...
				inputParameters.channelCount = audioInputFormat_.nb_channels = inputDeviceInfo->maxInputChannels;
