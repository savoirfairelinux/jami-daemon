...
MediaCodecFactory::initCodecConfig()
{
    ...
    mediaVideoCodec = new MediaVideoCodec(AV_CODEC_ID_H264, "H264","libx264", CODEC_ENCODER_DECODER);
    addCodec(mediaVideoCodec);
    mediaVideoCodec = new MediaVideoCodec(AV_CODEC_ID_H263P, "H263-2000","h263p", CODEC_ENCODER_DECODER);
    addCodec(mediaVideoCodec);
    ...
    mediaAudioCodec = new MediaAudioCodec(AV_CODEC_ID_PCM_ALAW, "PCMA","pcma", CODEC_ENCODER_DECODER, 64,48000,1,8);
    addCodec(mediaAudioCodec);

    mediaAudioCodec = new MediaAudioCodec(AV_CODEC_ID_OPUS, "opus","libopus", CODEC_ENCODER_DECODER, 0, 48000,2,104);
    addCodec(mediaAudioCodec);
    ...
    for (int i = 0; i < availableCodecList->size(); i++)
    {
        ...
        if (*codecType & CODEC_ENCODER)
        {
            ...
            if (!codec)
            {
                ...
                availableCodecList->at(i)->codecType_ = (CodecType)((unsigned)*codecType & ~CODEC_ENCODER);
            } else {
        ...
        if (*codecType & CODEC_DECODER)
        {
            ...
            if (!codec)
            {
                ...
                availableCodecList->at(i)->codecType_ = (CodecType)((unsigned)*codecType & ~CODEC_DECODER);
            } else {
...
MediaCodecFactory::getMediaCodecList(MediaType mediaType)
{
...
std::vector<int32_t>
MediaCodecFactory::getMediaCodecIdList(MediaType mediaType)
{
...
std::vector<int32_t>
MediaCodecFactory::getActiveMediaCodecIdList(MediaType mediaType)
{
    std::vector<int32_t> idList;
    for ( uint16_t i = 0; i < availableCodecList->size(); i++)
    {
...
void MediaCodecFactory::desactivateAllMedia(MediaType mediaType)
{
    for ( uint16_t i = 0; i < availableCodecList->size(); i++)
    {
        if (availableCodecList->at(i)->mediaType_ & mediaType)
...
ring::MediaCodec*
MediaCodecFactory::searchCodecById(uint16_t codecId, MediaType mediaType)
{
...
ring::MediaCodec*
MediaCodecFactory::searchCodecByName(std::string name, MediaType mediaType)
{
    ring::MediaCodec* foundCodec = NULL;
    for ( uint16_t i = 0; i < availableCodecList->size(); i++)
    {
...
ring::MediaCodec*
MediaCodecFactory::searchCodecByPayload(uint16_t payload, MediaType mediaType)
{
    ring::MediaCodec* foundCodec = NULL;
    for ( uint16_t i = 0; i < availableCodecList->size(); i++)
    {
