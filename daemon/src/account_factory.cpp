...
struct AccountMapBase {
        ...
        AccountMapBase(CreateAccountFunc&& f)
            : create_(std::forward<CreateAccountFunc>(f)) {
        }
...
{
    const auto f = [](const std::string& accountID) {
        auto account = std::make_shared<SIPAccount>(accountID, true);
        return std::dynamic_pointer_cast<Account>(account);
    };

    typedAccountMap_["SIP"] = new AccountMapBase(f);

#if HAVE_IAX
    typedAccountMap_["IAX"] = new AccountMapBase(
