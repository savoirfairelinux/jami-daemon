...
{
    auto sip_gen = new AccountGenerator<SIPAccount>("SIP");
    generators_.push_back(std::unique_ptr<AccountGeneratorBase>(sip_gen));

#if HAVE_IAX
    auto iax_gen = new AccountGenerator<SIPAccount>("IAX");
...
{
    // cache this often used account (weak ptr)
