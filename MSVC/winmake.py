...
def getVSEnv(arch='x64', platform='', version=''):
    env_cmd = 'set path=%path:"=% && ' + getVSEnvCmd(arch, platform, version) + ' && set'
...
def make_daemon(pkg_info, force, sdk_version, toolset):
    ...
    root_logger.warning("Building daemon with preferred sdk version %s and toolset %s", sdk_version, toolset)
    build('daemon', daemon_msvc_dir,
          pkg_info.get('project_paths', []),
          pkg_info.get('custom_scripts', {}),
          sdk_version if pkg_info.get('with_env', '') == '' else pkg_info.get('with_env', ''),
          toolset,
...
def build(pkg_name, pkg_dir, project_paths, custom_scripts, with_env, toolset, arch='x64', conf='Release'):
    ...
    for pp in project_paths:
        ...
        log.debug('Building: ' + pkg_name + " with sdk version " + with_env + " and toolset " + toolset)
        getMSbuilder().build(pkg_name, project_full_path, with_env, toolset)
        build_operations += 1
...
def parse_args():
    ...
    ap.add_argument(
        '-s', '--sdk', default='', type=str,
        help='Use specified windows sdk version')
    ap.add_argument(
        '-t', '--toolset', default='', type=str,
...
def main():
    ...
    if parsed_args.build:
        ...
        resolve(parsed_args.build, parsed_args.force, parsed_args.sdk, parsed_args.toolset)
