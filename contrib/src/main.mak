...
# Integrated cache rule: verify signature, rebuild package only if needed.
.cache-%: | $(STAMP_DIR) $(PREFIX)
	...
	need_build=0; \
	if [ ! -f ".$$pkg" ]; then need_build=1; fi; \
	if [ "$(CACHE_FORCE)" = "1" ] || [ "$$force_pkg" = "1" ]; then need_build=1; fi; \
	if [ "$$new_sig" != "$$old_sig" ]; then need_build=1; fi; \
	if [ "$$need_build" = "1" ]; then \
		echo "==> building $$pkg (cache miss)"; \
