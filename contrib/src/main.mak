...
# Portable SHA256 helper (for stamp signatures)
ifeq ($(shell sha256sum --version >/dev/null 2>&1 || echo FAIL),)
SHA256 = sha256sum
else ifeq ($(shell shasum --version >/dev/null 2>&1 || echo FAIL),)
SHA256 = shasum -a 256
else ifeq ($(shell openssl version >/dev/null 2>&1 || echo FAIL),)
...
SHA256 = $(error No SHA256 utility found (sha256sum, shasum, or openssl))
endif
...
$(STAMP_DIR)/%.sig: | $(STAMP_DIR) $(PREFIX)
	...
	rules_hash=$$( [ -f $$rules_file ] && $(SHA256) $$rules_file | cut -d' ' -f1 ); \
	flags_hash=$$( printf '%s\n' "$(CFLAGS) $(CXXFLAGS) $(LDFLAGS)" | $(SHA256) | cut -d' ' -f1 ); \
	# Derive (first) tarball target from rules.mak, then hash if present
	tar_spec=$$(sed -n 's|^\($(TARBALLS)/[^:]*tar[^:]*\):.*|\1|p' "$(SRC)/$$pkg/rules.mak" | head -n1); \
	tar_hash=none; if [ -n "$$tar_spec" ]; then [ -f "$$tar_spec" ] && tar_hash=$$( $(SHA256) "$$tar_spec" | cut -d' ' -f1 ); fi; \
	new_sig="rules=$$rules_hash flags=$$flags_hash tar=$$tar_hash"; \
	force_pkg=0; for p in $(CACHE_CLEAN_LIST); do [ "$$p" = "$$pkg" ] && force_pkg=1; done; \
	if [ "$(CACHE_FORCE)" = "1" ] || [ "$$force_pkg" = "1" ] || [ "$$new_sig" != "$$sig_before" ] || ! ls "$(PREFIX)/lib" 2>/dev/null | grep -q "$$pkg"; then \
