...
$(STAMP_DIR)/%.sig: | $(STAMP_DIR) $(PREFIX)
	...
	rules_hash=$$( [ -f $$rules_file ] && $(SHA512SUM) $$rules_file | awk '{print $$1}' ); \
	flags_hash=$$( printf '%s\n' "$(CFLAGS) $(CXXFLAGS) $(LDFLAGS)" | $(SHA512SUM) | awk '{print $$1}' ); \
	...
	tar_hash=none; if [ -n "$$tar_spec" ]; then [ -f "$$tar_spec" ] && tar_hash=$$( $(SHA512SUM) "$$tar_spec" | awk '{print $$1}' ); fi; \
	new_sig="rules=$$rules_hash flags=$$flags_hash tar=$$tar_hash"; \
	force_pkg=0; for p in $(CACHE_CLEAN_LIST); do [ "$$p" = "$$pkg" ] && force_pkg=1; done; \
	marker=."$$pkg"; \
	need_build=0; \
	if [ ! -f "$$marker" ]; then need_build=1; fi; \
	if [ "$(CACHE_FORCE)" = "1" ] || [ "$$force_pkg" = "1" ] || [ "$$new_sig" != "$$sig_before" ] || [ "$$need_build" = "1" ]; then \
