...
function main() {
    local list_cpe=$1
<<<<<<< PATCH SET (6367e8 sbom: add generation for windows packages in SBOM)
    local filename="${2:-sbom.cdx.json}"
=======
    local output="common-jami-daemon.cdx.json"
>>>>>>> BASE      (618858 sbom: add a cyclonedx generation for make rules)
...
main "$cpe_windows" "windows-sbom.cdx.json"
