...
else ifeq ($(IOS_TARGET_PLATFORM),$(filter $(IOS_TARGET_PLATFORM),iPhoneOS iPhoneSimulator))
	cd $< && mkdir build && cd build && $(CMAKE) -DDESTDIR=$(PREFIX) -DCMAKE_C_FLAGS='-miphoneos-version-min=9.3' .. && $(MAKE) && $(MAKE) install
else
