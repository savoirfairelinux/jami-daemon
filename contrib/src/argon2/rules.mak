...
.argon2: argon2
	cd $< && sed -i -e 's|@PREFIX@|$(PREFIX)|' -e "s|@HOST_MULTIARCH@||" -e "s|@UPSTREAM_VER@|$(ARGON2_VERSION)|" libargon2.pc
	cd $< && cp libargon2.pc $(PREFIX)/lib/pkgconfig/
	cd $< && $(HOSTVARS) $(MAKE) install PREFIX="$(PREFIX)"
	rm $(PREFIX)/lib/libargon2.so*
	touch $@
