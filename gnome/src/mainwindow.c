...
create_main_window(SFLPhoneClient *client)
{
    ...
    client->builder = uibuilder_new("./sflphone.glade");
    if (!client->builder) {
        g_warning("GUI: Could not load glade file.\n");
        exit(1);
    }
    uibuilder_build(client);

//     GtkWidget *window = gtk_window_new(GTK_WINDOW_TOPLEVEL);
    GtkWidget *window = client->win;
...
//     vbox = gtk_box_new(GTK_ORIENTATION_VERTICAL, 0);
    vbox  = uibuilder_get_widget(client, "vbox");
