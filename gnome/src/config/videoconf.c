...
#if 1
    gchar *verif = gtk_combo_box_text_get_active_text(combo);
    g_debug("combo box set active entry to '%s'", verif);
    g_free(verif);
#endif
}
...
#else
    if (!name) {
        g_debug("no name, stopping the cascade");
        return;
    }
#endif
...
#if 1
    g_debug("CHAN %s", dev->chan);
    g_debug("SIZE %s", dev->size);
    g_debug("RATE %s", dev->rate);
#endif
...
#else
    if (!chan) {
        g_debug("no channel, stopping the cascade");
        return;
    }
#endif
    g_free(dev->chan);
...
#else
    if (!size) {
        g_debug("no size, stopping the cascade");
        return;
    }
#endif
    g_free(dev->size);
...
#else
    if (!rate) {
        g_debug("no rate, stopping the cascade");
        return;
    }
#endif
    g_free(dev->rate);
