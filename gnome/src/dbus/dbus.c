...
static DBusGProxy *videoControlsProxy;
 ...
 */
static void process_existing_call_state_change(callable_obj_t *c, const gchar *state) 
{
    if (c == NULL) {
...
dbus_connect(GError **error)
{
    ...
    dbus_g_proxy_add_signal(callManagerProxy, "transferSucceeded", G_TYPE_INVALID);
    dbus_g_proxy_connect_signal(callManagerProxy, "transferSucceeded",
    ...
    videoControlsProxy = dbus_g_proxy_new_for_name(connection,
            "org.sflphone.SFLphone", "/org/sflphone/SFLphone/VideoControls",
            "org.sflphone.SFLphone.VideoControls");
    g_assert(videoControlsProxy != NULL);
    /* Video related signals */
    dbus_g_proxy_add_signal(videoControlsProxy, "deviceEvent", G_TYPE_INVALID);
    dbus_g_proxy_connect_signal(videoControlsProxy, "deviceEvent",
    ...
    dbus_g_proxy_add_signal(videoControlsProxy, "receivingEvent",
                            G_TYPE_INT, G_TYPE_INT, G_TYPE_INT, G_TYPE_INT,
                            G_TYPE_INT, G_TYPE_INVALID);
    dbus_g_proxy_connect_signal(videoControlsProxy, "receivingEvent",
    ...
    dbus_g_proxy_add_signal(videoControlsProxy, "stoppedReceivingEvent",
                            G_TYPE_INT, G_TYPE_INT, G_TYPE_INVALID);
    dbus_g_proxy_connect_signal(videoControlsProxy, "stoppedReceivingEvent",
    ...
    dbus_g_proxy_set_default_timeout(videoControlsProxy, DEFAULT_DBUS_TIMEOUT);
#endif
...
dbus_clean()
{
    g_object_unref(videoControlsProxy);
...
{
    GError *error = NULL;
    org_sflphone_SFLphone_ConfigurationManager_remove_account(configurationManagerProxy, accountID, &error);
    check_error(error);
}
...
dbus_set_account_details(account_t *a)
{
    g_assert(a);
    GError *error = NULL;
    org_sflphone_SFLphone_ConfigurationManager_set_account_details(configurationManagerProxy, a->accountID, a->properties, &error);
    check_error(error);
}
...
dbus_add_account(account_t *a)
{
    ...
    GError *error = NULL;
    g_free(a->accountID);
    org_sflphone_SFLphone_ConfigurationManager_add_account(configurationManagerProxy, a->properties, &a->accountID, &error);
    check_error(error);
}
...
{
    GError *error = NULL;
    org_sflphone_SFLphone_CallManager_set_volume(callManagerProxy, device, value, &error);
    check_error(error);
}
...
dbus_get_volume(const gchar *device)
{
    gdouble value;
    GError *error = NULL;
    org_sflphone_SFLphone_CallManager_get_volume(callManagerProxy, device, &value, &error);
    check_error(error);
...
dbus_start_tone(int start, guint type)
{
    ...
    org_sflphone_SFLphone_CallManager_start_tone(callManagerProxy, start, type, &error);
    check_error(error);
}
...
dbus_audio_codec_list()
{
    GError *error = NULL;
    GArray *array = NULL;
    org_sflphone_SFLphone_ConfigurationManager_get_audio_codec_list(configurationManagerProxy, &array, &error);
    check_error(error);
...
dbus_video_codec_list()
{
    GError *error = NULL;
    gchar **array = NULL;
    org_sflphone_SFLphone_VideoControls_get_codec_list(videoControlsProxy, &array, &error);
    check_error(error);
...
dbus_get_active_video_codec_list(const gchar *accountID)
{
    gchar **array = NULL;
    GError *error = NULL;
    org_sflphone_SFLphone_VideoControls_get_active_codec_list(videoControlsProxy, accountID, &array, &error);
    check_error(error);
...
{
    GError *error = NULL;
    org_sflphone_SFLphone_VideoControls_set_active_codec_list(videoControlsProxy, list, accountID, &error);
    check_error(error);
}
...
dbus_video_codec_details(const gchar *codec)
{
    GError *error = NULL;
    gchar **array = NULL;
    org_sflphone_SFLphone_VideoControls_get_codec_details(videoControlsProxy,
                                                          codec, &array, &error);
    check_error(error);
    return array;
}
...
dbus_get_current_video_codec_name(const callable_obj_t *c)
{
    ...
    org_sflphone_SFLphone_VideoControls_get_current_codec_name(videoControlsProxy,
...
dbus_get_video_input_device_channel()
{
    ...
    org_sflphone_SFLphone_VideoControls_get_input_device_channel(
        videoControlsProxy, &str, &error);
    check_error(error);
...
dbus_get_video_input_device_size()
{
    ...
    org_sflphone_SFLphone_VideoControls_get_input_device_size(
        videoControlsProxy, &str, &error);
    check_error(error);
...
dbus_get_video_input_device_rate()
{
    ...
    org_sflphone_SFLphone_VideoControls_get_input_device_rate(
        videoControlsProxy, &str, &error);
    check_error(error);
...
dbus_get_video_input_device()
{
    ...
    org_sflphone_SFLphone_VideoControls_get_input_device(videoControlsProxy,
                                                         &str, &error);
    check_error(error);
...
{
    GError *error = NULL;
    org_sflphone_SFLphone_VideoControls_set_input_device(
        configurationManagerProxy, device, &error);
    check_error(error);
}
...
{
    GError *error = NULL;
    org_sflphone_SFLphone_VideoControls_set_input_device_channel(
        videoControlsProxy, channel, &error);
    check_error(error);
}
...
{
    GError *error = NULL;
    org_sflphone_SFLphone_VideoControls_set_input_device_size(
        videoControlsProxy, size, &error);
    check_error(error);
}
...
{
    GError *error = NULL;
    org_sflphone_SFLphone_VideoControls_set_input_device_rate(
        videoControlsProxy, rate, &error);
    check_error(error);
}
...
dbus_get_video_input_device_list()
{
    ...
    if (!org_sflphone_SFLphone_VideoControls_get_input_device_list(videoControlsProxy, &array, &error)) {
        if (error->domain == DBUS_GERROR && error->code == DBUS_GERROR_REMOTE_EXCEPTION)
...
dbus_get_video_input_device_channel_list(const gchar *dev)
{
    ...
    if (!org_sflphone_SFLphone_VideoControls_get_input_device_channel_list(
                videoControlsProxy, dev, &array, &error)) {
        if (error->domain == DBUS_GERROR && error->code == DBUS_GERROR_REMOTE_EXCEPTION)
...
dbus_get_video_input_device_size_list(const gchar *dev, const gchar *channel)
{
    ...
    if (!org_sflphone_SFLphone_VideoControls_get_input_device_size_list(videoControlsProxy, dev, channel, &array, &error)) {
        if (error->domain == DBUS_GERROR && error->code == DBUS_GERROR_REMOTE_EXCEPTION)
...
dbus_get_video_input_device_rate_list(const gchar *dev, const gchar *channel, const gchar *size)
{
    ...
    if (!org_sflphone_SFLphone_VideoControls_get_input_device_rate_list(videoControlsProxy, dev, channel, size, &array, &error)) {
        if (error->domain == DBUS_GERROR && error->code == DBUS_GERROR_REMOTE_EXCEPTION)
...
{
    GError *error = NULL;
    org_sflphone_SFLphone_VideoControls_start_preview_async(videoControlsProxy,
...
{
    GError *error = NULL;
    org_sflphone_SFLphone_VideoControls_stop_preview_async(videoControlsProxy, preview_stopped_cb, &error);
    check_error(error);
}
