...
action_enable(GActionMap *map, const gchar* action_name, gboolean value)
{
    ...
    g_warning("%s action is%s enable", action_name, g_action_get_enabled(action)? "":" not");
}
...
*/
    ...
    gtk_widget_set_sensitive(GTK_WIDGET(newCallWidget_), TRUE);
/*
...
*/
    ...
    */
    ...
    g_warning("sensitivity: toolbar=%s call=%s pickup=%s",
            gtk_widget_get_sensitive(GTK_WIDGET(client->toolbar))? "O":"N",
            gtk_widget_get_sensitive(GTK_WIDGET(newCallWidget_))? "O":"N",
            gtk_widget_get_sensitive(GTK_WIDGET(pickUpWidget_))? "O":"N" );
    g_warning("sensitivity: toolbar=%s call=%s pickup=%s",
            gtk_widget_get_sensitive((client->toolbar))? "O":"N",
            gtk_widget_get_sensitive( GW(client->builder,""))? "O":"N" );
}
...
void
uibuilder_create_menubar(SFLPhoneClient *client)
{
//     gtk_application_set_menubar (GTK_APPLICATION (client),
//             G_MENU_MODEL (gtk_builder_get_object (client->builder, "menubar")));
}

void
uibuilder_create_menustatus(SFLPhoneClient *client)
{
//     G_MENU_MODEL (gtk_builder_get_object (client->builder, "menu-status"));
    return;
}

void
uibuilder_create_toolbar(SFLPhoneClient *client)
{
    GtkWidget *toolbar = GTK_WIDGET(gtk_toolbar_new()); //gtk_builder_get_object (client->builder, "toolbar"));
    guint i = 0;
