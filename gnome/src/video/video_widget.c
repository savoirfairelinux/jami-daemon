                      ...
                      G_GNUC_UNUSED gpointer data)
{
    ...
    /* we are only interested by the window behaviour when there is a remote
     * camera on screen */
    VideoArea *video_area = video_widget_video_area_get(self, VIDEO_AREA_REMOTE);
    if (video_area->show) {
        gint pos_x, pos_y;

        /* retrieve the window position with decoration */
        gtk_window_get_position(GTK_WINDOW(self), &pos_x, &pos_y);

        g_debug("SBN pos : %d %d\n", pos_x, pos_y);
        g_debug("SBN size : %d %d\n", event->width, event->height);
                         ...
                         gchar *video_id)
{
     ...
     * before hiding we memorise the window position */
    if (!g_hash_table_size(priv->video_handles)) {
        /* gint pos_x, pos_y; */

        /* #<{(| retrieve the window position with decoration |)}># */
        /* gtk_window_get_position(GTK_WINDOW(self), &pos_x, &pos_y); */
        /* g_debug("SBN window before hide  : %d %d\n", pos_x, pos_y); */
