...
namespace ring { namespace base64 { namespace test {
    ...
    {
        const std::string output = encode(test_bytes);
        CPPUNIT_ASSERT(test_base64.compare(output) == 0);
    }

    void Base64Test::testDecodingSuccess()
    {
        const std::vector<uint8_t> output = decode(test_base64);
    ...
    void Base64Test::testDecodingFail()
    {
        ...
        decode(test_invalid_base64);
        CPPUNIT_ASSERT(true);
    }
}}} // namespace ring::base64::test

RING_TEST_RUNNER(ring::base64::test::Base64Test::name())
