...
CertStoreTest::trustStoreTest()
{
    ...
    aliceAccount->setCertificateStatus(ca.second->getId().toString(), jami::tls::TrustStore::PermissionStatus::ALLOWED);
    ...
    CPPUNIT_ASSERT(aliceAccount->accountManager()->getCertificateStatus(id)
                   == jami::tls::TrustStore::PermissionStatus::ALLOWED);
    aliceAccount->setCertificateStatus(ca.second->getId().toString(), jami::tls::TrustStore::PermissionStatus::UNDEFINED);
    CPPUNIT_ASSERT(aliceAccount->accountManager()->getCertificateStatus(id)
                   == jami::tls::TrustStore::PermissionStatus::UNDEFINED);
    aliceAccount->setCertificateStatus(ca.second->getId().toString(), jami::tls::TrustStore::PermissionStatus::ALLOWED);
    CPPUNIT_ASSERT(aliceAccount->accountManager()->getCertificateStatus(id)
    ...
    JAMI_ERROR("@@@ {} vs {} ", pinned.size(), storeSize);
    ...
    // Ban device
    aliceAccount->setCertificateStatus(device.second->getId().toString(), jami::tls::TrustStore::PermissionStatus::BANNED);
    ...
    // Ban account
    aliceAccount->setCertificateStatus(account.second->getId().toString(), jami::tls::TrustStore::PermissionStatus::BANNED);
    ...
    // Unban account
    aliceAccount->setCertificateStatus(account.second->getId().toString(),
                                    jami::tls::TrustStore::PermissionStatus::ALLOWED);
    CPPUNIT_ASSERT(aliceAccount->accountManager()->getCertificateStatus(account.second->getId().toString())
                   == jami::tls::TrustStore::PermissionStatus::ALLOWED);
    CPPUNIT_ASSERT(aliceAccount->accountManager()->isAllowed(*ca.second));
    ...
    // Ban CA
    aliceAccount->setCertificateStatus(ca.second->getId().toString(), jami::tls::TrustStore::PermissionStatus::BANNED);
