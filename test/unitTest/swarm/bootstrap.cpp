...
BootstrapTest::testBootstrapOk()
{
    ...
    CPPUNIT_ASSERT(cv.wait_for(lk, 30s, [&]() {
        ...
        && aliceData.messages.size() == aliceMsgSize + 1
...
BootstrapTest::testBootstrapFailed()
{
    ...
    auto aliceMsgSize = aliceData.messages.size();
    libjami::acceptConversationRequest(bobData.accountId, convId);
    CPPUNIT_ASSERT(cv.wait_for(lk, 30s, [&]() { return bobData.conversationReady
        && aliceData.messages.size() == aliceMsgSize + 1
        && aliceData.bootstrap == Conversation::BootstrapStatus::SUCCESS
        && bobData.bootstrap == Conversation::BootstrapStatus::SUCCESS; }));
...
BootstrapTest::testBootstrapFallback()
{
    ...
    CPPUNIT_ASSERT(cv.wait_for(lk, 30s, [&]() { return bobData.bootstrap == Conversation::BootstrapStatus::SUCCESS
    ...
    CPPUNIT_ASSERT(cv.wait_for(lk, 60s, [&]() { return aliceData.bootstrap == Conversation::BootstrapStatus::FALLBACK; }));
    CPPUNIT_ASSERT(cv.wait_for(lk, 30s, [&]() { return aliceData.bootstrap == Conversation::BootstrapStatus::SUCCESS; }));
}
...
BootstrapTest::testBootstrapCompat()
{
    ...
    dynamic_cast<SwarmChannelHandler*>(aliceAccount->channelHandlers()[Uri::Scheme::SWARM].get())->disableSwarmManager = true;
