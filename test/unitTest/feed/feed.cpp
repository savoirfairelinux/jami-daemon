...
private:
    void testFeedCallIsRejected()
    {
        auto id = subscribedFeed();
        const auto media = quietMedia();
        for (const auto& actor : {bob, alice}) {
            auto acc = account(actor);
            auto destination = id + "/" + account(alice)->getUsername() + "/"
                               + std::string(account(alice)->currentDeviceId()) + "/feed-call";
            auto call = acc->convModule()->call(destination, media, [](const auto&, const auto&, const auto&) {});
            const bool started = bool(call);
            if (call)
                libjami::hangUp(actor, call->getCallId());
            CPPUNIT_ASSERT(!started);
        }
    }

    void testFeedDirectHostingIsRejected()
    {
        auto id = subscribedFeed();
        const auto media = quietMedia();
        for (const auto& actor : {bob, alice}) {
            auto acc = account(actor);
            acc->convModule()->hostConference(id, "feed-host", "", media);
            const bool hosted = bool(acc->getConference("feed-host"));
            if (hosted)
                libjami::hangUpConference(actor, "feed-host");
            CPPUNIT_ASSERT(!hosted);
        }
    }

    std::vector<libjami::MediaMap> quietMedia() const
    {
        using namespace libjami::Media;
        return {{{MediaAttributeKey::MEDIA_TYPE, MediaAttributeValue::AUDIO},
                 {MediaAttributeKey::ENABLED, "false"},
                 {MediaAttributeKey::MUTED, "true"},
                 {MediaAttributeKey::SOURCE, ""},
                 {MediaAttributeKey::LABEL, "audio_0"}}};
    }

    std::string subscribedFeed()
    {
        const auto id = libjami::createFeed(alice, "No calls", "", true);
        CPPUNIT_ASSERT(!id.empty());
        CPPUNIT_ASSERT(libjami::setFeedAccess(alice, id, account(bob)->getUsername(), true));
        CPPUNIT_ASSERT(poll([&] { return entry(bob, id)["available"] == "true"; }));
        CPPUNIT_ASSERT(libjami::subscribeFeed(bob, id, true));
    ...
    CPPUNIT_TEST(testFeedCallIsRejected);
    CPPUNIT_TEST(testFeedDirectHostingIsRejected);
