...
private:
    ...
    CPPUNIT_TEST(testBytesProgress);
...
FileTransferTest::testFileInfo()
{
    ...
    CPPUNIT_ASSERT(DRing::fileTransferInfo(aliceId, convId, tidBob, info) == DRing::DataTransferError::success);
    CPPUNIT_ASSERT(info.accountId == aliceId);
    ...
    CPPUNIT_ASSERT(info.author == aliceUri);
    CPPUNIT_ASSERT(info.conversationId == convId);
    CPPUNIT_ASSERT(info.displayName == "SEND");
    CPPUNIT_ASSERT(DRing::fileTransferInfo(bobId, convId, tidBob, info) == DRing::DataTransferError::success);
    CPPUNIT_ASSERT(info.accountId == bobId);
    CPPUNIT_ASSERT(info.lastEvent != DRing::DataTransferEventCode::finished);
    CPPUNIT_ASSERT(info.totalSize == 64000);
...
void
FileTransferTest::testBytesProgress()
{
    std::this_thread::sleep_for(std::chrono::seconds(5));
    // TODO remove. This sleeps is because it take some time for the DHT to be connected
    // and account announced
    ...
    CPPUNIT_ASSERT(DRing::dataTransferBytesProgress(bobId, convId, id, totalSize, bytesProgress) == DRing::DataTransferError::invalid_argument);
    ...
    CPPUNIT_ASSERT(DRing::dataTransferBytesProgress(bobId, convId, id, totalSize, bytesProgress) == DRing::DataTransferError::success);
