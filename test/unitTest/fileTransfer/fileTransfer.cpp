...
FileTransferTest::testCachedFileTransfer()
{
    ...
    std::map<std::string, std::shared_ptr<DRing::CallbackWrapperBase>> confHandlers;
    bool transferCreated = false, successfullyConnected = false, transferFinished = false;
    ...
    info.peer = bobUri;
    info.path = "/home/amarok/Downloads/Jami/Clean_Code.pdf";
    info.displayName = "Clean_Code.pdf";
    info.bytesProgress = 0;
    CPPUNIT_ASSERT(DRing::sendFile(info, id) == DRing::DataTransferError::success);

    JAMI_WARN("@@@WAIT RECEIVE...");
    cv.wait_for(lk, std::chrono::seconds(30));
    CPPUNIT_ASSERT(successfullyReceive);

    JAMI_WARN("@@@WAIT CREATED...");
    cv.wait_for(lk, std::chrono::seconds(30));
    CPPUNIT_ASSERT(transferCreated);

    JAMI_WARN("@@@ACCEPT...");
    CPPUNIT_ASSERT(DRing::acceptFileTransfer(finalId, "/home/amarok/Downloads/Jami/Clean_Code_rcv.pdf", 0) == DRing::DataTransferError::success);

    JAMI_WARN("@@@WAIT FINISHED...");
    cv.wait_for(lk, std::chrono::seconds(30));
