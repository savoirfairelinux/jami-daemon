...
FileTransferTest::testCachedFileTransfer()
{
    ...
    std::map<std::string, std::shared_ptr<DRing::CallbackWrapperBase>> confHandlers;
    std::atomic_bool transferCreated {false};
    std::atomic_int transferFinished {0};
    ...
    std::atomic_bool successfullyReceive {false};
    ...
    CPPUNIT_ASSERT(cv.wait_for(lk, std::chrono::seconds(30), [&]{ return successfullyReceive.load(); }));
    CPPUNIT_ASSERT(cv.wait_for(lk, std::chrono::seconds(30), [&]{ return transferCreated.load(); }));
    ...
    CPPUNIT_ASSERT(cv.wait_for(lk, std::chrono::seconds(30), [&]{ return transferFinished.load() == 2; }));
