...
ConversationFetchSentTest::testSyncFetch()
{
    ...
    CPPUNIT_ASSERT(cv.wait_for(lk, 30s, [&]() {
        JAMI_ERROR("@@@ {} {} {} {}", aliceData.messages.size(), aliceMsgSize, bob2Data.messages.size(), bob2MsgSize);
        return aliceData.messages.size() == aliceMsgSize + 1 && bob2Data.messages.size() == bob2MsgSize + 1; }));
    libjami::sendMessage(bobId, bobData.conversationId, "2"s, "");
    CPPUNIT_ASSERT(cv.wait_for(lk, 30s, [&]() { return aliceData.messages.size() == aliceMsgSize + 2 && bob2Data.messages.size() == bob2MsgSize + 2; }));
    libjami::sendMessage(bobId, bobData.conversationId, "3"s, "");
    CPPUNIT_ASSERT(cv.wait_for(lk, 30s, [&]() { return aliceData.messages.size() == aliceMsgSize + 3 && bob2Data.messages.size() == bob2MsgSize + 3; }));
    libjami::sendMessage(bobId, bobData.conversationId, "4"s, "");
    CPPUNIT_ASSERT(cv.wait_for(lk, 30s, [&]() { return aliceData.messages.size() == aliceMsgSize + 4 && bob2Data.messages.size() == bob2MsgSize + 4; }));

    // Second message is set to displayed by alice
    auto msgId1 = (aliceData.messages.rbegin() + 3)->id;
    auto msgId2 = (aliceData.messages.rbegin() + 2)->id;
    auto msgId3 = (aliceData.messages.rbegin() + 1)->id;
    auto msgId4 = aliceData.messages.rbegin()->id;
    aliceAccount->setMessageDisplayed("swarm:" + aliceData.conversationId, msgId2, 3);
    CPPUNIT_ASSERT(cv.wait_for(lk, 30s, [&]() {
        JAMI_ERROR("@@@ msgId1 {} msgId2 {} msgId3 {} msgId4 {}", msgId1, msgId2, msgId3, msgId4);
        for (const auto& [msgId, status] : bobData.status) {
            JAMI_ERROR("@@@ bobData: {} {}", msgId, status);
        }
        for (const auto& [msgId, status] : bob2Data.status) {
            JAMI_ERROR("@@@ bob2Data: {} {}", msgId, status);
        }
    ...
    auto msgId5 = (aliceData.messages.rbegin() + 1)->id;
    auto msgId6 = aliceData.messages.rbegin()->id;
    CPPUNIT_ASSERT(bobData.status[msgId5] == 1 && bobData.status[msgId6] == 1
                  && bob2Data.status[msgId5] == 1 && bob2Data.status[msgId6] == 1);
}
