...
ConversationFetchSentTest::testMissingMessagesOnLoad()
{
    ...
    // Set starting commit to load from as head (empty string)
    ...
    do {
        ...
        for (const auto& msg : aliceData.messages) {
            CPPUNIT_ASSERT(cv.wait_for(lk, 30s, [&]() { return getMsgStatus(aliceData, msg.id, aliceUri) == libjami::Account::MessageStates::DISPLAYED; }));
