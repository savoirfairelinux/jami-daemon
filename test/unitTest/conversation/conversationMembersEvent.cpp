...
ConversationMembersEventTest::testForgedDeviceCertificateRejected()
{
    ...
    CPPUNIT_ASSERT(cv.wait_for(lk, 30s, [&]() { return aliceData.errorDetected; }));
    for (auto it = aliceData.messages.begin() + aliceMsgSize; it != aliceData.messages.end(); ++it) {
        auto author = it->body.find("author");
        auto body = it->body.find("body");
        CPPUNIT_ASSERT(author == it->body.end() || body == it->body.end() || author->second != aliceUri
                       || body->second != "I am Alice");
    }
}
