...
ConversationMembersEventTest::testBanUnbanGotFirstConv()
{
    ...
    // Bobs re-add Alice
    bobAccount->addContact(aliceUri);
    CPPUNIT_ASSERT(cv.wait_for(lk, 30s, [&]() { return bobData.contactAdded && bob2Data.contactAdded; }));
    bobAccount->sendTrustRequest(aliceUri, {});
    CPPUNIT_ASSERT(cv.wait_for(lk, 30s, [&]() {
    ...
    CPPUNIT_ASSERT(cv.wait_for(lk, 30s, [&]() { return !getMessageFromBody(aliceData, "foo").empty(); }));
    msgId = getMessageFromBody(aliceData, "foo");
    CPPUNIT_ASSERT(cv.wait_for(lk, 30s, [&]() { return getMessage(bobData, msgId) && getMessage(bob2Data, msgId); }));
}
