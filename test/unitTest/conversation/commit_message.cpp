...
CommitMessageTest::testToStringFromStringRoundtrip()
{
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        assertRoundtrip(CommitMessage::fileSent(randomText(1, 50),
                                                randomHex(128),
                                                std::to_string(randomUint64()),
                                                randomFileSize()));
        ...
        assertRoundtrip(CommitMessage::initial(0, randomHex(40)));

        // CommitMessage::vote(userId)
        assertRoundtrip(CommitMessage::vote(randomHex(40)));
    }

    // CommitMessage::initial for group conversation
    assertRoundtrip(CommitMessage::initial(2));
