...
ConversationCallTest::connectSignals()
{
    ...
    confHandlers.insert(DRing::exportable_callback<DRing::CallSignal::ConferenceChanged>(
        [&](const std::string& accountId, const std::string& conferenceId, const std::string&) {
            if (accountId == aliceId)
...
ConversationCallTest::testActiveCalls()
{
    ...
    // should get message
    cv.wait_for(lk, 30s, [&]() {
        return !aliceData_.messages.empty();
    });
...
ConversationCallTest::testActiveCalls3Peers()
{
    ...
    cv.wait_for(lk, 30s, [&]() { return aliceData_.conferenceChanged && carlaData_.hostState == "CURRENT"; });

    // get 3 participants
    auto callList = DRing::getParticipantList(aliceId, bobData_.messages.rbegin()->at("confId"));
    // TODO this is failing, it should not.
