...
ConversationTest::testMergeAfterMigration()
{
    ...
    CPPUNIT_ASSERT(
        cv.wait_for(lk, 60s, [&] {
            return aliceData.messages.size() == aliceMsgSize + 1 && carlaData.messages.size() == carlaMsgSize + 2; }));
}
...
ConversationTest::testDoNotLoadIncorrectConversation()
{
    ...
    aliceAccount->convModule()->loadConversations(); // Refresh. This should detect the incorrect conversations.
