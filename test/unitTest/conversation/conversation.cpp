...
public:
    ...
    void commit(std::shared_ptr<JamiAccount> account,
                const std::string& convId,
                Json::Value& message);
    void commitInRepo(const std::string& repoPath,
...
END:VCARD";
    ...
    auto infos = aliceAccount->conversationInfos(convId);
    CPPUNIT_ASSERT(infos["title"] == "TITLE");
    CPPUNIT_ASSERT(infos["description"] == "DESC");
}
