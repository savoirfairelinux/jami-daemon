...
ConversationTest::testReplaceWithBadCertificate()
{
    ...
    auto repoPath = fmt::format("{}/{}/conversations/{}", fileutils::get_data_dir(), aliceAccount->getAccountID(), convId);
    auto aliceDevicePath = fmt::format("{}/devices/{}.crt", repoPath, aliceAccount->currentDeviceId());
...
ConversationTest::testNoBadCertInInitialCommit()
{
    ...
    auto carlaUri = carlaAccount->getUsername();
    auto aliceUri = aliceAccount->getUsername();
    auto fakeCert = jami::tls::CertificateStore::instance().getCertificate(std::string(aliceAccount->currentDeviceId()));
    auto carlaCert = jami::tls::CertificateStore::instance().getCertificate(std::string(carlaAccount->currentDeviceId()));
