...
private:
    ...
    void testStoredLegacyValuePreserved();
    ...
    CPPUNIT_TEST(testStoredLegacyValuePreserved);
...
void
VideoPreferencesTest::testStoredLegacyValuePreserved()
{
    // Stored values are left untouched: a config written under the previous
    // fixed default keeps pinning the mixer to 1280x720.
    YAML::Node root;
    root["video"]["conferenceResolution"] = "1280x720";

    VideoPreferences prefs;
    prefs.unserialize(root);
    CPPUNIT_ASSERT_EQUAL(std::string("1280x720"), prefs.getConferenceResolution());
}
