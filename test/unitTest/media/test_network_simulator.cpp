...
NetworkSimulatorTest::testJitter()
{
    ...
    // With 10 packets and avg 25ms jitter, expect ~250ms total
    // But random, so just verify it took more than 0ms (some jitter applied)
    CPPUNIT_ASSERT(ms > 0);
    // And less than 10 * 50ms = 500ms (worst case)
    CPPUNIT_ASSERT(ms < 600);
}
