...
void
NetworkSimulatorTest::testJitter()
{
    NetworkSimulator sim;
    sim.setEnabled(true);
    sim.setJitter(50); // max 50ms jitter

    // shouldSend() must NOT block — jitter is not applied in the send path.
    // Verify that 100 calls complete in negligible time (< 10ms total).
    ...
    // Must finish almost instantly — certainly less than 10ms for 100 boolean checks
    CPPUNIT_ASSERT_MESSAGE("shouldSend() must not block for jitter", ms < 10);
}
