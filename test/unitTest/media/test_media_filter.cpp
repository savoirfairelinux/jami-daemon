...
MediaFilterTest::testSimpleVideoFilter()
{
    ...
    rational<int> one = rational<int>(1, 1);
    ...
    // prepare filter
    CPPUNIT_ASSERT(filter_->initialize(filterSpec.c_str(), params) >= 0);
...
{
    std::string filterSpec = "overlay=main_w-overlay_w-10:main_h-overlay_h-10";
    ...
    CPPUNIT_ASSERT(filter_->initialize(filterSpec.c_str(), vec) >= 0);

    // apply filter
    CPPUNIT_ASSERT(filter_->feedInput(frame_) >= 0);
    CPPUNIT_ASSERT(filter_->feedInput(extra_) >= 0);
    frame_ = filter_->readOutput();
    CPPUNIT_ASSERT(frame_);

    // check if the filter worked
    CPPUNIT_ASSERT(frame_->width == width1 && frame_->height == height1);
    CPPUNIT_ASSERT(extra_->width == 0);
}
