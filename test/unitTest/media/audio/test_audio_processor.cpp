...
private:
    ...
    CPPUNIT_TEST(testReferenceConversion);
    CPPUNIT_TEST_SUITE_END();
};

CPPUNIT_TEST_SUITE_NAMED_REGISTRATION(AudioProcessorTest, AudioProcessorTest::name());

class InspectableProcessor : public AudioProcessor
{
public:
    using AudioProcessor::AudioProcessor;

    int referenceSamples() const { return playbackQueue_.samples(); }
    std::shared_ptr<AudioFrame> getProcessed() override { return {}; }
...
void
AudioProcessorTest::testReferenceConversion()
{
    const AudioFormat format(48000, 2, AV_SAMPLE_FMT_S16);
    for (const auto& playbackFormat : {AudioFormat(44100, 2, AV_SAMPLE_FMT_FLT), format}) {
        InspectableProcessor processor(format, 480);
        auto record = std::make_shared<AudioFrame>(format, 480);
        processor.putRecorded(std::move(record));

        const int inputSize = playbackFormat.sample_rate / 100;
        for (int index = 0; index < 200; ++index) {
            auto playback = std::make_shared<AudioFrame>(playbackFormat, inputSize);
            libav_utils::fillWithSilence(playback->pointer());
            processor.putPlayback(playback);
        }
        CPPUNIT_ASSERT_DOUBLES_EQUAL(200 * 480, processor.referenceSamples(), 480);
    }
}
