...
private:
    ...
    void testFeedMergePreservesRevocationsAndNewGrants();
    void testFeedRejectsMergeRestoringRevokedAccess();
    std::string addFeedProfileBranch(git_repository* repo,
                                     const std::shared_ptr<JamiAccount>& account,
                                     const std::string& base,
                                     const vCard::utils::VCardData& profile);
    ...
    CPPUNIT_TEST(testFeedMergePreservesRevocationsAndNewGrants);
    CPPUNIT_TEST(testFeedRejectsMergeRestoringRevokedAccess);
...
void
ConversationRepositoryTest::testFeedMergePreservesRevocationsAndNewGrants()
{
    auto alice = Manager::instance().getAccount<JamiAccount>(aliceId);
    auto bob = Manager::instance().getAccount<JamiAccount>(bobId);
    auto feed = ConversationRepository::createConversation(alice, ConversationMode::FEED);
    CPPUNIT_ASSERT(feed);
    const auto base = feed->updateInfos(
        {{"title", "Original"}, {"feedReplies", "true"}, {"feedAccess", bob->getUsername()}});
    CPPUNIT_ASSERT(!base.empty());
    const auto path = fileutils::get_data_dir() / aliceId / "conversations" / feed->id();
    auto bytes = fileutils::loadFile(path / "profile.vcf");
    auto remote = vCard::utils::toMap(std::string_view(reinterpret_cast<const char*>(bytes.data()), bytes.size()));
    remote[std::string(vCard::Property::FORMATTED_NAME)] = "Offline title";
    remote["X-JAMI-FEED-ACCESS"] = bob->getUsername() + "," + std::string(40, 'd');
    CPPUNIT_ASSERT(!feed->updateInfos({{"feedAccess", std::string(40, 'c')}, {"feedReplies", "false"}}).empty());

    git_repository* raw = nullptr;
    CPPUNIT_ASSERT_EQUAL(0, git_repository_open(&raw, path.c_str()));
    GitRepository handle(raw);
    const auto other = addFeedProfileBranch(handle.get(), alice, base, remote);
    auto [ok, merged] = feed->merge(other);
    CPPUNIT_ASSERT(ok && !merged.empty());
    CPPUNIT_ASSERT_EQUAL(std::string(40, 'c') + "," + std::string(40, 'd'), feed->infos().at("feedAccess"));
    CPPUNIT_ASSERT_EQUAL("false"s, feed->infos().at("feedReplies"));
    CPPUNIT_ASSERT_EQUAL("Offline title"s, feed->infos().at("title"));
    CPPUNIT_ASSERT(feed->validCommits({*feed->getCommit(merged)}));

    // A subsequent, explicit owner grant is still allowed.
    const auto regrant = feed->updateInfos({{"feedAccess", bob->getUsername()}, {"feedReplies", "true"}});
    CPPUNIT_ASSERT(!regrant.empty());
    CPPUNIT_ASSERT(feed->validCommits({*feed->getCommit(regrant)}));
}

void
ConversationRepositoryTest::testFeedRejectsMergeRestoringRevokedAccess()
{
    auto alice = Manager::instance().getAccount<JamiAccount>(aliceId);
    auto bob = Manager::instance().getAccount<JamiAccount>(bobId);
    auto feed = ConversationRepository::createConversation(alice, ConversationMode::FEED);
    CPPUNIT_ASSERT(feed);
    const auto base = feed->updateInfos({{"title", "Original"}, {"feedAccess", bob->getUsername()}});
    CPPUNIT_ASSERT(!base.empty());
    const auto path = fileutils::get_data_dir() / aliceId / "conversations" / feed->id();
    auto bytes = fileutils::loadFile(path / "profile.vcf");
    auto remote = vCard::utils::toMap(std::string_view(reinterpret_cast<const char*>(bytes.data()), bytes.size()));
    remote[std::string(vCard::Property::FORMATTED_NAME)] = "Offline title";
    CPPUNIT_ASSERT(!feed->updateInfos({{"feedAccess", ""}}).empty());
    git_repository* raw = nullptr;
    CPPUNIT_ASSERT_EQUAL(0, git_repository_open(&raw, path.c_str()));
    GitRepository handle(raw);
    const auto other = addFeedProfileBranch(handle.get(), alice, base, remote);

    std::ofstream(path / "profile.vcf") << vCard::utils::toString(remote);
    addAll(handle.get());
    const auto forged = addMergeCommit(handle.get(), alice, other);
    CPPUNIT_ASSERT(!forged.empty());
    CPPUNIT_ASSERT(!feed->validCommits({*feed->getCommit(forged)}));
}

std::string
ConversationRepositoryTest::addFeedProfileBranch(git_repository* repo,
                                                 const std::shared_ptr<JamiAccount>& account,
                                                 const std::string& base,
                                                 const vCard::utils::VCardData& profile)
{
    git_oid oid;
    CPPUNIT_ASSERT_EQUAL(0, git_oid_fromstr(&oid, base.c_str()));
    git_commit* rawCommit = nullptr;
    CPPUNIT_ASSERT_EQUAL(0, git_commit_lookup(&rawCommit, repo, &oid));
    GitCommit commit(rawCommit);
    git_reference* rawRef = nullptr;
    CPPUNIT_ASSERT_EQUAL(0, git_branch_create(&rawRef, repo, "feed-other", commit.get(), false));
    GitReference ref(rawRef);
    CPPUNIT_ASSERT_EQUAL(0, git_repository_set_head(repo, "refs/heads/feed-other"));
    git_checkout_options options = GIT_CHECKOUT_OPTIONS_INIT;
    options.checkout_strategy = GIT_CHECKOUT_FORCE;
    CPPUNIT_ASSERT_EQUAL(0, git_checkout_head(repo, &options));
    std::ofstream(std::filesystem::path(git_repository_workdir(repo)) / "profile.vcf")
        << vCard::utils::toString(profile);
    addAll(repo);
    const auto result = addCommit(repo, account, "feed-other", CommitMessage::updateProfile().toString());
    CPPUNIT_ASSERT(!result.empty());
    CPPUNIT_ASSERT_EQUAL(0, git_repository_set_head(repo, "refs/heads/main"));
    CPPUNIT_ASSERT_EQUAL(0, git_checkout_head(repo, &options));
    return result;
}
                                      ...
                                      const std::string& commit_msg)
{
    ...
    if (commit_str) {
        JAMI_LOG("New commit added with id: {}", commit_str);
