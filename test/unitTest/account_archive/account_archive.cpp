...
private:
    ...
    // void testExportDht();
    // void testExportDhtIncorrectPassword();
    ...
    // CPPUNIT_TEST(testExportDhtIncorrectPassword);
...
AccountArchiveTest::testExportImportNoPasswordDoubleGunzip()
{
    ...
    CPPUNIT_ASSERT(aliceAccount->exportArchive("test.jac"));
    auto dat = fileutils::loadFile("test.jac");
    archiver::compressGzip(dat, "test.jac");
...
AccountArchiveTest::testExportImportPassword()
{
    ...
    CPPUNIT_ASSERT(bobAccount->exportArchive("test.jac", "password", "test"));
