        ...
        libjami::exportable_callback<libjami::ConfigurationSignal::AddDeviceStateChanged>(
            [&](const std::string& accountId, uint32_t opId, int status, const std::map<std::string, std::string>& details) {
                ...
                JAMI_DEBUG("[ut_linkdevice:{}] AddDeviceStateChanged: status={}, authSignalCount={}", 
                          testTag, status, authSignalCount);
                cvOld.notify_one();
            }));
...
LinkDeviceTest::testExportNoPassword()
{
    ...
    confHandlers.insert(
        libjami::exportable_callback<libjami::ConfigurationSignal::AddDeviceStateChanged>(
            [&](const std::string& accountId, uint32_t opId, int status, const std::map<std::string, std::string>& details) {
                std::lock_guard<std::mutex> lock(mtxOld);
                JAMI_DEBUG("[ut_linkdevice:{} {}] cb -> AddDeviceStateChanged",
                           testTag,
                           accountId);
                ++authSignalCount;
                // auth status
...
LinkDeviceTest::testExportWithCorrectPassword()
{
    ...
    libjami::provideAccountAuthentication(newDeviceId, correctPassword, fileutils::ARCHIVE_AUTH_SCHEME_PASSWORD);
