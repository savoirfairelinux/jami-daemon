...
LinkDeviceTest::testQrConnection()
{
    ...
    confHandlers.insert(
        libjami::exportable_callback<libjami::ConfigurationSignal::VolatileDetailsChanged>(
            [&](const std::string& accountId, const std::map<std::string, std::string>& details) {
                // JAMI_DEBUG("[ut_linkdevice:{}] cb -> VolatileDetailsChanged {} ?= {} {}", testTag, accountId, oldDeviceId, accountId == oldDeviceId ?"match" : "no match");
                if (accountId != oldDeviceId) {
                    return;
                }
...
// tests the qr code generation that relies on Dht to generate an auth uri
// currently runs without asking for any password in order to simplify the pipeline
void
LinkDeviceTest::testExportToPeer()
{
    ...
    // CPPUNIT_ASSERT(cvNew.wait_for(lkNew, 60s, [&] { return not qrInfo.empty(); }));
    ...
    // TODO test the authentication input
...
{
    // this should not be done with mocking because it will introduce overhead... instead use a template setup and change parameters like password
