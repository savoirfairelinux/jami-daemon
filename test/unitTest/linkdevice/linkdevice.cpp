...
private:
    ...
    // TODO void testMultipleConnections(); // because should not be trying to send the password data to the wrong connection (test leaking password, archive)
...
{
    try {
        // try and cleanup the devices if the test fails or succeeds... this should usually fail due to newDeviceId already being destroyed and same with oldDeviceId
    ...
    confHandlers.insert(
        libjami::exportable_callback<libjami::ConfigurationSignal::VolatileDetailsChanged>(
            [&](const std::string& accountId, const std::map<std::string, std::string>& details) {
        		...
        		// JAMI_DEBUG("[ut_linkdevice::{} {}] oldDevice: daemonStatus = {}", testTag, accountId, daemonStatus);
                if (daemonStatus == "true") {
                    // JAMI_DEBUG("[ut_linkdevice:{}] cb -> VolatileDetailsChanged {}", testTag, accountId);
                    std::lock_guard<std::mutex> lock(mtxOld);
                    oldDeviceStarted = true;
    ...
    */
    ...
    confHandlers.insert(libjami::exportable_callback<libjami::ConfigurationSignal::AddDeviceStateChanged>(
        [&](const std::string& accountId, uint32_t opId, int status, const std::string& detail) {
                ...
                JAMI_DEBUG("[ut_linkdevice:{} {}] cb -> AddDeviceStateChanged w/ detail of {}", testTag, accountId, latestDetailInfo);
                ++authSignalCount;
                if (detail == "success") {
    ...
    JAMI_DEBUG("[ut_linkdevice:{}] [OldDevice] Simulating QR scan... testing libjami::exportToPeer({}, {})", testTag, oldDeviceId, qrInfo);
    libjami::exportToPeer(oldDeviceId, qrInfo);
...
// KESS difference between testProtocol and testExport is that protocol uses a mocked socket and export uses the dht
void LinkDeviceTest::testExportWithCorrectPassword()
{
    ...
    // TODO KESS
    /*
     * have a unittest signal here that allows a password to be provided to the context by enabling the temporary account to receive a signal somehow
     */
    // simulate add device on oldDevice
    confHandlers.insert(libjami::exportable_callback<libjami::ConfigurationSignal::AddDeviceStateChanged>(
        [&](const std::string& accountId, uint32_t opId, int status, const std::string& detail) {
                ...
                ++authSignalCount;
                // check for success
                if (accountId == newDeviceId && detail == "success") { // only success on new device counts because technically old device can succeed in it's job and new can still fail
                    archiveXferSuccessful = true;
                }
                // send password if ready for password
                if (status == 3 /*AUTH state*/ /*static_cast<uint8_t>(ArchiveAccountManager::DeviceAuthState::AUTH) */ && detail == "archive_with_auth") {
                    ...
                    // newAcc->provideAccountAuthentication(correctPassword);//, fileutils::ARCHIVE_AUTH_SCHEME_PASSWORD);
                    newAcc->provideCurrentAccountAuthentication(correctPassword, fileutils::ARCHIVE_AUTH_SCHEME_PASSWORD);
                }
                JAMI_DEBUG("[ut_linkdevice:{}] authSignalCount = {}", testTag, authSignalCount);
                cvOld.notify_one();
        }
    ...
    JAMI_DEBUG("[ut_linkdevice:{}] [OldDevice] Creating old device account with password '{}'.", testTag, correctPassword);
    std::map<std::string, std::string> detailsOldDevice = libjami::getAccountTemplate("RING");
    ...
    JAMI_DEBUG("[ut_linkdevice:{}] [New Device] Creating new device account.", testTag);
    // TODO KESS change this while loop to a wait_for lock in case it gets stuck forever and messes with automated testing
    while (!oldDeviceStarted) {
        JAMI_WARNING("[LinkDevice:{}] Waiting for oldDeviceStarted...", testTag);
    }
