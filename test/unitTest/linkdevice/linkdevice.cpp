...
void LinkDeviceTest::testCreateOldDevice() {
    ...
    JAMI_DEBUG("[ut_linkdevice::{}] [Old Device] Creating old device account.", testTag);
...
{
    const std::string testTag = "testExportToPeer";
    JAMI_DEBUG("\n[ut_linkdevice::{}] Starting ut_linkdevice::testExportToPeer\n", testTag);
    std::mutex mtxOld;
    std::unique_lock<std::mutex> lkOld {mtxOld};
    std::condition_variable cvOld;
    std::mutex mtxNew;
    ...
    confHandlers.insert(
        libjami::exportable_callback<libjami::ConfigurationSignal::VolatileDetailsChanged>(
            [&](const std::string& accountId, const std::map<std::string, std::string>& details) {
                if (accountId!= oldDeviceId) return;
                auto daemonStatus = details.at(libjami::Account::VolatileProperties::DEVICE_ANNOUNCED);
                if (daemonStatus == "true") {
                    JAMI_DEBUG("[ut_linkdevice:{}] cb -> VolatileDetailsChanged {}", testTag, accountId);
                    std::lock_guard<std::mutex> lock(mtxOld);
    ...
    confHandlers.insert(libjami::exportable_callback<libjami::ConfigurationSignal::AddDeviceStateChanged>(
        [&](const std::string& accountId, int status, const std::string& detail) {
            // if (!oldDeviceStarted)
            //     return;
            if (!tmpAccId.empty() && accountId == tmpAccId) {
                std::lock_guard<std::mutex> lock(mtxOld);
                ...
                JAMI_ERROR("[ut_linkdevice:{}] Invalid AddDeviceStateChanged emitted.", testTag);
            }
        }
    ));
    libjami::registerSignalHandlers(confHandlers);
    ...
    // std::map<std::string, std::string> detailsNewDevice;
    std::map<std::string, std::string> detailsNewDevice = libjami::getAccountTemplate("RING");
    detailsNewDevice[ConfProperties::ARCHIVE_URL] = "jami-auth";
    newDeviceId = jami::Manager::instance().addAccount(detailsNewDevice);
    CPPUNIT_ASSERT(cvNew.wait_for(lkNew, 15s, [&] { return not qrInfo.empty(); }));

    JAMI_DEBUG("[ut_linkdevice:{}] [Old Device] Simulating QR scan... testing libjami::exportToPeer({}, {})", testTag, oldDeviceId, qrInfo);
    // CPPUNIT_ASSE/home/kesslerd/Documents/Jami/gerrit_views/jami-daemon-linkdevice/test/unitTest/linkdevice/linkdevice.cppRT(libjami::exportToPeer(oldDeviceId, qrInfo) == 0);
    libjami::exportToPeer(oldDeviceId, qrInfo);
    CPPUNIT_ASSERT(cvOld.wait_for(lkOld, 60s, [&] { return authSignalCount > 0; }));
    ...
    JAMI_DEBUG("\n[ut_linkdevice::{}] Finished ut_linkdevice::testExportToPeer\n", testTag);
    libjami::unregisterSignalHandlers();
}
