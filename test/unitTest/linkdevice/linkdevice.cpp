...
private:
    ...
    // TODO if more testing is needed split into separate tests for:
    // 1) testProtocol: uses a mocked socket
    // 2) testExport:   uses the dht and is slower
};
...
void LinkDeviceTest::testExportWithCorrectPassword()
{
    ...
    JAMI_DEBUG("[ut_linkdevice:{}] [New Device] Creating new device account.", testTag);
    // TODO change this while loop to a wait_for lock in case it gets stuck forever and messes with automated testing
    while (!oldDeviceStarted) {
        JAMI_WARNING("[LinkDevice:{}] Waiting for oldDeviceStarted...", testTag);
    }
