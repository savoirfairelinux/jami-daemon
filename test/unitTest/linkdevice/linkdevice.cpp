...
{
    JAMI_WARN("starting testExportToPeer");
    std::cout << "starting testExportToPeer" << std::endl;
    ...
    CPPUNIT_ASSERT(libjami::exportToPeer(bobId, "jami-auth://" + bobUri) == 0);
    CPPUNIT_ASSERT(cv.wait_for(lk, 20s, [&] { return not qr_info.empty(); }));
    CPPUNIT_ASSERT(qr_info.find(bobUri) != std::string::npos);


    // auto bobAccount = Manager::instance().getAccount<JamiAccount>(bobId);

    // std::map<std::string, std::string> details = libjami::getAccountTemplate("RING");
    // details[ConfProperties:ARCHIVE_URL] = "jami-auth://" + qr_info;
