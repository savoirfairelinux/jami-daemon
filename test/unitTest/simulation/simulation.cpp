...
// the script as well!
// Contants used in the run() function
...
SimulationTest::setUp()
{
    ...
    // Reorder repositoryAccounts to match displayNames order
    // This ensures that repositoryAccounts[i] corresponds to displayNames[i]
    std::vector<RepositoryAccount> orderedAccounts;
    orderedAccounts.reserve(repositoryAccounts.size());
    for (const auto& displayName : displayNames) {
        auto it = std::find_if(repositoryAccounts.begin(), repositoryAccounts.end(), [&](const RepositoryAccount& acc) {
            return acc.account && acc.account->getDisplayName() == displayName;
        });
        if (it != repositoryAccounts.end()) {
            orderedAccounts.push_back(std::move(*it));
            // Stop once we've ordered all accounts
            if (orderedAccounts.size() == repositoryAccounts.size()) {
                break;
            }
        }
    }
    repositoryAccounts = std::move(orderedAccounts);
...
{
    resetRepositories();
    JAMI_LOG("Tearing down SimulationTest...");
}
 ...
 */
void
SimulationTest::eventLogger(const Event& event)
{
 ...
 */
void
SimulationTest::scheduleGitEvent(const ConversationEvent& gitOperation,
                                 int instigatorAccountIndex,
                                 int receivingAccountIndex,
                                 std::chrono::nanoseconds eventTimeOfOccurrence)
{
 ...
 */
void
SimulationTest::scheduleSideEffects(const Event& event)
{
    switch (event.type) {
    case ConversationEvent::ADD_MEMBER:
 ...
 */
bool
SimulationTest::isUserInRepo(int accountIndexToSearch, int accountIndexToFind)
{
 ...
 */
bool
SimulationTest::validateEvent(const Event& event)
{
 ...
 */
void
SimulationTest::triggerEvent(const Event& event)
{
 ...
 */
void
SimulationTest::triggerEvents(const std::vector<Event>& parallelEvents)
{
 ...
 */
bool
SimulationTest::verifyLoadConversationFromScratch()
{
    for (RepositoryAccount& repositoryAccount : repositoryAccounts) {
        if (repositoryAccount.repository) {
 ...
 * SimulationTest class. A sequence of events can be re-simulated provided that the same
 ...
 */
void
SimulationTest::generateEventSequence()
{
 ...
 */
void
SimulationTest::resetRepositories()
{
...
SimulationTest::getHistoricalOrder(std::vector<jami::ConversationCommit> messages)
{
    if (messages.empty())
        return {};
 ...
 */
bool
SimulationTest::checkAppearances(const RepositoryAccount& repoAcc)
{
    if (!repoAcc.repository) {
 ...
 */
bool
SimulationTest::loadUnitTestConfig(const std::string& unitTestName)
{
    ...
    // repositoryAccounts is already ordered by displayNames in setUp()
 ...
 */
void
SimulationTest::saveAsUnitTestConfig()
{
...
SimulationTest::testSimpleConversation()
{
    ...
    CPPUNIT_ASSERT(loadUnitTestConfig("simpleConversation"));
    runUnitTest();
    CPPUNIT_ASSERT(checkAppearances(repositoryAccounts[0]));
    CPPUNIT_ASSERT(checkAppearances(repositoryAccounts[1]));
    CPPUNIT_ASSERT(checkAppearances(repositoryAccounts[2]));
    resetRepositories();
}
...
SimulationTest::testMissingMessage()
{
    ...
    CPPUNIT_ASSERT(loadUnitTestConfig("missingMessage"));
    runUnitTest();
    // We only care to check BOB's message content since we know he has the most up-to-date info
    CPPUNIT_ASSERT(checkAppearances(repositoryAccounts[1]));
    resetRepositories();
}
