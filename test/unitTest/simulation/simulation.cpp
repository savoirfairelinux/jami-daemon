...
{
    UnitTest unitTest = dst_.loadUnitTestConfig("simulation/data/simpleConversation.json");
    CPPUNIT_ASSERT(unitTest.numAccounts > 0);
    CPPUNIT_ASSERT(!unitTest.events.empty());

    dst_.setUp(unitTest.numAccounts);
    dst_.connectSignals();
...
{
    UnitTest unitTest = dst_.loadUnitTestConfig("simulation/data/missingMessage.json");
    CPPUNIT_ASSERT(unitTest.numAccounts > 0);
    CPPUNIT_ASSERT(!unitTest.events.empty());
