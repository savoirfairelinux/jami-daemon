...
private:
    ...
    static void onMediaStateChanged(const std::string& callId,
    ...
    // request. The logic in this method must reflect the exact behavior of the
    // production code, and must be adapted when necessary.
    static bool shouldMediaReNegotiationBeExpected(const TestScenario& scenario);
...
bool
MediaNegotiationTest::shouldMediaReNegotiationBeExpected(const TestScenario& scenario)
{
    // TODO_MC. Review this.
#if 1
    return true;
#else
    CPPUNIT_ASSERT_EQUAL(scenario.offer_.size(), scenario.answer_.size());
    CPPUNIT_ASSERT_EQUAL(scenario.offerUpdate_.size(), scenario.answerUpdate_.size());

    bool expectNegotiation = scenario.offerUpdate_.size() != scenario.offer_.size();

    JAMI_INFO("Is media re-negotiation expected for the media change request: [%s]",
              expectNegotiation ? "YES" : "NO");

    return expectNegotiation;
#endif
}
                                       ...
                                       const TestScenario& scenario)
{
        ...
        Manager::instance().getCallFromCallID(bobData.callId_));
    {
        ...
        auto bobLocalMedia = bobCall->getSDP().getActiveMediaDescription(false);
        CPPUNIT_ASSERT_EQUAL(mediaCountOffer, bobLocalMedia.size());
        for (size_t idx = 0; idx < mediaCountOffer; idx++) {
            CPPUNIT_ASSERT_EQUAL(inferDirection(scenario.answer_[idx].muted_,
                                                scenario.offer_[idx].muted_),
                                 bobLocalMedia[idx].direction_);
        }
    }
...
#if 0
    Manager::instance().muteMediaCall(aliceData.callId_,
                                      MediaAttribute::mediaTypeToString(MediaType::MEDIA_VIDEO),
                                      true);
#else
    {
    ...
    auto mediaCountOfferUpdate = scenario.offerUpdate_.size();
    CPPUNIT_ASSERT_EQUAL(mediaCountOfferUpdate, scenario.answerUpdate_.size());
    ...
    if (shouldMediaReNegotiationBeExpected(scenario)) {
        // Wait for media negotiation complete signal.
        CPPUNIT_ASSERT_EQUAL(true,
                             waitForSignal(aliceData,
                                           DRing::CallSignal::MediaStateChanged::name,
        ...
        // Validate Alice's SDP
        auto aliceLocalMedia = aliceCall->getSDP().getActiveMediaDescription(false);
        CPPUNIT_ASSERT_EQUAL(mediaCountOfferUpdate, aliceLocalMedia.size());

        for (size_t idx = 0; idx < mediaCountOfferUpdate; idx++) {
            CPPUNIT_ASSERT_EQUAL(inferDirection(scenario.offerUpdate_[idx].muted_,
                                                scenario.answerUpdate_[idx].muted_),
                                 aliceLocalMedia[idx].direction_);
        }

        // Validate Bob's SDP
        auto bobLocalMedia = bobCall->getSDP().getActiveMediaDescription(false);
        CPPUNIT_ASSERT_EQUAL(mediaCountOfferUpdate, bobLocalMedia.size());
        for (size_t idx = 0; idx < mediaCountOfferUpdate; idx++) {
            CPPUNIT_ASSERT_EQUAL(inferDirection(scenario.answerUpdate_[idx].muted_,
                                                scenario.offerUpdate_[idx].muted_),
                                 bobLocalMedia[idx].direction_);
        }
