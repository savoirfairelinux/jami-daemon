...
public:
    ...
    /// Poll until @p pred is true or @p timeout elapsed; callbacks never take
    /// the fixture lock, so sleeping while holding it is safe here.
...
CollabTest::testRenameAdminOnly()
{
    ...
    CPPUNIT_ASSERT(poll([&] { return bobData.renamedTo[docId] == "Renamed"; }));
...
CollabTest::testRemoveDocumentEverywhere()
{
    ...
    CPPUNIT_ASSERT(poll([&] { return bobData.removedEverywhere[docId] > 0; }));
...
CollabTest::testRealtimeUpdatePropagation()
{
    ...
    CPPUNIT_ASSERT(poll([&] { return !bobData.docUpdates[docId].empty(); }, 5s));
    for (const auto& update : bobData.docUpdates[docId])
...
CollabTest::testAwareness()
{
    ...
    CPPUNIT_ASSERT(poll([&] { return bobData.participantLeft[docId] > 0; }));
}
