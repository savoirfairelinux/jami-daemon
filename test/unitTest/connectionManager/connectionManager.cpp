...
ConnectionManagerTest::testAcceptConnection()
{
    ...
    bobAccount->connectionManager().onPeerRequest(
...
ConnectionManagerTest::testSendReceiveData()
{
    ...
    bobAccount->connectionManager().onConnectionReady(
    [&cv, &dataOk, &dataOk2, &receiverConnected](const std::string&, const std::string& name, std::shared_ptr<ChannelSocket> socket) {
        if (socket && (name == "test" || name == "other")) {
            ...
            auto idx = 0;
            while (idx < 100) {
                auto res = socket->waitForData(std::chrono::milliseconds(0), ec);
                if (res == 4) {
                    uint8_t buf[4];
                    socket->read(&buf[0], 4, ec);
                    if (name == "test")
                        dataOk = buf[0] == 0x64 && buf[1] == 0x65 && buf[2] == 0x66 && buf[3] == 0x67;
                    else
                        dataOk2 = buf[0] == 0x68 && buf[1] == 0x69 && buf[2] == 0x70 && buf[3] == 0x71;
                    cv.notify_one();
                    break;
                } else {
                    std::this_thread::sleep_for(std::chrono::milliseconds(100));
    ...
    aliceAccount->connectionManager().connectDevice(bobDeviceId, "other",
        ...
        if (socket) {
            successfullyConnected2 = true;
            uint8_t buf[] = {0x68, 0x69, 0x70, 0x71};
            std::error_code ec;
            socket->write(&buf[0], 4, ec);
        }
        cv.notify_one();
