...
add_confirmed_contact(const std::string& accountId, const std::string& contactId)
{
    ...
    auto hasConversation = [](const auto& actor, const auto& uri) {
        return !actor->convModule()->getOneToOneConversation(uri).empty();
    };
    ...
    if (!(isConfirmed(account, contactUri) && isConfirmed(contact, accountUri) && hasConversation(account, contactUri)
          && hasConversation(contact, accountUri))) {
        ...
        lk.lock();
        CPPUNIT_ASSERT(cv.wait_for(lk, std::chrono::seconds(WAIT_FOR_ANNOUNCEMENT_TIMEOUT), [&] {
            return isConfirmed(account, contactUri) && isConfirmed(contact, accountUri)
                   && hasConversation(account, contactUri) && hasConversation(contact, accountUri);
        }));
        lk.unlock();
    }
