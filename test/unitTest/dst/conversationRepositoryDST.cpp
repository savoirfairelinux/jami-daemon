 ...
 * @param unitTestName The file name of the unit test configuration (relative to dst/data/)
 */
bool
ConversationRepositoryDST::loadUnitTestConfig(const std::string& unitTestName)
{
    JAMI_LOG("Loading unit test {}", unitTestName);
    std::filesystem::path configFile("dst/data/" + unitTestName + ".json");
    std::string configContent = fileutils::loadTextFile(configFile);

    // Replace all the accountDisplayNames with their corresponding account IDs
    ...
    for (const auto& [displayName, accountID] : displayNameToAccountID) {
        size_t pos = 0;
        while ((pos = configContent.find(displayName, pos)) != std::string::npos) {
            configContent.replace(pos, displayName.length(), accountID);
...
ConversationRepositoryDST::saveAsUnitTestConfig()
{
    ...
    const std::filesystem::path filePath("dst/data/" + dateStr + "-" + timeStr + ".json");
...
ConversationRepositoryDST::runMissingMessagesUnitTest()
{
    ...
    CPPUNIT_ASSERT(loadUnitTestConfig("20251110-11:48"));
    runUnitTest();
