...
struct RepositoryAccount
{
    ...
    /*!< The most recent commit that was sent when the account was last connected (i.e. "on their
     * chatview")*/
    std::string lastConnectedCommit;
...
// Presets for the DSTConfiguration which will determine what settings to enable
enum class PresetDSTConfiguration {
    SINGLE_CYCLE, /*!< Run a single cycle of random event generation using provided eventSeed (default: rd()) */
    MULTI_CYCLE,  /*!< Run 10 cycles of random event generation using provided eventSeed (default: rd()) */
    UNIT_TEST,    /*!< Run a specific unit test provided in the configurations folder*/
};
...
struct DSTConfiguration
{
    ...
    bool enableResetRepoBetweenIterations {false};
    ...
    bool enableGitLogging {false};

    // = Unit test Preferences =
    bool saveGenerationsAsUnitTests {false};
    bool runUnitTest {false};

    // Assumes MULTI_CYCLE configuration if no preset is defined
    DSTConfiguration()
        : DSTConfiguration(PresetDSTConfiguration::MULTI_CYCLE) {};

    // Default preferences
    DSTConfiguration(const PresetDSTConfiguration& preset, bool eventLogging = true, bool gitLogging = true)
    {
        switch (preset) {
        case PresetDSTConfiguration::SINGLE_CYCLE:
            numCycles = 1;
            enableResetRepoBetweenIterations = false;
            saveGenerationsAsUnitTests = true;
            break;
        case PresetDSTConfiguration::MULTI_CYCLE:
            numCycles = 10;
            enableResetRepoBetweenIterations = true;
            saveGenerationsAsUnitTests = false;
            break;
        case PresetDSTConfiguration::UNIT_TEST:
            saveGenerationsAsUnitTests = false;
            runUnitTest = true;
            break;
        default:
            JAMI_WARNING("Unknown preset, all preferences disabled!");
            break;
        }
...
public:
    ...
    bool checkAppearances(const int receivingAccountIndex, const std::string& oldReceiverHead);
    ...
    // Unit testing
    bool loadUnitTestConfig(const std::string& unitTestFolderName);
...
private:
    ...
    DSTConfiguration dstConfig = DSTConfiguration(PresetDSTConfiguration::SINGLE_CYCLE);
};
        ...
        [&](const std::string& accountId, const std::string& conversationId, std::map<std::string, std::string> message) {
            for (RepositoryAccount& repositoryAccount : repositoryAccounts) {
                if (accountId == repositoryAccount.account->getAccountID()) {
                    ...
                    repositoryAccount.messages.emplace_back(swarmMessage);
...
ConversationRepositoryDST::setUp()
{
    ...
    if (dstConfig.runUnitTest) {
        ...
        loadUnitTestConfig("20251029-09:59");

        // For the case of unit testing, we need the manager to start only after we load the particular configuration
        // from the unit test that we want
        libjami::init(libjami::InitFlag(libjami::LIBJAMI_FLAG_DEBUG | libjami::LIBJAMI_FLAG_CONSOLE_LOG));
        if (not Manager::instance().initialized)
            CPPUNIT_ASSERT(libjami::start("jami-sample.yml"));

        // Load the accounts from the data path
                                            ...
                                            std::chrono::nanoseconds eventTimeOfOccurrence)
{
    ...
    if (static_cast<int>(unvalidatedEvents.size()) >= 2) {
        ...
        if (recentTimeDiff > std::chrono::nanoseconds(0)) {
            ...
            adaptiveDelay = std::chrono::nanoseconds(unvalidatedEvents.back().timeOfOccurrence.count() / 1000);
        }
    } else {
    ...
    if (!unvalidatedEvents.empty() && scheduledTime <= unvalidatedEvents.back().timeOfOccurrence) {
        auto minIncrement = std::chrono::nanoseconds(unvalidatedEvents.back().timeOfOccurrence.count() / 10000);
    ...
    if (instigatorAccountIndex != receivingAccountIndex) {
        ...
        for (int i = 0; i < static_cast<int>(repositoryAccounts.size()); ++i) {
            if (repositoryAccounts[i].repository != nullptr && i != instigatorAccountIndex) {
                ...
                auto accountSpecificTime = scheduledTime + (adaptiveDelay * offsetPercentDist(gen) / 100);
...
ConversationRepositoryDST::triggerEvent(const Event& event)
{
    ...
    case ConversationEvent::CONNECT: {
        ...
        repositoryAccounts[event.instigatorAccountIndex].lastConnectedCommit
    ...
    case ConversationEvent::SEND_MESSAGE: {
        ...
        // Note for logging: commitMessage() returns the ID
        repositoryAccounts[event.instigatorAccountIndex].repository->commitMessage(messageContent, true);
        break;
    }
    case ConversationEvent::CLONE: {
        repositoryAccounts[event.receivingAccountIndex].repository = ConversationRepository::cloneConversation(
    ...
    case ConversationEvent::PULL: {
        ...
        if (!commits.empty()) {
            ...
            // We perform a check here for message to verify that the messages were "displayed" on
            // the client side
            if (!checkAppearances(event.receivingAccountIndex, oldHead)) {
                if (std::find(badSeeds.begin(), badSeeds.end(), eventSeed) == badSeeds.end())
                    badSeeds.push_back(eventSeed);
            }
        }

        // We point the last commit to the current head (i.e. latest pulled commit of the receiver)
        repositoryAccounts[event.receivingAccountIndex].lastConnectedCommit
...
/**
 * Checks whether the messages were sent were pulled were marked as displayed
 * @param receivingAccountIndex The index of the account that pulled from another member
 * @param oldReceiverHead The old HEAD of the receivingAccount's repository
 * @return bool Whether or not all the messages found in the repo were associated with a
 * OnMessageReceived signal
 */
bool
ConversationRepositoryDST::checkAppearances(const int receivingAccountIndex, const std::string& oldReceiverHead)
{
    // Get the new commits that have just been pulled
    std::vector<ConversationCommit> loggedCommitsDiff = repositoryAccounts[receivingAccountIndex].repository->log(
        LogOptions {/* from */ repositoryAccounts[receivingAccountIndex].repository->getHead(),
                    /* to */ oldReceiverHead,
                    /* nbOfCommits */ 0,
                    /* skipMerge */ false,
                    /* includeTo */ false});

    // We iterate through the commits in the log of the account that just pulled (the receiver)
    // and we check whether or not the messages that were sent an OnReceivedMessage
    // ConversationSignal were actually "displayed" or not
    for (const auto& commit : loggedCommitsDiff) {
        const auto found = std::find_if(repositoryAccounts[receivingAccountIndex].messages.begin(),
                                        repositoryAccounts[receivingAccountIndex].messages.end(),
                                        [&](const libjami::SwarmMessage& message) { return message.id == commit.id; });
        if (found == repositoryAccounts[receivingAccountIndex].messages.end()) {
            JAMI_LOG("Could not find matching commit ID in logged commits!");
            return false;
        }
    }
    return true;
...
ConversationRepositoryDST::verifyLoadConversationFromScratch()
{
    for (RepositoryAccount& repositoryAccount : repositoryAccounts) {
        if (repositoryAccount.repository) {
            ...
            JAMI_LOG("COMMITS FROM SWARMLOADED SIGNAL : {} vs COMMITS FROM LOG: {}",
                     repositoryAccount.messages.size(),
                     loggedMessages.size());
...
{
    if (dstConfig.enableResetRepoBetweenIterations || dstConfig.saveGenerationsAsUnitTests) {
...
/**
 * @brief Load the configuration for an existing, previously-ran test. This will configure the class with with accounts,
 * a seed, and the validated event sequence.
 * @param unitTestFolderName The folder name of the unit test configuration (relative to
 * daemon/test/unitTest/dst/conversationRepository/configurations/)
 */
bool
ConversationRepositoryDST::loadUnitTestConfig(const std::string& unitTestFolderName)
{
    JAMI_LOG("Loading unit test {}", unitTestFolderName);

    // Open unitTestFolderName and get text from config.json
    std::filesystem::path configPath
        = std::filesystem::path(
              "/home/ierdogan/Documents/jami-client-qt/daemon/test/unitTest/dst/conversationRepository/configurations")
          / unitTestFolderName;
    std::filesystem::path configFile = configPath / "config.json";
    const std::string configContent = fileutils::loadTextFile(configFile);

    // Copy the account folders from the configPath to HOME/.local/share/jami/
    for (const auto& entry : std::filesystem::directory_iterator(configPath)) {
        if (entry.is_directory()) {
            auto sourcePath = entry.path();
            auto destinationPath = fileutils::get_data_dir() / sourcePath.filename();
    ...
    if (!json::parse(configContent, unitTest)) {
        JAMI_ERROR("Failed to parse unit test {}!", unitTestFolderName);
        return false;
    }
    ...
    // Reference an index for each account ID
...
/**
 * When called, this function will save the DST current configuration (including account folders). Configurations get
 * saved as a subdirectory of the configurations directory. A configuration includes:
 * - Date and time of generation
 * - Seed used
 * - Account IDs used
 * - Validated events sequence
 * @note A key with the name "desc" with an empty string as its value. If pushing the configuration to Jami repository,
 * one should replace the value with a description of what the configuration tests.
 */
void
ConversationRepositoryDST::saveAsUnitTestConfig()
{
    ...
    for (const Event& validatedEvent : validatedEvents) {
        Json::Value validatedEventObject;
        validatedEventObject[unitTestKeys[UTKEY::EVENT_TYPE]] = eventNames[validatedEvent.type];
        validatedEventObject[unitTestKeys[UTKEY::INSTIGATOR_ACCOUNT_ID]]
            = repositoryAccounts[validatedEvent.instigatorAccountIndex].account->getAccountID();
        validatedEventObject[unitTestKeys[UTKEY::RECEIVING_ACCOUNT_ID]]
            = repositoryAccounts[validatedEvent.receivingAccountIndex].account->getAccountID();
    ...
    const std::filesystem::path filePath
        = std::filesystem::path(
              "/home/ierdogan/Documents/jami-client-qt/daemon/test/unitTest/dst/conversationRepository/configurations")
          / (dateStr + "-" + timeStr) / "config.json";

    // Create the directory if it doesn't exist
    std::filesystem::create_directories(filePath.parent_path());
    fileutils::saveFile(filePath, (const uint8_t*) dstConfig.data(), dstConfig.size());

    // Store the account folders from .local/share/jami into the configuration folder
    for (const RepositoryAccount& repoAcc : repositoryAccounts) {
        auto accountDataPath = fileutils::get_data_dir() / repoAcc.account->getAccountID();
        auto destinationPath = filePath.parent_path() / repoAcc.account->getAccountID();
        if (std::filesystem::exists(accountDataPath)) {
            std::filesystem::copy(accountDataPath,
                                  destinationPath,
...
ConversationRepositoryDST::runCycles()
{
    for (int i = 0; i < dstConfig.numCycles; ++i) {
        ...
        eventSeed = 1645552772;
        // Good testing seed: 1366566905
        // Possibly broken seed? : 4146118691, 1645552772<- nghia broken// 1105404777 <- MALFORMED JOINS
        ...
        if (dstConfig.saveGenerationsAsUnitTests && dstConfig.preset == PresetDSTConfiguration::SINGLE_CYCLE) {
