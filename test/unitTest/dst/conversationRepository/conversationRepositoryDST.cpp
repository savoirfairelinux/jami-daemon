...
struct Event
{
    ...
    {
        return instigatorAccountIndex == event.instigatorAccountIndex
               && receivingAccountIndex == event.receivingAccountIndex && type == event.type
               && timeOfOccurrence == event.timeOfOccurrence;
    }
};
...
// the tests will segfault When re-simulating a specific seed, this can be set to false so that the
// account's repo contents can be examined post-test if needed. However, do not forget to delete the
        ...
        [&](const std::string& accountId,
            ...
            for (RepositoryAccount& repositoryAccount : repositoryAccounts) {
                if (accountId == repositoryAccount.account->getAccountID()) {
                    repositoryAccount.messages.emplace_back(message);
                }
            }
            cv.notify_one();
        }));
...
ConversationRepositoryDST::triggerEvent(const Event& event)
{
    ...
    case ConversationEvent::ADD_MEMBER: {
        ...
        if (repositoryAccounts[event.instigatorAccountIndex].connected) {
            repositoryAccounts[event.instigatorAccountIndex].lastConnectedCommit
                = repositoryAccounts[event.instigatorAccountIndex].repository->getHead();
        }
        break;
    ...
    case ConversationEvent::SEND_MESSAGE: {
        ...
        if (repositoryAccounts[event.instigatorAccountIndex].connected) {
            repositoryAccounts[event.instigatorAccountIndex].lastConnectedCommit = id;
        }
        break;
    }
    case ConversationEvent::CLONE: {
                                            ...
                                            const std::string& oldReceiverHead)
{
    ...
    for (const auto& commit : loggedCommitsDiff) {
        const auto found = std::find_if(repositoryAccounts[receivingAccountIndex].messages.begin(),
                                        ...
                                        [&](const libjami::MediaMap& message) {
                                            return message.at("id") == commit.id;
                                        });
