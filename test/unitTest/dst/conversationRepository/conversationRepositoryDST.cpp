...
ConversationRepositoryDST::resetRepositories()
{
    ...
    for (RepositoryAccount& repoAcc : repositoryAccounts)
        if (repoAcc.repository)
            repoAcc.repository.reset(); // Delete the repository object
}
...
ConversationRepositoryDST::runCycles()
{
    ...
    for (int i = 0; i < numCycles; ++i) {
        ...
        eventSeed = 380940672; // 3468856969
    ...
    for (const int& seed : seedsTested) {
        JAMI_LOG("Seed tested: {}", seed);
    }

    if (!badSeeds.empty()) {
        JAMI_LOG("=================== Bad Seeds ==========================");
        for (const int& seed : badSeeds) {
            JAMI_LOG("Bad seed: {}", seed);
        }
    }
}
