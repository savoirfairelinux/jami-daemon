...
ConversationRepositoryDST::connectSignals()
{
    ...
    confHandlers.insert(libjami::exportable_callback<libjami::ConversationSignal::SwarmLoaded>(
        [&](uint32_t requestId,
            ...
            std::vector<libjami::SwarmMessage> messages) {
            JAMI_LOG("=== SWARMLOADED SIGNAL RECEIVED! ===");
            JAMI_LOG("RequestId: {}, AccountId: {}, ConvId: {}, Messages: {}",
                     requestId,
                     accountId,
                     conversationId,
                     messages.size());
            ...
            for (RepositoryAccount& repositoryAccount : repositoryAccounts) {
                if (repositoryAccount.account->getAccountID() == accountId) {
                    accountFound = true;
                    JAMI_LOG("Adding {} messages to account {}", messages.size(), accountId);
                    repositoryAccount.messages.insert(repositoryAccount.messages.end(),
                                                      messages.begin(),
                                                      messages.end());
                    JAMI_LOG("Total messages now: {}", repositoryAccount.messages.size());
                    break;
                }
            }
            if (!accountFound) {
...
ConversationRepositoryDST::setUp()
{
    ...
    // Check for existing accounts that we can reuse to skip setup
    auto existingAccounts = Manager::instance().getAllAccounts();
    if (static_cast<int>(existingAccounts.size()) < numAccounts) {
                                            ...
                                            std::chrono::nanoseconds eventTimeOfOccurrence)
{
    ...
    if (static_cast<int>(unvalidatedEvents.size()) >= 2) {
        // Calculate adaptive timing based on recent events
        ...
        if (recentTimeDiff > std::chrono::nanoseconds(0)) {
            // Git operations should happen within 1-10 times the typical event spacing
            std::uniform_int_distribution<> multiplierDist(1, 50);
            adaptiveDelay = recentTimeDiff * multiplierDist(gen);
        } else {
            // Use a small fraction of the last event's timestamp as fallback
            adaptiveDelay = std::chrono::nanoseconds(
                unvalidatedEvents.back().timeOfOccurrence.count() / 1000);
        }
    } else {
        // For the very first events, use a fraction of the current event's time
        adaptiveDelay = std::chrono::nanoseconds(eventTimeOfOccurrence.count() / 100);
    }

    // Ensure the scheduled time is always after the original event time
    auto scheduledTime = eventTimeOfOccurrence + adaptiveDelay;

    // Additional safety check: ensure we're not scheduling before the last event
    if (!unvalidatedEvents.empty() && scheduledTime <= unvalidatedEvents.back().timeOfOccurrence) {
        // Add a minimal increment that scales with existing timing
    ...
    if (instigatorAccountIndex != receivingAccountIndex) {
        ...
        for (int i = 0; i < static_cast<int>(repositoryAccounts.size()); ++i) {
            if (repositoryAccounts[i].repository != nullptr && i != instigatorAccountIndex) {
                // Generate a small additional offset for each account to spread out the operations
                // Use a percentage of the adaptive delay to maintain relative timing
                std::uniform_int_distribution<> offsetPercentDist(1, 20); // 1-20% of adaptive delay
                ...
                unvalidatedEvents
                    .insert(it, Event(instigatorAccountIndex, i, gitOperation, accountSpecificTime));
            }
        }
    }
}
    ...
    for (const RepositoryAccount& repositoryAccount : repositoryAccounts) {
        if (repositoryAccount.repository) {
            ...
            for (const auto& entry : log) {
                JAMI_LOG("Message: {}, Author: {}", entry.commit_msg, entry.author.name);
            }
        }
    }
}
                                            ...
                                            const std::string& peer)
{
    for (const auto& message : messages) {
        JAMI_LOG("THIS IS THE MESSAGE: {}", message.id);
        if (message.id == id && message.status.find(peer) != message.status.end()) {
            JAMI_LOG("MESSAGE STATUS IS {} AND WE WANT {}",
                     static_cast<int>(message.status.at(peer)),
                     static_cast<int>(libjami::Account::MessageStates::DISPLAYED));
            return static_cast<libjami::Account::MessageStates>(message.status.at(peer));
        }
    }
    JAMI_LOG("Failing ! No message found with id {}", id);
...
ConversationRepositoryDST::verifyLoadConversationFromScratch()
{
    ...
    for (RepositoryAccount& repositoryAccount : repositoryAccounts) {
        ...
        // initialized)
        if (repositoryAccount.repository) {
            JAMI_LOG("REPOSITORY FOUND");

            // Important: After cloning and joining, we need to ensure the conversation module
            // is aware of this conversation. Load the conversations to initialize them.
            auto convModule = repositoryAccount.account->convModule(true);
            if (convModule)
            ...
            // Load the conversation to get signals
            //libjami::loadConversation(accountID, convID, "", 0);
            repositoryAccount.account->convModule()->loadConversation(convID, "", 0);

            // Wait for SwarmLoaded signal
            std::this_thread::sleep_for(std::chrono::milliseconds(100));
            //            repositoryAccount.account->convModule()->loadConversation(convID);

            if (repositoryAccount.messages.empty())
                JAMI_LOG("NO MESSAGES !");
            ...
            for (auto it = std::rbegin(repositoryAccount.messages);
                 ...
                 ++it)
                CPPUNIT_ASSERT(getMessageStatus(repositoryAccount.messages,
                                                it->id,
