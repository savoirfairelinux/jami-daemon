...
ConversationRepositoryDST::setUp()
{
    ...
    eventSeed = 3628096625; //;401857997; // rd(); // 3817990027; // rd(); //2608735150; // rd(); //
                            //3186693956; //
                            //  301082494;  <- for now
                            //   reusing seed, but
                            //    should be rd();
...
{
    JAMI_LOG("======================= Git Logs ============================");
    for (const auto& repositoryAccount : repositoryAccounts) {
...
ConversationRepositoryDST::generateEventSequence()
{
    ...
    while (static_cast<int>(unvalidatedEvents.size()) < maxEvents) {
    ...
    for (int i = 1; i < unvalidatedEvents.size(); i++) {
