...
ConversationRepositoryDST::generateEventSequence()
{
    ...
    std::sort(unvalidatedEvents.begin(), unvalidatedEvents.end(),
    ...
    // Load all the messages from each repo from scratch and verify that they are marked as dispalyed
...
ConversationRepositoryDST::verifyLoadConversationFromScratch()
{
    ...
    for (RepositoryAccount& repositoryAccount : repositoryAccounts) {
        ...
        if (repositoryAccount.repository) {
            ...
            // Use loadMessages2 with a callback that calls addToHistory
            LogOptions options {};
            ...
            // sleep for 5s to allow signals to trigger
