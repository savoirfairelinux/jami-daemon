...
constexpr bool RESET_REPOS = true;
...
struct DSTConfiguration
{
    ...
    DSTConfiguration(int cycles,
        ...
        , unitTestFolderName(testFolderName)
    {}
};
...
ConversationRepositoryDST::setUp()
{
    ...
    if (dstConfig.runUnitTest) {
        ...
        loadUnitTestConfig(dstConfig.unitTestFolderName);

        // For the case of unit testing, we need the manager to start only after we load the particular
        // configuration from the unit test that we want
...
{
    if (dstConfig.enableResetRepoBetweenIterations) {
        resetRepositories();
    }
...
ConversationRepositoryDST::loadUnitTestConfig(const std::string& unitTestFolderName)
{
    ...
    std::filesystem::path configPath = src_dir / "configurations" / unitTestFolderName;
...
ConversationRepositoryDST::saveAsUnitTestConfig()
{
    ...
    const std::filesystem::path filePath = std::filesystem::path(
                                               "/home/ierdogan/Documents/jami-client-qt/daemon/test/unitTest/dst/"
                                               "conversationRepository/configurations")
