...
enum class GitOperation { PULL = 4, CLONE = 5 };
...
ConversationRepositoryDST::validateEvent(const Event& event)
{
    ...
    case ConversationEvent::ADD_MEMBER:
        ...
        if (event.instigatorAccountIndex == event.receivingAccountIndex || !instigatorInConv
            || receiverInConv) {
            invalidEventsCount++;
            return false;
        }
                                            ...
                                            std::chrono::nanoseconds eventTimeOfOccurrence)
{
    ...
    std::uniform_int_distribution<> multiplierDist(1, 10);
    std::chrono::nanoseconds adaptiveDelay = recentTimeDiff * multiplierDist(gen);
    ...
    if (instigatorAccountIndex != receivingAccountIndex) {
        ...
        auto it = std::lower_bound(unvalidatedEvents.begin(),
                                   ...
                                   eventTimeOfOccurrence + adaptiveDelay,
        ...
        for (int i = 0; i < repositoryAccounts.size(); ++i) {
            if (repositoryAccounts[i].repository != nullptr && i != instigatorAccountIndex) {
                // Generate a new adaptive delay for each account to spread out the operations
                auto recentTimeDiff = unvalidatedEvents.back().timeOfOccurrence
                                      - unvalidatedEvents[unvalidatedEvents.size() - 2]
                                            .timeOfOccurrence;
                std::uniform_int_distribution<> multiplierDist(1, 10);
...
ConversationRepositoryDST::generateEventSequence()
{
    ...
    for (int i = 1; i < maxEvents; i++) {
        ...
        } else if (unvalidatedEvents[i].type >= 4 && unvalidatedEvents[i].type <= 5) {
            // Git operations are between 4 and 5
