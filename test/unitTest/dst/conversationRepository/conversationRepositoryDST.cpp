...
/**
 * A structure containing the relevant data for account and repo simulation. Meant to avoid calls to
 * instance manager.
 */
...
struct Preferences
{
    ...
    const bool enableEventLogging {true};
    const bool enableGitLogging {true};
    ...
    const bool enableResetRepoBetweenIterations {true};
};
...
{
    if (preferences.enableEventLogging) {
        auto instigatorName = repositoryAccounts[event.instigatorAccountIndex]
        ...
        default:
            JAMI_WARNING(
                "Unknown ConversationEvent type received, this is a bug! No action has been "
                "triggered.");
            break;
        }
    }
 ...
 * equivalent of running a `git log` for each account that has a repository associated with it.
 */
void
ConversationRepositoryDST::displayGitLog()
{
    if (preferences.enableGitLogging) {
        JAMI_LOG("======================= Git Logs ============================");
...
ConversationRepositoryDST::triggerEvent(const Event& event)
{
    ...
    case ConversationEvent::SEND_MESSAGE: {
        // Pick a random message to send (message content does not affect determinism, using the
        // eventSeed is unnecessary  )
        ...
        std::string id = repositoryAccounts[event.instigatorAccountIndex]
    ...
    case ConversationEvent::PULL: {
        ...
        std::shared_ptr<Conversation> conversation
            = std::make_shared<Conversation>(repositoryAccounts[event.receivingAccountIndex].account,
                                             convID);

        // Use Conversation's pull method instead of repository's pull
        const std::string oldHead = repositoryAccounts[event.receivingAccountIndex]
                                        .repository->getHead();
 ...
 */
bool
ConversationRepositoryDST::checkAppearances(const int receivingAccountIndex,
                                            const std::string& oldReceiverHead)
{
    // Get the new commits that have just been pulled
    std::vector<ConversationCommit> loggedCommitsDiff
        = repositoryAccounts[receivingAccountIndex].repository->log(
...
ConversationRepositoryDST::verifyLoadConversationFromScratch()
{
    for (RepositoryAccount& repositoryAccount : repositoryAccounts) {
        if (repositoryAccount.repository) {
            ...
            std::shared_ptr<Conversation> conversation
                = std::make_shared<Conversation>(repositoryAccount.account, conversationId);
            ...
            std::vector<jami::ConversationCommit> loggedMessages = repositoryAccount.repository->log(
                repoLogOptions);
...
ConversationRepositoryDST::generateEventSequence()
{
    ...
    repositoryAccounts[instigatorAccountIndex].repository
        = ConversationRepository::createConversation(instigatorAccount);
...
{
    if (preferences.enableResetRepoBetweenIterations) {
        ...
        for (RepositoryAccount& repoAcc : repositoryAccounts) {
            if (repoAcc.repository) {
                jami::ConversationModule* convModule = repoAcc.account->convModule(false);
                std::string convId = repoAcc.repository->id();
...
ConversationRepositoryDST::runCycles()
{
    ...
    for (int i = 0; i < numCycles; ++i) {
        ...
        eventSeed = 1645552772;//rd();
        // Good testing seed: 1366566905
        // Possibly broken seed? : 4146118691
