...
/**
 * A structrue containing the relevant data for account and repo simulation. Meant to avoid calls to
 * instance manager.
 */
struct RepositoryAccount
{
    ...
    /*!< The most recent  (i.e. "on their chatview")*/
...
// This should always be true when running more than one cycle of random repo generation, otherwise
// the tests will segfault. When re-simulating a specific seed, set this to false so that the
// repo contents can be examined post-test if needed. However, do not forget to delete the
// created accounts manually, otherwise you will accumulate junk accounts.
...
ConversationRepositoryDST::setUp()
{
    ...
    for (int i = 0; i < static_cast<int>(displayNames.size()); ++i) {
                                            ...
                                            const std::string& oldReceiverHead)
{
    ...
    std::vector<ConversationCommit> loggedCommitsDiff
        = repositoryAccounts[receivingAccountIndex].repository->log(LogOptions {
              /* from */ repositoryAccounts[receivingAccountIndex].repository->getHead(),
              /* to */ oldReceiverHead,
              /* nbOfCommits */ 0,
              /* skipMerge */ false,
              /* includeTo */ false
...
ConversationRepositoryDST::verifyLoadConversationFromScratch()
{
    for (RepositoryAccount& repositoryAccount : repositoryAccounts) {
        if (repositoryAccount.repository) {
            ...
            // Clear messagess that may have been added from previous signal handling
            ...
            std::vector<jami::ConversationCommit> loggedMessages = repositoryAccount.repository->log(
                repoLogOptions);
            JAMI_LOG("COMMITS FROM SWARMLOADED SIGNAL : {} vs COMMITS FROM LOG: {}", repositoryAccount.messages.size(), loggedMessages.size());
...
{
    if (enableResetRepoBetweenIterations) {
...
ConversationRepositoryDST::runCycles()
{
    ...
    for (int i = 0; i < numCycles; ++i) {
        ...
        // Possibly borked seed? : 4146118691
        JAMI_LOG("Random seed generated: {}", eventSeed);
        generateEventSequence();

        // Load each converastion as if it were being opened for the first time
