        ...
        [&](const std::string& accountId, const std::string& conversationId, std::map<std::string, std::string> message) {
            for (RepositoryAccount& repositoryAccount : repositoryAccounts) {
                if (accountId == repositoryAccount.account->getAccountID()) {
                    ...
                    repositoryAccount.addConversationCommit(swarmMessage.id);
...
ConversationRepositoryDST::getHistoricalOrder(std::vector<jami::ConversationCommit> messages)
{
    if (messages.empty())
        return {};
 ...
 */
bool
ConversationRepositoryDST::checkAppearances(const RepositoryAccount& repoAcc)
{
    ...
    std::vector<jami::ConversationCommit> realRepoCommitIDs = repoAcc.repository->log();
    ...
    CPPUNIT_ASSERT_EQUAL(realRepoCommitIDs.size(), historic.size());

    // We can safely access borht vectors' content using the same index
    JAMI_LOG("Verifying linearized parents for account {}:", repoAcc.account->getDisplayName());
    for (size_t i = 0; i < historic.size(); i++) {
        if (historic[i].id == realRepoCommitIDs[i].id) {
            JAMI_LOG("Same commit IDs: {}", historic[i].id);
        } else {
            JAMI_LOG("Differing commit IDS! History ({}) != Log ({})", historic[i].id, realRepoCommitIDs[i].id);
...
ConversationRepositoryDST::loadUnitTestConfig(const std::string& unitTestName)
{
    ...
    std::map<std::string, std::string> displayNameToAccountID;

    for (const RepositoryAccount& repoAcc : repositoryAccounts) {
        displayNameToAccountID.insert({repoAcc.account->getDisplayName(), repoAcc.account->getAccountID()});
    }
    ...
    // Create a copy of configFile called unitTestName_copy.json
    std::filesystem::path copyConfigFile = src_dir / "data" / (unitTestName + "_copy.json");
    std::filesystem::copy_file(configFile, copyConfigFile, std::filesystem::copy_options::overwrite_existing);
    // Replace all the accountDisplayNames in the copy with their corresponding account IDs
    std::string copyConfigContent = fileutils::loadTextFile(copyConfigFile);
    for (const auto& [displayName, accountID] : displayNameToAccountID) {
        size_t pos = 0;
        while ((pos = copyConfigContent.find(displayName, pos)) != std::string::npos) {
            copyConfigContent.replace(pos, displayName.length(), accountID);
            pos += accountID.length();
        }
    }
    // Save the modified content back to the copy file
    std::ofstream ofs(copyConfigFile);
    ofs << copyConfigContent;
    ofs.close();
    const std::string configContent = fileutils::loadTextFile(copyConfigFile);
    ...
    // Delete the copy file (already parsed into the unitTest object)
    std::filesystem::remove(copyConfigFile);
...
ConversationRepositoryDST::runCycles()
{
    for (int i = 0; i < dstConfig.numCycles; ++i) {
        ...
        // Do verifications
        for (const RepositoryAccount& repoAcc : repositoryAccounts) {
            CPPUNIT_ASSERT(checkAppearances(repoAcc));
        }
...
ConversationRepositoryDST::runMissingMessagesUnitTest()
{
    ...
    runUnitTest();
    // We only care to check BOB's message content since we know he has the mose up-to-date info
