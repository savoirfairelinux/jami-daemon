...
function config_unit_test() {
    while true; do
        read -rp "Enter unit test folder name [current: ${UNIT_TEST_FOLDER_NAME}]: " input
        if [[ -z "$input" ]]; then
            break
        fi
        config_dir="$(dirname "$0")/configurations/$input"
        if [[ -d "$config_dir" ]]; then
            UNIT_TEST_FOLDER_NAME="$input"
            echo "Unit test folder name set to: $UNIT_TEST_FOLDER_NAME"
            break
        else
            echo "Directory '$config_dir' does not exist. Please try again."
        fi
    done
    while true; do
        read -rp "Enable event logger? (1=Yes, 0=No) [current ${ENABLE_EVENT_LOGGER}]: " input
