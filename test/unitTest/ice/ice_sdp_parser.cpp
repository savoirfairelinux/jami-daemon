...
void
MediaReceiver::update(Observable<std::shared_ptr<jami::MediaFrame>>* observer,
...
public:
    IceSdpParsingTest()
        ...
        , videoReceiver_(std::make_shared<MediaReceiver>(MediaType::MEDIA_VIDEO))
    {
        // Init daemon
        ...
        // We must have valid media receivers.
        CPPUNIT_ASSERT(audioReceiver_);
        CPPUNIT_ASSERT(videoReceiver_);
    }
...
private:
    ...
    std::shared_ptr<MediaReceiver> videoReceiver_;
};
