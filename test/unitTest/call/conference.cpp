...
ConferenceTest::setUp()
{
    ...
    std::atomic_bool aliceReady {false}, bobReady {false}, carlaReady {false};
    ...
    carlaAccount->addIdentityAnnouncedHook([&](bool ok) {
        if (ok) {
            carlaReady = true;
            cv.notify_one();
        }
    ...
    CPPUNIT_ASSERT(cv.wait_for(lk, std::chrono::seconds(60), [&] {
        return aliceReady.load() && bobReady.load() && carlaReady.load();
    }));
