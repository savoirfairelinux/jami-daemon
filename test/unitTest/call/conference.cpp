...
ConferenceTest::testMultipleActives()
{
    ...
    DRing::setActiveSink(aliceId, confId, bobUri, bobCall.device, bobCall.sinkId, true);
    CPPUNIT_ASSERT(cv.wait_for(lk, 5s, [&] { return bobCall.active.load(); }));

    DRing::setActiveSink(aliceId, confId, carlaUri, carlaCall.device, carlaCall.sinkId, true);
    CPPUNIT_ASSERT(cv.wait_for(lk, 5s, [&] { return carlaCall.active.load(); }));
    CPPUNIT_ASSERT(bobCall.active.load()); // Still active

    DRing::setActiveSink(aliceId, confId, bobUri, bobCall.device, bobCall.sinkId, false);
    CPPUNIT_ASSERT(cv.wait_for(lk, 5s, [&] { return !bobCall.active.load(); }));
...
ConferenceTest::testUnauthorizedSetActive()
{
    ...
    DRing::setActiveSink(carlaId, confId, bobUri, bobCall.device, bobCall.sinkId, true);
