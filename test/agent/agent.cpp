...
Agent::ensureAccount()
{
    ...
    if (details.empty()) {
        ...
        details[DRing::Account::ConfProperties::ID] = accountID_;
...
Agent::registerStaticCallbacks()
{
    ...
    onMessageReceived_.add([=](const std::string& accountID,
        ...
        /* TODO - Remove me when MessageReceived will guarantee to only announce messages once */
        long timestamp = atoi(message.at("timestamp").c_str());
...
Agent::run(const std::string& yaml_config)
{
    ...
    agent.age_ = std::chrono::duration_cast<std::chrono::seconds>(
...
Agent::echo()
{
    ...
    onMessageReceived_.add([=](const std::string& accountID,
        ...
        if (to == accountID and msg == messageSent) {
            *pongReceived = true;
            cv->notify_one();
            return false;
        }
...
Agent::makeCall()
{
    ...
    auto foo = DRing::getConversationMembers(accountID_, *it);

    auto callID = DRing::placeCall(accountID_, "");
