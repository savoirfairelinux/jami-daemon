...
Agent::registerStaticCallbacks(void)
{
    ...
    onIncomingAccountMessage_.add([=](const std::string& accountID,
                                      const std::string& from,
                                      const std::string& messageID,
                                      const std::map<std::string, std::string>& messages) {
        auto msg = messages.at("text/plain");

        AGENT_INFO("Incoming message `%s` from %s", msg.c_str(), from.c_str());
    ...
    onIncomingTrustRequest_.add([=](const std::string& accountID,
        ...
        for (const auto& peer : peers_) {
            if (peer == from) {
                ...
                AGENT_ASSERT(DRing::acceptTrustRequest(account_, from),
                             "Failed to accept trust request from `%s`",
                             from.c_str());
            } else {
                AGENT_ASSERT(DRing::discardTrustRequest(account_, from),
                             "Failed to discrd trust request from `%s`",
                             from.c_str());
            }
        }
...
Agent::sendMessage(const std::string& to, const std::string& msg)
{
    ...
    DRing::sendMessage(account_, details[DRing::Account::TrustRequest::CONVERSATIONID], msg, parent);
}
