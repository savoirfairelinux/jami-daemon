...
Agent::initBehavior()
{
    ...
    BT::register_behavior("start-recording", bind(&Agent::startRecording, this));
    BT::register_behavior("stop-recording", bind(&Agent::stopRecording, this));
}
...
Agent::startRecording()
{
    ...
    public:
        FileLogger(const std::string& context, const std::string& to)
            : Logger(context)
            , out(to)
        {}
    ...
    loggers_[context_] = std::make_unique<FileLogger>(context_, recordTo_);
