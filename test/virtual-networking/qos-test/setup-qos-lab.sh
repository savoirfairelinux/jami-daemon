...
do_up() {
    ...
    # Configure peer-a namespace
    ip -n ${NS_A} link set lo up
    ip -n ${NS_A} addr add ${IP_A}/24 dev ${VETH_A}
    ip -n ${NS_A} link set ${VETH_A} up

    # Configure peer-b namespace
    ip -n ${NS_B} link set lo up
    ip -n ${NS_B} addr add ${IP_B}/24 dev ${VETH_B}
    ip -n ${NS_B} link set ${VETH_B} up

    # Configure bridge-side interfaces on host
    ip addr add 10.99.0.254/24 dev ${BR_A} 2>/dev/null || true
    ip link set ${BR_A} up
    ip link set ${BR_B} up
    ...
    # Route peer-a -> peer-b through host
    ip -n ${NS_A} route add default via 10.99.0.254
    # peer-b needs a route too — use a secondary address on BR_B
    ip addr add 10.99.0.253/24 dev ${BR_B} 2>/dev/null || true
    ip -n ${NS_B} route add default via 10.99.0.253

    # NAT-less forwarding (both peers can reach each other via host routing)
    iptables -A FORWARD -i ${BR_A} -o ${BR_B} -j ACCEPT 2>/dev/null || true
    iptables -A FORWARD -i ${BR_B} -o ${BR_A} -j ACCEPT 2>/dev/null || true

    echo "[+] Lab is up."
    echo "    Peer A: sudo ip netns exec ${NS_A} bash"
    echo "    Peer B: sudo ip netns exec ${NS_B} bash"
...
do_down() {
    ...
    iptables -D FORWARD -i ${BR_A} -o ${BR_B} -j ACCEPT 2>/dev/null || true
    iptables -D FORWARD -i ${BR_B} -o ${BR_A} -j ACCEPT 2>/dev/null || true
    cleanup
    echo "[+] Done."
}
