...
do_up() {
    ...
    cat /proc/sys/net/ipv4/ip_forward > /tmp/.qos_lab_ipfwd_saved
...
do_down() {
    ...
    cleanup
    # Restore ip_forward state
    if [[ -f /tmp/.qos_lab_ipfwd_saved ]]; then
        sysctl -w net.ipv4.ip_forward="$(cat /tmp/.qos_lab_ipfwd_saved)" >/dev/null
        rm -f /tmp/.qos_lab_ipfwd_saved
    fi
    echo "[+] Done."
}
