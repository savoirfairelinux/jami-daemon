...
do_up() {
    ...
    STATE_DIR="$(mktemp -d /run/qos-lab.XXXXXX)"
    chmod 700 "$STATE_DIR"
...
do_down() {
    ...
    local state_dir
    state_dir="$(ls -d /run/qos-lab.* 2>/dev/null | head -1)"
    if [[ -n "$state_dir" && -f "$state_dir/ipfwd" ]]; then
        sysctl -w net.ipv4.ip_forward="$(cat "$state_dir/ipfwd")" >/dev/null
        rm -rf "$state_dir"
    fi
    echo "[+] Done."
}
