...
#
# This script:
# 1. Assumes two Jami instances are already running in their namespaces
#    (peer-a and peer-b) with an active call between them
# 2. Applies network degradation profiles in sequence
# 3. Collects RTCP statistics (packet loss) from Jami logs
...
for profile in "${PROFILE_LIST[@]}"; do
    ...
    for ns in ${NS_A} ${NS_B}; do
        ...
        # Try common Jami log locations (use $HOME explicitly to avoid
        # tilde expansion issues when running under ip netns exec as root)
        for log_path in /tmp/jami-${ns}.log "${HOME}/.local/share/jami/"jami*.log; do
