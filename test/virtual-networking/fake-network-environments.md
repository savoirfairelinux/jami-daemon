...
An `--ipv6` flag switches to a ULA-addressed IPv6 topology instead:

```
      ┌──────────────┐ ┌──────────────┐      ┌─────────────────────
 │  lan  (Jami) │──────│  rtr    (NAT6+miniupnpd)  │──────│  wan  (peer) │
fd00:lan::2   fd00:lan::1 ←→ fd00:wan::2│      │ fd00:wan::1  ││  │
 └──────────────┘      └───────────────────────────┘      └──────────────┘
```
...
- `miniupnpd-nftables`, `upnpc` (miniupnpc client) installed
...
sudo ./setup-fake-upnp-network.sh            # IPv4 (default)
sudo ./setup-fake-upnp-network.sh --ipv6     # IPv6
...
1. **Verify clean state** — no pre-existing mappings:

   ```bash
   sudo ip netns exec lan upnpc -l
   ```
...
2. **Start Jami** inside the `lan` namespace:
   ...
   jami &    # or jami-qt, or however you launch the client
   ```
...
5. **Check Jami logs** for UPnP activity:
   ...
   ```bash
   grep -iE "IGD|UPnP|mapping.*performed|public address" ~/.local/share/jami/jami*.log
   ```
...
## Test Case 2: Port Selection — Reuse and Fallback

**Goal:** Confirm that dhtnet reuses an existing valid mapping on restart, and
falls back to a different external port when the preferred one is occupied.

### Part A: Reuse

1. **Start Jami** in the lan namespace. Wait for UPnP mapping to appear.

2. **Record the mapping** created by dhtnet:

   ```bash
   sudo ip netns exec lan upnpc -l
   ```

   Note the external port (e.g., `22345`).

3. **Stop Jami** (close it normally).

4. **Verify the mapping still exists** (miniupnpd keeps it until expiry):

   ```bash
   sudo ip netns exec lan upnpc -l
   ```

5. **Restart Jami** in the lan namespace.

6. **Check the mapping table again:**
...
- dhtnet reuses the **same external port** as before (no new mapping created, or
  the mapping points to the same ports).
- Logs show `successfully performed` without `failed` preceding it.

### Part B: Fallback

1. **Stop Jami.**

2. **Create a conflicting mapping** that occupies dhtnet's preferred external
   port. Use the external port noted in step 2:

   ```bash
   # Replace 22345 with the actual external port dhtnet used:
   sudo ip netns exec lan upnpc -e "blocker" -a 192.168.100.2 9999 22345 UDP
   ```

   This maps a dummy internal port (9999) to the external port dhtnet would prefer.

3. **Restart Jami** in the lan namespace.
...
- dhtnet created a mapping on a **different external port** (not 22345).
- The new mapping is still in the 20000–25000 range.
- Logs may show a mapping failure on the original port followed by a successful
  mapping on the fallback port.

#### Cleanup

```bash
# Remove the blocker mapping:
sudo ip netns exec lan upnpc -d 22345 UDP
```

---

## Test Case 3: WAN Reachability — DHT Node Access Through UPnP
