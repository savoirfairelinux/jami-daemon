...
if [[ "$use_ipv6" == true ]]; then
    ...
    ip -n lan addr add fd00:aa01::2/64 dev eth0
    ip -n lan link set eth0 up
    ip -n lan route add default via fd00:aa01::1

    ip -n rtr addr add fd00:aa01::1/64 dev veth-rtr-lan
    ip -n rtr link set veth-rtr-lan up
    ip -n rtr addr add fd00:bb01::2/64 dev veth-rtr-wan
    ip -n rtr link set veth-rtr-wan up

    ip -n wan addr add fd00:bb01::1/64 dev veth-wan
    ip -n wan link set veth-wan up
    ip -n wan route add default via fd00:bb01::2
