                           ...
                           const std::map<std::string, std::string>& reaction)
{
    ...
    auto index = indexFromMessageId_.at(messageId);
    ...
    auto existing = std::find_if(reactions.begin(), reactions.end(), [&](const auto& r) {
        return r.at("id") == reactionId;
    });
    assert(existing == reactions.end());
                             ...
                             const std::string& reactionId)
{
    ...
    auto it = std::find_if(reactions.begin(), reactions.end(), [&](const auto& r) { return r.at("id") == reactionId; });
