...
public:
    ConversationDST() {}
    ~ConversationDST() { libjami::fini(); }

    // Setup and teardown functions
    void connectSignals();
    bool waitForDeviceAnnouncements();
    void setUp(int numAccountsToSimulate);
    void tearDown();

    // Logging
    void eventLogger(const Event& event);
    void displaySummary(int numSimulatedAccounts);
    void displayGitLog();
    ...
    void generateEventSequence(int numAccountsToSimulate);
    ...
    // Unit testing
    UnitTest loadUnitTestConfig(const std::string& unitTestName);
    void saveAsUnitTestConfig();

    // Test runner
    void runCycles(unsigned numCycles);
