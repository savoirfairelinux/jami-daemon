...
struct DSTConfiguration
{
    ...
    DSTConfiguration(bool resetRepositories, bool saveTests, bool runTests, const std::string& testFolderName)
...
public:
    ...
    void connectSignals();
    bool waitForDeviceAnnouncements();
    void setUp(int numAccountsToSimulate = MAX_ACCOUNTS);
    void tearDown();
    ...
    // Test runner
    bool run(uint64_t seed, unsigned maxEvents);
...
private:
    ...
    DSTConfiguration dstConfig = DSTConfiguration(RESET_REPOS, SAVE_AS_UNIT_TESTS, IS_UNIT_TEST, UNIT_TEST_FOLDER_NAME);
