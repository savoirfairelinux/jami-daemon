...
ConversationDST::setUp(int numAccountsToSimulate)
{
    ...
    confHandlers.insert(libjami::exportable_callback<libjami::ConfigurationSignal::RegistrationStateChanged>(
        [&](const std::string& accountId, const std::string&, int, const std::string&) {
            for (auto& repoAcc : repositoryAccounts) {
                if (accountId == repoAcc.account->getAccountID()) {
                    repoAcc.identityLoaded = repoAcc.account->identity().second != nullptr;
                    break;
                }
            }
            cv.notify_one();
...
ConversationDST::triggerEvent(const Event& event, EventQueue* queue)
{
    ...
    case ConversationEvent::EDIT_MESSAGE: {
        ...
        if (queue) {
            ...
            if (rand01() < 0.3f) {
                auto editTime = event.timeOfOccurrence
                                + randDuration(std::chrono::milliseconds(1), std::chrono::milliseconds(5000));
                queue->emplace(event.instigatorAccountIndex,
                               event.instigatorAccountIndex,
    ...
    case ConversationEvent::ADD_REACTION: {
        ...
        if (queue) {
            ...
            if (rand01() < 0.3f) {
                auto removeTime = event.timeOfOccurrence
                                  + randDuration(std::chrono::milliseconds(1), std::chrono::milliseconds(5000));
                queue->emplace(event.instigatorAccountIndex,
                               event.instigatorAccountIndex,
    ...
    case ConversationEvent::CLONE: {
        ...
        if (queue) {
            ...
            if (rand01() < 0.1f) {
                auto leaveTime = event.timeOfOccurrence
                                 + randDuration(std::chrono::milliseconds(1), std::chrono::milliseconds(15000));
...
ConversationDST::generateEventSequence(unsigned maxEvents)
{
    ...
    eventWeights[static_cast<uint8_t>(ConversationEvent::CONNECT)] = 4;
 ...
 * If the removed member owns the repository, all calls are removed because the client leaves the
 * conversation.
 *
    ...
    for (auto it = commits.rbegin(); it != commits.rend(); ++it) {
        ...
        if (typeIt->second == CommitType::MEMBER) {
            ...
            if (actionIt != commit.end() && uriIt != commit.end()
                ...
                if (memberUri == repoAcc.account->getUsername()) {
                    // The account left the conversation, so we expect the client to no longer have any active calls.
                    activeCalls.clear();
                } else {
