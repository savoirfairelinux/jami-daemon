...
ConversationDST::validateEvent(const Event& event)
{
    ...
    case ConversationEvent::DELETE_FILE: {
        ...
        const auto& target = instigatorRepoAcc.client.getMessageAtChronologicalIndex(event.targetMessageIndex);
...
ConversationDST::triggerEvent(const Event& event, EventQueue* queue)
{
    ...
    case ConversationEvent::SEND_FILE: {
        ...
        if (queue) {
            ...
            if (deleteProbDist(gen_) < 0.3) {
                int targetIdx = instigatorAccount.client.getChronologicalIndex(commitID);
                std::uniform_int_distribution<> delayDist(5, 50);
                auto deleteTime = event.timeOfOccurrence + std::chrono::nanoseconds(delayDist(gen_));
    ...
    case ConversationEvent::DELETE_FILE: {
        assert(event.targetMessageIndex >= 0);
        const auto& targetMessage = instigatorAccount.client.getMessageAtChronologicalIndex(event.targetMessageIndex);
