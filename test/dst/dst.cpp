...
// TODO Avoid doing unncessary work when called multiple times
bool
ConversationDST::setUp(int numAccountsToSimulate)
{
        ...
        [&](const std::string& accountId, const std::string& conversationId, const libjami::SwarmMessage& message) {
            for (RepositoryAccount& repositoryAccount : repositoryAccounts) {
                if (accountId == repositoryAccount.account->getAccountID()) {
                    ...
                    repositoryAccount.client.onSwarmMessageReceived(accountId, conversationId, message);
                }
            }
            cv.notify_one();
        }));
    confHandlers.insert(libjami::exportable_callback<libjami::ConversationSignal::SwarmMessageUpdated>(
        [&](const std::string& accountId, const std::string& conversationId, const libjami::SwarmMessage& message) {
            for (RepositoryAccount& repositoryAccount : repositoryAccounts) {
                if (accountId == repositoryAccount.account->getAccountID()) {
                    repositoryAccount.client.onSwarmMessageUpdated(accountId, conversationId, message);
                }
            }
        }));
        ...
        [&](uint32_t requestId,
            ...
            for (RepositoryAccount& repositoryAccount : repositoryAccounts) {
                if (repositoryAccount.account->getAccountID() == accountId) {
                    ...
                    repositoryAccount.client.onSwarmLoaded(requestId, accountId, conversationId, messages);
                    break;
                }
            }
            cv.notify_one();
    ...
    for (RepositoryAccount& repositoryAccount : repositoryAccounts) {
        ...
        fmt::print("{} account #{} [{:<7}]: {}\n",
                   (accountIndex < numberOfExistingAccounts) ? "Loaded" : "Created",
                   accountIndex,
    ...
    if (instigatorAccountIndex != receivingAccountIndex) {
        ...
        for (int i = 0; i < numAccountsToSimulate_; ++i) {
            if (repositoryAccounts[i].repository != nullptr && i != instigatorAccountIndex) {
                auto accountSpecificTime = scheduledTime + (std::chrono::nanoseconds(adaptiveDist(gen_)) * (i + 1));
                queue.emplace(instigatorAccountIndex, i, gitOperation, accountSpecificTime);
            }
        }
    }
}
 ...
 */
void
ConversationDST::scheduleSideEffects(EventQueue& queue, const Event& event)
{
    switch (event.type) {
    case ConversationEvent::ADD_MEMBER:
    ...
    case ConversationEvent::CLONE:
        ...
        scheduleGitEvent(queue,
                         ConversationEvent::FETCH,
                         event.receivingAccountIndex, // Same indicies so that everyone fetches from the joiner
                         event.receivingAccountIndex,
                         event.timeOfOccurrence);
        break;
    default:
...
ConversationDST::triggerEvent(const Event& event, EventQueue* queue)
{
    ...
    case ConversationEvent::CONNECT: {
        ...
        if (queue) {
            ...
            // TODO Given the current implementation of scheduleGitEvent, all fetch operations
            // scheduled here will occur at the end of the test instead of at the scheduled times.
            scheduleGitEvent(*queue,
                             ConversationEvent::FETCH,
                             event.instigatorAccountIndex,
                             event.receivingAccountIndex,
    ...
    case ConversationEvent::ADD_MEMBER: {
        ...
        repositoryAccounts[event.instigatorAccountIndex].conversation->announce(commitID, true);
        break;
    }
    case ConversationEvent::SEND_MESSAGE: {
        msgCount++;
    ...
    case ConversationEvent::CLONE: {
        auto repo = ConversationRepository::cloneConversation(
        ...
        repositoryAccounts[event.receivingAccountIndex].createConversation(std::move(repo));
        break;
    }
    case ConversationEvent::FETCH: {
        auto commitId = repositoryAccounts[event.instigatorAccountIndex].repository->getHead();
        assert(!commitId.empty());
        auto repo = repositoryAccounts[event.receivingAccountIndex].repository.get();
        assert(repo != nullptr);
        ...
        // Fetch from peer
        auto deviceId = repositoryAccounts[event.instigatorAccountIndex].account->getAccountID();
        assert(repo->fetch(deviceId));

        assert(!repositoryAccounts[event.receivingAccountIndex].devicesWithPendingFetch.contains(deviceId));
        repositoryAccounts[event.receivingAccountIndex].devicesWithPendingFetch.insert(deviceId);
        if (queue) {
            scheduleGitEvent(*queue,
    ...
    case ConversationEvent::MERGE: {
        auto deviceId = repositoryAccounts[event.instigatorAccountIndex].account->getAccountID();
        assert(repositoryAccounts[event.receivingAccountIndex].devicesWithPendingFetch.contains(deviceId));
        repositoryAccounts[event.receivingAccountIndex].devicesWithPendingFetch.erase(deviceId);

        auto commits = repositoryAccounts[event.receivingAccountIndex].repository->mergeHistory(deviceId,
                                                                                                [](const std::string&) {
                                                                                                });
        if (!commits.empty()) {
            // Messages have been found. This announce function should add the messages into the respective RepositoryAccount
            repositoryAccounts[event.receivingAccountIndex].conversation->announce(commits, false);
        }
        break;
    }
    default:
...
ConversationDST::generateEventSequence(unsigned maxEvents)
{
    ...
    while (unvalidatedEvents.size() < maxEvents) {
        ...
        scheduleSideEffects(unvalidatedEvents, event);
    }
    size_t unvalidatedEventsCount = unvalidatedEvents.size();
    ...
    float rejectionRate = (static_cast<float>(invalidEventsCount) / unvalidatedEventsCount);
...
ConversationDST::runCycles(unsigned numCycles, unsigned maxEvents)
{
    ...
    for (unsigned i = 0; i < numCycles; ++i) {
        ...
        allTestsPassed &= run(seed, maxEvents);
