...
git_apply_args = ["apply", "--reject", "--ignore-whitespace", "--whitespace=fix"]
patch_args = ["-flp1", "-i"]
...
def getVSEnv(arch="x64", platform="", version=""):
    env_cmd = (
        'set path=%path:"=% && ' + getVSEnvCmd(arch, platform, version) + " && set"
    )
    p = subprocess.Popen(env_cmd, shell=True, stdout=subprocess.PIPE)
...
def make_plugin(pkg_info, force, sdk_version, toolset):
    ...
    if not pkg_up_to_date or current_version is None or force:
        root_logger.warning(
            "Building plugin with preferred sdk version %s and toolset %s",
            sdk_version,
            toolset,
        )
        ...
        cmake_script = (
            "cmake -DCMAKE_SYSTEM_VERSION="
            + sdk_version
            + " -G "
            + getCMakeGenerator(getLatestVSVersion())
            + cmake_defines
            + " -T "
            + toolset
            + " -S "
...
def make_daemon(pkg_info, force, sdk_version, toolset):
    cmake_script = (
        'cmake -DCMAKE_CONFIGURATION_TYPES="ReleaseLib_win32" -DCMAKE_SYSTEM_VERSION='
        + sdk_version
        + ' -DCMAKE_VS_PLATFORM_NAME="x64" -G '
        + getCMakeGenerator(getLatestVSVersion())
    ...
    root_logger.warning(
        "Building daemon with preferred sdk version %s and toolset %s",
        sdk_version,
        toolset,
    )
    env_set = "false" if pkg_info.get("with_env", "") == "" else "true"
...
def make(pkg_info, force, sdk_version, toolset, isPlugin):
    ...
    if not pkg_up_to_date or current_version is None or force:
        ...
        if os.path.exists(should_fetch and pkg_build_path):
...
def fetch_pkg(pkg_name, version, url, force):
    ...
    if not os.path.exists(archive_path):
        ...
        log.warning(archive_name + " already exists in the tarball/archive directory")
...
def extract_tar(pkg_build_path, name, path, pkg_name):
    with tarfile.open(path, "r", encoding="utf8", errors="ignore") as tarball:
        ...
        if tar_common_prefix == "":
            ...
            os.rename(contrib_build_dir + "\\" + tar_common_prefix, pkg_build_path)
        return True
    return False
...
def extract_zip(pkg_build_path, name, path):
    with zipfile.ZipFile(path, "r") as ziparchive:
        ...
        os.rename(contrib_build_dir + "\\" + zip_common_prefix, pkg_build_path)
        return True
    return False
...
def build(
    ...
    ops = (
        len(build_scripts)
        + len(project_paths)
        + len(pre_build_scripts)
        + len(post_build_scripts)
    )
...
def parse_args():
    ...
    ap.add_argument("-F", "--force", action="store_true", help="Force action")

    # invoke a build for the target and its dependencies
    ap.add_argument("-b", "--build", help="Build latest contrib")

    # invoke a fetch for the target and its dependencies (default true when building)
    ap.add_argument("-f", "--fetch", default="true", help="Fetch latest contrib")

    # apply patches for the target and its dependencies (default true when building)
    ap.add_argument("-a", "--apply", default="true", help="Apply patches for contrib")

    # clean the target and its dependencies
    ap.add_argument("-c", "--clean", help="Cleans out build directory for a contrib")

    ap.add_argument("-d", "--debug", default="DEBUG", help="Sets the logging level")
    ap.add_argument(
        "-i",
        "--indent",
        action="store_true",
    ...
    if os.path.exists(contrib_build_dir) and parsed_args.clean == "all":
        ...
        with open(pkg_json_file, encoding="utf8", errors="ignore") as json_file:
            ...
            if not parsed_args.plugin:
                ...
                dir_to_clean = os.path.join(exclude_dirs, pkg_info["name"])
                file_to_clean = os.path.join(exclude_dirs, "." + pkg_info["name"])

                if os.path.exists(dir_to_clean) or os.path.exists(file_to_clean):
                    log.warning("Removing build {}".format(dir_to_clean))
                    getSHrunner().exec_batch("rmdir", ["/s", "/q", dir_to_clean])
                    getSHrunner().exec_batch("del", ["/s", "/f", "/q", file_to_clean])
                else:
                    log.warning("No builds to remove")
            else:
                exclude_dirs = plugins_bin_dir
                files_to_clean = [os.path.join(exclude_dirs, "." + pkg_info["name"])]
                for name in glob.iglob(
                    os.path.join(exclude_dirs, "**", pkg_info["name"] + ".jpl"),
                    recursive=True,
                ):
                    files_to_clean.append(name)
                for name in files_to_clean:
                    getSHrunner().exec_batch("del", ["/s", "/f", "/q", name])
                getSHrunner().exec_batch(
                    "rmdir",
                    ["/s", "/q", os.path.join(plugins_dir, parsed_args.clean, "msvc")],
                )
...
def main():
    ...
    if parsed_args.purge:
        if os.path.exists(contrib_tmp_dir):
            # These could be archives of multiple types. (*.tar.*, *.tgz.*, *.zip.*)
            log.warning("Removing contrib tarballs " + contrib_tmp_dir)
            getSHrunner().exec_batch("rmdir", ["/s", "/q", contrib_tmp_dir])
        else:
            log.warning("No tarballs to remove")

    if parsed_args.clean:
        clean_build(parsed_args)
