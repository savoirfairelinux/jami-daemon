...
#endif
#ifdef ENABLE_CONNSTAT
        std::unique_ptr<dht::net::connstat::ConnectivityStatus> connstat_;
#endif
};
...
#endif
#ifdef ENABLE_CONNSTAT
    , connstat_(new dht::net::connstat::ConnectivityStatus)
#endif
{
...
#undef PJSIP_TRY
	...
	dht::net::connstat::ConnectivityStatus::connstat_event_cb cc_cb = [] (unsigned int event)
									{ DRing::connectivityChanged(event); };
        unsigned int e = 0;
        /* wireless association is not of interest */
        // e |= connstat_->addtopic(NEWLINK);
        // e |= pimpl_->connstat_->addtopic(dht::net::connstat::DELLINK);
        // e |= pimpl_->connstat_->addtopic(dht::net::connstat::NEWROUTE4);
        // e |= pimpl_->connstat_->addtopic(dht::net::connstat::NEWROUTE6);
        // e |= pimpl_->connstat_->addtopic(dht::net::connstat::NEWADDR4);
        e |= pimpl_->connstat_->addtopic(dht::net::connstat::NEWADDR);
        e |= pimpl_->connstat_->addtopic(dht::net::connstat::DELADDR);
        if (pimpl_->connstat_)
            pimpl_->connstat_->setTopicListener(cc_cb, e);
    } else
...
#ifdef ENABLE_CONNSTAT
dht::net::connstat::ConnectivityStatus&
