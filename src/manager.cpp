...
void
Manager::removeCall(Call& call) // TODO take shared_ptr to avoid to retransform
{
    ...
    conf->removeCall(call.getCallId());
    ...
    for (const auto& [key, callInfo] : infos.callInfo_) {
        ...
        for (const auto& [streamId, streamInfo] : callInfo.streams) {
            ...
            if (streamInfo.w && streamInfo.h && !streamInfo.videoMuted) {
                ...
                if (!accountId.empty() &&
                    currentSink &&
                    string_remove_suffix(uri, '@') == getAccount(accountId)->getUsername() &&
