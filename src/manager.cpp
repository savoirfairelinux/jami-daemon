...
check_rename(const std::string& old_dir, const std::string& new_dir)
{
    ...
    if (not fileutils::isDirectory(new_dir)) {
        ...
        boost::nowide::rename(old_dir.c_str(), new_dir.c_str());
    } else {
        for (const auto& file : fileutils::readDirectory(old_dir)) {
            ...
            if (fileutils::isDirectory(old_dest) and fileutils::isDirectory(new_dest)) {
                ...
                boost::nowide::rename(old_dest.c_str(), new_dest.c_str());
            }
        }
...
{
#ifndef RING_UWP
    char* envvar = boost::nowide::getenv(DHTLOGLEVEL);
    int level = 0;
...
{
#ifndef RING_UWP
    char* envvar = boost::nowide::getenv(SIPLOGLEVEL);
...
{
#ifndef RING_UWP
    char* envvar = boost::nowide::getenv("RING_TLS_LOGLEVEL");
