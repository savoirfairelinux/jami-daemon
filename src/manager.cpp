...
// Modified version of mute to test updateMediaStreams method
bool
Manager::muteMediaCall(const std::string& callId, const std::string& mediaType, bool mute)
{
    ...
    // Add the (video) media if not found
    if (not foundMedia and type == MediaType::MEDIA_VIDEO) {
        MediaAttribute newMedia(type, mute);
        newMedia.label_ = "main video";
        // newMedia.sourceUri_ = mute ? "" : getVideoManager().videoDeviceMonitor.getMRLForDefaultDevice();
        mediaList.emplace_back(newMedia);
    }

    // Apply
    return call->updateMediaStreams(mediaList);
}
#else
bool
