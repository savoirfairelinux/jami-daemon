...
check_rename(const std::filesystem::path& old_dir, const std::filesystem::path& new_dir)
{
    ...
    if (not std::filesystem::is_directory(new_dir)) {
        ...
        for (const auto& file : std::filesystem::directory_iterator(old_dir, ec)) {
            auto old_path = file.path();
            auto new_path = new_dir / old_path.filename();
            if (std::filesystem::is_directory(old_path)
                and std::filesystem::is_directory(new_path)) {
                check_rename(old_path, new_path);
            } else {
                JAMI_WARNING("Migrating {} to {}", old_dir, new_path);
                std::filesystem::rename(old_path, new_path, ec);
                if (ec)
                    JAMI_ERROR("Failed to rename {} to {}: {}", old_path, new_path, ec.message());
            }
        }
