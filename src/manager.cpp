                                                 ...
                                                 const std::vector<DRing::MediaMap>& remoteMediaList)
{
    ...
    // If it's a conference, accept any media change request. Account
    // video settings (enable/disable) will be handled by the call.
    if (auto conf = Manager::instance().getConferenceFromCallID(call->getCallId())) {
        JAMI_DBG("Conf [%s]: Answer to media change request", conf->getConfID().c_str());
        ...
        // We need to check before it's changed.
        auto mediaChanged = call->checkMediaChangeRequest(remoteMediaList);

        call->answerMediaChangeRequest(remoteMediaList);
        call->enterConference(call->getConfId());

        if (mediaChanged and conf->getState() == Conference::State::ACTIVE_ATTACHED) {
            // Need to re-attach the conf to update the mixer.
            conf->detachLocalParticipant();
            conf->attachLocalParticipant();
        }
