...
Manager::ManagerPimpl::bindCallToConference(Call& call, Conference& conf)
{
    ...
    if (state == "HOLD") {
        ...
        base_.offHoldCall(callId);
    } else if (state == "INCOMING") {
        conf.bindParticipant(callId);
        base_.answerCall(callId);
...
bool
Manager::answerCall(const std::string& callId)
{
    JAMI_INFO("Answer call %s", callId.c_str());

    bool result = true;

    auto call = getCallFromCallID(callId);
    if (!call) {
        JAMI_ERR("Call %s is NULL", callId.c_str());
        return false;
    }
...
void
Manager::removeConference(const std::string& accountId, const std::string& conferenceId)
{
    emitSignal<DRing::CallSignal::ConferenceRemoved>(conferenceId);

    if (auto account = getAccount(accountId)) {
        if (account->removeConference(conferenceId)) {
            JAMI_DBG("Conference %s removed successfully", conferenceId.c_str());
        } else {
...
Manager::getConferenceFromCallID(const std::string& callId) const
{
    if (auto call = getCallFromCallID(callId))
...
bool
Manager::addParticipant(const std::string& accountId, const std::string& callId, const std::string& conferenceId)
{
    if (auto account = getAccount(accountId)) {
...
Manager::addParticipant(Call& call, Conference& conference)
{
    ...
    if (call.getConfId() == conference.getConfID()) {
        JAMI_WARN("Call %s already participant of conf %s", call.getCallId().c_str(), conference.getConfID().c_str());
        return true;
    }

    JAMI_DBG("Add participant %s to conference %s", call.getCallId().c_str(), conference.getConfID().c_str());
...
bool
Manager::addMainParticipant(const std::string& conferenceId)
{
    JAMI_INFO("Add main participant to conference %s", conferenceId.c_str());

    if (auto conf = getConferenceFromID(conferenceId)) {
        pimpl_->addMainParticipant(*conf);
...
bool
Manager::joinParticipant(const std::string& accountId, const std::string& callId1, const std::string& callId2, bool attached)
{
...
Manager::createConfFromParticipantList(const std::string& accountId, const std::vector<std::string>& participantList)
{
    ...
    for (const auto& numberaccount : participantList) {
        ...
        // Create call
        auto callId = outgoingCall(account, tostr, conf->getConfID());
        if (callId.empty())
            continue;
...
bool
Manager::detachLocalParticipant(const std::string& confId)
{
    JAMI_INFO("Detach local participant from conference %s", confId.c_str());

    if (auto conf = getConferenceFromID(confId.empty() ? getCurrentCallId() : confId)) {
        conf->detach();
        emitSignal<DRing::CallSignal::ConferenceChanged>(conf->getConfID(), conf->getStateStr());
        pimpl_->unsetCurrentCall();
        return true;
...
bool
Manager::joinConference(const std::string& accountId, const std::string& confId1, const std::string& confId2)
{
    auto account = getAccount(accountId);
    if (not account) {
...
Manager::ManagerPimpl::processIncomingCall(Call& incomCall, const std::string& accountId)
{
    ...
    if (account->isRendezVous()) {
        dht::ThreadPool::io().run([this, account, incomCall = incomCall.shared_from_this()] {
            base_.answerCall(incomCall->getCallId());
            for (const auto& call : base_.callFactory.getAllCalls()) {
                if (call->getState() != Call::CallState::ACTIVE)
                    continue;
                if (call->getAccount().lock() == account && call != incomCall) {
                    if (call->getConfId().empty()) {
                        base_.joinParticipant(account->getAccountID(), incomCall->getCallId(), call->getCallId(), false);
                    } else if (auto conf = account->getConference(call->getConfId())) {
                        base_.addParticipant(*call, *conf);
                    }
                    return;
                }
            }
        ...
        if (currentCall->getAccountId() == account->getAccountID()
            ...
            if (answerToCall) {
                runOnMainThread([currentCallID = currentCall->getCallId(), incomCallId] {
