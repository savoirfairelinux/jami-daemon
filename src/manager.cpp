...
Manager::ManagerPimpl::bindCallToConference(Call& call, Conference& conf)
{
    ...
    conf.add(callId);
    call.setConference(conf.shared_from_this());
                      ...
                      const std::map<std::string, std::string>& volatileCallDetails)
{
    ...
    try {
        if (auto call = newOutgoingCall(trim(to), account_id, volatileCallDetails)) {
            ...
            call->setConference(std::move(conference));
...
bool
Manager::answerCall(const std::string& accountId, const std::string& callId, const std::vector<DRing::MediaMap>& mediaList)
{
...
Manager::hangupConference(const std::string& accountId, const std::string& confId)
{
    if (auto account = getAccount(accountId)) {
        if (auto conference = account->getConference(confId)) {
            return hangupConference(*conference);
        } else {
...
bool
Manager::hangupConference(Conference& conference)
{
...
{
    emitSignal<DRing::CallSignal::ConferenceRemoved>(accountId, conferenceId);
...
Manager::holdConference(const std::string& accountId, const std::string& confId)
{
    ...
    if (const auto account = getAccount(accountId)) {
        if (auto conf = account->getConference(confId)) {
            conf->detach();
            emitSignal<DRing::CallSignal::ConferenceChanged>(accountId, conf->getConfId(), conf->getStateStr());
            return true;
        }
    }
    return false;
...
{
    conf.attach();
    emitSignal<DRing::CallSignal::ConferenceChanged>(conf.getAccountId(), conf.getConfId(), conf.getStateStr());
...
Manager::ManagerPimpl::processIncomingCall(Call& incomCall, const std::string& accountId)
{
    ...
    if (account->isRendezVous()) {
        ...
        dht::ThreadPool::io().run([this, incomCall = incomCall.shared_from_this()] { base_.answerCall(*incomCall); });
        ...
        if (currentCall->getAccountId() == account->getAccountID()
            ...
            if (answerToCall) {
                runOnMainThread([currentCallID = currentCall->getCallId(), incomCall = incomCall.shared_from_this()] {
