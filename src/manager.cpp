...
bool
Manager::requestMediaUpdate(const std::string& callID, const std::vector<DRing::MediaMap>& mediaList)
{
    ...
    return call->requestMediaUpdate(mediaAttrList);
}
                         ...
                         const std::vector<DRing::MediaMap>& mediaList)
{
    ...
    auto const& mediaAttrList = MediaAttribute::parseMediaList(mediaList);
    return account->newOutgoingCall(toUrl, mediaAttrList);
}
