...
{
    JAMI_ERROR("@@@ bindCall MODERATOR: {}", isModerator);
...
{
    JAMI_ERROR("@@@ isModerator {}", uri);
    for (auto& [key, value]: callInfo_)
        if (key.first == uri) {
    JAMI_ERROR("@@@ RET {}", value.isModerator);

            return value.isModerator;
        }
    JAMI_ERROR("@@@ RET FALSE");
    return false;
}
