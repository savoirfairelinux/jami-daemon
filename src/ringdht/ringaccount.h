...
class RingAccount : public SIPAccountBase {
    ...
    private:
        ...
        std::unique_ptr<IceTransaction> newDataTransaction(const std::string&, bool,
                                                           IceDataCandidates&& remote_ice={});

        bool initDataTransaction(const std::string&);
        void cancelPendingDataTransaction(const std::string&, DRing::DataTransferCode);
        ...
        std::list<std::shared_ptr<FileSender>> pendingFileSenderMap_;
};
