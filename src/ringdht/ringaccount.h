...
class RingAccount : public SIPAccountBase {
    ...
    private:
        ...
        std::mutex cnxMutex_ {}; // protect following maps
        std::map<std::string, std::shared_ptr<DataConnection>> dataConnectionMap_ {};

        std::shared_ptr<DataConnection> getDataConnection(const std::string&);
        std::shared_ptr<DataConnection> newDataConnection(const std::string&);
};
