...
class RingAccount : public SIPAccountBase {
    ...
    private:
         ...
         */
        std::pair<std::vector<uint8_t>, dht::InfoHash> computeKeys(const std::string& password, const std::string& pin);
        ...
        std::vector<uint8_t> makeArchive(const ArchiveContent& content);
        void saveArchive(const ArchiveContent& content, const std::string& pwd);
        ArchiveContent readArchive(const std::string& pwd);
        ArchiveContent loadArchive(const std::vector<uint8_t>& data);
