...
class RingAccount : public SIPAccountBase {
    ...
    private:
        ...
        std::shared_ptr<PeerConnection> getPeerConnection(const std::string& peer_id);
        void registerPeerConnection(std::shared_ptr<PeerConnection> cnx);
        std::shared_ptr<PeerConnection> makeMasterPeerConnection(const std::string& peer_id);
        std::shared_ptr<PeerConnection> makeSlavePeerConnection(const std::string& peer_id,
                                                                IceDataCandidates&& msg);
        void schedulePendingPeerConnectionTask();

        std::mutex peerConnectionMutex_ {};
        std::list<std::shared_ptr<PeerConnection>> pendingPeerConnectionList_ {};
        std::map<std::string, std::shared_ptr<PeerConnection>> peerConnectionMap_ {};
        ...
        //std::unique_ptr<SecureIceTransport> newSecureIceTransport(const std::string&, bool,
