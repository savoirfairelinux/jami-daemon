...
public:
    ...
    PeerConnection(const RingAccount& account, const std::string& pid, dht::Value::Id tid,
        ...
        , tid(tid)
        , tls_() {
        RING_DBG("[SIT:%lx] ctor, peer %s", tid, peer_id.c_str());
    }
    ...
    PeerConnection(const RingAccount& account, const std::string& pid, dht::Value::Id tid,
        ...
        : PeerConnection(account, pid, tid, id, dh_params)
        , remote_candidates(std::move(remote_cands)) {}

    ~PeerConnection() {
        RING_DBG("[SIT:%lx] dtor, peer %s", tid, peer_id.c_str());
...
private:
    ...
    void onStateCreated();
    void onStateInitIce();
    void onStateNegotiateIce();
    void onStateInitTls();
    void onStateHandshakeTls();
    void onConnected();
    void onFailure();
    ...
    try {
        ...
        dht_.listen<IceDataCandidates>(
            ...
            [shared] (IceDataCandidates&& msg) {
                if (msg.id & 1)
                    shared->onDataTransactionReply(std::move(msg));
                else
...
RingAccount::getPeerConnection(const std::string& peer_id)
{
    std::lock_guard<std::mutex> lock(peerConnectionListMutex_);
    auto item = peerConnectionMap_.find(peer_id);
    if (item != peerConnectionMap_.cend())
        return item->second;
    return {};
}

void
RingAccount::schedulePendingPeerConnectionTask()
{
    // TODO
}

void
RingAccount::registerPeerConnection(std::shared_ptr<PeerConnection> cnx)
{
    std::lock_guard<std::mutex> lk {peerConnectionMutex_};
    pendingPeerConnectionList.push_back(cnx);
    schedulePendingPeerConnectionTask();
}

std::shared_ptr<PeerConnection>
RingAccount::makeMasterPeerConnection(const std::string& peer_id)
{
    auto cnx = std::make_shared<PeerConnection>(peer_id);
    registerPeerConnection(cnx);
    return cnx;
}

std::shared_ptr<PeerConnection>
RingAccount::makeSlavePeerConnection(const std::string& peer_id, IceDataCandidates&& msg)
{
    auto cnx = std::make_shared<PeerConnection>(peer_id, msg);
    registerPeerConnection(cnx);
    return cnx;
}
...
RingAccount::onDataTransactionReply(const IceDataCandidates&& msg)
{
    ...
    const auto tid = msg.tid & ~1ul;

    std::lock_guard<std::mutex> lk(peerConnectionMutex_);
    for (auto& cnx : pendingPeerConnectionList_) {
        if (cnx->peerId == peer_id and cnx.tid == tid) {
            RING_DBG("[DHT:%s] Data cnx reply 0x%lx from %s:\n%s", getAccountID().c_str(), tid,
                     peer_id.c_str(), std::string(std::begin(msg.ice_data),
                                                  std::end(msg.ice_data)).c_str());
            cnx->onRemoteInfo(std::move(msg));
            return;
        }
    }
}
                                   ...
                                   IceDataCandidates&& remote_ice)
{
    ...
    auto trx = std::unique_ptr<SecureIceTransport>(
        new SecureIceTransport {getAccountID(), peer_id, id, std::move(remote_ice)});
    return std::move(trx);
}
...
std::shared_ptr<PeerConnection>
RingAccount::obtainDataConnection(const std::string& peer_id)
{
    std::lock_guard<std::mutex> lk {peerConnectionMapMutex_};
    auto it = peerConnectionMap_.find(peer_id);
    if (it == peerConnectionMap_.cend())
        return makeMasterPeerConnection(peer_id);
    return *it;
}
...
{
    // throw std::invalid_argument if uri is not a recognisable RingID
    ...
    return FTPControler::append_stream(obtainDataConnection(peer_id), stream);
}
...
void
PeerConnection::onStateCreated()
{}

void
PeerConnection::onStateInitIce()
{}

void
PeerConnection::onStateNegotiateIce()
{}

void
PeerConnection::onStateInitTls()
{}

void
PeerConnection::onStateHandshakeTls()
{}
...
{
    switch (state_) {
        case State::CREATED: onStateCreated(); break;
        case State::INIT_ICE: onStateInitIce(); break;
        case State::NEGOTIATE_ICE: onStateNegotiateIce(); break;
        case State::INIT_TLS: onStateInitTls(); break;
        case State::HANDSHAKE_TLS: onStateHandshakeTls(); break;
        case State::CONNECTED: onConnected(); break;
        case State::FAILURE: onFailure(); break;
    }
}
...
void
SecureIceTransport::onTlsStateChange(tls::TlsSessionState state)
{
    if (state == tls::TlsSessionState::ESTABLISHED) {
        RING_DBG("[sice:%s] TLS connected", peer_id.c_str());
        std::array<uint8_t, 3000> packet;
        tls_->async_send(packet.data(), packet.size(), [](ssize_t s){ RING_WARN("tx %zd", s); });
    } else if (state == tls::TlsSessionState::SHUTDOWN) {
        RING_WARN("[sice:%s] TLS shutdown", peer_id.c_str());
    }
}

// - DO NOT BLOCK - (Called in TlsSession thread)
void
SecureIceTransport::onRxData(std::vector<uint8_t>&& buf)
{
    RING_ERR("rx: %zu", buf.size());
}
...
int
SecureIceTransport::send(const void* buf, std::size_t size,
