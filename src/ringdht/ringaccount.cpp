...
RingAccount::removeDataConnection(const std::string& peer_id)
{
    ...
    if (item == std::end(dataConnectionMap_))
        return;
    // Do not release our shared_ptr token now, we may be called inside the ICE loop itself
    // and cause a deadlock exception if the ICE transport is destroyed.
    zombieDataConnectionList_.push_back(item->second);
                                   ...
                                   dht::IceCandidates&& remote_ice, bool initiator)
{
    ...
    const dht::Value::Id msg_id = initiator ? 0 : 1; // 0 = request, 1 = reply, other values are reserved
    ...
    dht::Value value {dht::IceCandidates(msg_id, candidates)};
...
RingAccount::onIncomingDataChannel(dht::IceCandidates&& msg)
{
    ...
    if (msg.id == 0) {
        RING_DBG("[DHT:%s] ICE data request from peer %s:\n%s", getAccountID().c_str(),
        ...
        RING_DBG("[DHT:%s] ICE data reply from peer %s:\n%s", getAccountID().c_str(),
                 peer_id.c_str(), std::string(std::begin(ice_data), std::end(ice_data)).c_str());
...
{
    {
        ...
        zombieDataConnectionList_.clear();
    }
