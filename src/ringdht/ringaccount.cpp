...
RingAccount::removeDataConnection(const std::string& peer_id)
{
    ...
    // TODO: stop related data transfers first
    zombieDataConnectionQueue_.push(item.second);
                                   ...
                                   dht::IceCandidates&& remote_ice, bool initiator)
{
    ...
    auto value = std::make_shared<dht::Value>(dht::IceCandidates(msg_id, candidates));
