...
struct IceTransaction {
    IceTransaction() : peer_id(), id(0), tls_() {}
    IceTransaction(const std::string& pid, dht::Value::Id tid, IceDataCandidates&& remote_cands)
        : peer_id(pid), id(tid), remote_candidates(remote_cands), tls_() {}
...
IceTransaction::onTlsStateChange(tls::TlsSessionState state)
{
    if (state == tls::TlsSessionState::ESTABLISHED)
        RING_WARN("[data:%s] TLS established", peer_id.c_str());
...
IceTransaction::onVerifyCertificate(gnutls_session_t session)
{
    ...
    try {
        ...
        RING_ERR("[peer:%s] TLS certificate check exception: %s", peer_id.c_str(), e.what());
        return GNUTLS_E_CERTIFICATE_ERROR;
    }
