                      ...
                      UNUSED const std::string& name)
{
    ...
    bool do_init = false;
    std::shared_ptr<SecureIceTransport> sit;
    {
        std::lock_guard<std::mutex> lock(sitMutex_);
        auto item = establishedSITMap_.find(peer_id);
        if (item == establishedSITMap_.cend()) {
            ...
            if (item == pendingSITMap_.cend()) {
                if (auto sit = newSecureIceTransport(peer_id, true)) {
                    sit = pendingSITMap_.emplace(peer_id, std::move(sit)).first->second;
                    do_init = true;
                } else {
                    return {};
                }
    ...
    auto ftx = FileTransfer::newFileTransfer(sit->cid, name, std::move(stream));
    return ftx->getId();
}
...
{
    if (auto ftx = FileTransfer::get(cid)) {
        if (state == tls::TlsSessionState::ESTABLISHED)
            ftx->setTransport(*this);
