...
RingAccount::onIncomingIceDataMsg(IceDataCandidates&& msg)
{
    ...
    if (is_request) {
        ...
        runOnMainThread([acc, peer_id, msg]() mutable {
                bool cancel;
                {
                    std::lock_guard<std::mutex> lock(acc->dataMutex_);
                    cancel  = acc->hasRunningDataConnection(peer_id);
                    cancel |= acc->hasPendingDataConnection(peer_id) < msg.timestamp;
                }
                // An established data connection exists, consider that the peer want to make a new one
                if (cancel)
                    acc->cancelDataTransaction(peer_id);
                acc->newDataTransaction(msg.from.toString(), false, std::move(msg)); });
    } else {
...
RingAccount::hasPendingDataConnection(const std::string& peer_id)
{
    ...
    if (item == std::end(pendingDataTransactionMap_))
        return {};
    return item->second->remote_candidates.timestamp;
}
                                ...
                                IceDataCandidates&& remote_ice)
{
    ...
    if (is_initiator) {
        std::lock_guard<std::mutex> lock(dataMutex_);
        if (hasRunningDataConnection(peer_id) or hasPendingDataConnection(peer_id).count() != 0) {
