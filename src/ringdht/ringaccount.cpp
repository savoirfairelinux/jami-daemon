...
RingAccount::doRegister_()
{
    try {
        ...
        RING_DBG("[DHT:%s] callto key: %s", getAccountID().c_str(), inboxKey.toString().c_str());
                                   ...
                                   dht::IceCandidates&& remote_ice, bool initiator)
{
    ...
    dht_.putEncrypted(
        ...
        [acc, peer_id, key, value](bool done) {
            if (!done) {
                RING_ERR("[Peer:%s] can't put data ICE descriptor on DHT", peer_id.c_str());
                emitSignal<DRing::ConfigurationSignal::DataConnectionStatus>(acc->getAccountID(), peer_id, "SYS_FAILURE");
                acc->removeDataConnection(peer_id);
            } else
                RING_DBG("[Peer:%s] data ICE descriptor sent", peer_id.c_str());

            acc->dht_.cancelPut(key, value->id);
        });
...
{
    if (ice.isFailed()) {
        RING_DBG("[Peer:%s] data transport negotiated (ice@%p)", peer_id.c_str(), &ice);
        emitSignal<DRing::ConfigurationSignal::DataConnectionStatus>(getAccountID(), peer_id, "CONNECTED");
    } else {
        RING_ERR("[Peer:%s] data transport negotiation failed (ice@%p)", peer_id.c_str(), &ice);
