...
RingAccount::addDevice(const std::string& password)
{
    ...
    std::vector<uint8_t> salt_key {pin_str.begin(), pin_str.end()};
    auto key = dht::crypto::stretchKey(password, salt_key);
    ...
    auto loc = dht::InfoHash::get(dht::crypto::stretchKey(password, salt_loc));

    auto archive = makeArchive(readArchive(password));
    dht_.put(loc, archive, [](bool ok){
        RING_WARN("Done publishing account archive: %s", ok ? "success" : "failure");
    });
    RING_WARN("Adding new device with PIN: %s at %s (size %zu)", pin_str.c_str(), loc.toString().c_str(), archive.size());
        ...
        if (hasSignedReceipt()) {
            ...
            if (archive_password.empty()) {
                ...
                if (not archive_pin.empty()) {
                    ...
                    // Generate key for archive encryption, using PIN as the salt
                    std::vector<uint8_t> salt_key {archive_pin.begin(), archive_pin.end()};
                    auto key = dht::crypto::stretchKey(archive_password, salt_key);

                    // Generate public storage location, using SHA1(PIN) as the salt
                    auto pin_h = dht::InfoHash::get(archive_pin);
                    std::vector<uint8_t> salt_loc {pin_h.begin(), pin_h.end()};
                    auto loc = dht::InfoHash::get(dht::crypto::stretchKey(archive_password, salt_loc));
                    ...
                    dht_.get(loc, [=](std::shared_ptr<dht::Value> val) {
                        ...
                        std::vector<uint8_t> decrypted = dht::crypto::aesDecrypt(val->data, key);
                        try {
