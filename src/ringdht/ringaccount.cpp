...
*/
    ...
    shared_this->dht_.get<DeviceAnnouncement>(toH, [=](DeviceAnnouncement&& dev) {
        ...
        if (auto call = weak_call.lock()) {
            ...
            call->subcalls.emplace(dev_call);
...
void
RingAccount::loadAccount(const std::string& archive_password)
{
    loadIdentity();
...
RingAccount::doRegister_()
{
    try {
        ...
        callKey_ = dht::InfoHash::get("callto:"+ringAccountId_);
        RING_DBG("Listening on callto:%s : %s", ringAccountId_.c_str(), callKey_.toString().c_str());
...
RingAccount::matches(const std::string &userName, const std::string &server) const
{
    ...
    if (userName == ringAccountId_ || server == ringAccountId_) {
