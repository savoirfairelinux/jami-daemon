    ...
    runOnMainThread([=]() {
        ...
        dht_.get(loc, [=](std::shared_ptr<dht::Value> val) {
            ...
            *found = true;
            ...
            RING_WARN("Done looking for account archive on DHT: %s", (ok and *found) ? "success" : "failure");
            if (not *found) {
                if (auto this_ = w.lock()) {
                    this_->setRegistrationState(RegistrationState::ERROR_GENERIC);
                    runOnMainThread([=]() {
            ...
            dht_.listen<DeviceAnnouncement>(h, [shared](DeviceAnnouncement&& dev) {
                shared->findCertificate(dev.dev, [shared](const std::shared_ptr<dht::crypto::Certificate> crt) {
                    ...
                    if (this_.knownDevices_.emplace(crt->getId(), crt).second) {
                        ...
                        devices.reserve(this_.knownDevices_.size());
                        for (const auto& d : this_.knownDevices_)
                            devices.emplace_back(d.first.toString());
                        emitSignal<DRing::ConfigurationSignal::KnownDevicesChanged>(this_.getAccountID(), devices);
                    }
                });
                return true;
            });
