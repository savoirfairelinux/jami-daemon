...
RingAccount::lookupName(const std::string& name)
{
    ...
    nameDir_.nameLookup(name, [acc,name](const std::string& result, NameDirectory::Response response) {
...
{
    auto acc = getAccountID();
    nameDir_.addrLookup(addr, [acc,addr](const std::string& result, NameDirectory::Response response) {
        if (response == NameDirectory::Response::found) {
