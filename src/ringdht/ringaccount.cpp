...
#if HAVE_RINGNS
    out << YAML::Key << DRing::Account::ConfProperties::RingNS::HOST << YAML::Value <<  nameDir_.get().getServer();
#endif
...
void RingAccount::unserialize(const YAML::Node &node)
{
    ...
    parseValue(node, DRing::Account::ConfProperties::RingNS::HOST, ringns_server);
...
#if HAVE_RINGNS
    std::string ringns_server;
    parseString(details, DRing::Account::ConfProperties::RingNS::HOST,     ringns_server);
    nameDir_ = NameDirectory::instance(ringns_server);
#endif
...
#if HAVE_RINGNS
    a.emplace(DRing::Account::ConfProperties::RingNS::HOST,                  nameDir_.get().getServer());
#endif
...
#if HAVE_RINGNS
    auto from_acc_id = from_cert ? (from_cert->issuer ? from_cert->issuer->getId().toString() : from_cert->getId().toString()) : from;
    nameDir_.get().lookupAddress(from_acc_id, [weak_call](const std::string& result, const NameDirectory::Response& response){
        if (response == NameDirectory::Response::found)
