...
RingAccount::addDevice(const std::string& password)
{
    ...
    auto pin = dis(rand_);
    RING_ERR("Adding new device with PIN: %u", pin);

    auto pin_str = ring::to_string(pin);
    auto archive = readArchive(password);
    dht_.put(pin_str, dht::crypto::aesEncrypt(makeArchive(archive), password + pin_str));
