...
void
RingAccount::cancelPendingDataTransaction(const std::string& peer_id, DRing::DataConnectionStatus status)
{
    std::lock_guard<std::mutex> lock(dataMutex_);
    emitDataCnxStatus(getAccountID(), peer_id, status);
...
RingAccount::onDataIceNegoSuccess(const std::string& peer_id)
{
    // Move pending transaction to running transaction list
    {
        ...
        try {
            ...
            RING_ERR("[DHT:%s] Fail to create TLS session on peer %s",
                     getAccountID().c_str(), peer_id.c_str());
            cancelPendingDataTransaction(peer_id);
            return;
        }
