...
RingAccount::newOutgoingCall(const std::string& toUrl)
{
    ...
    manager.addTask([=] {
        ...
        auto localCandidates = ice->getLocalAttributesAndCandidates();

        call->setState(Call::ConnectionState::TRYING);
        shared_this->dht_.putEncrypted(
            callkey, toH,
            dht::Value {
                dht::IceCandidates(localCandidates),
                callvid
            },
...
RingAccount::incomingCall(const dht::IceCandidates& msg)
{
    ...
    auto localCandidates = ice->getLocalAttributesAndCandidates();
    std::string foo = "Sffffffff 1 UDP 1862270975 208.88.110.45 1000 typ srflx\n";
    localCandidates.insert(localCandidates.end(), foo.begin(), foo.end());
