    ...
    try {
        ...
        dht_.listen<dht::IceCandidates>(
            ...
            [shared] (dht::IceCandidates&& msg) {
                ...
                if (trustStatus == DRing::Certificate::Status::BANNED) {
                ...
                if (not this_.dhtPublicInCalls_ and trustStatus != DRing::Certificate::Status::ALLOWED) {
...
RingAccount::acceptTrustRequest(const std::string& from)
{
    ...
    for (auto i = trustRequests_.begin(); i != trustRequests_.end(); ++i) {
        if (i->from == f) {
            trust_.setCertificateStatus(from, DRing::Certificate::Status::ALLOWED);
