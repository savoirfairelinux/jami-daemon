...
inline static int lenght(seqnum_t seq1, seqnum_t seq2) {
...
public:
    ...
    const DRing::DataConnectionId cid;
...
private:
    ...
    std::shared_ptr<DataConnection> dataConnection_ {};
    ...
    // Reliable stream members
    SeqNum::seqnum_t initialSeq_;
    SeqNum::seqnum_t txSeq_;
    ...
    std::size_t rxPkt_ {0};
    std::size_t rxPktTotal_ {0};
    std::size_t lossPkt_ {0};
    std::size_t lossPktTotal_ {0};
    std::size_t rxBytes_ {0};

    ssize_t lastTxResult_ {GNUTLS_E_SUCCESS};
    std::mutex txMutex_ {};
    std::condition_variable txCv_ {};

    void idleTask(void);
    ssize_t sendPacket(DataConnection::Packet&);
};
                      ...
                      UNUSED const std::string& name)
{
    ...
    // Find/create peer_id related SecureIceTransport
    DRing::DataConnectionId cid {0};
    {
        ...
        if (item == establishedSITMap_.cend()) {
            ...
            if (item == pendingSITMap_.cend()) {
                if (auto sit = newSecureIceTransport(peer_id, true)) {
                    cid = pendingSITMap_.emplace(peer_id, std::move(sit)).first->second->cid;
                } else {
                    return {};
                }
            } else {
...
SecureIceTransport::SecureIceTransport(RingAccount& account, const std::string& pid,
                                       dht::Value::Id tid, IceDataCandidates&& remote_cands)
    : cid(get_unique_connection_id())
    , account(account)
    , peer_id(pid)
    , tid(tid)
    ...
    , statSync_(clock::now())
{
    ...
    RING_DBG("[SIT:%lx] created (tid=%lx, iseq=%08x) for peer %s", cid, tid, initialSeq_,
             peer_id.c_str());
}

SecureIceTransport::~SecureIceTransport()
{
    RING_DBG("[SIT:%lx] destruction", cid);
    ...
    if (transport) {
        auto tr = transport;
        runOnMainThread([tr]() mutable { RING_WARN("reset ice %p", tr.get()); tr.reset(); });
    }
}

void
SecureIceTransport::idleTask(void)
{
    //auto data = DataConnection::Packet::vec(maxPayload_);
    //DataConnection::Packet ping(data);
    //sendPacket(ping);

    {
        std::lock_guard<std::mutex> lk {statMutex_};
        auto now = clock::now();
        float delta = std::chrono::duration_cast<std::chrono::seconds>(now - statSync_).count();
        if (delta > 2) {
            RING_WARN("Pkt: %.1f/s, Loss: %.1f/s, Bytes: %.1f/s",
                      rxPkt_ / delta, lossPkt_ / delta, rxBytes_ / delta);
            statSync_ = now;
            rxPkt_ = lossPkt_ = rxBytes_ = 0;
        }
    }
}

ssize_t
SecureIceTransport::sendPacket(DataConnection::Packet& pkt)
{
    auto& h = pkt.getHeader();
    h.seq = txSeq_;
    ...
    if (txCv_.wait_for(lk, std::chrono::milliseconds(500)) == std::cv_status::timeout) {
        RING_ERR("[SIT:%lx] send timeout (overloaded ICE thread?)", cid);
        throw std::runtime_error("ftp send timeout");
    }

    if (gnutls_error_is_fatal(lastTxResult_)) {
        RING_ERR("[SIT:%lx] send failed (fatal TLS error)", cid);
        return -1;
    }

    return lastTxResult_;
}
...
SecureIceTransport::onTlsStateChange(tls::TlsSessionState state)
{
    if (state == tls::TlsSessionState::ESTABLISHED) {
        maxPayload_ = tls_->getMaxPayload() - sizeof(DataConnection::PacketHeader);
        RING_WARN("[FTP] maxPayload=%zu", maxPayload_);
...
{
    uint8_t* data = buf.data() + sizeof(DataConnection::PacketHeader);
    std::size_t data_len = buf.size() - sizeof(DataConnection::PacketHeader);

    if (buf.size() < sizeof(DataConnection::PacketHeader)) {
        RING_ERR("too short (get %zub, wait %zub)", buf.size(),
                 sizeof(DataConnection::PacketHeader));
        return;
    }

    //RING_WARN("len %zu", data_len);

    auto pkt = reinterpret_cast<DataConnection::PacketHeader*>(buf.data());
    if (pkt->version > 0) {
        RING_WARN("bad version (%u)", pkt->version);
        // TODO: statistics
        return;
    }

    // sync stage
    if (peerInitSeqNum_ == SeqNum::INVALID) {
        peerInitSeqNum_ = pkt->seq;
        lastRxSeqNum_ = SeqNum::decrease(peerInitSeqNum_);
        lastRxAck_ = peerInitSeqNum_;
        RING_WARN("peer setup: %08lx", peerInitSeqNum_);
    }
    ...
    // TODO
    //if (pkt->ack)
    //    updateAckInfo(ntohl(pkt->ack));

    // check sequence number validity
    auto offset = SeqNum::offset(lastRxAck_, pkt->seq);
    if ((offset < 0) /*|| (offset >= m_pRcvBuffer->getAvailBufSize())*/) {
        RING_WARN("neg off (seq=%08lx, lack=%08lx)", pkt->seq, lastRxAck_);
        return;
    }
    ...
    // loss detection
    auto loss_cnt = SeqNum::cmp(pkt->seq, SeqNum::increase(lastRxSeqNum_));
    if (loss_cnt > 0) {
        RING_WARN("loss: %d", loss_cnt);
        // Insert to loss list and report to peer

        {
            std::lock_guard<std::mutex> lk {statMutex_};
            lossPktTotal_ += loss_cnt;
            lossPkt_ += loss_cnt;
        }
    }

    // update largest rx sequence number or remove retransmitted packet from loss list
    if (SeqNum::cmp(pkt->seq, lastRxSeqNum_) > 0) {
        lastRxSeqNum_ = pkt->seq;
    } else
        ; // TODO: remove from loss list
}
