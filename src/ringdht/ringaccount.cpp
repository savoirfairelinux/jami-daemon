...
RingAccount::handlePendingCall(PendingCall& pc, bool incoming)
{
    ...
    if (incoming) {
        ...
        transport->addStateListener(lid,
                                    [lid, wcall, waccount,
                                     remote_id, ice](pjsip_transport_state state,
                                                     const pjsip_transport_state_info* info) {
                                        if (auto call = wcall.lock()) {
                                            if (auto account = waccount.lock()) {
                                                account->onTransportStateChange(*call, *ice,
                                                                                remote_id, state,
                                                                                info);
                                                return;
                                            }
...
///
/// SIPTransport state change callback to react on transport connection/disconnection.
///
void
RingAccount::onTransportStateChange(SIPCall& call, IceTransport& ice, const std::string& remote_id,
                                    pjsip_transport_state state,
                                    UNUSED const pjsip_transport_state_info* info)
{
    if (state == PJSIP_TP_STATE_CONNECTED)
        onConnectedOutgoingCall(call, remote_id, ice.getRemoteAddress(ICE_COMP_SIP_TRANSPORT));
}
