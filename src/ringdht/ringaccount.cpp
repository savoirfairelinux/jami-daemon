...
RingAccount::doRegister_()
{
    try {
        ...
        dht_.listen<dht::IceCandidates>(
            inboxKey,
            [shared] (dht::IceCandidates&& msg) {
                runOnMainThread([=]() mutable { shared->incomingDataChannel(std::move(msg)); });
                return true;
            });
    } catch (const std::exception& e) {
                                   ...
                                   dht::IceCandidates&& remote_ice)
{
    ...
    const auto key = dht::InfoHash::get("callto:" + peer_id);
...
std::string
RingAccount::sendDataConnection(const std::string& peer_id, dht::IceCandidates&& remote_ice)
{
    ...
    if (auto ice = createIceTransport(("data:" + dht_.getId().toString()).c_str(), 1, true, ice_opts)) {
...
RingAccount::incomingDataChannel(dht::IceCandidates&& msg)
{
    ...
    if (auto ice = getDataConnection(peer_id)) {
        ...
        RING_DBG("Incoming data ICE msg:\n%s", ice_data.data());
        ...
        sendDataConnection(peer_id, std::move(msg));
    }
}
...
RingAccount::sendFile(const std::string& peer_uri, const std::string& filename)
{
    ...
    auto fstream = std::ifstream(filename, std::ios::in | std::ios::binary);
    if (not fstream.good()) {
    ...
    if (sendDataConnection(parseRingUri(peer_uri)).empty())
        return {};
