...
RingAccount::onIncomingIceDataMsg(dht::IceCandidates&& msg)
{
    ...
    const bool is_request = msg.id & 1;

    // TODO: check certificate for white/black listing

    RING_WARN("[dht:%s] rx data %s (id=0x%lx) from %s", getAccountID().c_str(),
              is_request ? "reply" : "request", msg.id, peer_id.c_str());
    ...
    if (is_request) {
        ...
        if (pendingDataTransactionMap_.empty())
            ...
            RING_WARN("[dht:%s] pending data queue full (DDOS?)", getAccountID().c_str());
    } else {
...
RingAccount::onDataTransactionReply(const dht::IceCandidates& msg)
{
    ...
    RING_DBG("[DHT:%s] data connection replied by %s:\n%s", getAccountID().c_str(), peer_id.c_str(),
