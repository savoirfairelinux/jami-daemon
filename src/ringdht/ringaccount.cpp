...
RingAccount::loadIdentity()
{
    ...
    try {
        ...
        fileutils::saveFile(idPath_ + DIR_SEPARATOR_STR "ca.key", ca.first->serialize(), 400);
...
{
    if (id.first)
        fileutils::saveFile(path + ".key", id.first->serialize(), 400);
    if (id.second)
        fileutils::saveFile(path + ".crt", id.second->getPacked(), 400);
}
