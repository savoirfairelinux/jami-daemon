                                   ...
                                   dht::IceCandidates&& remote_ice, bool initiator)
{
    ...
    auto value = std::make_shared<dht::Value>(dht::IceCandidates(msg_id, ice.getLocalAttributesAndCandidates()));
    const auto v_id = value->id;
    ...
    dht_.putEncrypted(
        key, destination, std::move(*value), // TODO: wait for adrien opendht patch
        // on done callback
        [acc, peer_id, key, v_id](bool done) {
            if (!done) {
