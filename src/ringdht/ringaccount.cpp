...
template<typename... Args>
inline void
emitFileXferStatus(Args... args)
{
    emitSignal<DRing::DataTransferSignal::FileTransferStatus>(args...);
}
                                          ...
                                          DRing::DataTransferError error)
{
    ...
    emitDataCnxStatus(getAccountID(), peer_id, DRing::DataConnectionStatus::DISCONNECTED, error);
}
    ...
    try {
        ...
        ice_opts.onInitDone = [acc, peer_id](IceTransport&, bool done) mutable {
            if (done)
                ...
                acc->cancelPendingDataTransaction(peer_id, DRing::DataTransferError::IO);
        ...
        cancelPendingDataTransaction(peer_id, DRing::DataTransferError::SYSTEM);
        return false;
    }
...
RingAccount::onDataIceInitSuccess(const std::string& peer_id)
{
...

    {
        ...
        emitDataCnxStatus(getAccountID(), trx.peer_id,
                          DRing::DataConnectionStatus::CONNECTING,
                          DRing::DataTransferError::NONE);
...
IceTransaction::onTlsStateChange(tls::TlsSessionState state)
{
    if (state == tls::TlsSessionState::ESTABLISHED) {
        ...
        emitDataCnxStatus(account_id, peer_id,
                          DRing::DataConnectionStatus::IDLE,
                          DRing::DataTransferError::NONE);
    } else if (state == tls::TlsSessionState::SHUTDOWN) {
        RING_WARN("[data:%s] TLS shutdown", peer_id.c_str());
        emitDataCnxStatus(account_id, peer_id,
                          DRing::DataConnectionStatus::DISCONNECTED,
                          DRing::DataTransferError::IO);
    }
}
