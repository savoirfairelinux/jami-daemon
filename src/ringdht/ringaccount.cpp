...
public:
    inline static int seqcmp(int32_t seq1, int32_t seq2) {
        return (std::abs(seq1 - seq2) < seqNoTH_) ? (seq1 - seq2) : (seq2 - seq1);
    }

    inline static int seqlen(int32_t seq1, int32_t seq2) {
        return (seq1 <= seq2) ? (seq2 - seq1 + 1) : (seq2 - seq1 + maxSeqNo_ + 2);
    }

    inline static int seqoff(int32_t seq1, int32_t seq2) {
        if (std::abs(seq1 - seq2) < seqNoTH_)
            return seq2 - seq1;

        if (seq1 < seq2)
            return seq2 - seq1 - maxSeqNo_ - 1;

        return seq2 - seq1 + maxSeqNo_ + 1;
    }

    inline static int32_t incseq(int32_t seq) {
        return (seq == maxSeqNo_) ? 0 : seq + 1;
    }

    inline static int32_t decseq(int32_t seq) {
        return (seq == 0) ? maxSeqNo_ : seq - 1;
    }

    inline static int32_t incseq(int32_t seq, int32_t inc) {
        return (maxSeqNo_ - seq >= inc) ? seq + inc : seq - maxSeqNo_ + inc - 1;
    }

public:
    static constexpr int32_t seqNoTH_ {0x3FFFFFFF}; // threshold for comparing seq. no.
    static constexpr int32_t maxSeqNo_ {0x7FFFFFFF}; // maximum sequence number used in UDT
};
...
SecureIceTransport::SecureIceTransport(RingAccount& account, const std::string& pid,
    ...
    , tls_()
{
    RING_DBG("[SIT:%lx] created (tid=%lx) for peer %s", cid, tid, peer_id.c_str());

    std::uniform_int_distribution<int32_t> dist(0, SeqNo::maxSeqNo_);
...
SecureIceTransport::onRxData(std::vector<uint8_t>&& buf)
{
    ...
    auto offset = SeqNo::seqoff(lastRxAck_, pkt->seq);
    RING_WARN("off: %08x", offset);
    //if ((offset < 0) || (offset >= m_pRcvBuffer->getAvailBufSize()))
    //  return -1;

#if 0
    switch (pkt->type) {
        case 0: pushData(data, data_len);
            break;
