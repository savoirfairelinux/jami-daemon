...
private:
    ...
    void idleTask(void);
    void sendCtrlPacket(DataConnection::Packet&);
    void sendAck(const std::array<uint32_t, 2>&, int);
...
void
SecureIceTransport::sendCtrlPacket(DataConnection::Packet& pkt)
{
...
void
SecureIceTransport::sendAck(const std::array<uint32_t, 2>& data, int type)
{
    DataConnection::Packet ack(reinterpret_cast<const uint8_t*>(data.data()), data.size());
    sendCtrlPacket(ack);
}
...
SecureIceTransport::onRxData(std::vector<uint8_t>&& buf)
{
    ...
    if (SeqNum::cmp(pkt->seq, waited_rx_seq) > 0) {
        std::array<uint32_t, 2> lossdata;
        lossdata[0] = waited_rx_seq | 0x80000000;
        lossdata[1] = SeqNum::decrease(pkt->seq);
        sendAck(lossdata, (waited_rx_seq == SeqNum::decrease(pkt->seq)) ? 1 : 2);

        auto loss = SeqNum::lenght(lastRxSeqNum_, pkt->seq) - 2;
        RING_WARN("loss: %d", loss);
