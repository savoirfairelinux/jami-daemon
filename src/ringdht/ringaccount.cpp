...
RingAccount::newIncomingCall(const std::string& from)
{
    ...
    while (call_it != pendingSipCalls_.end()) {
        ...
        } else if (call->getPeerNumber() == from || call_it->from_cert->issuer->getId().toString() == from) {
...
RingAccount::readArchive(const std::string& pwd)
{
    ...
    return loadArchive(file, pwd);
}


RingAccount::ArchiveContent
RingAccount::loadArchive(std::vector<uint8_t>& file, const std::string& pwd)
{
    ArchiveContent c;
...
RingAccount::addDevice(const std::string& password)
{
    ...
    auto pin = dis(rand_);
    auto pin_str = ring::to_string(pin);
    auto pin_h = dht::InfoHash::get(pin_str);

    auto archive = dht::crypto::aesEncrypt(makeArchive(readArchive(password)), password + pin_str);
    dht_.put(pin_h, archive, [](bool ok){
        RING_WARN("Done publishing account archive: %s", ok ? "success" : "failure");
    });
    RING_WARN("Adding new device with PIN: %u at %s (size %zu)", pin, pin_h.toString().c_str(), archive.size());
        ...
        if (hasSignedReceipt()) {
            ...
            if (archive_password.empty()) {
                ...
                if (not archive_pin.empty()) {
                    ...
                    RING_WARN("Trying to load account from DHT with %s at %s.", archive_pin.c_str(), pin_h.toString().c_str());

                    if (dht_.isRunning()) {
                        RING_ERR("DHT already running (stopping it first).");
                        dht_.join();
                    }
                    dht_.run((in_port_t)dhtPortUsed_, {}, false);

                    std::weak_ptr<RingAccount> w = std::static_pointer_cast<RingAccount>(shared_from_this());
                    auto pwd = archive_password + archive_pin;
                    dht_.get(pin_h, [=](std::shared_ptr<dht::Value> val) {
                        RING_WARN("Found something on the DHT");
                        try {
                            auto a = loadArchive(val->data, pwd);
                            if (auto this_ = w.lock()) {
                                RING_WARN("Found archive on the DHT");
                                this_->ringAccountId_ = a.id.second->getId().toString();
                                this_->username_ = RING_URI_PREFIX+ringAccountId_;
                                this_->ethAccount_ = dev::KeyPair(dev::Secret(a.eth_key)).address().hex();
...
RingAccount::doRegister_()
{
    try {
        ...
        if (announce_) {
            RING_DBG("Puting announce at %s: %s", dht::InfoHash(ringAccountId_).toString().c_str(), announce_->toString().c_str());
        ...
        dht_.listen<dht::IceCandidates>(
            ...
            [shared] (dht::IceCandidates&& msg) {
                ...
                this_.findCertificate( msg.from,
                    ...
                    RING_WARN("findCertificate: found");
                    auto& this_ = *shared;
