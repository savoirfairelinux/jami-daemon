...
RingAccount::lookupName(const std::string& name)
{
    ...
    nameDir_.nameLookup(name, [acc,name](const std::string& result, const NameDirectory::Response& response) {
...
{
    auto acc = getAccountID();
    nameDir_.addrLookup(addr, [acc,addr](const std::string& result, const NameDirectory::Response& response) {
        if (response == NameDirectory::Response::found) {
...
{
    RING_WARN("registerName %s: Not implemented yet.", name.c_str());
}
#endif
