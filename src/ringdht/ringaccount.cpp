...
public:
    ...
    std::shared_ptr<FileSender> client {};
                      ...
                      const std::string& name)
{
    ...
    const auto cid = getAccountID() + peer_id;

    bool do_init = false;

    auto fsender = FileSender::newFileSender(cid, name, std::move(stream));
...
IceTransaction::onTlsStateChange(tls::TlsSessionState state)
{
    if (state == tls::TlsSessionState::ESTABLISHED) {
        ...
        if (tls_->isServer()) {
            const auto cid = account_id + peer_id;
            FileReceiver::newFileReceiver(cid, "foobar");
        }
...
bool
DataXfertEndPoint::send(Packet& pkt)
{
    std::unique_lock<std::mutex> lk {txMutex_};
    txPkt_ = std::move(pkt);
    auto& header = txPkt_.getHeader();
    header.seq = ++lastTxSeq_;
    RING_DBG("tx seq = ", header.seq);
...
void
DataXfertEndPoint::txPktAck(const PacketHeader& head)
{
    transport_.send(&head, sizeof(head), [](std::size_t){});
}
