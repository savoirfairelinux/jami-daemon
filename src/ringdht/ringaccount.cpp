...
private:
    ...
    SeqNum::seqnum_t lastRxSeqNum_;
    SeqNum::seqnum_t lastRxAck_;
    std::size_t rxPkt_ {0};
    ...
    , statSync_(clock::now())
{
    ...
    txSeq_ = initialSeq_;
    lastRxSeqNum_ = SeqNum::decrease(initialSeq_);
    lastRxAck_ = initialSeq_;
...
{
    DataConnection::Packet ping(DataConnection::Packet::vec(1300));
    sendPacket(ping);
}
...
SecureIceTransport::onRxData(std::vector<uint8_t>&& buf)
{
    ...
    auto now = clock::now();
    if ((now - statSync_) > std::chrono::seconds(2)) {
        RING_WARN("Pkt: %zu/s, Loss: %zu/s, Bytes: %zu/s", rxPkt_ / 2, lossPkt_ / 2, rxBytes_ / 2);
        statSync_ = now;
