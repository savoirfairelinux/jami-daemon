                        ...
                        std::shared_ptr<dht::crypto::Certificate>& cert_out)
{
    ...
    // Only error accepted is SIGNER_NOT_FOUND
    if ((status & GNUTLS_CERT_INVALID) and status != (GNUTLS_CERT_INVALID|GNUTLS_CERT_SIGNER_NOT_FOUND)) {
        RING_WARN("[peer:%s] Certificate check failed: %u", from.toString().c_str(), status);
        return PJ_SSL_CERT_EUNKNOWN;
    }
