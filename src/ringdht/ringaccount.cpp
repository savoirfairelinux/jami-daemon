...
struct IceTransaction {
    IceTransaction() : account_id(), peer_id(), id(0), tls_() {}
...
IceTransaction::onTlsStateChange(tls::TlsSessionState state)
{
    if (state == tls::TlsSessionState::ESTABLISHED) {
        ...
        //emitDataCnxStatus(account_id, peer_id, DRing::DataTransferCode::CODE_OK);
    } else if (state == tls::TlsSessionState::SHUTDOWN) {
        RING_WARN("[data:%s] TLS shutdown", peer_id.c_str());
        //emitDataCnxStatus(account_id, peer_id, DRing::DataTransferCode::CODE_UNAUTHORIZED);
    }
}

// - DO NOT BLOCK - (Called in TlsSession thread)
void
IceTransaction::onRxData(UNUSED std::vector<uint8_t>&& buf)
{
    // TODO
}
