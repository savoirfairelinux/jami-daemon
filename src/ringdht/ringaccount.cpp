...
RingAccount::onIncomingIceDataMsg(dht::IceCandidates&& msg)
{
    ...
    if (is_request) {
        ...
        if (hasRunningDataConnection(peer_id)) {
            RING_WARN("[dht:%s] drop data request/reply: connection existing", getAccountID().c_str());
            return;
        }
        ...
        if (pendingDataTransactionMap_.empty()) {
            RING_DBG("[dht:%s] rx data request/0x%lx from %s", getAccountID().c_str(), msg.id,
                     peer_id.c_str());
            newDataTransaction(peer_id, false, std::move(msg));

        } else if (pendingDataRequests_.size() < PENDING_DATA_MSG_LENGHT) {
            pendingDataRequests_.emplace_back(std::move(msg));
            RING_DBG("[dht:%s] delaying data request/0x%lx", getAccountID().c_str(), msg.id);

        } else
            RING_WARN("[dht:%s] droped request/0x%lx: pending data queue full (DDOS?)",
                                ...
                                dht::IceCandidates&& remote_ice)
{
    ...
    IceTransaction* trx;
    {
        ...
        RING_DBG("[dht:%s] data transaction created for peer %s", getAccountID().c_str(), peer_id.c_str());
    }
