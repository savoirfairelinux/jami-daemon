...
template <class S, class T> S& operator<<(S& _out, std::shared_ptr<T> const& _p);
...
#endif
{
	...
	if (strftime(buffer, sizeof(buffer), _format.c_str(), &timeValue)) {
		std::stringstream ss;
		ss << millisRemainder;
		return std::string(buffer) + "." + (millisRemainder < 1 ? "000" : millisRemainder < 10 ? "00" : millisRemainder < 100 ? "0" : "") + ss.str() + "Z";
	}
	return std::string();
}
...
template <class S, class T> S& operator<<(S& _out, std::shared_ptr<T> const& _p) { if (_p) _out << "@" << (*_p); else _out << "nullptr"; return _out; }
