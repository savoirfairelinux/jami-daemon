...
{
public:
    Connector(Impl& parent, const dht::InfoHash& peer_h) : parent_ {parent}, peer_ {peer_h} {}
...
public:
    ...
    ClientConnector(Impl& parent,
        ...
        : Connector {parent, peer_h}, publicAddresses_ {public_addresses} {
    ...
    void ready() override {
        connection_ = std::make_unique<PeerConnection>(
            parent_.account, peer_.toString(), std::move(endpoint_));
...
public:
    ...
    void ready() override {
        connection_ = std::make_unique<PeerConnection>(
            parent_.account, peer_.toString(), std::make_unique<TcpTurnEndpoint>(*turn_, peerAddr_));
    }
...
DhtPeerConnector::sendRequest(const std::string& peer_id)
{
    ...
    pimpl_->account.forEachDevice(
        peer_h,
        [this, addresses](const std::shared_ptr<RingAccount>& account,
            ...
            if (iter == std::end(pimpl_->connectors)) {
                pimpl_->connectors.emplace(
                    dev_h,
                    std::make_unique<Impl::ClientConnector>(*pimpl_, dev_h, addresses));
            }
        },
