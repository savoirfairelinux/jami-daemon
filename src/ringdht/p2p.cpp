...
public:
    ...
    bool write(const std::vector<char>& buffer) override {
        if (fileSize_ > 0) {
            ...
            header_.insert(std::end(header_), std::begin(buffer), std::end(buffer));
            const auto& rc_pos = std::find(std::begin(header_), std::end(header_), '\n');
            if (rc_pos != std::end(header_)) {
                fileSize_ = ::strtoull(&header_[0], nullptr, 10);
                RING_DBG() << "Detected file size: " << fileSize_;

                // push remaining bytes into file
                auto size = std::distance(rc_pos, std::end(header_)) - 1;
                out_.write(&header_[0] + std::distance(std::begin(header_) + 1, rc_pos), size);
                rx_ = size;
