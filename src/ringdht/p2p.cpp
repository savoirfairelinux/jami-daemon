...
public:
    Connector(Impl& parent, const dht::InfoHash& peer_h,
              std::function<void(PeerConnection*)> connect_cb = [](PeerConnection*){})
...
public:
    ...
    void process() {
        ...
        RING_DBG() << "# of addresses: " << msg.addresses.size();
...
public:
    ...
    void reply() {
        RING_DBG() << "# of addresses: " << request_.addresses.size();
        for (auto& addr : request_.addresses) {
            RING_DBG() << "add peer " << addr;
                              ...
                              std::function<void(PeerConnection*)> connect_cb)
{
    ...
    RING_DBG() << "# of addresses: " << addresses.size();
