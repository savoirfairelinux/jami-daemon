...
public:
    ...
    void connectRelay(const IpAddr& relay) {
        ...
        relayConnect_ = std::async(std::launch::async, [this] {
                static const auto hello_packet = "hello";
                static const auto hello_answer_packet = "foobar";
                try {
                    ...
                    // If endpoint is connected to a TURN peer connection point,
                    // the server may not be bounded yet and the TURN server is going to bufferize
                    // sent packet by us. This buffer is limited so we need to ensure our connected
                    // on both endpoints first.
                    // Do to so, we proceed to a simple packet exchange: client -> server -> client
                    endpoint_->writeline(hello_packet, ::strlen(hello_packet));
                    RING_DBG("wait for HELLO answer");
                    auto answer = endpoint_->readline();
                    RING_DBG() << "client: rx " << std::string(std::begin(answer), std::end(answer));
                    if (!std::equal(&answer[0], &answer[answer.size()], &hello_answer_packet[0]))
                        throw std::logic_error("invalid HELLO answer");
                    ready();
                } catch (const std::exception& e) {
                    RING_ERR() << "client connection exception: " << e.what();
                    cancel();
                }
            });
    }
...
public:
    ...
    void onPeerConnection(uint32_t conn_id, const IpAddr& peer_addr) {
        (void)conn_id;
        peerAddr_ = peer_addr;
        peerHello_ = std::async(std::launch::async, [this] {
                static const auto hello_packet = "hello";
                static const auto hello_answer_packet = "foobar";
                try {
                    // 'HELLO' synchronization, see ClientConnector::connectRelay()
                    std::vector<char> answer;
                    RING_DBG("wait 'HELLO' from client");
                    turn_->readlinefrom(peerAddr_, answer);
                    RING_DBG() << "server: rx " << std::string(std::begin(answer), std::end(answer));
                    if (!std::equal(&answer[0], &answer[answer.size()], &hello_packet[0]))
                        throw std::logic_error("invalid HELLO sequence");
                    turn_->writelineto(peerAddr_, hello_answer_packet, ::strlen(hello_answer_packet));
                    ready();
                } catch (const std::exception& e) {
                    RING_ERR() << "server connection exception: " << e.what();
                    cancel();
                }
            });
    }
