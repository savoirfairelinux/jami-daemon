...
public:
    ...
    void process() {
            ...
            param.onPeerConnection = [this](uint32_t conn_id, const IpAddr& peer_addr,
                                            bool connected) {
                onPeerConnection(conn_id, peer_addr, connected);
            };
    ...
    void onPeerConnection(uint32_t conn_id, const IpAddr& peer_addr, bool connected) {
        (void)conn_id;
        if (connected)
            channel_ << peer_addr;
        else {
            RING_ERR("disconnected by peer");
            cancel();
        }
    }
...
DhtPeerConnector::Impl::onRequestMsg(PeerConnectionMsg&& request)
{
    ...
    // Attach a FTP server when connection is established
    connectors.emplace(
        request.from,
        std::make_unique<ServerConnector>(
            *this, std::move(request),
            [](PeerConnection* connection){
                if (connection) {
                    auto transfer = std::make_shared<FtpServer>();
