...
DhtPeerConnector::Impl::onTurnPeerDisconnection(const IpAddr& peer_addr)
{
    ...
    while (iter != servers_.end()) {
        if (iter->first.second == peer_addr) {
            ...
            case CtrlMsgType::CANCEL:
                {
                    ...
                    auto iter = servers_.begin();
                    while (iter != servers_.end()) {
                        if (iter->first.first == dev_h
                        && iter->second && iter->second->hasStreamWithId(id)) {
                            std::cout << "ERASE SERVER" << std::endl;
                            servers_.erase(iter);
                            connectedPeers_.erase(iter->first.second);
                            break;
                        }
                        iter++;
                    }
