...
public:
    ...
    static constexpr const char* BASEKEY = "peer:"; ///< base to compute the DHT listen key
    ...
    PeerConnectionMsg(dht::Value::Id id, uint32_t aprotocol, const std::string& arelay, const SecretType& asecret)
        : id {id}, protocol {aprotocol}, addresses {{arelay}}, secret {asecret} {}
...
private:
    ...
    void onAddDevice(const dht::InfoHash&, const std::vector<std::string>&,
...
/// /note TCP peer connection mode is enabled for reliable data transfer.
void
...
DhtPeerConnector::Impl::onTurnPeerConnection(const IpAddr& peer_addr)
{
    ...
    auto tls_ep = std::make_unique<TlsTurnEndpoint>(*turn_ep, account.identity(), account.dhParams());
                                            ...
                                            const std::shared_ptr<dht::crypto::Certificate>& cert)
{
    ...
    RING_WARN() << "req id: " << request.id;
    //authPeers_.emplace(request.id, cert);
                                    ...
                                    const std::function<void(PeerConnection*)>& connect_cb)
{
    ...
    if (iter == std::end(clients_)) {
        clients_.emplace(
            dev_h,
            std::make_unique<Impl::ClientConnector>(*this, dev_h, public_addresses, connect_cb));
    } else {
                                    ...
                                    const std::function<void(PeerConnection*)>& connect_cb)
{
    ...
    pimpl_->account.forEachDevice(
        ...
        [this, addresses, connect_cb](const std::shared_ptr<RingAccount>& account,
            ...
            pimpl_->ctrl << makeMsg<CtrlMsgType::ADD_DEVICE>(dev_h, addresses, connect_cb);
        },
