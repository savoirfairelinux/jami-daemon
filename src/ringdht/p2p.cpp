...
public:
    ...
    void onPeerConnection(uint32_t conn_id, const IpAddr& peer_addr) {
        (void)conn_id;
        peerAddr_ = peer_addr;
        peerHello_ = std::async(std::launch::async, [this]{ doSynchro(); });
    }
...
private:
    void doSynchro() {
        try {
            // PING/PONG synchronization, see ClientConnector::doSynchro()
            RING_DBG("wait PING from client");
            ...
            RING_DBG("sending PONG to client");
            turn_->writelineto(peerAddr_, PONG, ::strlen(PONG));
            ready();
        } catch (const std::exception& e) {
            RING_ERR() << "server connection failed: " << e.what();
            cancel();
        }
    }
...
DhtPeerConnector::Impl::onRequestMsg(PeerConnectionMsg&& request)
{
    ...
    if (iter != std::end(connectors)) {
        RING_WARN() << "Re-use existing E2E connection with " << request.from;
        return;
    }
