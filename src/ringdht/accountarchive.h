...
public:
    ...
    void load(const std::string& path, const std::string& password) { deserialize(read(path, password)); }

    /** Save archive to file, optionally encrypted with provided password. */
    void save(const std::string& path, const std::string& password) const { write(serialize(), path, password); }

private:
    static std::vector<uint8_t> read(const std::string& path, const std::string& password);
    static void write(const std::string& data, const std::string& path, const std::string& password);
};
