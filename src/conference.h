...
public:
     ...
     */
    void bindCall(const std::shared_ptr<Call>& call);
    /**
     ...
     * @param remoteHostUri
     */
    void mergeConfInfo(ConfInfo& newInfo, const std::string& remoteHostUri);

#ifdef ENABLE_VIDEO
    // TODO probably remove createSinks from conference and avoid access to videoMixer/input
    // and prefer top down approach to avoid noodle code.
    void createSinks(const ConfInfo& infos);
    std::shared_ptr<video::VideoMixer> getVideoMixer() const;
...
private:
    ...
    ConfInfo confInfo_ {};
    void sendConferenceInfos(); // TODO move in callstreammgr?
