...
constexpr std::string_view const UPLOAD_PACK_CMD = "git-upload-pack";
constexpr std::string_view const HOST_TAG = "host=";
 ...
 */
static int generateRequest(git_buf* request, const std::string& cmd, const std::string& url);
 ...
 * @return 0 on success
 */
static int sendCmd(P2PStream* s);
 ...
 * @return 0 on success
 */
static int P2PStreamRead(git_smart_subtransport_stream* stream,
                         char* buffer,
                         size_t buflen,
                         size_t* read);

static int P2PStreamWrite(git_smart_subtransport_stream* stream, const char* buffer, size_t len);

/**
 * Free resources used by the stream
 */
static void P2PStreamFree(git_smart_subtransport_stream* stream);
 ...
 * @return 0 on success
 */
static int P2PSubTransportAction(git_smart_subtransport_stream** out,
 ...
 */
static int P2PSubTransportClose(git_smart_subtransport*);

/**
 * Free resources used by a transport
 * @param transport     Transport to free
 */
static void P2PSubTransportFree(git_smart_subtransport* transport);
 ...
 * @return 0 on success
 */
static int P2PSubTransportNew(P2PSubTransport** out, git_transport* owner, void* payload);
