...
void
SocketPair::initRTCPXR()
{
    memset(&rtcpXRPacket_.xrHeader,0, sizeof(rtcpXRHeader));
    memset(&rtcpXRPacket_.blkHeader,0, sizeof(rtcpXRHeader));
}
...
SocketPair::checkRTPPacket(rtpHeader* header)
{
    ...
    auto rtpCheckTimer = std::chrono::duration_cast<std::chrono::seconds>
        (std::chrono::system_clock::now() - lastRTPCheck_);
...
SocketPair::sendRTCPXR()
{
    ...
    //block content in uint8_t format
    std::vector<uint8_t> blkContentSerialized;
    uint8_t unsigned8Val;
    uint16_t unsigned16Val;
    for (const auto& it : rtcpXRPacket_.blkContent) {
        unsigned16Val = it.to_ulong();
        unsigned8Val = (uint8_t)(unsigned16Val >> 8);
        blkContentSerialized.push_back(unsigned8Val);
        unsigned8Val = (uint8_t) unsigned16Val;
        blkContentSerialized.push_back(unsigned8Val);
    }

    //length
    auto pktLength = sizeof(rtcpXRHeader) + sizeof(rtcpXRBlockHeader) + blkContentSerialized.size();

    rtcpXRPacket_.xrHeader.len = htons(pktLength);

    rtcpXRPacket_.blkHeader.len = htons(sizeof(rtcpXRBlockHeader) + blkContentSerialized.size());

    //serialisation in packet for socket
    uint8_t* packet = (uint8_t*) malloc(sizeof(rtcpXRHeader) + sizeof(rtcpXRBlockHeader) + blkContentSerialized.size());
    ...
    memcpy(ptr_packet, blkContentSerialized.data(), blkContentSerialized.size());
