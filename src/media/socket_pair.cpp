...
SocketPair::saveRtcpRRPacket(uint8_t* buf, size_t len)
{
    ...
    listRtcpRRHeader_.push_back(*header);
...
SocketPair::readCallback(uint8_t* buf, int buf_size)
{
    ...
    if (datatype & static_cast<int>(DataType::RTCP)) {
        ...
        else if(header->pt == 206) //201 = RR PT
        {
    ...
    if (not fromRTCP and srtpContext_ and srtpContext_->srtp_in.aes) {
        ...
        int32_t delay;
        auto res = getOneWayDelayGradient2(curentSendTS, &delay);
        if (res)
...
bool
SocketPair::getOneWayDelayGradient2(uint32_t sendTS, int32_t* gradient)
{
    // First sample, fill Ts0 and Tr0
    if(not currentTS.send_ts) {
        currentTS.send_ts = sendTS;
        currentTS.receive_ts = std::chrono::steady_clock::now();
        return false;
    }

    // new frame
    if(currentTS.send_ts != sendTS) {
        // Second sample, fill Ts1 and Tr1 and replace Ts0 and Tr0
        if(not currentTS.last_send_ts) {
            currentTS.last_send_ts = currentTS.send_ts;
            currentTS.send_ts = sendTS;

            currentTS.last_receive_ts = currentTS.receive_ts;
            currentTS.receive_ts = std::chrono::steady_clock::now();
            return false;
        }
        JAMI_ERR("[PL] send_ts:%ld, last:%ld", currentTS.send_ts, currentTS.last_send_ts);
        uint32_t deltaS = ((currentTS.send_ts - currentTS.last_send_ts) / 90000.0) * 1000000;             // microseconds
        auto deltaR = std::chrono::duration_cast<std::chrono::microseconds>(currentTS.receive_ts - currentTS.last_receive_ts).count();
