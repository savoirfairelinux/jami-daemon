...
SocketPair::writeCallback(uint8_t* buf, int buf_size)
{
    ...
    if(buf[1] == 200) //Sender Report
    {
        ...
        currentSRTS = ts_MSB + (ts_LSB / std::pow(2,32));
        ...
        if(lastSRTS_ != 0 && lastDLSR_ != 0)   
        {
            ...
            dlsr2 = std::chrono::duration_cast<std::chrono::milliseconds>(end - lastRR_time).count(); //en millisecondes

            currentLatency = (currentSRTS - lastSRTS_) / 2;
            //currentLatency = (currentSRTS - lastSRTS_ - (lastDLSR_ / 65536) - (dlsr2/1000)) / 2;
