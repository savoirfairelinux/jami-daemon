...
static constexpr unsigned GMIN {64};
...
struct RtpStats {
    ...
    // Packets statistic sample (recorded at RTP arrival)
    uint16_t interval_base_seq;
    std::vector<char> rx_seq;
...
RtpStats::checkSeq(uint16_t seq)
{
    ...
    if (udelta < MAX_DROPOUT) {
        ...
        RING_ERR("RTP: dup/misorder of %u", udelta - RTP_SEQ_MOD);
    }
    ++received;
    return 1;
}
...
RtpStats::dumpInterval(uint32_t ssrc, uint8_t pt)
{
    ...
    for (auto v : rx_seq) {
        ...
        if (v == '1')
            ...
            ++loss_count;
            if (rx_since_loss >= GMIN or loss_count == 0) {
                if (lost == 1)
                    ++c14;
                else
                    ++c13;
    ...
    // remaining good packet sequence is it a gap?
    if (loss_count) {
    ...
    // Interpret our Markov model states with the Gilbert-Elliott model
    // See https://en.wikipedia.org/wiki/Burst_error
    ...
    float burst_density, gap_density;
    ...
    if (auto total_burst = c31 + c32 + c33) {
        ...
        burst_density = 0; // no burst detected, fully gap interval

    if (auto gap_count = c11 + c14)
        gap_density = (float)c14 / gap_count;
    else
        gap_density = 0; // no gap detected, fully bursted interval

    // loss rate and burst/gap length in packets
    auto loss_rate = (float)loss_count / ctotal;
    auto gap_length = (float)(c11 + c14 + c13) / c13;
    auto burst_length = (float)ctotal / c13 - gap_length;

    RING_DBG("[rtp@%08x,%u] States: %u, %u, %u, %u, %u, %u", ssrc, pt,
              c11, c13, c14, c22, c23, c33);
    RING_WARN("[rtp@%08x,%u] ctotal=%u, loss_rate=%.1f%%, burst_density=%.1f%%, gap_density=%.1f%%",
              ssrc, pt, ctotal, 100*loss_rate, 100*burst_density, 100*gap_density);
}
    ...
    if (valid) {
        ...
        if (auto lost_interval = expected_interval - received_interval) {
            ...
            for (unsigned i=0; i < lost_interval; ++i)
                rtpStats_->rx_seq.push_back('0');
        }

        // drop dup/misorder seq not in interval
        uint16_t udelta = seq - rtpStats_->interval_base_seq;
        if (udelta <= RTP_SEQ_MOD - MAX_MISORDER)
            rtpStats_->rx_seq.push_back('1');
    }

    // periodic report (each 5 seconds)
