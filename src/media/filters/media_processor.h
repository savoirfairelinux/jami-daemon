...
public:
    ...
    std::shared_ptr<Mask> getResult2() { return lastOutput2_; };
    void stop() {
        running = false;
        inputCv.notify_all();
    }
...
private:
    ...
    std::unique_ptr<cv::Mat> lastInput2_;
    std::shared_ptr<std::vector<tensorflow::Tensor>> lastOutput_ {nullptr};
    std::shared_ptr<Mask> lastOutput2_ ; // {nullptr};
    ...
    std::shared_ptr<Mask> process2(cv::Mat);
