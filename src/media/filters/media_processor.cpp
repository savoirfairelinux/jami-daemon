...
MediaProcessor::addFrame(AVFrame* frame)
{
    ...
    if (lastOutput_)
    {
        ...
        cout <<"in addFrame rects size " <<  (*lastOutput_).rects.size() << endl;
        cout <<"in addFrame labels size " <<  (*lastOutput_).labels.size() << endl;
        cout <<"in addFrame confidence size " <<  (*lastOutput_).confidence.size() << endl;
        cout <<"in addFrame label " <<  (*lastOutput_).labels[0] << endl;
        cout <<"in addFrame confidence " <<  (*lastOutput_).confidence[0] << endl;

        //cv::Mat newframe;
        //(*lastInput2_).copyTo(opencvframe, (*lastOutput_));
...
MediaProcessor::process(cv::Mat input)
{
    ...
    cascade_->cv::CascadeClassifier::detectMultiScale( input, faces, 1.1, 
    ...
    bool success = facemark_->fit(input,faces,landmarks);
    ...
    //cv::Mat mask_matrix = input2;
        ...
        for ( size_t i = 0; i <faces.size(); i++ )
        {
            ...
            {
                smallImgROI = faceCenterRotateCrop(input,landmarks[i],faces[i]);
            }
