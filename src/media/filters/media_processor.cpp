...
MediaProcessor::addFrame(AVFrame* frame)
{
	...
	{		
	    ...
	    if (auto out = getResult()){
	    	...
	    	libav_utils::fillWithBlack(output);
	    }
...
MediaProcessor::process(cv::Mat input)
{
	...
	for (int i = 0; i < rects2.size(); ++i) {
	    	rects.push_back(rects2[i]);
	}
    

    if (rects.size() == 0) {
        return lastOutput_; // return last in case no detection on one single frame, there is no sudden unblurring
    } else {
    ...
    for (int i = 0; i < rects.size(); ++i){
    	...
    	if (overlap ==false){
    		rects3.push_back(rects[i]); // push back rects not overlapping any other
    	}
    }

    // this works provided there is no overlapping with more than 2 rectangles, otherwise n double loops 
