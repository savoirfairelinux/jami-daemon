...
const std::string model_dir= "/home/tmenais/tutocpp/tensorflow/face_recognition_only_cpp/";

const std::string graph = model_dir + "20170512-110547.pb";
const std::string opencvcascade = model_dir + "haarcascade_frontalface_alt2.xml";
const std::string landmarkfile = model_dir + "lbfmodel.yaml";
const std::string label_database_file = model_dir + "face_embeddings_database.txt";
...
MediaProcessor::addFrame(AVFrame* frame)
{
    ...
    if (auto out = getResult()){
        for (int i = 0; i < out->rects.size(); ++i) {
            if (out->confidence[i] < 0.075) {
                std::cout <<"Hello " <<  out->labels[i] << ", confidence :" << out->confidence[i]  << std::endl;
                drawBox(opencvframe, out->rects[i],out->labels[i]);
            } else {
                std::cout << "Who are you ?" << ", confidence :" << out->confidence[i]  << std::endl;
                drawBox(opencvframe, out->rects[i],"404");
            }
        }
    }
