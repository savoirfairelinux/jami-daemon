...
MediaProcessor::addFrame(AVFrame* frame)
{
    ...
    if (lastOutput_)
    {
        ...
        {
            if ((*lastOutput_).confidence[i] < 0.075)
            {
                std::cout <<"Hello " <<  (*lastOutput_).labels[i] << ", confidence :" << (*lastOutput_).confidence[i]  << std::endl;
                drawBox(opencvframe, (*lastOutput_).rects[i],(*lastOutput_).labels[i]);
            }else{
                std::cout << "Who are you ?" << ", confidence :" << (*lastOutput_).confidence[i]  << std::endl;
                drawBox(opencvframe, (*lastOutput_).rects[i],"404");
            }
