...
//Subscription Class forwading
template <class T> class AsyncSubscription;

//================ Publisher
template <class T>  class AsyncPublisher : public Publisher<T>{
public:
...
protected:
    ...
    std::list<std::weak_ptr<AsyncSubscription<T>>> subscriptions;
};


template <class T>
void AsyncPublisher<T>::subscribe(Subscriber<T>& subscriber){
    auto subscription = std::make_shared<AsyncSubscription<T>>(subscriber);
