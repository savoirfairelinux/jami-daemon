...
constexpr auto OVERUSE_THRESH1 = std::chrono::milliseconds(75);
constexpr auto OVERUSE_THRESH2 = std::chrono::milliseconds(100);
constexpr auto OVERUSE_THRESH3 = std::chrono::milliseconds(125);
...
{
    float ky = 0.0f;
    if (std::abs(m) < last_thresh_y_)
        ky = kd;
    else
        ky = ku;
    float res = last_thresh_y_ + ((deltaT * ky) * (std::abs(m) - last_thresh_y_));
    last_thresh_y_ = res;
    return res;
}
...
CongestionControl::get_bw_state(float estimation, float thresh)
{
    if(estimation > thresh) {
        ...
        if((overuse_timer >= OVERUSE_THRESH1) and (overuse_counter_ > 1) and last_state_ != bwOverusing1) {
            last_state_ = bwOverusing1;
        }
        else if((overuse_timer >= OVERUSE_THRESH2) and (overuse_counter_ > 1) and last_state_ != bwOverusing2) {
            last_state_ = bwOverusing2;
        }
        else if((overuse_timer >= OVERUSE_THRESH3) and (overuse_counter_ > 1) ) {
            overuse_counter_ = 0;
            last_state_ = bwOverusing3;
        }
    }
