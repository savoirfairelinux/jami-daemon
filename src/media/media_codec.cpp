...
MediaAttribute::getMediaType(const std::vector<MediaMap>& mediaList, unsigned index)
{
    ...
    if (value.compare(MediaAttributeValue::AUDIO) == 0)
        return {true, MediaType::MEDIA_AUDIO};
    if (value.compare(MediaAttributeValue::VIDEO) == 0)
        return {true, MediaType::MEDIA_VIDEO};

    JAMI_ERR("Invalid value %s for a media type key for media @ index %u", value.c_str(), index);
    return {false, MediaType::MEDIA_NONE};
}
