...
{
#ifdef RING_VIDEO
    bitrate = systemCodecInfo.minBitrate + (systemCodecInfo.maxBitrate - systemCodecInfo.minBitrate) * (Manager::instance().videoPreferences.getBitrate() / 100);
    quality = systemCodecInfo.minQuality - (systemCodecInfo.minQuality - systemCodecInfo.maxQuality) * (Manager::instance().videoPreferences.getQuality() / 100);
    isAutoQualityEnabled = Manager::instance().videoPreferences.getAutoQuality();
#endif
    return {
        {DRing::Account::ConfProperties::CodecInfo::NAME, systemCodecInfo.name},
        {DRing::Account::ConfProperties::CodecInfo::TYPE, (systemCodecInfo.mediaType & MEDIA_AUDIO ? "AUDIO" : "VIDEO")},
