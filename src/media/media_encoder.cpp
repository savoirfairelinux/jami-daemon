...
{
    RING_WARN("|||||entered into set Device Options\n");
    device_ = args;
    if (device_.width)
...
{
    RING_WARN("|||||entered into set Options\n");
    codec_ = args.codec;
...
{
    RING_WARN("|||||entered into open Live Output");
    setOptions(args);
    ...
    if (args.codec->systemCodecInfo.mediaType == MEDIA_VIDEO)
        RING_WARN("Video:addstream result = %d", ret);
    else
        RING_WARN("Audio:addstream result = %d", ret);
}
...
MediaEncoder::addStream(const SystemCodecInfo& systemCodecInfo, std::string parameters)
{
    ...
    RING_WARN("systemCodecInfo.avcodecId = %d\n",systemCodecInfo.avcodecId);
...
{
    RING_WARN("$$$$in RING_VIDEO");
    RING_WARN("|||||entered into encode-1");
    /* Prepare a frame suitable to our encoder frame format,
     * keeping also the input aspect ratio.
     */
    int ret = 0;
    //RING_WARN("before scaledframe = %u, width = %d, height = %d\n",scaledFrame_.data,frame.width,frame.height);//start from here
    ...
    if (enc->framerate.num == enc->time_base.den && enc->framerate.den == enc->time_base.num)
        ...
        {
            RING_WARN("pointer framerate: %p",enc->framerate);
    ...
    RING_WARN("$$$Inside RING_ACCEL");
    VideoFrame v1;
    // v1.reserve(AV_PIX_FMT_NV12, frame->width, frame->height);
    //uto hw_frame = v1.pointer();
    AVFrame *hw_frame = NULL;
    if (frame)
        hw_frame = v1.pointer();

    RING_WARN("$$$accel or not = %s\n",accel_.name.c_str());
    if (!accel_.name.empty()) {
        if (frame && hw_frame){
            RING_WARN("in encode hw_frames_ctx = %p",enc->hw_frames_ctx);
...
{
    if (frame->width>0)
    RING_WARN("|||||entered into encode-2");
    int ret = 0;
    ...
    if (frame->width>0)
    RING_WARN("avframe data = %p, size = %d\n",frame->data,*(frame->linesize));

    while (ret >= 0) {
        ret = avcodec_receive_packet(encoderCtx, &pkt);
        if (frame->width>0){
            RING_WARN("after avcodec_receive_packet\n");
            RING_WARN("avpacket data = %p, size = %d\n",pkt.data,pkt.size);
        }
...
{
    RING_WARN("|||||prepare encoder context\n");
    ...
    if (is_video) {
        ...
        // time base = 1/FPS
        RING_WARN("()()()()prepareEncoderContext");
        RING_WARN("framerate: %d/%d",encoderCtx->framerate.num,encoderCtx->framerate.den);
        RING_WARN("device framerate: %d",device_.framerate);
...
void MediaEncoder::forcePresetX264(AVCodecContext* encoderCtx)
{
    ...
    RING_WARN("h264 force preset finished\n");
}
