...
MediaEncoder::addStream(const SystemCodecInfo& systemCodecInfo)
{
    ...
    if (auto index = createCodingSession(systemCodecInfo.name) < 0)
        return index;
...
int
MediaEncoder::createCodingSession(const std::string& codecName)
{
    auto stream = avformat_new_stream(outputCtx_, outputCodec_);

    if (stream == nullptr) {
        JAMI_ERR("[%p] Failed to create coding instance for %s", this, codecName.c_str());
        return -1;
    }
