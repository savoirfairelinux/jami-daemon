...
MediaEncoder::addStream(const SystemCodecInfo& systemCodecInfo, std::string parameters)
{
    ...
    auto maxBitrate = 8 * 1000 * std::atoi(libav_utils::getDictValue(options_, "max_rate")); // there is a ratio 8 between bit_rate and actual flow rate
    ...
    if (enableAccel_) {
        ...
        if (!accel_.name.empty()){
            RING_WARN("[Using encoder with codec %s and %s API.]", avcodec_get_name(encoderCtx->codec_id), accel_.name.c_str());
    ...
    if (systemCodecInfo.avcodecId == AV_CODEC_ID_H264) {
        ...
        encoderCtx->bit_rate = maxBitrate/2;
        encoderCtx->rc_buffer_size = bufSize;
        //encoderCtx->rc_min_rate = maxBitrate/4;
        //encoderCtx->rc_max_rate = maxBitrate;
        RING_WARN("buffer=%d",encoderCtx->rc_buffer_size);
        RING_WARN("minrate=%" PRIu64 "",encoderCtx->rc_min_rate);
        RING_WARN("maxrate=%" PRIu64 "",encoderCtx->rc_max_rate);
        RING_WARN("crf=%d",crf);
        RING_WARN("width=%d",encoderCtx->width);
        RING_WARN("height=%d",encoderCtx->height);
        RING_WARN("framerate=%d/%d",encoderCtx->framerate.num,encoderCtx->framerate.den);
        RING_WARN("bit rate=%dbit/s",encoderCtx->bit_rate/8);
                                         ...
                                         AVCodecContext *ctx)
{
    ...
    RING_DBG("Using profile %d and level %d", ctx->profile, ctx->level);
}
