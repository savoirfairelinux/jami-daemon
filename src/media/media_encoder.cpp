...
MediaEncoder::initAccel(AVCodecContext* encoderCtx, uint64_t br)
{
    ...
    if (accel_->getName() == "nvenc"sv) {
        ...
        // Use VBR Mode cause it produce the best results
        av_opt_set_int(encoderCtx, "vbr", 1, AV_OPT_SEARCH_CHILDREN);
        // Don't specify bufsize because it is not reset by QSV API
        av_opt_set_int(encoderCtx, "bufsize", 0, AV_OPT_SEARCH_CHILDREN);
        av_opt_set_int(encoderCtx, "b", br * 1000 * 0.8f, AV_OPT_SEARCH_CHILDREN);
    }
}
