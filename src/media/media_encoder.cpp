...
// maximum number of times accelerated encoding can fail in a row before falling back to software
...
#ifdef RING_ACCEL
    // TODO implement fallback and video preference
...
#ifdef RING_ACCEL
    // TODO implement video preference
    if (systemCodecInfo.mediaType == MEDIA_VIDEO && enableAccel_) {
...
//    } else if (Manager::instance().videoPreferences.getEncodingAccelerated()) {
//        RING_WARN() << "Hardware encoding disabled because of previous failure";
    } else {
        RING_WARN() << "Hardware encoding disabled";// by user preference";
    }
#endif
