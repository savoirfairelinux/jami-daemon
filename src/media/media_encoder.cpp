...
int MediaEncoder::encodeAudio(AVFrame* frame)
{
    ...
    for (int i = 0; i < frame->linesize[0]; ++i) {
        if (frame->data[0][i]) {
            RING_WARN("MediaEncoder: nonzero sample: frame:%d sample:%d val:%d",
                sent_samples / frame->nb_samples, i, frame->data[0][i]);
            // This makes the encode crash with alignment issues
            //frame->data[0][i] = 0;
        }
    }
