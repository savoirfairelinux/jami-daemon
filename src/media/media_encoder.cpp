...
MediaEncoder::setOptions(std::map<std::string, std::string> options)
{
    ...
    options.emplace("max_rate", ring::to_string(bitrate));
    options.emplace("crf", ring::to_string(quality));
    options.emplace("sample_rate", "8000");
    options.emplace("channels", "2");
    int sampleRate = atoi(options["sample_rate"].c_str());
    options.emplace("frame_size", ring::to_string(static_cast<unsigned>(0.02*sampleRate)));
    options.emplace("width", "320");
    options.emplace("height", "240");
    options.emplace("framerate", "30.00");
