...
MediaEncoder::addStream(const SystemCodecInfo& systemCodecInfo, std::string parameters)
{
    ...
    if (systemCodecInfo.avcodecId == AV_CODEC_ID_H264) {
        ...
        av_opt_set(encoderCtx->priv_data, "crf", av_dict_get(options_, "crf", NULL, 0)->value, 0);
    ...
    if (avcodec_open2(encoderCtx, outputCodec, NULL) < 0)
        throw MediaEncoderException("Could not open encoder");

    // add video stream to outputformat context
    AVStream* stream = avformat_new_stream(outputCtx_, 0);
    if (!stream)
...
#if LIBAVFORMAT_VERSION_INT >= AV_VERSION_INT(58, 7, 100)
        av_dump_format(outputCtx_, i, outputCtx_->url, 1);
#else
        av_dump_format(outputCtx_, i, outputCtx_->filename, 1);
#endif
}
