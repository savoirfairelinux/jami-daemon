...
static int vaapi_retrieve_data(AVCodecContext *avctx, AVFrame *input)
{
    ...
    if (input->format != AV_PIX_FMT_VAAPI) {
        RING_ERR("Frame format is not vaapi");
        abort(); // ffmpeg aborts here, apparently this is terrible
    }
                                      ...
                                      int fallback_allowed)
{
    ...
    // FF_ARRAY_ELEMS: (sizeof(a) / sizeof((a)[0])) (vaapi_profile_map)
    ...
    if (!exact_match) {
        if (fallback_allowed) {
            RING_DBG("No VAAPI support for codec %s profile %d", codec_desc->name, avctx->profile);
                // RING_WARN("If you want attempt decoding "
    ...
    // commented out because it is forced to none for now
    ...
    // should pass through here and find nv12
...
AVBufferRef *vaapi_device_init(const char *device, int *err)
{
...
int vaapi_decode_init(AVCodecContext *avctx)
{
    ...
    RING_DBG("Initializing VAAPI decoder");
