...
class VaapiAccel {
    public:
        ...
        void init(AVCodecContext* ctx) override;
        int getBuffer(AVFrame* frame, int flags) override;
        void retrieveData(AVFrame* frame) override;
...
int
getBuffer(AVFrame* frame, int flags)
{
//     RingHWContext *rhw = static_cast<RingHWContext*>(avctx->opaque);
//     VAAPIDecoderContext *ctx = static_cast<VAAPIDecoderContext*>(rhw->hwaccel_ctx);
//     int err;
...
{
    codecCtx_ = ctx;
    auto accelCtx_ = static_cast<HardwareAccelContext*>(codecCtx->opaque);
    AVVAAPIDeviceContext* hardwareCtx;
    AVVAAPIFramesContext* framesCtx;
    VAAPIDecoderContext* ctx;
    VAStatus status;
    int ret;

    // ctx->[field] are class members of this
    AVBufferRef* hardwareDevCtx = vaapiDeviceInit();
    deviceRef_ = av_buffer_ref(hardwareDevCtx);
    device_ = (AVHWDeviceContext*)deviceRef->data;
    ...
    frames_ = static_cast<AVVAAPIFramesContext*>(frames_->hwctx);

    status = vaCreateContext(hardwareCtx->display, vaConfig_,
                             decodeWidth_, decodeHeight_,
                             VA_PROGRESSIVE,
                             framesCtx->surface_ids, framesCtx->nb_surfaces,
                             &vaContext_);
