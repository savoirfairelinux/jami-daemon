...
VaapiAccel::init(AVCodecContext* codecCtx)
{
    ...
    deviceBufferRef_ = std::unique_ptr<AVBufferRef>(av_buffer_ref(hardwareDeviceCtx));
    auto device = static_cast<AVHWDeviceContext*>(deviceBufferRef_->data);
    ...
    if (status != VA_STATUS_SUCCESS) {
        RING_ERR("Failed to create VAAPI configuration: ", vaErrorStr(status));
        return false;
    }
    ...
    auto constraints = av_hwdevice_get_hwframe_constraints(deviceBufferRef_, hardwareConfig);
    ...
    framesBufferRef_ = std::unique_ptr<AVBufferRef>(av_hwframe_ctx_alloc(deviceBufferRef_.get()));
    auto frames = static_cast<AVHWFramesContext*>(framesBufferRef_->data);
    ...
    if (status != VA_STATUS_SUCCESS) {
        RING_ERR("Failed to create VAAPI context: ", vaErrorStr(status));
        return false;
    }
