...
void
VaapiAccel::init(AVCodecContext* avctx)
{
...
//
    hwctx = static_cast<AVVAAPIDeviceContext*>(device->hwctx);
//
...
//
    ...
    frames->width = decode_width;
    frames->height = decode_height;
    frames->initial_pool_size = decode_surfaces;
//
    if (av_hwframe_ctx_init(frames_ref) < 0)
//     if (err < 0) {
        RING_ERR("Failed to initialise VAAPI frame context");
//         goto fail;
//     }
//
    avfc = static_cast<AVVAAPIFramesContext*>(frames->hwctx);
//
...
//
    ...
    avctx->hwaccel_context = &decoder_vaapi_context;
//
...
int
VaapiAccel::getBuffer(AVCodecContext* avctx, AVFrame* frame, int flags)
{
...
int
VaapiAccel::retrieveData(AVCodecContext* avctx, AVFrame* input)
{
...
void
VaapiAccel::deviceInit(const char* device)
{
...
//
    if (!display) {
        ...
        throw std::runtime_error("No VA display found");
//         *err = AVERROR(EINVAL);
//         return NULL;
    }
//
    ...
    if (vas != VA_STATUS_SUCCESS) {
        ...
        throw std::runtime_error("Failed to initialise VAAPI connection");
//         *err = AVERROR(EIO);
//         return NULL;
    }
...
//
    ...
    if (!hw_device_ctx) {
        ...
        vaTerminate(display);
        throw std::runtime_error("Failed to create VAAPI hardware context");
//         *err = AVERROR(ENOMEM);
//         return NULL;
    }
//
...
//
    ...
    if (av_hwdevice_ctx_init(hw_device_ctx) < 0) {
        RING_ERR("Failed to initialise VAAPI hardware context");
        throw std::runtime_error("Failed to initialise VAAPI hardware context");
//         return NULL;
    }
//
...
void
VaapiAccel::config(AVCodecContext* avctx)
{
    const struct {
...
#undef MAP
    ...
    AVVAAPIDeviceContext *hwctx = static_cast<AVVAAPIDeviceContext*>(device->hwctx);
...
//
    ...
    if (!codec_desc) {
        RING_ERR("EINVAL at avcodec_descriptor_get");
//         err = AVERROR(EINVAL);
        goto fail;
    }
//
...
//
    vas = vaCreateConfig(hwctx->display, va_profile,
                         va_entrypoint, 0, 0, &va_config);
    if (vas != VA_STATUS_SUCCESS) {
        RING_ERR("Failed to create decode pipeline configuration: %d (%s)", vas, vaErrorStr(vas));
        // err = AVERROR(EIO);
        goto fail;
    }
//
    hwconfig = static_cast<AVVAAPIHWConfig*>(av_hwdevice_hwconfig_alloc(device_ref));
    if (!hwconfig) {
//         err = AVERROR(ENOMEM);
        goto fail;
    }
...
//
    constraints = av_hwdevice_get_hwframe_constraints(device_ref,
                                                      hwconfig);
    if (!constraints)
        goto fail;
//
...
//     // nicely based on the codec and input stream?
    decode_surfaces = DEFAULT_SURFACES;
//     // For frame-threaded decoding, one additional surfaces is needed for
//     // each thread.
    if (avctx->active_thread_type & FF_THREAD_FRAME)
        decode_surfaces += avctx->thread_count;
//
//     return 0;
//
fail:
    RING_ERR("Failed to configure VAAPI");
    av_hwframe_constraints_free(&constraints);
    av_freep(&hwconfig);
