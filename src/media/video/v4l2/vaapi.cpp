...
VaapiAccel::init(AVCodecContext* codecCtx)
{
    ...
    AVBufferRef* hardwareDevCtx = deviceInit("");
...
VaapiAccel::buildDecoderConfig()
{
    ...
    try {
        ...
        int nbSupportedProfiles = sizeof(vaapiProfileMap) / sizeof(vaapiProfileMap[0]);
        for (i = 0; i < nbSupportedProfiles; i++) {
            int profileMatch = 0;
