...
void destroyBufferRef(AVBufferRef* bufferRef) {
    av_buffer_unref(&bufferRef);
}

VaapiAccel::VaapiAccel(AccelInfo info) : HardwareAccel(info)
    , deviceBufferRef_(nullptr, [](AVBufferRef* buf){ av_buffer_unref(&buf); })
    , framesBufferRef_(nullptr, [](AVBufferRef* buf){ av_buffer_unref(&buf); })
{
}
...
VaapiAccel::init(AVCodecContext* codecCtx)
{
    ...
    deviceBufferRef_ = AVBufferRefPtr(av_buffer_ref(hardwareDeviceCtx),
    ...
    std::unique_ptr<VAProfile[]> profileList = std::unique_ptr<VAProfile[]>(new VAProfile[numProfiles * sizeof(VAProfile)]);
    status = vaQueryConfigProfiles(hardwareContext->display, profileList.get(), &numProfiles);
    ...
    framesBufferRef_ = AVBufferRefPtr(av_hwframe_ctx_alloc(deviceBufferRef_.get()),
        [](AVBufferRef* buf){ av_buffer_unref(&buf); });
