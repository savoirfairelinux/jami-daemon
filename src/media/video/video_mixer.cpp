                                 ...
                                 std::pair<int, int> bigCap)
{
    ...
    if (layout == Layout::ONE_BIG || layout == Layout::ONE_BIG_WITH_SMALL) {
        ...
        const SourceSpec* active = nullptr;
        ...
        if (layout == Layout::ONE_BIG_WITH_SMALL) {
            // The top preview line stays usable: each preview only gets a
            // 1/zoom-th of the surface (see calc_position), so a promoted
            // low-definition source must not collapse the whole composite
            // below what the previews can fill. Previews alone never push
            // the surface past the base cap.
            const int zoom = std::max(MIN_LINE_ZOOM, static_cast<int>(n));
            int cellW = 0, cellH = 0;
            for (const auto& s : sources) {
                if (s.active)
                    continue;
