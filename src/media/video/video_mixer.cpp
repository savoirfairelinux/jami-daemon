...
VideoMixer::render_frame(VideoFrame& output, const VideoFrame& input, int index)
{
    ...
    */
    ...
    size_t idx = index;
    if (index >= 0 && idx < videoRotationFilters_.size() && idx < rotations_.size()) {
        const constexpr char rotationFilterName[] = "mixin";
        if (angle != rotations_[idx]) {
            videoRotationFilters_[idx] = video::getTransposeFilter(angle, rotationFilterName, frame->width(), frame->height(), frame->format(), true);
            rotations_[idx] = angle;
        }
        if (videoRotationFilters_[idx]) {
            videoRotationFilters_[idx]->feedInput(frame->pointer(), rotationFilterName);
            frame = std::static_pointer_cast<VideoFrame>(std::shared_ptr<MediaFrame>(videoRotationFilters_[idx]->readOutput()));
        }
    }
