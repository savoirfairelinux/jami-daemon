                              ...
                              int index)
{
    ...
    if (input->getOrientation() % 180) {
        // Rotated frame
        zoomWidth = (float) input->height() / cellWidth;
        zoomHeight = (float) input->width() / cellHeight;
        frameHeight = std::round(input->width() / std::max(zoomWidth, zoomHeight));
        frameWidth = std::round(input->height() / std::max(zoomWidth, zoomHeight));
    } else {
        zoomWidth = (float) input->width() / cellWidth;
        zoomHeight = (float) input->height() / cellHeight;
        frameWidth = std::round(input->width() / std::max(zoomWidth, zoomHeight));
        frameHeight = std::round(input->height() / std::max(zoomWidth, zoomHeight));
    }
