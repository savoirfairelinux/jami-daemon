...
public:
    ...
    void setRecorderCallback(const std::function<void(const std::shared_ptr<VideoFrameActiveWriter>& vg, const MediaStream& ms)>& cb);
...
private:
    ...
    std::function<void(const std::shared_ptr<VideoFrameActiveWriter>& vg, const MediaStream& ms)> recorderCallback_;
};
