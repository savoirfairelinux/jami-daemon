        ...
        if (not conference_) {
            ...
            if (input) {
                videoLocal_->setRecorderCallback(
                    [this](std::shared_ptr<VideoFrameActiveWriter> vg, const MediaStream& ms) {
...
VideoRtpSession::startReceiver()
{
    ...
    if (receive_.enabled and not receive_.onHold) {
        ...
        receiveThread_->setRecorderCallback(
            [this](std::shared_ptr<VideoFrameActiveWriter> vg, const MediaStream& ms) { attachRecorder(vg, ms); });
...
void
VideoRtpSession::attachRecorder(std::shared_ptr<VideoFrameActiveWriter> vg, const MediaStream& ms)
{
    if (!recorder_)
        return;
