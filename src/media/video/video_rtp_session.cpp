...
VideoRtpSession::onAudioCongestion(float audioPacketLoss)
{
    unsigned int newBitrate;
    {
        ...
        // Do not clamp upward to videoBitrateMin — during audio congestion we
        // only want to reduce video bitrate, never raise it.
        newBitrate = std::min(newBitrate, oldBitrate);
