        ...
        if (not conference_) {
            ...
            if (input) {
                videoLocal_->setRecorderCallback(
                    [this](const std::shared_ptr<VideoFrameActiveWriter>& vg, const MediaStream& ms) {
                        attachRecorder(vg, ms);
                    });
...
VideoRtpSession::startReceiver()
{
    ...
    if (receive_.enabled and not receive_.onHold) {
        ...
        receiveThread_->setRecorderCallback(
            [this](const std::shared_ptr<VideoFrameActiveWriter>& vg, const MediaStream& ms) { attachRecorder(vg, ms); });
...
void
VideoRtpSession::attachRecorder(const std::shared_ptr<VideoFrameActiveWriter>& vg, const MediaStream& ms)
{
    if (!recorder_)
        return;
...
VideoRtpSession::deinitRecorder()
{
    ...
    if (videoLocal_) {
        if (auto ob = recorder_->getStream(videoLocal_->getInfo().name)) {
            videoLocal_->detach(ob);
        }
    }
}
