...
VideoRtpSession::decreaseBitrate(float packetLoss) {
    ...
    tableBitrate_[indexBitrate_].timeLastFailure = now;
...
VideoRtpSession::decreaseQuality(float packetLoss) {
    ...
    tableQuality_[indexQuality_].timeLastFailure = now;
    ...
    for (auto& qualityIt : tableQuality_) {
        if (qualityIt.state == QUALITY_KO) {
            auto elapsed_seconds = std::chrono::duration_cast<std::chrono::seconds>(
                std::chrono::system_clock::now() - qualityIt.timeLastFailure).count();

            //we consider that less than 2 failures are ok
            if ((qualityIt.statFailure <= 2) and (elapsed_seconds > 20)) {
                qualityIt.state = QUALITY_OK;
                RING_WARN("-> quality %u can be used (less than 2 failures last failure = %ld seconds ago) ",
                        qualityIt.val, elapsed_seconds);
            } else if ((qualityIt.statFailure != 0) and
                    (( qualityIt.statTotal / qualityIt.statFailure) >= 3) and
                    (elapsed_seconds > 20)) {
                RING_WARN("-> quality %u can be used (ratio ok, last_failure %lu seconds ago) ", qualityIt.val, elapsed_seconds);
                qualityIt.state = QUALITY_OK;
            }
        ...
        if (bitrateIt.state == QUALITY_KO) {
            ...
            if (bitrateIt.statFailure <= 2) {
                ...
                RING_WARN("-> bitrate %u can be used (less than 2 failures) ",
                        bitrateIt.val);
            } else if ((bitrateIt.statFailure != 0) and
                ((bitrateIt.statTotal / bitrateIt.statFailure) >= 3)) {
...
VideoRtpSession::dumpStat() {
    for (const auto& qualityIt : tableQuality_) {
        ...
        auto elapsed_seconds = std::chrono::duration_cast<std::chrono::seconds>(
            std::chrono::system_clock::now() - qualityIt.timeLastFailure).count();
        RING_ERR("quality: %u, last_failure_since = %ld", qualityIt.val, elapsed_seconds);
