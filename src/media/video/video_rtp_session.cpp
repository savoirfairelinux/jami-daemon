...
constexpr auto DELAY_AFTER_INCREASE = std::chrono::seconds(10);
...
VideoRtpSession::adaptQualityAndBitrate()
{
    ...
    JAMI_DBG("[AutoAdapt] pond loss: %f%, last loss: %f%, medium jitter: %dms" , pondLoss, rtcpi.packetLoss, rtcpi.jitter);

    // Fill histoLoss with samples
    if (restartTimer < DELAY_AFTER_RESTART + std::chrono::seconds(1)) {
        return;
    }

    auto jitterDev = getJitterDeviation(rtcpi.jitter);
    auto jitterAvg = getJitterAvg();
    auto oldBitrate = videoBitrateInfo_.videoBitrateCurrent;

    // Reduce bitrate if jitter average is low and jitter deviation high (that mean bitrate hit maximum bandwith)...
    // ... or if jitter average is too high and deviation is positive
    if ((jitterAvg < 50.0f && jitterDev > 30.0f) || (jitterAvg >= 50.0f && jitterDev > 5.0f)) {
        videoBitrateInfo_.videoBitrateCurrent *= 0.80f;
        histoJitter_.clear();
        JAMI_ERR("[AutoAdapt] Average Jitter: %fms, deviation Jitter: %fms, decrease bitrate from %d Kbps to %d Kbps, facteur %f", jitterAvg, jitterDev, oldBitrate, videoBitrateInfo_.videoBitrateCurrent, (float) videoBitrateInfo_.videoBitrateCurrent / oldBitrate);
    }
    else {
        ...
        if (pondLoss >= 10.0f && rtcpi.packetLoss > 0.0f) {
            ...
            histoLoss_.clear();
            JAMI_WARN("[AutoAdapt] pondLoss: %f%%, packet loss rate: %f%%, decrease bitrate from %d Kbps to %d Kbps, ratio %f", pondLoss, rtcpi.packetLoss, oldBitrate, videoBitrateInfo_.videoBitrateCurrent, (float) videoBitrateInfo_.videoBitrateCurrent / oldBitrate);
        }
        else { 
            ...
            JAMI_WARN("[AutoAdapt] pondLoss: %f%%, packet loss rate: %f%%, increase bitrate from %d Kbps to %d Kbps, facteur %f", pondLoss, rtcpi.packetLoss, oldBitrate, videoBitrateInfo_.videoBitrateCurrent, (float) videoBitrateInfo_.videoBitrateCurrent / oldBitrate);
            histoLoss_.clear();
        }
    }
...
float
VideoRtpSession::getJitterDeviation(unsigned lastJitter)
{
    if (histoJitter_.size() == 0)
        return 0.0f;

    float deviation = 0.0f;
    float mean = 0.0f;
    mean = getJitterAvg();
    deviation = lastJitter - mean;

    if (histoJitter_.size() >= MAX_SIZE_HISTO_JITTER_)
            histoJitter_.pop_front();
    histoJitter_.push_back(lastJitter);

    return deviation;
}

float
VideoRtpSession::getJitterAvg()
{
    if (histoJitter_.size() == 0)
        return 0.0f;

    return (float) std::accumulate(histoJitter_.begin(), histoJitter_.end(), 0) / histoJitter_.size();
}
