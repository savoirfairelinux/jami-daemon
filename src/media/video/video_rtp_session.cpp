...
VideoRtpSession::increaseBitrate() {
    ...
    if (indexBitrate_ < tableBitrate_.size()) {
        if (tableBitrate_[indexBitrate_ + 1].state != QUALITY_KO ) {
...
VideoRtpSession::decreaseBitrate(float packetLoss) {
    ...
    if (indexBitrate_ == 0) {
        RING_DBG("already reach lowest bitrate");
        return false;
    }
    ...
    auto jump = std::max((indexBitrate_ - step), (unsigned) 0);
...
VideoRtpSession::increaseQuality() {
    ...
    if (indexQuality_ == tableQuality_.size() - 1) {
        RING_DBG("already reach highest quality");
        return false;
    }


    auto distToMax = tableQuality_.size() - indexQuality_;

    if (distToMax <= tableQuality_.size() * 3/4)
        jump = std::min(indexQuality_ + 1, (unsigned) tableQuality_.size() - 1);
    else if (distToMax <= tableQuality_.size() * 2/4)
        jump = std::min(indexQuality_ + 2, (unsigned) tableQuality_.size() - 1);
    else if (distToMax <= tableQuality_.size() * 1/4)
        jump = std::min(indexQuality_ + 3, (unsigned) tableQuality_.size() - 1);
    else
        jump = std::min(indexQuality_ + 4, (unsigned) tableQuality_.size() - 1);

    RING_ERR("will perform a jump of %u", jump - indexQuality_);
...
VideoRtpSession::decreaseQuality(float packetLoss) {
    ...
    auto jump = std::max((indexQuality_ - step), (unsigned) 0);
    while ( indexQuality_ > jump) {
...
VideoRtpSession::updateQualityTable() {
    for (auto& qualityIt : tableQuality_) {
        if (qualityIt.state == QUALITY_KO) {
            ...
            RING_ERR("checking: quality %u [%u/%u]", qualityIt.val, qualityIt.statTotal, qualityIt.statFailure);
            if (qualityIt.statFailure <= 2) {
                qualityIt.state = QUALITY_OK;
                RING_ERR("put quality %u to OK (less than 2 failures) ", qualityIt.val);
#if 0
            } else if (( qualityIt.statTotal / qualityIt.statFailure) >= 2) {
                RING_ERR("put quality %u to OK (%u / %u >=3) ", qualityIt.val, qualityIt.statTotal, qualityIt.statFailure);
    ...
    for (auto& bitrateIt : tableBitrate_) {
        if (bitrateIt.state == QUALITY_KO) {
            ...
            if (bitrateIt.statFailure <= 2) {
                ...
                RING_ERR("put bitrate %u to OK (less than 3 failures) ", bitrateIt.val);
#if 0
            } else if (( bitrateIt.statTotal / bitrateIt.statFailure) >= 2) {
                RING_ERR("put bitrate %u to OK (%u / %u >=3) ", bitrateIt.val, bitrateIt.statTotal, bitrateIt.statFailure);
#endif
            }
        }
    }
...
VideoRtpSession::initBitrateAndQualityTable() {
    ...
    // create table of bitrate with a step of 100
    for (unsigned b = videoBitrateInfo_.videoBitrateMin; b <= videoBitrateInfo_.videoBitrateMax ; b+=100) {
