...
VideoRtpSession::onAudioCongestion(float audioPacketLoss)
{
    ...
    auto newBitrate = static_cast<unsigned int>(
        std::lround(static_cast<float>(oldBitrate) * reductionFactor));
    newBitrate = std::max(newBitrate, videoBitrateInfo_.videoBitrateMin);
