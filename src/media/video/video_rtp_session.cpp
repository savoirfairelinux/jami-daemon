...
VideoRtpSession::updateMedia(const MediaDescription& send, const MediaDescription& receive)
{
    ...
    if (codecVideo) {
        // In a conference the sent stream is the mixer composite, so size the
        // budget from the mixer surface; otherwise use the capture size.
        ...
        // Screen content (text, UI) needs roughly twice the bitrate of camera
        // video at the same resolution to stay legible.
        const bool isDisplay = !videoMixer_ && input_.rfind("display://", 0) == 0;
        ...
        try {
            ...
            if (videoMixer_) {
                ...
                if (codecVideo and not confBitrateSeeded_) {
                    const auto pixels = static_cast<unsigned>(ms.width * ms.height);
