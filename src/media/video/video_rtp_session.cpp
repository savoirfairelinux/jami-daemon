...
VideoRtpSession::onAudioCongestion(float audioPacketLoss)
{
    unsigned int newBitrate;
    {
        ...
        // Never raise above current, but enforce a safe floor so video doesn't
        // collapse to zero from repeated reductions.
        if (oldBitrate <= videoBitrateInfo_.videoBitrateMin)
            return;
