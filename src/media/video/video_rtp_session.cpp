...
VideoRtpSession::adaptQualityAndBitrate()
{
    ...
    if (checkMediumRCTPInfo(&rtcpi) == NO_INFO_CALCULATED) {
        JAMI_DBG("[AutoImprove] Sample not ready");
        return;
    }
    ...
    if (restartTimer < DELAY_AFTER_RESTART) {
        JAMI_DBG("[AutoImprove] Waiting for delay %ld ms", std::chrono::duration_cast<std::chrono::milliseconds>(restartTimer));
        return;
    }

    if (rtcpi.jitter > 5000) {
        JAMI_DBG("[AutoImprove] Jitter too high");
        return;
    }

    auto oldBitrate = videoBitrateInfo_.videoBitrateCurrent;

    if(histoLoss_.size() > 5)
        histoLoss_.pop_front();
    histoLoss_.push_back(std::make_pair(clock::now(), rtcpi.packetLoss));

    //Take action only when two successive drop superior to 1% are catched...
    //and when jitter is less than 5 seconds
    auto pondLoss = getPonderateLoss();
    JAMI_DBG("[AutoImprove] Ponderate packet loss rate: %f%, Last packet loss rate: %f%, Medium Jitter: %dms" , pondLoss, rtcpi.packetLoss, rtcpi.jitter);
    if(pondLoss >= 2.0f)
    {
        if(twoSuccesiveLosses_)  //Avoid pikes and residual drops after restarting the media
        {
            ...
            JAMI_DBG("[AutoImprove] packet loss >= 1 : decrease bitrate from %d Kbits to %d Kbits", oldBitrate, videoBitrateInfo_.videoBitrateCurrent);
        }
        else
    ...
    if(oldBitrate != videoBitrateInfo_.videoBitrateCurrent) {
        storeVideoBitrateInfo();
        JAMI_DBG("[AutoImprove] Restart media sender");
...
float
VideoRtpSession::getPonderateLoss()
{
    auto now = clock::now();
    ...
    for(auto& it : histoLoss_)
    {
        ...
        // 1ms      -> 100%
        // 2000ms   -> 1
        if(delay <= std::chrono::milliseconds(2000))
        {
            pond = std::min(-delay.count()/2000.0f + 1.0f, 1.0f);
            totalPond += pond;
