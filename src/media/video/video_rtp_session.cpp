...
void VideoRtpSession::startSender()
{
    if (send_.enabled and not send_.holding) {
        ...
        auto isAutoQualityEnabled = Manager::instance().videoPreferences.getAutoQuality();
...
VideoRtpSession::setupVideoBitrateInfo() {
    ...
    if (codecVideo) {
        ...
        videoBitrateInfo_.videoBitrateCurrent = videoBitrateInfo_.videoBitrateMin + (videoBitrateInfo_.videoBitrateMax - videoBitrateInfo_.videoBitrateMin) * (Manager::instance().videoPreferences.getBitrate() / 100);
        videoBitrateInfo_.videoQualityCurrent = videoBitrateInfo_.videoQualityMin - (videoBitrateInfo_.videoQualityMin - videoBitrateInfo_.videoQualityMax) * (Manager::instance().videoPreferences.getQuality() / 100);
    } else {
