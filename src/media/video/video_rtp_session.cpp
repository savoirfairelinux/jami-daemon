...
VideoRtpSession::adaptQualityAndBitrate()
{
    ...
    // Feed REMB signal to controller
    uint64_t br;
    if (check_RCTP_Info_REMB(&br)) {
        // Legacy REMB sentinels: 0x6803 = overuse, 0x7378 = normal/increase
        ...
        } else if (br > 0 && br < 100000) {
            // Real bitrate estimate from receiver (Kbps) — cap to it directly
            auto currentBitrate = bwController_.getTargetBitrate();
            if (br < currentBitrate) {
                bwController_.setTargetBitrate(static_cast<unsigned>(br));
                setNewBitrate(static_cast<unsigned>(br));
            }
        }
    }
    ...
    if (check_RCTP_Info_RR(rtcpi)) {
        // Guard: ignore stale RTCP reports arriving right after encoder restart
        auto sinceRestart = std::chrono::duration<float>(clock::now() - lastMediaRestart_).count();
        if (sinceRestart < 2.0f)
            return;
        // Guard: ignore pathological jitter values (codec bug or stale report)
        // Android reports larger jitter values that are expected; only guard on other platforms
#if !__ANDROID__
        if (rtcpi.jitter > 1000)
            return;
#endif
        auto newBitrate = bwController_.update(rtcpi.packetLoss, static_cast<float>(rtcpi.jitter), rtcpi.latency);
        setNewBitrate(newBitrate);
    }
}
...
VideoRtpSession::delayMonitor(int gradient, int deltaT)
{
    ...
    // Rate-limit REMB feedback to avoid flooding (max once per 500ms per direction)
    auto now = clock::now();
    static constexpr auto MIN_REMB_INTERVAL = std::chrono::milliseconds(500);

    if (bwState == BandwidthUsage::bwOverusing) {
        if (now - lastRembSent_ >= MIN_REMB_INTERVAL) {
            JAMI_LOG("[BandwidthCtrl] Reception overuse detected, sending REMB decrease");
            auto v = cc->createREMB(0x6803);
            socketPair_->writeData(v.data(), static_cast<int>(v.size()));
            lastRembSent_ = now;
        }
    } else if (bwState == BandwidthUsage::bwNormal) {
        if (now - lastRembSent_ >= std::chrono::seconds(1)) {
            auto v = cc->createREMB(0x7378);
            socketPair_->writeData(v.data(), static_cast<int>(v.size()));
            lastRembSent_ = now;
        }
    }
