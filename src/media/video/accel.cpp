...
int
HardwareAccel::encode(AVCodecContext* enc_ctx, AVFrame* frame, AVPacket* enc_pkt)
{
    // Send frame (decoded)
    int ret;
    ...
    while (1) {
        ...
        if (ret == AVERROR(EAGAIN) || ret == AVERROR_EOF)
            ...
            JAMI_ERR("Error during encoding");
            return -1;
        }

        av_packet_unref(enc_pkt);
    }
    JAMI_WARN("OK encode frame");
    return 0;
}

int
HardwareAccel::decode(AVCodecContext* dec_ctx)
{    
    AVFrame* frame = nullptr;
    AVFrame* sw_frame = nullptr;

    int ret = avcodec_send_packet(dec_ctx, test_AVPacket_);
    if (ret < 0) {
        JAMI_ERR("Error sending a packet for decoding\n");
        return -1;
    }
    JAMI_WARN("OK send packet");
    ...
    while (ret >= 0) {
        ...
        if (ret == AVERROR(EAGAIN) || ret == AVERROR_EOF) {
            ...
            JAMI_ERR("Error during decoding\n");
            return -1;
        }
        JAMI_WARN("OK received frame.");

        if ((ret = av_hwframe_transfer_data(sw_frame, frame, 0)) < 0) {
            JAMI_ERR("Error transferring the data to system memory");
            return -1;
        }
        JAMI_WARN("OK transferred frame");
    }
    JAMI_WARN("OK receive frame");
...
HardwareAccel::init_codec_ctx(AVCodecContext** actx, AVBufferRef* hw_device_ctx)
{
    ...
    // Link hw_device_ctx with the AVCodecContext
    ...
    if (type_ == CODEC_DECODER) {
        ...
        *actx = ctx;
        return 0;       // Ok for decoder -> continue the test
    }
...
void
HardwareAccel::close_codec_ctx(AVCodecContext* avctx, AVBufferRef* hw_device_ctx, AVFrame* sw_frame, AVFrame* hw_frame, AVPacket* pkt)
{
...
int
HardwareAccel::test_encode_black_frame(AVCodecContext* avctx, AVFrame* sw_frame, AVFrame* hw_frame, AVPacket* pkt)
{
    // Init sw frame
    ...
    // Encode test frame
    if ((err = encode(avctx, hw_frame, pkt)) < 0) {
        JAMI_DBG("Error while encoding frame."
                "Error code: %s.\n", libav_utils::getError(err).c_str());
        return -1;
    }
    JAMI_WARN("OK encoding frame");

    // Flush encoder
    encode(avctx, NULL, pkt);
                        ...
                        const char* device, int flags)
{
    ...
    // Link codec to device
    err = init_codec_ctx(&avctx, hw_device_ctx);
    if (err < 0) {                  // Fail
        JAMI_DBG("Failed to init hardware codec.");
        close_codec_ctx(avctx, hw_device_ctx, nullptr, nullptr, nullptr);
        return -1;
    } 
    else if (err == 0) {            // Decoding test
        err = decode(avctx);
        close_codec_ctx(avctx, hw_device_ctx, nullptr, nullptr, nullptr);
        if (err < 0) {
            JAMI_DBG("Failed to decode test packet.");
            return -1;
        }
    } else {                        // Encoding test
        ...
        err = test_encode_black_frame(avctx, sw_frame, hw_frame, enc_pkt);
        close_codec_ctx(avctx, hw_device_ctx, sw_frame, hw_frame, enc_pkt);
        if (err < 0) {
...
HardwareAccel::test_device_type(const HardwareAPI& api)
{
    ...
    for (unsigned j = 0; j < api.possible_devices.size(); j++) {
        JAMI_WARN("-- Starting %s test for %s with with device %s.", (type_ == CODEC_ENCODER) ? "encoding" : "decoding", name, api.possible_devices[j].c_str());
...
HardwareAccel::setupDecoder(AVCodecID id, int width, int height, AVPixelFormat pix_dec, AVPacket* pkt)
{
        ...
        if (it != api.supportedCodecs.end()) {
            ...
            if (avcodec_find_encoder_by_name(codecName.c_str())) {
                ...
                if(device != "" && accel->initDevice(device)) {
                    ...
                    // we don't need frame context for videotoolbox
                    if (api.format == AV_PIX_FMT_VIDEOTOOLBOX || link || accel->initFrame()) {
