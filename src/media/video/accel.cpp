...
#include <VideoToolbox/VideoToolbox.h> // to get output pixel format
...
transferFrameData(HardwareAccel accel, AVCodecContext* codecCtx, VideoFrame& frame)
{
    ...
    // most devices support YUV420P, but some AVFoundation devices don't
#ifndef __APPLE__
    output->format = AV_PIX_FMT_YUV420P;
#else
    auto pixbuf = (CVPixelBufferRef)src->data[3];
    auto pixelFormat = CVPixelBufferGetPixelFormatType(pixbuf);
    output->format = av_map_videotoolbox_format_to_pixfmt(static_cast<uint32_t>(pixelFormat));
#endif
