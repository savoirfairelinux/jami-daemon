...
getFormatCb(AVCodecContext* codecCtx, const AVPixelFormat* formats)
{
    ...
    RING_WARN("No hardware decoding possible");
...
static std::string
getDeviceName(AVBufferRef* hardwareDeviceCtx)
{
...
setupHardwareDecoding(AVCodecContext* codecCtx)
{
    ...
    for (auto info : accels) {
        if (std::find(info.supportedCodecs.begin(), info.supportedCodecs.end(), static_cast<AVCodecID>(codecCtx->codec_id)) != info.supportedCodecs.end()) {
            ...
            auto deviceName = getDeviceName(hardwareDeviceCtx);
            if (av_hwdevice_ctx_create(&hardwareDeviceCtx, hwType, deviceName.c_str(), nullptr, 0) >= 0) {
                codecCtx->hw_device_ctx = av_buffer_ref(hardwareDeviceCtx);
                codecCtx->get_format = getFormatCb;
                codecCtx->thread_safe_callbacks = 1;
                if (!deviceName.empty())
                    RING_DBG("Attempting to use '%s' hardware acceleration using device '%s'", info.name.c_str(), deviceName.c_str());
                else
