...
{
    // each item in this array represents an implemented hardware acceleration
    // enclose each item in an ifdef clause to prevent its compilation on a wrong platform
    // this will also prevent possibly hard to debug errors
    // ex: it makes no sense to have vaapi (linux intel) on a mac
    // when to add an item: when support for an acceleration has been added to Ring
    // if it does not yet exist, add an AccelID for the given acceleration
    // each accel has an AVPixelFormat associated to it by FFmpeg (check pixfmt.h)
    // the name is what will be printed in the daemon logs, so choose something that will
    // help devs easily figure out which acceleration is causing trouble
    // makeHardwareAccel is the factory function, it should specify the class that
    // inherits HardwareAccel
    // it is important to #include the accel's header in this file
    // wrap the include statement in the same ifdef as in this array
