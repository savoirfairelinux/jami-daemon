...
HardwareAccel::HardwareAccel(AccelInfo info)
{
    type_ = info.type;
    format_ = info.format;
    name_ = info.name;
...
const HardwareAccel::AccelInfo
HardwareAccel::getAccel(const HardwareAccel::AccelID* codecAccels, int size)
{
...
HardwareAccel::setupAccel(AVCodecContext* codecCtx)
{
    ...
    if (info.type != HardwareAccel::AccelID::NoAccel) {
...
getFormatCb(AVCodecContext* codecCtx, const AVPixelFormat* formats)
{
    ...
    for (int i = 0; formats[i] != AV_PIX_FMT_NONE; i++) {
        const auto desc = av_pix_fmt_desc_get(formats[i]);
...
allocateBufferCb(AVCodecContext* codecCtx, AVFrame* frame, int flags)
{
    ...
    if (accel && !accel->hasFailed()) {
        auto ret = accel->allocateBuffer(codecCtx, frame, flags);
        if (!ret) {
            accel->reset();
            return ret;
        }
