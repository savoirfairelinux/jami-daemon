...
int
HardwareAccel::decode(AVCodecContext* dec_ctx, AVPacket* pkt, AVFrame* frame)
{    
                        ...
                        const char* device, int flags)
{
    ...
    if (err < 0) {
        ...
        if (!(hw_frame = av_frame_alloc()) || !(pkt = av_packet_alloc())) {
            JAMI_DBG("Can not alloc frame and packet");
            return -1;
        }
        err = decode(avctx, pkt, hw_frame);
        close_codec_ctx(avctx, hw_device_ctx, sw_frame, hw_frame, pkt);
        if (err < 0) {
