...
setupHardwareDecoding(AVCodecContext* codecCtx)
{
    ...
    const HardwareAccel accels[] = {
        { "vaapi", true, AV_PIX_FMT_VAAPI, { AV_CODEC_ID_H264, AV_CODEC_ID_MPEG4, AV_CODEC_ID_H263, AV_CODEC_ID_VP8, AV_CODEC_ID_MJPEG } },
        { "vdpau", true, AV_PIX_FMT_VDPAU, { AV_CODEC_ID_H264, AV_CODEC_ID_MPEG4, AV_CODEC_ID_H263 } },
        { "videotoolbox", true, AV_PIX_FMT_VIDEOTOOLBOX, { AV_CODEC_ID_H264, AV_CODEC_ID_MPEG4, AV_CODEC_ID_H263 } },
    };
...
setupHardwareEncoding(AVCodecContext** codecCtx, AVCodec** codec)
{
    ...
    //select if initialize device based on the given API
    std::map<std::string,bool>map_init_device;
    map_init_device["vaapi"]={true};
    map_init_device["videotoolbox"]={true};
    map_init_device["omx"]={false};
	
    auto it = map_hw_encoders.find((*codecCtx)->codec_id);
    if (it != map_hw_encoders.end()) {
        for (unsigned int i=0 ; i < it->second.size() ; i++){
            RING_WARN("Found encoder with %s, attempt to initialize the device.", it->second[i].c_str());
            if ((*codec = avcodec_find_encoder_by_name((it->second[i]).c_str()))){
                ...
                *codecCtx = encoderCtx;
                ...
                auto it_id = map_init_device.find(accel.name);
                accel.init_device = it_id->second;
                encoderCtx->pix_fmt = it_pf->second;
                //No need to initialize device if using OpenMAX (Raspberry Pi)
                if (accel.init_device == false)
					return accel;
                if (initDevice(accel, *codecCtx) >= 0) {
