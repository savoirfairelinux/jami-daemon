...
HardwareAccel::test_device_type(std::string& dev)
{
    ...
    for (unsigned j = 0; j < possible_devices_.size(); j++) {
        JAMI_WARN("-- Starting %s test for %s with with device %s.", (type_ == CODEC_ENCODER) ? "encoding" : "decoding", name, possible_devices_[j].c_str());
        err = test_device(name, possible_devices_[j].c_str(), 0);
        if (err == 0) {
            JAMI_DBG("-- Test passed for %s with device %s.",
                    name, possible_devices_[j].c_str());
            dev = possible_devices_[j];
            return 0;
        }
        else {
...
HardwareAccel::setupDecoder(AVCodecID id, int width, int height)
{
    for (const auto& api : apiListDec) {
        if (std::find(api.supportedCodecs.begin(), api.supportedCodecs.end(), id) != api.supportedCodecs.end()) {
            ...
            accel->width_ = width;
            std::string device;
            auto ret = accel->test_device_type(device);
            if(ret == 0) {
                // we don't need frame context for videotoolbox
                if (api.format == AV_PIX_FMT_VIDEOTOOLBOX || accel->initFrame()) {
                    JAMI_WARN() << "Using hardware decoder " << accel->getCodecName() << " with " << api.name << ", " << device <<  " device";
                    return accel;
                }
            }
        }
...
HardwareAccel::initAPI(bool linkable, AVBufferRef* framesCtx)
{
    ...
    if(ret == 0) {
        ...
        if (format_ == AV_PIX_FMT_VIDEOTOOLBOX || link || initFrame()) {
            JAMI_WARN() << "Using hardware accel " << codecName << " with " << name_ << ", " << device <<  " device";
            return 0;
        }
    }
    return -1;
...
HardwareAccel::getAccelCompatible(AVCodecID id, int width, int height, CodecType type)
{
    ...
    for (auto api : apiListEnc) {
        ...
        if (it != api.supportedCodecs.end()) {
            ...
            if (avcodec_find_encoder_by_name(accel.getCodecName().c_str())) {
                l.push_back(std::move(accel));
            }
        }
    }
    return l;
