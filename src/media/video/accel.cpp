...
{
    int ret = -1; // set to 0 on allocateBuffer success
    if (auto accel = static_cast<HardwareAccel*>(codecCtx->opaque))
        if (!accel->hasFailed())
            if ((ret = accel->allocateBuffer(frame, flags)) < 0)
                accel->fail(false);
...
{
    ++accelFails_;
    if (accelFails_ >= MAX_ACCEL_FAILURES || forceFallback) {
...
HardwareAccel::extractData(VideoFrame& input)
{
    ...
    accelFails_ = 0; // reset failure count on success
    return true;
}
