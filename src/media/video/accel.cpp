...
const char*
HardwareAccel::getDeviceTypeName() const
{
    if (const auto* name = av_hwdevice_get_type_name(hwType_))
        return name;
    return "unknown";
}

const char*
HardwareAccel::getFormatName() const
{
    if (const auto* name = av_get_pix_fmt_name(format_))
        return name;
    return "unknown";
}
...
getFormatCb(AVCodecContext* codecCtx, const AVPixelFormat* formats)
{
    ...
    for (int i = 0; formats[i] != AV_PIX_FMT_NONE; ++i) {
        if (accel && formats[i] == accel->getFormat()) {
            ...
            JAMI_LOG("Found compatible hardware format for {} decoder with {}",
...
HardwareAccel::init_device(const char* name, const char* device, int flags)
{
    ...
    if (dev->type != hwType_) {
        JAMI_LOG("Device created as type {} has type {}.",
...
HardwareAccel::init_device_type(std::string& dev)
{
    ...
    const char* name = av_hwdevice_get_type_name(hwType_);
    if (!name) {
        JAMI_LOG("No name available for device type {}.", static_cast<int>(hwType_));
        return -1;
    }

    AVHWDeviceType check = av_hwdevice_find_type_by_name(name);
    if (check != hwType_) {
        JAMI_LOG("Type {} maps to name {} maps to type {}.", static_cast<int>(hwType_), name, static_cast<int>(check));
        return -1;
    }

    JAMI_WARNING("-- Starting {} init for {} with default device.",
    ...
    for (auto& device : *possible_devices_) {
        if (device.second == DeviceState::NOT_USABLE)
            continue;
        JAMI_WARNING("-- Init {} for {} with device {}.",
                     (type_ == CODEC_ENCODER) ? "encoding" : "decoding",
                     name,
                     device.first);
...
HardwareAccel::transfer(const VideoFrame& frame)
{
    ...
    if (type_ == CODEC_DECODER) {
        ...
        if (input->format != format_) {
            JAMI_ERROR("Frame format mismatch: expected {}, got {}",
        ...
        if ((ret = av_hwframe_get_buffer(framesCtx_, hwFrame, 0)) < 0) {
            JAMI_ERROR("Failed to allocate hardware buffer: {}", libav_utils::getError(ret));
            return nullptr;
        }
        ...
        if ((ret = av_hwframe_transfer_data(hwFrame, input, 0)) < 0) {
            JAMI_ERROR("Failed to push frame to GPU: {}", libav_utils::getError(ret));
            return nullptr;
        }
