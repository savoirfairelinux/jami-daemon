...
#if defined(HAVE_VAAPI_X11) || defined(HAVE_VAAPI_DRM)
...
allocateBufferCb(AVCodecContext* codecCtx, AVFrame* frame, int flags)
{
    if (auto accel = static_cast<HardwareAccel*>(codecCtx->opaque))
        return accel->allocateBuffer(codecCtx, frame, flags);
    // invalid state, try to recover
...
// int
// HardwareAccel::allocateBuffer(AVCodecContext* codecCtx, AVFrame* frame, int flags)
// {
//     // default implementation
//     // most just end up returning av_hwframe_get_buffer
//     // add fail call
// }

// bool
// HardwareAccel::extractData(AVCodecContext* codecCtx, VideoFrame& container)
// {
//     // default implementation
//     // av_hwframe_transfer_data followed by av_frame_copy_props
// }
