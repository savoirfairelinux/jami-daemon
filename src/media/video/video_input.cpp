...
VideoInput::configureFilePlayback(const std::string& path, std::shared_ptr<MediaDemuxer>& demuxer, int index)
{
    ...
    decoder->skipFrames();
    decoder->setMaxNumberOfFrames(MAX_NUMBER_OF_VIDEO_FRAMES);
    decoder->setMinNumberOfFrames(MIN_NUMBER_OF_VIDEO_FRAMES);
...
void
VideoInput::setNeedFrameCb(std::function<void()> cb)
{
    if (decoder_) {
