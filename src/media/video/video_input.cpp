...
VideoInput::initLinuxGrab(const std::string& display)
{
    ...
    // full screen or window sharing:  pipewire fd:23 node:68
    ...
    if (display.find("pipewire") != std::string::npos) {
        size_t fdPos = display.find("fd:") + 3;
        size_t nodePos = display.find("node:") + 5;
        p.fd = display.substr(fdPos, nodePos - fdPos - 6);
