...
void VideoInput::process()
{
    ...
    for (auto& buffer : buffers_) {
        if (buffer.status == BUFFER_FULL && buffer.index == publish_index_) {
            ...
            if (decOpts_.orientation != 0) {
                if (decOpts_.orientation != rotation_) {
                    setRotation(decOpts_.orientation);
                    rotation_ = decOpts_.orientation;
                }
                auto localFDB = frameDataBuffer_.load();
                av_frame_new_side_data_from_buf(frame.pointer(), AV_FRAME_DATA_DISPLAYMATRIX, localFDB);
            }
...
VideoInput::setRotation(int angle)
{
    ...
    localFrameDataBuffer = frameDataBuffer_.exchange(localFrmeDataBuffer);
