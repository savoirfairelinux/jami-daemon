...
class HardwareAccelProxy {
    public:
        ...
        using AccelCreator = std::unique_ptr<HardwareAccel> (*)();
        template <typename T>
        static std::unique_ptr<HardwareAccel> make() { return std::unique_ptr<T>(new T()); }
    ...
    private:
        std::unique_ptr<HardwareAccel> hardwareAccel_;
        int autoDetect_ = 0;
