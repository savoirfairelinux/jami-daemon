                   ...
                   const std::shared_ptr<MediaFrame>& frame_p)
{
    ...
    if (doTransfer) {
        ...
        if (y + height <= frame->pointer()->height) {
            frame->pointer()->crop_top = y;
            frame->pointer()->crop_bottom = frame->pointer()->height - y - height;
        }
        if (x + width <= frame->pointer()->width) {
            frame->pointer()->crop_left = x;
            frame->pointer()->crop_right = frame->pointer()->width - x - width;
        }
        if (height && width)
