                   ...
                   const std::shared_ptr<MediaFrame>& frame_p)
{
    ...
    if (doTransfer) {
        ...
        auto& swFrame = f;
#endif
        AVFrameSideData* side_data = av_frame_get_side_data(swFrame.pointer(), AV_FRAME_DATA_DISPLAYMATRIX);
        if (side_data) {
            ...
            if (!std::isnan(angle))
                setRotation(angle);
            if (filter_) {
                filter_->feedInput(swFrame.pointer(), FILTER_INPUT_NAME);
                auto filtered_frame = filter_->readOutput();
                if (filtered_frame) {
                    auto frame_ptr = filtered_frame->pointer();
                    swFrame.setFromMemory(frame_ptr->data[0], frame_ptr->format,
                                    frame_ptr->width, frame_ptr->height);
                    if (frame_ptr->height != height_ || frame_ptr->width != width_) {
                        setFrameSize(0, 0);
                        setFrameSize(frame_ptr->width, frame_ptr->height);
                    }
                }
            }
        }
