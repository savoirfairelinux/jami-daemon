...
ShmHolder::renderFrame(const VideoFrame& src) noexcept
{
    ...
    if (!resizeArea(frameSize)) {
        JAMI_ERROR("[ShmHolder:{}] Unable to resize area size: {}x{}, format: {}", openedName_, width, height, format);
        return;
    }
...
libjami::FrameBuffer
SinkClient::configureFrameDirect(const std::shared_ptr<jami::MediaFrame>& frame_p)
{
...
SinkClient::update(Observable<std::shared_ptr<MediaFrame>>* /*obs*/, const std::shared_ptr<MediaFrame>& frame_p)
{
    ...
    if (hasDirectListener) {
        notify(frame_p);
        auto outFrame = configureFrameDirect(frame_p);
        if (!outFrame) {
            return;
        }
