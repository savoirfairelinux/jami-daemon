...
const std::string
SinkClient::getId(size_t len) const noexcept
{
    if (len == 0 or len >= id_.size()) {
        return id_;
    } else {
        return id_.substr(0, len);
    }
}
...
SinkClient::setFrameSize(int width, int height)
{
    ...
    if (width > 0 and height > 0) {
        JAMI_DBG("[Sink <%s/%s>] Started - size=%dx%d, mixer=%s",
                 getId().c_str(),
                 openedName().c_str(),
                 width,
                 height,
        ...
        JAMI_DBG("[Sink <%s/%s>] Stopped - size=%dx%d, mixer=%s",
                 getId().c_str(),
                 openedName().c_str(),
                 width,
                 height,
