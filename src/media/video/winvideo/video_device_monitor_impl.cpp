...
VideoDeviceMonitorImpl::run()
{
    ...
    while (probing_) {
        ...
        auto deviceNodes = monitor_->getDeviceNodes();
        for (auto node : deviceNodes) {
            auto it = std::find(currentDeviceList.begin(), currentDeviceList.end(), node);
            if (it != currentDeviceList.end()) {
                currentDeviceList.erase(it);
            } else {
                RING_DBG() << "Removing device: " << node;
                monitor_->removeDevice(node);
            }
        }
        for (auto node : currentDeviceList) {
            RING_DBG() << "Adding device: " << node;
