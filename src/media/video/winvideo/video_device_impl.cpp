            ...
            if(deviceCounter == this->id)
            {
                ...
                if (SUCCEEDED(hr))
                {
                    ...
                    if (SUCCEEDED(hr))
                    {
                        ...
                        hr = captureGraph_->FindInterface(&PIN_CATEGORY_PREVIEW, &MEDIATYPE_Video, videoInputFilter_, IID_IAMStreamConfig, (void **)&streamConf_);
                        if(FAILED(hr))
                        {
        ...
        if (done && SUCCEEDED(hr)) {
            ...
            for (int i = 0; i < piCount; i++) {
                ...
                if (pmt->formattype == FORMAT_VideoInfo) {
                    ...
                    rateList_[sizeList_.back()].push_back(std::to_string(1e7 / pSCC.MinFrameInterval));
                    rateList_[sizeList_.back()].push_back(std::to_string(1e7 / pSCC.MaxFrameInterval));
...
VideoDeviceImpl::applySettings(VideoSettings settings)
{
    if (!settings.video_size.empty()) {
        AM_MEDIA_TYPE *pmt = capMap_[settings.video_size];
        ((VIDEOINFOHEADER*) pmt->pbFormat)->AvgTimePerFrame = settings.framerate;
        if (FAILED(streamConf_->SetFormat(capMap_[settings.video_size]))) {
...
VideoDeviceImpl::getSettings() const
{
    ...
    if (SUCCEEDED(hr)) {
        if (pmt->formattype == FORMAT_VideoInfo) {
            VIDEOINFOHEADER *videoInfo = (VIDEOINFOHEADER*) pmt->pbFormat;
            settings.video_size = std::to_string(videoInfo->bmiHeader.biWidth) + "x" + std::to_string(videoInfo->bmiHeader.biHeight);
