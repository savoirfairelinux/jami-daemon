 ...
 namespace ring { namespace video {
    ...
    for (int i = 0; i < piCount; i++) {
        ...
        if (pmt->formattype == FORMAT_VideoInfo) {
            ...
            rateList_[sizeList_.back()].push_back(std::to_string((double)videoInfo->AvgTimePerFrame / 1000000));
            std::cerr << "SIZE " << sizeList_.back() << std::endl;
...
VideoDeviceImpl::applySettings(VideoSettings settings)
{
    if (!settings.video_size.empty()) {
        streamConf_->SetFormat(capMap_[settings.video_size]);
    }
}
...
VideoDeviceImpl::getSettings() const
{
    ...
    if (SUCCEEDED(hr)) {
        if (pmt->formattype == FORMAT_VideoInfo) {
            ...
            settings.framerate = videoInfo->AvgTimePerFrame / 1000000;
        }
    }
    return settings;
}
