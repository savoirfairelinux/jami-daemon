...
// maximum number of times accelerated decoding can fail in a row before falling
// back to software
const constexpr unsigned MAX_ACCEL_FAILURES{5};

MediaDecoder::MediaDecoder()
    : inputCtx_(avformat_alloc_context()), startTime_(AV_NOPTS_VALUE), tfi{} {}
...
int MediaDecoder::openInput(const DeviceParams &params) {
  ...
  if (params.framerate) {
    ...
    if (params.framerate.denominator() != 4999998)
      av_dict_set(&options_, "framerate", jami::to_string(framerate).c_str(),
                  0);
#else
    av_dict_set(&options_, "framerate",
                jami::to_string(params.framerate.real()).c_str(), 0);
#endif
  }

  if (params.offset_x || params.offset_y) {
    av_dict_set(&options_, "offset_x", std::to_string(params.offset_x).c_str(),
                0);
    av_dict_set(&options_, "offset_y", std::to_string(params.offset_y).c_str(),
                0);
  }
  if (params.channel)
    av_dict_set(&options_, "channel", std::to_string(params.channel).c_str(),
                0);
  ...
  av_dict_set(&options_, "reorder_queue_size",
              std::to_string(jitterBufferMaxSize_).c_str(), 0);
  ...
  JAMI_DBG("Trying to open device %s with format %s, pixel format %s, size "
           "%dx%d, rate %lf",
           params.input.c_str(), params.format.c_str(),
           params.pixel_format.c_str(), params.width, params.height,
  ...
  int ret = avformat_open_input(&inputCtx_, params.input.c_str(), iformat,
                                options_ ? &options_ : NULL);

  if (ret) {
    JAMI_ERR("avformat_open_input failed: %s",
...
int MediaDecoder::selectStream(AVMediaType type) {
  return av_find_best_stream(inputCtx_, type, -1, -1, &inputDecoder_, 0);
}

int MediaDecoder::setupStream(AVMediaType mediaType) {
  int ret = 0;
  ...
  if (!fallback_) {
    ...
    if ((ret = avformat_find_stream_info(inputCtx_, nullptr)) < 0) {
      // Always fail here
      JAMI_ERR() << "Could not find " << streamType
                 << " stream info: " << libav_utils::getError(ret);
      return -1;
    }
  }
  ...
  decoderCtx_->thread_count =
      std::max(1u, std::min(8u, std::thread::hardware_concurrency() / 2));

  if (emulateRate_)
    JAMI_DBG() << "Using framerate emulation";
  startTime_ =
      av_gettime(); // used to set pts after decoding, and for rate emulation

#ifdef RING_ACCEL
  if (mediaType == AVMEDIA_TYPE_VIDEO) {
    if (enableAccel_) {
      accel_ = video::HardwareAccel::setupDecoder(
          decoderCtx_->codec_id, decoderCtx_->width, decoderCtx_->height);
      if (accel_) {
...
MediaDecoder::Status MediaDecoder::decode(VideoFrame &result) {
  AVPacket inpacket;
  ...
  if (frameFinished) {
    ...
    frame->pts = av_rescale_q_rnd(
        av_gettime() - startTime_, {1, AV_TIME_BASE}, decoderCtx_->time_base,
        static_cast<AVRounding>(AV_ROUND_NEAR_INF | AV_ROUND_PASS_MINMAX));
    lastTimestamp_ = frame->pts;

    if (emulateRate_ and packetTimestamp != AV_NOPTS_VALUE) {
      auto frame_time =
          getTimeBase() * (packetTimestamp - avStream_->start_time);
      auto target =
          startTime_ + static_cast<std::int64_t>(frame_time.real() * 1e6);
      auto now = av_gettime();
      if (target > now) {
...
MediaDecoder::Status MediaDecoder::decode(AudioFrame &decodedFrame) {
  ...
  if (frameFinished) {
    ...
    frame->pts = av_rescale_q_rnd(
        frame->pts, avStream_->time_base, decoderCtx_->time_base,
        static_cast<AVRounding>(AV_ROUND_NEAR_INF | AV_ROUND_PASS_MINMAX));
    lastTimestamp_ = frame->pts;
...
#ifdef ENABLE_VIDEO
#ifdef RING_ACCEL
void MediaDecoder::enableAccel(bool enableAccel) {
...
MediaDecoder::Status MediaDecoder::flush(VideoFrame &result) {
  AVPacket inpacket;
  av_init_packet(&inpacket);
...
int MediaDecoder::correctPixFmt(int input_pix_fmt) {
...
MediaStream MediaDecoder::getStream(std::string name) const {
