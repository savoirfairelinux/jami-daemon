...
int MediaDecoder::setupFromVideoData()
{
    ...
    // TODO set to 1 if using hardware acceleration? -> yes
    ...
    // gives same error message than none:
    //rhw->hwaccel_pix_fmt = AV_PIX_FMT_VAAPI;
...
MediaDecoder::decode(VideoFrame& result)
{
    ...
    bool should_retrieve = (frame->format == rhw->hwaccel_retrieved_pix_fmt);
