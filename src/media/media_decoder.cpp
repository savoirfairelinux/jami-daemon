    ...
    if (enableAccel_ and not fallback_) {
        ...
        for (const auto& it : APIs) {
            ...
            if (!decoderSupportsHardwareAccel(inputDecoder_, *accel_)) {
                JAMI_LOG("Skipping hardware decoder for {} with {}: decoder '{}' exposes no matching FFmpeg hw config", avcodec_get_name(avStream_->codecpar->codec_id), it.getName(), inputDecoder_->name);
                accel_.reset();
                continue;
            }
            if (prepareDecoderContext() < 0)
            ...
            if (avcodec_open2(decoderCtx_, inputDecoder_, &options_) < 0) {
                ...
                JAMI_WARNING("Fail to open hardware decoder for {} with {}", avcodec_get_name(decoderCtx_->codec_id), it.getName());
                avcodec_free_context(&decoderCtx_);
                decoderCtx_ = nullptr;
                accel_.reset();
                continue;
                ...
                JAMI_WARNING("Using hardware decoding for {} with {}", avcodec_get_name(decoderCtx_->codec_id), it.getName());
                break;
            }
        }
    }
