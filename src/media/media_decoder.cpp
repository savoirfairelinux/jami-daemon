...
{
    if (streamInfoTimer_) {
        // Cancel pending waits so handlers won't fire after destruction
    ...
    if (not streamInfoFound_) {
        ...
        if (videoStream && keyFrameRequestCb_) {
            ...
            if (streamInfoTimer_) {
                ...
                std::weak_ptr<MediaDemuxer> weak = weak_from_this();
                streamInfoTimer_->async_wait([weak](const std::error_code& ec) {
                    if (ec)
                        return;
