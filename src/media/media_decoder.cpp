...
MediaDemuxer::decode()
{
    if (exceedFramesLimit_) {
        return Status::ExceedBufferLimit;
    }
...
void
MediaDecoder::setNeedFrameCb( std::function<void()> cb)
{
...
MediaDecoder::emitNewFrame()
{
    ...
    if ((framesBuffer.size() < minNumberOfFrames_) &&
        (demuxer_->getCurrentState() != MediaDemuxer::CurrentState::Finished) && needFrameCb_) {
        needFrameCb_();
...
MediaDecoder::decode(AVPacket& packet)
{
    ...
    if (frameFinished) {
        ...
        if (syncToOtherStream_) {
            {
                std::lock_guard<std::mutex> lk {framesBufferMutex};
                if (framesBuffer.size() < maxNumberOfFrames_) {
                    framesBuffer[target_time] = f;
                } else {
                    demuxer_->setExceedLimit(true);
                }
            }
