...
MediaDecoder::openInput(const DeviceParams& params)
{
    ...
    RING_DBG() << "Trying to open device " << params.input << " with format " << params.format << ", pixel format " 
            << params.pixel_format << ", size " << params.width << "x" << params.height << ", rate " << params.framerate.real();
    ...
    if (ret) {
        RING_ERR() << "avformat_open_input failed: " << libav_utils::getError(ret);
    } else {
        RING_DBG() << "Using format " << params.format;
    }
...
int
MediaDecoder::setupFromAudioData()
{
...
#ifdef RING_VIDEO
int
MediaDecoder::setupFromVideoData()
{
...
MediaDecoder::decode(VideoFrame& result)
{
    ...
    if (ret == AVERROR(EAGAIN)) {
        ...
        RING_ERR() << "Couldn't read frame: " << libav_utils::getError(ret) << "\n";
