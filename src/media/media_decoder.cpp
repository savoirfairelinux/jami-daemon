...
int MediaDecoder::openInput(const DeviceParams& params)
{
    ...
    av_dict_set(&options_, "reorder_queue_size",ring::to_string(JITTER_BUFFER_MAX_SIZE_).c_str(), 0);
    av_dict_set(&options_, "max_delay",ring::to_string(JITTER_BUFFER_MAX_DELAY_).c_str(), 0);
