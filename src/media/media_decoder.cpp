...
MediaDemuxer::decode()
{
    ...
    JAMI_WARN("MediaDemuxer::decode() %d for stream %d", ret, packet->stream_index);
    ...
    if (streamIndex >= streams_.size() || streamIndex < 0) {
        JAMI_WARN("MediaDemuxer::decode() no callback");
        return Status::Success;
    }
...
MediaDecoder::decode(AVPacket& packet)
{
    ...
    if (frameFinished) {
        ...
        JAMI_WARN("decode: frameFinished");
        if (callback_)
