...
MediaDemuxer::decode()
{
    ...
    int ret = av_read_frame(inputCtx_, packet.get());
    JAMI_INFO() << this << " " << ((inputCtx_->streams[0]->codecpar->height >> 3) << 3) << "x"
                << ((inputCtx_->streams[0]->codecpar->width >> 3) << 3);
    JAMI_INFO() << this << " " << inputParams_.height << "x" << inputParams_.width;
    JAMI_INFO() << this << " " << ret;
    if (ret == AVERROR(EAGAIN)) {
...
MediaDecoder::decode(AVPacket& packet)
{
    ...
    JAMI_INFO() << this << " decoderCtx_ " << decoderCtx_->height << "x" << decoderCtx_->width;
    JAMI_INFO() << this << " decoded frame " << frame->height << "x" << frame->width;
