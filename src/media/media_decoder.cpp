...
#ifdef RING_ACCEL
        if (decoderCtx && decoderCtx->hw_device_ctx)
...
{
    return decode(result.pointer(), currentStreamIndex_);
}
#endif // RING_VIDEO
...
{
    return decode(decodedFrame.pointer(), currentStreamIndex_);
}
...
{
    auto decoderCtx = decoders_[currentStreamIndex_];
    auto ms = MediaStream("", decoderCtx, lastTimestamp_);
#ifdef RING_ACCEL
