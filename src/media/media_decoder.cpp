...
decoderSupportsHardwareAccel(const AVCodec* decoder, const video::HardwareAccel& accel)
{
    ...
    constexpr auto supportedMethods = AV_CODEC_HW_CONFIG_METHOD_HW_DEVICE_CTX | AV_CODEC_HW_CONFIG_METHOD_HW_FRAMES_CTX;
    ...
    if (enableAccel_ and not fallback_) {
        ...
        for (const auto& it : APIs) {
            ...
            if (!decoderSupportsHardwareAccel(inputDecoder_, *accel_)) {
                JAMI_LOG("Skipping hardware decoder for {} with {}: decoder '{}' exposes no matching FFmpeg hw config",
