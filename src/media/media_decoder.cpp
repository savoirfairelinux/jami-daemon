...
int MediaDecoder::setupFromAudioData(const AudioFormat format)
{
    ...
    return setupStream("audio");
}

int
MediaDecoder::setupStream(std::string streamType)
{
    int ret = 0;
    AVMediaType mediaType = (streamType == "video") ? AVMEDIA_TYPE_VIDEO : AVMEDIA_TYPE_AUDIO;
    ...
    if (streamIndex_ < 0 || !inputDecoder_) {
        RING_ERR() << "No " << streamType << " stream for selected codec";
        return -1;
    }
