...
MediaDemuxer::decode()
{
    ...
    if (inputParams_.format == "x11grab") {
        ...
        JAMI_INFO() << "\n\n";
        if (inputParams_.height < inputCtx_->streams[0]->codecpar->height
            || inputParams_.width < inputCtx_->streams[0]->codecpar->width) {
            inputParams_.height = inputCtx_->streams[0]->codecpar->height;
            inputParams_.width = inputCtx_->streams[0]->codecpar->width;
