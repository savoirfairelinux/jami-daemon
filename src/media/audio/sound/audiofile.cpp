    ...
    , updatePlaybackScale_(0)
{
    ...
    auto decoder = std::make_unique<MediaDecoder>(
        [&r, this, &buf, &total_samples](const std::shared_ptr<MediaFrame>& frame) mutable {
            ...
            buf.emplace_back(r.resample(std::move(autoFrame), format));
        });
    DeviceParams dev;
    dev.input = fileName;
    dev.name = fileName;
    ...
    buffer_->format = format.sampleFormat;
    buffer_->sample_rate = format.sample_rate;
    av_channel_layout_default(&buffer_->ch_layout, format.nb_channels);
    av_frame_get_buffer(buffer_.get(), 0);

    size_t outPos = 0;
    for (auto& frame : buf) {
        av_samples_copy(buffer_->data, frame->pointer()->data, outPos, 0, frame->getFrameSize(), format.nb_channels, format.sampleFormat);
        outPos += frame->getFrameSize();
    }
    auto end = std::chrono::steady_clock::now();
    auto duration = end-start;
    JAMI_LOG("AudioFile: loaded {} samples ({}) as {} in {} from {:s}", total_samples, std::chrono::duration<double>(total_samples/(double)format.sample_rate), format.toString(), dht::print_duration(duration), fileName);
}
