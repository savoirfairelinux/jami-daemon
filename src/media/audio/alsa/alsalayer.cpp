...
void AlsaLayer::playback()
{
    ...
    framesToGet = Manager::instance().getRingBufferPool().availableForGet(RingBufferPool::DEFAULT_ID);
    ...
    if (framesToGet <= 0) {
        ...
        playbackBuff_.setFormat(audioFormat_);
        playbackBuff_.resize(playbackAvailFrames);
        ...
        unsigned maxNbFramesToGet = playbackAvailFrames;

        if (resample) {
            resampleFactor = static_cast<double>(audioFormat_.sample_rate) / mainBufferFormat.sample_rate;
            maxNbFramesToGet = playbackAvailFrames / resampleFactor;
        }
