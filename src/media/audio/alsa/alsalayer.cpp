...
AlsaLayer::openDevice(snd_pcm_t** pcm, const std::string& dev, snd_pcm_stream_t stream, AudioFormat& format)
{
    ...
    if (err < 0) {
        JAMI_ERROR("Alsa: Unable to open {} device {} : {}", (stream == SND_PCM_STREAM_CAPTURE)    ? "capture"
                 : (stream == SND_PCM_STREAM_PLAYBACK) ? "playback"
                                                       : "ringtone", dev, snd_strerror(err));
        return false;
    }
...
#define ALSA_CALL(call, error) \
    ({ \
        int err_code = call; \
        if (err_code < 0) \
            JAMI_ERROR(error ": %s", snd_strerror(err_code)); \
        err_code; \
    })
...
#undef HW
    ...
    JAMI_LOG("{} using format {}", (snd_pcm_stream(pcm_handle) == SND_PCM_STREAM_PLAYBACK) ? "playback" : "capture", format.toString());
...
AlsaLayer::write(const AudioFrame& buffer, snd_pcm_t* handle)
{
    ...
    case -EIO: {
        ...
        ALSA_CALL(snd_pcm_writei(handle, (const void*) buffer.pointer()->data[0], buffer.pointer()->nb_samples),
        ...
        if (snd_ctl_open(&handle, name.c_str(), 0) == 0) {
            if (snd_ctl_card_info(handle, info) == 0) {
                ...
                if ((err = snd_ctl_pcm_info(handle, pcminfo)) < 0) {
                    JAMI_WARNING("Unable to get info for {} {}: {}", getCapture ? "capture device" : "playback device", name, snd_strerror(err));
                } else {
                    JAMI_LOG("card {} : {} [{}]", numCard, snd_ctl_card_info_get_id(info), snd_ctl_card_info_get_name(info));
...
AlsaLayer::soundCardIndexExists(int card, AudioDeviceType stream)
{
    ...
    snd_pcm_info_set_stream(pcminfo,
                            stream == AudioDeviceType::PLAYBACK ? SND_PCM_STREAM_PLAYBACK : SND_PCM_STREAM_CAPTURE);
