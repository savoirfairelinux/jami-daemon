...
static constexpr const char* PA_EC_SUFFIX {"/.echo-cancel(\\..+)?$/g"};
#else
...
const PaDeviceInfos* PulseLayer::getDeviceInfos(const std::vector<PaDeviceInfos>& list, const std::string& name, const std::string& defaultName) const
{
...
#ifdef USE_PCRE_REGEX
    const char *error;
    const static int resSize = 3;
    int erroffset, resPos[resSize] = {0};
    pcre* re = pcre_compile(PA_EC_SUFFIX, 0, &error, &erroffset, NULL);
    if (re == NULL) {
        RING_ERR("PCRE compilation failed at offset %d: %s\n", erroffset, error);
        return deviceName;
    }
    int rc = pcre_exec(re, NULL, deviceName.c_str(), deviceName.size(), 0, 0, resPos, resSize);
    if (rc > 0) {
        int start = resPos[0];
        int length = resPos[1];
        deviceName.replace(start, length, "");
        RING_ERR("%s", deviceName.c_str());
    } else {
        RING_ERR("NO RESULT %s", deviceName.c_str());
    }
    pcre_free(re);
    return deviceName;
#else
...
#endif
    /*return endsWith(deviceName, PA_EC_SUFFIX)
          ? deviceName.substr(0, deviceName.size() - PA_EC_SUFFIX.size())
          : deviceName;*/
}
...
void PulseLayer::waitForDeviceList()
{
    ...
    streamStarter_ = std::thread([this]() mutable {
        ...
        if (!playback_ || stripEchoSufix(playback_->getDeviceName()) != getDeviceInfos(sinkList_, getPreferredPlaybackDevice(), defaultSink_)->name
         || !record_   || stripEchoSufix(record_->getDeviceName())   != getDeviceInfos(sourceList_, getPreferredCaptureDevice(), defaultSource_)->name) {
