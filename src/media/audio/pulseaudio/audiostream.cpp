...
AudioStream::moved(pa_stream* s)
{
    ...
    if (audioType_ == AudioDeviceType::CAPTURE) {
        ...
        if (name == NULL) {
            JAMI_ERR("[audiostream] unable to get audio stream device");
            return;
        }

        JAMI_WARN("[audiostream] capture device moved: %s", name);
        ...
        auto* op = pa_context_get_source_info_by_name(
            ...
            [](pa_context* c, const pa_source_info* i, int eol, void* userdata) {
                ...
                JAMI_INFO("[audiostream] source info found for stream: %s, index: %d, driver: %s",
                          i->name,
                          i->index,
                          i->driver);
                bool usingEchoCancel = std::string_view(i->driver) == "module-echo-cancel.c";
                if (usingEchoCancel) {
                    JAMI_INFO("[audiostream] stream %s is using echo cancel!", i->name);
                } else {
                    JAMI_INFO("[audiostream] stream %s is not using echo cancel", i->name);
                }
