...
AudioLayer::checkAEC()
{
    ...
    if (not echoCanceller_ and shouldSoftAEC) {
        ...
        auto frameSize = sample_rate / 100u;
#if HAVE_WEBRTC_AP
        echoCanceller_.reset(new WebRTCEchoCanceller(format, frameSize));
#else
        echoCanceller_.reset(new NullEchoCanceller(format, frameSize));
#endif
