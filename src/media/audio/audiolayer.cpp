...
void AudioLayer::hardwareFormatAvailable(AudioFormat playback, size_t bufSize)
{
    ...
    if (bufSize) {
        mainRingBuffer_->setFrameSize(bufSize);
    }
}
...
{
    if (not echoState_ and not hasNativeAEC_ and playbackStarted_ and recordStarted_) {
        JAMI_WARN("Starting AEC");
        echoState_.reset(new EchoState(audioFormat_, nativeFrameSize_, audioFormat_.sample_rate / 4));
    } else if (not hasNativeAEC_ or not playbackStarted_ or not recordStarted_) {
