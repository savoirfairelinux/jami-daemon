...
HRESULT
CLoopbackCapture::InitializeLoopbackCapture()
{
...
HRESULT
CLoopbackCapture::ActivateAudioInterface(DWORD processId, bool includeProcessTree)
{
...
//
//  Callback implementation of ActivateAudioInterfaceAsync function.  This will be called on MTA
//  thread when results of the activation are available.
//
HRESULT
CLoopbackCapture::ActivateCompleted(IActivateAudioInterfaceAsyncOperation* operation)
{
    m_activateResult = SetDeviceStateErrorIfFailed([&]() -> HRESULT {
        ...
        m_CaptureFormat.nBlockAlign = m_CaptureFormat.nChannels * m_CaptureFormat.wBitsPerSample
                                      / BITS_PER_BYTE;
        m_CaptureFormat.nAvgBytesPerSec = m_CaptureFormat.nSamplesPerSec
...
HRESULT
CLoopbackCapture::StartCaptureAsync(DWORD processId,
...
//
HRESULT
CLoopbackCapture::OnStartCapture(IMFAsyncResult* pResult)
{
    return SetDeviceStateErrorIfFailed([&]() -> HRESULT {
        // Start the capture
        ...
        MFPutWaitingWorkItem(m_SampleReadyEvent.get(),
                             0,
...
//
HRESULT
CLoopbackCapture::StopCaptureAsync()
{
    RETURN_HR_IF(E_NOT_VALID_STATE,
                 (m_DeviceState != DeviceState::Capturing) && (m_DeviceState != DeviceState::Error));

    m_DeviceState = DeviceState::Stopping;

    RETURN_IF_FAILED(
        MFPutWorkItem2(MFASYNC_CALLBACK_QUEUE_MULTITHREADED, 0, &m_xStopCapture, nullptr));
...
//
HRESULT
CLoopbackCapture::OnStopCapture(IMFAsyncResult* pResult)
{
...
//
HRESULT
CLoopbackCapture::FinishCaptureAsync()
{
...
//
HRESULT
CLoopbackCapture::OnFinishCapture(IMFAsyncResult* pResult)
{
...
//
HRESULT
CLoopbackCapture::OnSampleReady(IMFAsyncResult* pResult)
{
    if (SUCCEEDED(OnAudioSampleRequested())) {
        if (m_DeviceState == DeviceState::Capturing) {
            return MFPutWaitingWorkItem(m_SampleReadyEvent.get(),
                                        0,
                                        m_SampleReadyAsyncResult.get(),
                                        &m_SampleReadyKey);
        }
...
//
HRESULT
CLoopbackCapture::OnAudioSampleRequested()
{
    ...
    // If this flag is set, we have already queued up the async call to finalize capture
    // So we don't want to grab or process any more data
    ...
    while (SUCCEEDED(m_AudioCaptureClient->GetNextPacketSize(&FramesAvailable))
           && FramesAvailable > 0) {
        // Get sample buffer
        RETURN_IF_FAILED(m_AudioCaptureClient->GetBuffer(&Data,
                                                         &FramesAvailable,
                                                         &dwCaptureFlags,
                                                         &u64DevicePosition,
                                                         &u64QPCPosition));
...
HRESULT
CLoopbackCapture::GetParameters(DWORD* pdwFlags, DWORD* pdwQueue)
{
...
HRESULT
CLoopbackCapture::Invoke(IMFAsyncResult* pAsyncResult)
{
    // Handle the asynchronous callback
    return OnAudioSampleRequested();
}
