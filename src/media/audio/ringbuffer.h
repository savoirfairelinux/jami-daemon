...
// Change to using StreamId from VideoMixer.h but itmay not work for audio input rbufs??
...
public:
     ...
     */
    void createReadOffset(const std::string& ringbuffer_id);

    void createReadOffset(const std::string& ringbuffer_id, FrameCallback cb);

    /**
     * Remove a readoffset for this ringbuffer
     */
    void removeReadOffset(const std::string& ringbuffer_id);
     ...
     * @return AudioFrame 
     */
    std::shared_ptr<AudioFrame> get(const std::string& ringbuffer_id);
    ...
    size_t getLength(const std::string& ringbuffer_id) const;
     ...
     */
    size_t waitForDataAvailable(const std::string& ringbuffer_id,
...
private:
     ...
     */
    size_t getReadOffset(const std::string& ringbuffer_id) const;

    /**
     * Move readoffset forward by offset
     */
    void storeReadOffset(size_t offset, const std::string& ringbuffer_id);

    /**
     * Test if readoffset coresponding to this call is still active
     */
    bool hasThisReadOffset(const std::string& ringbuffer_id) const;
