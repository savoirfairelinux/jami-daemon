...
typedef std::pair<std::string /*callId or AUDIO_LAYER_ID*/, std::string /*stream name*/> RingBufferId;
...
public:
     ...
     */
    RingBuffer(const RingBufferId& id, size_t size, AudioFormat format = AudioFormat::MONO());
    ...
    const RingBufferId& getId() const { return id; }
     ...
     */
    void createReadOffset(const RingBufferId& ringbuffer_id);

    void createReadOffset(const RingBufferId& ringbuffer_id, FrameCallback cb);

    /**
     * Remove a readoffset for this ringbuffer
     */
    void removeReadOffset(const RingBufferId& ringbuffer_id);
     ...
     * @return AudioFrame 
     */
    std::shared_ptr<AudioFrame> get(const RingBufferId& ringbuffer_id);
    ...
    size_t getLength(const RingBufferId& ringbuffer_id) const;
     ...
     *
     * @param ringbuffer_id The read offset for which data should be available.
     ...
     * @return available data for ringbuffer_id after the call returned (same as calling getLength(ringbuffer_id) ).
     */
    size_t waitForDataAvailable(const RingBufferId& ringbuffer_id,
...
private:
    ...
    using ReadOffsetMap = std::map<RingBufferId, ReadOffset>;
     ...
     */
    size_t getReadOffset(const RingBufferId& ringbuffer_id) const;

    /**
     * Move readoffset forward by offset
     */
    void storeReadOffset(size_t offset, const RingBufferId& ringbuffer_id);

    /**
     * Test if readoffset coresponding to this call is still active
     */
    bool hasThisReadOffset(const RingBufferId& ringbuffer_id) const;
