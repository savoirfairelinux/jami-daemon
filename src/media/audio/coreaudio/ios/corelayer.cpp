...
void
CoreLayer::initAudioLayerIO(StreamType stream)
{
    ...
    bool setUpOutput = stream == StreamType::DEFAULT || stream == StreamType::PLAYBACK;
    bool setUpInput = stream == StreamType::DEFAULT || stream == StreamType::CAPTURE;
    if (setUpOutput) {
        setupOutputBus();
    }
    if (setUpInput) {
...
void
CoreLayer::startStream(StreamType stream)
{
