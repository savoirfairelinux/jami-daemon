...
void
CoreLayer::initAudioLayerIO(const std::string& stream)
{
    ...
    bool setUpOutput = stream.empty() || stream == PLAYBACK_STREAM;
    bool setUpInput = stream.empty() || stream == CAPTURE_STREAM;
    if (setUpOutput) {
        setupOutputBus();
    }
    if (setUpInput) {
