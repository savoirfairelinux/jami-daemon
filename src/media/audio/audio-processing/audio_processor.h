...
public:
    ...
    {
        outputResampler_->setCompensation(0, compensationDistance());
    }
    ...
    virtual void putPlayback(const std::shared_ptr<AudioFrame>& buf)
    {
        ...
        outputResampler_->setCompensation((int) std::lround(driftCompensator_.correction() * distance), distance);
        auto compensated = std::make_shared<AudioFrame>(format_);
        if (outputResampler_->resample(buf->pointer(), compensated->pointer()) < 0)
            return;
        auto previousInput = std::llround(playbackInputSamples_);
        playbackInputSamples_ += (double) buf->pointer()->nb_samples * format_.sample_rate
                                 / buf->pointer()->sample_rate;
        auto in = std::llround(playbackInputSamples_) - previousInput;
...
private:
    ...
    double playbackInputSamples_ {0};
};
