...
// webrtc-audio-processing >= 1.0 (AudioProcessing::Config / AEC3 API).
// The 0.3.x API lives in webrtc_legacy.cpp.
    ...
    : AudioProcessor(format.withSampleFormat(AV_SAMPLE_FMT_FLTP), webrtcFrameSize(format))
{
    ...
    if (WebRtcVad_ValidRateAndFrameLength((int) format_.sample_rate, frameSize_) == 0) {
        ...
        JAMI_WARNING("[webrtc-ap] {:d} Hz unsupported by the WebRTC VAD, voice activity disabled",
                     format_.sample_rate);
    }
}
...
WebRTCAudioProcessor::getProcessed()
{
    ...
    if (detectVoice_ && vad_) {
        ...
        hasVoice = WebRtcVad_Process(vad_.get(),
