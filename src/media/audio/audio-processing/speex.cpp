...
SpeexAudioProcessor::getProcessed()
{
    ...
    for (int channel = 0; channel < format_.nb_channels; channel++) {
        procData[channel] = iProcBuffer.getChannel(channel)->data();
    }

    // overall voice activity, to be boolean OR'd with each channel's
    bool vad = false;
    ...
    for (auto& channelPreprocessorState : preprocessorStates) {
        ...
        // if (debugCount == 0) {
        //     JAMI_INFO("channel %d: channelVad = %d", channel, channelVad);
        // }
    ...
    // if (debugCount >= 100) {
    //     debugCount = 0;
    //     JAMI_INFO("vad = %s", vad ? "yes" : "no");
    // }

    // add the voice activity to the returned AudioFrame
    processed->has_voice = vad;

    // if there's a change in the voice activation status
    if (lastVoiceState != vad) {
        JAMI_INFO("voice now %s", vad ? "active" : "inactive");
        lastVoiceState = vad;
    }
