...
private:
    ...
    public:
        ...
        // Start a new segment: same slope, new offset
        void split();
        bool empty() const { return first_ < 0; }
        double first() const { return first_; }
        double segmentStart() const { return segmentStart_; }
        double slope() const { return varT_ > 0 ? cov_ / varT_ : 0; }
        double at(double t) const { return meanV_ + slope() * (t - meanT_); }

    private:
        double tau_;
        // Weight and means of the current segment
        double weight_ {0};
        double meanT_ {0};
        double meanV_ {0};
        // Pooled over all segments
        double varT_ {0};
        double cov_ {0};
        double first_ {-1};
        double segmentStart_ {0};
        double last_ {0};
    };

    struct Stream
    {
        explicit Stream(double tau)
            : fit(tau)
        {}
        LinearFit fit;
        double count {0};
        // Time of the first samples, negative until then
        double start {-1};
        double last {-1};
        // Mean square distance to the fit, in samples²
        double jitter {0};
        // Since when the stream is far from its fit, negative if it is not
        double outlierStart {-1};
    };

    double seconds(clock::time_point t);
    void add(Stream& stream, double t, size_t samples);
