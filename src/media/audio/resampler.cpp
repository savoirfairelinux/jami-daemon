...
Resampler::Resampler(const unsigned targetFrameSize)
    : swrCtx_(swr_alloc())
    , initCount_(0)
    , targetFrameSize_(targetFrameSize)
{}
...
Resampler::resample(const AVFrame* input, AVFrame* output)
{
    ...
    if (firstFrame) {
        ...
        auto targetOutputLength = (targetFrameSize_ ? targetFrameSize_
                                                    : av_rescale_rnd(input->nb_samples,
                                                                     output->sample_rate,
                                                                     input->sample_rate,
                                                                     AV_ROUND_UP));
        if (output->nb_samples < targetOutputLength) {
            if (!targetFrameSize_)
                JAMI_WARNING("Target frame size not set for resampler, calculated target frame size: {}",
                             targetOutputLength);
            else
                JAMI_DEBUG("Target frame size set for resampler: {}", targetOutputLength);
