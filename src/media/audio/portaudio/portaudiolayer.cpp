                                                    ...
                                                    PaStreamCallbackFlags statusFlags)
{
    ...
    if (resample) {
        auto sample_factor = static_cast<double>(mainBufferFormat.sample_rate) / parent.audioInputFormat_.sample_rate;
...
static void
openStreamDevice(PaStream** stream, PaDeviceIndex device,
                 PaStreamCallback* callback, void* user_data)
{
    ...
    params.channelCount = device_info->maxOutputChannels;
