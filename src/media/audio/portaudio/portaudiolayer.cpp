                                                     ...
                                                     PaStreamCallbackFlags statusFlags)
{
    ...
    const auto& ringBuff = getToRing(parent.audioFormat_, framesPerBuffer);
    const auto& playBuff = getToPlay(parent.audioFormat_, framesPerBuffer);
    auto toPlay = ringBuff ? ringBuff : playBuff;
    if (!toPlay)
        return;

    std::memcpy(outputBuffer, toPlay->pointer()->extended_data[0], toPlay->pointer()->nb_samples * sizeof(AudioSample));
                                                    ...
                                                    PaStreamCallbackFlags statusFlags)
{
    ...
    std::memcpy(inBuff->extended_data[0], inputBuffer, framesPerBuffer * parent.audioInputFormat_.getBytesPerFrame());
