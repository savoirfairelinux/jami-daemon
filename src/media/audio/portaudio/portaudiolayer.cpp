...
#include "audio/wasapi/wasapiloopbackcapture.h"
...
PortAudioLayer::startCaptureStream(const std::string& id)
{
    ...
    HRESULT hr = pimpl_->loopbackCapture_.StartCaptureAsync(
        ...
        [this, ringBuffer](const BYTE* audioData,
            ...
            auto capturedFrame = std::make_shared<AudioFrame>(AudioFormat {format.nSamplesPerSec,
                                                                           format.nChannels,
            ...
            std::memcpy(capturedFrame->pointer()->data[0],
                        audioData,
                        framesAvailable * format.nChannels * (format.wBitsPerSample / 8));
