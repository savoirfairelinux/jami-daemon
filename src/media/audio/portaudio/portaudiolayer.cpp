...
#include <audio/audio-loopback/LoopbackCapture.h>
...
PortAudioLayer::startCaptureStream(const std::string& id)
{
    ...
    const int targetPid = to_int<int>(id);
    auto& rbPool = Manager::instance().getRingBufferPool();
    auto ringBuffer = rbPool.createRingBuffer(id);

    JAMI_DBG("Initializing Capture for Input Stream");
...
PortAudioLayer::PortAudioLayerImpl::initInputStream(PortAudioLayer& parent)
{
    ...
    JAMI_DBG("Starting Capture Stream");
    parent.startCaptureStream("16140");
