    ...
    , pimpl_ {new PortAudioLayerImpl(*this, pref)}
{
    ...
    pimpl_->audioDeviceNotificationClient_->setDeviceEventCallback(
        [this](const std::string& deviceName, const DeviceEventType event) {
            ...
            if (deviceInUse) {
                JAMI_DBG("PortAudio device event for device in use: %s, event: %s",
                         deviceName.c_str(),
                         (event == DeviceEventType::BecameActive) ? "BecameActive"
                                                                  : "BecameInactive");

                // Determine if it's a playback, capture, or ringtone device
                AudioDeviceType deviceType = AudioDeviceType::ALL;

                if (pimpl_->getDeviceNameByType(pimpl_->getIndexByType(AudioDeviceType::PLAYBACK),
                                                AudioDeviceType::PLAYBACK)
                    == deviceName) {
                    deviceType = AudioDeviceType::PLAYBACK;
                } else if (pimpl_->getDeviceNameByType(pimpl_->getIndexByType(
...
PortAudioLayer::startStream(AudioDeviceType stream)
{
    ...
    case AudioDeviceType::ALL:
        ...
        // Try ringtone separately regardless of playback success
        // This ensures ringtone is treated independently
        pimpl_->initRingtoneStream(*this);
        break;
