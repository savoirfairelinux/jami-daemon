...
AudioBuffer::toAVFrame() const
{
    ...
    const int bytesNeeded = av_samples_get_buffer_size(nullptr, channels(), frames(), fmt, 0);
    if (bytesNeeded < 0) {
        RING_ERR() << "Failed to compute buffer size";
        return nullptr;
    }

    std::vector<AudioSample> samples(bytesNeeded / sizeof(AudioSample));
    ...
    frame = av_frame_clone(frame); // FIXME hack
