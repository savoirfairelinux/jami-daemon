...
size_t AudioBuffer::deinterleave(AudioSample** out) const
{
    for (unsigned c = 0, n = channels(), f = frames(); c < n; ++c)
        for (unsigned i = 0; i < f; ++i)
            out[c][i] = samples_[c][i];
...
{
    const constexpr AVSampleFormat fmt = AV_SAMPLE_FMT_S16P; // non interleaved
...
//        for (int i = 0; i < frame->linesize[c]; i += 2) {
//            AudioSample buf = samples_[c][i/2];
//            AudioSample avf = (frame->data[c][i+1] << 8) + frame->data[c][i];
//            if (buf != avf)
