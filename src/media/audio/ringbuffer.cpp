...
RingBuffer::RingBuffer(const RingBufferId& rbuf_id, size_t /*size*/, AudioFormat format)
    : id(rbuf_id)
    , endPos_(0)
    , format_(format)
    , lock_()
...
void
RingBuffer::flush(const RingBufferId& ringbuffer_id)
{
    storeReadOffset(endPos_, ringbuffer_id);
}
...
size_t
RingBuffer::getLength(const RingBufferId& ringbuffer_id) const
{
    const size_t buffer_size = buffer_.size();
    if (buffer_size == 0)
        return 0;
    return (endPos_ + buffer_size - getReadOffset(ringbuffer_id)) % buffer_size;
}
...
size_t
RingBuffer::getReadOffset(const RingBufferId& ringbuffer_id) const
{
    auto iter = readoffsets_.find(ringbuffer_id);
...
void
RingBuffer::storeReadOffset(size_t offset, const RingBufferId& ringbuffer_id)
{
    ReadOffsetMap::iterator iter = readoffsets_.find(ringbuffer_id);

    if (iter != readoffsets_.end())
        iter->second.offset = offset;
    else
        JAMI_ERR("RingBuffer::storeReadOffset() failed: unknown call '%s'", ringbuffer_id.c_str());
}

void
RingBuffer::createReadOffset(const RingBufferId& ringbuffer_id)
{
    std::lock_guard<std::mutex> l(lock_);
    if (!hasThisReadOffset(ringbuffer_id))
        readoffsets_.emplace(ringbuffer_id, ReadOffset {endPos_, {}});
}

void
RingBuffer::removeReadOffset(const RingBufferId& ringbuffer_id)
{
    std::lock_guard<std::mutex> l(lock_);
    auto iter = readoffsets_.find(ringbuffer_id);
    if (iter != readoffsets_.end())
...
bool
RingBuffer::hasThisReadOffset(const RingBufferId& ringbuffer_id) const
{
    return readoffsets_.find(ringbuffer_id) != readoffsets_.end();
}
...
size_t
RingBuffer::availableForGet(const RingBufferId& ringbuffer_id) const
{
    // Used space
    return getLength(ringbuffer_id);
}

std::shared_ptr<AudioFrame>
RingBuffer::get(const RingBufferId& ringbuffer_id)
{
    std::lock_guard<std::mutex> l(lock_);

    auto offset = readoffsets_.find(ringbuffer_id);
...
size_t
RingBuffer::waitForDataAvailable(const RingBufferId& ringbuffer_id, const time_point& deadline) const
{
    std::unique_lock<std::mutex> l(lock_);

    if (buffer_.empty())
        return 0;
    if (readoffsets_.find(ringbuffer_id) == readoffsets_.end())
        return 0;
    ...
    auto check = [=, &getl] {
        ...
        const size_t buffer_size = buffer_.size();
        const auto read_ptr = readoffsets_.find(ringbuffer_id);
...
size_t
RingBuffer::discard(size_t toDiscard, const RingBufferId& ringbuffer_id)
{
    std::lock_guard<std::mutex> l(lock_);
