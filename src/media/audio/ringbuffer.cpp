...
void
RingBuffer::flush(const std::string& ringbuffer_id)
{
...
size_t
RingBuffer::getLength(const std::string& ringbuffer_id) const
{
    const size_t buffer_size = buffer_.size();
    if (buffer_size == 0)
        return 0;
...
size_t
RingBuffer::getReadOffset(const std::string& ringbuffer_id) const
{
...
void
RingBuffer::storeReadOffset(size_t offset, const std::string& ringbuffer_id)
{
...
void
RingBuffer::createReadOffset(const std::string& ringbuffer_id)
{
...
void
RingBuffer::removeReadOffset(const std::string& ringbuffer_id)
{
    std::lock_guard<std::mutex> l(lock_);
    auto iter = readoffsets_.find(ringbuffer_id);
    if (iter != readoffsets_.end())
...
bool
RingBuffer::hasThisReadOffset(const std::string& ringbuffer_id) const
{
...
size_t
RingBuffer::availableForGet(const std::string& ringbuffer_id) const
{
    // Used space
...
RingBuffer::get(const std::string& ringbuffer_id)
{
    std::lock_guard<std::mutex> l(lock_);
...
size_t
RingBuffer::waitForDataAvailable(const std::string& ringbuffer_id, const time_point& deadline) const
{
...
size_t
RingBuffer::discard(size_t toDiscard, const std::string& ringbuffer_id)
{
    std::lock_guard<std::mutex> l(lock_);
