...
PipeWireStream::PipeWireStream(pw_core* core, const char* name, AudioDeviceType type, const PwDeviceInfo& dev_info, std::function<void(pw_buffer* buf)>&& onData, std::function<void(const AudioFormat&)>&& onFormatChange, std::function<void()>&& onReady)
    : ready_(false)
    , type_(type)
...
void PipeWireStream::start() 
{
    ...
    //pw_core* core = pw_stream_get_core(stream_);
...
{
    struct spa_pod *iter;
    /*SPA_POD_STRUCT_FOREACH(value, iter) {
        printPodValue(iter);
    }*/
...
void printPodValue(const spa_pod* value)
{
    ...
    case SPA_TYPE_Bool:
        ...
        JAMI_LOG("PipeWireStream: prop value: {}", b);
        break;
    case SPA_TYPE_Int:
        spa_pod_get_int(value, &val);
        JAMI_LOG("PipeWireStream: prop value: {:d}", val);
        break;
    case SPA_TYPE_Long:
        spa_pod_get_long(value, &valuel);
        JAMI_LOG("PipeWireStream: prop value: {:d}", valuel);
        break;
    case SPA_TYPE_Float:
        spa_pod_get_float(value, &f);
        JAMI_LOG("PipeWireStream: prop value float: {:f}", f);
        break;
    case SPA_TYPE_Double:
        spa_pod_get_double(value, &d);
        JAMI_LOG("PipeWireStream: prop value double: {:f}", d);
        break;
    case SPA_TYPE_String:
        spa_pod_get_string(value, &str);
        JAMI_LOG("PipeWireStream: prop value str: {:s}", str);
        break;
    case SPA_TYPE_Bytes:
        JAMI_LOG("PipeWireStream: prop value: bytes");
        break;
    case SPA_TYPE_Rectangle:
        JAMI_LOG("PipeWireStream: prop value: rectangle");
        break;
    case SPA_TYPE_Fraction:
    JAMI_LOG("PipeWireStream: prop value: fraction");
    break;
    case SPA_TYPE_Array:
        values = spa_pod_get_array(value, &n_values);
        JAMI_LOG("PipeWireStream: prop value: array size {}", n_values);
        SPA_POD_ARRAY_FOREACH((spa_pod_array*)value, iter) {
            printPodValue(iter);
        }
        break;
    case SPA_TYPE_Object:
        JAMI_LOG("PipeWireStream: prop value: object");
        printPodObject((spa_pod_object*)value);
        break;
    case SPA_TYPE_None:
        JAMI_LOG("PipeWireStream: prop value: none");
        break;
    case SPA_TYPE_Pod:
        JAMI_LOG("PipeWireStream: prop value: pod");
        break;
    case SPA_TYPE_Id:
        spa_pod_get_id(value, &choice);
        JAMI_LOG("PipeWireStream: prop value Id: {:d}", choice);
        break;
    case SPA_TYPE_Choice:
        JAMI_LOG("PipeWireStream: prop value: choice");
        printPodChoice(value);
        break;
    case SPA_TYPE_Fd:
        JAMI_LOG("PipeWireStream: prop value: fd");
        break;
    case SPA_TYPE_Pointer:
        JAMI_LOG("PipeWireStream: prop value: pointer");
        break;
    case SPA_TYPE_Struct:
        JAMI_LOG("PipeWireStream: prop value: struct");
        printPodStruct(value);
        break;
    default:
        JAMI_LOG("PipeWireStream: prop value: unknown type {}", value->type);
        break;
    }
}
...
std::string_view
getPropInfoName(uint32_t key)
{
    for (size_t i = 0; spa_type_prop_info[i].name; i++) {
        if (spa_type_prop_info[i].type == key)
            return spa_type_prop_info[i].name;
    }
    return "unknown";
}
...
void printPodObject(const spa_pod_object* param)
{
    ...
    SPA_POD_OBJECT_FOREACH(param, prop) {
        //JAMI_LOG("PipeWireStream: prop key: {:d}", prop->key);
...
void PipeWireStream::streamParamChanged(void* data, uint32_t id, const spa_pod* param)
{
    ...
    if (id == SPA_PARAM_Props) {
        ...
        if (param) {
            ...
            JAMI_LOG("PipeWireStream: Stream tag null");
        }
        return;
    }
