...
getDeviceInfo(const std::vector<PwDeviceInfo>& list, const std::string& name)
{
    ...
    auto dev_info = std::find_if(list.begin(), list.end(), [&name](const PwDeviceInfo& info) {
...
void
PipeWireLayer::startStream(AudioDeviceType type)
{
...

    {
        ...
        if (type == AudioDeviceType::PLAYBACK || type == AudioDeviceType::ALL) {
            ...
            if (!playbackInfo) {
                JAMI_ERROR("Failed to get device info for playback device: {:s}", pref_.getPulseDevicePlayback());
            }
        }
        ...
        if (type == AudioDeviceType::RINGTONE || type == AudioDeviceType::ALL) {
            ...
            if (!ringtoneInfo) {
                JAMI_ERROR("Failed to get device info for ringtone device: {:s}", pref_.getPulseDeviceRingtone());
            }
        }
        ...
        if (type == AudioDeviceType::CAPTURE || type == AudioDeviceType::ALL) {
            ...
            if (!captureInfo) {
                JAMI_ERROR("Failed to get device info for capture device: {:s}", pref_.getPulseDeviceRecord());
            }
        }
    }
...
void
PipeWireLayer::createStream(std::unique_ptr<PipeWireStream>& stream,
                            ...
                            std::function<void(pw_buffer* buf)>&& onData)
{
    ...
    pendingStreams++;
    stream = std::make_unique<PipeWireStream>(
        loop_,
        core_,
        name,
        type,
...
PipeWireLayer::startCaptureStream(const std::string& id)
{
    ...
    if (!ringBuffer) {
        JAMI_ERROR("Failed to get ring buffer for id {}", id);
        return;
    }

    JAMI_DEBUG("Starting Loopback Capture for ID {:s}", id);

    loopbackCapture_.startCaptureAsync("Playback", [ringBuffer](const AudioData& audioData) {
        if (audioData.size == 0 || !audioData.data) {
            JAMI_WARNING("No audio data captured.");
            return;
        }

        auto capturedFrame = std::make_shared<AudioFrame>(AudioFormat {audioData.rate,
                                                                       audioData.channels,
                                                                       AV_SAMPLE_FMT_S16},
...
PipeWireLayer::stopCaptureStream(const std::string& id)
{
    ...
    JAMI_DEBUG("Stopping Loopback Capture for ID {:s}", id);
...
void
PipeWireLayer::stopStream(AudioDeviceType type)
{
    pw_thread_loop_lock(loop_);
...
void
PipeWireLayer::updateDeviceList()
{
    ...
    if (!deviceListCv_.wait_for(lock, std::chrono::seconds(2), [this] {
        ...
        JAMI_LOG("Device enumeration complete: {} sinks, {} sources", sinkList_.size(), sourceList_.size());
    }
}

void
PipeWireLayer::registryEventCallback(void* data,
                                     uint32_t id,
                                     ...
                                     const struct spa_dict* props)
{
    ...
    if (stype == PW_TYPE_INTERFACE_Node) {
        ...
        if (name && std::string_view(name) == "default") {
            ...
            pw_metadata* metadata = (pw_metadata*)
                pw_registry_bind(self->registry_, id, PW_TYPE_INTERFACE_Metadata, PW_VERSION_CLIENT, 0);
...
PipeWireLayer::registryEventCallbackRemove(void* data, uint32_t id)
{
    ...
    bool deviceRemoved = false;
    {
        ...
        auto it = std::find_if(self->sinkList_.begin(), self->sinkList_.end(), [id](const PwDeviceInfo& info) {
            return info.id == id;
        });
        ...
        it = std::find_if(self->sourceList_.begin(), self->sourceList_.end(), [id](const PwDeviceInfo& info) {
            return info.id == id;
        });
...
void
PipeWireLayer::handleSinkEvent(uint32_t id, const struct spa_dict* props)
{
    ...
    {
        std::lock_guard<std::mutex> lock(deviceListMutex_);
        auto it = std::find_if(sinkList_.begin(), sinkList_.end(), [id](const PwDeviceInfo& info) {
            return info.id == id;
        });
...
void
PipeWireLayer::handleSourceEvent(uint32_t id, const struct spa_dict* props)
{
    const char* name = spa_dict_lookup(props, PW_KEY_NODE_NAME);
    const char* description = spa_dict_lookup(props, PW_KEY_NODE_DESCRIPTION);
    ...
    {
        std::lock_guard<std::mutex> lock(deviceListMutex_);
        auto it = std::find_if(sourceList_.begin(), sourceList_.end(), [id](const PwDeviceInfo& info) {
            return info.id == id;
        });
...
PipeWireLayer::writeToSpeaker(pw_buffer* buf)
{
    ...
    if (requestedSamples != 0) {
        ...
        if (not buff or isPlaybackMuted_)
            ...
            std::memcpy(data, buff->pointer()->data[0], buff->pointer()->nb_samples * frameSize);
...
PipeWireLayer::getAudioDeviceIndex(const std::string& descr, AudioDeviceType type) const
{
    ...
    case AudioDeviceType::RINGTONE:
        foundIndex = std::distance(sinkList_.begin(),
                                   std::find_if(sinkList_.begin(), sinkList_.end(), DescriptionComparator(descr)));
    ...
    case AudioDeviceType::CAPTURE:
        foundIndex = std::distance(sourceList_.begin(),
                                   std::find_if(sourceList_.begin(), sourceList_.end(), DescriptionComparator(descr)));
        // Add 1 to account for "default" at index 0
...
PipeWireLayer::getAudioDeviceIndexByName(const std::string& name, AudioDeviceType type) const
{
    ...
    case AudioDeviceType::RINGTONE:
        foundIndex = std::distance(sinkList_.begin(),
                                   std::find_if(sinkList_.begin(), sinkList_.end(), NameComparator(name)));
        // Add 1 to account for "default" at index 0
        return (foundIndex < static_cast<int>(sinkList_.size())) ? foundIndex + 1 : 0;

    case AudioDeviceType::CAPTURE:
        foundIndex = std::distance(sourceList_.begin(),
                                   std::find_if(sourceList_.begin(), sourceList_.end(), NameComparator(name)));
        // Add 1 to account for "default" at index 0
        return (foundIndex < static_cast<int>(sourceList_.size())) ? foundIndex + 1 : 0;
...
{
    return getAudioDeviceIndexByName(pref_.getPulseDevicePlayback(), AudioDeviceType::PLAYBACK);
}

int
PipeWireLayer::getIndexRingtone() const
{
    return getAudioDeviceIndexByName(pref_.getPulseDeviceRingtone(), AudioDeviceType::RINGTONE);
}
