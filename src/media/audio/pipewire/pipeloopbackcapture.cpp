...
bool
PipeWireLoopbackCapture::startCaptureAsync(const std::string& exclude_app, AudioCallback callback)
{
    if (is_running_.load()) {
...
void
PipeWireLoopbackCapture::stopCapture()
{
...
void
PipeWireLoopbackCapture::run_capture_loop()
{
...
bool
PipeWireLoopbackCapture::initialize_pipewire()
{
    int argc = 0;
    char** argv = nullptr;
    pw_init(&argc, &argv);
...
void
PipeWireLoopbackCapture::cleanup()
{
    // Cleanup streams
...
void
PipeWireLoopbackCapture::on_process(void* userdata)
{
...
void
PipeWireLoopbackCapture::mix_and_maybe_flush(uint32_t node_id, const void* data, size_t size)
{
...
void
PipeWireLoopbackCapture::flush_mix_locked()
{
    if (mix_samples_ == 0) {
    ...
    for (size_t i = 0; i < mix_samples_; ++i) {
        const int32_t sample = std::clamp(mix_accumulator_[i],
    ...
    if (callback_) {
        AudioData mixed_data = {.data = mix_output_.data(),
...
void
PipeWireLoopbackCapture::start_recording_stream(uint32_t node_id, const std::string& app_name)
{
    ...
    stream_data_ptr->stream = pw_stream_new_simple(
        ...
        pw_properties_new(PW_KEY_MEDIA_TYPE, "Audio", PW_KEY_MEDIA_CATEGORY, "Capture", PW_KEY_MEDIA_ROLE, "DSP", NULL),
        &stream_events,
        stream_data_ptr.get());
    ...
    struct spa_audio_info_raw audio_info = SPA_AUDIO_INFO_RAW_INIT(.format = SPA_AUDIO_FORMAT_S16,
                                                                   .rate = DEFAULT_RATE,
                                                                   .channels = DEFAULT_CHANNELS);
...
void
PipeWireLoopbackCapture::registry_event_global(
...
void
PipeWireLoopbackCapture::registry_event_global_remove(void* data, uint32_t id)
{
    auto* app = static_cast<AppData*>(data);
...
void
PipeWireLoopbackCapture::on_core_done(void* data, uint32_t id, int seq)
{
...
void
PipeWireLoopbackCapture::roundtrip()
{
    struct roundtrip_data
    {
        int pending;
