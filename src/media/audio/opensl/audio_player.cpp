...
AudioPlayer::AudioPlayer(ring::AudioFormat sampleFormat, SLEngineItf slEngine) :
    playQueue_(nullptr),freeQueue_(nullptr), devShadowQueue_(DEVICE_SHADOW_BUFFER_QUEUE_LEN),
...
void AudioPlayer::playAudioBuffers(int32_t count) {
    ...
    while(count--) {
        ...
        if(result != SL_RESULT_SUCCESS) {
            ...
            LOGE("%s Error @( %p, %d ), result = %d", __FUNCTION__, (void*)buf->buf_, buf->size_, result);
            /*
