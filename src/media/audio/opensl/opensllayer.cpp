...
bool
OpenSLLayer::engineService(uint32_t msg, void* data) {
    playCv.notify_one();
    recCv.notify_one();
    return true;
}
...
OpenSLLayer::initAudioPlayback()
{
    ...
    player_->registerCallback(std::bind(&OpenSLLayer::engineService, this, _1, _2));
    ...
    ringtone_->registerCallback(std::bind(&OpenSLLayer::engineService, this, _1, _2));
}
...
OpenSLLayer::initAudioCapture()
{
    ...
    recorder_->registerCallback(std::bind(&OpenSLLayer::engineService, this, _1, _2));
}
    ...
    playThread = std::thread([&]() {
        ...
        while (player_ || ringtone_) {
            ...
            while (process) {
                ...
                sample_buf* buf;
                if(player_) {
                    bool canplay = freePlayBufQueue_.front(&buf);
                    if (canplay) {
                        ...
                        if (dat.frames() != 0) {
                            ...
                            if (!playBufQueue_.push(buf))
                                player_->playAudioBuffers(PLAY_KICKSTART_BUFFER_COUNT);
                            process = true;
                        }
                    } else if (playBufQueue_.size() > 0) {
                        RING_WARN("playThread player_ PLAY_KICKSTART_BUFFER_COUNT %zu %zu %zu %d", freePlayBufQueue_.size(), playBufQueue_.size(), player_->dbgGetDevBufCount(), player_->started() ? 1 : 0);
                        player_->playAudioBuffers(playBufQueue_.size());
                    }
                }

                if (ringtone_) {
                    bool canplay = freeRingBufQueue_.front(&buf);
                    if (canplay) {
                        ...
                        if (dat.frames() != 0) {
                            ...
                            if (!ringBufQueue_.push(buf)) {
                                ...
                                ringtone_->playAudioBuffers(PLAY_KICKSTART_BUFFER_COUNT);
                            }
                            process = true;
                        }
                    } else if (ringBufQueue_.size() > 0) {
                        RING_WARN("playThread ringtone_ PLAY_KICKSTART_BUFFER_COUNT 2");
                        ringtone_->playAudioBuffers(ringBufQueue_.size());
                    }
                }
            }
        }
