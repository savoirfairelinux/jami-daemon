...
OpenSLLayer::dbgEngineGetBufCount() {
    ...
    count += freePlayBufQueue_.size();
    count += playBufQueue_.size();

    RING_ERR("Buf Disrtibutions: PlayerDev=%d, PlayQ=%d, FreeQ=%d",
...
OpenSLLayer::startAudioPlayback()
{
    ...
    playThread = std::thread([&]() {
        ...
        while (player_) {
            ...
            while (true) {
                sample_buf* buf;
                if(!player_ || !freePlayBufQueue_.front(&buf))
                    break;
                ...
                sample_buf *buf2;
                bool canplay = playBufQueue_.front(&buf2);
                playBufQueue_.push(buf);
                if (!canplay) {
                    RING_DBG("playThread PLAY_KICKSTART_BUFFER_COUNT");
                    player_->playAudioBuffers(PLAY_KICKSTART_BUFFER_COUNT);
                }
            }
        }
