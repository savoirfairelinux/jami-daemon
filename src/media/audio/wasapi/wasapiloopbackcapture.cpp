...
HRESULT
WasapiLoopbackCapture::SetDeviceStateErrorIfFailed(HRESULT hr)
{
    if (FAILED(hr)) {
...
HRESULT
WasapiLoopbackCapture::InitializeLoopbackCapture()
{
...
HRESULT
WasapiLoopbackCapture::ActivateAudioInterface(DWORD processId, bool includeProcessTree)
{
...
//
HRESULT
WasapiLoopbackCapture::ActivateCompleted(IActivateAudioInterfaceAsyncOperation* operation)
{
...
HRESULT
WasapiLoopbackCapture::StartCaptureAsync(DWORD processId, bool includeProcessTree, AudioDataCallback callback)
{
...
HRESULT
WasapiLoopbackCapture::OnStartCapture(IMFAsyncResult* pResult)
{
    return SetDeviceStateErrorIfFailed([&]() -> HRESULT {
        // Start the capture
...
HRESULT
WasapiLoopbackCapture::StopCaptureAsync()
{
...
HRESULT
WasapiLoopbackCapture::OnStopCapture(IMFAsyncResult* pResult)
{
...
HRESULT
WasapiLoopbackCapture::FinishCaptureAsync()
{
    return MFPutWorkItem2(MFASYNC_CALLBACK_QUEUE_MULTITHREADED, 0, &m_xFinishCapture, nullptr);
}

HRESULT
WasapiLoopbackCapture::OnFinishCapture(IMFAsyncResult* pResult)
{
...
HRESULT
WasapiLoopbackCapture::OnSampleReady(IMFAsyncResult* pResult)
{
...
HRESULT
WasapiLoopbackCapture::OnAudioSampleRequested()
{
    ...
    while (SUCCEEDED(m_AudioCaptureClient->GetNextPacketSize(&FramesAvailable)) && FramesAvailable > 0) {
        // Get sample buffer
        RETURN_IF_FAILED(m_AudioCaptureClient
                             ->GetBuffer(&Data, &FramesAvailable, &dwCaptureFlags, &u64DevicePosition, &u64QPCPosition));
...
HRESULT
WasapiLoopbackCapture::GetParameters(DWORD* pdwFlags, DWORD* pdwQueue)
{
...
HRESULT
WasapiLoopbackCapture::Invoke(IMFAsyncResult* pAsyncResult)
{
