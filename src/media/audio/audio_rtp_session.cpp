...
AudioRtpSession::startSender()
{
    ...
    audioInput_->setRecorderCallback([this](const std::shared_ptr<Observable<std::shared_ptr<MediaFrame>>>& obs,
...
AudioRtpSession::startReceiver()
{
    ...
    receiveThread_->setRecorderCallback([this](const std::shared_ptr<Observable<std::shared_ptr<MediaFrame>>>& obs,
                                               const MediaStream& ms) { attachRecorder(obs, ms); });
...
void
AudioRtpSession::attachRecorder(const std::shared_ptr<Observable<std::shared_ptr<MediaFrame>>>& obs, const MediaStream& ms)
{
    if (!recorder_)
        return;
    if (auto ob = recorder_->addStream(ms)) {
...
AudioRtpSession::initRecorder()
{
    ...
    if (receiveThread_)
        receiveThread_->setRecorderCallback(
            [this](const std::shared_ptr<Observable<std::shared_ptr<MediaFrame>>>& obs, const MediaStream& ms) {
