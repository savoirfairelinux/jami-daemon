...
AudioRtpSession::AudioRtpSession(const std::string& callId, const std::string& streamId, const std::string& audioInputId)
    ...
    , audioRingbufferId_(std::make_pair(callId, streamId))

{
    ...
    ringbuffer_ = Manager::instance().getRingBufferPool().createRingBuffer(audioRingbufferId_);
}
...
AudioRtpSession::startReceiver()
{
    ...
    receiveThread_.reset(new AudioReceiveThread(audioRingbufferId_,
