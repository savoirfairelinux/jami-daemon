...
class AudioLayer {
    ...
    public:
         ...
         */
        template< class Rep, class Period >
        bool waitForHardwareFormatAvailable(const std::chrono::duration<Rep, Period>& rel_time) const {
            std::unique_lock<std::mutex> lk(hwFormatMutex_);
    ...
    protected:
        ...
        mutable std::mutex hwFormatMutex_;
