...
AudioInput::process()
{
    ...
    if (ringBuffer->availableForGet(id_) < samplesToGet
        && not ringBuffer->waitForDataAvailable(id_, samplesToGet, std::chrono::high_resolution_clock::now() + msPerPacket_)) {
        return;
    }
...

    {
        ...
        if (rec && rec->isRecording()) {
            ...
            RING_DBG() << "recordData waiting, not started";
        }
    }
}
...
{
    RING_DBG("initing audio recorder");
