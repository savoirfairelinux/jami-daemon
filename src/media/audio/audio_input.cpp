...
AudioInput::readFromDevice()
{
    ...
    bufferPool.waitForDataAvailable(id_, wakeUp_);
...
AudioInput::initCapture(const std::string& device)
{
    ...
    std::string pattern = "window-id:hwnd=";
    size_t winHandlePos = device.find(pattern);

    if (winHandlePos != std::string::npos) {
        // Get HWND from device URI
        size_t startPos = winHandlePos + pattern.size();
