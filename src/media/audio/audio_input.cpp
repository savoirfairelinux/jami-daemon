...
AudioInput::readFromDevice()
{
    ...
    if (settingMS_.load()) {
        if (recorderCallback_)
            recorderCallback_(this, MediaStream("a:local", format_, sent_samples));
...
AudioInput::setRecorderCallback(
    const std::function<void(Observable<std::shared_ptr<MediaFrame>>* obs, const MediaStream& ms)>&
        cb)
{
