...
Recordable::toggleRecording()
{
    ...
    if (!recorder_) {
        RING_WARN("toggleRecording: can't toggle recording, recorder is NULL");
        return false;
    }
...
Recordable::startRecording(const std::string& path)
{
    std::lock_guard<std::mutex> lk {apiMutex_};
    if (!recorder_) {
        RING_WARN("startRecording: can't start recording, recorder is NULL");
        return false;
    }

    if (!recording_) {
        if (path.empty()) {
            RING_WARN("startRecording: can't start recording with empty path");
            return false;
        }
...
Recordable::stopRecording()
{
    std::lock_guard<std::mutex> lk {apiMutex_};
    if (!recorder_) {
        RING_WARN("stopRecording: can't stop recording, recorder is NULL");
        return;
    }

    if (not recording_) {
        RING_WARN("stopRecording: can't stop non-running recording");
        return;
    }
