...
void TestFrameInput::onNewFrame(AVFrame *frame) {
  ...
  av_frame_move_ref(frame, fcopy.predictionsFrame.get());
}
...
void TestFrameInput::feedInput() {
  //***********************************************************************
  // Feed data
  std::vector<uint8_t> in = fcopy.toInterleavedRGB(fcopy.resizedFrameRGB.get());
  mot.feedInput(in, fcopy.resizedFrameRGB->width, fcopy.resizedFrameRGB->height,
                3);
}
 ...
 * @param predictions
 */
std::shared_ptr<AVFrame> TestFrameInput::drawPredictionsOnCopyFrame(
    const AVFrame *frame,
    const std::vector<std::tuple<std::array<float, 4>, float, int>>
        &predictions) {
  //***********************************************************************
  fcopy.transformFrame(fcopy.originalRGBContext, frame,
                       fcopy.predictionsFrame.get());

  // Create images using CImg for final display/saving to file
  std::vector<uint8_t> originalImgRGBNonInterleaved =
      fcopy.toNonInterleavedRGB(fcopy.predictionsFrame.get());
 ...
 * @param frame
 */
void FrameCopy::createCopyFrames(const AVFrame *frame) {
 ...
 * @param frame
 */
void FrameCopy::copyOriginalFrame(const AVFrame *frame) {
