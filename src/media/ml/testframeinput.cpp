...
void TestFrameInput::onNewFrame(jami::VideoFrame &frame) {
  ...
  av_frame_unref(frame.pointer());
  av_frame_move_ref(frame.pointer(), fcopy.predictionsFrame.pointer());
}
...
void TestFrameInput::drawPredictionsOnCopyFrame(
  ...
  //  }
  fcopy.predictionsFrame.setFromMemory(result.data(), AV_PIX_FMT_RGB24,
...
void FrameCopy::createCopyFrames(const jami::VideoFrame &frame) {
  ...
  originalFrameRGB.reserve(AV_PIX_FMT_RGB24, 300, 300);
