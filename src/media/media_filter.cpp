...
MediaFilter::initialize(const std::string filterDesc, MediaFilterParameters mfp)
{
    ...
    mfps.push_back(mfp);
    return initialize(filterDesc, mfps);
}
...
MediaFilter::initialize(const std::string filterDesc, std::vector<MediaFilterParameters> mfps)
{
    ...
    AVFilterInOut* in;
    AVFilterInOut* out;
    if ((ret = avfilter_graph_parse2(graph_, filterDesc.c_str(), &in, &out)) < 0)
    ...
    RING_DBG() << "Filter graph initialized with: " << filterDesc;
    initialized_ = true;
    return 0;
}
