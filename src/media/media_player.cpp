...
// this is used for video only files
// if file has audio stream audio input loop running and ask for a new frame when needed
void
MediaPlayer::process()
{
    decode();
...
MediaPlayer::seekToTime(int64_t time) {
    ...
    if (hasAudio()) {
        audioInput_->setSeekTime(time / AV_TIME_BASE);
        audioInput_->updateStartTime(startTime_);
    }
    if (hasVideo()) {
        videoInput_->setSeekTime(time / AV_TIME_BASE);
        videoInput_->updateStartTime(startTime_);
        flushBuffers = true;
    }
    return true;
