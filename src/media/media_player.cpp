...
MediaPlayer::configureMediaInputs()
{
    ...
    try {
        ...
        if (hasAudio()) {
            ...
            audioInput_->setFileFinishedCb([this]() -> void {
                playFileFromBeginning();
            });
            audioInput_->setNeedFrameCb([this]() -> void {
                triggerReadFile();
            });
        }
    ...
    try {
        ...
        if (hasVideo()) {
            ...
            muteAudio(true);
            videoInput_->setNeedFrameCb([this]() -> void {
                triggerReadFile();
            });
        }
    } catch (const std::exception& e) {
    ...
    if (hasVideo() && hasAudio()) {
        audioInput_->useSynchronization();
        videoInput_->useSynchronization();
...
MediaPlayer::playFileFromBeginning()
{
    ...
    pauseInterval_ = 0;
    if (hasAudio()) {
        audioInput_->setSeekTime(0);
        audioInput_->updateStartTime(startTime_);
    }
    if (hasVideo()) {
        videoInput_->setSeekTime(0);
        videoInput_->updateStartTime(startTime_);
    }
}
...
MediaPlayer::readFile() {
    ...
    while (!paused_ && ret != MediaDemuxer::Status::EndOfFile &&
           ret != MediaDemuxer::Status::ExceedBufferLimit) {
        if (flushBuffers) {
            flushMediaBuffers();
            flushBuffers = false;
        }
    ...
    if (ret == MediaDemuxer::Status::EndOfFile) {
        demuxer_->updateCurrentState(MediaDemuxer::CurrentState::Finished);
    }
    if (ret == MediaDemuxer::Status::ExceedBufferLimit) {
        demuxer_->setExceedLimit(false);
    }
    readingFile = false;
}

void
MediaPlayer::triggerReadFile()
{
    if (!readingFile) {
        readingFile = true;
        std::thread([this] {
