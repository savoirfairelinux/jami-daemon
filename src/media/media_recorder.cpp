...
MediaRecorder::recordData(AVPacket* packet, int streamIdx)
{
    ...
    clone->pts = av_rescale_q_rnd(clone->pts, streamTimebase_[streamIdx], st->time_base, (AVRounding)(AV_ROUND_NEAR_INF|AV_ROUND_PASS_MINMAX));
    clone->dts = av_rescale_q_rnd(clone->dts, streamTimebase_[streamIdx], st->time_base, (AVRounding)(AV_ROUND_NEAR_INF|AV_ROUND_PASS_MINMAX));
    clone->duration = av_rescale_q(clone->duration, streamTimebase_[streamIdx], st->time_base);
    clone->pos = -1;

    int ret = 0;
    if ((ret = av_interleaved_write_frame(outputCtx_, clone)) < 0)
