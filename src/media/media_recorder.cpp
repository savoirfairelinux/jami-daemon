...
MediaRecorder::addStream(bool fromPeer, bool isVideo, AVStream* stream)
{
    ...
    int peerWidth = 0, peerHeight = 0;
    if (nbVideoStreams_ == 2) {
        auto localKey = std::make_pair(false, true);
        auto peerKey = std::make_pair(true, true);
        peerWidth = filterParams_[peerKey].width;
        peerHeight = filterParams_[peerKey].height;
        std::stringstream filterDesc;
        filterDesc << "[v:local] scale=" << peerWidth/4 << ":" << peerHeight/4 << " [v:scaled]; ";
        filterDesc << "[v:peer] [v:scaled] overlay=main_w-overlay_w-10:main_h-overlay_h-10";
        std::vector<MediaFilterParameters> vec; // order is important
        vec.push_back(filterParams_[localKey]); // local video
        vec.push_back(filterParams_[peerKey]); // peer video
