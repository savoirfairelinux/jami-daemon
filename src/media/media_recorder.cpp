...
MediaRecorder::addStream(bool fromPeer, bool isVideo, AVStream* stream)
{
    ...
    if (nbStreams_ != nbAudioStreams_ + nbVideoStreams_)
        return 0; // return 0, we got the stream params, stream was "added"
    ...
    if (!audioFilter_) {
        std::string filterDesc = "[a:local] [a:peer] amix, aresample=osr=48000:ocl=stereo:osf=s16";
    ...
    if (!videoFilter_) {
        ...
        filterDesc << "[v:local] format=pix_fmts=yuv420p, scale="
            << peerWidth / 4 << ":" << peerHeight / 4 << " [v:scaled]; "
            << "[v:peer] format=pix_fmts=yuv420p [v:pscaled]; "
            << "[v:pscaled] [v:scaled] overlay=main_w-overlay_w-10:main_h-overlay_h-10";
        std::vector<MediaFilterParameters> vec; // order is important
    ...
    for (const auto& it : filterParams_) {
        if (filterParams_[it.first].width > 0) {
            streamMap_[it.first] = videoIdx_;
        } else {
            streamMap_[it.first] = audioIdx_;
        }
    }
