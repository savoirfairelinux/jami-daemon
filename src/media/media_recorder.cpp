...
MediaRecorder::onFrame(const std::string& name, const std::shared_ptr<MediaFrame>& frame)
{
    ...
    if (ms.isVideo && videoFilter_ && outputVideoFilter_) {
        ...
        if (auto filteredAudioOutput = audioFilter_->readOutput()) {
            auto resampledFrame = outputAudioResampler_->resample(std::unique_ptr<AudioFrame>(static_cast<AudioFrame*>(
                                                                      filteredAudioOutput.release())),
                                                                  AudioFormat(48000, 2));
            if (!audioFrameResizer_) {
                audioFrameResizer_
                    = std::make_unique<AudioFrameResizer>(AudioFormat(48000, 2),
                                                          960,
                                                          [this](std::shared_ptr<AudioFrame>&& frame) {
                                                              auto uniqueFrame = std::make_unique<MediaFrame>();
                                                              uniqueFrame->copyFrom(*frame);
                                                              std::lock_guard lk(mutexFrameBuff_);
                                                              frameBuff_.emplace_back(std::move(uniqueFrame));
                                                              cv_.notify_one();
                                                          });
            }
            audioFrameResizer_->enqueue(std::move(resampledFrame));
        }
#ifdef ENABLE_VIDEO
    }
#endif // ENABLE_VIDEO
