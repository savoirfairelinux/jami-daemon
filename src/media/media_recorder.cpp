...
{
    /**
     * Need to map two booleans to outputCtx_->streams
     * 0: local audio
     * 1: remote audio
     * 2: local video
     * 3: remote video
...
MediaRecorder::copyStream(AVStream* input, AVPacket* packet, bool fromPeer, bool isVideo)
{
    ...
    if (packet && outStream->codecpar->extradata_size == 0) {
        // needs to be malloc'd because it is freed by FFmpeg in one of the free functions
    ...
    if ((ret = avformat_write_header(outputCtx_, nullptr)) < 0) {
        ...
        RING_ERR() << "Failed to write file header for recording: " << errbuf;
        return -1;
    }
...
//    copyPacket->pts = av_rescale_q_rnd(copyPacket->pts,
//                                       streamTimebase_[streamIdx],
//                                       outputCtx_->streams[streamIdx]->time_base,
//                                       static_cast<AVRounding>(AV_ROUND_NEAR_INF|AV_ROUND_PASS_MINMAX));
//    copyPacket->dts = av_rescale_q_rnd(copyPacket->dts,
//                                       streamTimebase_[streamIdx],
//                                       outputCtx_->streams[streamIdx]->time_base,
//                                       static_cast<AVRounding>(AV_ROUND_NEAR_INF|AV_ROUND_PASS_MINMAX));
//    copyPacket->duration = av_rescale_q(copyPacket->duration,
//                                        streamTimebase_[streamIdx],
//                                        outputCtx_->streams[streamIdx]->time_base);
    copyPacket->pts = streamTimestamps_[streamIdx];
    copyPacket->dts = streamTimestamps_[streamIdx];
    ++streamTimestamps_[streamIdx];
    copyPacket->duration = 0;
    copyPacket->pos = -1;
