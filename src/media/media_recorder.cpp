...
MediaRecorder::initRecord()
{
    ...
    isReady_ = (nbReceivedAudioStreams_ > 0 && audioIdx_ >= 0) // has audio and audio stream index is valid
        && (audioOnly_ || (nbReceivedVideoStreams_ && videoIdx_ >= 0)); // has video and video stream index is valid
    if (isReady_) {
...
MediaRecorder::recordData(AVFrame* frame, bool isVideo, bool fromPeer)
{
    ...
    if (inputName.empty()) // #nofilters
        return sendToEncoder(frame, streamIdx);

    // frame needs to be filtered before being encoded
    ...
    return sendToEncoder(f, streamIdx);
}
