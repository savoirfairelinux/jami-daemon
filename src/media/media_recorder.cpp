...
MediaRecorder::setupVideoOutput()
{
    ...
    if (secondaryFilter.height > secondaryFilter.width) {
        ret = outputVideoFilter_->initialize(
            "[input]scale=-2:720,pad=1280:720:(ow-iw)/2:(oh-ih)/2,format=pix_fmts=yuv420p,fps=30",
            {secondaryFilter});
    } else {
        ret = outputVideoFilter_->initialize(
            "[input]scale=1280:-2,pad=1280:720:(ow-iw)/2:(oh-ih)/2,format=pix_fmts=yuv420p,fps=30",
            {secondaryFilter}); 
    }
