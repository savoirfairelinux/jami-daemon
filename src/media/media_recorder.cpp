...
MediaRecorder::addStream(const MediaStream& ms)
{
    ...
    if (it == streams_.end()) {
        ...
        JAMI_INFO("[Recorder: %p] Recorder already has '%s' as input", this, ms.name.c_str());
    }
...
MediaRecorder::removeStream(const MediaStream& ms)
{
    ...
    auto it = streams_.find(ms.name);
    if (it == streams_.end()) {
        JAMI_INFO("[Recorder: %p] Recorder no stream to remove", this);
    } else {
        JAMI_INFO("[Recorder: %p] Recorder removing '%s'", this, ms.name.c_str());
        streams_.erase(it);
        if (ms.isVideo)
            setupVideoOutput();
        else
