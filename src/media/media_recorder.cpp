...
MediaRecorder::recordData(AVPacket* packet, int streamIdx)
{
    ...
    if (st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
        clone->pts = lastTimestamp_[streamIdx]++;
    } else {
        clone->pts = lastTimestamp_[streamIdx];
        lastTimestamp_[streamIdx] += 1;// += nb_samples;
    }
    clone->dts = clone->pts;
    if (lastDuration_.find(streamIdx) == lastDuration_.end()) {
        // TODO first packet for this stream, set its duration
        lastDuration_[streamIdx] = clone->duration;
    }
    clone->duration = ts - lastDuration_[streamIdx];
    clone->pos = -1;
...
MediaRecorder::recordData(AVFrame* frame, int streamIdx)
{
    ...
    // TODO encode AVFrame to get an AVPacket, call recordData(packet, streamIdx)
    // set lastTimestamp_[streamIdx] with frame->nb_samples
