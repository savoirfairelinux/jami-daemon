...
MediaRecorder::copyStream(AVStream* input, AVPacket* packet, bool fromPeer, bool isVideo)
{
    ...
    nextTimestamp_[outStream->index] = 0;
    lastDuration_[outStream->index] = packet->duration;
...
MediaRecorder::recordData(AVFrame* frame, bool fromPeer, bool isVideo)
{
    ...
    return recordData(frame, streamMap_[key]);
}
...
MediaRecorder::recordData(AVPacket* packet, int streamIdx)
{
    ...
    clone->pts = nextTimestamp_[streamIdx];
    clone->dts = clone->pts;
    clone->duration = (av_gettime() * st->time_base.num / st->time_base.den) - lastDuration_[streamIdx];
    clone->pos = -1;

    if (!skipTSIncrement_[streamIdx]) { // done in recordData with AVFrame
        if (st->codecpar->codec_type != AVMEDIA_TYPE_AUDIO)
            nextTimestamp_[streamIdx]++;
        else
            nextTimestamp_[streamIdx] += 1;// TODO += nb_samples;
    }
    lastDuration_[streamIdx] = clone->duration;
    skipTSIncrement_[streamIdx] = false;
...
int
MediaRecorder::recordData(AVFrame* frame, int streamIdx)
{
    if (!outputCtx_) {
        RING_ERR() << "Cannot record data (no output format)";
        return -1;
    ...
    int ret = 0;
    bool isVideo = outputCtx_->streams[streamIdx]->codecpar->codec_type == AVMEDIA_TYPE_VIDEO;

    if (!isVideo)
        nextTimestamp_[streamIdx] += frame->nb_samples;
    else
        nextTimestamp_[streamIdx]++;
    skipTSIncrement_[streamIdx] = true;
