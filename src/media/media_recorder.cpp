...
MediaRecorder::addStream(const MediaStream& ms)
{
    ...
    streams_.insert(std::make_pair(ms.name, ms));
...
MediaRecorder::setupVideoOutput()
{
    ...
    auto peer = std::find_if(streams_.begin(), streams_.end(), [](const auto& pair){
        return pair.second.isVideo && pair.second.name.find("remote") != std::string::npos;
    })->second;
    auto local = std::find_if(streams_.begin(), streams_.end(), [](const auto& pair){
...
MediaRecorder::process()
{
    ...
    auto ms = std::find_if(streams_.cbegin(), streams_.cend(), [recframe](const auto& pair){
