...
MediaRecorder::recordData(AVFrame* frame, bool isVideo, bool fromPeer)
{
    ...
    if (isVideo && !fromPeer) { // convert to rtp timestamp
...
MediaRecorder::buildVideoFilter()
{
    ...
    const bool needScale = (p.height < 720);
    const int newHeight = (needScale ? 720 : p.height);
    ...
    v << "[v:overlay] fps=" << newFps << ", scale=-2:" << newHeight / 4 << " [v:o]; ";
