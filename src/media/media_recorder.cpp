...
{
    RING_WARN() << "recordData " << (fromPeer ? "from peer " : "from me ") << (isVideo ? "is video" : "is audio");
}

void
MediaRecorder::flush()
{
    // private/static function that takes AVPacket and stream_index?
    // recordData would find the appropriate stream depending on its bool parameters
    // flush would call using every possible stream index
    if (outputCtx_) {
        RING_WARN() << "WOLOLO found " << outputCtx_->nb_streams << " streams";
        // all streams need to be flushed, video and audio, local and remote
        for (unsigned i = 0; i < outputCtx_->nb_streams; ++i) {
            AVPacket pkt; // flush packet (data is null, size is 0)
            av_init_packet(&pkt);
            pkt.data = nullptr;
            pkt.size = 0;

            bool fromPeer = true; // FIXME need to know if stream is local or remote
            bool isVideo = (outputCtx_->streams[i]->codecpar->codec_type == AVMEDIA_TYPE_VIDEO);

            recordData(&pkt, fromPeer, isVideo);
        }
    }
}
