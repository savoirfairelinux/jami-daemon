...
LocalRecorder::LocalRecorder(std::shared_ptr<ring::video::VideoInput> input):
    audioInput_(new ring::audio::AudioInput(RingBufferPool::DEFAULT_ID, false)) {
    if (input) {
        videoInput_ = input;
        videoInputSet_ = true;
    }
...
LocalRecorder::startRecording()
{
    ...
    if (startRecording && recorder_) {
        // audio recording
        ring::Manager::instance().getRingBufferPool().bindCallID(RingBufferPool::DEFAULT_ID, RingBufferPool::DEFAULT_ID);
