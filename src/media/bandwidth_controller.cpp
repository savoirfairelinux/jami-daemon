...
BandwidthController::applyDecrease(float congestion)
{
    ...
    targetBitrate_ = clampBitrate(static_cast<unsigned>(std::lround(targetBitrate_ * factor)));
...
BandwidthController::applyIncrease()
{
    ...
    targetBitrate_ = clampBitrate(
        static_cast<unsigned>(std::lround(targetBitrate_ * (1.0f + increaseFactor))));
...
BandwidthController::update(float packetLoss, float jitterMs, float rttMs)
{
    ...
    // Maintain history
    history_.push_back({now, packetLoss, rttMs});
    while (history_.size() > MAX_HISTORY
           || (!history_.empty() && (now - history_.front().time) > HISTORY_WINDOW)) {
        history_.pop_front();
    }
...
BandwidthController::onAudioCongestion(float audioLoss)
{
    ...
    auto oldBitrate = targetBitrate_;
    targetBitrate_ = clampBitrate(
        static_cast<unsigned>(std::lround(targetBitrate_ * reductionFactor)));
...
BandwidthController::onRembFeedback(bool isOveruse)
{
    ...
    if (isOveruse) {
        ...
        auto oldBitrate = targetBitrate_;
        targetBitrate_ = clampBitrate(
            static_cast<unsigned>(std::lround(targetBitrate_ * 0.85f)));
