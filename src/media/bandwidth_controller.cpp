...
BandwidthController::computeCongestionScore() const
{
    ...
    float rttComponent = (cfg_.congestionRttScale > 0)
        ? rttExcess / cfg_.congestionRttScale
        : 0.0f;
...
BandwidthController::update(float packetLoss, float jitterMs, float rttMs)
{
    ...
    if (rttMs > 0 && rttSampleCount_ < 5) {
        baselineRtt_ = (baselineRtt_ * static_cast<float>(rttSampleCount_) + rttMs) / static_cast<float>(rttSampleCount_ + 1);
        rttSampleCount_++;
    }
