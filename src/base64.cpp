...
decode(const std::string& str)
{
    ...
    if(pj_base64_decode(&input, output_buffer, &output_length) != PJ_SUCCESS) {
        throw new base64_exception();
    }

    auto output = std::vector<uint8_t>(output_buffer, output_buffer + output_length);
    delete[] output_buffer;
    return output;
}
