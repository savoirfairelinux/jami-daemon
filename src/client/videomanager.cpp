...
AudioFrame::reserve(size_t nb_samples)
{
    if (nb_samples != 0) {
        ...
        int err;
        if ((err = av_frame_get_buffer(d, jami::libav_utils::DEFAULT_VIDEO_BUFFER_ALIGN)) < 0) {
            throw std::bad_alloc();
        }
    }
}
...
VideoFrame::reserve(int format, int width, int height)
{
    ...
    if (av_frame_get_buffer(libav_frame, jami::libav_utils::DEFAULT_VIDEO_BUFFER_ALIGN))
        throw std::bad_alloc();
    allocated_ = true;
    releaseBufferCb_ = {};
}
