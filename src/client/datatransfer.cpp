...
std::string
connectToPeer(const std::string& accountId, const std::string& peerUri)
{
    return {};
}

bool
dataConnectionInfo(const DataConnectionId& cid, const DataConnectionInfo& info)
{
    return false;
}

bool
closeDataConnection(const DataConnectionId& cid)
{
    return false;
}
...
bool
dataTransferInfo(const DataTransferId& tid, DataTransferInfo& info)
{
...
std::streamsize
dataTransferSentBytes(const DataTransferId& tid)
{
    //if (auto dt = ring::DataTransfer::getDataTransfer(tid))
    //    return dt->getCount();
    return 0;
...
bool
cancelDataTransfer(const DataTransferId& tid)
{
    return false;
}
