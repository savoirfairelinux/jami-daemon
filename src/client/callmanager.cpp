...
std::shared_ptr<jami::Call>
getCall(const std::string& accountId, const std::string& callId)
{
...
std::shared_ptr<jami::Conference>
getConference(const std::string& accountId, const std::string& conferenceId)
{
    if (auto account = jami::Manager::instance().getAccount(accountId))
...
{
    if (auto call = getCall(accountId, callId))
...
{
    if (auto conf = getConference(accountId, confId))
        return {{"ID", confId},
