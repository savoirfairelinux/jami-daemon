              ...
              const bool& state)
{
    if (const auto account = jami::Manager::instance().getAccount(accountId)) {
        ...
        } else if (auto call = jami::Manager::instance().callFactory.getCall<jami::SIPCall>(confId)) {
            if (call->supportsNewConfProtocol()) {
                Json::Value sinkVal;
                sinkVal["muteAudio"] = state ? jami::TRUE_STR : jami::FALSE_STR;
              ...
              const bool& state)
{
    if (const auto account = jami::Manager::instance().getAccount(accountId)) {
        if (auto conf = account->getConference(confId)) {
            ...
            if (call->supportsNewConfProtocol()) {
                Json::Value sinkVal;
                sinkVal["active"] = state ? jami::TRUE_STR : jami::FALSE_STR;
                Json::Value mediasObj;
                mediasObj[sinkId] = sinkVal;
                Json::Value deviceVal;
                deviceVal["medias"] = mediasObj;
          ...
          const bool& state)
{
    if (const auto account = jami::Manager::instance().getAccount<jami::JamiAccount>(accountId)) {
        if (auto conf = account->getConference(confId)) {
            ...
            if (call->supportsNewConfProtocol()) {
                Json::Value deviceVal;
                deviceVal["raiseHand"] = state ? jami::TRUE_STR : jami::FALSE_STR;
                Json::Value deviceObj;
