...
CertificateStore::loadRevocations(crypto::Certificate& crt) const
{
    ...
    for (const auto& ocsp /*filename*/ : ocsp_dir_content) {
        try {
            ...
            // Update Trust Store
            //tls::TrustStore::setCertificateStatus(crt.getUID(), trustStatus);
