...
{
    return std::strcmp(str, DRing::Certificate::Status::ALLOWED) == 0 ? Status::ALLOWED : (
           std::strcmp(str, DRing::Certificate::Status::BANNED) == 0 ? Status::BANNED :
           Status::UNDEFINED );
}
...
bool
TrustStore::setCertificateStatus(const std::string& cert_id, const TrustStore::Status status)
{
    updateKnownCerts();
...
bool
TrustStore::setCertificateStatus(std::shared_ptr<crypto::Certificate>& cert, const TrustStore::Status status, bool local)
{
...
void
TrustStore::setStoreCertStatus(const crypto::Certificate& crt, TrustStore::Status status)
{
    ...
    RING_DBG("TrustStore: setting %s status to %s.", crt.getId().toString().c_str(), status == TrustStore::Status::ALLOWED ? "ALLOWED" : "BANNED");
}
...
TrustStore::generateCAList(const std::string& out_path)
{
    ...
    auto res = ss.str();
    fileutils::saveFile(out_path, {res.begin(), res.end()});
}*/
