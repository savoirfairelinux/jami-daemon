...
CertificateStore::addCertificate(std::shared_ptr<crypto::Certificate> crt)
{
    ...
    bool insert {false};
    {
        ...
        std::tie(it, insert) = certs_.emplace(std::move(id), std::move(crt));
        fileutils::saveFile(certPath_+DIR_SEPARATOR_CH+it->first, it->second->getPacked());
    }
    if (insert)
        emitSignal<DRing::ConfigurationSignal::CertificateAdded>(id);
    return id;
}
...
bool
CertificateStore::isBanned(const std::string& id) const {
    std::lock_guard<std::mutex> l(lock_);
    return bannedCerts_.find(id) != bannedCerts_.end();
}
