...
class TrustStore {
public:
    TrustStore(bool trust_system = true) {
        gnutls_x509_trust_list_init(&trust_, 0);
        if (trust_system)
            gnutls_x509_trust_list_add_system_trust(trust_, 0, 0);
    }
    virtual ~TrustStore() {
    ...
    bool setCertificateStatus(const std::string& cert_id, const DRing::Certificate::Status status) {
        ...
        if (s == certStatus_.end()) {
            ...
            if (status == DRing::Certificate::Status::ALLOWED)
                gnutls_x509_trust_list_add_cas(trust_, &s->second.first->cert, 1, 0);
            else if (status == DRing::Certificate::Status::BANNED)
                gnutls_x509_trust_list_remove_cas(trust_, &s->second.first->cert, 1);
        }
        return true;
    }
