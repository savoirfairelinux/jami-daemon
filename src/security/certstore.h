...
public:
    ...
    bool pinRevocationList(const std::string& id, const dht::crypto::RevocationList& crl);

private:
    NON_COPYABLE(CertificateStore);

    unsigned loadLocalCertificates(const std::string& path, const std::string& crl_path);
