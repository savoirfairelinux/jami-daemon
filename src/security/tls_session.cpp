...
TlsSession::handleStateMtuDiscovery(TlsSessionState state)
{
        ...
        RING_WARN("[TLS] Heartbeat PMTUD: new mtu set to %d which is %d of maxPayload for dtls", mtu, getMaxPayload());
...
TlsSession::pathMtuHeartbeat()
{
    ...
    if (isServer()) {
        ...
        while (mtu != mtus.cend()) {
            ...
            } else if (errno_recv > 0) {
                RING_WARN("[TLS] Heartbeat disabled: setting mtu to minimal value");
                usleep(1000 * HEARTBEAT_TOTAL_TIMEOUT);
                return mtus.front();
            } else {
        ...
        while (mtu != mtus.cend()){
            ...
            } else if (errno_send == GNUTLS_E_INVALID_REQUEST){
                RING_WARN("[TLS] Heartbeat disabled: setting mtu to minimal value");
                usleep(1000 * HEARTBEAT_TOTAL_TIMEOUT);
                return mtus.front();
            } else {
