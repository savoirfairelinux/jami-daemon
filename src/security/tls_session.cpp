...
static constexpr uint16_t DTLS_MTU {1280};
...
static constexpr uint16_t HEARTBEAT_RETRANS_TIMEOUT = 1000; // gnutls heartbeat retransmission timeout for each ping (in milliseconds)
static constexpr uint16_t HEARTBEAT_TOTAL_TIMEOUT = 1500; // gnutls heartbeat time limit for heartbeat procedure (in milliseconds)
...
TlsSession::handleStateMtuDiscovery(TlsSessionState state)
{
    ...
    if (gnutls_heartbeat_allowed(session_, GNUTLS_HB_PEER_ALLOWED_TO_SEND)) {
        ...
        RING_ERR("[TLS] HEARTBEAT DISABLED, SET MTU TO DEFAULT VALUE: %d", DTLS_MTU);
...
TlsSession::pathMtuHeartbeat()
{
    ...
    // neither under 128 which could result in underflow of certain variables
    ...
    RING_WARN("tls session overhead : %d", tls_overhead);
