        ...
        for (auto mtu = next(PossibleMTU::NONE); mtuToInt(mtu) != 0;){
            ...
            if(errno_ping == GNUTLS_E_HEARTBEAT_PING_RECEIVED){ //heartbeat ping received
                ...
                if (errno_pong != GNUTLS_E_SUCCESS){
                    RING_DBG("Heartbeat PMTUD: failed on pong with %s; aborting TLS connection",
            ...
            } else if (errno_ping == GNUTLS_E_TIMEDOUT){ // packet to big on current mtu
                mtu = pre(mtu);

                if (mtu == PossibleMTU::NONE) {
                    RING_DBG("Heartbeat PMTUD: no mtu determined, path mtu < 576 or link broken; aborting TLS connection");
                    shutdown();
                } else {
                    return mtuToInt(mtu);
                }
                ...
                RING_DBG("Heartbeat PMTUD: failed on ping never received with %s; aborting TLS connection",
                         gnutls_strerror(errno_ping));
                shutdown();
            }
        }
	...
	// client side: managing ping on heartbeat
        ...
        for (auto mtu = PossibleMTU::IPV4_MIN; mtu != PossibleMTU::END;){
            do {
                ...
                RING_DBG("Heartbeat PMTUD: Ping success with mtu %d", mtuToInt(mtu));
            }
            while (errno_ping == GNUTLS_E_AGAIN || errno_ping == GNUTLS_E_INTERRUPTED);

            if (errno_ping == GNUTLS_E_SUCCESS) {
                mtu = next(mtu);
            } else if (errno_ping == GNUTLS_E_TIMEDOUT){
                mtu = pre(mtu);
                ...
                RING_DBG("Heartbeat PMTUD: ping failed: %s; aborting TLS connection", gnutls_strerror(errno_ping));
                shutdown();
            }
	}
    }
