...
public:
     ...
     *     sequence: number of the AIA access descriptor (0 for first one, 1 for the second etc.)
     */
    void verifyOcsp(gnutls_x509_crt_t cert, gnutls_x509_crt_t issuer, gnutls_x509_crt_t signer,
                    const unsigned int sequence, OcspVerification cb);
    /*
     * Load X509 Certificates from file.
     */
...
TlsSession::TlsSessionImpl::verifyCertificateWrapper(gnutls_session_t session)
{
    ...
    // TODO init value and where is issuer, signer? files or mem?
    gnutls_x509_crt_t peer_crt, issuer_crt, signer_crt;
    ...
    auto peer_cert_chain = dht::crypto::Certificate(crt_data);
    std::string cert_id = peer_cert_chain.getUID();
    ...
    verifyOcsp(peer_crt, issuer_crt, signer_crt, 0 /*sequence*/,
               [&cert_id, &verified, &done, &cv, &cv_m, &cv_lk](const int status)
    {
        if (status != GNUTLS_E_SUCCESS)
            JAMI_ERR("OCSP Verification failed for %s", cert_id.c_str());
        std::lock_guard<std::mutex> cv_lk(cv_m);
        verified = status;
        done = true;
        cv.notify_all();
    });
    cv.wait_for(cv_lk, std::chrono::seconds(2), [&done]{ return done; });
    // } OCSP
                                       ...
                                       const unsigned int sequence, OcspVerification cb)
{
    ...
    std::string uri;
    std::string hostname;
    for (unsigned int seq = 0; seq <= sequence; seq++)
    {
        ...
        if (ret == GNUTLS_E_REQUESTED_DATA_NOT_AVAILABLE){
            JAMI_ERR("No URI was found in the certificate.");
            if (cb)
                cb(false);
            return;
        }
...
TlsSession::TlsSessionImpl::loadCertificate(const char* certFile, unsigned int certMax, unsigned flags)
{
    ...
    if (!data.data){
        JAMI_ERR("Cannot open file: %s", certFile);
        return NULL;
    }
    /*
    ...
    if (ret < 0){
        JAMI_ERR("Cannot import certificate in %s: %s", certFile, gnutls_strerror(ret));
        return NULL;
    }
