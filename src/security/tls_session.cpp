              ...
              [this] { cleanup(); })
{
    if (not transport_.isReliable()) {
        transport_.setOnRecv([this](const uint8_t* buf, size_t len) {
...
{
    std::error_code ec;
    auto n = transport_.write(reinterpret_cast<const uint8_t*>(buf), size, ec);
    if (!ec) {
        // log only on success
        ++stTxRawPacketCnt_;
        stTxRawBytesCnt_ += n;
...
TlsSession::TlsSessionImpl::recvRaw(void* buf, size_t size)
{
    if (transport_.isReliable()) {
        std::error_code ec;
        auto count = transport_.read(reinterpret_cast<uint8_t*>(buf), size, ec);
        if (!ec)
            return count;
        gnutls_transport_set_errno(session_, ec.value());
        return -1;
