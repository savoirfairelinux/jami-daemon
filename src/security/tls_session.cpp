...
static constexpr const char* TLS_PRIORITY_STRING {"SECURE192:-RSA:-VERS-TLS-ALL:+VERS-DTLS-ALL:%SERVER_PRECEDENCE:%SAFE_RENEGOTIATION"};
...
TlsSession::commonSessionInit()
{
    ...
    ret = gnutls_priority_set_direct(session_, TLS_ANON_PRIORITY_STRING, nullptr);
    if (ret != GNUTLS_E_SUCCESS) {
    ...
    if (ret == GNUTLS_E_SUCCESS) {
        ...
        if (not xcred_) {
            ...
            if (isServer_) {
                ...
                ret = gnutls_rehandshake(session_);
                if (ret != GNUTLS_E_SUCCESS) {
                    RING_ERR("[TLS] rehandshake failed: %s", gnutls_strerror(ret));
                    return TlsSessionState::SHUTDOWN;
                }
            }

            return state; // re-handshake
        }
