...
TlsSession::pathMtuHeartbeat() {
    int errno_recv = GNUTLS_E_TIMEDOUT, errno_send = GNUTLS_E_TIMEDOUT; //non zero initialisation
    ...
    if(isServer()){
        ...
        for (auto mtu = next(PossibleMTU::NONE); mtu != PossibleMTU::END;){
            ...
            do {
                buf = (char*) calloc(mtuToInt(mtu), sizeof(char));
                RING_DBG("[TLS] Heartbeat PMTUD: awaiting ping with buffer size %d", mtuToInt(mtu));
                errno_recv = gnutls_record_recv(session_, buf, mtuToInt(mtu)/8);
                free(buf);
	...
	// client side: managing ping on heartbeat
        ...
        for (auto mtu = PossibleMTU::IPV4_MIN; mtu != PossibleMTU::END;){
            do {
                errno_send = gnutls_heartbeat_ping(session_, mtuToInt(mtu), 2, GNUTLS_HEARTBEAT_WAIT);
