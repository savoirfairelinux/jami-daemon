...
static constexpr int MIN_MTU {512 - 20 - 8}; // minimal payload size of a DTLS packet carried by a IPv4 packet
...
public:
    ...
    TlsSessionImpl(GenericTransport& transport, const TlsParams& params,
    ...
    std::size_t send(const uint8_t*, std::size_t, std::error_code&);
...
TlsSession::TlsSessionImpl::TlsSessionImpl(GenericTransport& transport, const TlsParams& params,
                                           const TlsSessionCallbacks& cbs, bool anonymous)
    ...
    , thread_([this] { return setup(); },
              ...
              [this] { cleanup(); })
{
    transport_.setOnRecv([this](uint8_t* buf, size_t len) {
...
std::size_t
TlsSession::TlsSessionImpl::send(const uint8_t* tx_data, std::size_t tx_size, std::error_code& ec)
{
...
{
    std::error_code ec;
    auto n = transport_.send(reinterpret_cast<const unsigned char*>(buf), size, ec);
    if (!ec) {
        // log only on success
        ++stTxRawPacketCnt_;
        stTxRawBytesCnt_ += n;
...
TlsSession::TlsSessionImpl::flushRxQueue()
{
    ...
    RING_DBG("[TLS] push 0x%lx (%lu pkt)", first_offset, gapOffset_ - first_offset);
}
...
TlsSession::TlsSessionImpl::handleStateEstablished(TlsSessionState state)
{
    ...
    if (transport_.isReliable()) {
        ...
        auto ret = gnutls_record_recv(session_, rawPktBuf_.data(), rawPktBuf_.size());
        if (ret > 0) {
            rawPktBuf_.resize(ret);
            if (callbacks_.onRxData) {
                callbacks_.onRxData(std::move(rawPktBuf_));
            }
        } else if (ret == 0) {
            RING_DBG("[TLS] eof");
...
TlsSession::TlsSession(GenericTransport& transport, const TlsParams& params,
                       const TlsSessionCallbacks& cbs, bool anonymous)
...
std::size_t
TlsSession::send(const void* data, std::size_t size, std::error_code& ec)
{
    return pimpl_->send(static_cast<const uint8_t*>(data), size, ec);
}
