                                       ...
                                       gnutls_x509_crt_t& signer, OcspVerification cb)
{
    ...
    dht::Blob ocsp_req = {};
    try {
        ocsp_req = cert.generateOcspRequest(issuer, noncebuf);
    }
    ...
    sendOcspRequest(aia_uri, body, OCSP_REQUEST_TIMEOUT,
        ...
        dht::Blob ocspBlob {r.body.begin(), r.body.end()};
        // Verify response validity
        unsigned int verify = 0;
        try {
            cert.ocspResponse = std::make_shared<dht::crypto::OcspResponse>(ocspBlob);
            JAMI_DBG("%s", cert.ocspResponse->toString().c_str());
            verify = cert.ocspResponse->verifyDirect(cert.cert, signer, const_cast<dht::Blob&>(noncebuf));
        }
        catch (dht::crypto::CryptoException& e){
