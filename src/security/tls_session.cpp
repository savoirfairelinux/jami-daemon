...
static constexpr uint8_t HEARTBEAT_RETRIES = 2; // Number of tries at each heartbeat ping send (1 + 1 if error)
...
static constexpr std::array<uint16_t,4> mtus = {800, 1000, 1280, 1500};
static auto mtuProbe_ = mtus.cbegin();
...
TlsSession::pathMtuHeartbeat()
{
    ...
    if (errno_send == GNUTLS_E_SUCCESS) {
        ...
        mtuProbe_ = mtus.cend()-1; // for loop over, setting mtu to last valid mtu
    } else {
        RING_ERR("[TLS] Heartbeat PMTUD : fallback on untreated error");
    }
    ...
    if (!rxQueue_.empty()) {
        ...
        if (ret > 0) {
            ...
            if (errno_send != GNUTLS_E_SUCCESS){
                ...
                if (*mtuProbe_ < mtus.back()){
                    ++mtuProbe_;
                    gnutls_dtls_set_mtu(session_,*mtuProbe_ - transportOverhead_);
                    maxPayload_ = gnutls_dtls_get_data_mtu(session_);
                    RING_WARN("[TLS] maxPayload for dtls raised to %d B", getMaxPayload());
                }
            }
