...
public:
    ...
    unsigned int verifyOcspResponse(gnutls_datum_t* data, gnutls_x509_crt_t cert,
                                    gnutls_x509_crt_t signer, const std::string noncehex);
                                       ...
                                       gnutls_x509_crt_t signer, OcspVerification cb)
{
    ...
    if (ret == 0)
        noncehex = bytes_to_hex_string(noncebuf, sizeof(noncebuf));
    if (ret < 0){
        if (cb)
            cb(ret);
        return;
    ...
    sendOcspRequest(aia_uri, body.str(), OCSP_REQUEST_TIMEOUT,
                    [this, cb, &cert, &signer, noncehex](const dht::http::Response& response){
        // Read OCSP response
        gnutls_datum_t ocsp_resp;
        ocsp_resp.data = (unsigned char*)response.body.c_str();
        ocsp_resp.size = response.body.size();
        int verified = verifyOcspResponse(&ocsp_resp, cert, signer, noncehex);
                                               ...
                                               gnutls_x509_crt_t signer, const std::string noncehex)
{
    ...
    if (!noncehex.empty()){
         ...
         */
        std::string rnoncehex = "";
        ret = gnutls_ocsp_resp_get_nonce(resp, NULL, &rnonce);
        if (ret == 0)
            rnoncehex = bytes_to_hex_string(rnonce.data, rnonce.size);
        ...
        if (noncehex != rnoncehex){
            JAMI_ERR("Nonce does not match: sent=%s received=%s", noncehex.c_str(), rnoncehex.c_str());
