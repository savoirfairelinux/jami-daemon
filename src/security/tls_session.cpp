...
static constexpr uint16_t TLS_HEADER_SIZE = 16;  // Size in bytes of UDP packet header

static constexpr uint16_t HEARTBEAT_RETRIES = 2; // Number of tries at each heartbeat ping send (1 + 1 if error)
static constexpr uint16_t HEARTBEAT_RETRANS_TIMEOUT = 800; // gnutls heartbeat retransmission timeout for each ping (in milliseconds)
static constexpr uint16_t HEARTBEAT_TOTAL_TIMEOUT = 2000; // gnutls heartbeat time limit for heartbeat ping full procedure (in milliseconds)
...
TlsSession::pathMtuHeartbeat()
{
    ...
    uint8_t ip_header = (socket_->getTransportFamily() == AF_INET) ? IPV4_HEADER_SIZE : IPV6_HEADER_SIZE;
    ...
    if (isServer()) {
        ...
        while (mtu != mtus.cend()){
            do {
                bytesToSend = (*mtu) - 24 - TLS_HEADER_SIZE - UDP_HEADER_SIZE - ip_header;
