...
TlsSession::TlsSessionImpl::verifyCertificateWrapper(gnutls_session_t session)
{
    ...
    // Cache validation
    auto cert_serialhex = dht_peer_chain.getSerialNumber();


    unsigned int ocsp_status = 666;
    if (!dht_peer_chain.ocsp_response.empty())
        dht_peer_chain.getOcspResponseCertificateStatus();
    JAMI_DBG("Certificate UID=%s SerialNumber=%s OcspRespStatus=%i",
             dht_peer_chain.getUID().c_str(), cert_serialhex.c_str(), ocsp_status);
                                       ...
                                       gnutls_x509_crt_t& signer, OcspVerification cb)
{
    ...
    // Generate OCSP request
    std::vector<uint8_t> noncebuf(23);
    gnutls_datum_t nonce = { noncebuf.data(), (unsigned int)noncebuf.size() };
    ...
    sendOcspRequest(aia_uri, body.str(), OCSP_REQUEST_TIMEOUT,
                    [this, cb, &cert, &signer, noncebuf=std::move(noncebuf)](const dht::http::Response& response){
        // Prepare response data
        gnutls_datum_t ocsp_resp;
        ocsp_resp.data = (unsigned char*)response.body.c_str();
        ocsp_resp.size = response.body.size();
        // Verify response validity
        gnutls_datum_t nonce;
        nonce.data = (unsigned char*)noncebuf.data();
        nonce.size = noncebuf.size();
        int verified = verifyOcspResponse(ocsp_resp, cert, signer, nonce);
        // Save response into the certificate store
        tls::CertificateStore::instance().pinOcspResponse(dht::crypto::Certificate(cert).getUID(), &ocsp_resp);
        if (cb)
            cb(verified);
    });
}
...
{
    if (data.size == 0){
        JAMI_ERR("OCSP response verification: Received empty response");
    ...
    // Debug {
    gnutls_datum_t printdat;
    ret = gnutls_ocsp_resp_print(resp, GNUTLS_OCSP_PRINT_COMPACT /*_FULL*/, &printdat);
    if (ret == 0)
        JAMI_DBG("%s", printdat.data); // display ocsp response in readable format
    gnutls_free(printdat.data);
    if (ret < 0){
    ...
    if ((ret = gnutls_ocsp_resp_check_crt(resp, 0, cert)) < 0){
        JAMI_ERR("gnutls_ocsp_resp_check_crt: %s", gnutls_strerror(ret));
        goto end;
    }
#endif
    /*
     ...
     * structure except the final singleExtensions. TODO validate ocsp validity times
    ...
    if (status != GNUTLS_OCSP_CERT_GOOD){
        if (status == GNUTLS_OCSP_CERT_REVOKED)
            ...
            JAMI_ERR("Certificate is unknown");
        goto end;
    }
     ...
     */
    if ((verify > 0) or (status != GNUTLS_OCSP_CERT_GOOD))
        return GNUTLS_E_OCSP_RESPONSE_ERROR;
