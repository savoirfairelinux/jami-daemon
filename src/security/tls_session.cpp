...
TlsSession::TlsSessionImpl::verifyCertificateWrapper(gnutls_session_t session)
{
    ...
    gnutls_x509_crt_t peer_crt = dht_cert.getCopy();
    gnutls_x509_crt_t issuer_crt = dht_cert.issuer->getCopy(); // issuer is signer

    verifyOcsp(peer_crt, issuer_crt, issuer_crt /*signer*/, 0 /*sequence*/,
               [&cert_id, &verified, &ocsp_done, &cv, &cv_m, &cv_lk](const int status)
    {
        ...
        if (status == GNUTLS_E_REQUESTED_DATA_NOT_AVAILABLE){
            JAMI_WARN("Skipping OCSP verification: AIA not found for %s", cert_id.c_str());
        }
	...
	gnutls_x509_crt_deinit(peer_crt);
