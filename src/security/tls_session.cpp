...
static constexpr auto RX_OOO_TIMEOUT = std::chrono::milliseconds(250);
...
{
    RING_DBG("[dtls] rx 0x%lx", pkt_seq);
...
TlsSession::flushRxQueue()
{
    ...
    if ((clock::now() - lastReadTime_) >= RX_OOO_TIMEOUT) {
        ...
        auto lost = next_offset - gapOffset_;
        RING_WARN("[dtls] %lu lost(s) since 0x%lx", lost, gapOffset_);
    ...
    while (item != std::end(reorderBuffer_) and item->first <= next_offset) {
        ...
        if (callbacks_.onRxData) {
            lk.unlock();
            RING_DBG("[dtls] push 0x%lx", pkt_offset);
