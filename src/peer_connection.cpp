...
public:
    ...
    PeerConnectionImpl(Account& account, std::unique_ptr<ConnectionEndpoint> endpoint)
        : account {account}
        , peer_uri {}
...
PeerConnection::PeerConnection(Account& account, const std::string& peer_uri)
    : pimpl_(new PeerConnectionImpl{account, peer_uri})
{
    //auto connection = account.sendPeerConnectionRequest(*this, peer_uri);
    pimpl_->connectingFut =
        std::async(std::launch::async,
                   [&](){
                       try {
                           //pimpl_->endpoint = connection.get();
                       } catch (...) {
                           pimpl_->changeState(ConnectionState::FAILED);
                           return;
                       }
                       pimpl_->changeState(ConnectionState::CONNECTED);
                   });
}

PeerConnection::PeerConnection(Account& account, std::unique_ptr<ConnectionEndpoint> endpoint)
    : pimpl_(new PeerConnectionImpl{account, std::move(endpoint)})
{}
