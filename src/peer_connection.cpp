...
{
    AttachInputCtrlMsg(const std::shared_ptr<Stream>& stream)
        : CtrlMsg {}
        , stream {stream} {}
    virtual CtrlMsgType type() const override { return CtrlMsgType::ATTACH_INPUT; }
    const std::shared_ptr<Stream> stream;
};

struct AttachOutputCtrlMsg : public CtrlMsg
{
    AttachOutputCtrlMsg(const std::shared_ptr<Stream>& stream)
        : CtrlMsg {}
        , stream {stream} {}
        ...
        while (true) {
            ...
            switch (msg.type()) {
                case CtrlMsgType::ATTACH_INPUT:
                {
                    ...
                    RING_DBG() << "input stream" << id << " attached";
                ...
                case CtrlMsgType::ATTACH_OUTPUT:
                {
                    ...
                    RING_DBG() << "output stream" << id << " attached";
...
{
    pimpl_->ctrlChannel << AttachInputCtrlMsg {stream};
}

void
PeerConnection::attachOutputStream(const std::shared_ptr<Stream>& stream)
{
    pimpl_->ctrlChannel << AttachOutputCtrlMsg {stream};
}
