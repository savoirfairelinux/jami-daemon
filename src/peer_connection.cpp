...
static constexpr std::size_t RX_BUFFER_SIZE {3000};
...
std::vector<char>
TcpSocketEndpoint::read(std::size_t length) const
{
    std::vector<char> result(length);
    auto res = ::recv(sock_, result.data(), length, 0);
    if (res < 0)
        throw std::system_error(errno, std::system_category());
    result.resize(res);
    return result;
