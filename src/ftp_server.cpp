...
void
FtpServer::startNewFile(const std::function<void(bool)>& cb)
{
    ...
    Manager::instance().dataTransfers->onIncomingFileRequest(
        transferId_, [w = weak(), cb = std::move(cb)](const IncomingFileInfo& fileInfo) {
            auto shared = w.lock();
            if (!shared)
                return;
...
FtpServer::write(const std::vector<uint8_t>& buffer)
{
    ...
    case FtpState::PARSE_HEADERS:
        if (parseStream(buffer)) {
            ...
            startNewFile([w = weak()](bool success) {
                auto shared = w.lock();
                if (!shared)
                    return;
                if (!success) {
