...
getProtectedEnvString(const char* name)
{
    char* envvar_ = nowide::getenv(name);
    return envvar_ ? envvar_ : "";
}
#define PROTECTED_GETENV(str) getProtectedEnvString(str)
...
#ifndef _WIN32
    nowide::stat_t s;
    auto ret = nowide::stat(path.c_str(), &s);
    if (ret)
...
lastWriteTime(const std::string& p)
{
    ...
    return std::chrono::duration_cast<std::chrono::milliseconds>(
        ...
        .count();
#else
    nowide::stat_t result;
    if (nowide::stat(p.c_str(), &result) == 0)
