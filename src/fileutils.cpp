...
createSymLink(const std::string& linkFile, const std::string& target)
{
    ...
    realpath(target.c_str(), buf);
    if (buf)
        symlink(buf, linkFile.c_str());
#else
    // C++ 17 with filesystem
    check_dir(std::string(std::filesystem::path(linkFile).parent_path()).c_str());
    auto absolute_dest = std::string(std::filesystem::absolute(std::filesystem::path(target)));
