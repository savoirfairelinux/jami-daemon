...
#define PROTECTED_GETENV(str) \
    ({ \
        char* envvar_ = boost::nowide::getenv((str)); \
        envvar_ ? envvar_ : ""; \
    })
...
{
    HANDLE h
        = CreateFileW(boost::nowide::widen(path).c_str(), GENERIC_WRITE, 0, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
    if (h == INVALID_HANDLE_VALUE) {
...
remove(const std::string& path, bool erase)
{
    ...
    return boost::nowide::remove(path.c_str());
}
