...
struct FrameBuffer {
    std::vector<uint8_t> vector;    // data as an C++ vector (direct rendering)
    uint8_t* ptr;                   // data as a plain raw pointer (shm rendering). Shall point on vector.data() if vector is used
    unsigned ptrSize;               // size in byte of ptr array
