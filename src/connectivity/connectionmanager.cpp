...
{
public:
    explicit Impl(std::shared_ptr<dht::DhtRunner> dht_,
...
ConnectionManager::ConnectionManager(std::shared_ptr<dht::DhtRunner> dht_,
                                     const dht::crypto::Identity& id_,
                                     std::shared_ptr<jami::upnp::Controller> upnpCtrl_)
    : pimpl_ {std::make_shared<Impl>(dht_, id_, upnpCtrl_)}
{}
...
void
ConnectionManager::serializeTls(YAML::Emitter& out) const
{
    out << YAML::Key << Conf::CALIST_KEY << YAML::Value << pimpl_->tlsCaListFile_;
    out << YAML::Key << Conf::CERTIFICATE_KEY << YAML::Value << pimpl_->tlsCertificateFile_;
    out << YAML::Key << Conf::TLS_PASSWORD_KEY << YAML::Value << pimpl_->tlsPassword_;
    out << YAML::Key << Conf::PRIVATE_KEY_KEY << YAML::Value << pimpl_->tlsPrivateKeyFile_;
}

void
ConnectionManager::unserialize(const YAML::Node& node, std::string idPath_)
{
    ...
    parsePath(tlsMap, Conf::CERTIFICATE_KEY, pimpl_->tlsCertificateFile_, idPath_);
    parsePath(tlsMap, Conf::CALIST_KEY, pimpl_->tlsCaListFile_, idPath_);
    parseValue(tlsMap, Conf::TLS_PASSWORD_KEY, pimpl_->tlsPassword_);
    parsePath(tlsMap, Conf::PRIVATE_KEY_KEY, pimpl_->tlsPrivateKeyFile_, idPath_);
}

bool
ConnectionManager::isTurnEnabled()
{
    return pimpl_->turnEnabled_;
}

std::string
ConnectionManager::getTurnServer()
{
    return pimpl_->turnServer_;
}

std::string
ConnectionManager::getTlsCaListFile()
{
    return pimpl_->tlsCaListFile_;
}

std::string
ConnectionManager::getTlsCertificateFile()
{
    return pimpl_->tlsCertificateFile_;
}

std::string
ConnectionManager::getTlsPrivateKeyFile()
{
    return pimpl_->tlsPrivateKeyFile_;
}

std::string
ConnectionManager::getTlsPassword()
{
    return pimpl_->tlsPassword_;
}

void
ConnectionManager::setTlsCaListFile(std::string tlsCaListFile_)
{
    pimpl_->tlsCaListFile_ = tlsCaListFile_;
}

void
ConnectionManager::setTlsCertificateFile(std::string tlsCertificateFile_)
{
    pimpl_->tlsCertificateFile_ = tlsCertificateFile_;
}

void
ConnectionManager::setTlsPrivateKeyFile(std::string tlsPrivateKeyFile_)
{
    pimpl_->tlsPrivateKeyFile_ = tlsPrivateKeyFile_;
}

void
ConnectionManager::setTlsPassword(std::string tlsPassword_)
{
    pimpl_->tlsPassword_ = tlsPassword_;
}
