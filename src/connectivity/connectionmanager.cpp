...
{
public:
    explicit Impl(std::shared_ptr<ConnectionManager::Config> config)
    {
        config_ = std::move(config);
        cachePath_ = fmt::format("{}/{}", fileutils::get_cache_dir(), config_->id_.second->getId().toString()); // TODO pass accountId?
        // TODO: because before it was .cache/accountId now .cache/accountUri
...
bool
ConnectionManager::Impl::findCertificate(
    const dht::InfoHash& h, std::function<void(const std::shared_ptr<dht::crypto::Certificate>&)>&& cb)
{
