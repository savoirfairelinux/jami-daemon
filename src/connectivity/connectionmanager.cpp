...
public:
    ...
    {
        auto username = identity().second->getLongId().toString(); // TODO pass accountId;
        cachePath_ = fmt::format("{}/{}", fileutils::get_cache_dir(), username);
        JAMI_ERROR("@@@@@@@@@@@@@ {}", cachePath_);
    }
    ~Impl() {}
    ...
    dht()->listen<PeerConnectionRequest>(
        ...
        [w = weak()](PeerConnectionRequest&& req) {
            ...
            if (shared->isMessageTreated(to_hex_string(req.id))) {
                ...
                JAMI_ERROR("@@@ MESSAGE TREATED {}", req.id);
                return true;
            }
            if (req.isAnswer) {
