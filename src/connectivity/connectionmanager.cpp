...
ConnectionManager::Impl::loadTreatedMessages()
{
    ...
    auto path = config_->cachePath_ + DIR_SEPARATOR_STR "treatedMessages";
...
ConnectionManager::Impl::saveTreatedMessages() const
{
    dht::ThreadPool::io().run([w = weak()]() {
        if (auto sthis = w.lock()) {
            ...
            fileutils::check_dir(this_.config_->cachePath_.c_str());
            saveIdList<decltype(this_.treatedMessages_)>(this_.config_->cachePath_
