...
MessageEngine::trySend(decltype(MessageEngine::messages_)::iterator m)
{
    if (m->second.status != MessageStatus::IDLE &&
        ...
        RING_WARN("Can't send message in status %d", m->second.status);
        return;
    }
...
MessageEngine::load()
{
    try {
        ...
        // everything when fine, removing the file
        std::remove(savePath_.c_str());
    } catch (const std::exception& e) {
        RING_ERR("Could not load messages from %s", savePath_.c_str());
    }
}
...
MessageEngine::save() const
{
    try {
        ...
        RING_ERR("Could not save messages to %s", savePath_.c_str());
    }
}
