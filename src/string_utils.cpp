...
std::string
decodeMultibyteString(const std::string& str)
{
    if (!str.length())
        return {};
    std::wstring result = to_wstring(str, CP_UTF8);
    return std::string(result.begin(), result.end());
}

std::string
decodeMultibyteWString(const std::wstring& wstr)
{
    auto str = jami::to_string(wstr, CP_ACP);
    if (!str.length())
        return {};
    std::wstring result = to_wstring(str, CP_UTF8);
    return std::string(result.begin(), result.end());
