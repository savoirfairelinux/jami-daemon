...
private:
    ...
    // Get the protol instance
    std::shared_ptr<UPnPProtocol> getProtocolInstance(NatProtocolType type) const;
    ...
    void requestMappingOnValidIgd(const Mapping::sharedPtr_t& map);
    ...
    void registerAddMappingTimeout(const std::shared_ptr<UPnPProtocol>& protocol,
                                   const std::shared_ptr<IGD>& igd,
                                   const Mapping::sharedPtr_t& map);

    // Callback invoked when a request times-out
    void onRequestTimeOut(const std::shared_ptr<UPnPProtocol>& protocol,
                          const std::shared_ptr<IGD>& igd,
                          const Mapping::sharedPtr_t& map);
...
#ifdef HAVE_LIBNATPMP
    void updateRenewalTime(uint32_t lifeTime);
    ...
    void onIgdUpdate(const std::shared_ptr<IGD>& igd, UpnpIgdEvent event) override;
...
private:
...
#ifdef HAVE_LIBNATPMP
    std::chrono::system_clock::time_point renewal_ {
        std::chrono::system_clock::now() + std::chrono::seconds(MAPPING_ALLOCATION_LIFETIME)};
#endif
    ...
    std::set<std::pair<NatProtocolType, IpAddr>> igdPublicIp_ {};
};
