...
private:
    ...
    void onIgdUpdate(const std::shared_ptr<UPnPProtocol>& protocol,
                     const std::shared_ptr<IGD>& igd,
                     UpnpIgdEvent event);

    // Callback from the IGD used to report add request status.
    void onMappingAdded(const std::shared_ptr<IGD>& igd, const Mapping& map);

    // Callback from the IGD used to report remove request status.
    void onMappingRemoved(const std::shared_ptr<IGD>& igd, const Mapping& map);
