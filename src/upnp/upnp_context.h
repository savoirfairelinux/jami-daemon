...
private:
     ...
     * Also check the local mapping list against the list returned by the
     * IGD and remove all mappings which do not have a match. Only done
     * for UPNP protocol.
     * To avoid competing with allocation requests, these tasks are performed
     * only if there are no requests in progress.
     */
