...
private:
    ...
    std::map<Mapping::key_t, Mapping::sharedPtr_t>::iterator
        unregisterMapping(std::map<Mapping::key_t, Mapping::sharedPtr_t>::iterator it);

    // Perform the request on the provided IGD.
    void requestMapping(const std::shared_ptr<UPnPProtocol>& protocol, const std::shared_ptr<IGD>& igd, const Mapping::sharedPtr_t& map);
    ...
    void registerAddMappingTimeout(const std::shared_ptr<IGD>& igd, Mapping::key_t key);

    // Callback invoked when a request times-out
    void onRequestTimeOut(const std::shared_ptr<IGD>& igd, Mapping::key_t key);
    ...
    void onIgdChanged(const std::shared_ptr<UPnPProtocol>& protocol, const std::shared_ptr<IGD>& igd,
