...
using mapCb = std::function<void(Mapping*, bool)>;
using cnxChgCb = std::function<void()>;
...
public:
    ...
    void addCallback(const Mapping& map, const ControllerData& ctrlData);

    // Informs the UPnP context that the network status has changed.
    void connectivityChanged();

    // Checks if the desired port is already in use by an IGD.
    ...
    void requestMappingAdd(const ControllerData& ctrlData, uint16_t portDesired, uint16_t portLocal, PortType type, bool unique);

    // Callback function for when mapping is added.
    void onMappingAdded(IpAddr igdIp, Mapping* mapping, bool success);
    ...
    void onMappingRemoved(IpAddr igdIp, Mapping* mapping, bool success);

    // Tries to add or remove IGD to the list via callback.
    bool onIgdListChanged(UPnPProtocol* protocol, IGD* igd, const IpAddr publicIpAddr, bool added);
...
private:
    ...
    void registerCallback(const Mapping& map, const ControllerData& ctrlData);

    // Removes callback from callback list given a mapping.
    void unregisterCallback(const Mapping& map, bool force = false);

    // Removes callback from callback list given a mapping and controller Id.
    void unregisterCallback(const Mapping& map, const std::string& ctrlId);
    ...
    void registerRmMappingTimeout(const Mapping& map);

    // Unregisters a timeout for a given pending remove map request.
    void unregisterRmMappingTimeout(const Mapping& map);
