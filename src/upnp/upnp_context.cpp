...
UPnPContext::init()
{
    ...
    natPmp->setOnIgdUpdate(std::bind(&UPnPContext::onIgdUpdate, this, _1, _2, _3));
    natPmp->searchForIgd();
    ...
    pupnp->setOnIgdUpdate(std::bind(&UPnPContext::onIgdUpdate, this, _1, _2, _3));
    pupnp->searchForIgd();
...
{
    if (not isValidThread()) {
        runOnUpnpThread([this, map] { requestMappingOnValidIgd(map); });
        return;
    }
                         ...
                         UpnpIgdEvent event)
{
    ...
    if (not isValidThread()) {
        runOnUpnpThread([this, protocol, igd, event] { onIgdUpdate(protocol, igd, event); });
        return;
    }
    ...
    if (event == UpnpIgdEvent::INVALID_STATE) {
        JAMI_WARN("IGD %p [%s] is in an invalid state (will be disabled)",
                  igd.get(),
                  protocol->getProtocolName().c_str());
        igdPublicIp_.erase({protocol->getProtocol(), publicIp});
        return;
...
#else
    // Make the invalid if the max error count is reached.
    if (igd->incrementErrorsCounter() == IGD::MAX_ERRORS_COUNT) {
        JAMI_WARN("IGD %p is in invalid state. It will be disabled", igd.get());
        igd->setValid(false);
        std::lock_guard<std::mutex> lock(mappingMutex_);
        igdPublicIp_.erase({map->getProtocol(), igd->getPublicIp()});
    }
