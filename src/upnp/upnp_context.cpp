...
UPnPContext::UPnPContext()
{
...
#if HAVE_LIBNATPMP
	try {
		protocolList_.push_back(std::unique_ptr<UPnPProtocol>(new NatPmp()));
...
#if HAVE_LIBUPNP
	try {
		protocolList_.push_back(std::unique_ptr<UPnPProtocol>(new PUPnP()));
	} catch (std::runtime_error& e) {
	...
	for (auto const& item : protocolList_) {
		item->setOnIgdChanged(std::bind(&UPnPContext::igdListChanged, this, _1, _2, _3, _4));
		item->searchForIGD();
	}
...
UPnPContext::addIGDListener(IgdFoundCallback&& cb)
{
	...
	for (auto const& item : protocolList_) {
		token += item->addIGDListener(std::move(cb));
	}
