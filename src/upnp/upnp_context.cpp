...
bool
UPnPContext::releaseMapping(const Mapping::sharedPtr_t& map)
{
    if (not map) {
    ...
    if (map->isAvailable()) {
        JAMI_WARN("Trying to release an unused mapping %s", map->toString().c_str());
        res = false;
    } else {
...
UPnPContext::unregisterMapping(Mapping::sharedPtr_t map)
{
    ...
    if (mappingList.erase(map->getMapKey()) == 1) {
        JAMI_DBG("Unregister mapping %s succeeded", map->toString().c_str());
    } else {
