...
UPnPContext::UPnPContext()
{
	...
	protocolList_.push_back(natPmp_);
#endif
#if HAVE_LIBUPNP
	try {
		pupnp_ = getPUPnP();
	...
	protocolList_.push_back(pupnp_);

	for (auto const& item : protocolList_) {
		std::shared_ptr<UPnPProtocol> proto = item.lock();
		proto->setOnIgdChanged(std::bind(&UPnPContext::igdListChanged, this, _1, _2, _3, _4));
	}
...
UPnPContext::addIGDListener(IgdFoundCallback&& cb)
{
	...
	for (auto const& item : protocolList_) {
		std::shared_ptr<UPnPProtocol> proto = item.lock();
		token += proto->addIGDListener(std::move(cb));
	}
