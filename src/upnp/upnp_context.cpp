...
std::shared_ptr<UPnPProtocol>
UPnPContext::getProtocolInstance(NatProtocolType type) const
{
    CHECK_VALID_THREAD();
    auto it = protocolList_.find(type);
...
UPnPContext::getExternalIP() const
{
    ...
    if (not igdPublicIp_.empty())
        return (igdPublicIp_.begin()->second);
                            ...
                            const Mapping::sharedPtr_t& map)
{
    ...
    if (not isValidThread()) {
        runOnUpnpContextThread([this, protocol, igd, map] { requestMapping(protocol, igd, map); });
        return;
    }
    ...
    registerAddMappingTimeout(protocol, igd, map);
...
void
UPnPContext::requestMappingOnValidIgd(const Mapping::sharedPtr_t& map)
{
    if (not isValidThread()) {
        runOnUpnpContextThread([this, map] { requestMappingOnValidIgd(map); });
        return;
    }
...
UPnPContext::pruneMappingList()
{
    ...
    if (status.inProgressCount_ == 0 and status.pendingCount_ == 0) {
        ...
        auto protocol = getProtocolInstance(NatProtocolType::PUPNP);
...
void
UPnPContext::onIgdUpdate(const std::shared_ptr<IGD>& igd, UpnpIgdEvent event)
{
    assert(igd);

    if (not isValidThread()) {
        runOnUpnpContextThread([this, igd, event] { onIgdUpdate(igd, event); });
        return;
    }

    CHECK_VALID_THREAD();

    auto protocol = getProtocolInstance(igd->getProtocol());

    auto const& publicIp = igd->getPublicIp();
    // The IGD was removed.
    ...
    if (event == UpnpIgdEvent::REMOVED) {
        JAMI_WARN("IGD %p [%s] on address %s was removed",
                  igd.get(),
                  protocol->getProtocolName().c_str(),
                  publicIp.toString(true, true).c_str());

        igdPublicIp_.erase({protocol->getProtocol(), publicIp});
        return;
    }
    // IGD is an invalid state. Do not use it.
    if (event == UpnpIgdEvent::INVALID_STATE) {
        JAMI_WARN("IGD %s [%s] is in an invalid state (will be disabled)",
                  igd->getPublicIp().toString().c_str(),
                  protocol->getProtocolName().c_str());
        igdPublicIp_.erase({protocol->getProtocol(), publicIp});
        return;
    ...
    // Update the IGD list. The IGD must be valid.
    assert(igd->isValid());
    {
        std::lock_guard<std::mutex> lock(mappingMutex_);
        igdPublicIp_.emplace(protocol->getProtocol(), publicIp);
    }
...
UPnPContext::onMappingAdded(const std::shared_ptr<IGD>& igd, const Mapping& mapRes)
{
    ...
    if (not isValidThread()) {
        runOnUpnpContextThread([this, igd, mapRes] { onMappingAdded(igd, mapRes); });
        return;
    }
    ...
    if (map->isValid()) {
        ...
        if (map->getState() == MappingState::OPEN
            ...
            if (mapRes.getProtocol() == NatProtocolType::NAT_PMP) {
                map->setLifeTime(mapRes.getLifeTime());
                updateRenewalTime(mapRes.getLifeTime());
            }
#endif
            ...
            // We have a valid response, the IGD must be valid.
            // So, update if not already valid.
            if (not igd->isValid()) {
                igd->setValid(true);
                std::lock_guard<std::mutex> lock(mappingMutex_);
                igdPublicIp_.emplace(map->getProtocol(), igd->getPublicIp());
            }
        }
    } else {
...
UPnPContext::deleteMapping(const Mapping::sharedPtr_t& map)
{
    ...
    auto protocol = getProtocolInstance(map->getIgd()->getProtocol());
    protocol->requestMappingRemove(*map);
}
...
UPnPContext::registerMapping(Mapping& map)
{
    ...
    if (not hasValidIGD()) {
        ...
        requestMappingOnValidIgd(mapPtr);
    }
...
void
UPnPContext::registerAddMappingTimeout(const std::shared_ptr<UPnPProtocol>& protocol,
                                       const std::shared_ptr<IGD>& igd,
                                       const Mapping::sharedPtr_t& map)
{
    if (not map) {
    ...
    map->setTimeoutTimer(
        Manager::instance()
            .scheduler()
            .scheduleIn([this, protocol, igd, map] { onRequestTimeOut(protocol, igd, map); },
                        // The time-out is proportional to the number of "in-progress" requests.
                        MAP_REQUEST_TIMEOUT_UNIT * (status.inProgressCount_ + 1)));
}

void
UPnPContext::onRequestTimeOut(const std::shared_ptr<UPnPProtocol>& protocol,
                              const std::shared_ptr<IGD>& igd,
                              const Mapping::sharedPtr_t& map)
{
    CHECK_VALID_THREAD();
    ...
    if (not igd->isValid()) {
        std::lock_guard<std::mutex> lock(mappingMutex_);
        igdPublicIp_.erase({map->getProtocol(), igd->getPublicIp()});
    }
...
#ifdef HAVE_LIBNATPMP
void
UPnPContext::updateRenewalTime(uint32_t lifeTime)
{
    CHECK_VALID_THREAD();

    // The allocation is renewed when it reaches 75% of its lifetime.
    auto renewal = std::chrono::system_clock::now() + std::chrono::seconds(lifeTime * 3 / 4);

    if (renewal < renewal_) {
        std::time_t oldTime = std::chrono::system_clock::to_time_t(renewal_);
        std::time_t newTime = std::chrono::system_clock::to_time_t(renewal);
        std::ostringstream strm;

        JAMI_DBG() << "NAT-PMP renewal time " << oldTime << " updated to " << newTime;
...
UPnPContext::renewAllocations()
{
    ...
    if (std::chrono::system_clock::now() < renewal_)
        return;

    // Check if the we have valid PMP IGD.
    auto pmpProto = getProtocolInstance(NatProtocolType::NAT_PMP);
    ...
    for (auto type : {PortType::TCP, PortType::UDP}) {
        ...
        auto mappingList = getMappingList(type);
        for (auto const& [_, map] : mappingList) {
            if (map->getProtocol() == NatProtocolType::NAT_PMP && map->getIgd()->isValid()) {
