...
UPnPContext::requestMapping(const std::shared_ptr<IGD>& igd, const Mapping::sharedPtr_t& map)
{
    ...
    // Request the mapping.
    auto const& protocol = getProtocolInstance(igd->getProtocol());
...
UPnPContext::requestMappingOnValidIgds(const Mapping::sharedPtr_t& map)
{
    ...
    auto proto = getProtocolInstance(NatProtocolType::NAT_PMP);
    ...
    // Use PUPNP instead.
    proto = getProtocolInstance(NatProtocolType::PUPNP);
    if (proto->hasValidIgd()) {
        std::list<std::shared_ptr<IGD>> igdList;
        proto->getIgdList(igdList);
        for (auto const& igd : igdList)
...
UPnPContext::onIgdUpdate(const std::shared_ptr<IGD>& igd, UpnpIgdEvent event)
{
    ...
    assert(igd->isValid());
    {
        ...
        if (ret.second) {
            ...
            // Already in the list.
            JAMI_ERR("IGD %p [%s] on address %s already in the list",
                     igd.get(),
                     igd->getProtocolName().c_str(),
                     publicIp.toString(true, true).c_str());
            return;
                                       ...
                                       const Mapping::sharedPtr_t& map)
{
    ...
    map->setTimeoutTimer(
        ...
        Manager::instance().scheduler().scheduleIn([this, igd, map] { onRequestTimeOut(igd, map); },
                                                   MAP_REQUEST_TIMEOUT_UNIT
                                                       * (status.inProgressCount_ + 1)));
}
