...
UPnPContext::requestMapping(const std::shared_ptr<UPnPProtocol>& protocol,
    const std::shared_ptr<IGD>& igd, const Mapping::sharedPtr_t& map)
{
    assert(map);
...
void UPnPContext::requestMappingOnAllIgds(const Mapping::sharedPtr_t& map)
{
    if(not isValidThread()) {
...
void
UPnPContext::onIgdChanged(const std::shared_ptr<UPnPProtocol>& protocol, const std::shared_ptr<IGD>& igd,
...
void
UPnPContext::registerAddMappingTimeout(const std::shared_ptr<IGD>& igd, Mapping::key_t key)
{
...
void
UPnPContext::onRequestTimeOut(const std::shared_ptr<IGD>& igd, Mapping::key_t key)
{
    auto map = getMappingWithKey(key);
    if (not map)
        return;
