...
UPnPContext::clearCallbacks(const PortMapLocal& mapList, std::string ctrlId)
{
    ...
    for (auto& map : mapList) {
        JAMI_WARN("Evaluating controller mapping %u:%u %s",
                    map.second.getPortExternal(),
                    map.second.getPortInternal(),
                    map.second.getType() == upnp::PortType::UDP ? "UDP" : "TCP");
        for (auto& mapCb : mapCbList_) {
            JAMI_WARN("Compairing with context mapping %u:%u %s",
                       mapCb.first->getPortExternal(),
                       mapCb.first->getPortInternal(),
                       mapCb.first->getType() == upnp::PortType::UDP ? "UDP" : "TCP");
            if (map.second == *(mapCb.first)) {
                JAMI_WARN("found callback to remove");
...
UPnPContext::addIgdToList(UPnPProtocol* protocol, IGD* igd)
{
    ...
    for (auto& cbAdd : mapCbList_) {
        ...
        JAMI_WARN("UPnPContext: Sending out request in cb queue for mapping %u:%u %s",
...
UPnPContext::registerCallback(Mapping map, bool keepCb, std::string ctrlId, NotifyControllerCallback&& cbAdd, NotifyControllerCallback&& cbRm)
{
    ...
    JAMI_WARN("UPnPContext: Registering cb for mapping %u:%u %s",
              map.getPortExternal(),
              map.getPortInternal(),
