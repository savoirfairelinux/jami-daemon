...
UPnPContext::addMapping(NotifyControllerAddMapCallback&& cb, uint16_t port_desired, uint16_t port_local, PortType type, bool unique)
{
    ...
    if (not mapAlreadyPresent) {
        addPortMapCbList_.emplace(mapToAdd, cb);
    }
...
UPnPContext::onAddMapping(Mapping* mapping, bool success)
{
    ...
    for (const auto& cb: addPortMapCbList_) {
        if (cb.first->getPortExternal() == mapping->getPortExternal() and
            ...
            auto mapToErase = new Mapping(std::move(mapping->getPortExternal()), 
                                          std::move(mapping->getPortExternal()),
...
UPnPContext::addIgdToList(UPnPProtocol* protocol, IGD* igd)
{
    ...
    igdListLock.unlock();
    for (auto& cb : addPortMapCbList_) {
        addMapping(std::move(cb.second), cb.first->getPortExternal(), cb.first->getPortInternal(), cb.first->getType(), false);
    }
    igdListLock.lock();
