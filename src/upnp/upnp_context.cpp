...
UPnPContext::addAnyMapping(uint16_t port_desired, uint16_t port_local, PortType type, bool use_same_port, bool unique)
{
	...
	while (not mapping and
		  (upnp_err == UPnPProtocol::UpnpError::CONFLICT_IN_MAPPING or
		   upnp_err == UPnPProtocol::UpnpError::SOAP_INVALID_ARGS) and
		   numberRetries < MAX_RETRIES) {

		JAMI_DBG("UPnPContext: Failed to add mapping -> %s.", static_cast<int>(upnp_err));
