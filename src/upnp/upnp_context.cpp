...
void
UPnPContext::addCallback(Mapping map, bool keepCb, const std::string& ctrlId, NotifyControllerCallback&& cbAdd, NotifyControllerCallback&& cbRm)
{
...
void
UPnPContext::addMapping(NotifyControllerCallback&& cbAdd, NotifyControllerCallback&& cbRm, uint16_t portDesired, uint16_t portLocal, PortType type, bool unique, const std::string& ctrlId, bool keepCb)
{
    std::lock_guard<std::mutex> lk(igdListMutex_);
    ...
    if (igdList_.empty()) {
        ...
        registerCallback(Mapping(portDesired, portLocal, type, unique), keepCb, ctrlId, std::move(cbAdd), std::move(cbRm));
        return;
    }
...
UPnPContext::onAddMapping(IpAddr igdIp, Mapping* mapping, bool success)
{
    if (success and mapping) {
        JAMI_WARN("UPnPContext: Opened port %s", mapping->toString().c_str());
    } else {
        if (not success and mapping) 
            JAMI_WARN("UPnPContext: Failed to open port %s", mapping->toString().c_str());
        else
            JAMI_WARN("UPnPContext: Error during onAddMapping callback.");
    }
...
{
    if (success and mapping) {
        JAMI_WARN("UPnPContext: Closed port %s", mapping->toString().c_str());
    } else {
        if (not success and mapping) 
            JAMI_WARN("UPnPContext: Failed to close port %s", mapping->toString().c_str());
        else
            JAMI_WARN("UPnPContext: Error during onRemoveMapping callback.");
    }
...
UPnPContext::addIgdToList(UPnPProtocol* protocol, IGD* igd)
{
    ...
    for (auto& cbAdd : mapCbList_) {
        JAMI_WARN("UPnPContext: Sending out request in cb queue for mapping %s of controller %s", cbAdd.first->toString().c_str(), cbAdd.second.first.second.c_str());
        addMapping(std::move(cbAdd.second.second.first),
                   std::move(cbAdd.second.second.second),
                   cbAdd.first->getPortExternal(), cbAdd.first->getPortInternal(),
                   cbAdd.first->getType(), cbAdd.first->isUnique(),
                   cbAdd.second.first.second, cbAdd.second.first.first);
    }
    lk.lock();
...
UPnPContext::registerCallback(Mapping map, bool keepCb, std::string ctrlId, NotifyControllerCallback&& cbAdd, NotifyControllerCallback&& cbRm)
{
    ...
    JAMI_WARN("UPnPContext: Registering cb for mapping %s of %s", map.toString().c_str(), ctrlId.c_str());
    ...
    for (cbMapItr it = mapCbList_.begin(); it != mapCbList_.end(); it++) {
        if (*(it->first) == map) {
            if ((not it->second.first.first) or (force)) {
                JAMI_WARN("UPnPContext: Unregistering cb for mapping %s", map.toString().c_str());
                mapCbList_.erase(it);
            }
            return;
        }
...
UPnPContext::unregisterCallback(Mapping map, const std::string& ctrlId)
{
    ...
    for (cbMapItr it = mapCbList_.begin(); it != mapCbList_.end(); it++) {
        if ((*(it->first) == map) and (it->second.first.second == ctrlId)) {
            JAMI_WARN("UPnPContext: Unregistering cb for mapping %s of controller %s", map.toString().c_str(), ctrlId.c_str());
            mapCbList_.erase(it);
            return;
        }
    }
