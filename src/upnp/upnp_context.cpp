...
UPnPContext::updateMappingList(bool async)
{
    ...
    // Prune the mapping list. Dont perform if NAT-PMP is valid.
    if (not getProtocolInstance(NatProtocolType::NAT_PMP)->hasValidIgd()
        and getProtocolInstance(NatProtocolType::PUPNP)->hasValidIgd()) {
        pruneMappingList();
    }

#if HAVE_LIBNATPMP
    // Renew nat-pmp allocations
    if (getProtocolInstance(NatProtocolType::NAT_PMP)->hasValidIgd())
        renewAllocations();
#endif
}
