...
void
UPnPContext::addCallback(Mapping map, ControllerData ctrlData)
{
    std::lock_guard<std::mutex> lk(cbListMutex_);
...
void
UPnPContext::registerCallback(Mapping map, ControllerData ctrlData)
{
        ...
        Manager::instance().scheduler().scheduleIn(
            ...
            for (auto it = mapCbList_.cbegin(); it != mapCbList_.cend(); it++) {
                if (it->first == Mapping(portExt, portInt, type)) {
                    lk.unlock();
                    it->second.cbAdd(new Mapping(portExt, portInt, type), false);
                    lk.lock();
                }
            }
    ...
    pendingRmMapList_.emplace_back(
        ...
        Manager::instance().scheduler().scheduleIn(
            ...
            for (auto it = mapCbList_.cbegin(); it != mapCbList_.cend(); it++) {
                if (it->first == Mapping(portExt, portInt, type)) {
                    lk.unlock();
                    it->second.cbRm(new Mapping(portExt, portInt, type), false);
                    lk.lock();
                }
            }
    }, MAP_REQUEST_TIMEOUT)});
