...
UPnPContext::connectivityChanged()
{
    ...
    JAMI_DBG("connectivity changed");


    {
        LOCK_MUTEX(mappingListMutex_)
...
UPnPContext::getLocalIP() const
{
    ...
    for (auto& proto : protocolList_) {
        ...
        for (auto const& igd : igdList) {
            if (igd->getLocalIp()) {
                return igd->getLocalIp();
            }
        }
    }

    JAMI_WARN("No valid local IP available");
    return {};
}
...
UPnPContext::getExternalIP() const
{
    ...
    for (auto& proto : protocolList_) {
        ...
        for (auto const& igd : igdList) {
            if (igd->getPublicIp()) {
                return igd->getPublicIp();
            }
        }
    }

    JAMI_WARN("No valid public IP available");
    return {};
}
...
bool UPnPContext::closeUnusedMappings(PortType type, int portCount)
{
...
UPnPContext::updateMappingList(bool async)
{
    ...
    for (auto idx : {0, 1}) {
        ...
        if (toRequestCount > 0) {
            ...
            closeUnusedMappings(type, status.readyCount_ - maxOpenPortLimit_[idx]);
        }
    ...
    IpAddr publicIpAddr, bool added)
{
    ...
    // Assume invalid IGD, will be update at the end of this function.
    hasValidIgd_.store(false);
    ...
    // If we get here, we must have at least one valid IGD.
    hasValidIgd_.store(true);
...
UPnPContext::onMappingAdded(IpAddr igdIp, const Mapping& mapRes, bool success)
{
...

    {
        ...
        if (it == mappingList.end()) {
            JAMI_ERR("PUPnP: Received a response for an unknown mapping %s (on IGD %s)",
                mapRes.toString().c_str(), igdIp.toString().c_str());
            return;
        }
...
UPnPContext::unregisterMapping(Mapping::sharedPtr_t map)
{
    ...
    auto key = map->getMapKey();
    LOCK_MUTEX(mappingListMutex_)
    auto& mappingList = getMappingList(Mapping::getTypeFromMapKey(key));
    if (mappingList.erase(key) != 1) {
        JAMI_ERR("Trying to unregister an unknown mapping with key %lu ", key);
        return false;
    }
    return true;
}
