...
void
UPnPContext::addMapping(NotifyControllerCallback&& cbAdd, NotifyControllerCallback&& cbRm, uint16_t port_desired, uint16_t port_local, PortType type, bool unique, bool keepCb)
{
    ...
    for (auto const& igd : igdList_) {
        if (igd.second->isPortInUse(port_desired)) {
            igd.second->incrementNbOfUsers(port_desired);
            cbAdd(mapToAdd, true);
            return;
        }
    }
    ...
    for (auto& item : mapCbList_) {
        if (item.first->getPortExternal() == mapToAdd->getPortExternal() and
            ...
            JAMI_WARN("UPnPContext: Mapping is already in list");
            break;
        }
    }
    ...
    if (type == upnp::PortType::UDP) {
        PortMapGlobal::iterator it = igd->udpMappings_.find(port_desired);
        if (it == igd->udpMappings_.end())
            addMapping(igd, port_desired, port_local, type, upnp_err);
        else 
            JAMI_WARN("UPnPContext: IGD already contains mapping... ignoring request");
    } else {
        PortMapGlobal::iterator it = igd->tcpMappings_.find(port_desired);
        if (it == igd->tcpMappings_.end())
            addMapping(igd, port_desired, port_local, type, upnp_err);
        else
            JAMI_WARN("UPnPContext: IGD already contains mapping... ignoring request");
    }
...
UPnPContext::onAddMapping(Mapping* mapping, bool success)
{
    ...
    for (const auto& cb: mapCbList_) {
        if (cb.first->getPortExternal() == mapping->getPortExternal() and
            ...
            cb.second.second.first(cb.first, success);
        }
    }
    // Remove callbacks that we don't need to keep.
    for (const auto& cb: mapCbList_) {
        if (cb.first->getPortExternal() == mapping->getPortExternal() and
            cb.first->getPortInternal() == mapping->getPortInternal() and
            cb.first->getType() == mapping->getType()) {
...
UPnPContext::removeMapping(const Mapping& mapping)
{
    ...
    if (not igdList_.empty()) {
        for (auto const& igd : igdList_) {
            if (igd.second->isPortInUse(mapping.getPortExternal())) {
        ...
        for (auto& mapToRemove : mapCbList_) {
            if (mapToRemove.first->getPortExternal() == mapping->getPortExternal() and
                ...
                if (mapToRemove.second.second.second)
                    mapToRemove.second.second.second(mapToRemove.first, true);
            }
        }
        // Remove callbacks that we don't need to keep.
        for (const auto& cb: mapCbList_) {
...
UPnPContext::addMapping(IGD* igd, uint16_t port_external, uint16_t port_internal, PortType type, UPnPProtocol::UpnpError& upnp_error)
{
    ...
    // Check if this mapping already exists. If the mapping is the same, then we just need to 
    // increment the number of users globally. If the mapping is not the same, then we have to 
    // notify the controller that the adding failed, as the external port is used for something 
    // else. If the mapping doesn't exist, then try to add it.
    auto globalMappings = type == PortType::UDP ? &igd->udpMappings_ : &igd->tcpMappings_;
    ...
    if (iter != globalMappings->end()) {
        ...
        if (*mapping_ptr == mapping) {
            ...
            JAMI_DBG("PUPnP: Mapping already exists, incrementing number of users: %d",
                     iter->second.users_);
            Mapping* mapAdded = new Mapping(std::move(mapping.getPortExternal()),
                                            std::move(mapping.getPortInternal()),
                                            std::move(mapping.getType()));
            onAddMapping(mapAdded, true);
            return;
        } else {
            ...
            Mapping* mapError = new Mapping(std::move(mapping.getPortExternal()),
                                            std::move(mapping.getPortInternal()),
                                            std::move(mapping.getType()));
            onAddMapping(mapError, false);
            return;
        }
    }
