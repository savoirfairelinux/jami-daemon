...
{
    if (map.isValid()) {
        unregisterAddMappingTimeout(Mapping(map.getPortExternal(), map.getPortInternal(), map.getType(), map.isUnique()));
        if (success)
            addMappingToIgd(igdIp, Mapping(map.getPortExternal(), map.getPortInternal(), map.getType(), map.isUnique()));
        dispatchOnAddCallback(Mapping(map.getPortExternal(), map.getPortInternal(), map.getType(), map.isUnique()), success);
        if (success)
            unregisterCallback(Mapping(map.getPortExternal(), map.getPortInternal(), map.getType(), map.isUnique()));
    }
}
...
UPnPContext::onMappingRemoved(IpAddr igdIp, Mapping map, bool success)
{
    if (map.isValid()) {
        if (success)
            removeMappingFromIgd(igdIp, Mapping(map.getPortExternal(), map.getPortInternal(), map.getType(), map.isUnique()));
        dispatchOnRmCallback(Mapping(map.getPortExternal(), map.getPortInternal(), map.getType(), map.isUnique()), success);
        if (success)
            unregisterCallback(Mapping(map.getPortExternal(), map.getPortInternal(), map.getType(), map.isUnique()));
    }
}
...
UPnPContext::registerCallback(const Mapping& map, const ControllerData& ctrlData)
{
    ...
    mapCbList_.insert(std::make_pair<Mapping, ControllerData>(
                      std::move(Mapping(map.getPortExternal(), map.getPortInternal(), map.getType(), map.isUnique())),
    ...
    pendingAddMapList_.emplace_back(
        PendingMapRequest{Mapping(map), Manager::instance().scheduler().scheduleIn([this, mapReg = map] {
            ...
            for (auto const& cb : cbList)
                cb(new Mapping(mapReg.getPortExternal(), mapReg.getPortInternal(), mapReg.getType()), false);
    }, MAP_REQUEST_TIMEOUT)});
}
