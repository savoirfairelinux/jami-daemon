...
UPnPContext::addMapping(pControllerData ctrlData, uint16_t portDesired, uint16_t portLocal, PortType type, bool unique)
{
    ...
    registerCallback(Mapping(portDesired, portDesired, type), std::move(ctrlData));
...
UPnPContext::addIgdToList(UPnPProtocol* protocol, IGD* igd)
{
    ...
    for (auto& cbAdd : mapCbList_) {
        JAMI_WARN("UPnPContext: Sending out request in cb queue for mapping %s of controller %s", cbAdd.first->toString().c_str(), cbAdd.second.get()->memAddr.c_str());
...
UPnPContext::registerCallback(Mapping map, pControllerData ctrlData)
{
    ...
    JAMI_WARN("UPnPContext: Registering cb for mapping %s of %s", map.toString().c_str(), ctrlData.get()->memAddr.c_str());
    mapCbList_.emplace(std::move(new Mapping(map.getPortExternal(), map.getPortInternal(), map.getType(), map.isUnique())), 
                       std::make_unique<ControllerData>(ControllerData {ctrlData->memAddr, ctrlData->keepCb, std::move(ctrlData->cbAdd), std::move(ctrlData->cbRm)}));
}
...
UPnPContext::unregisterAddMappingTimeout(Mapping map)
{
    ...
    for (pendingMapItr it = pendingAddMapList_.begin(); it != pendingAddMapList_.end(); it++) {
        if (it->map == map) {
...
UPnPContext::unregisterRmMappingTimeout(Mapping map)
{
    ...
    for (pendingMapItr it = pendingRmMapList_.begin(); it != pendingRmMapList_.end(); it++) {
        if (it->map == map) {
            it->cleanupTask->cancel();
