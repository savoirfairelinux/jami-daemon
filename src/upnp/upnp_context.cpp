...
UPnPContext::addIgdToList(UPnPProtocol* protocol, IGD* igd)
{
    ...
    }
    {
        ...
        for (unsigned int i = 0; i < provisionPortList_.size(); i++) {
            JAMI_DBG("UPnPContext: sending out request for port %u provisioning", provisionPortList_[i]);
            protocol->requestMappingAdd(igd, provisionPortList_[i], provisionPortList_[i], (i%2 == 0)? upnp::PortType::UDP : upnp::PortType::TCP);
        }
    }
    return true;
}
