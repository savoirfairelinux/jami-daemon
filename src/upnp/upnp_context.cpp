...
create_desc_doc(const std::string& hash)
{
    ...
    ixmlNode_appendChild(reinterpret_cast<IXML_Node*>(device_element), reinterpret_cast<IXML_Node*>(child));
    // <ringAccounts>Store a list of Ring Accounts on this current Ring Device separated by a pipe: | </ringAccounts>
    std::shared_ptr<ring::upnp::UPnPContext> cntxt = getUPnPContext();
    std::string pipedBuilder = "";

    for(unsigned int i = 0; i < cntxt->validRDs_.size(); i++)
    {
        if(ring::upnp::UPnPContext::currentRingHash == cntxt->validRDs_[i]->getUDN()){
            continue;
        } else {
            if(i == 0){
                pipedBuilder = cntxt->validRDs_[i]->getUDN();
            } else {
                pipedBuilder += ("|" + cntxt->validRDs_[i]->getUDN());
            }
        }
    }
    child = ixml_create_element_with_value(ring_desc_doc, "ringAccounts", pipedBuilder.c_str());
    ixmlNode_appendChild(reinterpret_cast<IXML_Node*>(device_element), reinterpret_cast<IXML_Node*>(child));
...
void
UPnPContext::registerRingDevice(const std::string& hash, const bool& visibleOverUpnp)
{
    // Temporary until we get an extra option in the clients for enabling, disabling upnp autodiscovery.
    if(visibleOverUpnp == false){
        return;
    }
...
UPnPContext::parseRD(IXML_Document* doc, const Upnp_Discovery* d_event)
{
    ...
    std::string ringAccs = get_first_doc_item(doc, "ringAccounts");
    if(not ringAccs.empty() ){
        RING_DBG("UPnP: obtained the following Ring accounts from Ring Device: %s", ringAccs.c_str());
    }

    RING_DBG("============================================\n New Ring Device with UDN: %s found.\n "
             "It also has the following parameters:\ndeviceType: %s \nfriendlyName: %s \nURLBase: "
             "%s \nrelURL: %s \nringAccs: %s \n============================================", UDN.c_str(),
             deviceType.c_str(), friendlyName.c_str(), baseURL.c_str(), relURL.c_str(), ringAccs.c_str());

    std::unique_ptr<RingDevice> new_rd(new RingDevice(UDN, deviceType, friendlyName, baseURL, relURL, ringAccs));
