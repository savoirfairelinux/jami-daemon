...
void
UPnPContext::requestMappingAdd(pControllerData ctrlData, uint16_t portDesired, uint16_t portLocal, PortType type, bool unique)
{
    std::lock_guard<std::mutex> lk1(igdListMutex_);
    std::unique_lock<std::mutex> lk2(pendindRequestMutex_);
...
void
UPnPContext::onMappingAdded(IpAddr igdIp, Mapping* mapping, bool success)
{
    if (mapping) {
...
void
UPnPContext::requestMappingRemove(const Mapping& mapping)
{
    std::lock_guard<std::mutex> lk(igdListMutex_);
        ...
        for (auto const& igd : igdList_) {
            if (igd.second->isMapInUse(mapping.getPortExternal())) {
                if (igd.second->getNbOfUsers(mapping.getPortExternal()) > 1) {
                    ...
                    igd.first->requestMappingRemove(mapping);
                }
            }
        }
    }
...
void
UPnPContext::onMappingRemoved(IpAddr igdIp, Mapping* mapping, bool success)
{
    if (mapping) {
        if (success)
...
UPnPContext::addIgdToList(UPnPProtocol* protocol, IGD* igd)
{
    ...
    for (auto& cbAdd : mapCbList_) {
        JAMI_DBG("[upnp:controller@%s] sending out request in cb queue for mapping %s", cbAdd.second.get()->memAddr.c_str(), cbAdd.first->toString().c_str());
        registerAddMappingTimeout(Mapping(cbAdd.first->getPortExternal(), cbAdd.first->getPortInternal(), cbAdd.first->getType(), cbAdd.first->isUnique()));
...
UPnPContext::registerCallback(Mapping map, pControllerData ctrlData)
{
    ...
    JAMI_DBG("[upnp:controller@%s] registering cb for mapping %s", ctrlData.get()->memAddr.c_str(), map.toString().c_str());
    ...
    for (auto it = mapCbList_.begin(); it != mapCbList_.end(); it++) {
        if (*(it->first) == map) {
            if ((not it->second.get()->keepCb) or (force)) {
                JAMI_DBG("[upnp:controller@%s] unregistering cb for mapping %s",  it->second.get()->memAddr.c_str(), map.toString().c_str());
                mapCbList_.erase(it);
            }
            return;
        }
