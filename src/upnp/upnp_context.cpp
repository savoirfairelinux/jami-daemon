...
UPnPContext::onIgdUpdated(const std::shared_ptr<IGD>& igd, UpnpIgdEvent event)
{
    ...
    auto const& protocolName = igd->getProtocolName();

    // Check if IGD has a valid addresses.
    if (not igdAddr) {
        JAMI_ERR("[%s] IGD has an invalid local address", protocolName.c_str());
        return;
    }
    if (not igd->getPublicIp()) {
        JAMI_ERR("[%s] IGD IGD has an invalid public address", protocolName.c_str());
        return;
    }
