    ...
    pmpThread_ = std::thread([this]() {
        ...
        while (pmpRun_) {
            ...
            case NatPmpState::PMP_OPEN_PORT:
            {
                ...
                {
                    // Make a local copy of mappings we need to open.
            ...
            case NatPmpState::PMP_CLOSE_PORT:
            {
                ...
                {
                    // Make a local copy of mappings we need to close.
                    std::lock_guard<std::mutex> lk1(validIgdMutex_);
                    if (pmpIgd_) {
            ...
            case NatPmpState::PMP_RENEW:
            {
                ...
                {
                    // Make a local copy of mappings we need to renew.
                    std::lock_guard<std::mutex> lk1(validIgdMutex_);
                    if (pmpIgd_) {
                        if (not pmpIgd_->mapToRenewList_.empty()) {
                            for (unsigned int i = 0; i < pmpIgd_->mapToRenewList_.size(); i++) {
                                if (pmpIgd_->isMapUpForRenewal(pmpIgd_->mapToRenewList_[i], clock::now()))
