    ...
    pmpThread_ = std::thread([this]() {
        {
            ...
            while (pmpRun_) {
                ...
                if (ip_utils::getHostName(localHostBuf, INET_ADDRSTRLEN) < 0) {
                    ...
                    std::string gw = ip_utils::getGateway(localHostBuf,
                                                          ip_utils::subnet_mask::prefix_24bit);
...
NatPmp::addPortMapping(Mapping& mapping, bool renew)
{
    ...
    if (err < 0) {
        ...
        if (pmpIGD_) {
            ...
            lk2.unlock();
            mapToAdd.setInvalid();
            userCallbacks_.notifyRequestAddCb_(pmpIGD_->getPublicIp(), std::move(mapToAdd));
        }

        if (isErrorFatal(err)) {
            // Fatal error, disable this IGD.
            pmpIGD_->setValid(false);
            userCallbacks_.onIgdUpdate_(shared_from_this(),
                                        pmpIGD_,
                                        pmpIGD_->getPublicIp(),
                                        UpnpIgdEvent::FATAL_ERROR);
        }
            ...
            if (r < 0) {
                ...
                if (pmpIGD_) {
                    if (not renew) {
                        ...
                        lk2.unlock();
                        userCallbacks_.notifyRequestAddCb_(pmpIGD_->getPublicIp(),
                                                           std::move(mapToAdd));
                    } else {
...
NatPmp::removePortMapping(Mapping& mapping)
{
    ...
    if (err < 0) {
        ...
        if (pmpIGD_) {
            ...
            userCallbacks_.notifyRequestRemoveCb_(pmpIGD_->getPublicIp(), std::move(mapToRemove));
        }
        return;
    }
    while (pmpRun_) {
        ...
        if (r == NATPMP_TRYAGAIN) {
            ...
            if (pmpIGD_) {
                ...
                lk2.unlock();
                userCallbacks_.notifyRequestRemoveCb_(pmpIGD_->getPublicIp(),
