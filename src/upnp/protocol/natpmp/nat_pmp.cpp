...
NatPmp::NatPmp()
{
    ...
    pmpThread_ = std::thread([this]() {
        ...
        while (pmpRun_) {
            ...
            std::unique_lock<std::mutex> lkqueue(queueMutex_);
            pmpCv_.wait_for(lkqueue, std::chrono::milliseconds(750), [this, currentState = pmpState_] {
            ...
            case NatPmpState::INIT:
            {
                if (not initNatPmp()) {
                    ...
                    if (initRetry_ > MAX_INIT_RETRY) {
                        initRetry_ = 0;
                        updateState(NatPmpState::ERROR);
                        errorTimer_ = clock::now();
                        break;
                    }
                    restartTimer_ = clock::now();
                    updateState(NatPmpState::RESTART);
                    break;
                }
                isInit_ = true;
                initRetry_ = 0;
                updateState(NatPmpState::SEARCH);
                break;
            }
            ...
            case NatPmpState::SEARCH:
            {
                ...
                if (isRestart_)
                     isRestart_ = false;
                updateState(NatPmpState::CLOSE_ALL);
            }
            ...
            case NatPmpState::ERROR:
            {
                ...
                if (now - errorTimer_ >= std::chrono::seconds(30)) {
                    updateState(NatPmpState::RESTART, true);
                    errorTimer_ = clock::now();
                }
...
NatPmp::~NatPmp()
{
    ...
    updateState(NatPmpState::EXIT, true);
    pmpCv_.notify_all();
}
...
NatPmp::requestMappingAdd(IGD* igd, uint16_t port_external, uint16_t port_internal, PortType type)
{
    ...
    if (pmpIgd_->publicIp_ == igd->publicIp_) {
        ...
        updateState(NatPmpState::OPEN_PORT);
    }
...
NatPmp::requestMappingRemove(const Mapping& igdMapping)
{
    ...
    updateState(NatPmpState::CLOSE_PORT);
    pmpCv_.notify_all();
}

inline void
NatPmp::updateState(NatPmpState state, bool clearBeforeInsert)
{
