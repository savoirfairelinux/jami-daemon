...
void
NatPmp::addPortMapping(const PMPIGD& /*pmp_igd*/, natpmp_t& natpmp, Mapping& mapping, bool renew, bool remove)
{
    ...
    if (remove) {
        pendingRmMapRequestList_.emplace_back(PendingRmMapRequest{mapping.getPortExternal(),
        Manager::instance().scheduler().scheduleIn([this,
                                                    portExt = mapping.getPortExternal(),
                                                    type = mapping.getType()] {
            std::unique_lock<std::mutex> lk(validIgdMutex_);
            if (auto mapRmFail = getPmpIgd()->getMapping(portExt, type)){
                JAMI_WARN("NAT-PMP: Remove mapping request %s timed out", mapRmFail->toString().c_str());
                lk.unlock();
                notifyContextPortCloseCb_(getPmpIgd()->publicIp_, mapRmFail, false);
            }               
        }, MAP_REQUEST_TIMEOUT)});
    } else {
        if (not renew) {
            ...
            Manager::instance().scheduler().scheduleIn([this,
                                                        ...
                                                        type = mapping.getType()] {
                JAMI_WARN("NAT-PMP: Add maping request %u:%u %s timed out", portExt, portInt, type == upnp::PortType::UDP ? "UDP" : "TCP");
                std::unique_lock<std::mutex> lk(validIgdMutex_);
                Mapping* mapAddFail = new Mapping(portExt, portInt, type);
                lk.unlock();
                notifyContextPortOpenCb_(getPmpIgd()->publicIp_, mapAddFail, false);
            }, MAP_REQUEST_TIMEOUT)});
        }
    }   
                ...
                if (remove) {
                    JAMI_WARN("NAT-PMP: Closed port %s", mapping.toString().c_str());
                    {
                        ...
                        for (PendingRmMapItr it = pendingRmMapRequestList_.begin(); it != pendingRmMapRequestList_.end(); it++) {
                            if (it->externalPort == map->getPortExternal()) {
