...
#define NATPMP_INSERT_IN_QUEUE(state) \
{ \
    std::lock_guard<std::mutex> lk(queueMutex_); \
    stateQueue_.push(state); \
}

#define NATPMP_CLEAR_AND_INSERT_IN_QUEUE(state) \
{ \
    { \
        std::lock_guard<std::mutex> lk(queueMutex_); \
        ...
        while (pmpRun_) {
            ...
            case NatPmpState::INIT:
            {
                if (not initNatPmp()) {
                    ...
                    if (initRetry_ > MAX_INIT_RETRY) {
                        initRetry_ = 0;
                        NATPMP_INSERT_IN_QUEUE(NatPmpState::ERROR);
                ...
                initRetry_ = 0;
                if (isRestart_) {
                    NATPMP_INSERT_IN_QUEUE(NatPmpState::SEARCH);
                    isRestart_ = false;
                } else {
                    NATPMP_INSERT_IN_QUEUE(NatPmpState::IDLE);
                }
                break;
            }
            case NatPmpState::RESTART:
            {
                ...
                if (now - restartTimer_ >= std::chrono::seconds(2)) {
                    ...
                    NATPMP_INSERT_IN_QUEUE(NatPmpState::INIT);
                }
                break;
            }
            case NatPmpState::SEARCH:
            {
                ...
                if (pmpIgd_->clearAll_) {
                    NATPMP_INSERT_IN_QUEUE(NatPmpState::CLOSE_ALL);
                } else {
                    NATPMP_INSERT_IN_QUEUE(NatPmpState::IDLE);
                }
                break;
            ...
            case NatPmpState::ERROR:
            {
                ...
                if (now - errorTimer_ >= std::chrono::seconds(30)) {
                    NATPMP_CLEAR_AND_INSERT_IN_QUEUE(NatPmpState::RESTART);
                    errorTimer_ = clock::now();
                }
            ...
            if (pmpState_ != NatPmpState::ERROR and
                ...
                if (isIgdUpForRenewal(clock::now())) {
                    ...
                    if (isMappingUpForRenewal(clock::now())) {
                        NATPMP_INSERT_IN_QUEUE(NatPmpState::RENEW);
                    }
                }
            }
            updateState();
...
NatPmp::~NatPmp()
{
    ...
    NATPMP_INSERT_IN_QUEUE(NatPmpState::EXIT);
}
...
NatPmp::requestMappingAdd(IGD* igd, uint16_t port_external, uint16_t port_internal, PortType type)
{
    ...
    if (pmpIgd_->publicIp_ == igd->publicIp_) {
        ...
        NATPMP_INSERT_IN_QUEUE(NatPmpState::OPEN_PORT);
    }
...
NatPmp::requestMappingRemove(const Mapping& igdMapping)
{
    ...
    NATPMP_INSERT_IN_QUEUE(NatPmpState::CLOSE_PORT);
}
