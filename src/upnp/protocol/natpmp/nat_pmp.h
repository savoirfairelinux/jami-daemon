...
constexpr static auto IGD_RENEWAL_INTERVAL {std::chrono::seconds(60)};
...
private:
    ...
    // Get handle.
    natpmp_t& getHandle(const std::shared_ptr<PMPIGD>& igd);
    std::shared_ptr<PMPIGD> getIgd() const;
    bool isNewIgd(const IpAddr& igdAddr) const;
    // Add an IGD to local list.
    // Newly added IGDs are not used right away, at least, as long
    // the default IGD is valid.
    void addIgd(std::shared_ptr<PMPIGD> igd, bool blackList = false);
    ...
    bool matchLocalGateway(IpAddr* localGw, in_addr_t gateway) const;

    // Helpers to process user callbacks
    void processIgdUpdate(const std::shared_ptr<IGD>& igd, UpnpIgdEvent event);
    void processMappingAdded(const std::shared_ptr<IGD>& igd, const Mapping& map);
    void processMappingRemoved(const std::shared_ptr<IGD>& igd, const Mapping& map);
