...
constexpr static auto TIMEOUT_BEFORE_IGD_SEARCH_RETRY {std::chrono::seconds(5)};
// Time interval between two IGD refresh.
constexpr static auto IGD_RENEWAL_INTERVAL {std::chrono::seconds(60 * 5)};
...
public:
    ...
    virtual std::string getProtocolName() const override { return {"NAT-PMP"}; }
...
private:
    void initNatPmp();
    void searchForPmpIgd(const std::shared_ptr<PMPIGD>& igd);
    void removeAllMappings(const std::shared_ptr<PMPIGD>& igd) const;
    int readResponse(natpmp_t& handle, natpmpresp_t& response);
    int sendMappingRequest(const std::shared_ptr<PMPIGD>& igd,
    ...
    // Get first available IGD.
    std::shared_ptr<PMPIGD> getIgd() const;
    bool isNewIgd(std::shared_ptr<PMPIGD>& igd) const;
    // Add an IGD to local list.
    // Newly added IGDs are not used right away, at least, as long
    // the default IGD is valid.
    void addIgd(std::shared_ptr<PMPIGD> igd);
    // Get the matching instance from the local list.
    std::shared_ptr<PMPIGD> getIgdInstance(const std::shared_ptr<IGD>& igdIn);
    // Check if a gateway matches the local gateway
    bool matchLocalGateway(const IpAddr& localGw, in_addr_t gateway) const;

    // Helpers to process user callbacks
    void processIgdUpdate(const std::shared_ptr<PMPIGD>& igd, UpnpIgdEvent event);
    void processMappingAdded(const std::shared_ptr<PMPIGD>& igd, const Mapping& map);
    void processMappingRemoved(const std::shared_ptr<PMPIGD>& igd, const Mapping& map);
...
private:
    ...
    mutable std::mutex igdListMutex_;                 // Mutex used to protect igd list.
    std::list<std::shared_ptr<PMPIGD>> igdList_;      // List of valid IGDs.
    std::list<std::shared_ptr<PMPIGD>> igdBlackList_; // List of invalid IGDs.
};
