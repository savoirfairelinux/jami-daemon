...
constexpr static unsigned int MAX_OPEN_PORT_RETRY {5};
constexpr static unsigned int MAX_CLOSE_PORT_RETRY {5};
...
private:
    // Updates the state.
    void updateState(int state = -1, bool clearBeforeInsert = false);
    ...
    // Adds a port mapping.
    bool addPortMapping(Mapping map);

    // Removes a port mapping.
    bool removePortMapping(Mapping map);
...
private:
    ...
    NatPmpState pmpState_ {NatPmpState::PMP_INIT};  // State variable.
};
