...
PUPnP::PUPnP()
{
    ...
    if (static_cast<UPnPProtocol::UpnpError>(upnp_err) != UPnPProtocol::UpnpError::ERROR_OK) {
...
void
PUPnP::setOnIgdChanged(IgdListChangedCallback&& cb)
{
    ...
    *upnp_error = -1;
    ...
    if (iter != globalMappings->end()) {
        ...
        if (*mapping_ptr == mapping) {
			...
			*upnp_error = static_cast<int>(UPnPProtocol::UpnpError::ERROR_OK);
            ++(mapping_ptr->users);
            ...
            *upnp_error = static_cast<int>(UPnPProtocol::UpnpError::CONFLICT_IN_MAPPING);
            return {};
        }
    }
...
PUPnP::handleCtrlPtUPnPEvents(Upnp_EventType event_type, const void* event)
{
    ...
    case UPNP_DISCOVERY_SEARCH_RESULT:
    {
        ...
        if (static_cast<UPnPProtocol::UpnpError>(UpnpDiscovery_get_ErrCode(d_event)) != UPnPProtocol::UpnpError::ERROR_OK) {
        ...
        if (static_cast<UPnPProtocol::UpnpError>(upnp_err) != UPnPProtocol::UpnpError::ERROR_OK or not doc_desc_ptr) {
		...
		updateIgdListCb_(getPUPnP().get(), std::move(igd_candidate.get()), std::move(igd_candidate.get()->publicIp_), true);
    ...
    case UPNP_DISCOVERY_ADVERTISEMENT_BYEBYE:
    {
        ...
        for (auto it = validIGDs_.find(cpDeviceId); it != validIGDs_.end(); it++) {
			...
			updateIgdListCb_(getPUPnP().get(), igd_to_remove, igd_to_remove->publicIp_, false);
    ...
    return static_cast<int>(UPnPProtocol::UpnpError::ERROR_OK);
}

int
PUPnP::handleSubscriptionUPnPEvent(Upnp_EventType event_type, const void* event, const std::string& udn)
{
    std::lock_guard<std::mutex> cp_device_lock(cpDeviceMutex_);
...
PUPnP::actionRemoveMappingsByLocalIPAndDescription(const UPnPIGD& igd, const std::string& description)
{
    ...
    do {
        ...
        if( not response and static_cast<UPnPProtocol::UpnpError>(upnp_err) != UPnPProtocol::UpnpError::ERROR_OK) {
            return;
        }
        ...
        } else if (static_cast<UPnPProtocol::UpnpError>(std::stoi(errorCode)) == UPnPProtocol::UpnpError::ARRAY_IDX_INVALID or
				   static_cast<UPnPProtocol::UpnpError>(std::stoi(errorCode)) == UPnPProtocol::UpnpError::INVALID_ARGS) {
...
PUPnP::actionAddPortMapping(const UPnPIGD& igd, const Mapping& mapping, int* error_code)
{
    ...
    *error_code = static_cast<int>(UPnPProtocol::UpnpError::ERROR_OK);
