...
PUPnP::addMapping(IGD* igd, uint16_t port_external, uint16_t port_internal, PortType type, int *upnp_error)
{
    *upnp_error = -1;
    ...
    {
        JAMI_WARN("PUPnP: Port mapping %s of %s was ADDED successfully.", mapping.toString().c_str(), igd->publicIp_.toString().c_str());
...
PUPnP::removeMapping(const Mapping& mapping)
{
    ...
    std::shared_ptr<UPnPContext> context = getUPnPContext();
    std::lock_guard<std::mutex> igdListLock(context->igdListMutex_);

    // Get first valid IGD.
    IGD* igd = nullptr;
    if (not context->igdList_.empty()) {
        for (auto const& item : context->igdList_) {
            if (item.second) {
                igd = item.second;
                break;
            }
    ...
    if (not igd) {
        JAMI_WARN("PUPnP: no valid IGD available");
        return;
    }
...
PUPnP::actionRemoveMappingsByLocalIPAndDescription(const UPnPIGD& igd, const std::string& description)
{
    ...
    do {
        ...
        UpnpAddToAction(&action_ptr, "GetGenericPortMappingEntry", igd.getServiceType().c_str(), "NewPortMappingIndex", jami::to_string(entry_idx).c_str());
...
PUPnP::actionDeletePortMapping(const UPnPIGD& igd, const std::string& port_external, const std::string& protocol)
{
    ...
    JAMI_WARN("PUPnP: Port %s, %s of %s was REMOVED successfully.", port_external.c_str(), protocol.c_str(), igd.publicIp_.toString().c_str());
