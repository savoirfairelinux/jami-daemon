...
IGD::getMapping(in_port_t externalPort, upnp::PortType type) const
{
    ...
    if (it != mapList.end()) {
        if (it->first == externalPort) {
            return Mapping(it->second.getExternalPort(),
                           it->second.getInternalPort(),
                           it->second.getType());
        }
    }
    return Mapping(0, 0);
...
{
    if (not valid_ or errorsCounter_ >= MAX_ERRORS_COUNT)
        return MAX_ERRORS_COUNT;

    errorsCounter_++;
    return errorsCounter_;
}
