...
IGD::addMapInUse(Mapping map)
{
    ...
    for (GlobalMaptItr it = mapList.begin(); it != mapList.end(); it++) {
        if (it->second == map) {
            it->second.users_++;
            JAMI_WARN("IGD: Incrementing the number of users for %s to %u", it->second.toString().c_str(), it->second.users_);
            return;
        }
    }
    GlobalMapping globalMap = GlobalMapping(map);
    JAMI_WARN("IGD: Adding mapping %s to the list", globalMap.toString().c_str());
    ...
    for (GlobalMaptItr it = udpMappings_.begin(); it != udpMappings_.end(); it++) {
        if (it->first == externalPort) {
            if (it->second.users_ > 1) {
                it->second.users_--;
                JAMI_WARN("IGD: Decrementing the number of users for %s to %u", it->second.toString().c_str(), it->second.users_);
            } else {
                JAMI_WARN("IGD: Removing mapping %s from the list", it->second.toString().c_str());
    ...
    for (GlobalMaptItr it = tcpMappings_.begin(); it != tcpMappings_.end(); it++) {
        if (it->first == externalPort) {
            if (it->second.users_ > 1) {
                ...
                JAMI_WARN("IGD: Removing mapping %s from list", it->second.toString().c_str());
