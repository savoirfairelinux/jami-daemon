...
const std::string Mapping::MAPPING_STATE_STR[] = {"PENDING", "IN_PROGRESS", "FAILED", "OPEN"};
...
{
    std::lock_guard<std::mutex> lock(mutex_);
    std::string descr {UPNP_MAPPING_DESCRIPTION_PREFIX + "-" + getTypeStr(type_) + ":"
                       + std::to_string(externalAddr_.port_)};
    if (addState)
