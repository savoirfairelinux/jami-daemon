...
std::shared_ptr<NatPmp>
getNatPmp()
{
    static auto natPmp = std::make_shared<NatPmp>();
    return natPmp;
}
    ...
    : pmpThread_([this]() {
        ...
        while (pmpRun_) {
            ...
            if (pmpIGD_) {
					...
					updateIgdListCb_(getNatPmp().get(), pmp_igd.get(), pmp_igd.get()->publicIp_, false);
...
void
NatPmp::setOnIgdChanged(IgdListChangedCallback&& cb)
{
	updateIgdListCb_ = std::move(cb);
}

size_t
NatPmp::addIGDListener(IgdFoundCallback&& cb)
{
    JAMI_DBG("NAT-PMP: Adding IGD listener.");
...
void
NatPmp::removeIGDListener(size_t token)
{
    std::lock_guard<std::mutex> lock(validIGDMutex_);
    auto it = igdListeners_.find(token);
    if (it != igdListeners_.end()) {
        JAMI_DBG("NAT-PMP: Removing IGD listener.");
        igdListeners_.erase(it);
    }
}

Mapping
NatPmp::addMapping(IGD* igd, uint16_t port_external, uint16_t port_internal, PortType type, int *upnp_error)
{
    *upnp_error = -1;
        ...
        if (r < 0 && r != NATPMP_TRYAGAIN) {
            ...
            if (not pmpIGD_) {
                JAMI_DBG("NAT-PMP: Found device with external IP %s.", pmp_igd->publicIp_.toString().c_str());
                {
					...
					updateIgdListCb_(getNatPmp().get(), std::move(pmp_igd.get()), std::move(pmp_igd.get()->publicIp_), true);
