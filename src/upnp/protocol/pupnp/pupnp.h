...
private:
    ...
    // Helper to run tasks on PUPNP private execution queue.
    ...
    bool validateIgd(std::string location, IXML_Document* doc_container_ptr);
    ...
    // For some internal members (such as the validIgdList and the
    // hostAddress) that need to be synchronously accessed, are protected by
    // this mutex.
