...
PUPnP::PUPnP()
    : pupnpThread_([this]() { registerClientAsync(); })
{
    JAMI_WARN("PUPNP constructor begin");
    ...
    JAMI_WARN("PUPNP constructor end");
}
...
PUPnP::handleCtrlPtUPnPEvents(Upnp_EventType event_type, const void* event)
{
    ...
    std::lock_guard<std::mutex> cp_device_lock(pupnpMutex_);
