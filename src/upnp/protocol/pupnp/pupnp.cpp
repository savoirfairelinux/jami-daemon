...
PUPnP::incrementErrorsCounter(const std::shared_ptr<IGD>& igd)
{
    ...
    if (not igd->incrementErrorsCounter()) {
        ...
        igd->setValid(false);
        // Notify the listener.
        observer_->onIgdUpdate(igd, UpnpIgdEvent::INVALID_STATE);
    }
}
...
{
    return actionGetPortMappingsEntriesByDescr(igd, descr);
}
...
PUPnP::validateIgd(const IGDInfo& info)
{
    ...
    observer_->onIgdUpdate(igd_candidate, UpnpIgdEvent::ADDED);
...
{
    if (auto pupnp_igd = dynamic_cast<const UPnPIGD*>(igd.get())) {
    ...
    for (auto const& it : validIgdList_) {
        if (auto igd = std::dynamic_pointer_cast<UPnPIGD>(it)) {
            if (igd->getControlURL() == ctrlURL) {
                ...
                mapToAdd.setIgd(igd);
                observer_->onMappingAdded(igd, std::move(mapToAdd));
                return;
            }
        }
    }
    ...
    for (auto const& it : validIgdList_) {
        if (auto igd = std::dynamic_pointer_cast<UPnPIGD>(it)) {
            if (igd->getControlURL() == ctrlURL) {
                ...
                observer_->onMappingRemoved(igd, std::move(mapToRemove));
                return;
            }
        }
    }
...
PUPnP::processDiscoveryAdvertisementByebye(const std::string& cpDeviceId)
{
    ...
    for (auto it = validIgdList_.begin(); it != validIgdList_.end(); it++) {
        ...
        if (igd->getUID() == cpDeviceId) {
            // Notify the listener.
            observer_->onIgdUpdate(igd, UpnpIgdEvent::REMOVED);
            // Remove the IGD.
...
std::unique_ptr<std::map<Mapping::key_t, Mapping>>
PUPnP::actionGetPortMappingsEntriesByDescr(const std::shared_ptr<IGD>& igd,
                                           const std::string& description) const
{
    auto upnpIgd = dynamic_cast<UPnPIGD*>(igd.get());
    assert(upnpIgd);
    ...
    for (int entry_idx = 0;; entry_idx++) {
        ...
        if (IpAddr(client_ip) != upnpIgd->getLocalIp())
            continue;
    ...
    JAMI_DBG("PUPnP: Found %lu allocated mappings on IGD %s",
             mapList->size(),
             upnpIgd->getPublicIp().toString().c_str());
