...
PUPnP::incrementErrorsCounter(const std::shared_ptr<IGD>& igd)
{
    ...
    if (not igd->incrementErrorsCounter()) {
        ...
        igd->setValid(false);
        // Notify the listener.
        userCallbacks_.onIgdUpdate_(shared_from_this(), igd, UpnpIgdEvent::INVALID_STATE);
    }
}
...
PUPnP::validateIgd(const IGDInfo& info)
{
    ...
    userCallbacks_.onIgdUpdate_(shared_from_this(), igd_candidate, UpnpIgdEvent::ADDED);
    ...
    for (auto const& it : validIgdList_) {
        if (auto igd = std::dynamic_pointer_cast<UPnPIGD>(it)) {
            if (igd->getControlURL() == ctrlURL) {
                ...
                userCallbacks_.notifyRequestAddCb_(igd, std::move(mapToAdd));
                return;
            }
        }
    }
    ...
    for (auto const& it : validIgdList_) {
        if (auto igd = std::dynamic_pointer_cast<UPnPIGD>(it)) {
            if (igd->getControlURL() == ctrlURL) {
                ...
                userCallbacks_.notifyRequestRemoveCb_(igd, std::move(mapToRemove));
                return;
            }
        }
    }
...
PUPnP::processDiscoveryAdvertisementByebye(const std::string& cpDeviceId)
{
    ...
    for (auto it = validIgdList_.begin(); it != validIgdList_.end(); it++) {
        ...
        if (igd->getUID() == cpDeviceId) {
            // Notify the listener.
            userCallbacks_.onIgdUpdate_(shared_from_this(), igd, UpnpIgdEvent::REMOVED);
            // Remove the IGD.
