...
void
PUPnP::addMapping(IGD* igd, uint16_t port_external, uint16_t port_internal, PortType type)
{
        ...
        if (UpnpActionComplete_get_ErrCode(a_event) == UPNP_E_SUCCESS) {
            ...
            if (actionRequest) {
                ...
                if (xmlbuff) {
                    ...
                    case CtrlAction::ADD_MAP    : processAddMapAction(ctrlURL, actionRequest); break;
                    case CtrlAction::REMOVE_MAP : processRemoveMapAction(ctrlURL, actionRequest); break;
                    default: break;
                    }
                }
            }
...
PUPnP::getAction(char* xmlNode)
{
    if (strstr(xmlNode, ACTION_ADD_PORT_MAPPING)) {
        return CtrlAction::ADD_MAP;
    } else if (strstr(xmlNode, ACTION_DELETE_PORT_MAPPING)) {
        return CtrlAction::REMOVE_MAP;
    } else {
...
PUPnP::actionDeletePortMapping(const UPnPIGD& igd, const std::string& port_external, const std::string& protocol)
{
    ...
    UpnpAddToAction(&action_container_ptr, ACTION_ADD_PORT_MAPPING, igd.getServiceType().c_str(), "NewExternalPort", port_external.c_str());
    UpnpAddToAction(&action_container_ptr, ACTION_ADD_PORT_MAPPING, igd.getServiceType().c_str(), "NewProtocol", protocol.c_str());

    action.reset(action_container_ptr);

    int upnp_err = UpnpSendAction(ctrlptHandle_, igd.getControlURL().c_str(), igd.getServiceType().c_str(), nullptr, action.get(), &response_container_ptr);
    if(upnp_err != UPNP_E_SUCCESS) {
        JAMI_WARN("PUPnP: Failed to send %s from: %s, %d: %s", ACTION_ADD_PORT_MAPPING, igd.getServiceType().c_str(), upnp_err, UpnpGetErrorMessage(upnp_err));
        return false;
    }
