    ...
    pupnpThread_ = std::thread([this] {
        ...
        while (pupnpRun_) {
            ...
            if (clientRegistered_) {
                for (auto it = dwnldlXmlList_.begin(); it != dwnldlXmlList_.end();) {
                    auto future = *it;
                    lk.unlock();
                    if (future.wait_for(std::chrono::seconds(1)) == std::future_status::ready) {
                        ...
                        it = dwnldlXmlList_.erase(it);
                    } else {
                        lk.lock();
                        JAMI_WARN("PUPnP: XML download for %s timed out", future.get().location.c_str());
                        ++it;
                    }
                }
            }
...
PUPnP::handleCtrlPtUPnPEvents(Upnp_EventType event_type, const void* event)
{
    ...
    case UPNP_DISCOVERY_SEARCH_RESULT:
    {
        ...
        dwnldlXmlList_.emplace_back(dht::ThreadPool::io().getShared<IGDInfo>([this, location = std::move(igdLocationUrl)]{
            ...
            return IGDInfo {std::move(location), {nullptr, ixmlDocument_free}};
        }));
