...
PUPnP::~PUPnP()
{
    ...
    // Clear all the lists.
    {
        ...
        pendingAddMapRequestList_.clear();
        pendingRmMapRequestList_.clear();
        pupnpRun_ = false;
    }
...
PUPnP::processAddMapAction(const std::string& ctrlURL, IXML_Document* actionRequest)
{
    ...
    {
        // Cancel timeout task.
        std::lock_guard<std::mutex> lk(pendingMapRequestMutex_);
        for (PendingAddMapItr it = pendingAddMapRequestList_.begin(); it != pendingAddMapRequestList_.end(); it++) {
            if (it->map == *mapToAdd) {
...
PUPnP::processRemoveMapAction(const std::string& ctrlURL, IXML_Document* actionRequest)
{
    ...
    JAMI_WARN("PUPnP: Closed port %s %s", portExternal.c_str(), protocol.c_str());
    {
        // Cancel timeout task.
        std::lock_guard<std::mutex> lk(pendingMapRequestMutex_);
        for (PendingRmMapItr it = pendingRmMapRequestList_.begin(); it != pendingRmMapRequestList_.end(); it++) {
            if (it->externalPort == std::stoi(portExternal)) {
                it->cleanupTask->cancel();
            }
        }
    }
...
PUPnP::actionAddPortMappingAsync(const UPnPIGD& igd, const Mapping& mapping)
{
    ...
    pendingAddMapRequestList_.emplace_back(PendingAddMapRequest{Mapping(mapping.getPortExternal(),
                                                                        mapping.getPortInternal(),
                                                                        mapping.getType()),
    Manager::instance().scheduler().scheduleIn([this,
                                                portExt = mapping.getPortExternal(),
                                                portInt = mapping.getPortInternal(),
                                                type = mapping.getType(),controlUrl = igd.getControlURL()] {
        JAMI_WARN("PUPnP: Add maping request %u:%u %s timed out", portExt, portInt, type == upnp::PortType::UDP ? "UDP" : "TCP");
        std::unique_lock<std::mutex> lk(validIgdMutex_);
        for (auto const &it : validIgdList_) {
            if (auto igd = std::dynamic_pointer_cast<UPnPIGD>(it.second)) {
                if (igd->getControlURL() == controlUrl) {
                    Mapping* mapAddFail = new Mapping(portExt, portInt, type);
                    lk.unlock();
                    notifyContextPortOpenCb_(igd->publicIp_, mapAddFail, false);
                    return;
                }
            }
        }
...
PUPnP::actionDeletePortMappingAsync(const UPnPIGD& igd, const std::string& port_external, const std::string& protocol)
{
    ...
    pendingRmMapRequestList_.emplace_back(PendingRmMapRequest{std::stoi(port_external),
    Manager::instance().scheduler().scheduleIn([this,
                                                portExt = std::stoi(port_external),
                                                type = protocol == "UDP" ? "UDP" : "TCP",
                                                controlUrl = igd.getControlURL()] {
        std::unique_lock<std::mutex> lk(validIgdMutex_);
        for (auto const &it : validIgdList_) {
            if (auto igd = std::dynamic_pointer_cast<UPnPIGD>(it.second)) {
                if (igd->getControlURL() == controlUrl) {
                    if (auto mapToRemove = igd->getMapping(portExt, type == "UDP" ? upnp::PortType::UDP : upnp::PortType::TCP)) {
                        Mapping* mapRmFail = new Mapping(mapToRemove->getPortExternal(),
                                                    mapToRemove->getPortInternal(),
                                                    mapToRemove->getType());
                        JAMI_WARN("PUPnP: Remove mapping request %s timed out", mapRmFail->toString().c_str());
                        lk.unlock();
                        notifyContextPortCloseCb_(igd->publicIp_, mapRmFail, false);
                        return;
                    }
                }
            }
