    ...
    pupnpThread_ = std::thread([this] {
        ...
        while (pupnpRun_) {
            ...
            if (clientRegistered_) {
                ...
                for (const auto& item : downloadledXml_) {
                    ...
                    if (not item.second) {
                        ...
                        */
                        ...
                        if (upnp_err != UPNP_E_SUCCESS or not doc_desc_ptr) {
                            ...
                            validateIgd(doc_desc_ptr.get(), item.first);
                        }
                    }
                }
            }
...
PUPnP::handleCtrlPtUPnPEvents(Upnp_EventType event_type, const void* event)
{
    ...
    case UPNP_DISCOVERY_SEARCH_RESULT:
    {
        ...
        if ((downloadledXml_.count(igdLocationUrl) > 0) and 
            (downloadledXml_[igdLocationUrl])) {
                break;
        }
        downloadledXml_.emplace(std::move(igdLocationUrl), false);        
        pupnpCv_.notify_all();
    ...
    case UPNP_DISCOVERY_ADVERTISEMENT_BYEBYE:
    {
        ...
        downloadledXml_.erase(locationUrl);
