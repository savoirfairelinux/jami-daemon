...
PUPnP::setObserver(UpnpMappingObserver* obs)
{
    if (not isValidThread()) {
        getPUPnPScheduler()->run([w = weak(), obs] {
...
{
    if (not isValidThread()) {
        getPUPnPScheduler()->run([this] { terminate(); });
        return;
    }
...
bool
PUPnP::validateIgd(std::string location, IXML_Document* doc_container_ptr)
{
    CHECK_VALID_THREAD();
    ...
    runOnUpnpContextThread([w = weak(), igd_candidate] {
        if (auto upnpThis = w.lock()) {
...
{
    getPUPnPScheduler()->run([w = weak(), mapping] {
        if (auto upnpThis = w.lock()) {
...
PUPnP::processAddMapAction(const Mapping& map)
{
    ...
    runOnUpnpContextThread([w = weak(), map] {
        if (auto upnpThis = w.lock()) {
...
PUPnP::processRemoveMapAction(const Mapping& map)
{
    ...
    runOnUpnpContextThread([map, obs = observer_] {
...
PUPnP::downLoadIgdDescription(const std::string& locationUrl)
{
    ...
    if (upnp_err != UPNP_E_SUCCESS or not doc_container_ptr) {
        ...
        getPUPnPScheduler()->run([w = weak(), url = locationUrl, doc_container_ptr] {
            if (auto upnpThis = w.lock()) {
...
PUPnP::handleCtrlPtUPnPEvents(Upnp_EventType event_type, const void* event)
{
    ...
    case UPNP_DISCOVERY_ADVERTISEMENT_BYEBYE: {
        ...
        getPUPnPScheduler()->run([w = weak(), deviceId = std::move(deviceId)] {
            if (auto upnpThis = w.lock()) {
    ...
    case UPNP_EVENT_SUBSCRIPTION_EXPIRED: // This event will occur only if autorenewal is disabled.
    {
        ...
        // Process the response on the main thread.
        getPUPnPScheduler()->run([w = weak(), event_type, publisherUrl = std::move(publisherUrl)] {
            if (auto upnpThis = w.lock()) {
...
PUPnP::actionDeletePortMapping(const Mapping& mapping)
{
    ...
    processRemoveMapAction(mapping);
