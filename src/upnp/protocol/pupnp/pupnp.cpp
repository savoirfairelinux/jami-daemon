                ...
                if (xmlbuff) {
                    ...
                    if(removeMap) {
                        ...
                        for (auto const &it : validIgdList_) {
                            if (auto igd = std::dynamic_pointer_cast<UPnPIGD>(it.second)) {
                                if (igd->getControlURL() == ctrlURL) {
                                    if (igd->getNbOfUsers(std::stoi(port_external)) > 1) {
                                        igd->decrementNbOfUsers(std::stoi(port_external));
                                        break;
                                    } else {
                                        mapToRemove = igd->getMapping(std::stoi(port_external), protocol == "UDP" ? upnp::PortType::UDP : upnp::PortType::TCP);
                                        igd->removeMapInUse(std::stoi(port_external), protocol == "UDP" ? upnp::PortType::UDP : upnp::PortType::TCP);
                                        if (mapToRemove) { 
