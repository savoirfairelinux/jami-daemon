...
PUPnP::PUPnP()
{
    ...
    if (upnp_err != UPNP_E_SUCCESS) {
        ...
        if (ip_address6 and port6)
            JAMI_DBG("PUPnP: Initialiazed on address %s:%u | %s:%u", ip_address, port, ip_address6, port6);
        else
            JAMI_DBG("PUPnP: Initialiazed on address %s:%u", ip_address, port);
    ...
    pupnpThread_ = std::thread([this] {
        ...
        while (pupnpRun_) {
            ...
            if (not clientRegistered_) {
                ...
                int upnp_err = UpnpRegisterClient(ctrlPtCallback, this, &ctrlptHdl_);
                if (upnp_err != UPNP_E_SUCCESS) {
            ...
            if (clientRegistered_) {
                if (searchForIgd_.exchange(false)) {
                    ...
                    UpnpSearchAsync(ctrlptHdl_, SEARCH_TIMEOUT, UPNP_ROOT_DEVICE, this);
                    UpnpSearchAsync(ctrlptHdl_, SEARCH_TIMEOUT, UPNP_IGD_DEVICE, this);
                    UpnpSearchAsync(ctrlptHdl_, SEARCH_TIMEOUT, UPNP_WANIP_SERVICE, this);
                    UpnpSearchAsync(ctrlptHdl_, SEARCH_TIMEOUT, UPNP_WANPPP_SERVICE, this);
                }
...
PUPnP::validateIgd(const IgdInfo& info)
{
    ...
    int upnp_err = UpnpSubscribeAsync(ctrlptHdl_, eventSub.c_str(), SUBSCRIBE_TIMEOUT, subEventCallback, this);
    if (upnp_err != UPNP_E_SUCCESS)
        ...
        for (auto it = validIgdList_.begin(); it != validIgdList_.end(); it++) {
            if (auto igd = std::dynamic_pointer_cast<UPnPIGD>(it->second))
                if (igd->getEventSubURL() == eventSubUrl) {
                    UpnpSubscribeAsync(ctrlptHdl_, eventSubUrl.c_str(), SUBSCRIBE_TIMEOUT, subEventCallback, this);
                    break;
                }
        }
...
PUPnP::processRemoveMapAction(const std::string& ctrlURL, IXML_Document* actionRequest)
{
    ...
    for (auto const &it : validIgdList_) {
        if (auto igd = std::dynamic_pointer_cast<UPnPIGD>(it.second)) {
            if (igd->getControlURL() == ctrlURL) {
                Mapping mapToRemove = igd->getMapping(std::stoi(portExternal), protocol == "UDP" ? upnp::PortType::UDP : upnp::PortType::TCP);
                lk.unlock();
...
PUPnP::actionIsIgdConnected(const UPnPIGD& igd)
{
    ...
    int upnp_err = UpnpSendAction(ctrlptHdl_, igd.getControlURL().c_str(), igd.getServiceType().c_str(), nullptr, action.get(), &response_container_ptr);
    if (upnp_err != UPNP_E_SUCCESS) {
...
PUPnP::actionAddPortMappingAsync(const UPnPIGD& igd, const Mapping& mapping)
{
    ...
    int upnp_err = UpnpSendActionAsync(ctrlptHdl_, igd.getControlURL().c_str(), igd.getServiceType().c_str(), nullptr, action.get(), ctrlPtCallback, this);
