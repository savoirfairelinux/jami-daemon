...
bool
Controller::releaseMapping(Mapping::sharedPtr_t map)
{
    if (not upnpContext_)
        return false;
...
Controller::releaseAllMappings()
{
    ...
    for (auto [_,map] : mappingList_) {
        upnpContext_->releaseMapping(map);
    }
}

void
Controller::addLocalMap(Mapping::sharedPtr_t map)
{
...
bool
Controller::removeLocalMap(Mapping::sharedPtr_t map)
{
    if (not upnpContext_)
        return false;
