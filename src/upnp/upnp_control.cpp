    ...
    keepCb_(keepCb)
{
    ...
    JAMI_WARN("CREATING CONTROLLER %p : %s", memAddr_, memAddrStr_.c_str());
    try {
...
{
    JAMI_WARN("DESTROYING CONTROLLER %p : %s", memAddr_, memAddrStr_.c_str());
    if (upnpContext_) {
    ...
    if (isLocalMapPresent(portDesired, type)) {
        ...
        if (not unique) {
            ...
            if (upnpContext_->isMappingInUse(portDesired, type)) {
                ...
                upnpContext_->addCallback(std::move(Mapping(portDesired, portLocal, type)), keepCb_, memAddrStr_,
