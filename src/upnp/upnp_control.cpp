...
Controller::addMapping(NotifyServiceCallback&& cb, uint16_t port_desired, PortType type, bool unique, uint16_t port_local)
{
    ...
    if (portItr != instanceMappings.end()) {
        JAMI_WARN("Controller: TRYING TO NOTIFY SERVICE");
        cb(&port_desired, true);
    } else {
        // Check if another controller already has the mapping.
        for (Mapping& element : portList) {
            JAMI_WARN("Controller: port %u is in use", element.getPortExternal());
            JAMI_WARN("Controller: port desired is %u", port_desired);
            if (element.getPortExternal() == port_desired and 
                element.getPortInternal() == port_local and 
                element.getType() == type) {
                JAMI_WARN("Controller: Another controller has opened the port.");
                portList.emplace_back(std::move(Mapping(std::move(port_desired),
                                                        std::move(port_local),
                                                        std::move(type))));
                instanceMappings.emplace(port_desired, std::move(Mapping(std::move(port_desired),
                                                        std::move(port_local),
                                                        std::move(type))));                               
                cb(&port_desired, true);
                return;
        ...
        JAMI_WARN("Controller: REQUESTING MAPPING");
...
Controller::onAddMapping(Mapping* mapping, bool success)
{
    ...
    if (success and mapping) {
        ...
        Mapping portmap = Mapping(mapping->getPortExternal(),
                              mapping->getPortInternal(),
                              mapping->getType());
        portList.emplace_back(std::move(portmap));
...
Controller::removeMappings(PortType type) {
    ...
    auto& instanceMappings = type == PortType::UDP ? udpMappings_ : tcpMappings_;
    for (auto& map : instanceMappings) {
        Mapping mapToRemove = Mapping {map.second.getPortExternal(),
                                       map.second.getPortInternal(),
                                       map.second.getType()};
        std::vector<Mapping>::iterator it = std::find(portList.begin(), portList.end(), mapToRemove);
        if (it != portList.end())
            portList.erase(it);
