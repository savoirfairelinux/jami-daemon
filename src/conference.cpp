...
Conference::removeParticipant(const std::string& participant_id)
{
    ...
    if (auto call = getCall(participant_id)) {
        // TODO verify, does this work, else all sinks?
...
Conference::onConfOrder(const std::string& callId, const std::string& confOrder)
{
    ...
    if (auto call = Manager::instance().getCallFromCallID(callId)) {
        ...
        for (Json::Value::const_iterator itr = root.begin(); itr != root.end(); itr++) {
            ...
            if (isPeerModerator && key == "layout") {
                ...
                if (accValue.isMember("devices")) {
                    ...
                    for (Json::Value::const_iterator itrd = accValue["devices"].begin(); itrd != accValue["devices"].end(); itrd++) {
                        ...
                        if (deviceValue.isMember("medias")) {
                            for (Json::Value::const_iterator itrm = accValue["medias"].begin(); itrm != accValue["medias"].end(); itrm++) {
        ...
        // Old protocol (TODO move in another method)
...
void
Conference::muteSinkId(const std::string& accountUri, const std::string& deviceId, const std::string&, const bool& state)
{
...
Conference::updateMuted()
{
    ...
    for (auto& info : confInfo_) {
        ...
        } else if (auto call = getCallWith(info.uri, info.device)) {
...
Conference::kickDevice(const std::string& accountUri, const std::string& deviceId)
{
    ...
    // TODO new protocol
    auto remoteHost = findHostforRemoteParticipant(accountUri);
    if (remoteHost.empty()) {
