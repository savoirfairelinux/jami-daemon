...
#endif
{
     ...
     *
     * 1. Local is detached: mute state is irrelevant.
     * 2. Create a conference from existing calls:
     *  the mute state is set to false if the local mute state of
     *  at least one of the calls is false.
     *  If the video is disabled, the mute state is irrelevant.
     * 3. Attaching the local host to an existing conference:
     *  the audio and video is set to the default capture device
     *  (microphone and/or camera), and set to un-muted state. The
     *  local user can then mute the media if needed.
     *
     * Note that when a call is added to a conference, the media source
     * of the call is set to use the audio/video mixers, and the host
     * media source (e.g. camera), is added as a source to the mixer.
     * Also note that, by design, the mixers are never muted, but the
     * mixer can produce audio/video frames with no content (silence or
     * black video frames) if all the participants are muted.
     */
...
Conference::setLocalHostDefaultMediaSource()
{
    ...
    if (confState_ == State::ACTIVE_ATTACHED) {
        hostAudioSource_.type_ = MediaType::MEDIA_AUDIO;
        hostAudioSource_.enabled_ = true;
        hostAudioSource_.label_ = "audio_0";
        hostAudioSource_.sourceType_ = MediaSourceType::CAPTURE_DEVICE;
        hostAudioSource_.muted_ = false;
    } else {
        hostAudioSource_ = {};
    }
    ...
    if (isVideoEnabled()) {
        ...
        if (confState_ == State::ACTIVE_ATTACHED) {
            hostVideoSource_.type_ = MediaType::MEDIA_VIDEO;
            hostVideoSource_.enabled_ = true;
            hostVideoSource_.label_ = "video_0";
            hostVideoSource_.sourceType_ = MediaSourceType::CAPTURE_DEVICE;
            hostVideoSource_.sourceUri_
                = Manager::instance().getVideoManager().videoDeviceMonitor.getMRLForDefaultDevice();
            hostVideoSource_.muted_ = false;
        } else {
            hostVideoSource_ = {};
        }
...
Conference::addParticipant(const std::string& participant_id)
{
    ...
    if (participants_.insert(participant_id).second) {
        ...
        if (auto call = getCall(participant_id)) {
            ...
            // NOTE:
            // When a call joins a conference, the media sources of the
            // call will be set to the media mixer of the conference.
            // The current mute state of the call will be checked against the
            // mute state of the calls in the conference to determine the mute
            // state of the local host (if attached).
            // If the local host is muted, the local media source will be removed
            // from the mixer. By design, the mixer is never muted, the so mute state
            // of the call is set to false.
    ...
    if (getState() == State::ACTIVE_DETACHED) {
        ...
        if (isVideoEnabled()) {
            if (auto mixer = getVideoMixer()) {
                mixer->switchInput(hostVideoSource_.sourceUri_);
