...
Conference::~Conference()
{
    ...
    if (videoMixer_) { // TODO move confSinksMap_ in videoMixer
...
#ifdef ENABLE_VIDEO
    // TODO videoMixer_ => callStreamMgr
...
#ifdef ENABLE_VIDEO
        if (videoMixer_) { // TODO callStreamsMgr_
...
Conference::muteLocalHost(bool is_muted, const std::string& mediaType)
{
    ...
    if (mediaType.compare(libjami::Media::Details::MEDIA_TYPE_AUDIO) == 0) {
        ...
        if (is_muted) { // TODO test if necessary?
...
#ifdef ENABLE_VIDEO
    if (updateNeeded and videoMixer_) { // TODO!
...
Conference::getVideoMixer() const // TODO remove
{
    return std::dynamic_pointer_cast<video::VideoMixer>(callStreamsMgr_);
};
...
#ifdef ENABLE_VIDEO
    // Video
    if (videoMixer_) { // TODO delete video mixer
