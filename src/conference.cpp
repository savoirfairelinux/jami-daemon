                                     ...
                                     const std::vector<DRing::MediaMap>& remoteMediaList)
{
    ...
    // NOTE:
    // Since this is a conference, so accept any media change request.
    // This also means that if original call was an audio-only call, the
    // the local camera will enabled, unless the video is disabled in the
    // account settings.
