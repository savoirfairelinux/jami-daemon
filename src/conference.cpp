            ...
            for (const auto& info: infos) {
                ...
                if (auto videoMixer = shared->getVideoMixer())
                    active = info.source == videoMixer->getActiveParticipant()
                        or (uri.empty() and not videoMixer->getActiveParticipant()); // by default, local is shown as active
                newInfo.emplace_back(ParticipantInfo {
                    std::move(uri), active, info.x, info.y, info.w, info.h
                });
            }
            lk.unlock();
    ...
    for (const auto &participant_id : participants_) {
        if (auto call = Manager::instance().callFactory.getCall<SIPCall>(participant_id)) {
            ...
            call->sendTextMessage(
                    std::map<std::string, std::string> {{"application/confInfo+json", "[]"}},
