...
Conference::bindHostAudio()
{
    ...
    for (const auto& source : hostSources_) {
        ...
        if (hostAudioInput == hostAudioInputs_.end()) {
            hostAudioInput = hostAudioInputs_.emplace(source.label_, std::make_shared<AudioInput>(source.label_)).first;
        }
    ...
    for (const auto& item : getSubCalls()) {
        ...
        const auto medias = call->getAudioStreams();
...
Conference::unbindHostAudio()
{
    ...
    for (const auto& source : hostSources_) {
        ...
        // Reader bindings (host hearing participants) are intentionally
        // preserved so the host can still hear the conference while muted.
        auto bufferId = getHostSourceBufferId(source);
