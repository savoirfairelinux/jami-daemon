...
bool
Conference::matchesParticipant(const ParticipantInfo& info,
...
Conference::clearParticipantFromConference(const std::string& callId,
                                           ...
                                           const std::string& deviceId)
{
    JAMI_DEBUG("[conf:{:s}] Clearing participant: callId='{}', peerUri='{}', deviceId='{}'",
               id_,
               callId,
               peerUri,
               deviceId);
    ...
    {
        std::lock_guard lk(confInfoMutex_);
        // check for participant in confInfo_
        for (auto it = confInfo_.begin(); it != confInfo_.end();) {
            if (matchesParticipant(*it, callId, peerUri, deviceId)) {
        ...
        // check for participant in remoteHosts_
        ...
        if (layoutChanged) {
            JAMI_DEBUG("[conf:{:s}] Layout changed, sending updated infos", id_);
            sendConferenceInfos();
        }
    }
    // check if we have lingering voice activity for this callId
    if (!callId.empty()) {
