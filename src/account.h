...
public:
    ...
    // Check if a Daemon version (typically peer's version) supports
    // meets a minimum required version. Typically used to disable a
    // feature if it's not backward compatible with the peer's version.
    static bool hasMinimumRequiredVersion(std::vector<unsigned> jamiVersion,
