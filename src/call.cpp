...
{
    // ── OTel: start the daemon-side call span ──────────────────────────
    // Only create the span when there is an active parent scope on this
    // thread (i.e. the client's Scope for the root outgoing call, or an
    // incoming-call handler).  Outgoing subcalls are created on worker
    // threads with no scope; their span is set later in addSubCall().
    // For INCOMING calls we always create a root span here — it may be
    // re-parented later if the remote SDP carries a traceparent.
    try {
        if (type == CallType::INCOMING || jami::trace::hasActiveSpan()) {
            auto tracer = jami::trace::callsTracer();
            std::string spanName = (type == CallType::OUTGOING)
                                       ? "daemon.call.outgoing"
                                       : "daemon.call.incoming";
            auto span = tracer->StartSpan(spanName);
            span->SetAttribute("call.id", id_);
            span->SetAttribute("call.type",
                               jami::trace::callTypeStr(static_cast<uint8_t>(type_)));
    ...
    addStateListener([this](Call::CallState call_state, Call::ConnectionState cnx_state, int code) {
        ...
        if (type_ == CallType::INCOMING and cnx_state == ConnectionState::RINGING) {
            timeoutTimer_.expires_after(Manager::instance().getRingingTimeout());
...
Call::addSubCall(Call& subcall)
{
    ...
    // Create the subcall's daemon span as a child of this (parent) call's
    // span.  With hasActiveSpan() in Call::Call(), outgoing subcalls have no
    // span yet (callSpan_ == null).  End any stale span just in case.
    try {
        const char* name = (subcall.type_ == CallType::OUTGOING) ? "daemon.call.outgoing"
                                                                  : "daemon.call.incoming";
        if (auto* orphan = jami::trace::spanFromHandle(subcall.callSpan_))
            orphan->End();
        subcall.callSpan_ = jami::trace::makeSpanHandle(
            jami::trace::startChildSpan(callSpan_, name));
        if (auto* span = jami::trace::spanFromHandle(subcall.callSpan_)) {
            span->SetAttribute("call.id", subcall.id_);
            span->SetAttribute("call.type",
                               jami::trace::callTypeStr(static_cast<uint8_t>(subcall.type_)));
            if (auto acc = subcall.account_.lock())
                span->SetAttribute("call.account_id", acc->getAccountID());
        }
        subcall.onCallSpanReparented();
    } catch (...) {}
    ...
    subcall.addStateListener(
        [sub = subcall.weak_from_this(),
         parent = weak_from_this()](Call::CallState new_state, Call::ConnectionState new_cstate, int code) {
