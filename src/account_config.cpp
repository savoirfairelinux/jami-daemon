...
constexpr const char* DENY_SECOND_CALL_KEY = "denySecondCall";
...
AccountConfig::unserialize(const YAML::Node& node)
{
    ...
    parseValueOptional(node, DENY_SECOND_CALL_KEY, denySecondCallEnabled);
...
AccountConfig::toMap() const
{
    return {{Conf::CONFIG_ACCOUNT_ALIAS, alias},
            ...
            {Conf::CONFIG_ACCOUNT_DENY_SECOND_CALL, denySecondCallEnabled ? TRUE_STR : FALSE_STR},
...
AccountConfig::fromMap(const std::map<std::string, std::string>& details)
{
    ...
    parseBool(details, Conf::CONFIG_ACCOUNT_DENY_SECOND_CALL, denySecondCallEnabled);
