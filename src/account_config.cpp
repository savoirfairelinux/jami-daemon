...
constexpr const char* DENY_SECOND_CALLS_KEY = "denySecondCalls";
...
AccountConfig::serializeDiff(YAML::Emitter& out, const AccountConfig& DEFAULT_CONFIG) const
{
    ...
    SERIALIZE_CONFIG(DENY_SECOND_CALLS_KEY, denySecondCallsEnabled);
...
AccountConfig::unserialize(const YAML::Node& node)
{
    ...
    parseValueOptional(node, DENY_SECOND_CALLS_KEY, denySecondCallsEnabled);
...
AccountConfig::toMap() const
{
    return {{Conf::CONFIG_ACCOUNT_ALIAS, alias},
            ...
            {Conf::CONFIG_ACCOUNT_DENY_SECOND_CALLS, denySecondCallsEnabled ? TRUE_STR : FALSE_STR},
...
AccountConfig::fromMap(const std::map<std::string, std::string>& details)
{
    ...
    parseBool(details, Conf::CONFIG_ACCOUNT_DENY_SECOND_CALLS, denySecondCallsEnabled);
