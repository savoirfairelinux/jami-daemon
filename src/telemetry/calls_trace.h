...
namespace telemetry {
/**
 * Return a TracerProvider whose resource has a distinct service.name
 * ("jami-daemon-remote") so that spans created with it appear as a
 * separate service (different colour) in Jaeger.  Initialised once on
 * first call; shares the same exporter pipeline as the main provider.
 */
 ...
 * Uses a separate TracerProvider with service.name "jami-daemon-remote"
 * so the receiving side appears as a distinct service in Jaeger.
                          ...
                          const char* name)
{
    ...
    if (remote.IsValid())
        opts.parent = remote;
    auto provider = jami::telemetry::remoteTracerProvider();
    auto tracer = provider->GetTracer(kCallsTracerName, PACKAGE_VERSION);
    return tracer->StartSpan(name, opts);
}

/**
 * Start a span parented to a remote (cross-process) SpanContext using the
 * LOCAL callsTracer().  This keeps all spans — including the initial
 * incoming span and all its children — in the same TracerProvider, so the
 * exported OTLP data forms a coherent tree and Jaeger can resolve the
 * parentage without any "invalid parent" warnings.
 *
 * Prefer this over startSpanWithRemoteParent() for the receiving side's
 * root span; use startSpanWithRemoteParent() only when you deliberately
 * want a separate service/colour.
 */
inline opentelemetry::nostd::shared_ptr<opentelemetry::trace::Span>
startChildSpanWithRemoteContext(const opentelemetry::trace::SpanContext& remote,
                                const char* name)
{
    opentelemetry::trace::StartSpanOptions opts;
    if (remote.IsValid())
