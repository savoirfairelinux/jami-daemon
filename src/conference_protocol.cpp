        ...
        if (isPeerModerator && key == ProtocolKeys::LAYOUT) {
            ...
            if (accValue.isMember(ProtocolKeys::DEVICES)) {
                ...
                for (Json::Value::const_iterator itrd = accValue[ProtocolKeys::DEVICES].begin();
                    ...
                    if (deviceValue.isMember(ProtocolKeys::VOICEACTIVITY)) {
                        auto newState = deviceValue[ProtocolKeys::VOICEACTIVITY].asBool();
                        // TODO: check peerId_ == accountUri??
                        voiceActivity_(deviceId, newState);
                    }
