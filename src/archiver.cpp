                        ...
                        std::string password)
{
    ...
    fileutils::saveFile(toDir + DIR_SEPARATOR_STR + "test.ring", v);
...
Archiver::importAccounts(std::string archivePath, std::string password)
{
    ...
    try {
        ...
        for(int i = 0 ; i < root.size(); i++) {
            auto details = jsonValueToAccount(root["accounts"][i]);
            //ring::Manager::instance().addAccount(details);
        }
    } catch (const std::runtime_error& ex) {
...
Archiver::jsonValueToAccount(Json::Value& value) {
    ...
    for( Json::ValueIterator itr = value.begin() ; itr != value.end() ; itr++ ) {
        if (itr.key().asString().compare(DRing::Account::ConfProperties::TLS::CA_LIST_FILE) == 0 ||
                itr.key().asString().compare(DRing::Account::ConfProperties::TLS::CERTIFICATE_FILE) == 0 ||
                itr.key().asString().compare(DRing::Account::ConfProperties::TLS::PRIVATE_KEY_FILE) == 0) {
            // TODO: create files in correct path
        } else
