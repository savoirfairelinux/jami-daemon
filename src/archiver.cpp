...
Archiver::exportAccounts(std::vector<std::string> accountIDs,
                        ...
                        std::string password)
{
    //TODO: check if password is empty when it will be put in use
    if (toFile.empty() || !accountIDs.size()) {
        RING_ERR("Missing arguments");
        return;
    }

    std::size_t found = toFile.find_last_of(DIR_SEPARATOR_STR);
    auto toDir = toFile.substr(0,found);
    auto filename = toFile.substr(found+1);
    ...
    // Encrypt, using password provided
    ...
    // Write
    std::vector<uint8_t> v(compressed.begin(), compressed.end());
    fileutils::saveFile(toDir + DIR_SEPARATOR_STR + filename, v);
...
Archiver::importAccounts(std::string archivePath, std::string password)
{
    ...
    try {
        // Read
        auto fileContent = fileutils::loadFile(archivePath);
        std::string buffer = {fileContent.begin(), fileContent.end()};

        // Decompress
        std::string uncompressed = decompress_string(buffer);
        ...
        for (int i = 0 ; i < root["accounts"].size(); ++i) {
            ...
            auto newAccountID = ring::Manager::instance().getNewAccountId();
            auto details = jsonValueToAccount(root["accounts"][i], newAccountID);
            ring::Manager::instance().addAccount(details, newAccountID);
        }
    } catch (const std::runtime_error& ex) {
