                        ...
                        std::string password)
{
    ...
    auto key = dht::crypto::stretchKey(password);
    key.resize(256 / 8);
    auto encrypted = dht::crypto::aesEncrypt(compressed, key);
...
Archiver::importAccounts(std::string archivePath, std::string password)
{
    ...
    try {
        ...
        auto key = dht::crypto::stretchKey(password);
        key.resize(256 / 8);
        auto decrypted = dht::crypto::aesDecrypt(fileutils::loadFile(archivePath), key);

        // Decompress
        auto uncompressed = decompress(decrypted);

        // Decode string
        std::string decoded {uncompressed.begin(), uncompressed.end()};
