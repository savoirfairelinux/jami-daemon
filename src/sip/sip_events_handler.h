...
public:
    ...
    virtual std::weak_ptr<SipEventsHandler> weakPtr() = 0;
    /**
     * Get call Id.
     */
    virtual const std::string& getSipCallId() = 0;

    virtual void onReceivedCall(const std::string& from,
     ...
     * (aka re-invite)
     */
    [[deprecated("Replaced by onReceiveReinvite")]] virtual int onReceiveOffer(
        const pjmedia_sdp_session* offer, const pjsip_rx_data* rdata)
        = 0;

    virtual void onReceiveReinvite(const pjmedia_sdp_session* offer, pjsip_rx_data* rdata) = 0;

    virtual void onReceivedTextMessage(std::map<std::string, std::string> messages) = 0;

    virtual void onRequestInfo(pjsip_inv_session* inv, pjsip_rx_data* rdata, pjsip_msg* msg) = 0;
