                         ...
                         bool legacySdesOffer)
{
    ...
    if (mediaAttr.enabled_ and direction != DIRECTION_STR[MediaDirection::INACTIVE]) {
        ...
        if (type == MediaType::MEDIA_VIDEO) {
            ...
            if (sdpDirection_ != SdpDirection::ANSWER and not previousLocalSession) {
                nackPayloads.emplace_back("*");
            } else if (feedbackSession && remoteMedia) {
                nackPayloads = negotiatedExactRtcpFeedbackPayloads(feedbackSession,
                                                                   remoteMedia,
                                                                   med,
                                                                   "nack");
                pliFeedback = hasAnyRtcpFeedback(feedbackSession, remoteMedia, "nack pli");
                firFeedback = hasAnyRtcpFeedback(feedbackSession, remoteMedia, "ccm fir");
            }
