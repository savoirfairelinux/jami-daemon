...
static bool
isSupportedMedia(const pjmedia_sdp_media* media)
{
    return media
           and (pj_stricmp2(&media->desc.media, "audio") == 0
                or pj_stricmp2(&media->desc.media, "video") == 0);
}

static bool
matchesMediaType(const pjmedia_sdp_media* media, MediaType type)
{
    if (not media)
        return false;
    switch (type) {
    case MediaType::MEDIA_AUDIO:
        return pj_stricmp2(&media->desc.media, "audio") == 0;
    case MediaType::MEDIA_VIDEO:
        return pj_stricmp2(&media->desc.media, "video") == 0;
    default:
        return false;
    }
}
...
findEnabledMedia(const pjmedia_sdp_session* session, unsigned enabledIndex)
{
    ...
    for (unsigned index = 0; index < session->media_count; ++index) {
        ...
        if (media->desc.port == 0 or not isSupportedMedia(media))
            continue;
        if (enabledIndex-- == 0)
            return media;
    }
...
exactFeedbackPayloads(const pjmedia_sdp_session* remoteSession,
                         ...
                         bool legacySdesOffer)
{
    ...
    const auto* previousMediaAtIndex = findEnabledMedia(previousLocalSession, mediaIndex);
    const auto* previousLocalMedia = sdpDirection_ == SdpDirection::OFFER
                                             and matchesMediaType(previousMediaAtIndex, type)
                                         ? previousMediaAtIndex
                                         : nullptr;
    ...
    if (useBundle && mediaAttr.enabled_) {
        ...
        const auto* extmapSession = sdpDirection_ == SdpDirection::ANSWER
                                        ? remoteSession_
                                        : previousLocalMedia ? previousLocalSession : nullptr;
        if (extmapSession) {
    ...
    if (mediaAttr.enabled_ and direction != DIRECTION_STR[MediaDirection::INACTIVE]) {
        ...
        auto transportCcFeedback = sdpDirection_ != SdpDirection::ANSWER and not previousLocalMedia;
        auto googRembFeedback = sdpDirection_ != SdpDirection::ANSWER and not previousLocalMedia;

        const auto* feedbackSession = sdpDirection_ == SdpDirection::ANSWER
                                          ? remoteSession_
                                          : previousLocalMedia ? previousLocalSession : nullptr;
        if (feedbackSession) {
        ...
        if (type == MediaType::MEDIA_VIDEO) {
            ...
            if (sdpDirection_ != SdpDirection::ANSWER and not previousLocalMedia) {
                ...
                nackPayloads = exactFeedbackPayloads(feedbackSession, feedbackMedia, med, "nack");
