    ...
    if (stream.mediaAttribute_->type_ == MediaType::MEDIA_VIDEO) {
        ...
        if (auto const& rtpSession = stream.rtpSession_) {
            if (auto codec = rtpSession->getCodec()) {
                ...
                details.emplace(DRing::Call::Details::VIDEO_MIN_BITRATE, std::to_string(codec->systemCodecInfo.minBitrate));
                details.emplace(DRing::Call::Details::VIDEO_MAX_BITRATE, std::to_string(codec->systemCodecInfo.maxBitrate));
                details.emplace(DRing::Call::Details::VIDEO_BITRATE, std::to_string(codec->systemCodecInfo.bitrate));            
            }
            else
