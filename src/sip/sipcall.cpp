...
SIPCall::SIPSessionReinvite(const std::vector<MediaAttribute>& mediaAttrList, bool needNewIce)
{
    ...
    size_t previousMediaIndex = 0;
    for (auto& media : offerMediaAttrList) {
        if (not media.enabled_)
            continue;
        while (previousMediaIndex < previousLocalMedia.size()
               and not previousLocalMedia[previousMediaIndex].enabled) {
            ++previousMediaIndex;
        }
        if (previousMediaIndex < previousLocalMedia.size()
            and media.type_ == previousLocalMedia[previousMediaIndex].type
            and not previousLocalMedia[previousMediaIndex].mid.empty()) {
            media.label_ = previousLocalMedia[previousMediaIndex].mid;
        }
        ++previousMediaIndex;
    }
...
void
SIPCall::rejectMediaChangeRequest(int statusCode)
{
    if (not inviteSession_ or not inviteSession_->invite_tsx)
        return;

    pjsip_tx_data* tdata = nullptr;
    auto status = pjsip_inv_answer(inviteSession_.get(), statusCode, nullptr, nullptr, &tdata);
    if (status != PJ_SUCCESS or not tdata) {
        JAMI_ERROR("[call:{}] Unable to reject media change request (pjsip: {})",
                   getCallId(),
    ...
    status = pjsip_inv_send_msg(inviteSession_.get(), tdata);
    if (status != PJ_SUCCESS) {
        JAMI_ERROR("[call:{}] Unable to send media change rejection (pjsip: {})",
                   getCallId(),
                   sip_utils::sip_strerror(status));
        setInviteSession();
    }
...
void
SIPCall::answerMediaChangeWithExternalSdp(const std::string& sdp)
{
    std::lock_guard lk {callMutex_};

    if (not hasExternalMedia()) {
        JAMI_ERROR("[call:{}] Cannot use an external SDP answer for local media", getCallId());
        rejectMediaChangeRequest(PJSIP_SC_NOT_ACCEPTABLE_HERE);
        return;
    }
    auto account = getSIPAccount();
    if (not account or not inviteSession_ or not sdp_) {
        ...
        rejectMediaChangeRequest(PJSIP_SC_INTERNAL_SERVER_ERROR);
        return;
    }
    ...
    pjsip_tx_data* tdata = nullptr;
    if (pjsip_inv_answer(inviteSession_.get(), PJSIP_SC_OK, nullptr, nullptr, &tdata) != PJ_SUCCESS or not tdata) {
...
SIPCall::onReceiveReinvite(const pjmedia_sdp_session* offer, pjsip_rx_data* rdata)
{
    ...
    const auto sendTrying = [&] {
        pjsip_tx_data* tdata = nullptr;
        const auto status
            = pjsip_inv_initial_answer(inviteSession_.get(), rdata, PJSIP_SC_TRYING, nullptr, nullptr, &tdata);
        if (status != PJ_SUCCESS)
