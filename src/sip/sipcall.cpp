...
SIPCall::SIPSessionReinvite(const std::vector<MediaAttribute>& mediaAttrList)
{
    ...
    if (isIceEnabled()) {
        reinvIceMedia_ = createIceMediaTransport();
        if (initIceMediaTransport(reinvIceMedia_, true))
            addLocalIceAttributes(reinvIceMedia_);
    }
...
SIPCall::answer(const std::vector<DRing::MediaMap>& mediaList)
{
    ...
    if (newMediaAttrList.empty()) {
        ...
        if (newMediaAttrList.size() != rtpStreams_.size()) {
        ...
        if (isIceEnabled()) {
            ...
            if (publicAddr) {
                ...
                if (auto interfaceAddr = ip_utils::getInterfaceAddr(account->getLocalInterface(),
                    ...
                    setIceMedia(createIceMediaTransport());
                    if (initIceMediaTransport(getIceMedia(), true, std::move(opts)))
                        addLocalIceAttributes(getIceMedia());
                } else {
...
SIPCall::onIceNegoSucceed()
{
    ...
    // If this is a re-invite, update the transport to use the new one.
    if (reinvIceMedia_ and iceMedia_) {
        std::lock_guard<std::mutex> lk(transportMtx_);
        resetTransport(std::move(iceMedia_));
        iceMedia_ = std::move(reinvIceMedia_);
    }
...
SIPCall::setupIceResponse(bool isReinvite)
{
    ...
    if (isReinvite) {
        reinvIceMedia_ = createIceMediaTransport();
        success = initIceMediaTransport(reinvIceMedia_, false, opt);
    } else {
        setIceMedia(createIceMediaTransport());
        success = initIceMediaTransport(getIceMedia(), false, opt);
    }
