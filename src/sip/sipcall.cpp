...
SIPCall::setupLocalSDPFromIce()
{
    ...
    header << "[call:" << getCallId().c_str() << "] Local SDP session offer:\n";
    Sdp::printSession(sdp_->getLocalSdpSession(), header.str().c_str());
}
...
#else
// TODO_MC. Delete this version ASAP.
void
SIPCall::muteMedia(const std::string& mediaType, bool mute)
{
...
{
    // TODO_MC. sdp argument is never used in fact. The validation
