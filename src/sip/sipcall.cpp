...
SIPCall::onIceNegoSucceed()
{
    ...
    std::unique_lock<std::mutex> lk2(rtpStreamsMtx_);
...
SIPCall::createSinks(ConfInfo& infos)
{
    ...
    for (auto& [key, callInfo] : infos.callInfo_) {
        auto& uri = key.first; auto& device = key.second;
        std::map<std::string, std::string> replaceStreams;
        for (auto& [streamId, streamInfo] : callInfo.streams) {
            if (string_remove_suffix(uri, '@') == account_.lock()->getUsername()
                && device == std::dynamic_pointer_cast<JamiAccount>(account_.lock())->currentDeviceId()) {
                for (auto iter = rtpStreams_.begin(); iter != rtpStreams_.end(); iter++) {
                    ...
                    if (streamId.find(mediaAttribute->label_) != std::string::npos) {
                        replaceStreams[streamId] = mediaAttribute->sourceUri_;
                        local2RemoteSinks_[mediaAttribute->sourceUri_] = streamId;
                        streamInfo.videoMuted = mediaAttribute->muted_;
                        streamInfo.w = size.first;
                        streamInfo.h = size.second;
                        streamInfo.x = 0;
                        streamInfo.y = 0;
                    }
                }
        ...
        for (const auto& [k,v]: replaceStreams) {
            if (k != v) {
                callInfo.streams[v] = std::move(callInfo.streams[k]);
                callInfo.streams.erase(k);
            }
        }
    }
...
SIPCall::peerMuted(bool muted)
{
    ...
    peerMuted_ = muted;
// TODO    if (auto conf = conf_.lock())
// TODO        conf->updateMuted();
}
