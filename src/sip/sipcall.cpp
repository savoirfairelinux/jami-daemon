...
static constexpr int ICE_AUDIO_RTP_COMPID {1};
static constexpr int ICE_AUDIO_RTCP_COMPID {2};
static constexpr int ICE_VIDEO_RTP_COMPID {3};
static constexpr int ICE_VIDEO_RTCP_COMPID {4};
#endif
...
SIPCall::createRtpSession(RtpStream& stream)
{
    ...
    JAMI_DBG("[call:%s] Added media stream - type: %s",
             getCallId().c_str(),
...
SIPCall::SIPSessionReinvite()
{
    ...
    getMediaAttributeList(mediaList);
    assert(not mediaList.empty());

    // TODO_MC.
    // We should erase existing streams only after the ones where
...
SIPCall::setupLocalSDPFromIce()
{
    ...
    for (auto const& stream : rtpStreams_) {
        JAMI_DBG("Add ICE candidates for media [%s] at index %u",
    ...
    Sdp::printSession(sdp_->getLocalSdpSession(), "Local SDP session offer:\n");
}
...
SIPCall::addMediaStream(const MediaAttribute& mediaAttr)
{
...
#if 1
    // TODO_MC. Set default media source. This should be temporary, as the caller
    // is required to provide the source.
#ifdef ENABLE_VIDEO
    ...
    return static_cast<int>(rtpStreams_.size()) - 1;
}
...
SIPCall::startAllMedia()
{
    ...
    for (const auto& slot : slots) {
        ...
        configureRtpSession(rtpStream.rtpSession_,
                            rtpStream.mediaAttribute_->sourceUri_,
                            local,
                            remote);
...
SIPCall::muteMedia(const std::string& mediaType, bool mute)
{
    ...
    JAMI_ERR("[call:%s] muteMedia must not be called!", getCallId().c_str());
    return;
#endif
...
bool
SIPCall::updateMediaStream(const MediaAttribute& newMediaAttr)
{
    auto streamIdx = findRtpStreamIndex(newMediaAttr.label_);
    if (streamIdx == rtpStreams_.size()) {
        JAMI_ERR("[call:%s] Unknown media %s", getCallId().c_str(), newMediaAttr.label_.c_str());
        return false;
    }

    auto requireSipReinvite = updateMediaStreamInternal(newMediaAttr, streamIdx);

    if (requireSipReinvite) {
        // Send a re-invite and re-start media.
        requestReinvite();
    }
...
SIPCall::updateMediaStreams(const std::vector<MediaAttribute>& mediaAttrList)
{
    ...
    for (auto const& newMediaAttr : mediaAttrList) {
        ...
        if (streamIdx == rtpStreams_.size()) {
            ...
            // TODO_MC.
            // We creating an RTP that will never be used. we only need
            // a place holder for the media attribute.
            ...
            requireSipReinvite |= updateMediaStreamInternal(newMediaAttr, streamIdx);
        }
    }
