...
SIPCall::SIPCall(std::weak_ptr<SIPAccountBase> account, const std::string& id, Call::CallType type,
...
SIPCall::hangup(int reason)
{
    ...
    JAMI_ERR("LOCK? HANGUP START FOR %s", shared->getAccountID().c_str());
    std::lock_guard<std::recursive_mutex> lk {callMutex_};
    if (inv and inv->dlg) {
...
SIPCall::toggleRecording()
{
    ...
    if (not Call::isRecording()) {
        ...
        ss << "Conversation at %TIMESTAMP between "
            << account->getUserUri() << " and " << peerUri_;
