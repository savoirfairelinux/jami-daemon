...
SIPCall::rebindSipDialogLocked(const std::shared_ptr<SipTransport>& transport, const std::string& contact)
{
    ...
    auto* dialog = inviteSession_->dlg;
    auto status = pjsip_dlg_set_transport(dialog, &selector);
    if (status != PJ_SUCCESS) {
        JAMI_ERROR("[call:{}] Could not rebind SIP dialog: {}", getCallId(), sip_utils::sip_strerror(status));
        return false;
    }

    // The peer's Contact may use another address family than the new channel,
    // which pjsip refuses for in-dialog requests: target the peer at the other
    // end of the channel, and advertise our end of it.
    pjsip_dlg_inc_lock(dialog);
    auto* target = pjsip_uri_get_uri(dialog->target);
    if (PJSIP_URI_SCHEME_IS_SIP(target) or PJSIP_URI_SCHEME_IS_SIPS(target)) {
        auto* uri = static_cast<pjsip_sip_uri*>(pjsip_uri_clone(dialog->pool, target));
        pj_strdup(dialog->pool, &uri->host, &transport->get()->remote_name.host);
        uri->port = transport->get()->remote_name.port;
        dialog->target = reinterpret_cast<pjsip_uri*>(uri);
    }
    static constexpr pj_str_t STR_CONTACT = sip_utils::CONST_PJ_STR("Contact");
    pj_str_t localContact;
    pj_strdup2_with_null(dialog->pool, &localContact, contact.c_str());
    if (auto* header = static_cast<pjsip_contact_hdr*>(
            pjsip_parse_hdr(dialog->pool, &STR_CONTACT, localContact.ptr, localContact.slen, nullptr)))
        dialog->local.contact = header;
    pjsip_dlg_dec_lock(dialog);

    // Requests sent by the peer before following us must not bring us back.
    std::erase_if(abandonedTransports_, [](const auto& abandoned) { return abandoned.expired(); });
    abandonedTransports_.emplace_back(sipTransport_);
    setSipTransport(transport, contact);
    return true;
}

bool
SIPCall::isAbandonedTransportLocked(const std::shared_ptr<SipTransport>& transport) const
{
    return std::any_of(abandonedTransports_.begin(), abandonedTransports_.end(), [&](const auto& abandoned) {
        return abandoned.lock() == transport;
    });
}
