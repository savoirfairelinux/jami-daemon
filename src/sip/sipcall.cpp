    ...
    , sdp_(new Sdp(callId))
{
    ...
    getSIPAccount()->createDefaultMediaList(mediasList,
                                            getSIPAccount()->isVideoEnabled() and not isAudioOnly(),
                                            getState() == CallState::HOLD);
    initMediaStreams(mediasList);
}
...
RtpSession*
SIPCall::getRptSessionAtIndex(unsigned index)
{
    if (index < rtpStreams_.size()) {
        return rtpStreams_.at(index).get();
    }
...
RtpSession*
SIPCall::addRtpSession(unsigned int streamIndex, MediaType type)
{
    RtpSession* rtpSession = nullptr;

    if (type == MediaType::MEDIA_AUDIO) {
        auto& it = rtpStreams_.emplace_back(std::make_unique<AudioRtpSession>(id_, streamIndex));
        rtpSession = it.get();
    }
#ifdef ENABLE_VIDEO
    else if (type == MediaType::MEDIA_VIDEO) {
        auto& it = rtpStreams_.emplace_back(
            std::make_unique<video::VideoRtpSession>(id_, streamIndex, getVideoSettings()));
        rtpSession = it.get();
    }
#endif
    else {
...
void
SIPCall::configureRtpSession(RtpSession* rtpSession,
                             ...
                             const MediaDescription& remoteMedia)
{
    ...
    if (localMedia.type == MediaType::MEDIA_VIDEO) {
        auto videoRtp = dynamic_cast<video::VideoRtpSession*>(rtpSession);
        assert(videoRtp != nullptr);
...
SIPCall::SIPSessionReinvite()
{
    ...
    // TODO_MC. This list should be provided by the client.
    std::vector<MediaAttributes> mediasList;
    acc->createDefaultMediaList(mediasList,
                                acc->isVideoEnabled() and not isAudioOnly(),
                                getState() == CallState::HOLD);

    if (not sdp_->createOffer(mediasList))
        return !PJ_SUCCESS;
...
size_t
SIPCall::initMediaStreams(std::vector<MediaAttributes> mediasList)
{
    for (size_t idx = 0; idx < mediasList.size(); idx++) {
        const auto& local = mediasList.at(idx);

        if (local.type_ != MEDIA_AUDIO && local.type_ != MEDIA_VIDEO) {
            JAMI_ERR("[call:%s] Unexpected media type %u", getCallId().c_str(), local.type_);
            assert(false);
        }

        // Create and the RTP session.
        auto rtpSession = addRtpSession(idx, local.type_);
        assert(rtpSession != nullptr);
    }
...
SIPCall::startAllMedia()
{
    ...
    for (const auto& slot : slots) {
        ...
        auto rtpSession = getRptSessionAtIndex(slotN);
        assert(rtpSession != nullptr);
...
SIPCall::onReceiveOffer(const pjmedia_sdp_session* offer)
{
    ...
    // TODO_MC. If ICE is enabled, it must server-reflexive candidates if available.
    openPortsUPnP(acc->isVideoEnabled() ? 4 : 2);
    ...
    // TODO_MC. This list should be provided by the client.
    std::vector<MediaAttributes> mediasList;
    acc->createDefaultMediaList(mediasList, acc->isVideoEnabled(), getState() == CallState::HOLD);

    sdp_->receiveOffer(offer, mediasList);
...
bool
SIPCall::openPortsUPnP(int portCount)
{
    if (not upnp_)
        return false;

    // Try to allocate the requested number of port mappings.
    JAMI_DBG("[call:%s] Opening ports via UPNP for SDP session", getCallId().c_str());

    for (int i = 0; i < portCount; i++) {
        auto map = upnp_->reserveMapping(0, upnp::PortType::UDP);
        if (map and map->isValid()) {
            // Append
        } else {
            // Abort on first error.
            JAMI_WARN("[call:%s] Failed to allocate %i requested mapping",
                      getCallId().c_str(),
                      portCount);
            return false;
        }
