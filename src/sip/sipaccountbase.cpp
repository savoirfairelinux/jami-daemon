...
std::string
getDisplayed(const std::string& conversationUri, const std::string& messageId)
{
    ...
    if (uri.scheme() == URI::SchemeType::SWARM)
        ss << "<conversation>" << uri.userinfo() << "</conversation>";
...
bool
SIPAccountBase::setMessageDisplayed(const std::string& conversationUri, const std::string& messageId, int status)
{
    if (status == (int) DRing::Account::MessageStates::DISPLAYED)
        sendInstantMessage(conversationUri, {{MIME_TYPE_IMDN, getDisplayed(conversationUri, messageId)}});
    return true;
}
