...
SIPAccountBase::sendPaymentVerificationMessage(const std::string& conversationUri, bool isRequiring)
{
    ...
    sendInstantMessage(uid, {{MIME_TYPE_IM_PAYMENT, getPayment(conversationId, isRequiring, isClient, isService, allowCall)});
}
        ...
        for (const auto& m : payloads) {
            ...
            if (m.first == MIME_TYPE_IM_COMPOSING) {
                ...
                try {
                    ...
                    onIsAnswering(conversationId, from, isAnswering);
                    if (payloads.size() == 1)
                        return;
                } catch (const std::exception& e) {
