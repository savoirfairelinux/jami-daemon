...
static constexpr const char MIME_TYPE_IM_PAYMENT[] {"plugin/im-payment+xml"};
...
std::string
getPayment(const std::string& conversationId,
           bool isRequiring,
           bool isClient,
           bool isService,
           bool allowCall)
{
    ...
    return fmt::format("<?xml version=\"1.0\" encoding=\"utf-8\" ?>\n"
                       ...
                       "<client>{}</client>"
                       "<service>{}</service>"
                       "<allowcall>{}</allowcall>{}</payment>",
                       isRequiring ? "true"sv : "false"sv,
                       isClient ? "true"sv : "false"sv,
                       isService ? "true"sv : "false"sv,
                       allowCall ? "true"sv : "false"sv,
                       conversationId.empty()
                           ? ""
                           : "<conversation>" + conversationId + "</conversation>");
}
...
void
SIPAccountBase::sendPaymentVerificationMessage(const std::string& conversationUri, bool isRequiring)
{
...
SIPAccountBase::setIsComposing(const std::string& conversationUri, bool isWriting)
{
        ...
        if (isWriting) {
            ...
            composingUri_.clear();
            composingUri_.insert(composingUri_.end(),
                                 conversationUri.begin(),
                                 conversationUri.end());
                              ...
                              const std::map<std::string, std::string>& payloads)
{
        ...
        for (const auto& m : payloads) {
            ...
            } else if (m.first == MIME_TYPE_IM_PAYMENT) {
                JAMI_INFO() << "CALL REQUISITION EXCHANGE!";
                try {
                    static const std::regex PAYMENT_REQUEST_REGEX(
                        "<request>\\s*(\\w+)\\s*<\\/request>");
                    std::smatch matched_pattern;
                    std::regex_search(m.second, matched_pattern, PAYMENT_REQUEST_REGEX);
                    bool isRequest {false};
                    if (matched_pattern.ready() && !matched_pattern.empty()
                        && matched_pattern[1].matched) {
                        isRequest = matched_pattern[1] == "true";
                    }
                    ...
                    if (isRequest) {
                        JAMI_INFO() << "CALL REQUESTED!";
                        static const std::regex PAYMENT_CLIENT_REGEX(
                            "<client>\\s*(\\w+)\\s*<\\/client>");
                        std::smatch matched_pattern;
                        std::regex_search(m.second, matched_pattern, PAYMENT_CLIENT_REGEX);
                        bool isClient {false};
                        if (matched_pattern.ready() && !matched_pattern.empty()
                            && matched_pattern[1].matched) {
                        ...
                        static const std::regex PAYMENT_ALLOWCALL_REGEX(
                            "<allowcall>\\s*(\\w+)\\s*<\\/allowcall>");
                        std::smatch matched_pattern;
                        std::regex_search(m.second, matched_pattern, PAYMENT_ALLOWCALL_REGEX);
                        bool allowCall {false};
                        if (matched_pattern.ready() && !matched_pattern.empty()
                            && matched_pattern[1].matched) {
                            allowCall = matched_pattern[1] == "true";
                        }
                        if (allowCall)
                            JAMI_INFO() << "CALL IM PAYMENT ANSWERED POSITIVE!";
                        else
                            JAMI_INFO() << "CALL IM PAYMENT ANSWERED NEGATIVE!";
                    }

                    onIsAnswering(conversationId, from, isRequest);
                    if (payloads.size() == 1)
                        return;
                } catch (const std::exception& e) {
                    JAMI_WARN("Error parsing payment state: %s", e.what());
                }
            } else if (m.first == MIME_TYPE_IMDN) {
                try {
                    static const std::regex IMDN_MSG_ID_REGEX(
                        "<message-id>\\s*(\\w+)\\s*<\\/message-id>");
                    static const std::regex IMDN_STATUS_REGEX(
                        "<status>\\s*<(\\w+)\\/>\\s*<\\/status>");
                    std::smatch matched_pattern;
                    ...
                    if (conversationId.empty()) // Old method
                        messageEngine_.onMessageDisplayed(from,
                                                          from_hex_string(messageId),
                                                          isDisplayed);
                    else if (isDisplayed) {
        ...
        emitSignal<DRing::ConfigurationSignal::IncomingAccountMessage>(accountID_,
                                                                       from,
                                                                       id,
                                                                       cm->data);
