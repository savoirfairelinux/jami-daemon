...
{
    JAMI_WARN("SIPAccount::doRegister() call");
    if (not isUsable()) {
        JAMI_WARN("Account must be enabled and active to register, ignoring");
        return;
    }

    bool isUpnpPortOpened = false;
    std::list<uint16_t> openedPorts = upnp_->getPorts();
    for (auto& port : openedPorts) {
        JAMI_WARN("SIPAccount: Port %u is opened", port);
        if (port == publishedPort_) {
            isUpnpPortOpened = true;
            break;
        }
    }
    ...
    if ((upnp_ ) and (not isUpnpPortOpened)) {
