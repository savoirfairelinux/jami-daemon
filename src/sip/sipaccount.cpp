...
SIPAccount::sendTextMessage(const std::string& to, const std::map<std::string, std::string>& payloads, uint64_t id)
{
    ...
    *std::remove(date, date+strlen(date), '\n') = '\0';
    ...
    status = pjsip_endpt_send_request(link_->getEndpoint(), tdata, -1, t, &onComplete);
...
SIPAccount::onComplete(void *token, pjsip_event *event)
{
    ...
    if (code == PJSIP_SC_UNAUTHORIZED || code == PJSIP_SC_PROXY_AUTHENTICATION_REQUIRED) {
        ...
        if (status == PJ_SUCCESS) {
            ...
            // Resend request
            status = pjsip_endpt_send_request(acc->link_->getEndpoint(), new_request, -1, c, &onComplete);
    ...
    status = pjsip_auth_clt_deinit(&(acc->auth_sess_));

    if (status != PJ_SUCCESS) {
        JAMI_ERR("Unable to deinitilize auth session: %s", sip_utils::sip_strerror(status).c_str());
    }
