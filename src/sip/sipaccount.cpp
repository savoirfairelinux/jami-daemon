...
SIPAccount::SIPAccount(const std::string& accountID, bool presenceEnabled)
    ...
    , regc_(nullptr)
    , bRegister_(false)
    , registrationExpire_(DEFAULT_REGISTRATION_TIME)
    , serviceRoute_()
    , cred_()
    , tlsSetting_()
    , ciphers_(100)
...
SIPAccount::serialize(YAML::Emitter& out) const
{
    ...
    out << YAML::Key << Conf::KEEP_ALIVE_ENABLED << YAML::Value << autoRegistrationEnabled_;
...
SIPAccount::unserialize(const YAML::Node& node)
{
    ...
    if (not isIP2IP()) {
        unsigned expire = 0;
        parseValue(node, Conf::CONFIG_ACCOUNT_REGISTRATION_EXPIRE, expire);
        if (expire < MIN_REGISTRATION_TIME) {
            JAMI_WARN("SIP registration EXPIRE %u is lower than min value %u",
                      expire,
                      MIN_REGISTRATION_TIME);
            expire = MIN_REGISTRATION_TIME;
        }
        setRegistrationExpire(expire);

        parseValue(node, Conf::KEEP_ALIVE_ENABLED, autoRegistrationEnabled_);
...
SIPAccount::setAccountDetails(const std::map<std::string, std::string>& details)
{
    ...
    parseBool(details, Conf::CONFIG_KEEP_ALIVE_ENABLED, autoRegistrationEnabled_);
...
SIPAccount::getAccountDetails() const
{
    ...
    a.emplace(Conf::CONFIG_KEEP_ALIVE_ENABLED, autoRegistrationEnabled_ ? TRUE_STR : FALSE_STR);
...
SIPAccount::sendRegister()
{
    ...
    // Enable auto-regsiter after expire
    if ((status
...
SIPAccount::onRegister(pjsip_regc_cbparam* param)
{
...
#if 1
                // Use PJSIP auto-register after expire.
#else
    ...
    if (param->expiration != registrationExpire_) {
        JAMI_WARN("Registrar set new register expire value %u", param->expiration);
