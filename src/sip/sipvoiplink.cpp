...
transaction_request_cb(pjsip_rx_data* rdata)
{
    ...
    // Use SipEventsHandler instead of SIPCall. We dont want to give access
    // to SIPCall from this file.
    std::shared_ptr<SipEventsHandler> call = account->newIncomingCall(std::string(remote_user),
                                                                      remoteMediaList,
                                                                      transport);
    call->onReceivedCall(std::string(remote_user), peerNumber, r_sdp, rdata, transport);
...
#undef TRY
    ...
    eventsPollScheduler_ = std::make_shared<ScheduledExecutor>();
    eventsPollScheduler_->run([this] {
        if (sip_utils::register_thread() == PJ_SUCCESS) {
            JAMI_DBG("Events thread registered. Start probing SIP events");
        } else {
            running_ = false;
        }
    });
    ...
    sipExecQueue_ = std::make_unique<AsyncExecutionQueue>();

    // Use the manager (main) scheduler for SIP processing.
    auto scheduler = &Manager::instance().instance().scheduler();
    sipExecQueue_->initialize(scheduler);

    sipExecQueue_->runOnExecQueue([this] {
        if (sip_utils::register_thread() != PJ_SUCCESS) {
            JAMI_DBG("Failed to register SIP execution thread");
        }
    });
...
{
    // TODO_MC. Fix this. Needs multi-stream flag.
#if 1
    if (auto call = getSipEventsHandler(inv)) {
...
transaction_state_changed_cb(pjsip_inv_session* inv, pjsip_transaction* tsx, pjsip_event* event)
{
    ...
    if (methodName == "REFER")
        ...
        call->onRequestInfo(inv, rdata, msg);
...
bool
SIPVoIPLink::registerCall(void* call, pjsip_inv_session* invite)
{
    pjsip_inv_add_ref(invite);

    std::lock_guard<std::mutex> lock(inviteMutex_);
    invite->mod_data[getModId()] = call;

    if (not(inviteMaps_.emplace(call, invite)).second) {
        JAMI_ERR("Call [%p] already has a registered invite session", call);
        return false;
    }

    JAMI_DBG("Call [%p] registered (invite session [%p])", call, invite);

    return true;
}

bool
SIPVoIPLink::unregisterCall(void* call, pjsip_inv_session* invite)
{
    {
        std::lock_guard<std::mutex> lock(inviteMutex_);

        if (inviteMaps_.erase(call) != 1) {
            JAMI_ERR("Failed to remove call [%p]", call);
            return false;
        }
    }

    pjsip_inv_dec_ref(invite);

    JAMI_DBG("Call [%p] unregistered (invite session [%p])", call, invite);
...
{
    // The invite session pointer must be valid.
    if (inv == nullptr) {
        throw VoipLinkException("Unexpected null pointer for invite session");
    }

    std::lock_guard<std::mutex> lock(inviteMutex_);

    auto const& iter = inviteMaps_.find(inv->mod_data[mod_ua_.id]);

    // Ignore if the call was already removed.
    if (iter == inviteMaps_.end()) {
        JAMI_WARN("Invite session [%p] does not match any call", inv);
        return {};
    }

    // The invite session pointers must match.
    if (iter->second != inv) {
        throw VoipLinkException("Mismatch in invite session");
    }

    if (auto call_ptr = static_cast<Call*>(inv->mod_data[mod_ua_.id])) {
        return std::dynamic_pointer_cast<SipEventsHandler>(call_ptr->shared_from_this());
    } else {
        throw VoipLinkException("Static cast to SipEventHandler pointer failed");
    }
}
