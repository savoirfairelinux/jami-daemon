...
#define TRY(ret) do { \
    ...
    if (not ns.empty()) {
        ...
        if (auto ret = pjsip_endpt_create_resolver(endpt_, &resv)) {
            RING_ERR("Error creating SIP DNS resolver: %s", sip_utils::sip_strerror(ret).c_str());
        } else {
            if (auto ret = pj_dns_resolver_set_ns(resv, ns.size(), dns_nameservers.data(), nullptr)) {
                RING_ERR("Error setting SIP DNS servers: %s", sip_utils::sip_strerror(ret).c_str());
            } else {
                if (auto ret = pjsip_endpt_set_resolver(endpt_, resv)) {
                    RING_ERR("Error setting pjsip DNS resolver: %s", sip_utils::sip_strerror(ret).c_str());
                }
            }
        }
    }
