...
[[deprecated("Replaced on_rx_reinvite")]] static void sdp_request_offer_cb(
    pjsip_inv_session* inv, const pjmedia_sdp_session* offer);
...
static pj_status_t
on_rx_reinvite(pjsip_inv_session* inv, const pjmedia_sdp_session* offer, pjsip_rx_data* rdata)
{
    if (auto call = getCallFromInvite(inv)) {
        auto const& account = call->getAccount().lock();
        if (account and account->isMultiStreamEnabled()) {
            return call->onReceiveReinvite(inv, offer, rdata);
        }
    }
...
sdp_request_offer_cb(pjsip_inv_session* inv, const pjmedia_sdp_session* offer)
{
    if (auto call = getCallFromInvite(inv)) {
        auto const& account = call->getAccount().lock();
        if (account and not account->isMultiStreamEnabled()) {
            return call->onReceiveOffer(offer);
        }
    }
}
