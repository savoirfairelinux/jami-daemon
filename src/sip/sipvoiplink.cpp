...
handleIncomingOptions(pjsip_rx_data* rdata)
{
    ...
    if (dlg) {
        JAMI_LOG("Processing in-dialog option request");
        ...
        JAMI_LOG("Processing out-of-dialog option request");
...
#define ADD_CAP(cap) ADD_HDR(pjsip_endpt_get_capability(endpt_, cap, NULL));
    ...
    if (dlg) {
        ...
        JAMI_LOG("Sent in-dialog response for option request");
        return PJ_TRUE;
    }
    ...
    JAMI_LOG("Sent out-of-dialog response for option request");
    return PJ_TRUE;
}
...
transaction_request_cb(pjsip_rx_data* rdata)
{
    ...
    if (method->id == PJSIP_INVITE_METHOD) {
        ...
        JAMI_LOG("Received a SIP INVITE request");
    ...
    if (not ns.empty()) {
        ...
        for (unsigned i = 0, n = ns.size(); i < n; i++) {
            ...
            if (auto ret
                ...
                JAMI_WARNING("Error printing SIP nameserver: {}", gai_strerrorA(ret));
            } else {
