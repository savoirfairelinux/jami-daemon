...
#undef TRY
    ...
    sipExecQueue_ = std::make_shared<AsyncExecutionQueue>();
    sipScheduler_ = std::make_shared<ScheduledExecutor>();
    sipExecQueue_->initialize(sipScheduler_);
    ...
    JAMI_WARN("Start polling SIP events");
    pollTask_ = sipExecQueue_->getScheduler()->scheduleAtFixedRate(
        [this] {
            handleEvents({0, 0});
            ...
            try {
                if (s != PJ_SUCCESS || !r) {
                    ...
                    dht::ThreadPool::io().run([=, cb = std::move(cb)]() {
                        ...
                        sipExecQueue_->runOnExecQueue(std::bind(cb, std::move(ips)));
                    });
                } else {
