...
public:
     ...
     * offer/answer negotiation internally (initial outgoing/incoming calls).
     * This is a no-op if the span was already closed by startNegotiation().
     * Must be called after setActiveLocalSdpSession() so the media count is
     * available.
     */
