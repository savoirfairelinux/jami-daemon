...
constexpr const char * const SIPAccountDB::ACCOUNT_TYPE;
...
registration_cb(pjsip_regc_cbparam *param)
{
    ...
    auto account = static_cast<SIPAccountDB *>(param->token);
    if (!account) {
...
SIPAccountDB::SIPAccountDB(const std::string& accountID)
...
void
SIPAccountDB::onTransportStateChanged(pjsip_transport_state state, const pjsip_transport_state_info *info)
{
...
void
SIPAccountDB::setTransport(const std::shared_ptr<SipTransport>& t)
{
    if (t == transport_)
        return;
    if (transport_) {
    ...
    if (transport_)
        transport_->addStateListener(reinterpret_cast<uintptr_t>(this), std::bind(&SIPAccountDB::onTransportStateChanged, this, std::placeholders::_1, std::placeholders::_2));
}

pjsip_tpselector
SIPAccountDB::getTransportSelector() {
    if (!transport_)
        return SIPVoIPLink::getTransportSelector(nullptr);
    return SIPVoIPLink::getTransportSelector(transport_->get());
}

void SIPAccountDB::usePublishedAddressPortInVIA()
{
...
void SIPAccountDB::useUPnPAddressPortInVIA()
{
...
bool SIPAccountDB::mapPortUPnP()
{
    ...
    std::weak_ptr<SIPAccountDB> w = std::static_pointer_cast<SIPAccountDB>(shared_from_this());
...
void SIPAccountDB::doRegister()
{
    ...
    if ( upnpEnabled_ ) {
        ...
        std::thread{ [shared] {
            ...
            auto this_ = std::static_pointer_cast<SIPAccountDB>(shared).get();
...
void SIPAccountDB::doRegisterUpnpDisabled()
{
    if (isIP2IP()) {
        doRegisterIPToIP();
        return;
...
void SIPAccountDB::doRegisterIPToIP()
{
    bool ipv6 = false;
    if (isIP2IP()) {
...
void SIPAccountDB::doUnregister(std::function<void(bool)> released_cb)
{
    tlsListener_.reset();
    if (isIP2IP()) {
        if (released_cb)
...
void
SIPAccountDB::connectivityChanged()
{
    if (not isUsable()) {
        // nothing to do
        return;
    ...
    auto shared = std::static_pointer_cast<SIPAccountDB>(shared_from_this());
...
void SIPAccountDB::startKeepAliveTimer()
{
    if (isTlsEnabled())
        return;
    ...
    keepAliveTimer_.cb = &SIPAccountDB::keepAliveRegistrationCb;
...
void SIPAccountDB::stopKeepAliveTimer()
{
    if (keepAliveTimerActive_) {
...
void
SIPAccountDB::onRegister(pjsip_regc_cbparam *param)
{
...
pj_uint32_t
SIPAccountDB::tlsProtocolFromString(const std::string& method)
{
    if (method == "Default")
...
void SIPAccountDB::initTlsConfiguration()
{
...
void SIPAccountDB::initStunConfiguration()
{
    size_t pos;
...
bool SIPAccountDB::fullMatch(const std::string& username, const std::string& hostname) const
{
    return userMatch(username) and hostnameMatch(hostname);
}

bool SIPAccountDB::userMatch(const std::string& username) const
{
    return !username.empty() and username == username_;
}

bool SIPAccountDB::hostnameMatch(const std::string& hostname) const
{
...
bool SIPAccountDB::proxyMatch(const std::string& hostname) const
{
...
std::string SIPAccountDB::getLoginName()
{
#ifndef _WIN32
...
std::string SIPAccountDB::getFromUri() const
{
    std::string scheme;
...
std::string SIPAccountDB::getToUri(const std::string& username) const
{
    std::string scheme;
    std::string transport;
    std::string hostname;
...
std::string SIPAccountDB::getServerUri() const
{
    std::string scheme;
    std::string transport;
...
pj_str_t
SIPAccountDB::getContactHeader(pjsip_transport* t)
{
...
pjsip_host_port
SIPAccountDB::getHostPortFromSTUN(pj_pool_t *pool)
{
    std::string addr;
    pj_uint16_t port;
...
SIPAccountDB::getSupportedTlsCiphers()
{
...
const std::vector<std::string>&
SIPAccountDB::getSupportedTlsProtocols()
{
    static std::vector<std::string> availProtos {VALID_TLS_PROTOS, VALID_TLS_PROTOS + arraySize(VALID_TLS_PROTOS)};
    return availProtos;
}

void SIPAccountDB::keepAliveRegistrationCb(UNUSED pj_timer_heap_t *th, pj_timer_entry *te)
{
    SIPAccountDB *sipAccount = static_cast<SIPAccountDB *>(te->user_data);
...
void
SIPAccountDB::Credentials::computePasswordHash()
{
...
void SIPAccountDB::setCredentials(const std::vector<std::map<std::string, std::string> >& creds)
{
    if (creds.empty()) {
...
SIPAccountDB::getCredentials() const
{
...
void
SIPAccountDB::setRegistrationState(RegistrationState state, unsigned details_code,
...
std::string SIPAccountDB::getUserAgentName() const
{
...
SIPAccountDB::getTlsSettings() const
{
    return {
...
bool SIPAccountDB::isIP2IP() const
{
    return hostname_.empty();
}

MatchRank
SIPAccountDB::matches(const std::string &userName, const std::string &server) const
{
...
void
SIPAccountDB::destroyRegistrationInfo()
{
    if (!regc_) return;
...
void
SIPAccountDB::resetAutoRegistration()
{
...
bool
SIPAccountDB::checkNATAddress(pjsip_regc_cbparam *param, pj_pool_t *pool)
{
 ...
 */
void
SIPAccountDB::scheduleReregistration()
{
    if (!isUsable())
        return;
    ...
    auto_rereg_.timer.cb = [](pj_timer_heap_t* /*th*/, pj_timer_entry* te) {
        static_cast<SIPAccountDB*>(te->user_data)->autoReregTimerCb();
    };
