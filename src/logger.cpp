...
{
public:
    static Handler* instance(bool create = false)
    {
        static ConsoleLog* self = nullptr;

        if (create and not self) {
...
{
    ConsoleLog::instance(true)->enable(en);
}
...
{
    SysLog::instance(true)->enable(en);
}
...
public:
    static Handler* instance(bool create = false)
    {
        static MonitorLog* self = nullptr;
...
{
    MonitorLog::instance(true)->enable(en);
}
        ...
        thread_ = std::thread([this] {
            while (isEnable()) {
...

                {
                    ...
                    cv_.wait(lk, [&] { return isEnable() or not messagesQ_.empty(); });
...
{
    dynamic_cast<FileLog*>(FileLog::instance(true))->setFile(path);
}
...
{
    auto if_enable = [&](Handler* handler) {
        return handler and handler->isEnable();
    };

    if (not(if_enable(ConsoleLog::instance()) or if_enable(SysLog::instance())
            or if_enable(MonitorLog::instance()) or if_enable(FileLog::instance()))) {
        return;
    }
    ...
    auto log_to_if_enable = [&](Handler* handler) {
        if (handler and handler->isEnable()) {
            handler->consume(msg);
        }
    };
...
Logger::fini()
{
    ...
    auto logger = FileLog::instance();

    if (logger) {
        dynamic_cast<FileLog*>(logger)->setFile(nullptr);
    }
}
