...
std::string
formatHeader(const char* const file, int line, int level, std::string_view msg, bool with_color)
{
#ifdef __linux__
    ...
    if (with_color && (level == LOG_WARNING || level == LOG_ERR)) {
        auto color = (level == LOG_WARNING) ? fmt::color::yellow : ( level == LOG_ERR ? fmt::color::red : fmt::color::white );
        auto header = file
            ? fmt::format(fmt::emphasis::bold | fmt::fg(color), FMT_STRING("[{: >3d}.{:0<3d}|{: >4}|{: <24s}:{: <4d}] "), secs, milli, tid, stripDirName(file), line)
            : fmt::format(fmt::emphasis::bold | fmt::fg(color), FMT_STRING("[{: >3d}.{:0<3d}|{: >4}] "), secs, milli, tid);
        return header + msg;
    } else {
        if (file) {
            return fmt::format(FMT_COMPILE("[{: >3d}.{:0<3d}|{: >4}|{: <24s}:{: <4d}] {:s}"), secs, milli, tid, stripDirName(file), line, msg);
        } else {
            return fmt::format(FMT_COMPILE("[{: >3d}.{:0<3d}|{: >4}] {:s}"), secs, milli, tid, msg);
        }
    }
}
...
struct Logger::Msg
{
    ...
    {
        return formatHeader(file_, line_, level_, payload_, with_color);
    }
...
public:
    ...
    void printLogImpl(jami::Logger::Msg& msg, bool with_color)
    {
        ...
        if (with_color) {
            ...
            fwrite(msg.header_.data(), msg.header_.size(), 1, stderr);
        ...
        fwrite(msg.payload_.data(), msg.payload_.size(), 1, stderr);
    ...
    void consume(const Logger::Msg& msg)
    {
...
public:
    ...
    void consume(const Logger::Msg& msg)
    {
        ...
        emitSignal<libjami::ConfigurationSignal::MessageSend>(tmp);
    }
};
...
public:
    ...
    void consume(Logger::Msg&& msg)
    {
        notify([&] { currentQ_.emplace_back(std::move(msg)); });
    }
...
void log_to_if_enabled(T& handler, M&& msg)
{
    if (handler.isEnable()) {
        handler.consume(std::forward<M>(msg));
    }
}

inline
void log_message(Logger::Msg&& msg) {
    log_to_if_enabled(ConsoleLog::instance(), msg);
    log_to_if_enabled(SysLog::instance(), msg);
    log_to_if_enabled(MonitorLog::instance(), msg);
    log_to_if_enabled(FileLog::instance(), std::move(msg));
}
...
Logger::vlog(int level, const char* file, int line, bool linefeed, const char* fmt, va_list ap)
{
    ...
    log_message(Msg(level, file, line, linefeed, fmt, ap));
}

void
Logger::write(int level, const char* file, int line, std::string&& message)
{
    log_message(Msg(level, file, line, true, std::move(message)));
}
