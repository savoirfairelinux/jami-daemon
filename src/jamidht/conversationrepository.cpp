...
public:
                                        ...
                                        bool includeLeft = false) const
    {
        ...
        for (const auto& member : members_) {
            ...
            if (not includeLeft and member.role == MemberRole::LEFT)
                continue;
            if (not filter.empty() and filter == member.uri)
                continue;
     ...
     * @param oldCert       Previous certificate. Ids must be the same
     * @note There is a few exception because JAMS certificates are bugguy right now
     */
    bool verifyCertificate(const std::string& certContent, const std::string& userUri, const std::string& oldCert = "") const {
        ...
        if (isDeviceCertificate) {
            ...
            // NOTE: Still one case due to bugguy certificate from JAMS
            // here we can verify.
                                                 ...
                                                 const std::string& parentId) const
{
    ...
    // If modified, it should only be certificates from device.
    // Retrieve userUri
    ...
    for (const auto& changedFile : changedFiles) {
        if (changedFile == adminsFile || changedFile == membersFile) {
            ...
            if (oldFile) {
                auto* blob = reinterpret_cast<git_blob*>(oldFile.get());
                auto oldCert = std::string(static_cast<const char*>(
                                                  ...
                                                  const std::string& commitId) const
{
    ...
    auto pin = not cert or not cert->issuer;
    auto repo = repository();
    if (not repo)
        return false;
    ...
    if (userUri.empty()) {
        if (pin) {
            ...
            // HACK: JAMS device's certificate do not contains any issuer
                                                 ...
                                                 const std::string& commitMsg) const
{
    ...
    git_oid oid;
    git_commit* commit = nullptr;
    if (git_oid_fromstr(&oid, commitId.c_str()) < 0 || git_commit_lookup(&commit, repo.get(), &oid) < 0) {
        JAMI_WARN("Failed to look up commit %s", commitId.c_str());
        return false;
    }
...
std::vector<std::string>
ConversationRepository::memberUris(std::string_view filter) const
{
    return pimpl_->memberUris(filter);
}
