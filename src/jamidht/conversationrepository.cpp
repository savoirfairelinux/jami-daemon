...
public:
    ...
    std::optional<std::set<const git_diff_delta*, GitDiffDeltaComparator>> getDeltasFromDiff(const GitDiff& diff) const;
 ...
 */
std::optional<std::set<const git_diff_delta*, GitDiffDeltaComparator>>
ConversationRepository::Impl::getDeltasFromDiff(const GitDiff& diff) const
{
    std::set<const git_diff_delta*, GitDiffDeltaComparator> deltas_set = {};
    for (size_t delta_idx = 0; delta_idx < git_diff_num_deltas(diff.get()); delta_idx++) {
                                                    ...
                                                    const std::vector<std::string>& parents) const
{
    ...
    auto first_parent_deltas_set = getDeltasFromDiff(first_diff);
    // Get the deltas of the second parent's commit
    auto second_parent_deltas_set = getDeltasFromDiff(second_diff);
    ...
    std::set<const git_diff_delta*, GitDiffDeltaComparator> parent_deltas_intersection_set = {};
    ...
    if (parent_deltas_intersection_set.size() > 0) {
        return false;
    }
    return true;
}
