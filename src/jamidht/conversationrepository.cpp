...
ConversationRepository::validate() const
{
    ...
    // the full tree each of them points at - is present and readable in the
    // database. A truncated or missing pack can drop objects anywhere in history
    // (older commits, sub-trees or blobs) while leaving HEAD resolvable; such a
    // gap would only surface later, and resume the fetch/merge retry loop, when
    // that part of the conversation is walked. Each reachable object is checked
    // with git_odb_read_header, which reads the object's type and size - failing
    // if the pack entry is truncated or unreadable - without inflating the blob
    // payload into memory. Visited object ids are remembered (by raw value,
    ...
    auto checkEntry = [](const char*, const git_tree_entry* entry, void* payload) -> int {
        ...
        if (git_odb_read_header(&len, &type, ctx->odb, id) < 0)
            return -1; // missing or unreadable object: report corruption
        return 0;
    };
