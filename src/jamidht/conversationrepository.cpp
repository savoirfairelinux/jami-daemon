...
public:
    ...
    void checkLocks()
    {
        ...
        if (std::filesystem::exists(indexPath, ec)) {
            JAMI_WARNING("[Account {}] [Conversation {}] Conversation is locked, removing lock {}",
                         accountId_,
                         id_,
                         indexPath);
            std::filesystem::remove(indexPath, ec);
            if (ec)
                JAMI_ERROR("[Account {}] [Conversation {}] Unable to remove lock {}: {}",
                           accountId_,
                           id_,
                           indexPath,
                           ec.message());
        ...
        for (const auto& fileIt : std::filesystem::directory_iterator(remotePath, ec)) {
            ...
            if (std::filesystem::exists(refPath, ec)) {
                JAMI_WARNING("[Account {}] [Conversation {}] Conversation is locked for remote {}, removing lock",
                             accountId_,
                             id_,
                             fileIt.path().filename());
                std::filesystem::remove(refPath, ec);
                if (ec)
                    JAMI_ERROR("[Account {}] [Conversation {}] Unable to remove lock {}: {}",
                               accountId_,
        ...
        if (err < 0) {
            JAMI_ERROR("[Account {}] [Conversation {}] Unable to clean up the repository: {}",
                       accountId_,
                       id_,
    ...
    // NOTE! We use temporary GitRepository to avoid keeping the file opened
    // TODO: check why git_remote_fetch() leaves pack-data opened
    GitRepository repository() const
    {
        auto path = fmt::format("{}/{}/conversations/{}", fileutils::get_data_dir().string(), accountId_, id_);
...
initial_commit(GitRepository& repo, const std::shared_ptr<JamiAccount>& account, const CommitMessage& message)
{
    ...
    if (git_commit_create_with_signature(&commit_id, repo.get(), to_sign.ptr, signed_str.c_str(), "signature") < 0) {
        ...
        JAMI_ERROR("Unable to sign the initial commit");
        return {};
    }
    git_buf_dispose(&to_sign);
...
ConversationRepository::Impl::createMergeCommit(git_index* index, const std::string& wanted_ref)
{
...
#endif
    if (git_commit_create_buffer(
            &to_sign, repo.get(), sig.get(), sig.get(), nullptr, commitMsg.c_str(), tree.get(), 2, &parents_ptr[0])
        < 0) {
        const git_error* err = giterr_last();
        if (err)
            JAMI_ERROR("[Account {}] [Conversation {}] Unable to create commit buffer: {}",
                       accountId_,
                       id_,
                       err->message);
        return {};
    ...
    git_oid commit_oid;
    if (git_commit_create_with_signature(&commit_oid, repo.get(), to_sign.ptr, signed_str.c_str(), "signature") < 0) {
        git_buf_dispose(&to_sign);
    ...
    if (commit_str) {
        ...
        if (git_reference_create(&ref_ptr, repo.get(), "refs/heads/main", &commit_oid, true, nullptr) < 0) {
            const git_error* err = giterr_last();
            if (err) {
                JAMI_ERROR("[Account {}] [Conversation {}] Unable to move commit to main: {}",
                           accountId_,
                           id_,
                           err->message);
                emitSignal<libjami::ConversationSignal::OnConversationError>(accountId_, id_, ECOMMIT, err->message);
            }
            return {};
        }
    ...
    // We're done merging. Clean up the repository state and index
    ...
    if (git_object_lookup(&target_ptr, repo.get(), &commit_oid, GIT_OBJ_COMMIT) != 0) {
        const git_error* err = giterr_last();
        if (err)
            JAMI_ERROR("[Account {}] [Conversation {}] failed to look up OID {}: {}",
                       accountId_,
                       id_,
...
ConversationRepository::Impl::mergeFastforward(const git_oid* target_oid, int is_unborn)
{
    ...
    if (is_unborn) {
        ...
        if (git_reference_create(&target_ref_ptr, repo.get(), symbolic_ref, target_oid, 0, nullptr) < 0) {
            const git_error* err = giterr_last();
            if (err)
                JAMI_ERROR("[Account {}] [Conversation {}] failed to create main reference: {}",
                           accountId_,
                           id_,
                           err->message);
            return false;
        ...
        // HEAD exists, just look up and resolve
    ...
    if (git_checkout_tree(repo.get(), target.get(), &ff_checkout_options) != 0) {
        if (auto err = git_error_last())
            ...
            JAMI_ERROR("[Account {}] [Conversation {}] failed to checkout HEAD reference: unknown error",
                       accountId_,
                       id_);
        return false;
    }
                                                   ...
                                                   const std::string& parentId) const
{
    ...
    for (const auto& changedFile : changedFiles) {
        ...
        if (changedFile == userDeviceFile) {
            auto newFile = fileAtTree(changedFile, treeNew);
            if (!newFile) {
                JAMI_ERROR("Device certificate removed in checkpoint commit {}: {}", commitId, changedFile);
                return false;
            }
            auto oldFile = fileAtTree(changedFile, treeOld);
            std::string_view oldCert;
                                            ...
                                            const std::string& parentId) const
{
    ...
    std::string bannedFile = fmt::format("{}/{}/{}.crt",
                                         MemberPath::BANNED.string(),
                                         MemberPath::MEMBERS.string(),
                                         uriMember);
                                                       ...
                                                       const std::string& voteType) const
{
    ...
    const std::regex regex_votes("votes." + voteType + ".(members|devices|admins|invited).(\\w+).(\\w+)");
    static const std::regex regex_devices("devices.(\\w+)\\.crt");
                                         ...
                                         const std::string& at) const
{
    ...
    if (owners.empty() || author.empty())
        return false;
    const auto& owner = owners.front();
...
ConversationRepository::Impl::feedPolicy(const std::string& at) const
{
    ...
    if (bytes.size() > 512 * 1024)
        return {};
    return FeedPolicy::fromInfos(ConversationRepository::infosFromVCard(vCard::utils::toMap(bytes)));
}
                                                      ...
                                                      const std::string& parentId) const
{
    ...
    if (mode() == ConversationMode::FEED) {
        ...
        if (owners.empty() || userUri != owners.front() || !before || !after || (before->closed && !after->closed))
            return false;
    }
                                                    ...
                                                    const std::vector<std::string>& parents) const
{
    ...
    if (static_cast<int>(parents.size()) != 2)
        return false;
    if (mode() == ConversationMode::FEED) {
        const auto merged = feedPolicy(mergeId);
        if (!merged)
            return false;
        for (const auto& parent : parents) {
            const auto policy = feedPolicy(parent);
            if (!policy || (policy->closed && !merged->closed)) {
                JAMI_WARNING("[Feed {}] Merge would reopen a closed Feed", id_);
                return false;
            }
        }
    }
...
ConversationRepository::Impl::validateDevice()
{
    ...
    if (wrongDeviceFile) {
        JAMI_WARNING(
            "[Account {}] [Conversation {}] Device certificate is no longer valid. Attempting to update certificate.",
            accountId_,
            id_);
        // Replace certificate with current cert
        auto cert = account->identity().second;
        if (!cert || !account->isValidAccountDevice(*cert)) {
            JAMI_ERROR("[Account {}] [Conversation {}] Current device's certificate is invalid. A migration is needed",
                       accountId_,
                       id_);
            return false;
        }
    ...
    if (wrongDeviceFile) {
        JAMI_WARNING(
            "[Account {}] [Conversation {}] Account certificate is no longer valid. Attempting to update certificate.",
            accountId_,
            id_);
        auto cert = account->identity().second;
...
ConversationRepository::Impl::commit(const std::string& msg, bool verifyDevice)
{
    ...
    git_oid tree_id;
    if (git_index_write_tree(&tree_id, index.get()) < 0) {
        JAMI_ERROR("[Account {}] [Conversation {}] commit failed: Unable to write initial tree from index",
                   accountId_,
                   id_);
        return {};
    }
...
#endif
    if (git_commit_create_buffer(
            &to_sign, repo.get(), sig.get(), sig.get(), nullptr, msg.c_str(), tree.get(), 1, &head_ref[0])
        < 0) {
...
ConversationRepository::Impl::mode() const
{
    ...
    int mode = initialCommit->commitMsg.mode;
    switch (mode) {
    case 0:
...
GitDiff
ConversationRepository::Impl::diff(git_repository* repo, const std::string& idNew, const std::string& idOld) const
{
    if (!repo) {
...
GitObject
ConversationRepository::Impl::memberCertificate(std::string_view memberUri, const GitTree& tree) const
{
...
ConversationRepository::Impl::resolveConflicts(git_index* index, const std::string& other_id)
{
    ...
    while (git_index_conflict_next(&ancestor_out, &our_out, &their_out, ci.get()) != GIT_ITEROVER) {
        if (ancestor_out && ancestor_out->path && our_out && our_out->path && their_out && their_out->path) {
            if (std::string_view(ancestor_out->path) == "profile.vcf"sv) {
                // Checkout the wanted version. Copy the index_entry.
    ...
    if (mode() == ConversationMode::ONE_TO_ONE) {
        for (const auto& member : getInitialMembers()) {
            if (std::find(uris.begin(), uris.end(), member) == uris.end()) {
                // If member is in the initial commit, but not in invited, this means that user left.
...
ConversationRepository::Impl::convCommitToMap(const ConversationCommit& commit) const
{
    ...
    if (type.empty()) {
        Json::Value cm = commit.commitMsg.toJson();
        for (auto const& id : cm.getMemberNames()) {
            if (id == CommitKey::TYPE) {
                type = cm[id].asString();
                continue;
            }
    ...
    if (type.empty()) {
        return std::nullopt;
    } else if (type == CommitType::DATA_TRANSFER) {
        // Avoid the client to do the concatenation
        auto tid = message[CommitKey::TID];
        if (not tid.empty()) {
            message["fileId"] = getFileId(commit.id, tid, message[CommitKey::DISPLAY_NAME]);
        } else {
            message["fileId"] = "";
        }
    }
...
ConversationRepository::Impl::parseCommit(git_repository* repo, const git_commit* commit) const
{
    ...
    const char* commitMsgStr = git_commit_message(commit);
    auto commitMsg = CommitMessage::fromString(commitMsgStr);
    if (commitMsg) {
        convCommit.commitMsg = *commitMsg;
    } else {
        JAMI_WARNING("[Account {}] [Conversation {}] Unable to parse commit message for commit {}: '{}'",
                     accountId_,
                     id_,
                     convCommit.id,
                     commitMsgStr);
...
std::unique_ptr<ConversationRepository>
ConversationRepository::createRepository(const std::shared_ptr<JamiAccount>& account,
                                         ConversationMode mode,
                                         const std::string& otherMember,
                                         const CommitMessage& initialMessage)
{
    ...
    if (!dhtnet::fileutils::recursive_mkdir(tmpPath, 0700)) {
        JAMI_ERROR("An error occurred when creating {}. Abort create conversations.", tmpPath);
        return {};
    }
    ...
    if (!add_initial_files(repo, account, mode, otherMember)) {
        JAMI_ERROR("An error occurred while adding the initial files.");
...
ConversationRepository::Impl::validCommits(const std::vector<ConversationCommit>& commitsToValidate) const
{
    ...
    for (const auto& commit : commitsToValidate) {
        ...
        if (commit.parents.size() == 0) {
            if (!checkInitialCommit(userDevice, commit.id, commit.commitMsg)) {
            ...
            if (type == CommitType::VOTE) {
            ...
            } else if (type == CommitType::MEMBER) {
                std::string action = commit.commitMsg.action;
                std::string uriMember = commit.commitMsg.uri;
                ...
                } else if (action == CommitAction::JOIN) {
                ...
                } else if (action == CommitAction::REMOVE) {
                ...
                } else if (action == CommitAction::BAN || action == CommitAction::UNBAN) {
                    // Note device.size() == "member".size()
                    if (!checkValidVoteResolution(userDevice, uriMember, commit.id, commit.parents[0], action)) {
                        JAMI_WARNING("[Account {}] [Conversation {}] Malformed removes commit {}. "
                                     "Please ensure that you are using the latest "
                                     "version of Jami, or that one of your contacts is not "
                                     "performing any unwanted actions.",
            ...
            } else if (type == CommitType::UPDATE_PROFILE) {
...
ConversationRepository::ConversationRepository(const std::shared_ptr<JamiAccount>& account, const std::string& id)
                                     ...
                                     std::function<void(const std::string&)>&& disconnectFromPeerCb)
{
    ...
    for (auto& commit : result) {
        ...
        if (it != commit.end() && it->second == CommitType::MEMBER) {
            refreshMembers();

            if (commit[CommitKey::ACTION] == CommitAction::BAN)
                disconnectFromPeerCb(commit[CommitKey::URI]);
        }
    }
    return result;
}
...
ConversationRepository::merge(const std::string& merge_id, bool force)
{
    ...
    if (state != GIT_REPOSITORY_STATE_NONE) {
        ...
        int state = git_repository_state(repo.get());
        if (state != GIT_REPOSITORY_STATE_NONE) {
            JAMI_ERROR("[Account {}] [Conversation {}] Merge operation aborted: repository is in unexpected state {}",
                       pimpl_->accountId_,
                       pimpl_->id_,
                       state);
            return {false, ""};
    ...
    if (git_repository_set_head(repo.get(), "refs/heads/main") < 0) {
        JAMI_ERROR("[Account {}] [Conversation {}] Merge operation aborted: unable to checkout main branch",
                   pimpl_->accountId_,
                   pimpl_->id_);
        return {false, ""};
    }
    ...
    if (git_oid_fromstr(&commit_id, merge_id.c_str()) < 0) {
        JAMI_ERROR("[Account {}] [Conversation {}] Merge operation aborted: unable to look up commit {}",
                   pimpl_->accountId_,
                   pimpl_->id_,
                   merge_id);
        return {false, ""};
    ...
    if (analysis & GIT_MERGE_ANALYSIS_UP_TO_DATE) {
        ...
        if (analysis & GIT_MERGE_ANALYSIS_UNBORN)
            ...
            JAMI_LOG("[Account {}] [Conversation {}] Merge analysis result: Fast-forward",
                     pimpl_->accountId_,
                     pimpl_->id_);
        ...
        if (!pimpl_->mergeFastforward(target_oid, (analysis & GIT_MERGE_ANALYSIS_UNBORN))) {
            const git_error* err = giterr_last();
            if (err)
                JAMI_ERROR("[Account {}] [Conversation {}] Fast forward merge failed: {}",
                           pimpl_->accountId_,
                           pimpl_->id_,
                           err->message);
            return {false, ""};
    ...
    git_index* index_ptr = nullptr;
    if (git_merge_commits(&index_ptr, repo.get(), head_commit.get(), other_commit.get(), &merge_opts) < 0) {
        const git_error* err = giterr_last();
        if (err)
            JAMI_ERROR("[Account {}] [Conversation {}] Git merge failed: {}",
                       pimpl_->accountId_,
                       pimpl_->id_,
                       err->message);
        return {false, ""};
...
ConversationRepository::leave()
{
...

    {
        ...
        pimpl_->members_.erase(std::remove_if(pimpl_->members_.begin(),
                                              pimpl_->members_.end(),
                                              [&](auto& member) { return member.uri == pimpl_->userId_; }),
                               pimpl_->members_.end());
        pimpl_->saveMembers();
    }
...
ConversationRepository::addAttachment(const std::vector<uint8_t>& data)
{
    if (data.empty())
        return {};
    if (mode() != ConversationMode::DOCUMENT) {
        JAMI_ERROR("[Account {}] [Conversation {}] Refusing to attach to a non-document repository",
                   pimpl_->accountId_,
                   pimpl_->id_);
        return {};
    }
...
ConversationRepository::Impl::resolveBan(const std::string_view type, const std::string& uri)
{
    ...
    if (!dhtnet::fileutils::recursive_mkdir(destPath, 0700)) {
        JAMI_ERROR("An error occurred while creating the {} directory. Abort resolving vote.", destPath);
        return false;
    }
    ...
    if (ec) {
        JAMI_ERROR("An error occurred while moving the {} origin file path to the {} destination "
                   "file path. Abort resolving vote.",
                   originFilePath,
                   destFilePath);
        return false;
    }
...
ConversationRepository::Impl::resolveUnban(const std::string_view type, const std::string& uri)
{
    ...
    auto destFilePath = destPath / crtStr;
    if (!dhtnet::fileutils::recursive_mkdir(destPath, 0700)) {
        JAMI_ERROR("An error occurred while creating the {} destination path. Abort resolving vote.", destPath);
        return false;
    }
    std::error_code ec;
    std::filesystem::rename(originFilePath, destFilePath, ec);
...
std::string
ConversationRepository::resolveVote(const std::string& uri, const std::string_view type, const std::string& voteType)
{
    std::lock_guard lkOp(pimpl_->opMtx_);
    pimpl_->resetHard();
    ...
    if (nbAdmins > 0 && (static_cast<double>(nbVotes) / static_cast<double>(nbAdmins)) > .5) {
        JAMI_WARNING("More than half of the admins voted to ban {}, applying the ban.", uri);

        // Remove vote directory
        dhtnet::fileutils::removeAll(voteDirectory, true);

        if (voteType == CommitAction::BAN) {
            if (!pimpl_->resolveBan(type, uri))
                return {};
        } else if (voteType == CommitAction::UNBAN) {
...
{
    auto commits = log({});
    if (!pimpl_->validCommits(commits))
        return {{}, false};
    return {std::move(commits), true};
}
...
bool
ConversationRepository::validCommits(const std::vector<ConversationCommit>& commitsToValidate) const
{
...
std::set<std::string>
ConversationRepository::memberUris(std::string_view filter, const std::set<MemberRole>& filteredRoles) const
{
...
ConversationRepository::pinCertificates(bool blocking)
{
    ...
    std::string repoPath = git_repository_workdir(repo.get());
    std::vector<std::string> paths = {repoPath + MemberPath::ADMINS.string(),
                                      repoPath + MemberPath::MEMBERS.string(),
                                      repoPath + MemberPath::DEVICES.string()};
    ...
    if (auto repo = pimpl_->repository()) {
        try {
            ...
            if (std::filesystem::is_regular_file(profilePath, ec)) {
                ...
                result = ConversationRepository::infosFromVCard(
