 ...
 * @note Both PkId (current device ids) and InfoHash (legacy ids) are accepted.
 */
inline bool
isCertificateOfDevice(const dht::crypto::Certificate& cert, std::string_view deviceId)
{
    return cert.getLongId().toString() == deviceId || cert.getId().toString() == deviceId;
}
...
public:
                                          ...
                                          const GitTree& tree) const
    {
        ...
        if (uri.empty()) {
            // HACK: JAMS device's certificate may not contain any issuer UID. Use the
            // issuer found by the certificate store (verified when pinned), if any.
            auto acc = account_.lock();
            if (!acc)
                return {};
            if (auto cert = acc->certStore().getCertificate(std::string(deviceId)); cert && cert->issuer)
