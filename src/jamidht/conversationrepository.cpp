...
add_initial_files(GitRepository& repo, const std::shared_ptr<JamiAccount>& account)
{
    ...
    std::string crlsPath = repoPath + "CRLs" + "/" + deviceId;
...
ConversationRepository::join()
{
    ...
    std::string memberFile = membersPath + "/" + uri + ".crt";
    std::string adminsPath = repoPath + "admins" + "/" + uri + ".crt";
...
ConversationRepository::voteKick(const std::string& uri, bool isDevice)
{
    ...
    auto relativeVotePath = std::string("votes") + "/" + (isDevice ? "devices" : "members") + "/"
                            + uri;
...
ConversationRepository::resolveVote(const std::string& uri, bool isDevice)
{
    ...
    auto voteDirectory = repoPath + "/" + "votes" + "/" + (isDevice ? "devices" : "members") + "/"
                         + uri;
    ...
    if (nbAdmins > 0 && (static_cast<double>(nbVotes) / static_cast<double>(nbAdmins)) > .5) {
        ...
        originFilePath += "/" + uri + ".crt";
        auto destPath = bannedPath + "/" + (isDevice ? "devices" : "members");
        auto destFilePath = destPath + "/" + uri + ".crt";
