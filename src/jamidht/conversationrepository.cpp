                                                  ...
                                                  const std::string& commitId) const
{
    ...
    int sig_extract_res = git_commit_extract_signature(&sig, &sig_data, repo.get(), &oid, "signature");
    ...
    if (sig_extract_res != 0) {
        switch (sig_extract_res)
        {
            case GIT_ERROR_INVALID:
                JAMI_ERROR("Error, the provided commmit id ({}) does not correspond to a commit!", commitId.c_str());
                break;
            case GIT_ERROR_OBJECT:
                JAMI_ERROR("Error, the provided commit id ({}) does not have a signature!", commitId.c_str());
                break;
            default:
                JAMI_ERROR("Unkown error occurred while extracting signature for commit ID {}", commitId.c_str());
                break;
    ...
    const std::vector<ConversationCommit>& commitsToValidate) const
{
    for (const auto& commit : commitsToValidate) {
        ...
        // device cert MUST be in /devices,
        // and commit MUST be signed by device
