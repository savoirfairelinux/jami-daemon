...
ConversationRepository::join()
{
    ...
    if (fileutils::isFile(membersPath) or fileutils::isFile(adminsPath)) {
    ...
    // Add members/uri.crt
    auto file = fileutils::ofstream(membersPath, std::ios::trunc | std::ios::binary);
    if (!file.is_open()) {
        JAMI_ERR("Could not write data to %s", membersPath.c_str());
        return {};
    }
    file << parentCert->toString(true);
    file.close();
