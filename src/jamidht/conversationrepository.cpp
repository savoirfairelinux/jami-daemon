...
public:
    ...
    bool isValidUserAtCommit(const std::string& userDevice, const std::string& commitId, const bool isRemoveCommit = false, const std::string& parentCommitId = "") const;
...
ConversationRepository::Impl::isValidUserAtCommit(const std::string& userDevice,
                                                  const std::string& commitId,
                                                  const bool isRemoveCommit,
                                                  const std::string& parentCommitId) const
{
    auto acc = account_.lock();
    if (!acc)
        return false;
    ...
    // If handling a remove commit, we get the tree of parent commit
    // so that we can retrieve the certificate pre-removal
    auto tree = treeAtCommit(repo.get(), isRemoveCommit ? parentCommitId : commitId);
    ...
    const std::vector<ConversationCommit>& commitsToValidate) const
{
    for (const auto& commit : commitsToValidate) {
        ...
        if (commit.parents.size() == 0) {
            ...
            if (!isValidUserAtCommit(userDevice, validUserAtCommit, isRemoveCommit, commit.parents[0])) {
