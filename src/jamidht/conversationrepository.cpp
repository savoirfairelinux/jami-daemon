...
void
ConversationRepository::Impl::forEachCommit(
    PreConditionCb&& preCondition,
    std::function<void(ConversationCommit&&)>&& emplaceCb,
    PostConditionCb&& postCondition,
    const std::string& from,
                                  ...
                                  const std::string& authorUri) const
{
    ...
    forEachCommit(
        ...
        [&](auto&& cc) {
            commits.emplace(commits.end(), std::move(cc));
        },
        [](auto, auto, auto) { return false; }, from, logIfNotFound);
    return commits;
}
...
ConversationRepository::Impl::search(const Filter& filter) const
{
    ...
    forEachCommit(
        [&](const auto& id, const auto& author, auto& commit) {
            ...
            if (filter.after && filter.after > commitTime) {
                ...
                if (git_commit_parentcount(commit.get()) <= 1)
                    ...
                    return CallbackResult::Skip; // Because we are sorting it with GIT_SORT_TOPOLOGICAL | GIT_SORT_TIME
            }
        ...
        [&](auto&& cc) {
            ...
            if (content && content->at("type") != "text/plain") {
                // Not searchable, at least for now
                return;
            }
            auto body = content->at("body");
