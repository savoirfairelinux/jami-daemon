...
public:
    ...
    std::string uriFromDevice(const std::string& deviceId) const
    {
        ...
        if (cert->getIssuerUID().empty())
            return {};
        deviceToUri_.insert({deviceId, cert->getIssuerUID()});
        return cert->getIssuerUID();
    }
...
ConversationRepository::Impl::validateDevice()
{
    ...
    if (!account->isValidAccountDevice(deviceCert)) {
        JAMI_WARN("Device's certificate is not valid anymore. Trying to replace certificate with current one.");
