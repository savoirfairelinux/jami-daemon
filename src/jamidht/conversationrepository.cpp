...
public:
    ...
    std::optional<ConversationCommit> getCommit2(const std::string& commitId) const
    {
        ...
        convCommit.id = commitId;
        convCommit.commit_msg = git_commit_message(commit.get());
        convCommit.timestamp = git_commit_time(commit.get());
...
ConversationRepository::Impl::commit(const std::string& msg, bool verifyDevice)
{
    ...
    fmt::print("  ### old commits ###\n");
    {
        git_oid oid_head;
        git_reference_name_to_id(&oid_head, repo.get(), "HEAD");
        auto messages = debugLog(git_oid_tostr_s(&oid_head));
    ...
    auto commit_str = git_oid_tostr_s(&commit_id);
    std::string commitId = commit_str ? std::string(commit_str) : "";
    if (commit_str) {
        fmt::print("  ### new commits ###\n");
        {
            auto messages = debugLog(commitId);
            for (size_t i = 1; i < messages.size(); i++) {
                fmt::print("    {}\n", messages[i - 1]);
            }
        }
                                     ...
                                     const std::vector<ConversationCommit>& messagesFromFetch) const
{
    ...
    for (auto& commit : commitsToAnnounce) {
        fmt::print("  >>> behind: {} -> {}\n", commit.id, commit.linearized_parent);
    }
    fmt::print("===\n");
...
std::string
ConversationRepository::localHead() const
{
    auto repo = pimpl_->repository();
    if (!repo)
        return {};
    ...
    auto commit_str = git_oid_tostr_s(&oid);
    if (!commit_str)
        return {};
    return commit_str;
}
