                                        ...
                                        const std::string& parentId) const
{
    ...
    std::string voteContent = static_cast<const char*>(
        git_blob_rawcontent(reinterpret_cast<git_blob*>(vote.get())));
                                               ...
                                               const std::string& parentId) const
{
    ...
    for (const auto& deviceF : devicesRemoved) {
        std::filesystem::path p(deviceF);
        std::string deviceUri = p.filename();
    ...
    adminFile = std::string("admins") + DIR_SEPARATOR_STR + uriMember + ".crt";
    if (!fileAtTree(adminFile, treeOld)) {
