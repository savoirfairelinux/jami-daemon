...
public:
    ...
    std::optional<ConversationCommit> parseCommit(git_repository* repo, const git_commit* commit) const;
...
ConversationRepository::Impl::checkEdit(const std::string& userDevice, const ConversationCommit& commit) const
{
    ...
    if (editedCommitMap->at("type") == CommitType::TEXT) {
        return true;
    }
    if (editedCommitMap->at("type") == CommitType::DATA_TRANSFER) {
                                            ...
                                            const std::string& to) const
{
    ...
    while (!git_revwalk_next(&oid, walker.get())) {
        ...
        if (!cc) {
            JAMI_WARNING("[Account {}] [Conversation {}] Failed to parse commit {}", accountId_, id_, id);
            break;
        }
...
ConversationRepository::Impl::parseCommit(git_repository* repo, const git_commit* commit) const
{
    ...
    auto commitMsg = CommitMessage::fromString(git_commit_message(commit));
    if (!commitMsg) {
...
ConversationRepository::Impl::validCommits(const std::vector<ConversationCommit>& commitsToValidate) const
{
    ...
    for (const auto& commit : commitsToValidate) {
        ...
        if (commit.parents.size() == 0) {
            ...
            std::string editId = commit.commitMsg.edit;
            if (type.empty()) {
                emitSignal<libjami::ConversationSignal::OnConversationError>(accountId_,
                                                                             id_,
                                                                             EVALIDFETCH,
            ...
            } else if (type == CommitType::EDITED_MESSAGE || !editId.empty()) {
                                     ...
                                     std::function<void(const std::string&)>&& disconnectFromPeerCb)
{
    ...
    for (auto& commit : result) {
        ...
        if (it != commit.end() && it->second == CommitType::MEMBER) {
            ...
            if (commit["action"] == CommitAction::BAN)
