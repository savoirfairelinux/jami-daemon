    ...
    for (const auto& commit : commitsToValidate) {
        ...
        if (commit.parents.size() == 0) {
            ...
            if (!isValidUserAtCommit(userDevice, validUserAtCommit)) {
                JAMI_WARNING(
                    "[Account {}] [Conversation {}] Malformed commit {}.Please ensure that you are using the latest "
                    "version of Jami, or that one of your contacts is not performing any unwanted actions. {}", accountId_, id_,
                    validUserAtCommit,
                    commit.commit_msg);
                emitSignal<libjami::ConversationSignal::OnConversationError>(
                    accountId_, id_, EVALIDFETCH, "Malformed commit");
            ...
            if (!isValidUserAtCommit(userDevice, validUserAtCommit)) {
                JAMI_WARNING(
                    "[Account {}] [Conversation {}] Malformed merge commit {}. Please ensure that you are using the latest "
                    "version of Jami, or that one of your contacts is not performing any unwanted actions.", accountId_, id_,
                    validUserAtCommit);
                emitSignal<libjami::ConversationSignal::OnConversationError>(
                    accountId_, id_, EVALIDFETCH, "Malformed commit");
                             ...
                             std::function<void(const std::vector<std::map<std::string, std::string>>&)> announce)
{
    ...
    if (!commits.empty()) {
        ...
        std::string baseToMerge = oldHead; // If fast-forward, the merge base is the previous head
        auto newHeadCommit = getCommit(newHead);
        if (newHeadCommit != std::nullopt && newHeadCommit->parents.size() > 1) {
            baseToMerge = mergeBase(newHeadCommit->parents[0], newHeadCommit->parents[1]);
            LogOptions options;
            options.to = baseToMerge;
