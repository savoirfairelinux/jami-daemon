...
{
    // auto name = AUTH_URI + deviceId.toString();
    JAMI_DEBUG("[AuthChannel {}] (legacy) name = {}", deviceId.toString(), name);
                            ...
                            ConnectCallbackLegacy&& cb)
{
    ...
    // TODO implement connectionmanager::isConnecting && connectionmanager::connectDevice for InfoHash
    // instead of DeviceId data type if (connectionManager_.isConnecting(infoHash, channelPath)) {
