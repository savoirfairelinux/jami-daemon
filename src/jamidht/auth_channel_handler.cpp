...
void
AuthChannelHandler::connect(const dht::InfoHash& infoHash, const std::string&, ConnectCallbackLegacy&& cb)
{
    const std::string channelName = AUTH_URI + infoHash.toString();
    // TODO check if this is needed & reimplement if necessary
    // it is probably DEPRECATED
    // if (connectionManager_.isConnecting(infoHash, channelName)) {
...
AuthChannelHandler::onReady(const std::shared_ptr<dht::crypto::Certificate>& cert,
    ...
    JAMI_DBG("[Auth Channel] Auth channel ready.");
    auto acc = account_.lock();
    if (!cert || !cert->issuer || !acc)
        return;
    if (!channel)
        JAMI_ERR("[Auth Channel] ChannelSocket invalid!");
