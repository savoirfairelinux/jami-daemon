                                                ...
                                                size_t length)
{
    ...
    try {
        ...
        std::string contextStr = devA + ":" + devB + ":" + std::to_string(operationId);
        std::vector<uint8_t> context(contextStr.begin(), contextStr.end());

        JAMI_DEBUG("[DeviceVerification] Exporting keying material with context: {}", contextStr);
