...
JamiAccountConfig::serialize(YAML::Emitter& out) const
{
    ...
    SERIALIZE_CONFIG(DRing::Account::ConfProperties::DHT_PEER_DISCOVERY, dhtPeerDiscovery);
    SERIALIZE_CONFIG(DRing::Account::ConfProperties::ACCOUNT_PEER_DISCOVERY, accountPeerDiscovery);
    SERIALIZE_CONFIG(DRing::Account::ConfProperties::ACCOUNT_PUBLISH, accountPublish);
    SERIALIZE_CONFIG(Conf::PROXY_ENABLED_KEY, proxyEnabled);
    SERIALIZE_CONFIG(Conf::PROXY_SERVER_KEY, proxyServer);
    SERIALIZE_CONFIG(DRing::Account::ConfProperties::DHT_PROXY_LIST_URL, proxyListUrl);
    SERIALIZE_CONFIG(DRing::Account::ConfProperties::RingNS::URI, nameServer);
    SERIALIZE_CONFIG(DRing::Account::VolatileProperties::REGISTERED_NAME, registeredName);
    SERIALIZE_CONFIG(DRing::Account::ConfProperties::ARCHIVE_PATH, archivePath);
    SERIALIZE_CONFIG(DRing::Account::ConfProperties::ARCHIVE_HAS_PASSWORD, archiveHasPassword);
    SERIALIZE_CONFIG(DRing::Account::ConfProperties::DEVICE_NAME, deviceName);
    SERIALIZE_CONFIG(DRing::Account::ConfProperties::MANAGER_URI, managerUri);
    SERIALIZE_CONFIG(DRing::Account::ConfProperties::MANAGER_USERNAME, managerUsername);
    ...
    SERIALIZE_CONFIG(Conf::CALIST_KEY, tlsCaListFile);
    SERIALIZE_CONFIG(Conf::CERTIFICATE_KEY, tlsCertificateFile);
    SERIALIZE_CONFIG(Conf::TLS_PASSWORD_KEY, tlsPassword);
    SERIALIZE_CONFIG(Conf::PRIVATE_KEY_KEY, tlsPrivateKeyFile);
    out << YAML::EndMap;
...
JamiAccountConfig::unserialize(const YAML::Node& node)
{
    ...
    parsePathOptional(node, DRing::Account::ConfProperties::ARCHIVE_PATH, archivePath, path);
