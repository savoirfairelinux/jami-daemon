...
JamiAccountConfig::serialize(YAML::Emitter& out) const
{
    ...
    out << YAML::Key << libjami::Account::ConfProperties::ARCHIVE_PATH << YAML::Value
        << fileutils::getCleanPath(path, archivePath);
    ...
    out << YAML::Key << Conf::CALIST_KEY << YAML::Value
        << fileutils::getCleanPath(path, tlsCaListFile);
    out << YAML::Key << Conf::CERTIFICATE_KEY << YAML::Value
        << fileutils::getCleanPath(path, tlsCertificateFile);
    ;
    out << YAML::Key << Conf::TLS_PASSWORD_KEY << YAML::Value << tlsPassword;
    out << YAML::Key << Conf::PRIVATE_KEY_KEY << YAML::Value
        << fileutils::getCleanPath(path, tlsPrivateKeyFile);
    ;
    out << YAML::EndMap;
