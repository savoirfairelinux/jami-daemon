...
ArchiveAccountManager::onArchiveLoaded(
    ...
    AccountArchive&& a)
{
    auto contactList = std::make_unique<ContactList>(a.id.second, path_, onChange_);
    ...
    JAMI_WARN("[Auth] created new device: %s", deviceId.toString().c_str());
    ...
    OnChangeCallback onChange)
{
    ...
    auto contactList = std::make_unique<ContactList>(accountCertificate, path_, onChange_);
    ...
    JAMI_DBG("[Auth] Auth device %s receipt checked successfully", id.c_str(), info_->deviceId.c_str());
    return info_.get();
}
...
ArchiveAccountManager::revokeDevice(const std::string& password, const std::string& device, RevokeDeviceCallback cb)
{
    ...
    getAccount().findCertificate(dht::InfoHash(device),
...
/*
bool
AccountManager::removeAccountDevice(const dht::InfoHash& device)
{
    return info_->contacts->removeAccountDevice(device);
}
*/

void
AccountManager::setAccountDeviceName(const dht::InfoHash& device, const std::string& name)
{
    info_->contacts->setAccountDeviceName(device, name);
}
...
AccountManager::addContact(const std::string& uri, bool confirmed)
{
    ...
    info_->contacts->addContact(h, confirmed);
}
...
AccountManager::removeContact(const std::string& uri, bool banned)
{
    ...
    info_->contacts->removeContact(h, banned);
}
...
void
AccountManager::sendTrustRequestConfirm(const std::string& to)
{
    /*dht::TrustRequest answer {DHT_TYPE_NS};
    answer.confirm = true;
    forEachDevice(to, [this,to,answer](const dht::InfoHash& dev) {
        JAMI_WARN("sending trust request reply: %s / %s", to.toString().c_str(), dev.toString().c_str());
...
{
    JAMI_DBG("Building device sync from %s"/*, ringDeviceName_.c_str() */, info_->deviceId.c_str());
...
ArchiveAccountManager::startSync()
{
    ...
    dht_->listen<dht::TrustRequest>(
        ...
        [this](dht::TrustRequest&& v) {
            ...
            findCertificate(v.from, [this, v](const std::shared_ptr<dht::crypto::Certificate>& cert) mutable {
                ...
                if (info_)
                    info_->contacts->onTrustRequest(peer_account, v.from, time(nullptr), v.confirm, std::move(v.payload));
                    //onChange_.trustRequest(peer_account, v.from, time(nullptr), v.confirm, std::move(v.payload));
    ...
    //auto syncDeviceKey = dht::InfoHash::get("inbox:"+info_->deviceId);
