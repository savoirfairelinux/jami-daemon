 ...
 *
 * startSync() and ArchiveAccountManager::revokeDevice() publish from different threads, so the
 * whole decision runs under a lock and is ordered by update time. Serializing only the bookkeeping
 * would not be enough: an unserialized cancel/put pair lets a slower call from an in-flight
 * registration cancel the list a concurrent revocation has just published, leaving the account
 * announcing the pre-revocation list. Ordering by update time makes the result independent of which
 * thread wins the race. Holding the lock across the DHT calls is cheap, as put() and cancelPut()
 * only queue an operation on the DhtRunner.
 */
void
                                  ...
                                  const std::shared_ptr<dht::crypto::RevocationList>& crl)
{
    ...
    if (publishedCrlId_ and updateTime < publishedCrlTime_)
        return; // superseded by a list already published
    if (publishedCrlId_ and publishedCrlId_ != id)
