...
AccountManager::foundPeerDevice(const std::shared_ptr<dht::crypto::Certificate>& crt, dht::InfoHash& account_id)
{
    ...
    if (top_issuer == crt) {
        JAMI_WARN("Foucb(cert, peer_account_id);nd invalid peer device: %s", crt->getId().toString().c_str());
        return false;
    }
...
AccountManager::onPeerMessage(const dht::InfoHash& peer_device, bool allowPublic, std::function<void(const std::shared_ptr<dht::crypto::Certificate>& crt, const dht::InfoHash& peer_account)>&& cb)
{
    ...
    findCertificate(peer_device,
        ...
        /*if (not AccountManager::foundPeerDevice(cert, peer_account_id)) {
            JAMI_WARN("[Auth] Discarding message from invalid peer certificate %s.", peer_device.toString().c_str());
            return;
        }

        if (not isAllowed(*cert, allowPublic)) {
            JAMI_WARN("[Auth] Discarding message from unauthorized peer %s.", peer_device.toString().c_str());
            return;
        }*/
    });
}
