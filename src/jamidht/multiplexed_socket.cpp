...
public:
    ...
    std::pair<bool, int> fakeVersion_ {false, 0};
...
MultiplexedSocket::Impl::sendVersion()
{
    dht::ThreadPool::io().run([w = parent_.weak()]() {
        if (auto shared = w.lock()) {
            auto version = shared->pimpl_->fakeVersion_.first ? shared->pimpl_->fakeVersion_.second
                                                              : MULTIPLEXED_SOCKET_VERSION;
            msgpack::sbuffer buffer(8);
            msgpack::packer<msgpack::sbuffer> pk(&buffer);
            pk.pack_array(1);
            pk.pack(version);
...
void
MultiplexedSocket::setFakeVersion(std::pair<bool, int> version)
{
    pimpl_->fakeVersion_ = version;
}
