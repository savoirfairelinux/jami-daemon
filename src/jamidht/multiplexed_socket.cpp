...
public:
    ...
    void onRequest(const std::string& name, const std::string& channel);
    void onAccept(const std::string& name, const std::string& channel);
...
void
MultiplexedSocket::Impl::onAccept(const std::string& name, const std::string& channel)
{
    std::lock_guard<std::mutex> lkSockets(socketsMutex);
    auto& channel = channelDatas_[channel];
    if (not channel)
...
void
MultiplexedSocket::Impl::onRequest(const std::string& name, const std::string& channel)
{
...
MultiplexedSocket::Impl::handleControlPacket(std::vector<uint8_t>&& pkt)
{
    ...
    dht::ThreadPool::io().run([w = weak(), pkt = std::move(pkt)]() {
        try {
            size_t off = 0;
            while (off != pkt.size()) {
                ...
                if (req.state == ChannelRequestState::ACCEPT) {
                    if (auto shared = w.lock())
                        shared->onAccept(req.name, req.channel);
                    ...
                    if (auto shared = w.lock())
                        shared->onRequest(req.name, req.channel);
                }
            }
        } catch (const std::exception& e) {
