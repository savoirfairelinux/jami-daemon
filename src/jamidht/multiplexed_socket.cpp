...
MultiplexedSocket::Impl::eventLoop()
{
    ...
    while (!stop) {
        ...
        int size = endpoint->read(&buf[0], IO_BUFFER_SIZE, ec);
        if (size < 0) {
        ...
        do {
            ...
            // Get data
            if (static_cast<int>(pkt_len + RtpPkt_.size()) >= wantedSize_) {
    ...
    dht::ThreadPool::io().run([this, pkt = std::move(pkt)]() {
        ...
        while (pac.next(oh)) {
            try {
                ...
                if (req.state == ChannelRequestState::ACCEPT) {
                ...
                } else if (this->onRequest_) {
                    auto accept = this->onRequest_(this->deviceId, req.channel, req.name);
                    ...
                    std::error_code ec;
                    auto toSend = ss.str();
                    int wr = this->parent_.write(CONTROL_CHANNEL, reinterpret_cast<const uint8_t*>(&toSend[0]), toSend.size(), ec);
                    if (wr < 0) {
                        if (ec)
...
MultiplexedSocket::write(const uint16_t& channel, const uint8_t* buf, std::size_t len, std::error_code& ec)
{
    ...
    int res = pimpl_->endpoint->write(&toSend[0], len + 4, ec);
    if (res < 0) {
        if (ec)
...
public:
    ...
    : endpoint(std::move(endpoint)), name(name), channel(channel) {}
