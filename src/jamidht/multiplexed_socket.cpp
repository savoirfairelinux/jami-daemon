...
public:
    ...
    std::map<uint16_t, std::deque<uint8_t>> channelDatas_ {};
...
{
    if (channel > 0 && sockets[channel]) {
        channelDatas_[channel].insert(channelDatas_[channel].end(), pkt.begin(), pkt.end());
    } else {
...
MultiplexedSocket::read(const uint16_t& channel, uint8_t* buf, std::size_t len, std::error_code& ec)
{
    ...
    auto& chanBuf = pimpl_->channelDatas_[channel];
...
MultiplexedSocket::waitForData(const uint16_t& channel, std::chrono::milliseconds timeout, std::error_code& ec) const
{
    ...
    return pimpl_->channelDatas_[channel].size();
}
