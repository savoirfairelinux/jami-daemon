...
Typers::addTyper(const std::string &typer)
{
    ...
    if (!acc || !acc->isComposingEnabled())
        return;
    if (typers_.insert(typer).second) {
        auto& watcher = watcher_[typer];
        watcher = std::make_unique<asio::steady_timer>(*ioContext_);
        // Check next member
...
Typers::removeTyper(const std::string &typer)
{
    ...
    if (!acc || !acc->isComposingEnabled())
        return;
    if (typers_.erase(typer)) {
        watcher_.erase(typer);
        if (typer == selfUri_) {
            acc->sendInstantMessage(convId_,
