    ...
    if (needSocketCb_)
        needSocketCb_(
            ...
            [w = weak(), nodeId, noNewSocket](const std::shared_ptr<dhtnet::ChannelSocketInterface>& socket) {
                ...
                // Only a failed full connection attempt against a node that
                // is not already connected counts towards the backoff: with
                // noNewSocket the failure only says that no reusable
                // transport was available, and a stale failure callback can
                // arrive after a successful (e.g. incoming) channel was
                // added for the same node.
                std::chrono::steady_clock::time_point retryAt {};
                if (!noNewSocket && !shared->isConnectedWith(nodeId))
