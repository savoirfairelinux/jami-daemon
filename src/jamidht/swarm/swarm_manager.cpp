...
SwarmManager::scheduleReconnect(const NodeId& nodeId, bool countFailure)
{
    ...
    if (countFailure) {
        // Saturate the counter at the on-demand threshold so a peer that keeps
        // failing over a long-running daemon cannot overflow it and wrap back to
        // a small backoff. Past the threshold the exact count is irrelevant.
    ...
    auto retryAt = info.nextRetry;
    info.timer->async_wait([w = weak(), nodeId, retryAt](const asio::error_code& ec) {
        if (ec)
            return;
        auto shared = w.lock();
        if (!shared)
            return;
        {
            ...
            // Ignore a stale expiry: the socket may already have been cancelled
            // and its handler queued, or a newer scheduleReconnect()/markConnected()
            // may have superseded this generation, or the node may have
            // reconnected in the meantime.
            if (shared->isShutdown_)
                return;
            auto it = shared->reconnectInfos_.find(nodeId);
            if (it == shared->reconnectInfos_.end() || it->second.nextRetry != retryAt
                || shared->isConnectedWith(nodeId))
                return;
        }
