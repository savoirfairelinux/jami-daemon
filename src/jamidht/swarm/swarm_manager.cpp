...
SwarmManager::setKnownNodes(const std::vector<NodeId>& known_nodes)
{
    for (const auto& nodeId : known_nodes)
        if (myId != nodeId) {
            auto test = routing_table.findBucket(nodeId);
            if (!test->hasNodeId(nodeId)) {
...
SwarmManager::addKnownNodes(const NodeId& nodeId)
{
    ...
    if (myId != nodeId) {
        /*         JAMI_ERROR("######## MY ID {:s} // ID OF NODE ADDING {:s} #########",
         * this->getMyId().toString(), nodeId.toString());
         */
...
SwarmManager::sendAnswer(const std::shared_ptr<ChannelSocketInterface>& socket, const Message& msg_)
{
    ...
    if (msg_.request->q == Query::FIND) {
        /*      JAMI_ERROR("LOOKING FOR CLOSEST NODES FROM SM {:s} to {:s}",
                        this->getMyId().toString(),
        ...
        JAMI_ERROR("Nodes Sent: ");
        for (const auto& node : nodesToSend) {
            JAMI_ERROR("node: {:s}", node.toString());
        }
        msgpack::sbuffer buffer;
        msgpack::packer<msgpack::sbuffer> pk(&buffer);
        std::error_code ec;
...
{
    socket->setOnRecv([w = weak(), socket, this](const uint8_t* buf, size_t len) {
        auto shared = w.lock();
        ...
        try {
            ...
            if (msg.request) {
                JAMI_ERROR("Request received by {:s} from {:s}:",
                           shared->myId.toString(),
                           socket->deviceId().toString());
                JAMI_ERROR("NodeId: {:s}  Number: {:d}",
                           msg.request->nodeId.toString(),
                           msg.request->num);
                shared->sendAnswer(socket, msg);
            }
            if (msg.response) {
                JAMI_ERROR("Response received by {:s} from {:s}:",
                           shared->myId.toString(),
                           socket->deviceId().toString());
                this->setKnownNodes(msg.response->nodes);
            }
...
SwarmManager::tryConnect(const NodeId& nodeId)
{
    ...
    if (needSocketCb_)
        needSocketCb_(nodeId.toString(), [&](const std::shared_ptr<ChannelSocketInterface>& socket) {
            ...
            if (socket) {
                JAMI_ERROR("IN socket");

                auto bucket = routing_table.findBucket(nodeId);

                if (!bucket->hasNodeId(nodeId)) {
...
SwarmManager::addChannel(const std::shared_ptr<ChannelSocketInterface>& channel)
{
    ...
    bucket->removeKnownNode(channel->deviceId());
    bucket->removeConnectingNode(channel->deviceId());
