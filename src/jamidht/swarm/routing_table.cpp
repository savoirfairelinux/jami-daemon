...
{
    if (isFull() || hasNodeId(socket->deviceId())) {
        return 0;
    }
...
Bucket::getKnownNodesRandom(int numberNodes) const
{
    ...
    std::uniform_int_distribution<> distrib(0, getKnownNodesSize() - 1);
    int counter = 0;
    int a;
...
void
Bucket::printBucket(int number) const
{
...
RoutingTable::contains(std::list<Bucket>::iterator& bucket, const NodeId& nodeId)
{
    return Hash<32>::cmp(bucket->getLowerLimit(), nodeId) <= 0
           && (std::next(bucket) == buckets.end()
               || Hash<32>::cmp(nodeId, std::next(bucket)->getLowerLimit()) < 0);
}
...
RoutingTable::split(std::list<Bucket>::iterator& bucket)
{
    ...
    JAMI_ERROR("middle of bucket {:s} splitting", id.toString());
    Bucket newBucket(id);
    buckets.insert(std::next(bucket), std::move(newBucket));

    // Re-assign nodes
    auto sockets = bucket->getNodes();
    std::set<std::shared_ptr<ChannelSocketInterface>> nodeSwap(sockets.begin(), sockets.end());
...
{
    if (bucket->hasNodeId(socket->deviceId())) {
        return 0;
    }

    if (bucket->isFull()) {
        bool cont = contains(bucket, myId);
        JAMI_ERROR("CONT = {:b}", cont);

        if (contains(bucket, myId))
            split(bucket);
        else
            return false;
    }

    JAMI_ERROR("BUCKET ENCORE PLACE");
...
RoutingTable::closestNodes(const NodeId& nodeId, int count)
{
    ...
    /*     JAMI_DEBUG("Closest Nodes for {:s}", nodeId.toString());
        for (auto it = closestNodes.begin(); it != closestNodes.end(); ++it) {
            JAMI_DEBUG("Closest Node Id: {:s}", it->toString());
        } */
