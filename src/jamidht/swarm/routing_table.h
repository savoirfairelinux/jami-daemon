...
public:
    ...
    asio::steady_timer& getNodeTimer(const std::shared_ptr<ChannelSocketInterface>& socket)
    {
        ...
        if (node == nodes.end()) {
            throw std::range_error("Can't find timer");
        }
        return node->second;
    }
    ...
    {
        while (getNodesSize() != 0) {
            shutdownNode(nodes.begin()->first);
        }
    };

    void printBucket(int number) const;
...
public:
    ...
    void setMyId(const NodeId& node) { myId = node; };
