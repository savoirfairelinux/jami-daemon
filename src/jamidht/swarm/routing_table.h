...
public:
     ...
     */
    std::set<std::shared_ptr<ChannelSocket>> getNodeSockets() const
    {
    ...
    std::vector<NodeId> getKnownNodesVector() const
    {
        std::vector<NodeId> nodesToReturn;
        nodesToReturn.reserve(known_nodes.size());
        for (auto it = known_nodes.begin(); it != known_nodes.end(); ++it) {
            nodesToReturn.emplace_back(*it);
        }
    ...
    void resetNodeExpiry(const asio::error_code& ec, const std::shared_ptr<ChannelSocket>& socket)
    {
        ...
        // SENDFINDREQUEST
        node->second.expires_after(FIND_PERIOD);
        node->second.async_wait(
            std::bind(&Bucket::resetNodeExpiry, this, std::placeholders::_1, socket));
    }
