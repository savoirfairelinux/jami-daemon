...
private:
    static uint64_t versionOf(const Info& info, const char* key);
    static void advanceVersion(Info& info, const char* key);
    static void replaceDescriptor(Info& target, Info incoming);
    static void cancelIntent(Info& info);
    static Info descriptor(Info info, const std::string& id);
    bool mergeSyncedDescriptor(const std::string& id,
                               const Info& incoming,
                               std::vector<std::pair<std::string, bool>>& choices);
    std::optional<Info> describe(const std::string& id, Conversation& conversation, const std::string& accountUri) const;
    bool validSettings(const std::string& id, const Info& changes) const;
    void onSubscribeRequest(const std::string& from, const std::string& id);
    void onSubscriptionDenied(const std::string& from, const std::string& id);
    bool unsubscribe(const std::string& id, bool propagate);
    bool requestSubscription(const std::string& id, bool propagate);
    std::vector<std::string> applyCatalogue(const std::string& from, std::map<std::string, Info> received);
    std::optional<std::string> ownerOf(const std::string& id, const Conversation& conversation) const;
