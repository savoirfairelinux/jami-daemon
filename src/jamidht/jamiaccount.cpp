...
// bool
// JamiAccount::provideAccountAuthentication(const std::string& credentialsFromUser, const std::string_view scheme)
// {
//     return accountManager_ and accountManager_->provideAccountAuthentication2(credentialsFromUser, scheme);
// }
...
bool
JamiAccount::provideAccountAuthentication(const std::string& passwordFromUser)
{
    if (auto manager = dynamic_cast<ArchiveAccountManager*>(this->accountManager_.get())) {
    // if (auto manager = dynamic_cast<ArchiveAccountManager*>(accountManager_.get())) {
        return manager->provideAccountAuthentication2(passwordFromUser, fileutils::ARCHIVE_AUTH_SCHEME_PASSWORD);
    }
    return false;
}
...
// JamiAccount::addDevice(std::string_view scheme, const std::string& uriProvided)
        ...
        authHandler->second->connect(
            ...
            [this, token] (std::shared_ptr<dhtnet::ChannelSocket> socket, const dht::InfoHash& infoHash) {
                if (!socket) {
                    ...
                    // KESS pass scheme from here to simplify reading archive
                    config().archiveHasPassword;
                    bool needsPasswordScheme = config().archiveHasPassword;
                    accountManager_->addDevice(needsPasswordScheme ? fileutils::ARCHIVE_AUTH_SCHEME_PASSWORD : fileutils::ARCHIVE_AUTH_SCHEME_NONE, token, socket);
                }
            }
        );
        return token;
                         ...
                         const std::string& archive_path)
{
    ...
    try {
        ...
        if (auto info = accountManager_->useIdentity(getAccountID(),
            ...
            JAMI_DEBUG("[LinkDevice {:s}] loaded account identity CALLED", getAccountID());
            ...
            if (conf.managerUri.empty()) {
                ...
                if (!archive_path.empty()) {
                    ...
                    if (conf.archive_path == "jami-auth") {
...
JamiAccount::onTrackedBuddyOnline(const dht::InfoHash& contactId)
{
    ...
    if (state < PresenceState::AVAILABLE) {
        ...
        emitSignal<libjami::PresenceSignal::NewBuddyNotification>(getAccountID(), id, static_cast<int>(PresenceState::AVAILABLE), "");
    }
...
{
    // KESS: this is to find if the cert exists given the infohash and has a callback with the vaule
    if (accountManager_)
    ...
    if (deviceId.empty()) {
        ...
        accountManager_->forEachDevice(
            ...
            [this, to, devices, payload_type](
                ...
                auto deviceId = dev->getLongId();
                if (!devices->emplace(deviceId).second || deviceId.toString() == currentDeviceId()) {
                    return;
                }
            ...
            if (it == sipConns_.end()) {
                ...
                if (state == PresenceState::CONNECTED) {
                    ...
                    if (buddy == trackedBuddies_.end())
                        ...
                        state = buddy->second.devices_cnt > 0 ? PresenceState::AVAILABLE : PresenceState::DISCONNECTED;
                    emitSignal<libjami::PresenceSignal::NewBuddyNotification>(
                        getAccountID(),
                        peerId,
