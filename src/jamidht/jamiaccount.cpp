...
JamiAccount::sendTrustRequest(const std::string& to, const std::vector<uint8_t>& payload)
{
    ...
    if (not conversation.empty()) {
        ...
        JAMI_WARN("[Account %s] sendTrustRequest: account not loaded", getAccountID().c_str());
    // Update infos
    // TODO avoid duplicate with conversationrepository
    auto details = vCard::utils::toMap(std::string(payload.data(), payload.data() + payload.size()));
    ...
    if (!infos.empty()) {
        updateConversationInfos(conversation, infos, false);
    }
}
