...
void
JamiAccount::loadConfig(const std::string& convId)
{
...
void
JamiAccount::doRegister(const std::string& convId)
{
    std::lock_guard lock(configurationMutex_);
    if (not isUsable()) {
...
void
JamiAccount::doRegister_(const std::string& convId)
{
    ...
    try {
        ...
        convModule(false, convId)->clearPendingFetch();
