    ...
    const std::function<void(const std::string&)>& onChanneledCancelled)
{
    ...
    if (!convId.empty()) {
        Json::Value value;
        value["id"] = std::to_string(tid);
        value["to"] = info.peer;
        value["type"] = "application/data-transfe+json";
                                   ...
                                   bool sendRequest)
{
    ...
    if (message.at("type") == "member")
        announceMemberMessage(conversationId, message);
    emitSignal<DRing::ConversationSignal::MessageReceived>(getAccountID(), conversationId, message);
    if (sendRequest)
        ...
        if (conversation->second->removeMember(contactUri, isDevice)) {
            ...
            for (const auto& msg : messages) {
                ...
                if (msg.at("type") == "member")
                    announceMemberMessage(conversationId, msg);
