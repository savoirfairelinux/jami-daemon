...
JamiAccount::sendMessage(const std::string& conversationId, const std::string& message, const std::string& parent, const std::string& type)
{
    auto conversation = conversations_.find(conversationId);
    if (conversation != conversations_.end() && conversation->second) {
        auto messageId = conversation->second->sendMessage(message, type, parent);
        if (!messageId.empty()) {
            auto text = "{\"id\":\"" + conversationId + "\", \"commit\":\"" + messageId + "\"}";
...
void
JamiAccount::onNewGitCommit(const std::string& peer, const std::string& conversationId, const std::string& commitId)
{
    ...
    if (conversation != conversations_.end() && conversation->second) {
        ...
        if (!conversation->second->fetchFrom(peer)) {
            JAMI_WARN("Could not fetch new commit from %s for %s", peer.c_str(), conversationId.c_str());
        }
    } else {
