        ...
        if (sipConnIt != sipConnections_.end() && !sipConnIt->second.empty()) {
            for (auto dit = sipConnIt->second.rbegin(); dit != sipConnIt->second.rend(); ++dit) {
                for (auto it = dit->second.rbegin(); it != dit->second.rend(); ++it) {
                    ...
                    auto call = Manager::instance().callFactory.newCall<SIPCall, JamiAccount>(shared(), Manager::instance().getNewCallID(), Call::CallType::INCOMING);
                    if (!call) return {};
                             ...
                             const std::map<std::string, std::string>& volatileCallDetails)
{
    ...
    auto call = manager.callFactory.newCall<SIPCall, JamiAccount>(shared(), manager.getNewCallID(),
...
JamiAccount::startOutgoingCall(const std::shared_ptr<SIPCall>& call, const std::string& toUri)
{
    ...
    auto sendDhtRequest = [this, wCall, toUri, peer_account](const std::string& deviceId) {
        ...
        auto& manager = Manager::instance();
        auto dev_call = manager.callFactory.newCall<SIPCall, JamiAccount>(shared(), manager.getNewCallID(),
                                                                          Call::CallType::OUTGOING,
                                                                          call->getDetails());
    ...
    auto& manager = Manager::instance();
    auto dummyCall = manager.callFactory.newCall<SIPCall, JamiAccount>(shared(), manager.getNewCallID(),
                                                                            Call::CallType::OUTGOING,
                                                                            call->getDetails());
