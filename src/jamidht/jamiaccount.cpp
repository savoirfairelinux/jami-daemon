                                     ...
                                     bool sync)
{
    ...
    it->second->updateInfos(infos, [&](bool ok, const std::string& commitId) {
        if (ok) {
            if (sync)
                                   ...
                                   bool sendRequest)
{
    ...
    auto ok = it->second->addMember(contactUri, [&](bool ok, const std::string& commitId) {
        if (ok) {
            sendMessageNotification(*it->second, commitId, true);
            if (sendRequest) {
                auto invite = it->second->generateInvitation();
                                      ...
                                      bool isDevice)
{
    ...
    if (it != conversations_.end() && it->second) {
        auto lastCommit = it->second->lastCommitId();
        return it->second->removeMember(contactUri,
                                        isDevice,
                                        [&](bool ok, const std::string& commitId) {
                                            if (ok)
                                                sendMessageNotification(*it->second, commitId, true);
                                        });
    }
                         ...
                         bool announce)
{
    ...
    auto conversation = conversations_.find(conversationId);
    if (conversation != conversations_.end() && conversation->second) {
        conversation->second->sendMessage(value, parent, [&](bool ok, const std::string& commitId) {
            if (!announce)
                return;
            if (ok)
    ...
    if (conversation != conversations_.end() && conversation->second) {
        ...
        if (gitSocket(deviceId, conversationId)) {
            conversation->second->pull(
                deviceId,
                [deviceId, conversationId, w = weak()](bool ok, auto&& messages) {
                    auto shared = w.lock();
                    if (!shared)
                        return;
                    if (!ok) {
                    ...
                    dht::ThreadPool::io().run(
                        [w = weak(), conversationId, socket = std::move(socket), deviceId, commitId] {
                            ...
                            if (socket) {
                                ...
                                conversation->second->pull(
                                    deviceId.toString(),
                                    [deviceId, conversationId, w](bool ok, auto&& messages) {
                                        auto shared = w.lock();
                                        if (!shared)
                                            return;
                                        if (!ok) {
...
JamiAccount::saveConvRequests()
{
    ...
    msgpack::pack(file, info->conversationsRequests);
}
