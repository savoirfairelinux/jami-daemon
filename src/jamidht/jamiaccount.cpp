        ...
        connectionManager_->onChannelRequest(
            [this](const DeviceId& deviceId, const std::string& name) {
                ...
                if (name == "sip") {
                    ...
                    return cert->issuer->getId().toString() == getUsername();
            ...
            if (channel) {
                ...
                if (name == "sip") {
                    ...
                    if (convModule()->isBannedDevice(conversationId, remoteDevice)) {
                        JAMI_WARN("[Account %s] Git server requested (conv: %s), but the device is "
                                  "unauthorized (%s) ",
                                  getAccountID().c_str(),
            ...
            std::move([this](const auto& convId, const auto& deviceId, auto&& cb) {
                ...
                connectionManager_
                    ->connectDevice(DeviceId(deviceId),
                                    "git://" + deviceId + "/" + convId,
                                    [this, cb, convId](std::shared_ptr<ChannelSocket> socket,
                                                       const DeviceId&) {
                                        if (socket) {
                                            if (!cb(socket)) {
                                                socket->shutdown();
                                                removeGitSocket(socket->deviceId(), convId);
                                            }
                                            return;
                                        }
                                        cb({});
...
void
JamiAccount::sendProfiles(const std::string& deviceId)
{
    try {
                                ...
                                const DeviceId& deviceId)
{
    ...
    sendProfiles(deviceId.toString());
...
JamiAccount::initConnectionManager()
{
    if (!connectionManager_) {
        ...
        channelHandlers_[Uri::Scheme::DATA_TRANSFER]
            = std::make_unique<TransferChannelHandler>(std::move(weak()), *connectionManager_.get());
    }
}
