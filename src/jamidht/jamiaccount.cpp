...
JamiAccount::doRegister()
{
    ...
    for (const auto& repository : conversationsRepositories) {
        try {
            conversations_[repository] = std::move(std::make_unique<Conversation>(weak(), repository));
        } catch (const std::logic_error& e) {
            JAMI_WARN("[Account %s] Conversations not loaded : %s", getAccountID().c_str(), e.what());
        }
    }
