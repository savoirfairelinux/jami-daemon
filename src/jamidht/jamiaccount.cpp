        ...
        connectionManager_->onConnectionReady([this](const DeviceId& deviceId,
            ...
            if (channel) {
                ...
                if (name == "sip") {
                    ...
                    gs->setOnFetched([w = weak(), conversationId]() {
                        // TODO avoid code duplication
                        auto shared = w.lock();
                        if (!shared)
                            return;
                        auto itConv = shared->conversations_.find(conversationId);
                        if (itConv != shared->conversations_.end() && itConv->second->isRemoving()) {
                            auto it = shared->conversations_.find(conversationId);
                            if (it == shared->conversations_.end())
                                return;

                            it->second->erase();
                            shared->conversations_.erase(it);
                            // Update convInfos
                            for (auto& info : shared->convInfos_) {
...
JamiAccount::removeConversation(const std::string& conversationId)
{
    ...
    auto needCommit = !(members.size() == 1
    ...
    if (needCommit) {
        ...
        if (!commitId.empty()) {
            // TODO avoid code duplicate with sendMessage
            Json::Value message;
            ...
            for (const auto& members : it->second->getMembers()) {
