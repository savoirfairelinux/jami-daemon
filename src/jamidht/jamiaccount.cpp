...
ConversationRequest::ConversationRequest(const Json::Value& json)
{
    ...
    for (const auto& value : json["members"]) {
        members.emplace_back(value["uri"].asString());
    }
...
JamiAccount::startOutgoingCall(const std::shared_ptr<SIPCall>& call, const std::string& toUri)
{
                ...
                if (state != Call::ConnectionState::PROGRESSING
                    ...
                    if (auto shared = w.lock()) {
                        ...
                        if (it != shared->onConnectionClosed_.end()) {
                            if (it->second)
                                it->second(dht::InfoHash(deviceId), true);
    ...
    accountManager_->forEachDevice(
        peer_account,
        [this, toUri, devices = std::move(devices), call, sendRequest](const dht::InfoHash& dev) {
            ...
            if (channel) {
                ...
                if (name == "sip") {
                    ...
                    auto conversationId = name.substr(name.find_last_of("/") + 1);
                    {
                        ...
                        if (pendingConversationsClone_.find(conversationId)
                            != pendingConversationsClone_.end()) {
        ...
        dht_->listen<dht::ImMessage>(inboxDeviceKey, [this, inboxDeviceKey](dht::ImMessage&& v) {
            ...
            accountManager_->onPeerMessage(
                ...
                [this, v, inboxDeviceKey, msgId](const std::shared_ptr<dht::crypto::Certificate>&,
                    ...
                    auto itInvite = payloads.find("application/invite+json");
                    if (itInvite != payloads.end()) {
                        std::string err;
                        Json::Value value;
                        Json::CharReaderBuilder rbuilder;
                        Json::CharReaderBuilder::strictMode(&rbuilder.settings_);
                        auto reader = std::unique_ptr<Json::CharReader>(rbuilder.newCharReader());
                        if (!reader->parse(itInvite->second.c_str(),
                                           itInvite->second.c_str() + itInvite->second.size(),
                                           &value,
                                           &err)) {
                            JAMI_ERR() << "JSON parsing error: " << err;
                            return;
                        }
                        ...
                        emitSignal<DRing::ConversationSignal::ConversationRequestReceived>(
                            accountID_, convId, metadatas);
                        return;
                    }
                    auto itNeedsInvite = payloads.find("application/invite");
                    if (itNeedsInvite != payloads.end()) {
                        auto convId = itNeedsInvite->second;
                        ...
                        JAMI_INFO()
                            << peer_account << " asks for a new invite for conversation " << convId;
                        sendTextMessage(peer_account.toString(),
                                        conversation->second->generateInvitation());
                        return;
                    }
        ...
        if (conversation != conversations_.end() && conversation->second) {
            ...
            for (const auto& message : messages) {
                if (!message.at("type").empty())
                    JAMI_DBG("Message (%s) - type (%s): %s",
                             message.at("id").c_str(),
                             message.at("type").c_str(),
                             message.at("body").c_str());
                else
                                ...
                                const dht::InfoHash& deviceId)
{
    ...
    auto onShutdown = [w = weak(), peerId, key, socket]() {
        ...
        {
            std::lock_guard<std::mutex> lk(shared->onConnectionClosedMtx_);
            if (shared->onConnectionClosed_[key.second]) {
                cb = std::move(shared->onConnectionClosed_[key.second]);
                shared->onConnectionClosed_.erase(key.second);
            }
        }
        if (cb) {
    ...
    socket->setOnRecv([this, deviceId](const uint8_t* buf, size_t len) {
            ...
            for (const auto& jsonConv : value["conversations"]) {
                ...
                if (not removed) {
                    if (conversations_.find(convId) == conversations_.end()) {
                        {
                            std::lock_guard<std::mutex> lk(pendingConversationsCloneMtx_);
                            auto it = pendingConversationsClone_.find(convId);
                            if (it != pendingConversationsClone_.end()) // Already pending
                                return len;
                            pendingConversationsClone_[convId] = PendingConversationClone {};
                        }

                        connectionManager().connectDevice(
                            DeviceId(deviceId),
                            "git://" + deviceId + "/" + convId,
                            [this, deviceId, convId](std::shared_ptr<ChannelSocket> socket) {
                                if (socket) {
                                    std::unique_lock<std::mutex> lk(pendingConversationsCloneMtx_);
                                    auto& pending = pendingConversationsClone_[convId];
                                    if (!pending.ready) {
                                        pending.ready = true;
        ...
        if (value.isMember("conversationsRequests")) {
            for (const auto& jsonReq : value["conversationsRequests"]) {
                ...
                JAMI_INFO("[Account %s] New request detected for %s",
                          getAccountID().c_str(),
                          convId.c_str(),
                          deviceId.c_str());
...
void
JamiAccount::loadConvInfo()
{
    decltype(convInfos_) convInfo;
    try {
        // read file
...
void
JamiAccount::saveConvInfo() const
{
