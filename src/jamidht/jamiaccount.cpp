        ...
        connectionManager_->onConnectionReady([this](const std::string& deviceId, const std::string& name, std::shared_ptr<ChannelSocket> channel) {
            if (channel) {
                ...
                if (name == "sip") {
                    ...
                    std::istringstream iss(tid_str);
                    iss >> tid;
                    dhtPeerConnector_->onIncomingConnection(peerId, tid, std::move(channel), isVCard);
                }
            }
        });
    ...
    try {
        ...
        while (vCard.size()) {
            ...
            if (deviceConnIt != sipConns.end()) {
                ...
                ctx->success = success;
                ctx->total = total;
                ctx->path = cachePath_ + DIR_SEPARATOR_STR + "vcard" + DIR_SEPARATOR_STR + deviceId;

                try {
                    sendSIPMessage(it, peerId, ctx.release(), token, chunk,
                        [](void *token, pjsip_event *event)
                        {
