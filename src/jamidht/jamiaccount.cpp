                              ...
                              const std::string& interactionId)
{
    ...
    if (conversation != conversations_.end() && conversation->second) {
        if (conversation->second->isMember(peer, true)) {
            ...
            auto path = fileutils::get_data_dir() + DIR_SEPARATOR_STR + getAccountID()
                        + "conversation_data" + DIR_SEPARATOR_STR + conversationId
                        + DIR_SEPARATOR_STR + tid_str;
            ...
            if (commit->at("sha3sum") != fileutils::sha3File(path)) {
    ...
    // TODO create symlink when send or received
}
                      ...
                      DRing::DataTransferId resendId)
{
    ...
    if (isConversation) {
        ...
        auto symlinkPath = fileutils::get_data_dir() + DIR_SEPARATOR_STR + getAccountID()
                           + "conversation_data" + DIR_SEPARATOR_STR + to + DIR_SEPARATOR_STR
                           + std::to_string(tid);
...
void
JamiAccount::askForTransfer(const std::string& conversationUri, const std::string& interactionId)
{
    ...
    askTransferValue["deviceId"] = currentDeviceId();
    Json::StreamWriterBuilder builder;
