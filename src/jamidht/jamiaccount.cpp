    ...
    try {
        ...
        if(accountPeerDiscovery_){
            if(!config.peer_discovery && !config.peer_publish){
                peerDiscovery_.reset(new dht::PeerDiscovery(PEER_DISCOVERY_PORT));
            } else {
                peerDiscovery_ = dht_.getPeerDiscovery();
            }
...
JamiAccount::startJamiAccountPublish()
{
    ...
    peerDiscovery_->startPublish<JamiAccountPeerInfo>(PEER_DISCOVERY_JAMI_SERVICE, info_pub);
}

void
JamiAccount::startJamiAccountDiscovery()
{
    peerDiscovery_->startDiscovery<JamiAccountPeerInfo>(PEER_DISCOVERY_JAMI_SERVICE,[this](JamiAccountPeerInfo&& v, dht::SockAddr&& add){
        ...
        if(v.accountId != dht::InfoHash(ringAccountId_)){
            ...
            dp.cleanupTask = Manager::instance().scheduler().scheduleIn([w = weak(), p = v.accountId, a = v.displayName]{
                if (auto this_ = w.lock()){
                    {
                        std::lock_guard<std::mutex> lc(this_->dismtx_);
