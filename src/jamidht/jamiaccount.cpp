        ...
        if (not call) {
            ...
            call_it = pendingSipCalls_.erase(call_it);
                   ...
                   || (call_it->from_cert and call_it->from_cert->issuer
                       and call_it->from_cert->getIssuerUID() == from)) {
...
{
    saveConvInfo(); // Refresh members known
    ...
    if (upnp_) {
        ...
        dhtUpnpMapping_.setNotifyCallback([w = weak(),
            ...
            if (auto accPtr = w.lock()) {
                ...
                if (*update) {
                    // Update if the mapping or its state changed.
                    if (dhtMap != *mapRes or dhtMap.getState() != mapRes->getState()) {
                        dhtMap = upnp::Mapping {*mapRes};

                        // If state changes to "OPEN" or "FAILED", connectivity must be restarted.
                        if (mapRes->getState() == upnp::MappingState::OPEN
                            or (mapRes->getState() == upnp::MappingState::FAILED)) {
                            JAMI_WARN(
                                "[Account %s] DHT UPNP mapping updated to %s. Restarting network",
                                accId.c_str(),
        ...
        // Request the mapping.
        upnp_->reserveMapping(dhtUpnpMapping_);
                    ...
                    if (itManager == transferManagers_.end()) {
                        ...
                        auto res = transferManagers_.insert(
                            std::make_pair(peerId,
                                           std::make_unique<TransferManager>(accId, peerId)));
                        if (!res.second) {
...
JamiAccount::removeContact(const std::string& uri, bool ban)
{
    ...
    std::vector<std::string> toRm;
    {
        ...
        for (const auto& [key, conv] : conversations_) {
            try {
                if (conv->mode() == ConversationMode::ONE_TO_ONE && conv->isInitialMember(uri))
...
{
    auto conversation = startConversation(ConversationMode::ONE_TO_ONE, to);
    ...
    const std::function<void(const std::string&)>& onChanneledCancelled)
{
    ...
    if (!info.conversationId.empty()) {
        ...
        // md5sum can take quite some time to computer if the user decide
        // to send a big file
        dht::ThreadPool::computation().run([w = weak(), info, tid]() {
            if (auto shared = w.lock()) {
                ...
                value["totalSize"] = fileutils::size(info.path);
                value["md5sum"] = fileutils::md5File(info.path);
        ...
        for (auto& info : convInfos_) {
            if (info.id == conversationId) {
                ...
                runOnMainThread([w = weak()]() {
                    // Send to connected devices
                    if (auto shared = w.lock()) {
                        shared->saveConvInfo(); // will lock conversationsMtx_
                        shared->syncWithConnected();
                    }
                });
                break;
                      ...
                      const InternalCompletionCb& icb)
{
    ...
    if (it == transferManagers_.end()) {
        std::string accId = getAccountID();
        auto res = transferManagers_.insert(
            std::make_pair(to, std::make_unique<TransferManager>(accId, to, isConversation)));
        if (!res.second) {
                                   ...
                                   const InternalCompletionCb& icb)
{
    ...
    std::unique_lock<std::mutex> lk(transferMutex_);
    auto it = transferManagers_.find(to);
    if (it == transferManagers_.end() || !it->second) {
