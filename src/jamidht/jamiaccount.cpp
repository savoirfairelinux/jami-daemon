        ...
        if (auto info = accountManager_->useIdentity(id,
            ...
            accountManager_->initAuthentication(
                ...
                [this, migrating](const AccountInfo& info,
                    ...
                    std::string pkFile, certFile;
                    std::tie(pkFile, certFile) = saveIdentity(id, idPath_, DEVICE_ID_PATH);
                    connectionManager_->setTlsPrivateKeyFile(pkFile);
                    connectionManager_->setTlsCertificateFile(certFile);
                    id_ = std::move(id);
                    connectionManager_->setTlsPassword("");
...
JamiAccount::setAccountDetails(const std::map<std::string, std::string>& details)
{
    ...
    // TLS
    std::string caListFile, certFile, pkFile, tlsPass;
    parsePath(details, Conf::CONFIG_TLS_CA_LIST_FILE, caListFile, idPath_);
    parsePath(details, Conf::CONFIG_TLS_CERTIFICATE_FILE, certFile, idPath_);
    parsePath(details, Conf::CONFIG_TLS_PRIVATE_KEY_FILE, pkFile, idPath_);
    parseString(details, Conf::CONFIG_TLS_PASSWORD, tlsPass);

    connectionManager_->setTlsCaListFile(caListFile);
    connectionManager_->setTlsCertificateFile(certFile);
    connectionManager_->setTlsPrivateKeyFile(pkFile);
    connectionManager_->setTlsPassword(tlsPass);
