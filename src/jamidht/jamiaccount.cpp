                ...
                [this,
                 ...
                 msgId = to_hex_string(v.id)](const std::shared_ptr<dht::crypto::Certificate>& cert,
                    ...
                    dht::ThreadPool::io().run(
                        [w = weak(), payloads = std::move(payloads), msgId, peer_account, cert] {
                            if (auto shared = w.lock()) {
            ...
            [this](const auto& convId, const auto& deviceId, auto cb, const auto& type) {
                dht::ThreadPool::io().run([w = weak(), convId, deviceId, cb = std::move(cb), type] {
                    ...
                    shared->connectionManager_->connectDevice(
                        ...
                        [shared, cb = std::move(cb), convId](std::shared_ptr<dhtnet::ChannelSocket> socket,
                                             const DeviceId&) {
                            dht::ThreadPool::io().run([cb = std::move(cb), socket=std::move(socket)] {
                                if (socket) {
                         ...
                         bool onlyConnected)
{
    ...
    // get request type
    auto getPayloadType = [payloads, extractIdFromJson]() -> std::string {
    ...
    if (deviceId.empty()) {
        ...
        accountManager_->forEachDevice(
            toH,
            [this, confirm, to, token, payloads, devices, getPayloadType](
                const std::shared_ptr<dht::crypto::PublicKey>& dev) {
                ...
                auto payload_type = getPayloadType();
