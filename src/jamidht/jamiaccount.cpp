...
JamiAccount::startOutgoingCall(const std::shared_ptr<SIPCall>& call, const std::string& toUri)
{
    ...
    accountManager_->forEachDevice(
        ...
        [this, devices = std::move(devices), sendRequest](const DeviceId& dev) {
            ...
            JAMI_ERR("@@@ SEND REQUEST FOR %s", dev.to_c_str());
                                ...
                                const DeviceId& deviceId)
{
    ...
    auto onShutdown = [w = weak(), peerId, key, socket]() {
        JAMI_ERR("@@@ on shutdown %p", socket.get());
        auto shared = w.lock();
        if (!shared)
            return;
        {
            ...
            if (connections.empty()) {
                ...
                JAMI_ERR("@@@ STILL SOME SIP CONNECTIONS BETWEEN %s %s!",
                         key.first.c_str(),
                         key.second.to_c_str());
            }
        }
        ...
        JAMI_ERR("@@@ CALL CONNECTION CLOSED FOR %s", key.second.to_c_str());
    ...
    JAMI_ERR("@@@New SIP channel opened with %s, device: %s. NB: %u, socket: %p",
             peerId.c_str(),
             deviceId.to_c_str(),
             connections.size(),
             socket.get());
