                            ...
                            runOnMainThread([w, conversationId, commitId]() {
                                if (auto shared = w.lock()) {
                                    ...
                                    if (it != shared->conversations_.end())
                                        shared->convModule_->sendMessageNotification(*it->second,
                                                                                     commitId,
                                                                                     false);
                                }
                            });
                                     ...
                                     bool sync)
{
    ...
    it->second->updateInfos(infos,
                            [w = weak(), conversationId, sync](bool ok,
...
JamiAccount::loadConversationMessages(const std::string& conversationId,
                                      ...
                                      size_t n)
{
    if (!convModule_->isConversation(conversationId))
        return 0;
