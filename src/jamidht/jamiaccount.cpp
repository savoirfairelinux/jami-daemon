        ...
        connectionManager_->onChannelRequest([this](const DeviceId& deviceId,
            ...
            if (name == "sip") {
                ...
                if (fileId == "profile.vcf") {
                    ...
                    return std::find_if(members.begin(), members.end(), [&](auto m) {
                        return m["uri"] == cert->getIssuerUID();
                    }) != members.end();
                }

                return convModule()->onFileChannelRequest(cert->getIssuerUID(),
                                                                    fileId,
                                 ...
                                 const DeviceId& device)
{
    ...
    socket->setOnRecv([this, peerId, deviceId](const uint8_t* buf, size_t len) {
        if (!buf)
            return len;
        ...
        convModule()->onSyncData(msg, peerId, deviceId);
        return len;
    });
}
...
JamiAccount::dataTransfer(const std::string& id) const
{
    ...
    return convModule()->dataTransfer();
}
