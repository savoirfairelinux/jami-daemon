        ...
        if (needsConvSync_.exchange(false)) {
            ...
            for (const auto& item : conversations_) {
                ...
                for (const auto& member : conv->getMembers()) {
                    ...
                    accountManager_->forEachDevice(
                        dht::InfoHash(peer), [w = weak(), peer, convId](const std::shared_ptr<dht::crypto::PublicKey>& dev) {
                            auto shared = w.lock();
                            if (!shared)
                                return;
                            ...
                            JAMI_INFO("[Account %s] Start syncing conversation %s with %s (%s)",
                                      ...
                                      peer.c_str(),
                                      deviceId.c_str());
                            shared->fetchNewCommits(peer, deviceId, convId);
                        });
                }
            }
                            ...
                            if (member != getUsername()) {
                                ...
                                accountManager_
                                    ->forEachDevice(dht::InfoHash(member),
                                                    [w = weak(), convId = c.id, willClone](const std::shared_ptr<dht::crypto::PublicKey>& dev) {
                                                        if (willClone->exchange(true))
                                                            return;
                                                        auto shared = w.lock();
                                                        if (!shared)
                                                            return;
                                                        shared->cloneConversation(dev->getLongId().toString(),
                                                                                  convId);
                                                    });
                            }
                        }
                ...
                = dht_->listen<dht::ImMessage>(h, [this, to, token, confirm](dht::ImMessage&& msg) {
...

                      {
                          ...
                          if (e == sentMessages_.end()
                              or e->second.to.find(msg.fromId) == e->second.to.end()) {
...
JamiAccount::acceptConversationRequest(const std::string& conversationId)
{
    ...
    forEachDevice(memberHash, [this, request = request->second](const std::shared_ptr<dht::crypto::PublicKey>& dev) {
        auto deviceId = dev->getLongId();
        if (deviceId == dht()->getLongId())
            return;
