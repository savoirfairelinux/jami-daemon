...
JamiAccount::doRegister_()
{
    ...
    try {
        ...
        if (upnpCtrl_ and dhtUpnpMapping_.getInternalPort() != dhtBoundPort_) {
            ...
            if (dhtBoundPort_ != 0) {
                JAMI_LOG("[Account {:s}] UPnP: requesting mapping for DHT port {}",
                         getAccountID(),
                         dhtBoundPort_);

                dhtUpnpMapping_.enableAutoUpdate(true);

                dhtnet::upnp::Mapping desired(dhtnet::upnp::PortType::UDP,
                                              dhtBoundPort_,
                                              dhtBoundPort_);
                    ...
                    if (auto accPtr = w.lock()) {
                        ...
                        if (dhtMap.getMapKey() != mapRes->getMapKey()
                            ...
                            if (mapRes->getState() == dhtnet::upnp::MappingState::OPEN) {
                                JAMI_LOG("[Account {:s}] Mapping {:s} successfully allocated",
                                         accId,
                                         dhtMap.toString());
                    ...
                    [w, cb = std::move(cb), socket = std::move(socket), convId, requestedDeviceId] {
                        if (socket) {
                            ...
                            socket->onShutdown([w,
                                                ...
                                                convId,
                                                ws = std::weak_ptr(socket)](const std::error_code&) {
...
{
    if (not upnpCtrl_ or dhtUpnpMapping_.getInternalPort() == 0)
        return;
                                      ...
                                      const std::string& connectionType)
{
    ...
    handler->connect(
        deviceId,
        "",
        [](const std::shared_ptr<dhtnet::ChannelSocket>&, const DeviceId&) {},
        connectionType);
}
