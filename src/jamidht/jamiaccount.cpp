        ...
        authHandler->second->connect(
            ...
            [this, token] (std::shared_ptr<dhtnet::ChannelSocket> socket, const dht::InfoHash& infoHash) {
                if (!socket) {
                    ...
                    emitSignal<libjami::ConfigurationSignal::AddDeviceStateChanged>(accountID_, token, (uint8_t)DeviceAuthState::DONE, std::map{std::pair("error"s, "network"s)});
                } else {
                    if (!accountManager_->addDevice(config().archiveHasPassword ? fileutils::ARCHIVE_AUTH_SCHEME_PASSWORD : fileutils::ARCHIVE_AUTH_SCHEME_NONE, token, socket))
                        emitSignal<libjami::ConfigurationSignal::AddDeviceStateChanged>(accountID_, token, (uint8_t)DeviceAuthState::DONE, std::map{std::pair("error"s, "state"s)});
                }
            }
        );
        runOnMainThread([token, id = accountID_] {
            emitSignal<libjami::ConfigurationSignal::AddDeviceStateChanged>(id, token, (uint8_t)DeviceAuthState::CONNECTING, std::map<std::string, std::string>{});
        });
        return token;
    } catch (const std::exception& e) {
