    ...
    , connectionManager_ {}
{
    ...
    //
    //    try {
    //        std::istringstream is(fileutils::loadCacheTextFile(cachePath_ + DIR_SEPARATOR_STR
    //        "dhtproxy",
    ...
    setActiveCodecs({});
    JAMI_DBG() << "&&&&&&using proxyServer_" << proxyServer_;
}
...
JamiAccount::startOutgoingCall(const std::shared_ptr<SIPCall>& call, const std::string& toUri)
{
    ...
    auto sendRequest =
        [this, wCall, toUri, dummyCall = std::move(dummyCall)](const DeviceId& deviceId,
            ...
            requestSIPConnection(toUri, deviceId, "call", true);
        };
