...
JamiAccount::loadConfig()
{
    ...
    try {
        ...
        if (reader->parse(is, &root, &err)) {
            proxyServerCached_ = root[config().proxyServer + config().proxyListUrl];
        }
    } catch (const std::exception& e) {
...
JamiAccount::getDhtProxyServer(const std::string& serverList)
{
    if (proxyServerCached_.empty()) {
        ...
        node[config().proxyServer + config().proxyListUrl] = proxyServerCached_;
        if (file.is_open())
            file << node;
        else
