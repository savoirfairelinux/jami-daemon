...
JamiAccount::onPortMappingAdd(uint16_t* port_used, bool success)
{
    ...
    if (success) {
        JAMI_WARN("JamiAccount: Opened port %u", port);
        if (port == dhtPort_) 
            JAMI_WARN("[Account %s] Successfully mapped port %u for DHT", getAccountID().c_str(), dhtPort_);
        else
            JAMI_WARN("[Account %s] Could not map port %u for DHT, using %u instead", getAccountID().c_str(), dhtPort_, port);
        dhtPortUsed_ = port;
        upnpCv_.notify_all();
    } else { 
        JAMI_WARN("JamiAccount: Coult not open port %u", port);
    }
}
