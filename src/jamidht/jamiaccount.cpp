...
JamiAccount::startOutgoingCall(const std::shared_ptr<SIPCall>& call, const std::string& toUri)
{
    ...
    accountManager_->forEachDevice(peer_account, [this, wCall, toUri, peer_account](const dht::InfoHash& dev)
    {
        runOnMainThread([this, dev]{
            ...
            this->connectionManager_->connectDevice(dev.toString(), "git://xxxx", [this, dev](std::unique_ptr<PeerSocket> socket) {
                if (!socket) {
                    // TODO error code?
...
JamiAccount::doRegister_()
{
    ...
    try {
        ...
        connectionManager_->onConnectionReady([this](const std::string& deviceId, const std::string& uri, std::unique_ptr<PeerSocket> socket) {
            if (!socket) {
                // TODO error code?
