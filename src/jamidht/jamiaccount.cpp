            ...
            if (key.first == from) {
                ...
                for (auto cit = value.rbegin(); cit != value.rend(); ++cit) {
                    ...
                    auto call = Manager::instance()
                                    .callFactory.newCall<SIPCall>(shared(),
                             ...
                             const std::map<std::string, std::string>& volatileCallDetails)
{
    ...
    auto call = manager.callFactory.newCall<SIPCall>(shared(),
...
JamiAccount::startOutgoingCall(const std::shared_ptr<SIPCall>& call, const std::string& toUri)
{
    ...
    auto& manager = Manager::instance();
    auto dummyCall = manager.callFactory.newCall<SIPCall>(shared(),
                                                          Call::CallType::OUTGOING,
                                                          call->getDetails());
    ...
    auto sendRequest = [this,
        ...
        auto dev_call = Manager::instance().callFactory.newCall<SIPCall>(shared(),
                                                                         Call::CallType::OUTGOING,
                                                                         call->getDetails());
    ...
    for (auto& [key, value] : sipConns_) {
        ...
        auto dev_call = manager.callFactory.newCall<SIPCall>(shared(),
                                                             Call::CallType::OUTGOING,
                                                             call->getDetails());
        dev_call->setIPToIP(true);
        dev_call->setSecure(isTlsEnabled());
        ...
        accountManager_->startSync([this](const std::shared_ptr<dht::crypto::Certificate>& crt) {
            ...
            connectionManager_->connectDevice(crt->getId(),
                                              ...
                                              [this](std::shared_ptr<ChannelSocket> socket,
                                                  ...
                                                  JAMI_ERR("ON SYNC %u", (bool) socket);
                                                  if (socket)
        ...
        connectionManager_->onConnectionReady([this](const DeviceId& deviceId,
            ...
            if (channel) {
                ...
                if (name == "sip") {
                    ...
                    if (hasGitSocket(deviceId.toString(), conversationId)) {
