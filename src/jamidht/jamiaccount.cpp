...
JamiAccount::registerAsyncOps()
{
    ...
    if (upnp_) {
        upnp_->requestMappingAdd([this, onLoad](uint16_t port_used, bool success) {
            ...
            if (not success and registrationState_ == RegistrationState::UNREGISTERED) {
                JAMI_WARN("[Account %s] Failed to open port %u: starting DHT anyways", getAccountID().c_str(), oldPort);
                onLoad();
                return;
            }

            if (oldPort != newPort or registrationState_ == RegistrationState::UNREGISTERED){
                dhtPortUsed_ = newPort;
                if (registrationState_ == RegistrationState::UNREGISTERED) {
