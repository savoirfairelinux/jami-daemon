                          ...
                          DRing::DataTransferId tid)
{
    ...
    connectionManager_
        ->connectDevice(DeviceId(deviceId),
                        channelName,
                        [this, path = std::move(path), tid](std::shared_ptr<ChannelSocket> socket,
                                                            const DeviceId&) {
                            if (!socket)
                                return;
                            // TODO move
                            ...
                            dht::ThreadPool::io().run(
                                [w = weak(), path = std::move(path), socket = std::move(socket)] {
                                    if (auto shared = w.lock()) {
                                        ...
                                        try {
                                            ...
                                            while (!file.eof()) {
                                                ...
                                                socket->write(reinterpret_cast<const uint8_t*>(
                                                                  buffer.data()),
                                                              file.gcount(),
                                                              ec);
                                                if (ec)
