...
void
JamiAccount::addDevice(const std::string& uri)
{
    if (not accountManager_) {
        // emitSignal<libjami::ConfigurationSignal::ExportOnRingEnded>(getAccountID(), 2, "");
        emitSignal<libjami::ConfigurationSignal::AddDeviceStateChanged>(getAccountID(), libjami::ConfigurationSignal::FAILURE, "");
        return;
    }

    channelHandlers_[Uri::Scheme::AUTH]->connect(crt->getLongId(), "", [this](std::shared_ptr<dhtnet::ChannelSocket> socket, const DeviceId& deviceId) {
    if (!socket) { return; }
        // KESS this is where old device starts and the callback is also defined here
        ArchiveAccountManager::addDevice([]() {});
    });

    accountManager_
        ->addDevice(uri, [this](AccountManager::AddDeviceResult result) {
            switch (result) {
            case AccountManager::AddDeviceResult::SUCCESS_CREATE_URI:
                emitSignal<libjami::ConfigurationSignal::AddDeviceStateChanged>(0, "create_uri");
                break;
