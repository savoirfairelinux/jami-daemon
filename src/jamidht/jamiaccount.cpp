...
std::shared_ptr<Call>
JamiAccount::newOutgoingCall(std::string_view toUrl, const std::vector<MediaMap>& mediaList)
{
...
JamiAccount::createSubCall(const std::shared_ptr<SIPCall>& mainCall)
{
    ...
    auto const& mediaAttrList = mainCall->getMediaAttributeList();

    if (not mediaAttrList.empty()) {
        newCall = manager.callFactory.newCall<JamiAccount>(shared(),
                                                           manager.getNewCallID(),
                                                           Call::CallType::OUTGOING,
                                                           mediaAttrList);
    } else {
    ...
    auto subCall = std::dynamic_pointer_cast<SIPCall>(newCall);
    assert(subCall);
                                     ...
                                     IpAddr target)
{
    ...
    opts.onInitDone = [w = weak(), target = std::move(target), call](bool ok) {
        ...
        auto const& mediaAttrList = call->getMediaAttributeList();
...
JamiAccount::registerAsyncOps()
{
    ...
    if (upnp_ != nullptr) {
        ...
        dhtUpnpMapping_.setNotifyCallback([w = weak(),
            ...
            if (auto weakThis = w.lock()) {
                auto& dhtMap = weakThis->dhtUpnpMapping_;
                auto& accId = weakThis->getAccountID();
                ...
                if (*update) {
                    ...
                    if (dhtMap != *mapRes or dhtMap.getState() != mapRes->getState()) {
                        ...
                        if (mapRes->getState() == upnp::MappingState::OPEN
                            ...
                            weakThis->dht_->connectivityChanged();
                        }
                    }
                } else {
                    *update = true;
