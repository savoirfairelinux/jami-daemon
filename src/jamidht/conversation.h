...
public:
    ...
    /**
     * Check if a device is authorized to clone or interact with the conversation.
     * Assumption: the deviceId must already be a confirmed device of the user.
     * @param uri               URI of the user
     * @param deviceId          Device id to check
     * @param includeInvited    If true, consider invited members as authorized.
     */
     ...
     * stores it in pluginData and emits SwarmMessageUpdated for the affected parent message
     * (edition commits signal their parent; originals signal themselves).
     * No-op when no plugin chat handlers are registered (ENABLE_PLUGIN guard).
     */
    ...
    /**
     * Stores or removes the plugin-provided bodyOverwrite for a single message.
     * An empty bodyOverwrite erases the existing entry; a non-empty value replaces it.
     * If messageId refers to an edition commit, SwarmMessageUpdated is emitted for its
     * parent (the message clients display); otherwise it is emitted for messageId itself.
     ...
     * SwarmMessageUpdated for each affected parent message so clients revert to the original
     * body.  Edition commits are resolved to their parent before signalling.
     */
