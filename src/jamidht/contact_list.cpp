...
{
    if (cert) {
        accountUri_ = cert->getId().toString();
        trust_ = std::make_unique<tls::TrustStore>(Manager::instance().certStore(accountUri_));
...
{
    auto& certStore = Manager::instance().certStore(accountUri_);
    try {
        // read file
                                ...
                                const time_point& updated)
{
    ...
    if (it.second) {
        ...
        Manager::instance().certStore(accountUri_).pinCertificate(crt);
        if (crt->ocspResponse) {
