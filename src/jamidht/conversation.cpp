...
public:
    ...
    Impl(const std::weak_ptr<JamiAccount>& account, const std::string& remoteDevice, const std::string& conversationId)
    : account_(account)
    {
                                 ...
                                 size_t n)
{
    ...
    if (toMessage.empty())
        convCommits = repository_->log(fromMessage, n);
    else
...
Conversation::getMembers()
{
    ...
    auto repoPath = fileutils::get_data_dir() + DIR_SEPARATOR_STR + shared->getAccountID()
                    + DIR_SEPARATOR_STR + "conversations" + DIR_SEPARATOR_STR + pimpl_->repository_->id();
