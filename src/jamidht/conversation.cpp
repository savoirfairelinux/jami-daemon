...
Conversation::mergeHistory(const std::string& uri)
{
    ...
    auto result = pimpl_->repository_->convCommitToMap(newCommits);
...
void
Conversation::search(uint32_t req, const Filter& filter) const
{
    // Because logging a conversation can take quite some time,
    // do it asynchronously
    dht::ThreadPool::io().run([w = weak(), req, filter] {
        if (auto sthis = w.lock()) {
            ...
            emitSignal<DRing::ConversationSignal::MessagesFound>(req, acc->getAccountID(),
                                                                 sthis->id(),
                                                                 std::move(commits));
        }
    });
