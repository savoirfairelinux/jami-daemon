...
Conversation::Impl::loadMissingBodyOverwrites()
{
    ...
    // Write results back and collect unique parent ids to signal once each.
    std::set<std::string, std::less<> > parentsToSignal;
    {
        std::lock_guard const lk(loadedHistory_.mutex);
        for (const auto &transformed: batch) {
            ...
            // Edition commit → signal parent; original → signal itself.
            const auto editIt = transformed.body.find(CommitKey::EDIT);
            const bool isEdit = editIt != transformed.body.end() && !editIt->second.empty();
            parentsToSignal.insert(isEdit ? editIt->second : transformed.id);
        }
    }

    // Collect signal messages under lock, emit outside — one per unique parent.
    std::vector<libjami::SwarmMessage> signals;
    signals.reserve(parentsToSignal.size());
    {
        std::lock_guard const lk(loadedHistory_.mutex);
        for (const auto &parentId: parentsToSignal) {
            if (auto it = loadedHistory_.quickAccess.find(parentId);
                it != loadedHistory_.quickAccess.end()) {
...
Conversation::Impl::updateMessageBodyOverwrite(const std::string &messageId, const std::string_view bodyOverwrite) {
    ...
    libjami::SwarmMessage signalMsg;
    {
        ...
        // Edition commit: also store on parent and signal parent (client tracks parent).
        // Original message: signal itself.
        if (const auto editIt = it->second->body.find(CommitKey::EDIT);
            editIt != it->second->body.end() && !editIt->second.empty()) {
            const auto parentIt = loadedHistory_.quickAccess.find(editIt->second);
            ...
            signalMsg = injectEditionOverwrites(*parentIt->second, loadedHistory_.quickAccess);
        } else {
...
Conversation::Impl::clearBodyOverwrites()
{
    ...
    std::vector<libjami::SwarmMessage> updated;
    {
        ...
        std::set<std::string, std::less<> > toSignal;
        ...
        for (const auto& parentId : toSignal) {
            if (auto parentIt = loadedHistory_.quickAccess.find(parentId);
                parentIt != loadedHistory_.quickAccess.end()) {
