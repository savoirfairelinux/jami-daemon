                                       ...
                                       History& history) const
{
    ...
    bool hasBeenRectified = false;
    auto parentIt = history.quickAccess.find(message->linearizedParent);
    auto currentMessage = message;

    while(hasBeenRectified == false && parentIt != history.quickAccess.end()){
        const std::shared_ptr<libjami::SwarmMessage>& parent = parentIt->second;
                      ...
                      bool emit)
{
    ...
    for (const auto& [cid, _]: optHistory.quickAccess) {
        ...
        //rectifyStatus(message->second, optHistory);
    }
}
