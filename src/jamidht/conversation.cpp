        ...
        if (ok) {
            ...
            for (const auto& c : commits) {
                ...
                if (pluginChatManager.hasHandlers()) {
                    ...
                    if (editIt != c.end() && !editIt->second.empty()) {
                        // Edit commit: transform both the edition commit (for history) and the
                        // parent (for current body), then signal the parent with injected overwrites.
                        const std::string parentId = editIt->second;
                        const std::string edId = (idIt != c.end()) ? idIt->second : "";

                        // Step 1: collect snapshots under lock.
                        bool doTransform = false;
                        libjami::SwarmMessage edSnap;
                        {
                            ...
                            if (parentIt != loadedHistory_.quickAccess.end()) {
                                ...
                                auto reactToIt = parent.body.find(CommitKey::REACT_TO);
                                if ((reactToIt == parent.body.end() || reactToIt->second.empty())
                                    && parent.type == CommitType::TEXT && !edId.empty()) {
                                    auto edIt = loadedHistory_.quickAccess.find(edId);
                                    if (edIt != loadedHistory_.quickAccess.end()) {
                                        edSnap = *edIt->second;
                                        edSnap.pluginData.erase("bodyOverwrite");
                                        doTransform = true;
                                    }
                                }
                            }
                        ...
                        // Step 2: run plugin transform outside lock.
                        std::optional<std::string> newOverwrite;
                        if (doTransform) {
                            ...
                            if (boIt != single[0].pluginData.end() && !boIt->second.empty()) {
                                newOverwrite = boIt->second;
                                JAMI_DEBUG("announce: plugin set bodyOverwrite on edit {} (parent {}) in {}",
                                           edId,
                                           parentId,
                                           convId);
                            } else {
                                JAMI_DEBUG("announce: plugin cleared bodyOverwrite on edit {} (parent {}) in {}",
                                           edId,
                                           parentId,
                                           convId);
                            }
                        ...
                        // Step 3: write back under lock and build signal copy.
                        ...
                        bool shouldSignal = false;
                        {
                            ...
                            if (parentIt != loadedHistory_.quickAccess.end()) {
                                ...
                                auto reactToIt = parent.body.find(CommitKey::REACT_TO);
                                if (reactToIt == parent.body.end() || reactToIt->second.empty()) {
                                    if (doTransform && !edId.empty()) {
                                        auto edIt = loadedHistory_.quickAccess.find(edId);
                                        if (edIt != loadedHistory_.quickAccess.end()) {
                                            if (newOverwrite) {
                                                edIt->second->pluginData["bodyOverwrite"] = *newOverwrite;
                                                // Parent now shows this same text, so share the overwrite.
                                                parent.pluginData["bodyOverwrite"] = *newOverwrite;
                                            } else {
                                                edIt->second->pluginData.erase("bodyOverwrite");
                                                parent.pluginData.erase("bodyOverwrite");
                                            }
                                        } else {
                                            JAMI_DEBUG("announce: edit {} evicted before write-back in {}",
                                                       edId,
                                                       convId);
                                        }
                                    }
                                    // Always notify the client: the body changed regardless of
                                    // whether a plugin overwrite was applied.
                                    signalMsg = injectEditionOverwrites(parent, loadedHistory_.quickAccess);
                                    shouldSignal = true;
                                }
                            } else {
                                JAMI_DEBUG("announce: parent {} evicted before write-back in {}", parentId, convId);
                            }
                        }

                        // Step 4: emit outside lock.
                        if (shouldSignal)
                            emitSignal<libjami::ConversationSignal::SwarmMessageUpdated>(accountId_, convId, signalMsg);
                    } else if (typeIt != c.end() && idIt != c.end() && typeIt->second == CommitType::TEXT) {
                        // New text message: apply the plugin overwrite and notify only if set.
                        ...
                        {
                            std::lock_guard lk(loadedHistory_.mutex);
                            auto it = loadedHistory_.quickAccess.find(msgId);
                        ...
                        if (found) {
                            ...
                            auto boIt = single[0].pluginData.find("bodyOverwrite");
                            if (boIt != single[0].pluginData.end() && !boIt->second.empty()) {
                                const std::string overwrite = boIt->second;
                                JAMI_DEBUG("announce: plugin set bodyOverwrite on message {} in {}", msgId, convId);
                                ...
                                bool shouldSignal = false;
                                {
                                    ...
                                    auto it = loadedHistory_.quickAccess.find(msgId);
                                    if (it != loadedHistory_.quickAccess.end()) {
                                        it->second->pluginData["bodyOverwrite"] = overwrite;
                                        signalMsg = *it->second;
                                        shouldSignal = true;
                                    } else {
                                        JAMI_DEBUG("announce: message {} evicted before write-back in {}",
                                                   msgId,
                                                   convId);
                                    }
                                }
                    ...
                    // No active handlers: still notify clients when an edit arrives so the
                    // updated body is visible even when no translation plugin is loaded.
                    const auto editIt = c.find(CommitKey::EDIT);
                    if (editIt != c.end() && !editIt->second.empty()) {
                        const std::string parentId = editIt->second;
                        libjami::SwarmMessage signalMsg;
...
#else
                // Plugins disabled: unconditionally notify clients when an edit arrives.
                {
                    const auto editIt = c.find(CommitKey::EDIT);
                    if (editIt != c.end() && !editIt->second.empty()) {
                        const std::string parentId = editIt->second;
