    ...
    void announce(const std::vector<std::map<std::string, std::string>>& commits) const
    {
        ...
        if (ok) {
            ...
            for (const auto& c : commits) {
                ...
                } else if (c.at("author") == userId_) {
                    updateCached = true;
                }

                if (updateCached) {
                    lastDisplayed_[userId_] = c.at(ConversationMapKeys::ID);
                    saveLastDisplayed();
                    lk.unlock();
...
Conversation::setMessageDisplayed(const std::string& uri, const std::string& interactionId)
{
    if (auto acc = pimpl_->account_.lock()) {
        ...
        if (uri == pimpl_->userId_ && pimpl_->lastDisplayedUpdatedCb_)
...
Conversation::updateLastDisplayed(const std::string& lastDisplayed)
{
    ...
    auto& currentLastDisplayed = pimpl_->lastDisplayed_[pimpl_->userId_];
