...
public:
    ...
    void loadStatus()
    {
        try {
            ...
            for (const auto& [uri, status] : messagesStatus_) {
                for (const auto& [key, value] : status) {
                    JAMI_ERROR("@@@ {} {} {}", uri, key, value);
                }
            }
        } catch (const std::exception& e) {
        }
                                 ...
                                 History* optHistory) const
{
    ...
    auto addCommit = [&](const auto& commit) {
        ...
        JAMI_ERROR("@@@ COMMIT {}", sharedCommit->id);
            ...
            for (const auto& member: repository_->members()) {
                ...
                if (!messageReceived) {
                    ...
                    if (cache == static_cast<int32_t>(libjami::Account::MessageStates::SENT)) {
                        ...
                        JAMI_ERROR("@@@ {} VS {} FOR {}", messagesStatus_[member.uri]["fetched"], sharedCommit->id, member.uri);
                        if (messagesStatus_[member.uri]["fetched"] == sharedCommit->id) {
                            cache = static_cast<int32_t>(libjami::Account::MessageStates::SENT);
                        }
                    }
                    JAMI_ERROR("@@@ SET: {} to {}", cache, sharedCommit->id);
