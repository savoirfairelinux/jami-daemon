    ...
    auto addCommit = [&](const auto& commit) {
        ...
        if (reactToIt != mss.end() && !reactToIt->second.empty()) {
            ...
            if (it != history->quickAccess.end()) {
                ...
                emitSignal<libjami::ConversationSignal::ReactionAdded>(accountId_, repository_->id(), commit.id, sharedCommit->body);
            } else {
                        ...
                        if (it != history->quickAccess.end()) {
                            ...
                            if (itPreviousReact != baseCommit->reactions.end()) {
                                ...
                                if (sharedCommit->body.at("body").empty()) {
                                    ...
                                    emitSignal<libjami::ConversationSignal::ReactionRemoved>(accountId_, repository_->id(), itReact->second);
                                }
                            }
            ...
            if (messageReceived) history->messageList.emplace_front(sharedCommit);
        ...
        if (messageReceived) {
            // TODO signal emition for edition
                // client: if message is there for edition update body
                // else ignore
                // client: if message is there for reaction update body
                // and notify
