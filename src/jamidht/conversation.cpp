...
public:
    ...
    void initActiveCalls(const std::vector<std::map<std::string, std::string>>& commits) const
    {
        ...
        for (const auto& commit : commits) {
            if (commit.at("type") == CommitType::MEMBER) {
    ...
    void announce(const std::vector<std::map<std::string, std::string>>& commits, bool commitFromSelf = false)
    {
        ...
        if (ok) {
            ...
            for (const auto& c : commits) {
                ...
                if (c.at("type") == CommitType::MEMBER) {
                ...
                } else if (c.at("type") == CommitType::CALL_HISTORY) {
                    updateActiveCalls(c);
                }
#ifdef ENABLE_PLUGIN
...
Conversation::Impl::commitsEndedCalls()
{
    ...
    for (const auto& hostedCall : hostedCalls_) {
        ...
        commitMessage.duration = (nowConverted - hostedCall.second) * 1000;
                                  ...
                                  bool messageReceived) const
{
    ...
    if (it != history.quickAccess.end()) {
        ...
        if (baseCommit) {
            ...
            std::string toReplace = (baseCommit->type == CommitType::DATA_TRANSFER) ? "tid" : "body";
                                   ...
                                   std::string& sha3sum) const
{
    ...
    if (commit == std::nullopt || commit->find("type") == commit->end() || commit->find("tid") == commit->end()
        || commit->find("sha3sum") == commit->end() || commit->at("type") != CommitType::DATA_TRANSFER) {
...
{
    auto commit = getCommit(interactionId);
    if (commit == std::nullopt || commit->at("type") != CommitType::DATA_TRANSFER) {
        ...
        [w = weak(), devices, members = std::move(members), uri, fallbackFailed = std::move(fallbackFailed)](bool ok) {
...
#endif
                JAMI_LOG("{} Bootstrap: Fallback with member: {}", sthis->pimpl_->toString(), uri);
            ...
            if (checkNext) {
                ...
                sthis->pimpl_->fallbackTimer_->async_wait(
                    std::bind(&Conversation::checkBootstrapMember, sthis, std::placeholders::_1, std::move(members)));
            } else {
...
Conversation::bootstrap(std::function<void()> onBootstrapped, const std::vector<DeviceId>& knownDevices)
{
    ...
    auto fallback = [](std::shared_ptr<Conversation> sthis, bool now = false) {
        ...
        if (now) {
            ...
            JAMI_DEBUG("{} Fallback in {} seconds", sthis->pimpl_->toString(), (20 - timeForBootstrap));
        }
        sthis->pimpl_->fallbackTimer_->async_wait(
            std::bind(&Conversation::checkBootstrapMember, sthis, std::placeholders::_1, std::move(members)));
    };
            ...
            for (auto& message : history.messageList) {
                ...
                if (filter.type.empty() && !isSearchable) {
                    ...
                    if (isSearchable) {
                        ...
                        auto body = contentType == CommitType::TEXT ? message->body.at("body")
