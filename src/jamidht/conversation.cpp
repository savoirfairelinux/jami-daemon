 ...
 * Only edition commits (body contains CommitKey::EDIT) are processed; the original
 * message entry in the editions list is left untouched.
 * Must be called with loadedHistory_.mutex held (or from the daemon's processing thread).
 * @param msg         Parent message whose editions list is to be annotated.
 * @param quickAccess Locked snapshot of the conversation's id → SwarmMessage map.
 */
                        ...
                        const std::map<std::string, std::shared_ptr<libjami::SwarmMessage>>& quickAccess)
{
    ...
    for (auto& edition : copy.editions) {
        ...
        if (idIt == edition.end()) { continue; }
        const auto msgIt = quickAccess.find(idIt->second);
        if (msgIt == quickAccess.end()) { continue; }
        const auto &edMsg = *msgIt->second;
        if (!edMsg.body.contains(CommitKey::EDIT)) { continue; }
        if (const auto boIt = edMsg.pluginData.find("bodyOverwrite");
            boIt != edMsg.pluginData.end() && !boIt->second.empty()) {
                ...
                if (auto &pluginChatManager = Manager::instance().getJamiPluginManager().getChatServicesManager();
                    ...
                    if (const auto idIt = c.find("id");
                        ...
                        if (!snap.id.empty()) {
                            ...
                            const bool hasOverwrite = boIt != transformed.pluginData.end()
                                                      && !boIt->second.empty();
                            ...
                            libjami::SwarmMessage signalMsg;
                            {
                                ...
                                if (isEdit || hasOverwrite) {
                                    const std::string& signalId = isEdit ? editIt->second : msgId;
                                    if (auto sigIt = loadedHistory_.quickAccess.find(signalId);
                                        sigIt != loadedHistory_.quickAccess.end()) {
                                        signalMsg = injectEditionOverwrites(*sigIt->second,
    ...
    if (it != history.quickAccess.end()) {
        ...
        if (baseCommit) {
            ...
            if (itReact != baseCommit->body.end()) {
                ...
                editionBody["id"] =
                    it->second->latestEditionId.empty() ? it->second->id : it->second->latestEditionId;
                                  ...
                                  bool messageReceived) const
{
    ...
    if (peditIt != history.pendingEditions.end()) {
        auto oldBody = sharedCommit->body;
        // Tag original body with its own message id so injectEditionOverwrites skips it
        // (the original message has no CommitKey::EDIT, so it is not an edition commit).
...
Conversation::Impl::loadMissingBodyOverwrites()
{
    ...
    // Write results back and collect unique message ids to signal once each.
    std::set<std::string> toSignal;
    {
        ...
        for (const auto &transformed: batch) {
            ...
            if (auto it = loadedHistory_.quickAccess.find(transformed.id);
                it != loadedHistory_.quickAccess.end()) {
                it->second->pluginData["bodyOverwrite"] = boIt->second;
            }
            // Edition commit → signal original message; original → signal itself.
            const auto editIt = transformed.body.find(CommitKey::EDIT);
            const bool isEdit = editIt != transformed.body.end() && !editIt->second.empty();
            toSignal.insert(isEdit ? editIt->second : transformed.id);
        }
    }

    // Collect signal messages under lock, emit outside — one per unique message.
    std::vector<libjami::SwarmMessage> signals;
    signals.reserve(toSignal.size());
    {
        std::lock_guard const lk(loadedHistory_.mutex);
        for (const auto &msgId: toSignal) {
            if (auto it = loadedHistory_.quickAccess.find(msgId);
                it != loadedHistory_.quickAccess.end()) {
                signals.push_back(injectEditionOverwrites(*it->second, loadedHistory_.quickAccess));
            }
        }
    }
    for (const auto &msg: signals) {
...
Conversation::Impl::updateMessageBodyOverwrite(const std::string &messageId, const std::string_view bodyOverwrite) {
    ...
    libjami::SwarmMessage signalMsg;
    {
        ...
        // Store on the message itself (edition commit or original).
        ...
        // Edition commit: also store on original message and signal it (client displays original).
        // Original message: signal itself.
        if (const auto editIt = it->second->body.find(CommitKey::EDIT);
            editIt != it->second->body.end()) {
            ...
            if (bodyOverwrite.empty()) {
                originalIt->second->pluginData.erase("bodyOverwrite");
            } else {
                originalIt->second->pluginData["bodyOverwrite"] = bodyOverwrite;
            }
            signalMsg = injectEditionOverwrites(*originalIt->second, loadedHistory_.quickAccess);
        } else {
            signalMsg = injectEditionOverwrites(*it->second, loadedHistory_.quickAccess);
        }
    }
...
Conversation::Impl::clearBodyOverwrites()
{
    ...
    std::vector<libjami::SwarmMessage> updated;
    {
        ...
        for (const auto &[id, msg]: loadedHistory_.quickAccess) {
            bool changed = false;
            if (auto boIt = msg->pluginData.find("bodyOverwrite");
                boIt != msg->pluginData.end()) {
        ...
        for (const auto& msgId : toSignal) {
            if (auto msgIt = loadedHistory_.quickAccess.find(msgId);
                msgIt != loadedHistory_.quickAccess.end()) {
