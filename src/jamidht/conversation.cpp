...
Conversation::Impl::disconnectFromPeer(const std::string& peerUri)
{
    ...
    {
        std::lock_guard lk(trackedMembersMtx_);
        if (auto it = trackedMembers_.find(peerUri); it != trackedMembers_.end()) {
            devicesToRemove.insert(it->second.devices.begin(), it->second.devices.end());
            devicesToRemove.insert(it->second.failedDevices.begin(), it->second.failedDevices.end());
        }
    }
...
bool
Conversation::isPeerAuthorized(const std::string& uri,
                               const std::string& deviceId,
                               bool includeInvited) const
{
    if (isMemberBanned(uri) || isDeviceBanned(deviceId) || !isMember(uri, includeInvited))
        return false;

    auto mappedUri = uriFromDevice(deviceId);
    return mappedUri.empty() || mappedUri == uri;
}
