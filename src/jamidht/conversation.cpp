                                  ...
                                  bool messageReceived) const
{
    ...
    if (messageReceived) {
        if (sharedCommit->reannounce == "1") {
            fmt::print("    @@@ Updating message: {} -> {}\n", sharedCommit->id, sharedCommit->linearizedParent);
...
Conversation::Impl::rectifyStatus(const std::shared_ptr<libjami::SwarmMessage>& message, History& history) const
{
    ...
    std::string log = fmt::format("@@@ rectifyStatus: {}\n", message->id);
    size_t maxIter = history.quickAccess.size() + 1;
    while (parentIt != history.quickAccess.end() && maxIter > 0) {
        maxIter--;
