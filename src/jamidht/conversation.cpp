    ...
    void init()
    {
        if (auto shared = account_.lock()) {
            ...
            for (const auto& confId: hostedCalls_) {
                Json::Value value;
                value["uri"] = shared->getAccountID();
                value["device"] = std::string(shared->currentDeviceId());
                value["confId"] = confId;
                value["type"] = "application/call-history+json";
                value["duration"] = "1"; // In this case, this means that we left
                ...
                // TODO!!!!!
                JAMI_WARN("@@@ TODO Removing hosted conference... %s", confId.c_str());
            }
            hostedCalls_.clear();
        }
    }
