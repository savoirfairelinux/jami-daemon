...
public:
    ...
    // Note : lastSwarmMessageIdLoaded stores an interactionId and is used to prevent from clearing the memberToStatus
    ...
    auto addCommit = [&](const auto& commit) {
        ...
        if (!commitFromSelf && optHistory == nullptr) {
            ...
            for (const auto& member: repository_->members()) {
                ...
                // And we need to update lastSwarmMessageIdLoaded
                // To garantee that we will clear the cache if
                // we don't load messages from a specific message
                // in order to get the correct status everytime we call this function
