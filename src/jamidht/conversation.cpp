...
ConversationRequest::toJson() const
{
    ...
    json[ConversationMapKeys::RECEIVED] = static_cast<uint32_t>(toSecondsSinceEpoch(received));
    json[ConversationMapKeys::RECEIVED_MS] = Json::Int64(toMillisecondsSinceEpoch(received));
    if (declined != TimePoint {}) {
        json[ConversationMapKeys::DECLINED] = static_cast<uint32_t>(toSecondsSinceEpoch(declined));
