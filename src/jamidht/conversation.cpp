        ...
        if (ok) {
            ...
            for (const auto& c : commits) {
                ...
                if (pluginChatManager.hasHandlers()) {
                    ...
                    const auto typeIt = c.find("type");
                    const auto idIt = c.find("id");
                    if (typeIt != c.end() && idIt != c.end() && typeIt->second == "text/plain") {
        ...
        if (baseCommit) {
            ...
            if (itReact != baseCommit->body.end()) {
                ...
                if (it->second->type == "text/plain") {
                    for (auto jt = it->second->pluginData.begin(); jt != it->second->pluginData.end();) {
        ...
        for (auto& [id, msg] : loadedHistory_.quickAccess) {
            ...
            if (!needsWork) {
                for (std::size_t i = 0; i < msg->editions.size(); ++i) {
                    if (msg->pluginData.find("bodyOverwrite_e" + std::to_string(i)) == msg->pluginData.end()) {
                        needsWork = true;
                        break;
                    }
                }
