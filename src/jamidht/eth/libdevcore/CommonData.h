...
std::string
toHex(Iterator _it, Iterator _end, std::string _prefix)
{
...
inline bytes
toCompactBigEndian(uint8_t _val, unsigned _min = 0)
{
...
unsigned
commonPrefix(T const& _t, _U const& _u)
{
...
template<class T>
void
trimFront(T& _t, unsigned _elements)
{
...
std::set<T>&
operator+=(std::set<T>& _a, U const& _b)
{
    for (auto const& i : _b)
        _a.insert(i);
    return _a;
...
template<class T, class U>
std::unordered_set<T>&
operator+=(std::unordered_set<T>& _a, U const& _b)
{
    for (auto const& i : _b)
        _a.insert(i);
    return _a;
...
template<class T, class U>
std::vector<T>
keysOf(std::unordered_map<T, U> const& _m)
{
    std::vector<T> ret;
    for (auto const& i : _m)
        ret.push_back(i.first);
...
template<class T, class U>
std::vector<U>
valuesOf(std::unordered_map<T, U> const& _m)
{
    std::vector<U> ret;
    ret.reserve(_m.size());
    for (auto const& i : _m)
...
bool
contains(T const& _t, V const& _v)
{
