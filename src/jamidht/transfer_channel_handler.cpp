...
TransferChannelHandler::TransferChannelHandler(std::weak_ptr<JamiAccount>&& account,
...
TransferChannelHandler::onRequest(const DeviceId& deviceId, const std::string& name)
{
    ...
    if (fileId == "profile.vcf") {
        ...
        return std::find_if(members.begin(),
                            members.end(),
                            [&](auto m) { return m["uri"] == uri; })
               != members.end();
    } else if (fileId.find("profile") != std::string::npos) {
                                ...
                                std::shared_ptr<ChannelSocket> channel)
{
    ...
    auto fileHost = idstr.substr(sep + 1, lastSep - sep - 1);
    auto fileId = idstr.substr(lastSep + 1);
    if (fileHost == acc->currentDeviceId()) // This means we are the host, so the file is
                                            // outgoing, ignore
    {
        return;
    }
    ...
    if (fileId == "profile.vcf") {
        ...
        auto path = std::string(
            acc->dataTransfer()->profilePath(fileId.substr(0, fileId.size() - 4 /*.vcf*/)));
        acc->dataTransfer()->transferFile(channel, fileId, "", path);
        return;
    }
