    ...
    OnChangeCallback onChange)
{
    ...
    request->add_on_state_change_callback([request, reqid, ctx, onAsync = onAsync_]
                                          (Request::State state, const dht::http::Response& response){
        JAMI_ERR("[Auth] Got server response: %d", (int)state);
        if (state == Request::State::SENDING && request->get_connection()) {
            request->get_connection()->timeout(std::chrono::seconds(30),
...
ServerAccountManager::syncDevices()
{
    ...
    setHeaderFields(*request);
    request->add_on_state_change_callback([request, reqid, onAsync = onAsync_]
                                            (Request::State state, const dht::http::Response& response){
        onAsync([request, reqid, state, response] (AccountManager& accountManager) {
            JAMI_ERR("[Auth] Got server response: %d", (int)state);
            auto& this_ = *static_cast<ServerAccountManager*>(&accountManager);
            JAMI_ERR("@@@ %s", request->to_string().c_str());
            JAMI_ERR("@@@ %i", response.status_code);
            JAMI_ERR("@@@ %s", response.body.c_str());
            if (state == Request::State::SENDING && request->get_connection()) {
                request->get_connection()->timeout(std::chrono::seconds(30),
                    [request](const asio::error_code& ec) {
                        if (request) request->cancel();
