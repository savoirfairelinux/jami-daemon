    ...
    OnChangeCallback onChange)
{
    ...
    dht::ThreadPool::computation().run([onAsync = onAsync_, ctx, url]{
        onAsync([&](AccountManager& accountManager){
            ...
            auto request = std::make_shared<Request>(*Manager::instance().ioContext(), url, this_.logger_, body, [ctx, onAsync]
...
ServerAccountManager::authenticateDevice() {
    const std::string url = managerHostname_ + PATH_AUTH;
    JAMI_WARN("[Auth] getting a device token", creds_->username.c_str(), url.c_str());
    auto request = std::make_shared<Request>(*Manager::instance().ioContext(), url, logger_, [onAsync = onAsync_] (Json::Value json, const dht::http::Response& response){
        onAsync([&] (AccountManager& accountManager) {
            auto& this_ = *static_cast<ServerAccountManager*>(&accountManager);
            if (response.status_code >= 200 || response.status_code < 300) {
                JAMI_WARN("[Auth] Got server response: %s", response.body.c_str());
                this_.deviceToken_ = json["access_token"].asString();
            }
        });
    });
...
ServerAccountManager::revokeDevice(const std::string& password, const std::string& device, RevokeDeviceCallback cb)
{
    ...
    auto request = std::make_shared<Request>(*Manager::instance().ioContext(), url, logger_, [cb, onAsync = onAsync_] (Json::Value json, const dht::http::Response& response){
        onAsync([&] (AccountManager& accountManager) {
