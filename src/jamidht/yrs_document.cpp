...
// Whether a delta op carries formatting attributes that are safe to hand to
// libyrs. yinput_json() only stores the pointer, but converting it later runs
// CStr::to_str().unwrap() then serde_json::from_str().unwrap(), so malformed
// UTF-8 or malformed JSON panics inside Rust and aborts the daemon. Attributes
// that are not a JSON object are dropped, which is what libyrs does with them
// anyway.
bool
usableAttrs(const std::string& attrs)
{
    if (attrs.empty())
        return false;
    Json::Value value;
    return json::parse(attrs, value) && value.isObject();
}
...
YrsDocument::applyDelta(const std::vector<RichOp>& ops)
{
    ...
    for (const auto& op : ops) {
        ...
        case RichOp::Kind::Insert: {
            if (!op.text.empty()) {
                const std::string text = utf8_make_valid(op.text);
                if (usableAttrs(op.attrs)) {
                    YInput attr = yinput_json(op.attrs.c_str());
                    ytext_insert(pimpl_->text, txn, index, text.c_str(), &attr);
                } else {
                    ytext_insert(pimpl_->text, txn, index, text.c_str(), nullptr);
                }
                index += utf16Len(text);
            }
            break;
    ...
    for (uint32_t i = 0; i < n; ++i) {
        ...
        if (chunks[i].fmt_len > 0) {
            ...
            for (uint32_t j = 0; j < chunks[i].fmt_len; ++j) {
                ...
                attrs[entry.key] = entry.value ? yOutputToJson(*entry.value) : Json::Value(Json::nullValue);
            }
            op["attributes"] = attrs;
        }
        arr.append(op);
...
{
    const std::string name = utf8_make_valid(utf8Name);
    std::lock_guard<std::recursive_mutex> lk(pimpl_->mutex);
    YTransaction* txn = ydoc_write_transaction(pimpl_->doc, 0, nullptr);
    ...
    if (!name.empty())
        ytext_insert(pimpl_->nameBranch, txn, 0, name.c_str(), nullptr);
    ytransaction_commit(txn);
...
{
    // libyrs decodes the C string with str::to_str().unwrap(), which panics and
    // aborts the daemon on malformed UTF-8, so never hand it unvalidated input.
    const std::string text = utf8_make_valid(utf8Text);
    std::lock_guard<std::recursive_mutex> lk(pimpl_->mutex);
    YTransaction* txn = ydoc_write_transaction(pimpl_->doc, 0, nullptr);
    // Same safety net as applyDelta(): a stale index coming from the client
    // would make yrs panic and abort the whole daemon.
    uint32_t len = ytext_len(pimpl_->text, txn);
    ytext_insert(pimpl_->text, txn, std::min(index, len), text.c_str(), nullptr);
    ytransaction_commit(txn);
}
...
YrsDocument::remove(uint32_t index, uint32_t length)
{
    ...
    if (uint32_t n = std::min(length, len - index); n > 0)
        ytext_remove_range(pimpl_->text, txn, index, n);
    ytransaction_commit(txn);
}
...
YrsDocument::applyUpdate(const Bytes& update, bool silent)
{
    ...
    YTransaction* txn = ydoc_write_transaction(pimpl_->doc, 0, nullptr);
    // A remote peer can send a malformed update; ytransaction_apply() reports it
    // through its return code instead of applying anything, so don't ignore it.
    const auto err = ytransaction_apply(txn, reinterpret_cast<const char*>(update.data()), update.size());
    if (err != 0)
        JAMI_WARNING("[YrsDocument] discarding a malformed update of {} bytes (error {})",
                     update.size(),
