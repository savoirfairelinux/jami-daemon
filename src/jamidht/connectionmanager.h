...
public:
     ...
     */
    void connectDevice(const dht::InfoHash& deviceId, const std::string& name, ConnectCallback cb);

    /**
     * Close all connections with a current device
     * @param deviceId      Remote device
     */
    void closeConnectionsWith(const dht::InfoHash& deviceId);

    /**
     * Method to call to listen to incoming requests
     * @param deviceId      Account's device
     */
    void onDhtConnected(const dht::InfoHash& deviceId);
