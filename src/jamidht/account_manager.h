...
public:
    ...
    using OnExportConfig = std::function<const std::map<std::string, std::string>&()>;
    ...
    : path_(path), onAsync_(std::move(onAsync)), onExportConfig_(std::move(onExportConfig)), dht_(dht)  {};
