...
protected:
    ...
    /** The revocation list currently kept alive by a permanent put: its value id, 0 if none, and
     *  its update time. Guarded by publishedCrlMutex_, since startSync() and
     *  ArchiveAccountManager::revokeDevice() publish from different threads. */
    std::mutex publishedCrlMutex_;
    uint64_t publishedCrlId_ {0};
    std::chrono::system_clock::time_point publishedCrlTime_ {};
};
