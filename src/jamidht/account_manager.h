...
public:
    ...
    //virtual bool addDevice(std::string_view /*scheme*/, uint32_t /*token*/, const std::shared_ptr<dhtnet::ChannelSocket>& channel) {
    //    channel->shutdown();
    //    return false;
    //};
    virtual uint32_t addDevice(const std::string& /*uri*/, std::string_view /*auth_scheme*/, AuthChannelHandler*) { return 0; };
