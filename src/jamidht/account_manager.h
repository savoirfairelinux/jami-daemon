...
public:
    ...
    AccountManager(const std::string& path, OnAsync&& onAsync, std::shared_ptr<dht::DhtRunner> dht) : path_(path), onAsync_(std::move(onAsync)), dht_(dht)  {};
...
public:
    ArchiveAccountManager(const std::string path, std::shared_ptr<dht::DhtRunner> dht, OnAsync&& onAsync, std::string archivePath, const std::string& nameServer)
     : AccountManager(path, std::move(onAsync), dht), archivePath_(std::move(archivePath)), nameDir_(NameDirectory::instance(nameServer)) {};
