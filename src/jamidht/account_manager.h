...
public:
    ...
    AccountManager(const std::string& path, OnAsync&& onAsync) : path_(path), onAsync_(std::move(onAsync))  {};
...
public:
    ArchiveAccountManager(const std::string path, std::shared_ptr<dht::DhtRunner> dht, OnAsync&& onAsync, std::string archivePath, const std::string& nameServer)
     : AccountManager(path, std::move(onAsync)), dht_(dht), archivePath_(std::move(archivePath)), nameDir_(NameDirectory::instance(nameServer)) {};
