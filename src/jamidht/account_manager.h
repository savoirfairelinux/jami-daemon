...
public:
    ...
    AccountManager(OnAsync&& onAsync) : onAsync_(std::move(onAsync))  {};
...
public:
    ArchiveAccountManager(std::shared_ptr<dht::DhtRunner> dht, OnAsync&& onAsync, std::string archivePath, const std::string& nameServer)
     : AccountManager(std::move(onAsync)), dht_(dht), archivePath_(std::move(archivePath)), nameDir_(NameDirectory::instance(nameServer)) {};
