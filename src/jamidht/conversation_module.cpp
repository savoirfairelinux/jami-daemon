                                          ...
                                          const std::string& commitId)
{
    ...
    if (conv->conversation) {
        ...
        if (!conv->conversation->isPeerAuthorized(peer, deviceId, true)) {
            if (conv->conversation->isMemberBanned(peer)) {
                JAMI_WARNING("[Account {}] [Conversation {}] {} is banned", accountId_, conversationId, peer);
            } else if (conv->conversation->isDeviceBanned(deviceId)) {
                JAMI_WARNING("[Account {}] [Conversation {}] device {} is banned", accountId_, conversationId, deviceId);
            } else if (!conv->conversation->isMember(peer, true)) {
