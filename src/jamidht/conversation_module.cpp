...
public:
    ...
    /**
     * Handle events to receives new commits
     */
    ...
    void startFetch(const std::string& convId)
    {
...
ConversationModule::acceptConversationRequest(const std::string& conversationId)
{
    ...
    pimpl_->startFetch(conversationId);
                               ...
                               const std::string& deviceId)
{
    ...
    for (const auto& [convId, req] : msg.cr) {
        ...
        // New request
        pimpl_->addConversationRequest(convId, req);
