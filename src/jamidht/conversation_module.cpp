                                                  ...
                                                  const std::string& deviceId)
{
    ...
    if (!acc)
        return;
    auto commit = commitId.empty() ? conversation.lastCommitId() : commitId;
    Json::Value message;
...
ConversationModule::loadConversations()
{
                ...
                try {
                    ...
                    }
                    {
                        ...
                        if (convInfo == pimpl_->convInfos_.end()) {
                            JAMI_ERROR("[Account {}] Missing conv info for {}. This is a bug!",
                                       pimpl_->accountId_,
                                       repository);
    ...
    if (ec) {
        JAMI_ERROR("[Account {}] Failed to read conversations directory {}: {}",
                   pimpl_->accountId_,
                   conversationPath,
                   ec.message());
    }
