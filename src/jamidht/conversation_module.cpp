    ...
    if (!isConversation(convId)) {
        ...
        onNeedGitSocket_(convId, deviceId, [&](const auto& channel) {
            ...
            if (channel && !pending.ready) {
                ...
                lk.unlock();
                // Save the git socket
                acc->addGitSocket(deviceId, convId, channel); // TODO move from there
                                    ...
                                    const std::string& commitId)
{
    ...
    if (conversation != conversations_.end() && conversation->second) {
        ...
        onNeedGitSocket_(conversationId, deviceId, [&](const auto& channel) {
            {
                std::lock_guard<std::mutex> lk(pendingConversationsFetchMtx_);
                pendingConversationsFetch_.erase(conversationId);
            }
            auto conversation = conversations_.find(conversationId);
            if (!channel && conversation != conversations_.end() && conversation->second)
                return false;
...
ConversationModule::removeContact(const std::string& uri, bool ban)
{
    ...
    for (const auto& id : toRm)
        ban? removeRepository(id, false, true) : removeConversation(id);
}
...
bool
ConversationModule::onFileChannelRequest(const std::string& conversationId, const std::string& member,
...
ConversationModule::dataTransfer(const std::string& id) const
{
    ...
    if (conversation != conversations_.end() && conversation->second)
        return conversation->second->ConversationModule(id);
    return {};
}
