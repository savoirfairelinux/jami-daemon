...
ConversationModule::onConversationRequest(const std::string& from, const Json::Value& value)
{
    ...
    // Exception: a group conversation we previously left can be legitimately
    // re-invited (same convId reused).
    if (auto conv = pimpl_->getConversation(convId)) {
        std::lock_guard lk(conv->mtx);
        if (conv->info.mode == ConversationMode::ONE_TO_ONE || !conv->info.isRemoved())
            return;
    }
    req.received = nowMs();
    req.from = from;
