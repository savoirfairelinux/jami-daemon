...
ConversationModule::loadConversations()
{
    ...
    // fresh copy from another member device. The ctx->cv.wait above blocks until
    // PendingConvCounter has observed every per-conversation loader finish, so
    // all corrupted repositories have already been inserted into ctx->toReclone
    // by now. Take the set under convMtx (the same mutex the loaders used to
    // insert) to make that hand-off explicit.
    std::set<std::string> toReclone;
    {
        std::lock_guard lkMtx {ctx->convMtx};
        toReclone = std::move(ctx->toReclone);
    }
    if (!toReclone.empty()) {
        dht::ThreadPool::io().run([w = pimpl_->weak(), toReclone = std::move(toReclone)]() {
            auto shared = w.lock();
            if (!shared)
                return;
            for (const auto& convId : toReclone) {
                // Resolve the clone sources from the persisted members, which are
                // kept in convInfos_ independently of the corrupted git
                // repository, and re-clone from the known peers. This mirrors the
                // regular fallbackClone() path: cloneConversationFrom() shares a
                // single pending fetch per conversation, so trying several members
                // starts parallel connection attempts but only one clone actually
                // runs, and it retries with exponential backoff until a peer is
                // reachable.
                bool scheduled = false;
                for (const auto& member : shared->getConversationMembers(convId)) {
                    auto uriIt = member.find("uri");
                    if (uriIt == member.end() || uriIt->second.empty()) {
                        JAMI_WARNING("[Account {}] [Conversation {}] Ignoring malformed member metadata "
                                     "during recovery",
                                     shared->accountId_,
                                     convId);
                        continue;
                    }
                    if (uriIt->second != shared->username_) {
                        scheduled = true;
                        shared->cloneConversationFrom(convId, uriIt->second);
                    }
                }
                if (!scheduled)
                    JAMI_WARNING("[Account {}] [Conversation {}] No remote member available to recover the "
                                 "corrupted repository from",
                                 shared->accountId_,
                                 convId);
            }
        });
