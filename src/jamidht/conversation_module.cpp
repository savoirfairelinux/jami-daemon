...
ConversationModule::loadConversations()
{
    ...
    // The ctx->cv.wait above blocks until PendingConvCounter has observed every
    // per-conversation loader finish, so all corrupted repositories have already
    // been inserted into ctx->toReclone by now. Take the set under convMtx (the
    // same mutex the loaders used to insert) to make that hand-off explicit.
    std::set<std::string> toReclone;
    {
        std::lock_guard lkMtx {ctx->convMtx};
