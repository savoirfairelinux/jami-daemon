...
ConversationModule::Impl::cloneConversationFrom(const ConversationRequest& request)
{
    ...
    if (conv->info.created == 0) {
        ...
        // Re-invitation for a conversation that was removed locally: refresh
        // the request-derived metadata (in memory only) so the resurrected
        // conversation is not based on stale tombstone data, but keep the
        // tombstone fields (removed/erased) and their persisted state
        // untouched until the clone succeeds: clearing them now would leave
        // the conversation wrongly resurrected if the clone fails, and block
        // future invitations. handlePendingConversation() clears them on
        // clone success, sets the new created timestamp, and mode/members
        // are then also refreshed from the cloned repository, which is the
