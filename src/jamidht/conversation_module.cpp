...
ConversationModule::loadConversations()
{
    ...
    if (!toReclone.empty()) {
        dht::ThreadPool::io().run([w = pimpl_->weak(), toReclone = std::move(toReclone)]() {
            ...
            if (!shared)
                return;
            // Reuse the regular clone fallback: it resolves the source devices
            // from the conversation members and retries with exponential backoff
            // until a peer is reachable, and is simply a no-op if no other member
            // is known (so a missing source never spins in a tight loop).
