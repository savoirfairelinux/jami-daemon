...
ConversationModule::ConversationModule(
    ...
    SengMsgCb&& sendMsgCb,
    NeedsGitCb&& onNeedGitSocket)
    : pimpl_ {std::make_unique<Impl>(std::move(account), std::move(needsSyncingCb), std::move(sendMsgCb), std::move(onNeedGitSocket))}
{
    loadConversations();
}
...
ConversationModule::loadConversations()
{
    ...
    for (const auto& repository : conversationsRepositories) {
        try {
            ...
            JAMI_WARN("[Account %s] Conversations not loaded : %s", pimpl_->accountId_.c_str(), e.what());
        }
    }
    convInfos_ = convInfos(pimpl_->accountId_);
    conversationsRequests_ = convRequests(pimpl_->accountId_);
...
{
    JAMI_DBG("[Account %s] Clone conversation on device %s", pimpl_->accountId_.c_str(), deviceId.c_str());
    ...
    if (!pimpl_->isConversation(convId)) {
        {
            ...
            if (it == pimpl_->pendingConversationsFetch_.end()) {
                pimpl_->pendingConversationsFetch_[convId] = PendingConversationFetch {};
            }
        }
        pimpl_->onNeedGitSocket_(convId, deviceId, [=](const auto& channel) {
            auto acc = pimpl_->account_.lock();
            std::unique_lock<std::mutex> lk(pimpl_->pendingConversationsFetchMtx_);
            auto& pending = pimpl_->pendingConversationsFetch_[convId];
        ...
        JAMI_INFO("[Account %s] Already have conversation %s", pimpl_->accountId_.c_str(), convId.c_str());
    }
}
                                    ...
                                    const std::string& commitId)
{
    ...
    if (conversation != pimpl_->conversations_.end() && conversation->second) {
        ...
        if (lastMessageId.empty()) {
            JAMI_ERR("[Account %s] No message detected. This is a bug", pimpl_->accountId_.c_str());
            return;
        }
        ...
        pimpl_->onNeedGitSocket_(conversationId,
                         deviceId,
                         [this,
                          ...
                          commitId = std::move(commitId)](const auto& channel) {
                             {
                                 std::lock_guard<std::mutex> lk(pimpl_->pendingConversationsFetchMtx_);
                                 pimpl_->pendingConversationsFetch_.erase(conversationId);
                             }
                             auto conversation = pimpl_->conversations_.find(conversationId);
                             auto acc = pimpl_->account_.lock();
                             if (!channel || !acc || conversation == pimpl_->conversations_.end()
                                 || !conversation->second)
                                 return false;
            ...
            return;
        {
            ...
            std::lock_guard<std::mutex> lk(pimpl_->pendingConversationsFetchMtx_);
            if (pimpl_->pendingConversationsFetch_.find(conversationId) != pimpl_->pendingConversationsFetch_.end())
                return;
        }
...
ConversationModule::removeRepository(const std::string& conversationId, bool sync, bool force)
{
    std::unique_lock<std::mutex> lk(pimpl_->conversationsMtx_);
    auto it = pimpl_->conversations_.find(conversationId);
    if (it != pimpl_->conversations_.end() && it->second && (force || it->second->isRemoving())) {
        try {
        ...
        it->second->erase();
        pimpl_->conversations_.erase(it);
        lk.unlock();
...
ConversationModule::startConversation(ConversationMode mode, const std::string& otherMember)
{
    ...
    auto conversation = std::make_shared<Conversation>(pimpl_->account_, mode, otherMember);
...
{
    saveConvInfos(pimpl_->accountId_, convInfos_);
}
...
{
    saveConvRequests(pimpl_->accountId_, conversationsRequests_);
}
...
ConversationModule::acceptConversationRequest(const std::string& conversationId)
{
    ...
    acc->forEachDevice(
        ...
        [this, request = *request, conversationId](
            ...
            if (pk->getLongId().toString() == pimpl_->deviceId_)
                return;

            pimpl_->onNeedGitSocket_(conversationId, pk->getLongId().toString(), [=](const auto& channel) {
                auto acc = pimpl_->account_.lock();
                std::unique_lock<std::mutex> lk(pimpl_->pendingConversationsFetchMtx_);
                auto& pending = pimpl_->pendingConversationsFetch_[conversationId];
                if (channel && !pending.ready) {
                    pending.ready = true;
                    pending.deviceId = channel->deviceId().toString();
                    lk.unlock();
...
ConversationModule::onConversationRequest(const std::string& from, const Json::Value& value)
{
    ...
    emitSignal<DRing::ConversationSignal::ConversationRequestReceived>(pimpl_->accountId_, convId, reqMap);
}
...
ConversationModule::checkConversationsEvents()
{
    ...
    std::lock_guard<std::mutex> lk(pimpl_->pendingConversationsFetchMtx_);
    if (not pimpl_->pendingConversationsFetch_.empty() and not hasHandler) {
    ...
    } else if (pimpl_->pendingConversationsFetch_.empty() and hasHandler) {
...
{
    std::lock_guard<std::mutex> lk(pimpl_->pendingConversationsFetchMtx_);
    for (auto it = pimpl_->pendingConversationsFetch_.begin(); it != pimpl_->pendingConversationsFetch_.end();) {
        if (it->second.ready) {
            dht::ThreadPool::io().run([this, // TODO weak()? Verify lifetime
                ...
                try {
                    auto conversation = std::make_shared<Conversation>(pimpl_->account_,
                                                                       deviceId,
                                                                       conversationId);
                    if (!conversation->isMember(pimpl_->username_, true)) {
                    ...
                    if (conversation) {
                        ...
                        {
                            std::lock_guard<std::mutex> lk(pimpl_->conversationsMtx_);
                            pimpl_->conversations_.emplace(conversationId, std::move(conversation));
                        }
                        if (!commitId.empty())
                            pimpl_->sendMessageNotification(conversationId, commitId, false);
                        // Inform user that the conversation is ready
                        emitSignal<DRing::ConversationSignal::ConversationReady>(pimpl_->accountId_,
                                                                                 conversationId);
                        pimpl_->needsSyncingCb_();
                    }
                } catch (const std::exception& e) {
                    emitSignal<DRing::ConversationSignal::OnConversationError>(pimpl_->accountId_,
                                                                               conversationId,
                                                                               EFETCH,
                                                                               e.what());
            ...
            it = pimpl_->pendingConversationsFetch_.erase(it);
        } else {
            ++it;
        }
    }
    return !pimpl_->pendingConversationsFetch_.empty();
}
