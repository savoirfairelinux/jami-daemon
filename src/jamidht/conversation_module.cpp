...
ConversationModule::loadConversations()
{
    ...
    auto conversationsRepositories = dhtnet::fileutils::readDirectory(fileutils::get_data_dir() / pimpl_->accountId_
                                                                      / "conversations");
...
ConversationModule::hostConference(const std::string& conversationId,
                                   ...
                                   const std::vector<libjami::MediaMap>& mediaList)
{
    JAMI_LOG("Hosting conference {} in conversation {}", confId, conversationId);
