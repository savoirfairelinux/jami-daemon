    ...
    if (!toReclone.empty()) {
        dht::ThreadPool::io().run([w = pimpl_->weak(), toReclone = std::move(toReclone)]() {
            ...
            for (const auto& convId : toReclone) {
                ...
                // repository, and re-clone from one known peer.
                // cloneConversationFrom() resolves the member's devices and
                // retries with exponential backoff, falling back to the other
                // members on failure, so a single source is scheduled here to
                // avoid competing clones racing over the same staging path.
