...
ConversationModule::loadConversations()
{
    ...
    for (auto itInfo = pimpl_->convInfos_.begin(); itInfo != pimpl_->convInfos_.end();) {
        ...
        if (itConv == pimpl_->conversations_.end())
            pimpl_->conversations_[info.id] = std::make_shared<SyncedConversation>(info);
        else if (itConv != pimpl_->conversations_.end() && itConv->second && itConv->second->conversation && info.removed)
