...
ConversationChannelHandler::ConversationChannelHandler(std::weak_ptr<JamiAccount> acc, ConnectionManager& cm)
    : ChannelHandler()
    , account_(acc)
...
ConversationChannelHandler::onRequest(const DeviceId&, const std::string& name)
{
    ...
    if (auto acc = account_.lock())
        if (auto convModule = acc->convModule())
            return !convModule->isBannedDevice(conversationId, remoteDevice);
    return false;
}
