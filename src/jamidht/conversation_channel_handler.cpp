                                      ...
                                      const std::string& name)
{
    ...
    if (auto acc = account_.lock()) {
        if (auto convModule = acc->convModule(true)) {
            ...
            if (!res) {
                JAMI_ERROR("Received ConversationChannel request for '{}' but user {} is banned", name, cert->issuer->getId().toString());
            } else {
                res &= !convModule->isBanned(conversationId, cert->getLongId().toString());
                if (!res) {
                    JAMI_ERROR("Received ConversationChannel request for '{}' but device {} is banned", name, cert->getLongId().toString());
                }
            }
            return res;
        } else {
