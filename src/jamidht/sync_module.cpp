...
SyncModule::Impl::syncInfos(const std::shared_ptr<ChannelSocket>& socket)
{
    ...
    std::error_code ec;
    // Send contacts infos
    // This message can be big. TODO rewrite to only take UINT16_MAX bytes max or split it multiple messages.
    // For now, write 3 messages (UINT16_MAX*3 should be enough for all informations).
