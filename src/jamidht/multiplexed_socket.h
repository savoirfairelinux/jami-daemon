...
using OnConnectionRequestCb = std::function<bool(const dht::InfoHash& /* device id */,
                                                 const uint16_t& /* id */,
                                                 const std::string& /* name */)>;
using OnConnectionReadyCb = std::function<void(const dht::InfoHash& /* device id */,
...
{
public:
    MultiplexedSocket(const dht::InfoHash& deviceId, std::unique_ptr<TlsSocketEndpoint> endpoint);
    ~MultiplexedSocket();
