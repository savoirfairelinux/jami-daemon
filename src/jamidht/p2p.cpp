...
DhtPeerConnector::Impl::onTurnPeerConnection(const IpAddr& peer_addr)
{
    ...
    std::shared_ptr<dht::InfoHash> peer_h;
    tls_turn_ep_[peer_addr] =
    std::make_unique<TlsTurnEndpoint>(*turn_ep,
                                      account.identity(),
                                      account.dhParams(),
                                      [peer_h, this] (const dht::crypto::Certificate& cert) { return validatePeerCertificate(cert, *peer_h); });
    tls_turn_ep_[peer_addr]->setOnStateChange([this, peer_addr, peer_h] (tls::TlsSessionState state)
    {
        if (state == tls::TlsSessionState::SHUTDOWN) {
            ...
            connection->attachOutputStream(std::make_shared<FtpServer>(account.getAccountID(), peer_h.get()->toString()));
