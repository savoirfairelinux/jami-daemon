...
public:
    ...
    // TODO scope + mutexes
    std::map<std::pair<dht::InfoHash, DRing::DataTransferId>, std::shared_ptr<PeerConnection>> clientsChanneled_;
...
DhtPeerConnector::Impl::onRequestMsg(PeerConnectionMsg&& request)
{
    ...
    if (receivedTransfers_.find(request.tid) != receivedTransfers_.end()) {
        JAMI_WARN("Request already treated for tid %u (via channeled transport)", request.tid);
        return;
    }
    if (request.tid != 0)
    ...
    pimpl_->account.forEachDevice(
        ...
        [this, addresses, connect_cb, tid](const dht::InfoHash& dev_h) {
            ...
            pimpl_->account.connectionManager().connectDevice(dev_h.toString(), "file://" + std::to_string(tid),
                ...
                // TODO create PeerConnection, chck this or weak()
                ...
                pimpl_->clientsChanneled_.emplace(std::make_pair(dev_h, tid), std::move(connection));
