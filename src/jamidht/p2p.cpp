...
public:
    ...
    std::map<DRing::DataTransferId,
        std::map<std::string /* deviceId (multi-device) */, std::shared_ptr<ChanneledOutgoingTransfer>>> channeledOutgoing_;
...
DhtPeerConnector::Impl::cancelChanneled(const std::string& peerId, const DRing::DataTransferId& tid) {
    dht::ThreadPool::io().run([w=weak(), peerId, tid] {
        auto shared = w.lock();
        if (!shared) return;
        // Cancel outgoing files
        ...
        }
        {
            ...
            auto outgoingMap = shared->channeledOutgoing_.find(tid);
            if (outgoingMap != shared->channeledOutgoing_.end()) {
                outgoingMap->second.erase(peerId);
                if (outgoingMap->second.empty())
                    shared->channeledOutgoing_.erase(outgoingMap);
            }
        }
    ...
    acc->forEachDevice(
        ...
        [this, addresses, connect_cb, tid, channeledConnectedCb, onChanneledCancelled](const dht::InfoHash& dev_h) {
            ...
            acc->connectionManager().connectDevice(dev_h.toString(), "file://" + std::to_string(tid),
                [this, peerId=dev_h.toString(), tid, channeledConnectedCb, onChanneledCancelled, connect_cb](const std::shared_ptr<ChannelSocket>& channel) {
                ...
                auto outgoingFile = std::make_shared<ChanneledOutgoingTransfer>(channel);
                {
                    ...
                    pimpl_->channeledOutgoing_[tid][peerId] = outgoingFile;
                }

                channel->onShutdown([this, tid, peerId, onChanneledCancelled]() {
                    JAMI_INFO("Channel down for outgoing transfer with id(%lu)", tid);
                    onChanneledCancelled(peerId);
                    dht::ThreadPool::io().run([w=pimpl_->weak(), tid, peerId] {
                        auto shared = w.lock();
                        if (!shared) return;
                        // Cancel outgoing files
                        {
                        ...
                        Manager::instance().dataTransfers->close(tid); // TODO check close
                    });
                });
