...
public:
    ...
    // A single sync/import cannot jump the local Lamport clock close to overflow.
    static constexpr uint64_t MAX_REMOTE_CLOCK_ADVANCE = uint64_t {1} << 32;
    ...
    explicit AccountMetadataStore(std::filesystem::path path)
        : path_(std::move(path))
    {}

    AccountMetadata state() const;
    std::map<std::string, std::string> values() const;
    Change update(const std::string& writer,
