...
ConnectionManager::Impl::addNewMultiplexedSocket(const std::string& deviceId, const dht::Value::Id& vid, std::unique_ptr<TlsSocketEndpoint>&& tlsSocket)
{
    ...
    mSock->onShutdown([this, deviceId, vid]() {
        ...
        dht::ThreadPool::io().run([this, deviceId, vid] {

            // Delete the socket
            JAMI_WARN("SHUT BEFORE LOCK");
            std::lock_guard<std::mutex> lk(msocketsMutex_);
            JAMI_WARN("SHUT AFTER LOCK");
            
            if (multiplexedSockets_.find(deviceId) != multiplexedSockets_.end()) {
                if (multiplexedSockets_[deviceId].find(vid) != multiplexedSockets_[deviceId].end()) {
                    multiplexedSockets_[deviceId][vid]->shutdown();
                }
            }

            if (connectionsInfos_.find(deviceId) != connectionsInfos_.end()) {
                auto it = connectionsInfos_[deviceId].find(vid);
                if (it != connectionsInfos_[deviceId].end()) {
                    if (it->second.ice_) it->second.ice_->cancelOperations();
                    connectionsInfos_[deviceId].erase(vid);
                    if (connectionsInfos_[deviceId].empty()) {
                        connectionsInfos_.erase(deviceId);
                    }
                }
                // This will close the TLS Session
                std::lock_guard<std::mutex> lk (nonReadySocketsMutex_);
                if (nonReadySockets_[deviceId].find(vid) != nonReadySockets_[deviceId].end()) {
                    nonReadySockets_[deviceId].erase(vid);
                    if (nonReadySockets_[deviceId].empty()) {
                        nonReadySockets_.erase(deviceId);
                    }
                }
            }
            ...
            if (multiplexedSockets_.find(deviceId) != multiplexedSockets_.end()) {
                ...
                JAMI_WARN("EMPTY? %u", multiplexedSockets_[deviceId].empty());

                if (multiplexedSockets_[deviceId].empty())
                    multiplexedSockets_.erase(deviceId);
            }
            JAMI_WARN("SHUT AFTER LOCK END");
        });
    });
    if (multiplexedSockets_.find(deviceId) == multiplexedSockets_.end()) {
