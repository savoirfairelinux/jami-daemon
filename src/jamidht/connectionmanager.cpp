...
public:
    ...
    ~Impl() {
        ...
        JAMI_ERR("@@@ ~Impl END");
        for (auto& sockets: multiplexedSockets_) {
            for (auto& info: sockets.second) {
                if (info.second) info.second->shutdown();
                info.second.reset();
            }
        ...
        JAMI_ERR("@@@ ~Impl END 2");
        for (auto& certificate: certMap_) {
            certificate.second.first.reset();
        }
        JAMI_ERR("@@@ ~Impl END 3");
    }
...
{
    JAMI_WARN("@@@ ~ConnectionManager");
    pimpl_.reset();
    JAMI_WARN("@@@ ~ConnectionManager end");
}
