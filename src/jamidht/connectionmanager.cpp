...
public:
    ...
    std::map<std::string, MultiplexedSocket> multiplexedSockets_;
...
ConnectionManager::Impl::connectDevice(const std::string& deviceId, const std::string& uri, ConnectCallback cb)
{
    ...
    [this, deviceId, uri, cb=std::move(cb)] (const std::shared_ptr<dht::crypto::Certificate>& cert) {
        ...
        runOnMainThread([this, deviceId, uri, cert, cb=std::move(cb)] {
            ...
            cb(multiplexedSockets_.find(deviceId)->second.addChannel(uri));
        });
    });
...
ConnectionManager::Impl::onPeerRequest(const PeerConnectionRequest& req, const std::shared_ptr<dht::crypto::Certificate>& cert)
{
    ...
    if (connectionsInfos_[deviceId + "," + req.uri].ice_) {
        // TODO multiplexed object. The ICE infos changes if we are here (ie invite from DHT)
        connectionsInfos_.erase(deviceId + "," + req.uri);
    }
    auto& connectionInfo = connectionsInfos_[deviceId + "," + req.uri];
    auto& ice = connectionInfo.ice_;
    ...
    multiplexedSockets_.emplace(deviceId, MultiplexedSocket { deviceId, std::move(tlsSocket) });
    connReadyCb_(deviceId, req.uri, multiplexedSockets_.find(deviceId)->second.addChannel(req.uri));
}
