...
public:
     ...
     */
    void onTlsReady(bool ok, const DeviceId& deviceId, const dht::Value::Id& vid, const std::string& name = "");
    ...
    std::vector<PendingCb> getPendingCallbacks(const DeviceId& deviceId, const dht::Value::Id vid, bool erase = false)
    {
    ...
    info->tls_->setOnReady(
        [w = weak(), deviceId = std::move(deviceId), vid = std::move(vid), name = std::move(name)](
            ...
            if (auto shared = w.lock())
                shared->onTlsReady(ok, deviceId, vid, name);
    });
}
...
void
ConnectionManager::Impl::onTlsReady(bool ok, const DeviceId& deviceId, const dht::Value::Id& vid, const std::string& name)
{
    auto info = getInfo(deviceId, vid);
    if (!info)
        return;
    // Note: only handle pendingCallbacks here for TLS launched by connectDevice()
...
ConnectionManager::Impl::onRequestOnNegoDone(const PeerConnectionRequest& req)
{
    ...
    info->tls_->setOnReady(
        [w = weak(), deviceId = std::move(req.from), vid = std::move(req.id)](bool ok) {
            if (auto shared = w.lock())
                shared->onTlsReady(ok, deviceId, vid);
        });
}
    ...
    info->socket_->onShutdown([w = weak(), deviceId, vid]() {
        ...
        dht::ThreadPool::io().run([w, deviceId = dht::InfoHash(deviceId), vid] {
            ...
            for (const auto& cbId : ids)
                for (const auto& pending : sthis->getPendingCallbacks(cbId.first, cbId.second, true))
                    pending.cb(nullptr, deviceId);
