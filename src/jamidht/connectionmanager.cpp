    ...
    [this, deviceId, name, cb=std::move(cb)] (const std::shared_ptr<dht::crypto::Certificate>& cert) {
        ...
        dht::ThreadPool::io().run([this, deviceId, name, cert, cb=std::move(cb)] {
...

            {
                ...
                if (it != multiplexedSockets_.end() && !it->second.empty()) {
                    JAMI_DBG("Peer already connected. Add a new channel");
                    sendChannelRequest(it->second.rbegin()->second, name, deviceId, vid);
                    return;
                }
            }
            ...
            connectionInfo.ice_ = std::move(iceTransportFactory.createTransport(account.getAccountID().c_str(), 1, false, ice_config));
            ...
            if (ice->waitForInitialization(ICE_INIT_TIMEOUT) <= 0) {
                ...
                std::lock_guard<std::mutex> lk(connectCbsMtx_);
                if (!pendingCbs_.empty() && pendingCbs_.find(cbId) != pendingCbs_.end()) {
                    if (pendingCbs_[cbId]) pendingCbs_[cbId](nullptr);
            ...
            nonReadySockets_[deviceId][vid]->setOnReady([this, deviceId=std::move(deviceId), vid=std::move(vid), name=std::move(name)] (bool ok) {
                ...
                if (!ok) {
                    ...
                    if (!multiplexedSockets_.at(deviceId).empty())
                        sendChannelRequest(multiplexedSockets_.at(deviceId).rbegin()->second, name, deviceId, vid);
                }
            });
        });
    });
