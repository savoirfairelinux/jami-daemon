...
public:
    ...
    std::map<std::string, std::shared_ptr<PeerSocket>> sockets;
};
...
std::shared_ptr<PeerSocket>
MultiplexedSocket::addChannel(const std::string uri)
{
    pimpl_->sockets[uri] = std::make_shared<PeerSocket>(*this, uri);
...
PeerSocket::PeerSocket(MultiplexedSocket& endpoint, const std::string& uri)
...
std::string
PeerSocket::deviceId() const
{
    return pimpl_->endpoint.deviceId();
}

std::string
PeerSocket::uri() const
{
    return pimpl_->uri;
}

bool
PeerSocket::isReliable() const
{
    return pimpl_->endpoint.isReliable();
}

bool
PeerSocket::isInitiator() const
{
    return pimpl_->endpoint.isInitiator();
}

int
PeerSocket::maxPayload() const
{
    return pimpl_->endpoint.maxPayload();
}

std::size_t
PeerSocket::read(ValueType* buf, std::size_t len, std::error_code& ec)
{
    return pimpl_->endpoint.read(pimpl_->uri, buf, len, ec);
}

std::size_t
PeerSocket::write(const ValueType* buf, std::size_t len, std::error_code& ec)
{
    return pimpl_->endpoint.write(pimpl_->uri, buf, len, ec);
}

int
PeerSocket::waitForData(std::chrono::milliseconds timeout, std::error_code& ec) const
{
...
struct ConnectionInfo {
    ...
    std::unique_ptr<MultiplexedSocket> multiplexedSocket_ {nullptr};
};
...
ConnectionManager::Impl::connectDevice(const std::string& deviceId, const std::string& uri, ConnectCallback cb)
{
    ...
    [this, deviceId, uri, cb=std::move(cb)] (const std::shared_ptr<dht::crypto::Certificate>& cert) {
        ...
        connectionInfo.multiplexedSocket_ = std::make_unique<MultiplexedSocket>(deviceId, std::move(tlsSocket));
        // TODO NAME PEERSOCKET/CHANNELSOCKET
        cb(connectionInfo.multiplexedSocket_->addChannel(uri));
    });
...
ConnectionManager::Impl::onPeerRequest(const PeerConnectionRequest& req, const std::shared_ptr<dht::crypto::Certificate>& cert)
{
    ...
    connectionInfo.multiplexedSocket_ = std::make_unique<MultiplexedSocket>(req.from.toString(), std::move(tlsSocket));
    // TODO NAME PEERSOCKET/CHANNELSOCKET
    connReadyCb_(req.from.toString(), req.uri, connectionInfo.multiplexedSocket_->addChannel(req.uri));
}
