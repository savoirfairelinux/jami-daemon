...
// TODO avoid already tested connection
...
ConnectionManager::Impl::connectDevice(const std::string& deviceId, const std::string& uri /* OnConnect */)
{
    ...
    [this, deviceId, uri] (const std::shared_ptr<dht::crypto::Certificate>& cert) {
        if (!cert) {
        ...
        JAMI_WARN("@@@ TODO onConnect success");
    });
...
ConnectionManager::Impl::onPeerRequest(const PeerConnectionRequest& req, const std::shared_ptr<dht::crypto::Certificate>& cert)
{
    ...
    JAMI_WARN("@@@ TODO On connection success");
}
