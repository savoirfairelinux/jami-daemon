...
constexpr const char* const HTTP_URI {"http://"};
...
{
    // resolve once
    resolver_ = std::make_shared<http::Resolver>(httpContext_, serverHost_, HTTP_PROTO, logger_);
    // run http client
...
NameDirectory::setHeaderFields(std::shared_ptr<http::Request> request){
    request->set_header_field(restinio::http_field_t::host, serverHost_.c_str());
...
NameDirectory::lookupAddress(const std::string& addr, LookupCallback cb)
{
    ...
    try {
        ...
        const std::string uri = HTTP_URI + serverHost_ + QUERY_ADDR + addr;
        JAMI_DBG("Address lookup for %s: %s", addr.c_str(), uri.c_str());

        auto status_code = std::make_shared<std::atomic_size_t>();
...
NameDirectory::lookupName(const std::string& n, LookupCallback cb)
{
    ...
    try {
        ...
        const std::string uri = HTTP_URI + serverHost_ + QUERY_NAME + name;
