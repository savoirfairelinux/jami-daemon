...
{
    // build http client
    httpClient_ = std::make_unique<http::Client>(httpContext_, serverHost_/*, HTTPS_PROTO*/);
    // run http client
...
NameDirectory::lookupAddress(const std::string& addr, LookupCallback cb)
{
    ...
    httpClient_->async_connect([this, &addr, cb] (std::shared_ptr<http::Connection> conn){
        ...
        context->cb = [this, wcontext, cb, addr](const std::string& name, Response response){
        ...
        httpClient_->async_request(conn, request, std::move(parser), std::move(parser_s),
                                  [this, addr, context](const asio::error_code& ec){
            if (ec)
                JAMI_ERR("Adress lookup for %s failed: %s", addr.c_str(), ec.message().c_str());
        });
    });
}
...
NameDirectory::lookupName(const std::string& n, LookupCallback cb)
{
    ...
    httpClient_->async_connect([this, name, cb](std::shared_ptr<http::Connection> conn)
    {
        restinio::http_request_header_t header;
        ...
        context->name = name;
        std::weak_ptr<GetContext> wcontext = context;
        context->cb = [this, wcontext, cb](const std::string& addr, Response response){
            auto context = wcontext.lock();
        ...
        // keeping context data alive
        httpClient_->async_request(conn, request, std::move(parser), std::move(parser_s),
                                  [this, name, context](const asio::error_code& ec){
            if (ec)
                JAMI_ERR("Name lookup for %s failed: %s", name.c_str(), ec.message().c_str());
        });
    });
}
                                 ...
                                 const std::string& signedname, const std::string& publickey)
{
    ...
    httpClient_->async_connect([this, name, addr, body, cb](std::shared_ptr<http::Connection> conn)
    {
        restinio::http_request_header_t header;
        header.request_target(QUERY_NAME + name);
        ...
        context->addr = addr;
        context->name = name;
        std::weak_ptr<PostContext> wcontext = context;
        context->cb = [this, wcontext, cb](RegistrationResponse response){
            auto context = wcontext.lock();
        ...
        httpClient_->async_request(conn, request, std::move(parser), std::move(parser_s),
                                  [this, name, context](const asio::error_code& ec){
            if (ec)
                JAMI_ERR("Name register for %s failed: %s", name.c_str(), ec.message().c_str());
        });
    });
