...
validateValue(const std::string& key, const std::string& value)
{
    if (key.empty() || key.size() > AccountMetadataStore::MAX_KEY_BYTES
        || value.size() > AccountMetadataStore::MAX_VALUE_BYTES
        || !utf8_validate(key) || !utf8_validate(value)
...
AccountMetadataStore::update(const std::string& writer, const std::map<std::string, std::string>& updates,
                             bool onlyIfAbsent)
{
...
AccountMetadataStore::save(const AccountMetadata& next) const
{
    ...
    try {
#ifdef _WIN32
        auto handle = CreateFileW(staging.c_str(), GENERIC_WRITE, 0, nullptr, CREATE_ALWAYS,
