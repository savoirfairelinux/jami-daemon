...
CollaborativeEditing::replicaId()
{
    ...
    if (auto acc = account_.lock()) {
        ...
        if (!deviceId.empty()) {
            clientId_ = static_cast<uint64_t>(std::hash<std::string> {}(deviceId)) & ((uint64_t(1) << 53) - 1);
            if (clientId_ == 0)
                clientId_ = 1;
        }
    }
                                    ...
                                    bool allowRepoCreation)
{
    ...
    session->doc->setUpdateCallback([wthis, wsession](const YrsDocument::Bytes& update, bool isLocal) {
        if (!isLocal)
...
std::string
CollaborativeEditing::documentContentDelta(const std::string& conversationId, const std::string& documentId)
{
    auto session = ensureSession(conversationId, documentId);
                                      ...
                                      const std::string& jsonPayload)
{
    ...
    auto session = ensureSession(conversationId, documentId, isAnnouncedDocument(conversationId, documentId));
