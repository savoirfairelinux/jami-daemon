...
#define COLLAB_DBG(...) __android_log_print(ANDROID_LOG_ERROR, "JAMICOLLAB", __VA_ARGS__)
...
CollaborativeEditing::openDocument(const std::string& conversationId, const std::string& documentId)
{
    ...
    auto delta = session->doc->contentDelta();
    COLLAB_DBG("openDocument doc=%s textLen=%zu deltaLen=%zu delta=%.200s",
               documentId.c_str(), text.size(), delta.size(), delta.c_str());
    return text;
}
                                         ...
                                         const std::shared_ptr<Session>& session)
{
    ...
    COLLAB_DBG("loadPersistedState doc=%s applied=%d textLen=%zu",
               documentId.c_str(), applied, session->doc->text().size());
}
