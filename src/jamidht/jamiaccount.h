...
private:
    ...
    // peer too.
    std::map<std::string /* account id */,
        std::map<std::string /* device id */, std::vector<SipConnection>>
    > sipConnections_ {};
    // However, we only negotiate one socket from our side
    std::set<std::pair<std::string, std::string>> pendingSipConnections_ {};

    /**
     * Ask a device to open a channeled SIP socket
     * @param contact       The contact to ask
     * @param deviceId      The specific device to ask
     * @note triggers cacheSIPConnection
     */
    void askForSIPConnection(const std::string& contactUri, const std::string& deviceId);
    /**
     * Store a new SIP connection into sipConnections_
     * @param socket        The new sip channel
     * @param contactUri    Contact that owns the device
     * @param deviceId      Device linked to that transport
     */
    void cacheSIPConnection(std::shared_ptr<ChannelSocket>&& socket, const std::string& contactUri, const std::string& deviceId);
};
