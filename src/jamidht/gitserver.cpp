...
GitServer::Impl::sendPackData()
{
    ...
    while (!git_revwalk_next(&oid, walker_ptr)) {
        ...
        parents.erase(std::remove(parents.begin(), parents.end(), id));
        JAMI_ERR("@@@ parents.size() %u", parents.size());
        ...
        for (unsigned int p = 0; p < parentsCount; ++p) {
            ...
            if (pid) {
                ...
                JAMI_ERR("@@@ PARENT %s - %u", parent.c_str(), parents.size());
            }
        }
    }

    if (!parents.empty()) {
        JAMI_ERR("@@@@@@ NOT ALL PARENTS!");
    }
