...
readBlob(git_repository* repo, const git_tree_entry* entry)
{
    ...
    if (git_blob_lookup(&blob_ptr, repo, git_tree_entry_id(entry)) < 0)
        return {};
    std::unique_ptr<git_blob, void (*)(git_blob*)> blob {blob_ptr, [](git_blob* b) { git_blob_free(b); }};
...
readBlobAt(git_repository* repo, git_tree* tree, const std::string& path)
{
    ...
    if (git_tree_entry_bypath(&entry_ptr, tree, path.c_str()) < 0)
        return {};
    std::unique_ptr<git_tree_entry, void (*)(git_tree_entry*)> entry {entry_ptr, [](git_tree_entry* e) {
...
public:
    ...
    && (LIBGIT2_VER_REVISION == 0 || LIBGIT2_VER_REVISION == 1 || LIBGIT2_VER_REVISION == 3)
        auto** parentsPtr = const_cast<git_commit* const*>(parents.data());
#else
        auto** parentsPtr = const_cast<const git_commit**>(parents.data());
#endif
                         ...
                         const std::string& kind)
{
    ...
    if (path.empty() || std::filesystem::exists(path / "HEAD")) {
        JAMI_WARNING("[Account {}] [Document {}] Repository already exists",
                     account->getAccountID(),
                     documentId);
        return nullptr;
    }
    ...
    if (commitId.empty())
        return nullptr;
    JAMI_LOG("[Account {}] [Document {}] Repository created at {}",
             account->getAccountID(),
             documentId,
             self->path_);
    return self;
                       ...
                       const std::string& documentId)
{
    ...
    if (!std::filesystem::exists(path / "HEAD"))
        return nullptr;
    return std::shared_ptr<CollabRepository>(
        new CollabRepository(account, conversationId, documentId, path.string()));
}
...
CollabRepository::appendCheckpoint(const std::vector<std::string>& updates, const std::string& projection)
{
    ...
    return pimpl_->commit(repo.get(),
                          newRoot,
                          fmt::format("checkpoint: {} update(s)", updates.size()),
                          parents);
}
