...
struct ArchiveAccountManager::PeerLoadContext
{
    ...
    bool archiveTransferredWithoutFailure {false};
    std::string accData; // serialized account data
    // jsonData will replace accData?
    // std::string jsonData; // serialized account data
...
bool
ArchiveAccountManager::provideAccountAuthentication2(const std::string& credentialsFromUser, const std::string_view scheme)
{
    ...
    AuthMsg toSend;
    JAMI_DEBUG("[LinkDevice] NEW: PASSWORD IS '{}'.", credentialsFromUser);
    // toSend.payload["password"] = credentialsFromUser;
    toSend.payload["debug"] = "this should appear on OLD when recv passwd";
...
ArchiveAccountManager::startLoadArchiveFromDevice(const std::shared_ptr<AuthContext>& ctx)
{
    ...
    dht::ThreadPool::computation().run([this, ctx = std::move(ctx), wthis = weak_from_this()] {
        ...
        auto accountScheme = fmt::format("jami-auth://{}/{}",
            ctx->linkDevCtx->tmpId.second->getId(),
            ctx->linkDevCtx->opId);
        JAMI_INFO("[LinkDevice] auth scheme will be: {}", accountScheme);
                    ...
                    if (ctx->linkDevCtx->state == AuthDecodingState::HANDSHAKE) {
                        ...
                        if (ctx->linkDevCtx->authEnabled) {
                            ...
                            if (auto this_ = std::static_pointer_cast<ArchiveAccountManager>(wthis.lock())) {
                                // this_->authChannel_ = std::move(wsocket);//ctx->linkDevCtx->channel;//std::make_shared(socket);
...
ArchiveAccountManager::addDevice(std::string_view scheme, uint32_t token, const std::shared_ptr<dhtnet::ChannelSocket>& channel)
{
    ...
    // this was moved to onRecv cbk for clarity and conciseness
    // try {
