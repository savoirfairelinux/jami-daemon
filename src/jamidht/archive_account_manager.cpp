...
ArchiveAccountManager::addDevice(std::string_view scheme, uint32_t token, const std::shared_ptr<dhtnet::ChannelSocket>& channel)
{
    ...
    auto ctx = std::make_shared<AuthContext>();
    authCtx_ = ctx;
    JAMI_DEBUG("[LinkDevice] EXPORTING: Creating peerLoadCtx.");
    ...
    ctx->peerLoadCtx->authScheme = scheme;
    ctx->token = token;
    ...
    channel->onShutdown([ctx]() {
        JAMI_DEBUG("[LinkDevice] EXPORTING: Shutdown with state {}... xfer {}uccessful", ctx->peerLoadCtx->formattedAuthState(), ctx->peerLoadCtx->archiveTransferredWithoutFailure ? "s" : "uns");
        ...
        if (ctx->peerLoadCtx->archiveTransferredWithoutFailure) {
            ...
            emitSignal<libjami::ConfigurationSignal::AddDeviceStateChanged>(
                ctx->accountId, ctx->token, static_cast<uint8_t>(DeviceAuthState::DONE), std::map{std::pair("error"s, "success"s)});
        }
    });
    ...
    channel->setOnRecv([ctx,
        ...
        JAMI_DEBUG("[LinkDevice] EXPORTING: State is '{}'", ctx->peerLoadCtx->formattedAuthState());
        ...
        if (toRecv.schemeId == 0) {
            ...
            if (ctx->peerLoadCtx->state == AuthDecodingState::HANDSHAKE) {
                ...
                JAMI_DEBUG("[LinkDevice] EXPORTING: Packing first message for NEW and switching to state: {}", ctx->peerLoadCtx->formattedAuthState());
            ...
            // we expect to be receiving credentials in this state and we know the archive is encrypted
                ...
                JAMI_WARNING("[LinkDevice] EXPORTING: Exceptional auth state.");
            }
            // END logic

            if (shouldSendArchive) {
                JAMI_DEBUG("[LinkDevice] EXPORTING: Archive in message has encryption scheme '{}'", ctx->peerLoadCtx->authScheme);
                emitSignal<libjami::ConfigurationSignal::AddDeviceStateChanged>(
                        ctx->accountId,
                        ctx->token,
                        static_cast<uint8_t>(DeviceAuthState::IN_PROGRESS),
...
ArchiveAccountManager::cancelAddDevice(uint32_t token)
{
    if (auto ctx = authCtx_) {
        if (ctx->token == token && ctx->peerLoadCtx) {
            ctx->peerLoadCtx->state = AuthDecodingState::ERR;
