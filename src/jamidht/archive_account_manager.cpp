...
struct ArchiveAccountManager::LinkDeviceContext
{
    ...
    std::string credentialsFromUser {""}; // DANGEROUS?
    bool authEnabled {false};
    bool archiveTransferredWithoutFailure {false};

    std::string accData; // serialized account archive data
...
void
ArchiveAccountManager::provideCurrentAccountAuthentication(const std::string& credentialsFromUser, const std::string_view scheme)
{
    JAMI_DEBUG("[LinkDevice] ArhiveAccountManager::provideCurrentAccountAuthentication");
    ...
    dht::ThreadPool::io().run([credentialsFromUser, scheme, wthis = weak_from_this()] {
        AuthMsg toSend;
        toSend.payload["password"] = std::move(credentialsFromUser);
        msgpack::sbuffer buffer(UINT16_MAX);
        toSend.logMsg();
        msgpack::pack(buffer, toSend);
        std::error_code ec;
        if (auto this_ = std::static_pointer_cast<ArchiveAccountManager>(wthis.lock())) {
            try {
                JAMI_DEBUG("[LinkDevice] NEW: Sending password to old device.");
                if (this_->linkDeviceContext_->channel)
                    this_->linkDeviceContext_->channel->write(reinterpret_cast<const unsigned char*>(buffer.data()), buffer.size(), ec);
                else
                    JAMI_WARNING("[LinkDevice] NEW: Failed to write to TLS channel: {}", ec.message());
                // this_->linkDeviceContext_->channel->write(reinterpret_cast<const unsigned char*>(buffer.data()), buffer.size(), ec);
            } catch (std::exception e) {
                JAMI_WARNING("[LinkDevice] Failed to send password over auth ChannelSocket. Channel may be invalid.");
            }
        }
        else {
            // JAMI_WARNING("[LinkDevice] NEW: Channel for providing credentials is invalid.");
            JAMI_WARNING("[LinkDevice] Failed to acquire ArchiveAccountManager.");
        }
...
ArchiveAccountManager::startLoadArchiveFromDevice(const std::shared_ptr<AuthContext>& ctx)
{
    ...
    dht::ThreadPool::computation().run([this, ctx = std::move(ctx), wthis = weak_from_this()] {
        ...
        // Generate the identity for the temporary device.
        // auto tempDeviceIdentity = dht::crypto::generateIdentity("Jami Temporary device", user);
        // if (!tempDeviceIdentity.first || !tempDeviceIdentity.second) {
        //     throw std::runtime_error("[LinkDevice] Can't generate identity for the temporary device.");
        // }

        if (auto this_ = std::static_pointer_cast<ArchiveAccountManager>(wthis.lock())) {
            // this_->linkDeviceContext_ = LinkDeviceContext(tempDeviceIdentity);
            this_->linkDeviceContext_ = std::make_shared<LinkDeviceContext>(dht::crypto::generateIdentity("Jami Temporary device", user));
            // ctx->linkDevCtx = std::make_shared<LinkDeviceContext>(this_->linkDeviceContext_);
            ctx->linkDevCtx = this_->linkDeviceContext_;  // Assuming ctx->linkDevCtx is also a shared_ptr
            JAMI_LOG("[LinkDevice] Established channel lifetime.");
        } else {
            JAMI_WARNING("[LinkDevice] Failed to create the temporary device.");
        }
        // auto user = dht::crypto::generateIdentity("Jami Temporary User", ca, 4096, true);
        // if (!user.first || !user.second) {
        //     throw std::runtime_error("[LinkDevice] Can't generate identity for this account.");
        // }
        // // ctx->linkDevCtx = std::make_shared<LinkDeviceContext>(dht::crypto::generateIdentity("Jami Temporary device", user));
        // // ctx->linkDevCtx = std::make_unique<LinkDeviceContext>(dht::crypto::generateIdentity("Jami Temporary device", user));
        // if (auto this_ = std::static_pointer_cast<ArchiveAccountManager>(wthis.lock())) {
        //     this_->linkDeviceContext_ = std::make_shared<LinkDeviceContext>(dht::crypto::generateIdentity("Jami Temporary device", user));
        //     // ctx->linkDevCtx = std::make_shared<LinkDeviceContext>(*linkDeviceContext_);
        //     ctx->linkD>linkDevCtx = this_->linkDeviceContext_;
        // }
        // else {
        //     JAMI_WARNING("[LinkDevice] Failed to create the temporary device.");
        // }
            ...
            ctx->dhtContext->dht.put(
                ...
                [=](bool ok) {
                    emitSignal<libjami::ConfigurationSignal::DeviceAuthStateChanged>(
                        ctx->accountId,
                        // KESS TODO change this to proper DeviceAuthState status code
                        // static_cast<uint8_t>(AuthDecodingState::HANDSHAKE),
                        static_cast<uint8_t>(DeviceAuthState::TOKEN_AVAIL),
                        accountScheme);
                },
                {},
                true);
        ...
        // TODO discuss making this configurable
        ...
        ctx->linkDevCtx->tempConnMgr.onConnectionReady([ctx, accountScheme, wthis](
            ...
            socket->onShutdown([ctx, deviceId, name]() {
                ...
                ctx->linkDevCtx->numOpenChannels--;
                // TODO handle success failure here with signals
                // check linkDevCtx->state and use that
            });
            ...
            socket->setOnRecv([/*wsocket = std::weak_ptr<dhtnet::ChannelSocketInterface>(socket),*/ ctx, decodingCtx = std::make_shared<DecodingContext>(), wthis](const uint8_t* buf, size_t len) {
                ...
                JAMI_DEBUG("[LinkDevice] NEW: Successfully unpacked message from OLD\n{}", toRecv.formatMsg());
                JAMI_DEBUG("[LinkDevice] NEW: State is {}:{}", ctx->linkDevCtx->scheme, ctx->linkDevCtx->formattedAuthState());

                // TODO remove this comment: begin new rewrite
                ...
                if (toRecv.schemeId == 0) {
                    ...
                    auto accDataItr = toRecv.payload.find("accData");
                    bool shouldLoadArchive = accDataItr != toRecv.payload.end();

                    if (ctx->linkDevCtx->state == AuthDecodingState::HANDSHAKE) {
                        // since we received a zero we know we are in protocol v0... can get auth scheme here as well
                        // if the scheme is none we will need to load the account
                        ctx->linkDevCtx->authEnabled = toRecv.payload["authScheme"] != fileutils::ARCHIVE_AUTH_SCHEME_NONE;
                        JAMI_DEBUG("[LinkDevice] NEW: Auth scheme from payload is '{}'", toRecv.payload["authScheme"]);
                        ...
                        if (ctx->linkDevCtx->authEnabled) {
                            // expose channel for entering password
                            JAMI_DEBUG("[LinkDevice] Exposing channel socket interface.");
                            if (auto this_ = std::static_pointer_cast<ArchiveAccountManager>(wthis.lock())) {
                                JAMI_DEBUG("[LinkDevice] Successfully exposed channel socket interface.");
                                // KESS two extra signals
                            ...
                            // KESS this should fix the auth page not loadnig
                            emitSignal<libjami::ConfigurationSignal::DeviceAuthStateChanged>(
                                ctx->linkDevCtx->tmpId.second->getId().toString(),
                                static_cast<int>(DeviceAuthState::AUTH),
                                "credentials_required");
                        }
                    }
                    else if (ctx->linkDevCtx->state == AuthDecodingState::DATA) {
                        // message may have archive... check for it, extract it, and do Jami loading things
                        if (!shouldLoadArchive) {
                            // TODO
                            // emit signal invalid credentials provided
                            emitSignal<libjami::ConfigurationSignal::AddDeviceStateChanged>(
                                ctx->accountId,
                                ctx->token,
                                static_cast<uint8_t>(DeviceAuthState::AUTH),
                            ...
                            // or fail if tried too many times (handled in shutdown of channel by OLD)
                        }
                    }
                    else {}

                    // check if an account archive is ready to be sent
                    if (shouldLoadArchive) {
                        try {
                            auto archive = AccountArchive(
                                std::string_view(accDataItr->second)
                            );
                        ...
                        // TODO check if should move shouldShutDown = true to here
                    }

                    // END logic
                    // handle the cases where the protocol should respond to the OLD device
                    if (shouldSendMsg) {
                        JAMI_DEBUG("[LinkDevice] Sending msg from NEW to OLD\t{}", toSend.formatMsg());
                        msgpack::sbuffer buffer(UINT16_MAX);
                        msgpack::pack(buffer, toSend);
                        std::error_code ec;
                        if (ctx->linkDevCtx->channel)
                ...
                // catch unsupported operations
                    ...
                    JAMI_WARNING("[LinkDevice] NEW: Unsupported linkdev scheme id of {}", toRecv.schemeId);
                    ctx->linkDevCtx->state = AuthDecodingState::ERR;
                    return (size_t) 0;
                }
                // TODO remove this comment: END new rewrite
            ...
            // TODO KESS this shouldn't be needed anymore since the dht callback will should be the one to notify
            // emitSignal<libjami::ConfigurationSignal::DeviceAuthStateChanged>(
            //     ctx->linkDevCtx->tmpId.second->getId().toString(),
...
// linkdeviec: oldDev: configure communication protocol callbacks once auth_channel_handler::onReady is
// called and ChannelSocket is provided
// TODO add opId
void
ArchiveAccountManager::addDevice(std::string_view scheme, uint32_t token, const std::shared_ptr<dhtnet::ChannelSocket>& channel)
{
    JAMI_DEBUG("[LinkDevice] Setting up addDevice logic on OLD device.");
    // this is where old device context is initiated
    this->authContext_ = std::make_unique<AuthContext>();
    auto ctx = this->authContext_.get();
    JAMI_DEBUG("[LinkDevice] creating peerLoadCtx.");
    ctx->peerLoadCtx = std::make_unique<PeerLoadContext>(channel);
    ctx->accountId = accountId_;
    ctx->peerLoadCtx->authScheme = std::move(scheme);
    ctx->token = token;
    ctx->peerLoadCtx->state = AuthDecodingState::HANDSHAKE;
    dht::ThreadPool::computation().run([ctx, channel, wthis = weak_from_this()] {
        if (true) {//auto this_ = std::static_pointer_cast<ArchiveAccountManager>(wthis.lock())) {
            channel->onShutdown([ctx]() {
                JAMI_DEBUG("[LinkDevice] OLD: Shutdown with state {}... xfer {}uccessful", ctx->peerLoadCtx->formattedAuthState(), ctx->peerLoadCtx->archiveTransferredWithoutFailure ? "s" : "uns");
            ...
            channel->setOnRecv([ctx,
              ...
              decodeCtx = std::make_shared<ArchiveAccountManager::DecodingContext>()](
                const uint8_t* buf, size_t len) {
                  JAMI_DEBUG("[LinkDevice] Setting up receiver callback for communication logic on OLD device.");
                  ...
                  try {
                    ...
                    JAMI_DEBUG("[LinkDevice] OLD: Successfdully unpacked message from NEW (NEW->OLD)\n{}", toRecv.formatMsg());
                  } catch (std::exception e) {
                  ...
                  JAMI_DEBUG("[LinkDevice] OLD: State is '{}'", ctx->peerLoadCtx->formattedAuthState());
                  ...
                  if (toRecv.schemeId == 0) {
                    ...
                    // JAMI_WARNING("[LinkDevice] KESS testing early shutdown behavior on OLD device.")
                    // so the channel is shutting down but still able to send messages down the pipe... this needs a check to make sure that messages aren't shown in the logs as sent if the channel is already closed
                    // ctx->peerLoadCtx->channel->shutdown();

                    // logic
                    // state typically reflects what data we expect to receive from the other end, as well as what data we will be sending
                    // for example, we expect to receive a msg containing handshaking information in the first if statement
                    if (ctx->peerLoadCtx->state == AuthDecodingState::HANDSHAKE) {
                      // since we know scheme is v0 in received message, we know that we can agree on protocol v0
                      ctx->peerLoadCtx->state = ctx->peerLoadCtx->authScheme == fileutils::ARCHIVE_AUTH_SCHEME_NONE ? AuthDecodingState::DATA : AuthDecodingState::AUTH;
                      // if scheme is none we can send account data now TODO
                      // otherwise we need to enter AUTH
                      if (ctx->peerLoadCtx->state == AuthDecodingState::DATA) {
                        ctx->peerLoadCtx->accData = fileutils::readArchive(fileutils::getFullPath(this_->path_, "archive.gz"), fileutils::ARCHIVE_AUTH_SCHEME_NONE, "").data;
                        shouldSendArchive = true; // this bool is for sending acrhive later
                        // send archive and shutdown
                      } // else do nothing and wait for passwords in state AUTH
                      JAMI_DEBUG("[LinkDevice] OLD: Packing first message for NEW and switching to state: {}", ctx->peerLoadCtx->formattedAuthState());
                      toSend.payload["authScheme"] = ctx->peerLoadCtx->authScheme;
                      shouldSendMsg = true;
                    }
                    // we expect to be in an error state on our end
                    ...
                    // we expect to be receiving credentials in this state and we know the archive is encrypted
                      ...
                      JAMI_DEBUG("[LinkDevice] OLD: {} password received for authentication.", passValid ? "Valid" : "Invalid");
                      if (passValid) {
                        ...
                        try {
                          ...
                          if (ctx->peerLoadCtx->numTries < ctx->peerLoadCtx->maxTries) {
                            ...
                            if (shouldSendArchive) {
                              JAMI_DEBUG("[LinkDevice] OLD: Archive in message has encryption scheme '{}'", ctx->peerLoadCtx->authScheme);
                              shouldShutDown = true;
                              shouldSendMsg = true;
                              ctx->peerLoadCtx->archiveTransferredWithoutFailure = true;
                              try {
                                JAMI_DEBUG("[LinkDevice] OLD: Trying to send archive to NEW.");
                                toSend.payload["accData"] = ctx->peerLoadCtx->accData;
                                JAMI_DEBUG("[LinkDevice] OLD: Sending archive to NEW.");
                              } catch (const std::exception& e) {
                                JAMI_ERROR("[LinkDevice] OLD: Error preparing archive for xfer: {}", e.what());
                                ctx->peerLoadCtx->archiveTransferredWithoutFailure = false;
                                toSend.payload.clear();
                                toSend.payload["archiveTransferredWithoutFailure"] = "false";
                              }
                            }
                            if (shouldSendMsg) {
                              JAMI_DEBUG("[LinkDevice] OLD: Sending msg to NEW:\n{}", toSend.formatMsg());
                              msgpack::sbuffer buffer(UINT16_MAX);
                              msgpack::pack(buffer, toSend);
                              std::error_code ec;
                              if (ctx->peerLoadCtx->channel)
                              ctx->peerLoadCtx->channel->write(reinterpret_cast<const unsigned char*>(buffer.data()), buffer.size(), ec);
                              else
                              JAMI_WARNING("[LinkDevice] OLD: Failed to write to TLS channel: {}", ec.message());
                            }
            ...
            throw std::runtime_error("[LinkDevice] Error while creating old device context.");
        }
...
ArchiveAccountManager::onArchiveLoaded(AuthContext& ctx, AccountArchive&& a, bool isLinkDevProtocol)
{
    ...
    ctx.onSuccess(*info_,
                  ...
                  // TODO add log on successful load for linkdeviceprotocol
}
