...
ArchiveAccountManager::addDevice(std::string_view scheme, uint32_t token, const std::shared_ptr<dhtnet::ChannelSocket>& channel)
{
    ...
    // KESS TODO this may be unecessary duplicate work
            ...
            // we expect to be receiving credentials in this state and we know the archive is encrypted
                ...
                if (passValid) {
                    ...
                    try {
                        ...
                        // ctx->peerLoadCtx->accData = fileutils::readArchive(fileutils::getFullPath(this_->path_, "archive.gz"), fileutils::ARCHIVE_AUTH_SCHEME_PASSWORD, toRecv.payload["password"]).data;

                        // ctx->peerLoadCtx->accData = fileutils::readArchive(
                        //     fileutils::getFullPath(this_->path_, "archive.gz"),
