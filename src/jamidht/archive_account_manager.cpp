...
ArchiveAccountManager::provideCurrentAccountAuthentication(const std::string& credentialsFromUser, const std::string_view scheme)
{
    ...
    dht::ThreadPool::io().run([credentialsFromUser, scheme, wthis = weak_from_this()] {
        ...
        if (auto this_ = std::static_pointer_cast<ArchiveAccountManager>(wthis.lock())) {
            try {
                JAMI_DEBUG("[LinkDevice] NEW: Sending password to old device. {} {} {}", fmt::ptr(this_), fmt::ptr(this_->authContext_), fmt::ptr(this_->authContext_ ? this_->authContext_->linkDevCtx : nullptr));
                if (!this_->authContext_->linkDevCtx)
                    JAMI_WARNING("[LinkDevice] NEW: linkDeviceContext_ is null");
                if (this_->authContext_->linkDevCtx->channel)
                    this_->authContext_->linkDevCtx->channel->write(reinterpret_cast<const unsigned char*>(buffer.data()), buffer.size(), ec);
                else
...
{
    // move the account creation to another thread because crypto operations will be attempted
    dht::ThreadPool::computation().run([this, ctx = std::move(ctx), wthis = weak_from_this()] {
        ...
        JAMI_LOG("[LinkDevice] Establishing linkDevCtx.");
        if (auto this_ = std::static_pointer_cast<ArchiveAccountManager>(wthis.lock())) {
            ctx->linkDevCtx = std::make_shared<LinkDeviceContext>(dht::crypto::generateIdentity("Jami Temporary device", user));
            JAMI_LOG("[LinkDevice] Establishing linkDevCtx... {} {} {}", fmt::ptr(this_), fmt::ptr(ctx), fmt::ptr(ctx->linkDevCtx));
        ...
        ctx->linkDevCtx->tempConnMgr.onConnectionReady([ctx = std::move(ctx), accountScheme, wthis](
            ...
            socket->setOnRecv([ctx, decodingCtx = std::make_shared<DecodingContext>(), wthis](const uint8_t* buf, size_t len) {
                if (!buf) {
                    return len;
                }
                ...
                if (toRecv.schemeId == 0) {
                    ...
                    if (ctx->linkDevCtx->state == AuthDecodingState::HANDSHAKE) {
                        ...
                        if (ctx->linkDevCtx->authEnabled) {
                            ...
                            emitSignal<libjami::ConfigurationSignal::DeviceAuthStateChanged>(ctx->linkDevCtx->tmpId.second->getId().toString(), static_cast<int>(DeviceAuthState::AUTH), "archive_with_auth");
                        }
                    }
