...
ArchiveAccountManager::provideAccountAuthentication(const std::string& credentialsFromUser, const std::string_view scheme)
{
    ...
    bool retVal = false;
    try {
        if (auto channel = authCtx_->linkDevCtx->channel)
...
{
    this->authCtx_ = ctx; // make a shared_ptr reference to this object
                    ...
                    if (ctx->linkDevCtx->state == AuthDecodingState::HANDSHAKE) {
                        ...
                        if (ctx->linkDevCtx->authEnabled) {
                            ...
                            if (auto this_ = std::static_pointer_cast<ArchiveAccountManager>(wthis.lock())) {
                                JAMI_DEBUG("[LinkDevice] NO LONGER NEEDED Successfully exposed channel socket interface.");
                            }
