...
ArchiveAccountManager::startLoadArchiveFromDevice(const std::shared_ptr<AuthContext>& ctx)
{
    ...
    dht::ThreadPool::computation().run([ctx, wthis = weak_from_this()] {
        ...
        auto device = std::make_unique<LinkDeviceContext>(dht::crypto::generateIdentity("Jami Temporary device", user));

        JAMI_LOG("[LinkDevice] Established linkDevCtx.");
        ctx->linkDevCtx = device;
        ...
        ctx->linkDevCtx->tempConnMgr.onConnectionReady([ctx, accountScheme, wthis](
            ...
            socket->setOnRecv([socket, ctx, decodingCtx = std::make_shared<DecodingContext>(), wthis](const uint8_t* buf, size_t len) {
                ...
                if (toRecv.schemeId == 0) {
                    ...
                    if (shouldLoadArchive) {
                        try {
                            auto archive = AccountArchive(accDataItr->second);
