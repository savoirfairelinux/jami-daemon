...
// TODO make this reflect v2 state machine
//  will require changing the SCHEME to ESTABLISHED and adding both devices states to the enum
...
enum AuthDecodingState {SCHEME=0, ATTEMPT, ARCHIVE, GENERIC_ERROR, AUTH_ERROR, ARCHIVE_RECEIVED};

// unused
static const std::map<std::string, AuthDecodingStateOpId> opIdMap = {
    {"SCHEME", SCHEME},
    {"ATTEMPT", ATTEMPT},
    {"ARCHIVE", ARCHIVE},
...
struct AuthDecodingState
{
    ...
    AuthDecodingStateOpId newDeviceState {SCHEME}; // 0: scheme, 1: attempt loop, 2+: close
    AuthDecodingStateOpId oldDeviceState {SCHEME}; // 0: scheme, 1: attempt loop, 2+: close
...
struct AuthMsg {
    ...
    // std::map<std::string, std::map<std::string, std::string>> payload;
    AccountArchive archive;
    MSGPACK_DEFINE_MAP(scheme, payload, archive)
};

// TODO REMOVE
        // // UNUSED
        // // communicates which of the three channels should be used to parse the next incoming message
        // struct LinkChannelIdMessage // current version (1.0)
        // {
        ...
        // {
        //     std::string protocolVersion; // for versioning
        //     bool passwordEnabled {false};
        //     std::string password;
        //     MSGPACK_DEFINE_MAP(protocolVersion, passwordEnabled, password)
        // };
        //
...
bool compareAsFloats(const std::string& a, const std::string& b) {
...
void
ArchiveAccountManager::onAuthRecv(std::shared_ptr<LinkDeviceContext> ctx/*const::std string& deviceId*/, const uint8_t* buf, size_t len) {
    // ctx = std::make_shared<AuthContext>();

    if (!buf) { return len; }

    if (ctx->newDeviceState == AuthDecodingState::GENERIC_ERROR) { return len; }
    ...
    try {
        ...
        ctx->newDeviceState = AuthDecodingState::GENERIC_ERROR; // set the generic error state in the context
        JAMI_WARN("[Link Device] error unpacking message from msgpack"); // also warn in logs
    }

    // establish a protocol version
    // - this switch will let protocols mixin new features such as new extra states while keeping the base protocol viable
    std::vector<std::string> supportedProtocols;
    std::transform(
        msg.payload["supportedSchemes"].begin(),
        msg.payload["supportedSchemes"].end(),
        std::back_inserter(supportedProtocols),
        [](const auto& entry) { return entry.first; }
    );

    // - TODO be able to make a union set of protocols from both ends of connection, then sort that to pick highest number protocol version

    // - Sort the list numerically
    std::sort(supportedProtocolsFloat.begin(), supportedProtocolsFloat.end(), compareAsFloats);

    ctx->linkDevCtx->highestScheme = supportedProtocols.back();

    // protocol logic goes here
    switch ctx->linkDevCtx->highestScheme {
        case: "1.0":
            switch msg.payload["name"]:
                default:
                    // do nothing
                case "established":
                    // agree on version to use
                    break;
                case "schemeKnown":
                    if msg.payload["hasPassword"]
                    break;
                case "requestTransmitted":
                    const std::map<std::string, std::string>& obj = msg.payload["passwordScheme"]; // this is equal to the data that needs to be processed
                    ctx->linkDevCtx->passwordEnabled = obj["passwordEnabled"];
                    if (ctx->linkDevCtx->passwordEnabled) {
                        ctx->emitSignal<libjami::ConfigurationSignaol::AddDeviceStateChanged(ctx->accountId, 1, "requestStringPassword", [socket = std::make_shared<ChannelSocket>(socket)](const std::string& passwordString) {
                            AuthMsg toSend;
                            toSend.payload["scheme"] = msg.payload["scheme"];
                            toSend.payload["password"] = std::move(passwordString);
                            toSend.payload["name"] = "requestTransmitted";
                            socket->write(toSend);
                        }); // prompt user to enter the respective type of passowrd, for now just a string but in other versions maybe biometric hash or something fancier
                    }
                    break;
                case "archiveReceived":
                	// auto archive = msg.archive;
                	ctx->linkDevCtx->transferredAccArchive = std::move(msg.archive);
                	ctx->emitSignal<libjami::ConfigurationSignal::AddDeviceStateChanged(ctx->accountId, 0, "archiveLoaded"); // TODO use some string from JAMI_WARN, etc., that is standardized across the project

                    // TODO saveArchive into accountManager?

                    break;
                case "error":
                    ctx->genericError = true;
                    JAMI_WARN("[Link Device] generic error in link protocol version " + msg.payload["scheme"]);
                    break;
                case "authError":
                    ctx->authError = true;
                    break;
    ...
    // if (ctx->linkDevCtx->authDecodingState->newDeviceState == AuthDecodingState::SCHEME) {
	// 	PassowrdSchemeMsg passwordSchemeMsg;
    // 	oh.get().convert(passwordSchemeMsg);
    //     if (ctx->versionMsg == "") {
    //         // scheme is not set yet so this should be set
    //         if (!passwordSchemeMsg.versionMsg.empty()) { ctx->versionMsg = passwordSchemeMsg.versionMsg; }
    //         else { ctx->newDeviceState = AuthDecodingState::GENERIC_ERROR; }
    //     } else {
    //         if (ctx->versionMsg != "1.0") {
    //             JAMI_WARN("[Link Device] unimplemented protocol verssion... exiting");
	...
	// 	// do nothing
    // } */
    // else if (ctx->linkDevCtx->authDecodingState->newDeviceState == AuthDecodingState::ARCHIVE) {
    //     if (ctx->versionMsg != "1.0") {
    //         JAMI_WARN("[Link Device] unimplemented protocol verssion... exiting");
    //         // throw a generic error here
    //         ctx->newDeviceState = AuthDecodingState::GENERIC_ERROR; // set the generic error state in the context
    ...
    //         ctx->newDeviceState = AuthDecodingState::AUTH_ERROR; // set the generic error state in the context
    ...
    // }
    // else {
    //     if (ctx->newDeviceState != AuthDecodingState::GENERIC_ERROR && ctx->newDeviceState != AuthDecodingState::AUTH_ERROR) {
    //         JAMI_WARN("[Link Device] channel operation mode is unspecified (unspecified auth protocol)");
    //         ctx->newDeviceState = AuthDecodingState::GENERIC_ERROR; // set the generic error state in the context
    //     } else {
    //         JAMI_WARN("[Link Device] error");
    //     }
    // }
}

void
ArchiveAccountManager::startLoadArchiveFromDevice(const std::shared_ptr<text>& ctx)
{
    ...
    dht::ThreadPool::computation().run([password, cb = std::move(cb), w=weak_from_this()] {
        ...
        const std::string accountScheme = authUri + ctx->linkDevCtx->tmpId.second->getId();
...
void
ArchiveAccountManager::addDevice(const std::string& uri, std::shared_ptr<dhtnet::ChannelSocket> socket, AddDeviceCallback cb)
{
    dht::ThreadPool::computation().run([password, cb = std::move(cb), w=weak_from_this()] {
        auto this_ = std::static_pointer_cast<ArchiveAccountManager>(w.lock());
        if (not this_) return;
	    ...
	    if (!passwordSchemeMsg.password.empty()) {
	        auto pw = passwordSchemeMsg.password;
		        ...
		        JAMI_WARN("[Link Device] password flag is empty!");
          	    ...
          	    if (ctx->linkDevCtx->authDecodingState->oldDeviceState == AuthDecodingState::SCHEME) {
                    // do nothing
                    JAMI_WARN("[Link Device] this scheme should not be used on this device in the Link Device mode");
          	    } else if (ctx->linkDevCtx->authDecodingState->oldDeviceState == AuthDecodingState::ATTEMPT) {
          	    ...
          	    else if (ctx->linkDevCtx->authDecodingState->oldDeviceState == AuthDecodingState::ARCHIVE) {
                    // do nothing
                    JAMI_WARN("[Link Device] this scheme should not be used on this device in the Link Device mode");
          	    }
          	    else if (ctx->linkDevCtx->authDecodingState->oldDeviceState == AuthDecodingState::ASK) {
...
// get the uri and use to create a tls conncection using dhtnet

        // set the callback for the channel being ready
