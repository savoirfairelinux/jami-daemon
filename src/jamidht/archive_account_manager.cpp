...
struct ArchiveAccountManager::LinkDeviceContext
{
    ...
    constexpr std::string_view formattedAuthState()
    {
        switch (state) {
...
/** Holds state for EXPORTING device add logic */
struct ArchiveAccountManager::AddDeviceContext
{
    uint64_t opId;
...
bool
ArchiveAccountManager::provideAccountAuthentication(const std::string& credentialsFromUser, const std::string& scheme)
{
    ...
    dht::ThreadPool::io().run([credentialsFromUser, scheme, ctx] {
        AuthMsg toSend;
...
ArchiveAccountManager::startLoadArchiveFromDevice(const std::shared_ptr<AuthContext>& ctx)
{
    ...
    dht::ThreadPool::computation().run([ctx, wthis = weak_from_this()] {
        ...
        JAMI_LOG("[LinkDevice] Established linkDevCtx. {} {} {}.", fmt::ptr(this_), fmt::ptr(ctx), fmt::ptr(ctx->linkDevCtx));
        ...
        emitSignal<libjami::ConfigurationSignal::DeviceAuthStateChanged>(
            ctx->accountId,
            static_cast<uint8_t>(DeviceAuthState::TOKEN_AVAILABLE),
            std::map{std::pair("token"s, accountScheme)});

        ctx->linkDevCtx->tempConnMgr.onICERequest([
            wctx = std::weak_ptr(ctx)
        ...
        ctx->linkDevCtx->tempConnMgr.onConnectionReady([ctx, accountScheme, wthis](
            ...
            ctx->timeout->expires_from_now(1min);
            ctx->timeout->async_wait([c=std::weak_ptr(ctx), socket](const std::error_code& ec) {
                if (ec) {
                    return;
                }
                if (auto ctx = c.lock()) {
                    if (ctx->linkDevCtx->state != AuthDecodingState::DONE) {
            ...
            socket->onShutdown([ctx, deviceId, name]() {
                ...
                emitSignal<libjami::ConfigurationSignal::DeviceAuthStateChanged>(
                    ...
                    std::map<std::string, std::string>{
                        {"error", ctx->linkDevCtx->archiveTransferredWithoutFailure ? "" : "network"}
                    });
            });
            ...
            socket->setOnRecv([ctx, decodingCtx = std::make_shared<DecodingContext>(), wthis](const uint8_t* buf, size_t len) {
                ...
                if (toRecv.schemeId == 0) {
                    ...
                    if (ctx->linkDevCtx->state == AuthDecodingState::HANDSHAKE) {
                        ...
                        auto authScheme = toRecv.payload.at("authScheme");
                        ...
                        emitSignal<libjami::ConfigurationSignal::DeviceAuthStateChanged>(
                            ...
                            std::map<std::string, std::string> {
                                ...
                                {"peer_id", peerCert->issuer->getId().toString()},
                                {"peer_device_id", peerCert->getLongId().toString()},
                            }
                        );
                    }
                        ...
                        if (!shouldLoadArchive) {
                            ...
                            emitSignal<libjami::ConfigurationSignal::DeviceAuthStateChanged>(
                                ctx->accountId,
                                static_cast<uint8_t>(DeviceAuthState::AUTHENTICATING),
                                std::map{std::pair("auth_error"s, "invalid_credentials"s)}
                            );
                            // or fail if tried too many times (handled in shutdown of channel by EXPORTING)
                        }
                    }
                    else {}
                    ...
                    if (shouldLoadArchive) {
                        ...
                        try {
                            auto archive = AccountArchive(std::string_view(accDataItr->second));
                    ...
                    // END logic
                    // handle the cases where the protocol should respond to the EXPORTING device
                    if (shouldSendMsg) {
                        JAMI_DEBUG("[LinkDevice] Sending msg from NEW to EXPORTING\t{}", toSend.formatMsg());
                        msgpack::sbuffer buffer(UINT16_MAX);
                        msgpack::pack(buffer, toSend);
                        std::error_code ec;
                        ctx->linkDevCtx->channel->write(reinterpret_cast<const unsigned char*>(buffer.data()), buffer.size(), ec);
...
uint32_t
ArchiveAccountManager::addDevice(const std::string& uriProvided, std::string_view auth_scheme, AuthChannelHandler* channelHandler)
{
    if (authCtx_) {
    ...
    try {
        ...
        auto gen = Manager::instance().getSeededRandomEngine();
        auto token = std::uniform_int_distribution<uint32_t>{1}(gen);
        JAMI_DEBUG("[LinkDevice] EXPORTING: Creating auth context.");
        auto ctx = std::make_shared<AuthContext>();
        ...
        channelHandler->connect(
            ...
            [this, auth_scheme, ctx] (std::shared_ptr<dhtnet::ChannelSocket> socket, const dht::InfoHash& infoHash) {
                if (!socket) {
                    JAMI_WARNING("[LinkDevice] Invalid socket event while AccountManager connecting.");
                    emitSignal<libjami::ConfigurationSignal::AddDeviceStateChanged>(accountId_, ctx->token, (uint8_t)DeviceAuthState::DONE, std::map{std::pair("error"s, "network"s)});
                } else {
                    if (!doAddDevice(auth_scheme, ctx, socket))
                        emitSignal<libjami::ConfigurationSignal::AddDeviceStateChanged>(accountId_, ctx->token, (uint8_t)DeviceAuthState::DONE, std::map{std::pair("error"s, "state"s)});
                }
            }
        );
...
bool
ArchiveAccountManager::doAddDevice(std::string_view scheme, const std::shared_ptr<AuthContext>& ctx, const std::shared_ptr<dhtnet::ChannelSocket>& channel)
{
    if (ctx->canceled) {
    ...
    ctx->timeout->async_wait([wthis = weak_from_this(), wctx=std::weak_ptr(ctx)](const std::error_code& ec) {
        if (ec) {
            return;
        }
        if (auto ctx = wctx.lock()) {
            if (ctx->addDeviceCtx->state != AuthDecodingState::DONE 
            ...
            && ctx->addDeviceCtx->state != AuthDecodingState::AUTH_ERROR) {
                if (auto this_ = wthis.lock()) {
                    ...
                    emitSignal<libjami::ConfigurationSignal::AddDeviceStateChanged>(
                        ctx->accountId, ctx->token, static_cast<uint8_t>(DeviceAuthState::DONE), std::map{std::pair("error"s, "timeout"s)});
                    this_->cancelAddDevice(ctx->token);
                }
            }
        }
    });
    ...
    channel->onShutdown([ctx]() {
        JAMI_DEBUG("[LinkDevice] EXPORTING: Shutdown with state {}... xfer {}uccessful", ctx->addDeviceCtx->formattedAuthState(), ctx->addDeviceCtx->archiveTransferredWithoutFailure ? "s" : "uns");
        // check if the archive was successfully loaded and emitSignal
        if (ctx->timeout)
            ctx->timeout->cancel();
        ctx->timeout.reset();
        if (ctx->addDeviceCtx->archiveTransferredWithoutFailure) {
            emitSignal<libjami::ConfigurationSignal::AddDeviceStateChanged>(
                ctx->accountId, ctx->token, static_cast<uint8_t>(DeviceAuthState::DONE), std::map{std::pair("error"s, ""s)});
        } else {
            emitSignal<libjami::ConfigurationSignal::AddDeviceStateChanged>(
                ctx->accountId, ctx->token, static_cast<uint8_t>(DeviceAuthState::DONE), std::map{std::pair("error"s, "network"s)});
        }
    });
    ...
    channel->setOnRecv([ctx,
        ...
        JAMI_DEBUG("[LinkDevice] Setting up receiver callback for communication logic on EXPORTING device.");
        ...
        try {
            ...
            JAMI_DEBUG("[LinkDevice] EXPORTING: Successfdully unpacked message from NEW (NEW->EXPORTING)\n{}", toRecv.formatMsg());
        } catch (const std::exception& e) {
        ...
        if (toRecv.schemeId == 0) {
            ...
            // we expect to be receiving credentials in this state and we know the archive is encrypted
            else if (ctx->addDeviceCtx->state == AuthDecodingState::AUTH) {
                // receive the incoming password, check if the password is right, and send back the
                // archive if it is correct
                JAMI_DEBUG("[LinkDevice] EXPORTING: addDevice: setOnRecv: verifying sent credentials from NEW");
                shouldSendMsg = true;
                // check if the password is valid... if so then send the unencrypyted archive data over the tls channel as a compressed file
                // this will involve (EXPORTING:) unzip, unencrypt, send, (NEW:) recv, encrypt w new salt, compress, write to system file (ONLY DO EXPORTING HERE)
                // check if password is valid
                bool passValid = this_->isPasswordValid(toRecv.payload["password"]);
                JAMI_DEBUG("[LinkDevice] EXPORTING: {} password received for authentication.", passValid ? "Valid" : "Invalid");
                if (passValid) {
                    ...
                    try {
                        ...
                        ctx->addDeviceCtx->accData = this_->readArchive(
                    ...
                    if (ctx->addDeviceCtx->numTries < ctx->addDeviceCtx->maxTries) {
                        ...
                        JAMI_DEBUG("[LinkDevice] Incorrect password was submitted to this server... "
                            "allowing a retry. {}", ctx->addDeviceCtx->numTries);
            ...
            else if (ctx->addDeviceCtx->state == AuthDecodingState::AUTH_ERROR) {
                // update the flag and
                ...
                emitSignal<libjami::ConfigurationSignal::AddDeviceStateChanged>(
                    ...
                    ctx->token,
                    static_cast<uint8_t>(DeviceAuthState::DONE),
                    std::map{std::pair("error"s, "auth_error"s)}); // let the client know that the auth has failed
            }
            else {
                JAMI_WARNING("[LinkDevice] EXPORTING: Unexpected auth state.");
            }
            // END logic

            if (shouldSendArchive) {
                JAMI_DEBUG("[LinkDevice] EXPORTING: Archive in message has encryption scheme '{}'", ctx->addDeviceCtx->authScheme);
                emitSignal<libjami::ConfigurationSignal::AddDeviceStateChanged>(
                        ctx->accountId,
                        ctx->token,
                        static_cast<uint8_t>(DeviceAuthState::IN_PROGRESS),
                ...
                try {
                    ...
                    toSend.payload["accData"] = ctx->addDeviceCtx->accData;
    ...
    if (ctx->addDeviceCtx->state == AuthDecodingState::HANDSHAKE) {
        ...
        emitSignal<libjami::ConfigurationSignal::AddDeviceStateChanged>(
                ...
                static_cast<uint8_t>(DeviceAuthState::AUTHENTICATING),
                std::map<std::string, std::string> {
                        std::pair{"peer_address", channel->getRemoteAddress().toString(true)},
                });
    }
    ...
    if (auto ctx = authCtx_) {
        if (ctx->token == token && ctx->addDeviceCtx && ctx->addDeviceCtx->state == AuthDecodingState::EST) {
            dht::ThreadPool::io().run([ctx] {
                ...
                AuthMsg toSend;
                JAMI_DEBUG("[LinkDevice] EXPORTING: Packing first message for NEW and switching to state: {}", ctx->addDeviceCtx->formattedAuthState());
                toSend.payload["authScheme"] = ctx->addDeviceCtx->authScheme;
                msgpack::sbuffer buffer(UINT16_MAX);
                msgpack::pack(buffer, toSend);
                std::error_code ec;
                ctx->addDeviceCtx->channel->write(reinterpret_cast<const unsigned char*>(buffer.data()), buffer.size(), ec);
