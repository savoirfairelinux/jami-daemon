...
void
ArchiveAccountManager::addDevice(std::string_view scheme, const std::string& password, AddDeviceCallback cb)
{
    dht::ThreadPool::computation().run([scheme=std::string(scheme), password, cb = std::move(cb), w=weak_from_this()] {
        auto this_ = std::static_pointer_cast<ArchiveAccountManager>(w.lock());
        if (not this_) return;
        ...
        try {
            ...
            a = this_->readArchive(scheme, password);
