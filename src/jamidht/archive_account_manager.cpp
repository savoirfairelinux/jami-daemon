            ...
            if (ctx->credentials->scheme == "file") {
                ...
                if (hasArchive) {
                    ...
                    auto future_keypair = dht::ThreadPool::computation().get<dev::KeyPair>(
                        &dev::KeyPair::create);
                    AccountArchive a;
                    JAMI_WARNING("[Auth] Converting certificate from old account {}",
...
ArchiveAccountManager::loadFromFile(AuthContext& ctx)
{
    ...
    try {
        ...
        JAMI_WARNING("[Auth] Unable to read file: {}", ex.what());
...
ArchiveAccountManager::provideCurrentAccountAuthentication(const std::string& credentialsFromUser, const std::string_view scheme)
{
    ...
    // this->authChannel_->write(reinterpret_cast<const unsigned char*>(buffer.data()), buffer.size(), ec);
    this->authCtx_->linkDevCtx->channel->write(reinterpret_cast<const unsigned char*>(buffer.data()), buffer.size(), ec);
    ...
    //         try {
    //             JAMI_DEBUG("[LinkDevice] NEW: Sending password to old device.");
    //             this_->authCtx_->linkDevCtx->channel->write(reinterpret_cast<const unsigned char*>(buffer.data()), buffer.size(), ec);
...
{
    this->authCtx_ = ctx;
    // this->authCtx_ = std::move(ctx);
    // move the account creation to another thread
    dht::ThreadPool::computation().run([this, ctx, wthis = weak_from_this()] {
        ...
        if (auto this_ = std::static_pointer_cast<ArchiveAccountManager>(wthis.lock())) {
            ...
            this_->authCtx_->linkDevCtx = std::make_unique<LinkDeviceContext>(dht::crypto::generateIdentity("Jami Temporary device", user));
            // this_->authCtx_->linkDevCtx = std::make_shared<LinkDeviceContext>(dht::crypto::generateIdentity("Jami Temporary device", user));
            JAMI_LOG("[LinkDevice] Established linkDevCtx.");
            // JAMI_LOG("[LinkDevice] NO LONGER NEEDED Established channel lifetime.");
        } else {
        ...
        emitSignal<libjami::ConfigurationSignal::DeviceAuthStateChanged>(
            ctx->accountId,
            static_cast<uint8_t>(DeviceAuthState::NONE),
            accountScheme);
        // makes sure certificate is properly loaded
        ...
        ctx->linkDevCtx->tempConnMgr.onConnectionReady([ctx, accountScheme, wthis](
            ...
            socket->setOnRecv([socket, ctx, decodingCtx = std::make_shared<DecodingContext>(), wthis](const uint8_t* buf, size_t len) {
                if (!buf) {
                    return len;
                }
                ...
                if (toRecv.schemeId == 0) {
                    ...
                    if (ctx->linkDevCtx->state == AuthDecodingState::HANDSHAKE) {
                        // since we received a zero we know we are in protocol v0 so it is possible to get auth scheme here as well
                        ...
                        if (ctx->linkDevCtx->authEnabled)
                            // emit signal to open the credentials page in the client GUI
                            emitSignal<libjami::ConfigurationSignal::DeviceAuthStateChanged>(
                                ctx->accountId,
                            ...
                            JAMI_WARNING("[LinkDevice] NEW: authScheme is NONE");
                    }
...
ArchiveAccountManager::addDevice(std::string_view scheme, uint32_t token, const std::shared_ptr<dhtnet::ChannelSocket>& channel)
{
    ...
    JAMI_DEBUG("[LinkDevice] OLD: Creating auth context.");
    this->authCtx_ = std::make_shared<AuthContext>();
    auto ctx = this->authCtx_;
    JAMI_DEBUG("[LinkDevice] OLD: Creating peerLoadCtx.");
    ...
    JAMI_DEBUG("[LinkDevice] OLD: Creating callbacks.");
        ...
        if (s.stateOld.first && s.stateNew.first) {
            dht::ThreadPool::computation().run(
                [ctx, network_error = !s.stateOld.second && !s.stateNew.second] {
                    ...
                    JAMI_WARNING("[Auth] Failure looking for archive on DHT: {}",
