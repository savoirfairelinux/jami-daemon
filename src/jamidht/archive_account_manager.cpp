...
struct ArchiveAccountManager::DeviceAuthInfo : public std::map<std::string, std::string>
{
    ...
    // Add error enum
    enum class Error { INVALID_CREDENTIALS, NETWORK, STATE, TIMEOUT, AUTH_ERROR, NONE, CANCELED };
    ...
    dht::ThreadPool::computation().run([ctx, wthis = weak()] {
        ...
        ctx->linkDevCtx->tempConnMgr.onConnectionReady([ctx,
            ...
            socket->onShutdown([ctx, name, wthis]() {
                ...
                // - If authentication failed (AUTH_ERROR state), return INVALID_CREDENTIALS
                // - If archive was successfully transferred, return NONE
                // - Otherwise assume network error
                DeviceAuthInfo::Error error;
                if (ctx->linkDevCtx->state == AuthDecodingState::AUTH_ERROR) {
                    error = DeviceAuthInfo::Error::INVALID_CREDENTIALS;
                } else if (ctx->linkDevCtx->state == AuthDecodingState::TIMEOUT) {
                    error = DeviceAuthInfo::Error::TIMEOUT;
                } else if (ctx->linkDevCtx->archiveTransferredWithoutFailure) {
                    error = DeviceAuthInfo::Error::NONE;
                } else {
                    error = DeviceAuthInfo::Error::NETWORK;
                }
            ...
            socket->setOnRecv([ctx,
                ...
                if (toRecv.schemeId == 0) {
                    ...
                    if (ctx->linkDevCtx->state == AuthDecodingState::HANDSHAKE) {
                        ...
                        ctx->linkDevCtx->state = ctx->linkDevCtx->authEnabled
                                                     ? AuthDecodingState::AUTH
                                                     : AuthDecodingState::DATA;
                        DeviceAuthInfo info;
                        info.set(DeviceAuthInfo::auth_scheme, authScheme);
                        info.set(DeviceAuthInfo::peer_id, peerCert->issuer->getId().toString());
                        info.set(DeviceAuthInfo::peer_device_id, peerCert->getLongId().toString());
                        emitSignal<libjami::ConfigurationSignal::DeviceAuthStateChanged>(
                            ctx->accountId,
                        ...
                        if (!shouldLoadArchive) {
                            ...
                            emitSignal<libjami::ConfigurationSignal::DeviceAuthStateChanged>(
                                ...
                                DeviceAuthInfo::createError(
                                    DeviceAuthInfo::Error::INVALID_CREDENTIALS));
                    ...
                    if (shouldSendMsg) {
                        JAMI_DEBUG("[LinkDevice] Sending msg from NEW to EXPORTING\t{}",
                                   toSend.formatMsg());
                        msgpack::sbuffer buffer(UINT16_MAX);
                        msgpack::pack(buffer, toSend);
                        std::error_code ec;
                        ctx->linkDevCtx->channel->write(reinterpret_cast<const unsigned char*>(
                                                            buffer.data()),
                                                        buffer.size(),
                                                        ec);
                    }
                ...
                // catch unsupported operations
                    ...
                    JAMI_WARNING("[LinkDevice] NEW: Unsupported linkdev scheme id of {}",
                                 toRecv.schemeId);
                    ctx->linkDevCtx->state = AuthDecodingState::ERR;
                    return (size_t) 0;
                }
                ...
                if (!socket) {
                    ...
                    if (!doAddDevice(auth_scheme, ctx, socket))
                        emitSignal<libjami::ConfigurationSignal::AddDeviceStateChanged>(
                            ...
                            ctx->token,
                            static_cast<uint8_t>(DeviceAuthState::DONE),
                            DeviceAuthInfo::createError(DeviceAuthInfo::Error::STATE));
                }
            });
                                   ...
                                   const std::shared_ptr<dhtnet::ChannelSocket>& channel)
{
                ...
                if (ctx->addDeviceCtx->state != AuthDecodingState::DONE
                    ...
                    if (auto this_ = wthis.lock()) {
                        ...
                        emitSignal<libjami::ConfigurationSignal::AddDeviceStateChanged>(
                            ...
                            ctx->token,
                            static_cast<uint8_t>(DeviceAuthState::DONE),
                            DeviceAuthInfo::createError(DeviceAuthInfo::Error::TIMEOUT));
    ...
    channel->onShutdown([ctx, w = weak()]() {
        ...
        DeviceAuthInfo::Error error;
        if (ctx->canceled) {
            error = DeviceAuthInfo::Error::CANCELED;
        } else if (ctx->addDeviceCtx->state == AuthDecodingState::AUTH_ERROR) {
            error = DeviceAuthInfo::Error::INVALID_CREDENTIALS;
        } else if (ctx->addDeviceCtx->state == AuthDecodingState::EST) {
            // When in EST state, we're waiting for user confirmation to export the account
            // If we reach here while still in EST, it means we timed out waiting for user action
            error = DeviceAuthInfo::Error::TIMEOUT;
        } else if (ctx->addDeviceCtx->archiveTransferredWithoutFailure) {
            error = DeviceAuthInfo::Error::NONE;
        } else {
            error = DeviceAuthInfo::Error::NETWORK;
        }
    ...
    channel->setOnRecv([ctx,
        ...
        if (toRecv.schemeId == 0) {
            ...
            // logic
            // state typically reflects what data we expect to receive from the other end, as well
            // as what data we will be sending for example, we expect to receive a msg containing
            // handshaking information in the first if statement
            ...
            // we expect to be receiving credentials in this state and we know the archive is encrypted
                ...
                if (passValid) {
                    ...
                    try {
                        ...
                        JAMI_DEBUG(
                            "[LinkDevice] Injected account archive into outbound message.\n{}",
                            ctx->addDeviceCtx->accData);
                    } catch (...) {
                    ...
                    if (ctx->addDeviceCtx->numTries < ctx->addDeviceCtx->maxTries) {
                        ...
                        JAMI_DEBUG(
                            "[LinkDevice] Incorrect password was submitted to this server... "
                            "allowing a retry. {}",
                            ctx->addDeviceCtx->numTries);
                ...
                emitSignal<libjami::ConfigurationSignal::AddDeviceStateChanged>(
                    ...
                    ctx->token,
                    static_cast<uint8_t>(DeviceAuthState::DONE),
                    DeviceAuthInfo::createError(DeviceAuthInfo::Error::AUTH_ERROR));
            } else {
            ...
            if (shouldSendArchive) {
                ...
                try {
                    ...
                    JAMI_ERROR("[LinkDevice] EXPORTING: Error preparing archive for xfer: {}",
                               e.what());
