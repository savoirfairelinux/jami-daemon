...
struct ArchiveAccountManager::LinkDeviceContext
{
    ...
    {
        AuthMsg timeoutMsg;
        timeoutMsg.set(PayloadKey::stateMsg, std::string(formattedAuthState()));
        return timeoutMsg;
    }
};
...
struct ArchiveAccountManager::AddDeviceContext
{
    ...
    bool handleTimeoutMessage(const AuthMsg& msg)
    {
        ...
        if (stateMsgIt != msg.payload.end()) {
            auto state = stateMsgIt->second;
            if (state == "TIMEOUT") {
                // Update state to be handled in onShutdown
                                   ...
                                   const std::shared_ptr<dhtnet::ChannelSocket>& channel)
{
    ...
    channel->onShutdown([ctx, w = weak()]() {
        ...
        if (ctx->canceled) {
            ...
            error = DeviceAuthInfo::Error::INVALID_CREDENTIALS;
        } else if (ctx->addDeviceCtx->state == AuthDecodingState::TIMEOUT) {
            error = DeviceAuthInfo::Error::TIMEOUT;
