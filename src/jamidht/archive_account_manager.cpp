...
struct ArchiveAccountManager::LinkDeviceContext
{
    ...
    uint64_t opId; // TODO use this in the function calls

    uint8_t numOpenChannels;
    uint8_t maxOpenChannels {1};
    ...
    uint8_t failedPasswordAttempts {0};
    uint8_t numTries {0};
    uint8_t maxTries {0};
...
ArchiveAccountManager::startLoadArchiveFromDevice(const std::shared_ptr<AuthContext>& ctx)
{
    dht::ThreadPool::computation().run([ctx = std::move(ctx), w=weak_from_this()] {
        ...
        std::uniform_int_distribution<uint64_t> distrib(100000, 999999);
        ...
        const std::string accountScheme = "jami-auth://" + ctx->linkDevCtx->tmpId.second->getId().toString() + "/" + std::to_string(distrib(generator));
        // JAMI_DBG("[LinkDevice] Built account scheme.");
        // JAMI_INFO("[LinkDevice %s] Generated temporary account.", ctx->linkDevCtx->tmpId.second->getId().toString().c_str());

        ctx->linkDevCtx->opId = distrib(generator);
        // JAMI_DBG("[LinkDevice] Assigned an opId to a temporary account.");
        ...
        JAMI_DBG("[LinkDevice] Emitting account ready signal");
        ...
        // see how this is handled in auth_channel_handler
