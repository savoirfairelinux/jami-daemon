...
ArchiveAccountManager::loadFromDHT(const std::shared_ptr<AuthContext>& ctx)
    ...
    void ArchiveAccountManager::migrateAccount(AuthContext & ctx)
    {
    ...
    void ArchiveAccountManager::onArchiveLoaded(AuthContext & ctx,
                                                AccountArchive && a,
    ...
    {
        JAMI_LOG("[Account {}] [Auth] Signing receipt for device {}",
                 accountId_,
                 device.getLongId());
    ...
    bool ArchiveAccountManager::needsMigration(const std::string& accountId,
    ...
    void ArchiveAccountManager::syncDevices()
    {
        ...
        for (const auto& dev : getKnownDevices()) {
            ...
            if (!dev.second.certificate) {
                JAMI_WARNING("[Account {}] Unable to find certificate for {}",
                             accountId_,
                             dev.first);
                continue;
            }
    ...
    void ArchiveAccountManager::startSync(const OnNewDeviceCb& cb,
    ...
    AccountArchive ArchiveAccountManager::readArchive(std::string_view scheme,
    ...
    void ArchiveAccountManager::updateArchive(AccountArchive & archive) const
    {
        ...
        for (const auto& it : onExportConfig_()) {
            // filter-out?
            if (std::any_of(std::begin(filtered_keys),
                            std::end(filtered_keys),
    ...
    void ArchiveAccountManager::saveArchive(AccountArchive & archive,
    ...
    bool ArchiveAccountManager::changePassword(const std::string& password_old,
    ...
    std::vector<uint8_t> ArchiveAccountManager::getPasswordKey(const std::string& password)
    {
        try {
    ...
    bool ArchiveAccountManager::revokeDevice(const std::string& device,
                                             std::string_view scheme,
                                             const std::string& password,
    ...
    bool ArchiveAccountManager::exportArchive(const std::string& destinationPath,
                                              std::string_view scheme,
                                              const std::string& password)
    {
        try {
    ...
    bool ArchiveAccountManager::isPasswordValid(const std::string& password)
    {
        try {
...
#if HAVE_RINGNS
    void ArchiveAccountManager::registerName(const std::string& name,
                                             std::string_view scheme,
                                             const std::string& password,
