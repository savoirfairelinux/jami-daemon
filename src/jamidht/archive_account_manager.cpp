...
ArchiveAccountManager::provideAccountAuthentication(const std::string& key, const std::string& scheme)
{
    ...
    dht::ThreadPool::io().run([key=key, scheme, ctx] mutable {
        AuthMsg toSend;
        toSend.payload["password"] = std::move(key);
...
#if TARGET_OS_IOS
        ctx->linkDevCtx->tempConnMgr.oniOSConnected([&](const std::string& connType, dht::InfoHash peer_h) {
            return false;
        });
#endif
                ...
                if (toRecv.schemeId == 0) {
                    ...
                    if (ctx->linkDevCtx->state == AuthDecodingState::HANDSHAKE) {
                        ...
                        if (passwordCorrectIt != toRecv.payload.end() && passwordCorrectIt->second == "false") {
                            ...
                            // At this point AUTHENTICATING state has already been emitted.
                            // Peer should be aware of authentication status.
                            auto peerCert = ctx->linkDevCtx->channel->peerCertificate();
                            auto peer_id = peerCert->issuer->getId().toString();
                            auto authScheme = "password";
