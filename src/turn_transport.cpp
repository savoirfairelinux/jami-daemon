...
public:
    ...
    void onRxData(void* pkt, unsigned pkt_len, const pj_sockaddr_t* peer_addr, unsigned addr_len);
...
void
TurnTransportPimpl::onRxData(void* pkt, unsigned pkt_len,
                             const pj_sockaddr_t* peer_addr, unsigned addr_len)
{
    (void)pkt;

    RING_WARN("rx %u bytes from %s", pkt_len,
              IpAddr((const sockaddr*)peer_addr, addr_len).toString(true, true).c_str());
}
...
{
    IpAddr peer_addr { (const sockaddr*)addr, addr_len };
    ...
    auto status = pj_turn_connect_peer(relay, conn_id, addr, addr_len);
}
...
class PjsipError : public std::runtime_error {
public:
    PjsipError() : std::runtime_error("PJSIP api error") {}
};

template <class Callable, class... Args>
inline void PjsipCall(Callable& func, Args... args)
{
    if (func(args...) != PJ_SUCCESS)
        throw PjsipError();
}

template <class Callable, class... Args>
inline auto PjsipCall(const Callable& func, Args... args) -> decltype(func(args...))
{
    auto res = func(args...);
    if (!res)
        throw PjsipError();
    ...
    : pimpl_ {new TurnTransportPimpl}
{
    ...
    RING_WARN("Connecting to TURN %s", params.server.toString(true, true).c_str());
    ...
    pimpl_->pool = PjsipCall(pj_pool_create, &pimpl_->poolCache.factory,
    ...
    relay_cb.on_rx_data = [](pj_turn_sock* relay, void* pkt, unsigned pkt_len,
        ...
        tr->pimpl_->onRxData(pkt, pkt_len, peer_addr, addr_len);
    };
    ...
    // Connect ourself to the TURN as a peer
...
bool
TurnTransport::sendto(const IpAddr& peer, const std::vector<uint8_t>& buffer)
{
    auto status = pj_turn_sock_sendto(pimpl_->relay, buffer.data(), buffer.size(),
                                      peer.pjPtr(), peer.getLength());
    if (status != PJ_SUCCESS) {
        RING_ERR("pj_turn_sock_sendto() failed");
        return false;
    }
...
void
TurnTransport::recvfrom(std::pair<IpAddr, std::vector<uint8_t>>&)
{
}
