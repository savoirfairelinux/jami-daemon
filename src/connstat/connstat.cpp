...
int connstat::registerCB(cb ucb, unsigned int topics)
{
	...
	if (nclients == CS_MAX_CB_PERTOPIC * CONNSTAT_TOPICS_MAX) {
		pretty_logger("FIXME", __func__,
			"we're full! raise the limit, CS_MAX_CB_PERTOPIC");
		return status;
	}
	...
	if ( (t = topics & addtopic(NEWNEIGH)) == addtopic(NEWNEIGH) ) {
		pretty_logger("DEBUG", __func__, "got NEWNEIGH");
	...
	if (!topics)
		...
		fprintf(stderr, "%*s %s\n",
			...
			__PRETTY_FUNCTION__);
ret:
	pretty_logger("DEBUG", __func__, "DONE");
...
{
	pretty_logger("ENTERED", __func__, "");
	int status = FAILURE;

	// nlsk = (struct nl_sock *)nlibsk;

	if (nlsk && nl_connect(nlsk, NETLINK_ROUTE) == -NLE_BAD_SOCK) {
		pretty_logger("INSIDE", __func__, "socket is already connected");
		return SUCCESS;
	}
...
#define MAX_MSGTYPE 64
	...
	pretty_logger("INFO", "got ", msgtypestr);
...
#define MAX_MSGTYPE 64
	...
	pretty_logger("INFO", __func__, msgtypestr);
	// pretty_logger("TODO", __func__, "parse msg for v4/v6");
	switch (h->nlmsg_type) {
	case RTM_NEWNEIGH:
	case RTM_DELNEIGH:
	case RTM_NEWNEIGHTBL:
