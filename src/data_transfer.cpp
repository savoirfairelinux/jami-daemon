...
public:
    ...
    virtual bool start() {
        bool expected = false;
        return started_.compare_exchange_weak(expected, true);
    }
                             ...
                             const std::string& file_path, const std::string& display_name)
{
    ...
    auto id = generateUID();
    pimpl_->createFileTransfer(id, file_path, display_name);
    ...
    account->requestPeerConnection(
        peer_uri,
        [this, id] (PeerConnection* connection) { pimpl_->onConnectionRequestReply(id, connection); });
