...
IncomingFile::process()
{
    ...
    channel_->onShutdown([this] {
        ...
        // TODO rework status as closed_by_peer will come back with offsed
        auto code = correct ? DRing::DataTransferEventCode::finished
                            : DRing::DataTransferEventCode::closed_by_peer;
        emit(code);
    });
...
TransferManager::info(const DRing::DataTransferId& id, DRing::DataTransferInfo& info) const noexcept
{
    ...
    if (pimpl_->isConversation_) {
        ...
        // int64_t totalSize {0};               ///< Total number of bytes to sent/receive, 0 if not
        // known int64_t bytesProgress {0};           ///< Number of bytes sent/received std::string
        // author; std::string peer; ///< Identifier of the remote peer (in the semantic of the
        // associated account) std::string conversationId; std::string displayName; ///< Human
        // oriented transfer name std::string path;        ///< associated local file path if
        // supported (empty, if not)
        return true;
    }
    auto it = pimpl_->iMap_.find(id);
    if (it != pimpl_->iMap_.end()) {
                               ...
                               const std::shared_ptr<ChannelSocket>& channel)
{
    ...
    // TODO shared_ptr + no double lock
