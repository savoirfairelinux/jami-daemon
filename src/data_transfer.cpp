...
ssize_t
DataStream::recvData(uint8_t* buffer, std::size_t buffer_size)
{
    std::unique_lock<std::mutex> lk {mtx_};
    //RING_ERR("wait %lx", rxOffset_);
    ...
    auto pair_it = std::begin(reorderBuffer_);
    while (pair_it != std::end(reorderBuffer_) and pair_it->first < rxOffset_) {
        // copy data to application buffer until filled.
        auto& vec = pair_it->second;
        auto bytes = std::min(vec.size(), buffer_size);
        std::copy_n(vec.data(), bytes, buffer);
        bytes_read += bytes;
        //RING_ERR("read %zub at %lx", bytes, pair_it->first);
        if (bytes < vec.size()) {
            reorderBuffer_.erase(pair_it);

            // re-insert non consumed data for later.
            reorderBuffer_.emplace(pair_it->first + bytes, std::vector<uint8_t>(std::begin(vec)+bytes,
                                                                              std::end(vec)));
            return bytes_read;
        }
        pair_it = reorderBuffer_.erase(pair_it);
    }
...
{
    // Put frame into a reorder buffer (implemented as an ordered map)
    std::lock_guard<std::mutex> lk {mtx_};
    auto inserted = reorderBuffer_.emplace(offset,
                                           std::vector<uint8_t>(buffer, buffer + length)).second;

    // Only update highest rx offset if in advance
    if (offset > rxOffset_) {
        rxOffset_ = offset;
        return;
    }
    ...
    // If insertion has been made, we may filled a frame gap at beggining.
    // Notify the application in such case.
    flags_.set(FlagsBit::BYTES_AVAILABLE);
    //RING_ERR("rx %lx", offset);
    cv_.notify_one();
}
