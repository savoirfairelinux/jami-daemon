...
{
public:
    DataTransfer(DRing::DataTransferId id, bool isInternal = false, InternalCompletionCb cb = {})
        : Stream(), id {id}, isInternal {isInternal}, cb_ {cb} {}
...
protected:
    ...
    std::atomic_bool isInternal {false};
...
SubOutgoingFileTransfer::SubOutgoingFileTransfer(DRing::DataTransferId tid,
                                           ...
                                           InternalCompletionCb cb,
                                           std::shared_ptr<OptimisticMetaOutgoingInfo> metaInfo)
    : DataTransfer(tid, static_cast<bool>(cb), cb), metaInfo_(std::move(metaInfo)), peerUri_(peerUri)
{
...
{
public:
    OutgoingFileTransfer(DRing::DataTransferId tid, const DRing::DataTransferInfo& info, InternalCompletionCb cb);
    ~OutgoingFileTransfer() {}

    std::shared_ptr<DataTransfer> startNewOutgoing(const std::string& peer_uri) {
        auto newTransfer = std::make_shared<SubOutgoingFileTransfer>(id, peer_uri, cb_, metaInfo_);
...
OutgoingFileTransfer::OutgoingFileTransfer(DRing::DataTransferId tid, const DRing::DataTransferInfo& info, InternalCompletionCb cb)
: DataTransfer(tid, static_cast<bool>(cb), cb)
{
...
IncomingFileTransfer::IncomingFileTransfer(DRing::DataTransferId tid,
                                           ...
                                           InternalCompletionCb cb)
    : DataTransfer(tid, static_cast<bool>(cb), cb), internalId_(internalId)
{
...
DataTransferFacade::sendFile(const DRing::DataTransferInfo& info,
                             DRing::DataTransferId& tid,
                             InternalCompletionCb cb) noexcept
{
...
DRing::DataTransferId
DataTransferFacade::createIncomingTransfer(const DRing::DataTransferInfo &info, const DRing::DataTransferId& internal_id, InternalCompletionCb cb)
{
