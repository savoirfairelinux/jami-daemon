...
template< class T >
constexpr const T& bounds( const T& a, const T& x, const T& b )
{
                         ...
                         uint64_t offset=0, uint16_t data_length=0)
{
    ...
    if (data_length)
        frame_size += 2;
    if (offset) {
        off_size = bounds(2, static_cast<int>(std::log2(offset)) / 8, 8);
        frame_size += off_size;
        --off_size;
    } else
        off_size = 0;
...
private:
    unsigned maxPacket_;
    std::mutex accMtx_ {};
    std::queue<std::shared_ptr<std::vector<uint8_t>>> outQ_ {}; // packets to transmit
};

bool
TxFrameBuffer::push(const std::vector<uint8_t>& packet)
{
    std::lock_guard<std::mutex> lk {accMtx_};
    if (outQ_.size() == maxPacket_)
        return false; // full
    outQ_.emplace(std::make_shared<std::vector<uint8_t>>(packet));
    return true;
}

bool
TxFrameBuffer::push(std::vector<uint8_t>&& packet)
{
    std::lock_guard<std::mutex> lk {accMtx_};
    if (outQ_.size() == maxPacket_)
        return false; // full
    outQ_.emplace(std::make_shared<std::vector<uint8_t>>(std::move(packet)));
    return true;
}
...
DataConnection::idleJob()
{
    if (protocol_) {
        ...
        if (isClient_) {
            ...
            Packet::packStreamFrame(pkt, 0, 1, 0, data_length);
    ...
    if (delta > 5) {
        ...
        RING_WARN("\nrx: Bytes: %.3fMbits/s, Bad: %zu, Rx Loss/OoO: %zu, RTT (ms): %lu, RTT var.: %.1f, PPS: %.1f, Power: %.1f",
        ...
        cumRtt_ = std::chrono::microseconds(0);
        cumRttVar_ = 0;
        cumPps_ = 0;
        statSync_ = now;
    }
...
bool
DataConnection::pushPacket(const std::vector<uint8_t>& pkt)
{
...
bool
DataConnection::pushPacket(std::vector<uint8_t>&& pkt)
{
...
DataConnection::processAcknowledgmentFields(const std::vector<uint8_t>& pkt)
{
    ...
    if (ack_delta > 0) {
        ...
        // TODO: acknowledge upper layer about all ack pkts
    ...
    // Network statistics is trigged when the marked sequence as made a round-trip
    if (ack_seq >= netStatSeq_) {
        std::lock_guard<std::mutex> lk {txMtx_};

        // Compute RTT, clamp to [1, MAX_RTT]
        auto rtt = cast_to_us(Clock::now() - netStatTime_).count();
        rtt = std::max(1l, std::min(rtt, cast_to_us(MAX_RTT).count()));
        auto cum_rtt = ((cumRtt_.load().count() * 7.f) + rtt) / 8.f;
        cumRtt_ = std::chrono::microseconds(static_cast<unsigned long>(cum_rtt));

        // Compute an RTT variance measure
		float rtt_var = fabsf(rtt - cum_rtt);
		cumRttVar_ = ((cumRttVar_ * 7.f) + rtt_var) / 8.f;

        // Compute number of unique ACK during this round-trip
        float pps = cumAckCount_ * 1000000.f / rtt;
		cumPps_ = ((cumPps_ * 7.f) + pps) / 8.f;

        //RING_DBG("rtt (ms): %.1f, pps: %.1f", rtt/1000.f, pps);
...
DataStream::sendData(const uint8_t* buffer, std::size_t buffer_size)
{
    ...
    while (bytes_send < buffer_size) {
        ...
        std::unique_lock<std::mutex> lk {mtx_};
        cv_.wait(lk, [this]{ return flags_[FlagsBit::WRITE]; });
        ...
        std::vector<uint8_t> pkt;
        Packet::packStreamFrame(pkt, 0, sid_, 0, buffer_size, buffer);
