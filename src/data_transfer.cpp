...
FileTransfer::read(std::vector<char>& buf) const
{
    ...
    if (headerSent_) {
        ...
        auto str_size = "Content-Length: " + to_string(input_.tellg()) + "\n\n";
        input_.seekg(0, std::ios_base::beg);
        buf.resize(str_size.size());
        std::copy(std::begin(str_size), std::end(str_size), std::begin(buf));
        headerSent_ = true;
    }
...
DataTransferFacade::cancel(const DRing::DataTransferId& id)
{
    ...
    emitSignal<DRing::DataTransferSignal::DataTransferEvent>(id, int(DRing::DataTransferEventCode::CLOSED_BY_HOST));
}
