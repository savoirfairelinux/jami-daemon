...
DataConnection::nextTxPacket(TimePoint& when)
{
    ...
    if (!protocol_) {
        ...
        // Missed packets are re-sent in priority
        {
            ...
            std::lock(lk1, lk2);
            if (txSeq_ > MISS_ORDERING_LIMIT and !ackWaitMap_.empty()) {
                auto last_missed = txSeq_ - MISS_ORDERING_LIMIT;
                ...
                if (pair->first <= last_missed) {
                    ...
                    txDelay_ = std::chrono::microseconds(std::max(cast_to_us(cumRtt_).count() / 20ul, 10ul));
                    goto end;
                }
            } else
                txDelay_ = std::chrono::microseconds(std::max(cast_to_us(cumRtt_).count() / rxAckMask_.size(), 10ul));
