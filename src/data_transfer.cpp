...
public:
    ...
    void cancel() override {
        ...
        if (account) account->closePeerConnection(peerUri_, id);
    }
...
public:
    ...
    bool cancel(bool channeled) {
        if (channeled)
            ...
            cancelIce_ = true;
        JAMI_ERR("Cancel: %u - result: %u", channeled, cancelChanneled_ && cancelIce_);
                                                     ...
                                                     DRing::DataTransferId& tid)
{
    ...
    {
        std::lock_guard<std::mutex> lk {mapMutex_};
        JAMI_WARN("CREATE IOUTGOING %u", tid);
...
DataTransferFacade::Impl::createIncomingFileTransfer(const DRing::DataTransferInfo& info, const DRing::DataTransferId& internal_id)
{
    ...
    {
        std::lock_guard<std::mutex> lk {mapMutex_};
        JAMI_WARN("CREATE INCOMING %u", tid);
        map_.emplace(tid, transfer);
    }
    transfer->emit(DRing::DataTransferEventCode::created);
    return transfer;
...
{
    std::lock_guard<std::mutex> lk {pimpl_->mapMutex_};
    for (const auto& item : pimpl_->map_) {
        if ((int)item.first == (int)id) { // TODO Clean this, find is not working
            item.second->close();
        }
    }
...
IncomingFileInfo
DataTransferFacade::onIncomingFileRequest(const DRing::DataTransferInfo &info, const DRing::DataTransferId& id) {
  auto transfer = pimpl_->createIncomingFileTransfer(info, id);
  if (!transfer)
      return {};
