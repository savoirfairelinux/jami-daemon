...
{
public:
    SubOutgoingFileTransfer(DRing::DataTransferId tid, bool channeled, const std::string& peerUri, std::shared_ptr<OptimisticMetaOutgoingInfo> metaInfo);
...
SubOutgoingFileTransfer::SubOutgoingFileTransfer(DRing::DataTransferId tid,
    ...
    : DataTransfer(tid), channeled(channeled), metaInfo_(std::move(metaInfo)), peerUri_(peerUri)
{
...
public:
    ...
    std::shared_ptr<DataTransfer> startNewOutgoing(const std::string& peer_uri, bool channeled = false) {
        auto newTransfer = std::make_shared<SubOutgoingFileTransfer>(id, channeled, peer_uri, this->metaInfo_);
        subtransfer_.emplace_back(newTransfer);
        newTransfer->start();
        return newTransfer;
    }

    bool cancel(bool channeled, const std::string& peerId = "") {
        if (!channeled) {
            // Cancel if all transfers are not channeled
            for (const auto& subtransfer: subtransfer_)
                if (subtransfer->channeled)
                    return false;
            return true;
        }
...
{
    std::lock_guard<std::recursive_mutex> lk {mapMutex_};
...
{
    std::lock_guard<std::recursive_mutex> lk {pimpl_->mapMutex_};
        ...
        account->requestPeerConnection(
            ...
            [this, tid](const std::shared_ptr<ChanneledOutgoingTransfer>& out) {
                if (auto transfer = pimpl_->getTransfer(tid))
                    if (out)
                        out->linkTransfer(
                            std::dynamic_pointer_cast<OutgoingFileTransfer>(transfer)->startNewOutgoing(out->peer(), true)
                        );
            },
            [this, tid](const std::string& peerId) {
                if (auto transfer = pimpl_->getTransfer(tid))
                    if (std::dynamic_pointer_cast<OutgoingFileTransfer>(transfer)->cancel(true, peerId)
                    and not transfer->hasBeenStarted()) {
                        transfer->emit(DRing::DataTransferEventCode::unjoinable_peer);
