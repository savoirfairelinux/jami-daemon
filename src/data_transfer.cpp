...
DataStream::recvData(uint8_t* buffer, std::size_t buffer_size)
{
    ...
    // Loop on each received frame until we reach a gap
    std::size_t bytes_read = 0;
    auto pair_it = std::begin(reorderBuffer_);
    while (pair_it != std::end(reorderBuffer_) and pair_it->first <= waitOffset_) {
        // copy data on selected block to application buffer until filled or no more data
        ...
        waitOffset_ += bytes;
        if (bytes < vec.size()) {
            // re-insert non consumed data for a later call
...
DataStream::onRxPayload(const uint8_t* buffer, std::size_t length, uint64_t offset)
{
    ...
    // Try to fill a slot for this offset into our reordering buffer
    auto result = reorderBuffer_.emplace(offset, std::vector<uint8_t>());

    // Insert data by copy only if vector has been inserted
    if (result.second) {
        auto& vec =result.first->second;
        vec.insert(vec.begin(), buffer, buffer + length);
    }

    // Only wake-up application if we receive its waited bytes
    if (offset == waitOffset_) {
        flags_.set(FlagsBit::BYTES_AVAILABLE);
        cv_.notify_one();
    }
}
