...
{
public:
    DataTransfer(DRing::DataTransferId id, bool isVCard) : Stream(), id {id}, isVCard {isVCard} {}
        ...
        dht::ThreadPool::computation().run([this]() {
            ...
            if (isVCard) {
                ...
                auto path = fileutils::get_cache_dir() + DIR_SEPARATOR_STR
                            + info_.accountId + DIR_SEPARATOR_STR
                            + "vcard" + DIR_SEPARATOR_STR + info_.peer;
...
SubOutgoingFileTransfer::SubOutgoingFileTransfer(DRing::DataTransferId tid,
                                           ...
                                           bool isVCard,
                                           std::shared_ptr<OptimisticMetaOutgoingInfo> metaInfo)
    : DataTransfer(tid, isVCard), metaInfo_(std::move(metaInfo)), peerUri_(peerUri)
{
...
{
public:
    OutgoingFileTransfer(DRing::DataTransferId tid, const DRing::DataTransferInfo& info, bool isVCard);
    ~OutgoingFileTransfer() {}

    std::shared_ptr<DataTransfer> startNewOutgoing(const std::string& peer_uri) {
        auto newTransfer = std::make_shared<SubOutgoingFileTransfer>(id, peer_uri, isVCard, metaInfo_);
...
OutgoingFileTransfer::OutgoingFileTransfer(DRing::DataTransferId tid, const DRing::DataTransferInfo& info, bool isVCard)
: DataTransfer(tid, isVCard)
{
...
IncomingFileTransfer::IncomingFileTransfer(DRing::DataTransferId tid,
                                           ...
                                           bool isVCard)
    : DataTransfer(tid, isVCard), internalId_(internalId)
{
...
IncomingFileTransfer::close() noexcept
{
    ...
    if (info_.bytesProgress >= info_.totalSize) {
        if (isVCard) {
            try {
                auto vCard = fileutils::loadTextFile(info_.path);
                emitSignal<DRing::ConfigurationSignal::ProfileReceived>(info_.accountId, info_.peer, vCard);
...
public:
    ...
    std::shared_ptr<DataTransfer> createOutgoingFileTransfer(const DRing::DataTransferInfo& info,
                                                             ...
                                                             bool isVCard = false);
    std::shared_ptr<IncomingFileTransfer> createIncomingFileTransfer(const DRing::DataTransferInfo&, const DRing::DataTransferId&, bool);
...
{
    tid = generateUID();
    auto transfer = std::make_shared<OutgoingFileTransfer>(tid, info, isVCard);
    {
        std::lock_guard<std::mutex> lk {mapMutex_};
...
DataTransferFacade::Impl::createIncomingFileTransfer(const DRing::DataTransferInfo& info, const DRing::DataTransferId& internal_id, bool isVCard)
{
    auto tid = generateUID();
    auto transfer = std::make_shared<IncomingFileTransfer>(tid, info, internal_id, isVCard);
    {
        std::lock_guard<std::mutex> lk {mapMutex_};
        map_.emplace(tid, transfer);
    }
                             ...
                             bool isVCard) noexcept
{
    ...
    try {
        pimpl_->createOutgoingFileTransfer(info, tid, isVCard);
...
DRing::DataTransferId
DataTransferFacade::createIncomingTransfer(const DRing::DataTransferInfo &info, const DRing::DataTransferId& internal_id, bool isVCard)
{
    auto transfer = pimpl_->createIncomingFileTransfer(info, internal_id, isVCard);
    if (!transfer)
        return {};
