...
public:
    ...
    void insert(SeqNum::seqnum_t begin, SeqNum::seqnum_t end);
    void remove(SeqNum::seqnum_t seq);

private:
    NON_COPYABLE(LossRecorder);
    std::list<std::pair<SeqNum::seqnum_t, SeqNum::seqnum_t>> records_ {};
    std::size_t limit_;
...
void
LossRecorder::insert(UNUSED SeqNum::seqnum_t first, UNUSED SeqNum::seqnum_t last)
{
    // TODO
}

void
LossRecorder::remove(UNUSED SeqNum::seqnum_t seq)
{
    // TODO
}
...
DataConnection::connect(tls::TlsSession* tls)
{
    ...
    lossRecords_.reset(new LossRecorder());
...
DataConnection::idleJob()
{
    ...
    if (delta > 5) {
        RING_WARN("Pkt: %.1f/s, Bytes: %.1f/s, Loss: %.1f/s, Bad:%zu",
                  rxPkt_ / delta, rxBytes_ / delta, rxLossPkt_ / delta, rxBadPkt_);
        statSync_ = now;
        rxPkt_ = rxLossPkt_ = rxBytes_ = rxBadPkt_ = 0;
    }
}
...
// Unprotected method, but SHALL never be called in parallel. Up to caller to handle that
std::shared_ptr<std::vector<uint8_t>>
DataConnection::nextTxPacket(TimePoint& when)
{
    ...
    // Priority to lost packets
    {
        ...
        pkt = txBuffer_->getSentPacket(txLossList_->getFirst());
    }
...
DataConnection::sendNack(const std::array<uint32_t, 2>& data, int len)
{
    ...
    if (len == 1)
        ...
        Packet::initCtrl(pkt, PacketType::NACK, 0, &data[0], len);
...
{
    std::vector<uint8_t> pkt;
    Packet::initCtrl(pkt, PacketType::ACK, lastRxSeqNum_);
    txQueue_->send(pkt);
}
    ...
    switch (Packet::getCtrlType(pkt)) {
        case PacketType::HANDSHAKE:
        {
            ...
            if (handshakeState_ == 0) {
                ...
                // enable rx data
                rxBuffer_.reset(new RxBuffer(2000));
                handshakeState_ = 1;
            }

            if (peer_state > 0) {
                // enable tx data
                txBuffer_.reset(new TxBuffer(2000, maxPayload_));
                handshakeState_ = 2;
                handshakeComplete_ = true;
            }
        ...
        case PacketType::ACK:
        {
            auto ack_seq = Packet::getCtrlAddInfo(pkt);
            //RING_DBG("rx: ACK 0x%08x", ack_seq);
            txBuffer_->remove(ack_seq);
        }
        break;
...
DataConnection::processDataPacket(std::vector<uint8_t>&& pkt)
{
    ...
    auto offset = SeqNum::offset(lastRxAck_, seq);
    if (offset < 0)
        return;
    ...
    if (SeqNum::cmp(seq, waited_rx_seq) > 0) {
        // record loss range
        lossRecords_->insert(waited_rx_seq, SeqNum::decrease(seq));
