...
static constexpr int MAX_ACK_CT = 15; // Channel packet format uses 4 bits to encode AckCt
static constexpr auto HEARTBEAT_DELAY = std::chrono::milliseconds(0);
...
DataConnection::sendPkt(std::vector<uint8_t>& pkt)
{
    ...
    Packet::setAsnAckCnt(pkt, txAckSeq_, txAckCt_);
    //Packet::dump(pkt);
...
DataConnection::sendData(const std::vector<uint8_t>& payload)
{
    ...
    std::vector<uint8_t> pkt;
    Packet::initData(pkt, payload.data(), payload.size(), dataOffset_);
    if (txBuffer_->push(std::move(pkt))) {
        dataOffset_ += payload.size();
        txQueue_->update();
        return true;
    }
    return false;
...
void
DataConnection::sendNack(const std::array<uint32_t, 2>& data)
{
    std::vector<uint8_t> pkt;
    if (data[0] == data[1])
        Packet::initCtrl(pkt, PacketType::NACK, 0, &data[0], 4);
    else
        Packet::initCtrl(pkt, PacketType::NACK, 0, &data[0], 8);
...
DataConnection::onPeerHandshake(int protocol, const uint32_t* peer_data)
{
    ...
    RING_DBG("[FTP:%lx] peer handshake (%u), MSS=%u", id_, peer_data[2], maxPayload_);
}
#endif
...
DataConnection::processPacket(std::vector<uint8_t>&& pkt)
{
    ...
    SeqNum peer_seq = rxSeq_ + seq_delta;
    ...
    if (seq_delta <= 0) {
        if (seq_delta <= MISS_ORDERING_LIMIT) {
            RING_WARN("[FTP:%zu] drop too old pkt %016lx", id_, peer_seq);
            ++rxOldPkt_;
            return;
        }
    ...
    // TODO: do this on a regulare pace, but not systematic at each rx pkt
    std::lock_guard<std::mutex> lk {txMtx_};
    int32_t ack_seq_delta = peer_seq - txAckSeq_;
    RING_DBG("ack_seq_delta: %d", ack_seq_delta);
    if (ack_seq_delta == 1) {
        txAckSeq_ = peer_seq;
        txAckCt_ = std::max(txAckCt_ + 1, MAX_ACK_CT);
