...
DataConnection::idleJob()
{
    ...
    if (delta > 5) {
        ...
        float cum_pps;
        float cum_loss;
        std::size_t cum_tx_bytes, ptx, pl, pf;

        {
            std::lock_guard<std::mutex> lk {txMtx_};
            //cum_rtt = cast_to_ms(cumRtt_).count();
            //cum_rtt_var = cast_to_ms(cumRtt_).count();
            //cum_pps = cumPps_;
            //cum_loss = cumLoss_;
            //cum_tx_bytes = cumTxBytes_;
        ...
        //RING_WARN("[TX] RTT (ms): %u, RTT var.: %.1f, PPS: %.1f, Power: %.1f, Loss: %u, Bw: %.3fMb/s",
        //          cum_rtt, cum_rtt_var * 1e-3, cum_pps, cum_pps / cum_rtt,
        //          static_cast<unsigned>(cum_loss / delta), cum_tx_bytes / delta * 8e-6);
        //RING_WARN("[RX] Bytes: %.3fMbits/s, Bad: %zu, Rx Loss/OoO: %zu",
        //          rxBytes_.load() * 8 / delta / 1e6, rxBadPkt_.load(), rxLossPkt_.load());
        RING_WARN("Pkt: %zu, late:%zu, forced:%zu", ptx, pl, pf);
        ...
        // Old missed packets are re-sent in priority over new packets.
        {
            ...
            if (!ackWaitMap_.empty()) {
                ...
                if (txBuffer_->empty()) {
                    ...
                    ++pktForced_;
                    txDelay_ = (cumRtt_ * 7 + txDelay_) / 8;
                    goto end;
                }

                auto delay = ((Clock::now() - pair->second->time_tx) - cumRtt_).count();
                if (delay > 0) {
                    pkt = std::move(pair->second);
                    ackWaitMap_.erase(pair);
                    ++pktLate_;
                    txDelay_ = (cumRtt_ / 2 + std::chrono::microseconds(300 * 15)) / 16;
                    goto end;
                }
            }
            ...
            if (!pkt and txBuffer_) {
                pkt = txBuffer_->pop();
                ++pktSnd_;
                txDelay_ = std::chrono::microseconds(300);
            }
        }
        if (pkt)
...
DataConnection::processAcknowledgmentFields(const std::vector<uint8_t>& pkt, Clock::time_point tp)
{
    ...
    if (ack_seq >= netStatSeq_) {
        ...
        auto cum_rtt_us = ((cumRtt_.count() * 7) + rtt) / 8;
...
ssize_t
DataStream::recvData(uint8_t*, std::size_t)
{
    std::unique_lock<std::mutex> lk {mtx_};
    if (!dc_) {
        errno = ENOTCONN;
...
void
DataStream::onRxPayload(const uint8_t*, std::size_t length, uint64_t offset)
{
    // Note: Called in the context of RxQueue's thread
    RING_ERR("recv: %zu %lu", length, offset);
}
