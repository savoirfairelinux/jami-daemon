...
IncomingFile::process()
{
    ...
    channel_->onShutdown([w = weak_from_this()](const std::error_code& /*error_code*/) {
        ...
        if (correct) {
            ...
            } else if (destinationStatus.type() != std::filesystem::file_type::not_found) {
                                    ...
                                    bool independent)
{
    auto canonicalPath = path(fileId);
    {
        ...
        auto canonicalIsValid = !ec && isExpectedFile(canonicalPath, sha3sum, total);
        ...
        if (!canonicalIsValid || needsIndependentStorage) {
            ...
            if (ec || stagedStatus.type() != std::filesystem::file_type::not_found) {
                                 ...
                                 std::size_t total)
{
    ...
    std::error_code ec;
    if (std::filesystem::symlink_status(temporaryPath(fileId, destination), ec).type()
