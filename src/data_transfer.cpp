...
void
TxQueue::init(tls::TlsSession* tls, SeqNum::seqnum_t seq)
{
    tls_ = tls;
    txSeq_ = seq;
...
ssize_t
TxQueue::send(DataConnection::Packet& pkt)
{
    std::lock_guard<std::mutex> lk {txMutex_};
    auto& h = pkt.getHeader();
    h.seq = txSeq_;
    txSeq_ = SeqNum::increase(txSeq_);
    return tls_->send(pkt.data_);
}

void
TxQueue::schedule()
{
    std::lock_guard<std::mutex> lk {jMutex_};
    scheduled_ = true;
    jCv_.notify_one();
}

void
TxQueue::schedule(const clock::time_point& ts, const PopPacketFunc& func)
{
    std::lock_guard<std::mutex> lk {jMutex_};
    senders_.emplace(func, ts);
    scheduled_ = true;
    jCv_.notify_one();
}

void
TxQueue::schedule(const clock::time_point& ts, PopPacketFunc&& func)
{
    std::lock_guard<std::mutex> lk {jMutex_};
    senders_.emplace(std::move(func), ts);
    scheduled_ = true;
    jCv_.notify_one();
}
...
TxQueue::threadJob()
{
    while (running_) {
        ...
        // wait until next scheduled event or event list changes
        auto& when = senders_.top().when_;
        jCv_.wait_until(lk, when, [this]{ return !running_ or scheduled_; });
        if (!running_)
            return;

        auto& sender = senders_.top(); // in case of queue modifications during previous wait
        DataConnection::Packet pkt;
        auto res = sender.popPacket_(pkt);
        senders_.pop();
