...
private: // Tx related members
    ...
    std::atomic<std::chrono::microseconds> cumRtt_ {std::chrono::milliseconds(500)}; // cumulative RTT
    std::atomic<float> cumRttVar_ {0}; // cumulative RTT variance
    std::atomic<float> cumPps_ {0}; // cumulative packet
    std::atomic<float> cumLoss_ {0}; // cumulative loss packet
    ...
    ssize_t sendPkt(std::vector<uint8_t>&, SeqNum&, SeqNum, unsigned);
    ssize_t sendPkt(std::vector<uint8_t>&, SeqNum&);
...
private: // Rx related members
    ...
    void onAckPkt(SeqNum seq, unsigned count=1);
    void savePkt(SeqNum, std::shared_ptr<TxPacket>);
