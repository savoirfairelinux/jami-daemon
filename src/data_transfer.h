 ...
 * Connection:
 *
 * Represent a reliable data connection with a remote peer, build over a TLS session.
 * A Connection instance is owned by the ctor given transport and must be destroyed by this later.
 *
 */
...
private:
    NON_COPYABLE(ReliableConnection);

    ReliableConnection(IceTransaction& transport) : transport_(transport) {};

    IceTransaction& transport_;

    friend class IceTransaction;
};
...
private:
    ...
    struct DataPacketHeader {
        PacketHeader head; // type = 0
    };

    struct AckPacketHeader {
        PacketHeader head; // type = 1
    };
...
private:
    IceTransaction& transport_;
    std::shared_ptr<DataTransfer> client_ {};
