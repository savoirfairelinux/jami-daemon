...
/**
 * Build the identifier (and on-disk name) of a transferred file from its commit.
 * @return "<commitId>_<tid>[.<extension>]", or an empty string when tid is not the decimal
 *         transfer id generated by the sender, so a forged commit can never name a file.
 */
std::string getFileId(const std::string& commitId, const std::string& tid, const std::string& displayName);

/**
 * Check that a file transfer identifier, whether built locally or received from a
 * peer, is a plain file name of the form produced by getFileId(): it is joined to
 * the conversation data directory so it MUST NOT contain any path component.
 */
bool isValidFileId(std::string_view fileId) noexcept;
