...
class DataConnection
{
    ...
    static constexpr uint8_t DATATRANSFER_VERSION {0};

public:
#pragma pack(1)
    struct PacketHeader {
        uint8_t version {DATATRANSFER_VERSION};
        uint8_t type {0};
        uint8_t rsv0 {0};
        uint32_t seq {0};
        uint32_t ack {0};
...
{
public:
    using PopPacketFunc = std::function<bool(DataConnection::Packet&)>;
    using TimePoint = typename T::time_point;

    PktSndNode(const PopPacketFunc& func, const TimePoint& ts) : popPacket_(func), when_(ts) {};
    PktSndNode(PopPacketFunc&& func, const TimePoint& ts) : popPacket_(std::move(func)), when_(ts) {};

    constexpr bool operator > (const PktSndNode<T>& rhs) const {
        return when_ > rhs.when_;
    }
...
public:
    ...
    using PopPacketFunc = PktSndNode<clock>::PopPacketFunc;

    TxQueue() {};
    ~TxQueue();

    void init(tls::TlsSession* tls, SeqNum::seqnum_t seq);
    ssize_t send(DataConnection::Packet& pkt);
    void schedule();
    void schedule(const clock::time_point& ts, const PopPacketFunc& func);
    void schedule(const clock::time_point& ts, PopPacketFunc&& func);
...
private:
    ...
    using Node = PktSndNode<clock>;
    std::priority_queue<Node, std::vector<Node>, std::greater<Node>> senders_ {};
