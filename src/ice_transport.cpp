...
{
    JAMI_DBG("[ice:%p] Destroying ice_strans %p", this, icest_);
    if (icest_) {
        shutDown();
    }
        ...
        for (unsigned i = 1; i <= compCountPerStream_; i++) {
            ...
            if (laddr and laddr.getPort() != 0 and raddr and raddr.getPort() != 0) {
                out << " [" << i << "] " << laddr.toString(true, true) << " ["
                    << selectedCand_[absIdx].localType << "] "
                    << " <-> " << raddr.toString(true, true) << " ["
                    << selectedCand_[absIdx].remoteType << "] " << '\n';
            } else {
                out << " [" << i << "] disabled\n";
            }
        }
...
IceTransport::Impl::updateSelectedCandidates()
{
    ...
    for (unsigned compId = 1; compId <= compCount_; compId++) {
        ...
        selectedCand_[compId].localType = sess->lcand ? pj_ice_get_cand_type_name(sess->lcand->type) : "?";
        selectedCand_[compId].remote = sess->rcand->addr;
        selectedCand_[compId].remoteType = sess->rcand ? pj_ice_get_cand_type_name(sess->rcand->type) : "?";
    }
}
...
IceTransport::~IceTransport()
{
    ...
    JAMI_DBG("[ice:%p] Waiting for shutdown ...", pimpl_.get());

    if (waitForShutDown()) {
        JAMI_DBG("[ice:%p] Shutdown completed", pimpl_.get());
    } else {
        JAMI_ERR("[ice:%p] Shutdown timed-out", pimpl_.get());
    }
}
...
{
    JAMI_DBG("[ice:%p] Shuting down ...", pimpl_.get());
    cancelOperations();
    pimpl_->shutDown();
