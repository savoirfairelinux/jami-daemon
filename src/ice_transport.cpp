...
IceTransport::handleEvents(unsigned max_msec)
{
    ...
    do {
        ...
        if (n_events < 0) {
            ...
            last_errmsg = sip_utils::sip_strerror(err);
            RING_DBG("IceIOQueue: error %d - %s", err, last_errmsg.c_str());
...
IceTransport::setInitiatorSession()
{
    ...
    if (isInitialized()) {
        ...
        if (status != PJ_SUCCESS) {
            last_errmsg = sip_utils::sip_strerror(status);
            RING_ERR("ICE role change failed: %s", last_errmsg.c_str());
            return false;
        }
        return true;
    }
                    ...
                    const std::vector<IceCandidate>& rem_candidates)
{
    ...
    if (status != PJ_SUCCESS) {
        last_errmsg = sip_utils::sip_strerror(status);
        RING_ERR("ICE start failed: %s", last_errmsg.c_str());
        return false;
    }
    return true;
}
...
IceTransport::stop()
{
    if (isStarted()) {
        ...
        if (status != PJ_SUCCESS) {
            last_errmsg = sip_utils::sip_strerror(status);
            RING_ERR("ICE stop failed: %s", last_errmsg.c_str());
            return false;
        }
    }
    return true;
...
IceTransport::addReflectiveCandidate(int comp_id, const IpAddr& base, const IpAddr& addr)
{
    ...
    if (ret != PJ_SUCCESS) {
        last_errmsg = sip_utils::sip_strerror(ret);
...
IceTransport::send(int comp_id, const unsigned char* buf, size_t len)
{
    ...
    if (status != PJ_SUCCESS) {
        if (status == PJ_EBUSY) {
            ...
            last_errmsg = sip_utils::sip_strerror(status);
            RING_ERR("ice send failed: %s", last_errmsg.c_str());
            errno = EIO;
        }
        return -1;
    }
