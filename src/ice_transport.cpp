...
struct IceSTransDeleter
{
    void operator()(pj_ice_strans* ptr)
    {
        JAMI_INFO("[ice:%p] Destroying ice_strans %p",
                  pj_ice_strans_get_user_data(ptr), ptr);
...
IceTransport::Impl::~Impl()
{
    ...
    /*
     * Make sure that `onComplete()` has not reset the transport again by
     * accident.  Otherwise this would be a double destroy of the transport!
     */
    if (icest_) {
        JAMI_ERR("[ice:%p] Double free of ice_strans %p. This is a bug!",
                 this, icest_.get());
        icest_.release();
    }
...
IceTransport::Impl::onComplete(pj_ice_strans* ice_st, pj_ice_strans_op op, pj_status_t status)
{
...

    {
        ...
        if (not icest_.get() and not threadTerminateFlags_) {
            JAMI_INFO("[ice:%p] Using ice_strans %p", this, ice_st);
            icest_.reset(ice_st);
        }
    }
