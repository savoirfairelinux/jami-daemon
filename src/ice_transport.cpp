...
IceTransport::getCandidateFromSDP(const std::string& line, IceCandidate& cand)
{
    ...
    cnt = sscanf(line.c_str(), "%s %d %s %d %s %d typ %s tcptype %s\n",
                 ...
                 tcp_type);
    if (cnt != 7 && cnt != 8) {
      RING_WARN("[ice:%p] Invalid ICE candidate line", this);
      return false;
    }
    ...
    if (is_tcp) {
      if (strcmp(tcp_type, "active") == 0)
        ...
        RING_WARN("[ice:%p] invalid transport type type '%s'", this, tcp_type);
        return false;
      }
    } else {
    ...
    if (status != PJ_SUCCESS) {
      RING_WARN("[ice:%p] invalid IP address '%s'", this, ipaddr);
      return false;
    }
...
ssize_t
IceTransport::waitForData(int comp_id, long int timeout, std::error_code& ec)
{
    ...
    if (timeout != -1 &&
        !io.cv.wait_for(lk, std::chrono::milliseconds(timeout), [this, &io]() {
          return !io.queue.empty() or !isRunning();
        }))
        return 0;

    if (timeout == -1 && isRunning() && io.queue.empty())
      io.cv.wait(lk);
    else if (!isRunning())
