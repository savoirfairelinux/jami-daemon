...
IceTransport::startIce(const SDP& sdp)
{
    if (pimpl_->streamsCount_ != 1) {
        JAMI_ERR("This method expects exactly one stream per SDP (found %u streams)",
                 pimpl_->streamsCount_);
        return false;
    }
                                    ...
                                    IceCandidate& cand) const
{
    ...
    // TODO_MC.
    // Check user-agent version to know which compId format is used ?
