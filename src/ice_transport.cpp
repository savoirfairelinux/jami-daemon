...
public:
    ...
    bool aggressiveNomination_ {false};
...
IceTransport::Impl::Impl(const char* name, const IceTransportOptions& options)
    ...
    , aggressiveNomination_(options.aggressive)
...
IceTransport::Impl::initIceInstance()
{
    ...
    pj_status_t status
        = pj_ice_strans_create(sessionName_.c_str(), &config_, compCount_, this, &icecb, &icest_);
