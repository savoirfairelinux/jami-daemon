...
public:
    ...
    const std::string getCandidateType(const pj_ice_sess_cand* cand) const;
...
{
    auto cand = getSelectedCandidate(comp_id, false);
...
{
    auto cand = getSelectedCandidate(comp_id, true);
...
{
    const char* type = pj_ice_get_cand_type_name(cand->type);
    if (type == nullptr)
        return { "?" };
    return { type };
}
