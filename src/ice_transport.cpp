...
public:
    ...
    void requestUpnpMappings(uintptr_t ice_session_id);
    ...
    , iceDefaultRemoteAddr_(component_count)
{
    ...
    requestUpnpMappings(reinterpret_cast<uintptr_t>(this));
...
// TODO. comp_id is actually comp index. Fix it.
IpAddr
...
void
IceTransport::Impl::requestUpnpMappings(uintptr_t ice_session_id)
{
    // Must be called once !

    std::stringstream descr;
    descr << ice_session_id;

    JAMI_DBG("[ice:%p]: Setup local host/upnp candidates for ICE session ID %lu",
        this, ice_session_id);

    IpAddr localIp {};
    IpAddr publicIp {};

    // Curly-brackets to limit the scope of the locked mutex
    {
        ...
        for (unsigned compId = 1; compId <= component_count_; compId++) {
            ...
            Mapping map {0, 0, portType, descr.str()};
            // Request the mapping
