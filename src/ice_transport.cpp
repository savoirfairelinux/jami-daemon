...
IceTransport::_isInitialized() const
{
    if (auto icest = icest_.get())
        return pj_ice_strans_get_state(icest) >= PJ_ICE_STRANS_STATE_SESS_READY;
    return false;
}
...
{
    if (auto icest = icest_.get())
        return pj_ice_strans_get_state(icest) >= PJ_ICE_STRANS_STATE_NEGO;
    return false;
}
...
{
    if (auto icest = icest_.get())
        return pj_ice_strans_get_state(icest) >= PJ_ICE_STRANS_STATE_RUNNING;
    return false;
}
