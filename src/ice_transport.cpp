...
IceTransport::Impl::setupHostAndUpnpCandidates()
{
    ...
    for (auto const& [_, map] : upnpMappings_) {
        ...
        IpAddr publicIp {map.getExternalAddress()};
        publicIp.setPort(map.getExternalPort());
        IpAddr localIp {map.getInternalAddress()};
        localIp.setPort(map.getInternalPort());
