...
toMap(const std::string& content)
{
    std::map<std::string, std::string> vCard;

    std::stringstream ss(content);
    std::string line, previousValue;
    while (std::getline(ss, line, '\n')) {
        if (line.size()) {
            // Some properties are over multiple lines
            if (line[0] == ' ') {
                previousValue += line.substr(1);
            } else {
                previousValue = "";
            }
            ...
            if (dblptPos == std::string::npos)
                continue;
            const std::string k(line.substr(0, dblptPos)), v(line.substr(dblptPos + 1));
            vCard[k] = v;
        }
    }
    return vCard;
