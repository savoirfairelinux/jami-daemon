...
public:
    ...
    ~CallStreamsManager() = delete;

    // General
    void setLayout(int layout);
    Layout getLayout() const { return currentLayout_; }

    void setOnInfoUpdated(std::function<void(const StreamInfoMap& info)> onInfoUpdated) {
        onInfoUpdated_ = std::move(onInfoUpdated);
    }

    // Per streams
    void setVoiceActivity(const std::string& streamId, bool newState);
    void muteStream(const std::string& streamId, bool newState);
    void setActiveStream(const std::string& streamId, bool newState);
    virtual void detachStream(const std::string& streamId);

    // Per calls
    void bindCall(const std::shared_ptr<SIPCall>& call);
    void setHandRaised(const std::string& uri, const std::string& deviceId, bool newState);
    void setRecording(const std::string& uri, const std::string& deviceId, bool newState);
    void setModerator(const std::string& uri, const std::string& deviceId, bool newState);
    void removeCall(const std::string& uri, const std::string& deviceId);
