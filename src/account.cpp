...
Account::setAccountDetails(const std::map<std::string, std::string>& details)
{
    ...
    enableUpnp(upnpEnabled_ && isEnabled());
    parseString(details, Conf::CONFIG_DEFAULT_MODERATOR, defaultModerators_);
    parseBool(details, Conf::CONFIG_LOCAL_MODERATOR_ENABLED, localModeratorsEnabled_);
}
...
Account::getAccountDetails() const
{
    return {{Conf::CONFIG_ACCOUNT_ALIAS, alias_},
            ...
            {Conf::CONFIG_DEFAULT_MODERATOR, defaultModerators_},
            {Conf::CONFIG_LOCAL_MODERATOR_ENABLED, localModeratorsEnabled_ ? TRUE_STR : FALSE_STR}};
}
