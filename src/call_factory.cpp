...
std::shared_ptr<Call>
CallFactory::createCall(const std::shared_ptr<Account>& account,
                        const std::string& id,
                        Call::CallType type,
...
{
    std::lock_guard<std::recursive_mutex> lk(callMapsMutex_);
    const auto map = getMap(link);
    return !map or map->empty();

}

std::shared_ptr<Call>
CallFactory::getCall(const std::string& id, Call::LinkType) const
{
    std::lock_guard<std::recursive_mutex> lk(callMapsMutex_);
...
const CallMap* CallFactory::getMap(Call::LinkType link) const
{
    std::lock_guard<std::recursive_mutex> lk(callMapsMutex_);
    const auto& itermap = callMaps_.find(link);
