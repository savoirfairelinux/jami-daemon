...
ChatServicesManager::registerChatService(PluginManager& pluginManager)
{
    ...
    auto sendTextMessage = [](const DLPlugin*, void* data) {
        const auto* cm = static_cast<JamiMessage*>(data);
    ...
    auto clearSwarmBodyOverwriteConversation = [this](const DLPlugin* plugin, void* data) {
        const auto* p = static_cast<std::pair<std::string, std::string>*>(data);
        if (!p) {
            return 0;
        }
        if (const auto enabled = getEnabledConversationsForPlugin(plugin);
    ...
    // data: std::pair<std::string,std::string>* {accountId, conversationId}
    auto bodyOverwriteLoadedConversation = [this](const DLPlugin* plugin, void* data) {
        const auto* p = static_cast<std::pair<std::string, std::string>*>(data);
        if (!p) {
            return 0;
        }
