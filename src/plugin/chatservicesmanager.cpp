...
ChatServicesManager::registerComponentsLifeCycleManagers(PluginManager& pluginManager)
{
    ...
    auto registerChatHandler = [this](void* data, std::mutex& pmMtx_) {
        ...
        PluginPreferencesUtils::addAlwaysHandlerPreference(handlerName,
                                                           ptr->id().substr(0,
                                                                            ptr->id().find_last_of(DIR_SEPARATOR_CH)));
        ...
        std::vector<NotifyItem> toNotify;
        {
            ...
            for (auto& [key, allowDenySet] : allowDenyList_) {
                if (const auto it = allowDenySet.find(handlerName); it == allowDenySet.end() || !it->second) {
                    continue;
                }
    ...
    auto unregisterChatHandler = [this](void* data, std::mutex& pmMtx_) {
        ...
        std::vector<std::pair<std::string, std::string>> toClearOverwrites;
        {
            ...
            const auto handlerIt = std::ranges::find_if(chatHandlers_,
                                                        [data](const ChatHandlerPtr& h) { return h.get() == data; });
        ...
        // clearBodyOverwrites via invokeService fails after handler is erased from
        // chatHandlers_ (getEnabledConversationsForPlugin returns empty). Clear directly.
...
{
    pluginManager.registerService("sendTextMessage", [](const DLPlugin* p, void* d) {
        return sendTextMessage(p, static_cast<JamiMessage*>(d));
    });
    pluginManager.registerService("editMessage", [this](const DLPlugin* p, void* d) {
        return editMessage(p, static_cast<const JamiMessage*>(d));
    });
    pluginManager.registerService("reloadBodyOverwritesAllConversations", [this](const DLPlugin* p, void* d) {
        return reloadBodyOverwritesAllConversations(p, d);
    });
    pluginManager.registerService("updateMessageBodyOverwrite", [this](const DLPlugin* p, void* d) {
        return updateMessageBodyOverwrite(p, static_cast<const MessageBodyOverwriteUpdate*>(d));
    });
    pluginManager.registerService("clearBodyOverwritesAllConversations", [this](const DLPlugin* p, void* d) {
        return clearBodyOverwritesAllConversations(p, d);
    });
    pluginManager.registerService("loadMissingBodyOverwritesAllConversations", [this](const DLPlugin* p, void* d) {
        return loadMissingBodyOverwritesAllConversations(p, d);
    });
    pluginManager.registerService("clearBodyOverwrites", [this](const DLPlugin* p, void* d) {
        return clearBodyOverwrites(p, static_cast<const std::pair<std::string, std::string>*>(d));
    });
    pluginManager.registerService("loadMissingBodyOverwrites", [this](const DLPlugin* p, void* d) {
        return loadMissingBodyOverwrites(p, static_cast<const std::pair<std::string, std::string>*>(d));
    });
}
...
int
ChatServicesManager::reloadBodyOverwritesAllConversations(const DLPlugin* plugin, void*)
{
...
int
ChatServicesManager::clearBodyOverwritesAllConversations(const DLPlugin* plugin, void*)
{
    for (const auto& [accountId, convId] : getEnabledConversationsForPlugin(plugin)) {
        if (const auto acc = Manager::instance().getAccount<JamiAccount>(accountId)) {
            if (auto* convMod = acc->convModule()) {
...
int
ChatServicesManager::loadMissingBodyOverwritesAllConversations(const DLPlugin* plugin, void*)
{
    for (const auto& [accountId, convId] : getEnabledConversationsForPlugin(plugin)) {
        if (const auto acc = Manager::instance().getAccount<JamiAccount>(accountId)) {
            if (auto* convMod = acc->convModule()) {
...
ChatServicesManager::editMessage(const DLPlugin* plugin, const JamiMessage* cm)
{
    ...
    if (const auto enabled = getEnabledConversationsForPlugin(plugin); !enabled.contains({cm->accountId, cm->peerId})) {
...
int
ChatServicesManager::clearBodyOverwrites(const DLPlugin* plugin, const std::pair<std::string, std::string>* p)
{
    if (!p) {
        return 0;
    }
    if (const auto enabled = getEnabledConversationsForPlugin(plugin); !enabled.contains({p->first, p->second})) {
        return -1;
    }
...
int
ChatServicesManager::loadMissingBodyOverwrites(const DLPlugin* plugin, const std::pair<std::string, std::string>* p)
{
    if (!p) {
        return 0;
    }
...
{
    // Snapshot active handlers under mtx_, then call outside: a plugin's
    // transformSwarmMessages may invoke a service (e.g., clearBodyOverwritesAllConversations)
    // which calls getEnabledConversationsForPlugin → mtx_ → deadlock.
    std::vector<ChatHandlerPtr> toCall;
    {
...
ChatServicesManager::publishMessage(const pluginMessagePtr& message)
{
    ...
    using ActivateItem = std::pair<ChatHandlerPtr, chatSubjectPtr>;
...

    {
        ...
        for (auto& chatHandler : chatHandlers_) {
            ...
            bool toggle = PluginPreferencesUtils::getAlwaysPreference(
                ...
                chatHandlerName,
                message->accountId);
            if (const auto allowedIt = chatAllowDenySet.find(chatHandlerName); allowedIt != chatAllowDenySet.end()) {
            ...
            if (toggle || toggled) {
                ...
                if (!toggled) {
                    ...
                    toActivate.emplace_back(chatHandler, subject);
                }
                shouldPublish = true;
            }
        }
...
ChatServicesManager::setPreference(const std::string& key, const std::string& value, const std::string& rootPath)
{
    ...
    // setPreferenceAttribute callbacks into invokeService (clearBodyOverwritesAllConversations,
    // reloadBodyOverwritesAllConversations) which acquires mtx_ — holding it here deadlocks.
    std::vector<ChatHandlerPtr> toNotify;
    {
        std::lock_guard const lk(mtx_);
        for (auto& chatHandler : chatHandlers_) {
            if (chatHandler->id().find(rootPath) != std::string::npos && chatHandler->preferenceMapHasKey(key)) {
                toNotify.push_back(chatHandler);
            }
        }
    } // release mtx_

    for (const auto& handler : toNotify) {
