...
namespace jami {
std::vector<MapStrStr> PluginPreferencesManager::parse(const std::string &preferenceFilePath)
{
    ...
    if(file) {
        ...
        if(ok && root.isArray()) {
            for(int i=0; i< static_cast<int>(root.size()); i++) {
                ...
                if(type != "None") {
                    preferences.push_back(parsePreference(jsonPreference, type, category));
                }
            }
        } else {
            JAMI_ERR() << "PluginPreferencesManager:: Failed to parse preferences.json for plugin: "
                       << preferenceFilePath;
        }
    }
...
MapStrStr PluginPreferencesManager::parsePreference(const Json::Value& jsonPreference,
...
std::string PluginPreferencesManager::convertArrayToString(const Json::Value jsonArray)
