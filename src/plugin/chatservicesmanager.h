...
private:
    ...
    int reloadBodyOverwritesAllConversations(const DLPlugin* plugin, void*);
    int updateMessageBodyOverwrite(const DLPlugin* plugin, const MessageBodyOverwriteUpdate* upd);
    int clearBodyOverwritesAllConversations(const DLPlugin* plugin, void*);
    int loadMissingBodyOverwritesAllConversations(const DLPlugin* plugin, void*);
    int editMessage(const DLPlugin* plugin, const JamiMessage* cm);
    int clearBodyOverwrites(const DLPlugin* plugin, const std::pair<std::string, std::string>* p);
    int loadMissingBodyOverwrites(const DLPlugin* plugin, const std::pair<std::string, std::string>* p);

    // Protects chatHandlers_, chatHandlerToggled_, allowDenyList_, and handlersNameMap_.
    // Must NOT be held while calling into plugin code (notifyChatSubject, detach,
    // transformSwarmMessages, setPreferenceAttribute, publish): those can call back
    // into ChatServicesManager services which re-acquire mtx_ and deadlock.
    // Pattern: snapshot pointers under mtx_, release, then call plugin code.
