...
public:
    void registerComponentsLifeCycleManagers(PluginManager& pm)
    {
        auto registerChatHandler = [this](void* data) {
            ...
            PluginPreferencesManager::addAlwaysHandlerPreference(ptr->getChatHandlerDetails().at("name") + "Always", ptr->id().substr(0, found));
        ...
        for (auto& chatHandler : chatHandlers_) {
            ...
            if ((preferences.at(chatHandlerName+"Always") == "1" || toggled) && !denyListed) {
                ...
                if (!toggled) {
                    ...
                    PluginPreferencesManager::setAllowDenyListPreferences(allowList_);
                }
    ...
    void setAllowDenyListsFromPreferences() {
        PluginPreferencesManager::getAllowDenyListPreferences(allowList_);
        PluginPreferencesManager::getAllowDenyListPreferences(denyList_, false);
    }
...
private:
                           ...
                           const bool toggle)
    {
        ...
        if (chatHandlerIt != chatHandlers_.end()) {
            ...
            PluginPreferencesManager::setAllowDenyListPreferences(allowList_);
            PluginPreferencesManager::setAllowDenyListPreferences(denyList_, false);
        }
    }
