...
public:
    ...
    /**
     * @brief Called by the daemon to get information about the plugin
     * @returns a map<string, string> with the handler's name, iconPath, and pluginId (actually the
     *          datapath) as keys
     */
     ...
     * @returns The local path to an HTML file that the client webview should load
     */
