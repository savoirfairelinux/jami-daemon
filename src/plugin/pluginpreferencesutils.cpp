...
PluginPreferencesUtils::getPreferences(const std::filesystem::path& rootPath, const std::string& accountId)
{
    ...
    if (file) {
        ...
        auto locales = PluginUtils::getLocales(rootPath.string(), std::string(string_remove_suffix(lang, '.')));
                ...
                if (type != "None" && key != "None") {
                    if (keys.find(key) == keys.end()) {
                        ...
                        if (type == "Path" && defaultValue != preferenceAttributes.end()) {
                            ...
                            defaultValue->second = (rootPath / defaultValue->second).string();
                        }
                                               ...
                                               const std::string& value)
{
    ...
    if (std::ifstream rfs(preferencesValuesFilePath, std::ios::binary); rfs.good()) {
        ...
        if (const auto fileSize = static_cast<size_t>(rfs.tellg());
            fileSize > 0) {
