...
{
    JAMI_INFO() << "PluginPreferencesUtils::processLocaleFile: " << preferenceLocaleFilePath;
...
{
    JAMI_INFO() << "PluginPreferencesUtils::getLocales: " << lang;
...
PluginPreferencesUtils::getPreferences(const std::string& rootPath, const std::string& accountId)
{
    ...
    if (file) {
        ...
        std::string lang = std::setlocale(LC_ALL, NULL);
        lang = std::string(string_remove_suffix(lang, '_'));
        JAMI_INFO() << "\n\n\n\n\n\nLanguage: " << lang;
        auto locales = getLocales(rootPath, lang);
