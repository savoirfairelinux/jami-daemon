...
std::string
PluginPreferencesManager::getPreferencesConfigFilePath(const std::string& rootPath)
{
    return rootPath + DIR_SEPARATOR_CH + "data" + DIR_SEPARATOR_CH + "preferences.json";
}

std::string
PluginPreferencesManager::valuesFilePath(const std::string& rootPath)
{
...
PluginPreferencesManager::parsePreferenceConfig(const Json::Value& jsonPreference, const std::string& type)
{
    ...
    if (file) {
        ...
        if (ok && root.isArray()) {
            for (unsigned i = 0; i < root.size(); i++) {
                ...
                if (type != "None" && type != "always" && key != "None") {
...
std::map<std::string, std::string>
PluginPreferencesManager::getUserPreferencesValuesMap(const std::string& rootPath)
{
...
std::map<std::string, std::string>
PluginPreferencesManager::getPreferencesValuesMap(const std::string& rootPath)
{
    std::map<std::string, std::string> rmap;
...
bool
PluginPreferencesManager::resetPreferencesValuesMap(const std::string& rootPath)
{
...
PluginPreferencesManager::addAlwaysHandlerPreference(const std::string& handlerName, const std::string& rootPath) {
    ...
    for (auto preference : preferences){
        if (preference.at("type") == "always" && preference.at("key").find(handlerName) != std::string::npos)
            return;
    }
...
PluginPreferencesManager::getAlwaysPreference(const std::string rootPath, std::string& handlerName)
{
    ...
    for (auto preference : preferences) {
        ...
        if (preference.at("type") == "always" && key.find(handlerName) != std::string::npos && preferencesValues.find(key)->second == "1")
            return true;
    }
