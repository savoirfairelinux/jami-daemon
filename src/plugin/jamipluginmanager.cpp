...
static const std::regex DATA_REGEX("^data" ESCAPED_DIR_STR ".+");
static const std::regex SO_REGEX("([a-z0-9]+(?:[_-]?[a-z0-9]+)*)" ESCAPED_DIR_STR "([a-z0-9_]+\\.(so|dll))");

auto  uncompressJplFunction = [](const std::string& relativeFileName) {
    std::smatch match;
