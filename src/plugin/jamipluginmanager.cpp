...
/*!
 * \brief Parses a manifest file and return it's content
...
JamiPluginManager::getInstalledPlugins()
{
    ...
    /// Gets plugins installed in non stardand path
 ...
 * Plugin is reloaded only if the preference cannot take effect immediatly.
...
/*!
 * \brief Register services than can be called from plugin implementation side.
 */
void
JamiPluginManager::registerServices()
{
    ...
    /// Rgister getPluginDataPath so that plugin's can receive the path to it's data folder
