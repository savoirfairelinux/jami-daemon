...
#define	PLUGIN_ALREADY_INSTALLED	  100	/* Found end of archive. */
#define	PLUGIN_OLD_VERSION	  200	/* Operation was successful. */
...
{
    try {
        std::vector<uint8_t> manifestData = archiver::readFileFromArchiveToStream(jplPath,"manifest.json");
        std::stringstream vts;
        std::copy(manifestData.begin(), manifestData.end(), std::ostream_iterator<int>(vts));
        return checkManifestValidity(vts);
    } catch (const std::exception& e) {
        JAMI_ERR() << e.what();
    }
}
