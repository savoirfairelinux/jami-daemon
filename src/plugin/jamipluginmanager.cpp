            ...
            for (int i = 0; i < static_cast<int>(root.size()); i++) {
                ...
                if (type != "None" && key != "None") {
                    if (keys.find(key) == keys.end()) {
                        std::map<std::string, std::string> preferenceAttributes = parsePreferenceConfig(jsonPreference,
                                                                                 type);
    ...
    for (auto& preference : preferences) {
        if (preference["key"] == preferenceKey) {
            ...
            if (!copyFileToPluginData(pluginId, value, preference["category"], fileName, fileExt)) {
                return setPluginPreference(pluginId, preferenceKey, fileName);
            }
            setPluginPreference(pluginId, preferenceKey, fileName);
