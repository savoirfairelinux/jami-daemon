...
JamiPluginManager::registerServices()
{
    ...
    // triggering handler callbacks.  Used by plugins to sync the UI display of account
    // preferences when a global setting changes, without creating in-memory overrides.
    // data must point to std::tuple<std::string,std::string,std::string>: {accountId, key, value}.
    pm_.registerService("setPluginAccPreference",
        [](const DLPlugin* plugin, void* data) {
            ...
            if (PluginPreferencesUtils::getPreferencesValuesMap(rootPath, accountId).find(key)
                == PluginPreferencesUtils::getPreferencesValuesMap(rootPath, accountId).end())
                return FAILURE;

            auto userPrefs = PluginPreferencesUtils::getUserPreferencesValuesMap(rootPath, accountId);
            userPrefs[key] = value;
            const auto valuesFile = PluginPreferencesUtils::valuesFilePath(rootPath, accountId);
            std::lock_guard guard(dhtnet::fileutils::getFileLock(valuesFile));
            if (std::ofstream fs(valuesFile, std::ios::binary); fs.good()) {
                msgpack::pack(fs, userPrefs);
                return SUCCESS;
            }
            return FAILURE;
        });
