...
void
deviceAuthStateChanged(const std::string& accountId, int state, const std::string& detail)
{
    std::lock_guard lock(pendingSignalsLock);
    pendingSignals.emplace([accountId, state, detail]() {
        Local<Function> func = Local<Function>::New(Isolate::GetCurrent(), deviceAuthStateChangedCb);
        if (!func.IsEmpty()) {
            SWIGV8_VALUE callback_args[] = {V8_STRING_NEW_LOCAL(accountId),
                                            SWIGV8_INTEGER_NEW(state),
                                            V8_STRING_NEW_LOCAL(detail)};
            func->Call(SWIGV8_CURRENT_CONTEXT(), SWIGV8_NULL(), 3, callback_args);
        }
    });
...
void
addDeviceStateChanged(const std::string& accountId, uint32_t opId, int state, const std::string& detail)
{
    std::lock_guard lock(pendingSignalsLock);
    pendingSignals.emplace([accountId, opId, state, detail]() {
