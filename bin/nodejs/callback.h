...
inline napi_value
napiString(const std::string& str)
{
    napi_value result;
...
inline void
napiSetElement(napi_value arr, uint32_t index, napi_value val)
{
    napi_set_element(g_env, arr, index, val);
}

inline void
napiSetProperty(napi_value obj, const char* key, napi_value val)
{
    napi_set_named_property(g_env, obj, key, val);
}

inline void
napiSetProperty(napi_value obj, const std::string& key, napi_value val)
{
...
void
parseCbMap(napi_env env, napi_value callbackMap)
{
...
void
callCallback(std::string_view signal, size_t argc, const napi_value* argv)
{
...
void
handlePendingSignals(uv_async_t*)
{
...
{
    std::lock_guard lock(pendingSignalsLock);
    pendingSignals.emplace([]() { callCallback("AccountsChanged", 0, nullptr); });
...
registrationStateChanged(const std::string& accountId, const std::string& state, int code, const std::string& detail_str)
{
    ...
    pendingSignals.emplace([accountId, state, code, detail_str]() {
        napi_value args[] = {napiString(accountId), napiString(state), napiInt32(code), napiString(detail_str)};
                       ...
                       int state)
{
    ...
    pendingSignals.emplace([accountId, conversationId, from, state]() {
        napi_value args[] = {napiString(accountId), napiString(conversationId), napiString(from), napiInt32(state)};
                       ...
                       const std::map<std::string, std::string>& payloads)
{
    ...
    pendingSignals.emplace([accountId, from, payloads]() {
        napi_value args[] = {napiString(accountId), napiString(from), stringMapToJsMap(payloads)};
                            ...
                            int state)
{
    ...
    pendingSignals.emplace([account_id, message_id, peer, state]() {
        napi_value args[] = {napiString(account_id), napiString(message_id), napiString(peer), napiInt32(state)};
                   ...
                   const std::vector<std::map<std::string, std::string>>& activeCalls)
{
    ...
    pendingSignals.emplace([account_id, conversationId, activeCalls]() {
        napi_value args[] = {napiString(account_id), napiString(conversationId), stringMapVecToJsMapArray(activeCalls)};
...
deviceAuthStateChanged(const std::string& accountId, int state, const std::map<std::string, std::string>& details)
{
    ...
    pendingSignals.emplace([accountId, state, details]() {
        napi_value args[] = {napiString(accountId), napiInt32(state), stringMapToJsMap(details)};
                      ...
                      const std::map<std::string, std::string>& details)
{
    ...
    pendingSignals.emplace([accountId, opId, state, details]() {
        napi_value args[] = {napiString(accountId), napiUint32(opId), napiInt32(state), stringMapToJsMap(details)};
...
deviceRevocationEnded(const std::string& accountId, const std::string& device, int status)
{
    ...
    pendingSignals.emplace([accountId, device, status]() {
        napi_value args[] = {napiString(accountId), napiString(device), napiInt32(status)};
...
accountProfileReceived(const std::string& accountId, const std::string& displayName, const std::string& photo)
{
    ...
    pendingSignals.emplace([accountId, displayName, photo]() {
        napi_value args[] = {napiString(accountId), napiString(displayName), napiString(photo)};
...
profileReceived(const std::string& accountId, const std::string& from, const std::string& path)
{
    ...
    pendingSignals.emplace([accountId, from, path]() {
        napi_value args[] = {napiString(accountId), napiString(from), napiString(path)};
...
callStateChanged(const std::string& accountId, const std::string& callId, const std::string& state, int detail_code)
{
    ...
    pendingSignals.emplace([accountId, callId, state, detail_code]() {
        napi_value args[] = {napiString(accountId), napiString(callId), napiString(state), napiInt32(detail_code)};
                     ...
                     const std::vector<std::map<std::string, std::string>>& mediaList)
{
    ...
    pendingSignals.emplace([accountId, callId, mediaList]() {
        napi_value args[] = {napiString(accountId), napiString(callId), stringMapVecToJsMapArray(mediaList)};
                ...
                const std::map<std::string, std::string>& messages)
{
    ...
    pendingSignals.emplace([accountId, id, from, messages]() {
        napi_value args[] = {napiString(accountId), napiString(id), napiString(from), stringMapToJsMap(messages)};
...
void
videoOrientationChanged(const std::string& accountId,
                        const std::string& callId,
                        int streamIdx,
                        int rotation)
{
    std::lock_guard lock(pendingSignalsLock);
    pendingSignals.emplace([accountId, callId, streamIdx, rotation]() {
        napi_value args[] = {
            napiString(accountId), napiString(callId), napiInt32(streamIdx), napiInt32(rotation)};
        callCallback("VideoOrientationChanged", 4, args);
    });
    uv_async_send(&signalAsync);
}
                     ...
                     const libjami::SwarmMessage& message)
{
    ...
    pendingSignals.emplace([accountId, conversationId, message]() {
        napi_value args[] = {napiString(accountId), napiString(conversationId), swarmMessageToJs(message)};
                           ...
                           const std::map<std::string, std::string>& profile)
{
    ...
    pendingSignals.emplace([accountId, conversationId, profile]() {
        napi_value args[] = {napiString(accountId), napiString(conversationId), stringMapToJsMap(profile)};
                            ...
                            const std::map<std::string, std::string>& message)
{
    std::lock_guard lock(pendingSignalsLock);
    pendingSignals.emplace([accountId, conversationId, message]() {
        napi_value args[] = {napiString(accountId), napiString(conversationId), stringMapToJsMap(message)};
                        ...
                        int event)
{
    ...
    pendingSignals.emplace([accountId, conversationId, memberUri, event]() {
        napi_value args[] = {napiString(accountId), napiString(conversationId), napiString(memberUri), napiInt32(event)};
                    ...
                    const std::string& what)
{
    ...
    pendingSignals.emplace([accountId, conversationId, code, what]() {
        napi_value args[] = {napiString(accountId), napiString(conversationId), napiUint32(code), napiString(what)};
...
conferenceCreated(const std::string& accountId, const std::string& conversationId, const std::string& confId)
{
    ...
    pendingSignals.emplace([accountId, confId, conversationId]() {
        napi_value args[] = {napiString(accountId), napiString(conversationId), napiString(confId)};
...
conferenceChanged(const std::string& accountId, const std::string& confId, const std::string& state)
{
    ...
    pendingSignals.emplace([accountId, confId, state]() {
        napi_value args[] = {napiString(accountId), napiString(confId), napiString(state)};
                               ...
                               const std::map<std::string, std::string>& preferences)
{
    ...
    pendingSignals.emplace([accountId, convId, preferences]() {
        napi_value args[] = {napiString(accountId), napiString(convId), stringMapToJsMap(preferences)};
...
subscriptionStateChanged(const std::string& accountId, const std::string& buddy_uri, int state)
{
    ...
    pendingSignals.emplace([accountId, buddy_uri, state]() {
        napi_value args[] = {napiString(accountId), napiString(buddy_uri), napiInt32(state)};
                       ...
                       const std::string& displayName)
{
    ...
    pendingSignals.emplace([accountId, buddy_uri, state, displayName]() {
        napi_value args[] = {napiString(accountId), napiString(buddy_uri), napiInt32(state), napiString(displayName)};
                     ...
                     const std::string& line_status)
{
    ...
    pendingSignals.emplace([accountId, buddy_uri, status, line_status]() {
        napi_value args[] = {napiString(accountId), napiString(buddy_uri), napiInt32(status), napiString(line_status)};
...
serverError(const std::string& accountId, const std::string& error, const std::string& msg)
{
    ...
    pendingSignals.emplace([accountId, error, msg]() {
        napi_value args[] = {napiString(accountId), napiString(error), napiString(msg)};
...
initJami(napi_env env, napi_value callbackMap, uint16_t flags = libjami::LIBJAMI_FLAG_DEBUG)
{
    ...
    const std::map<std::string, SharedCallback> dataTransferEvHandlers = {
        exportable_callback<libjami::DataTransferSignal::DataTransferEvent>(
            bind(&dataTransferEvent, _1, _2, _3, _4, _5)),
    };
    ...
    const std::map<std::string, SharedCallback> conversationHandlers
        = {exportable_callback<ConversationSignal::SwarmLoaded>(bind(&swarmLoaded, _1, _2, _3, _4)),
           ...
           exportable_callback<ConversationSignal::ConversationMemberEvent>(
               bind(&conversationMemberEvent, _1, _2, _3, _4)),
           ...
           exportable_callback<ConversationSignal::ConversationPreferencesUpdated>(
               bind(&conversationPreferencesUpdated, _1, _2, _3))};
