...
RestConfigurationManager::populateResources()
{
    ...
    resources_.push_back(std::make_shared<restbed::Resource>());
    resources_.back()->set_path("/isIax2Enable");
    resources_.back()->set_method_handler("GET",
        std::bind(&RestConfigurationManager::isIax2Enabled, this, std::placeholders::_1));
...
void
RestConfigurationManager::isIax2Enabled(const std::shared_ptr<restbed::Session> session)
{
    RING_INFO("[%s] GET /isIax2Enable", session->get_origin().c_str());

    bool status = DRing::isIax2Enabled();
    std::string body = (status ? "true" : "false");
