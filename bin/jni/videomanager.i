...
void rotateNV21(uint8_t* yinput, uint8_t* uvinput, unsigned ystride, unsigned uvstide, unsigned width, unsigned height, int rotation, uint8_t* youtput, uint8_t* uvoutput)
{
    if (rotation == 0) {
        std::copy_n(yinput, width * height, youtput);
        std::copy_n(uvinput, width * height, uvoutput);
        return;
    }
...
JNIEXPORT void JNICALL Java_cx_ring_daemon_RingserviceJNI_captureVideoFrame(JNIEnv *jenv, jclass jcls, jobject image, jint rotation)
{
    ...
    if (avframe->format == AV_PIX_FMT_YUV420P) {
        ...
        if (rotation == 0) {
            ...
            int w = swap ? avframe->height : avframe->width;
            int h = swap ? avframe->width : avframe->height;
            av_image_alloc(avframe->data, avframe->linesize, w, h, (AVPixelFormat)avframe->format, 1);
            rotateNV21(ydata, uvdata, ystride, uvstride, avframe->width, avframe->height, rotation, avframe->data[0], avframe->data[1]);
            avframe->width = w;
            avframe->height = h;

            jenv->CallVoidMethod(image, jenv->GetMethodID(imageClass, "close", "()V"));
            frame->setReleaseCb([avframe](uint8_t *){
