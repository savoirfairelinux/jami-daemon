...
std::map<std::string, std::function<bool(void)>> registered_tasks;
...
public:
    Execute(const std::string& task_name)
    {
        try {
            todo_ = registered_tasks.at(task_name);
        } catch (const std::exception& E) {
            JAMI_ERR("AGENT: Invalid task name %s: %s", task_name.c_str(), E.what());
        }
    }
