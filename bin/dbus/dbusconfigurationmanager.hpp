...
{
public:
    using DBusSwarmMessage = sdbus::Struct<std::string, std::string, std::map<std::string, std::string>, std::vector<std::map<std::string, std::string>>, std::vector<std::map<std::string, std::string>>>;
...
private:
    ...
    registerSignalHandlers()
    {
        ...
        const std::map<std::string, SharedCallback> convEvHandlers = {
            ...
            exportable_serialized_callback<ConversationSignal::SwarmLoaded>([this](const auto& id, const auto& account_id, const auto& conversation_id, const auto& messages) {
                std::vector<DBusSwarmMessage> msgList;
                for (const auto& message: messages) {
                    DBusSwarmMessage msg {message.id, message.type, message.body, message.reactions, message.editions};
            ...
            exportable_serialized_callback<ConversationSignal::SwarmMessageReceived>([this](const auto& account_id, const auto& conversation_id, const auto& message) {
                DBusSwarmMessage msg {message.id, message.type, message.body, message.reactions, message.editions};
                DBusConfigurationManager::emitSwarmMessageReceived(account_id, conversation_id, msg);
            }),
            exportable_serialized_callback<ConversationSignal::SwarmMessageUpdated>([this](const auto& account_id, const auto& conversation_id, const auto& message) {
                DBusSwarmMessage msg {message.id, message.type, message.body, message.reactions, message.editions};
            ...
            exportable_serialized_callback<ConversationSignal::ReactionRemoved>(
                std::bind(&DBusConfigurationManager::emitReactionRemoved, this, _1, _2, _3)),
