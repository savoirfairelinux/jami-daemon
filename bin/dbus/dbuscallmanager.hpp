...
public:
    ...
    ~DBusCallManager() { unregisterAdaptor(); }

    auto placeCall(const std::string& accountId, const std::string& to) -> decltype(libjami::placeCall(accountId, to))
    {
        return libjami::placeCall(accountId, to);
    }
    auto placeCallWithMedia(const std::string& accountId,
                            const std::string& to,
    ...
    auto requestMediaChange(const std::string& accountId,
    ...
    auto acceptWithMedia(const std::string& accountId,
                         const std::string& callId,
                         const std::vector<std::map<std::string, std::string>>& mediaList)
    ...
    auto answerMediaChangeRequest(const std::string& accountId,
                                  const std::string& callId,
                                  const std::vector<std::map<std::string, std::string>>& mediaList)
    ...
    auto muteLocalMedia(const std::string& accountId,
                        ...
                        const bool& mute) -> decltype(libjami::muteLocalMedia(accountId, callId, mediaType, mute))
    {
    ...
    auto attendedTransfer(const std::string& accountId, const std::string& callId, const std::string& targetId)
    ...
    auto getCallList(const std::string& accountId) -> decltype(libjami::getCallList(accountId))
    {
    ...
    auto joinParticipant(const std::string& accountId,
    ...
    auto addParticipant(const std::string& accountId,
                        const std::string& callId,
                        const std::string& account2Id,
                        const std::string& confId)
    ...
    auto detachLocalParticipant() -> decltype(libjami::detachLocalParticipant())
    {
    ...
    auto joinConference(const std::string& accountId,
    ...
    auto disconnectConference(const std::string& accountId,
                          const std::string& confId) -> decltype(libjami::disconnectConference(accountId, confId))
    {
        return libjami::disconnectConference(accountId, confId);
    }
    ...
    void startTone(const int32_t& start, const int32_t& type) { libjami::startTone(start, type); }
