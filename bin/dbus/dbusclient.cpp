...
DBusClient::initLibrary(int flags)
{
    ...
    const std::map<std::string, SharedCallback> callEvHandlers
        = {exportable_callback<CallSignal::StateChange>(
           ...
           exportable_callback<CallSignal::MediaNegotiationComplete>(
               bind(&DBusCallManager::mediaNegotiationComplete, callM, _1)),
            exportable_callback<CallSignal::MediaStreaming>(
               bind(&DBusCallManager::mediaStreaming, callM, _1, _2))};
