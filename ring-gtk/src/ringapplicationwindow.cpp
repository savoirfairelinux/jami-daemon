...
static void
pickup_clicked(G_GNUC_UNUSED GtkToolButton *toolbutton,
                             gpointer       user_data)
{
  ...
  Call *call = NULL;
  QModelIndex idx = get_index_from_selection(gtk_tree_view_get_selection(GTK_TREE_VIEW(priv->treeview_calls)));
  if (idx.isValid())
...
static void
hangup_clicked(G_GNUC_UNUSED GtkToolButton *toolbutton,
                             gpointer       user_data)
{
  RingApplicationWindow* win = RING_APPLICATION_WINDOW(user_data);
  RingApplicationWindowPrivate *priv;
...
static void
dialing_changed(GtkEditable *editable,
                gpointer     user_data)
{
  RingApplicationWindow* win = RING_APPLICATION_WINDOW(user_data);
  RingApplicationWindowPrivate *priv;
  ...
  g_debug("call entry chagned");

  Call *call = NULL;
  Call::State state;

  /* try to get the selected call */
  QModelIndex call_idx = get_index_from_selection(gtk_tree_view_get_selection(GTK_TREE_VIEW(priv->treeview_calls)));
  if (call_idx.isValid())
    call = CallModel::instance()->getCall(call_idx);

  if (call) {
     state = call->state();
     if (state != Call::State::DIALING) {
      g_debug("selected call in not in DIALING state");
      call = NULL;
     }
  } else {
    g_debug("no call selected");
    call = NULL;
  }

  /* if no selected call, or if its not in a dialing state, create a new call */
  if (!call) {
    g_debug("creating new dialing call");
    /* try to get selected account */
    Account *account = NULL;
    QModelIndex acc_idx = get_index_from_selection(gtk_tree_view_get_selection(GTK_TREE_VIEW(priv->treeview_accounts)));
    if (acc_idx.isValid()) {
      qDebug() << "making call with account: " << AccountListModel::instance()->data(acc_idx, Account::Role::Alias).toString();
      account = AccountListModel::instance()->getAccountByModelIndex(acc_idx);
    }

    call = CallModel::instance()->dialingCall(QString(), account);
    /* make this the currently selected call */
    QModelIndex idx = CallModel::instance()->getIndex(call);
    GtkTreeIter iter;
    GtkTreeModel *model = gtk_tree_view_get_model(GTK_TREE_VIEW(priv->treeview_calls));
    if (gtk_q_tree_model_source_index_to_iter(GTK_Q_TREE_MODEL(model), idx, &iter)) {
      gtk_tree_selection_select_iter(gtk_tree_view_get_selection(GTK_TREE_VIEW(priv->treeview_calls)), &iter);
    }
  }

  /* update the dial number */
  call->setDialNumber(gtk_entry_get_text(GTK_ENTRY(editable)));
}
...
ring_application_window_init (RingApplicationWindow *win)
{
  ...
  g_signal_connect(priv->entry_call, "changed", G_CALLBACK(dialing_changed), win);
}
