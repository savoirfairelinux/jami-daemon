...
static void              gtk_q_list_model_tree_model_init (GtkTreeModelIface * );
static void              gtk_q_list_model_finalize        (GObject *           );
static GtkTreeModelFlags gtk_q_list_model_get_flags       (GtkTreeModel *      );
static gint              gtk_q_list_model_get_n_columns   (GtkTreeModel *      );
static GType             gtk_q_list_model_get_column_type (GtkTreeModel *,
                                                      gint                );
static gboolean          gtk_q_list_model_get_iter        (GtkTreeModel *,
                                                      GtkTreeIter *,
                                                      GtkTreePath *       );
static GtkTreePath *     gtk_q_list_model_get_path        (GtkTreeModel *,
                                                      GtkTreeIter *       );
static void              gtk_q_list_model_get_value       (GtkTreeModel *,
						                                          GtkTreeIter *,
						                                          gint,
						                                          GValue *            );
static gboolean          gtk_q_list_model_iter_next       (GtkTreeModel *,
						                                          GtkTreeIter *       );
static gboolean          gtk_q_list_model_iter_previous   (GtkTreeModel *,
						                                          GtkTreeIter *       );
static gboolean          gtk_q_list_model_iter_children   (GtkTreeModel *,
						                                          GtkTreeIter *,
						                                          GtkTreeIter *       );
static gboolean          gtk_q_list_model_iter_has_child  (GtkTreeModel *,
						                                          GtkTreeIter *       );
static gint              gtk_q_list_model_iter_n_children (GtkTreeModel *,
						                                          GtkTreeIter *       );
static gboolean          gtk_q_list_model_iter_nth_child  (GtkTreeModel *,
						                                          GtkTreeIter *,
						                                          GtkTreeIter *,
						                                          gint                );
static gboolean          gtk_q_list_model_iter_parent     (GtkTreeModel *,
						                                          GtkTreeIter *,
						                                          GtkTreeIter *       );

/* implementation prototypes */
static void gtk_q_list_model_increment_stamp (GtkQListModel * );

static gint gtk_q_list_model_length          (GtkQListModel * );

static void gtk_q_list_model_set_n_columns   (GtkQListModel *,
                                            gint     );
static void gtk_q_list_model_set_column_type (GtkQListModel *,
                                            gint    ,
                                            gint    ,
                                            GType    );
...
G_DEFINE_TYPE_WITH_CODE (GtkQListModel, gtk_q_list_model, G_TYPE_OBJECT,
                         G_ADD_PRIVATE (GtkQListModel)
       G_IMPLEMENT_INTERFACE (GTK_TYPE_TREE_MODEL,
            gtk_q_list_model_tree_model_init))

static void
gtk_q_list_model_class_init (GtkQListModelClass *klass)
{
  GObjectClass *object_class;

  object_class = (GObjectClass*) klass;

  object_class->finalize = gtk_q_list_model_finalize;
}

static void
gtk_q_list_model_tree_model_init (GtkTreeModelIface *iface)
{
  iface->get_flags = gtk_q_list_model_get_flags;
  iface->get_n_columns = gtk_q_list_model_get_n_columns;
  iface->get_column_type = gtk_q_list_model_get_column_type;
  iface->get_iter = gtk_q_list_model_get_iter;
  iface->get_path = gtk_q_list_model_get_path;
  iface->get_value = gtk_q_list_model_get_value;
  iface->iter_next = gtk_q_list_model_iter_next;
  iface->iter_previous = gtk_q_list_model_iter_previous;
  iface->iter_children = gtk_q_list_model_iter_children;
  iface->iter_has_child = gtk_q_list_model_iter_has_child;
  iface->iter_n_children = gtk_q_list_model_iter_n_children;
  iface->iter_nth_child = gtk_q_list_model_iter_nth_child;
  iface->iter_parent = gtk_q_list_model_iter_parent;
}

static void
gtk_q_list_model_init (GtkQListModel *q_list_model)
{
  GtkQListModelPrivate *priv;

  q_list_model->priv = (GtkQListModelPrivate *)gtk_q_list_model_get_instance_private (q_list_model);
  priv = q_list_model->priv;
 ...
 * are supported.
 *
 * As an example, <literal>gtk_q_list_model_new (3, G_TYPE_INT, G_TYPE_STRING,
 * GDK_TYPE_PIXBUF);</literal> will create a new #GtkQListModel with three columns, of type
 * int, string and #GdkPixbuf respectively.
 *
 * Return value: a new #GtkQListModel
 */
GtkQListModel *
gtk_q_list_model_new (QAbstractItemModel *model, gint n_columns, ...)
{
  GtkQListModel *retval;
  va_list args;
  gint i;

  g_return_val_if_fail (n_columns > 0, NULL);

  retval = (GtkQListModel *)g_object_new (GTK_TYPE_Q_LIST_MODEL, NULL);
  gtk_q_list_model_set_n_columns (retval, n_columns);
  ...
  for (i = 0; i < n_columns; i++)
    {
      ...
      gtk_q_list_model_set_column_type (retval, i, role, type);
    }

  va_end (args);

  gtk_q_list_model_length(retval);
...
static void
gtk_list_store_increment_stamp (GtkQListModel *q_list_model)
{
  GtkQListModelPrivate *priv = q_list_model->priv;
...
static gint
gtk_q_list_model_length (GtkQListModel *q_list_model)
{
  GtkQListModelPrivate *priv = q_list_model->priv;
...
static void
gtk_q_list_model_set_n_columns (GtkQListModel *q_list_model,
            gint          n_columns)
{
  GtkQListModelPrivate *priv = q_list_model->priv;
  int i;
...
static void
gtk_q_list_model_set_column_type (GtkQListModel *q_list_model,
        gint          column,
        gint          role,
        GType         type)
{
...
static void
gtk_q_list_model_finalize (GObject *object)
{
  // is there anything to do?


  // GtkQListModel *q_list_model = GTK_Q_LIST_MODEL (object);
  // GtkQListModelPrivate *priv = q_list_model->priv;
  ...
  G_OBJECT_CLASS (gtk_q_list_model_parent_class)->finalize (object);
}

static gboolean
iter_is_valid (GtkTreeIter *iter,
               GtkQListModel   *q_list_model)
{
  gboolean retval;
  ...
  // return q_list_model->priv->stamp == iter->stamp &&
  //        row.value >= 0 &&
  //        row.value < gtk_q_list_model_length(q_list_model);
  retval = q_list_model->priv->model->index(row.value, 0).isValid();
...
gtk_q_list_model_get_flags (GtkTreeModel *tree_model)
{
...
static gint
gtk_q_list_model_get_n_columns (GtkTreeModel *tree_model)
{

  GtkQListModel *q_list_model = GTK_Q_LIST_MODEL (tree_model);
  GtkQListModelPrivate *priv = q_list_model->priv;
...
static GType
gtk_q_list_model_get_column_type (GtkTreeModel *tree_model,
				gint          index)
{
  ...
  g_return_val_if_fail (index < gtk_q_list_model_get_n_columns(tree_model), G_TYPE_INVALID);
 ...
 */
static gboolean
gtk_q_list_model_get_iter (GtkTreeModel *tree_model,
			 GtkTreeIter  *iter,
			 GtkTreePath  *path)
{
  ...
  if (!iter_is_valid(iter, q_list_model)){
    iter->stamp = 0;
    return FALSE;
  }
 ...
 */
static GtkTreePath *
gtk_q_list_model_get_path (GtkTreeModel *tree_model,
			 GtkTreeIter  *iter)
{
  ...
  g_return_val_if_fail (iter_is_valid(iter, q_list_model), NULL);
...
static void
gtk_q_list_model_get_value (GtkTreeModel *tree_model,
			  GtkTreeIter  *iter,
			  gint          column,
			  GValue       *value)
{
  ...
  g_return_if_fail (iter_is_valid(iter, q_list_model));
 ...
 */
static gboolean
gtk_q_list_model_iter_next (GtkTreeModel  *tree_model,
			  GtkTreeIter   *iter)
{
  ...
  row.value++;
  iter->user_data = row.ptr;
  retval = iter_is_valid(iter, q_list_model);
 ...
 */
static gboolean
gtk_q_list_model_iter_previous (GtkTreeModel *tree_model,
                              GtkTreeIter  *iter)
{
 ...
 */
static gboolean
gtk_q_list_model_iter_children (GtkTreeModel *tree_model,
			      GtkTreeIter  *iter,
			      GtkTreeIter  *parent)
{
  // g_debug("getting model children");

  GtkQListModel *q_list_model = (GtkQListModel *) tree_model;
  GtkQListModelPrivate *priv = q_list_model->priv;
...
static gboolean
gtk_q_list_model_iter_has_child (GtkTreeModel *tree_model,
			       GtkTreeIter  *iter)
{
 ...
 */
static gint
gtk_q_list_model_iter_n_children (GtkTreeModel *tree_model,
				GtkTreeIter  *iter)
{
  ...
  if (iter == NULL)
    return gtk_q_list_model_length(q_list_model);
  // if (iter == NULL)
 ...
 */
static gboolean
gtk_q_list_model_iter_nth_child (GtkTreeModel *tree_model,
			       GtkTreeIter  *iter,
			       GtkTreeIter  *parent,
			       gint          n)
{
 ...
 */
static gboolean
gtk_q_list_model_iter_parent (GtkTreeModel *tree_model,
			    GtkTreeIter  *iter,
			    GtkTreeIter  *child)
{
