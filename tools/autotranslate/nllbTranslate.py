...
class Translator:
    def __init__(self):
        ...
        self.sflResources = self.sflTransifex.get_resources()

    def translate(self):
        for language in self.languages:
            nnlb_language = languagesList.get(language)
            if nnlb_language is not None:
                printc(f"Using language: {language}")
                self.translate_resources(language, nnlb_language)

    def translate_resources(self, language, nnlb_language):
        for r in self.sflResources:
            ...
            try:
                ...
                for t in toTranslate:
                    ...
                    translated_string_other = self.translator.translate_nllb(source_string_other, nnlb_language)

                    if t.resource_string.pluralized:
                        source_string_one = t.resource_string.strings["one"]
                        translated_string_one = self.translator.translate_nllb(source_string_one, nnlb_language)
                ...
                if len(translateStrings) > 0:
                    self.sflTransifex.create_resource_file(rslug, translateStrings, language)
